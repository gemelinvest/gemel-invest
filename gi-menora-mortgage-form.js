/* GEMEL INVEST — טופס מקורי ריסק משכנתא מנורה Class A (08.2025, 124-5)
   נטען לפי דרישה ממסמכי לקוח. ממלא רק ערכים שכבר שמורים בתיק. */
(function installMenoraMortgageForm(global){
  "use strict";
  ...
  const MenoraMortgageForm = {
    ...
    LOAN_YEAR_FIELDS: ["Text39", "Text46", "Text52", "Text58"],
    ...
    isMenoraMortgagePolicy(policy){
      if(!policy || typeof policy !== "object") return false;
      if(safeTrim(policy.company) !== "מנורה") return false;
      return /משכנתא/.test(this.policyBlob(policy));
    },
    listMenoraMortgagePolicies(payload){
      const list = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      return list.filter((p) => this.isMenoraMortgagePolicy(p));
    },
    qualifies(payload){
      return this.listMenoraMortgagePolicies(payload).length > 0;
    },
    ...
    applyPerson(form, person, isSpouse, font){
      ...
      if(person.phone){
        if(isSpouse) this.setTextSafe(form, "PhoneNumberSpouse", person.phone, font);
        else this.setTextSafe(form, "CellPhoneNumber", person.phone, font);
      }
    ...
    applyLoaner(form, loaner, font){
      if(!loaner) return;
      this.setTextSafe(form, "Text80", loaner.name, font);
      this.setTextSafe(form, "Text81", loaner.branch, font);
      this.setTextSafe(form, "Text82", loaner.address, font);
    },
    applyLoans(form, loans, font){
      (loans || []).slice(0, 4).forEach((loan, idx) => {
        this.setTextSafe(form, this.LOAN_SUM_FIELDS[idx], loan.amount, font);
        this.setTextSafe(form, this.LOAN_YEAR_FIELDS[idx], loan.years, font);
      });
    },
    ...
    async fillOriginalTemplate(draft){
      ...
      const templateBytes = await this.fetchFirstOk(
        this.candidateUrls("forms/menora-mortgage/", this.TEMPLATE_FILE),
        "לא נמצא טופס משכנתא של מנורה"
      );
      ...
      this.setTextSafe(form, "Text3", draft.discountPack, font);
    ...
    fileName(draft){
      ...
      return "ריסק_משכנתא_מנורה_" + name.replace(/[\\/:*?\"<>|]/g, "_") + "_" + nowISO().slice(0, 10) + ".pdf";
    },

    fieldRow(label, name, value, extra){
      return `<label class="menoraMortForm__field">
        <span>${escapeHtml(label)}</span>
        <input class="input" type="text" data-menormort="${escapeHtml(name)}" value="${escapeHtml(value || "")}" ${extra || ""} />
      </label>`;
    },
    personBlock(title, prefix, person){
      const p = person || {};
      return `<section class="menoraMortForm__block">
        <div class="menoraMortForm__blockTitle">${escapeHtml(title)}</div>
        <div class="menoraMortForm__grid">
    ...
    collectDraftFromModal(root, draft){
      ...
      root.querySelectorAll("[data-menormort]").forEach((el) => {
        const path = safeTrim(el.getAttribute("data-menormort"));
        if(!path) return;
    ...
    ensureStyles(){
      if(document.getElementById("menoraMortFormStyle")) return;
      const style = document.createElement("style");
      style.id = "menoraMortFormStyle";
      style.textContent = `
        .menoraMortFormModal .giValModal__card{ max-width:min(980px,96vw); width:100%; height:min(92vh,920px); max-height:min(92vh,920px); }
        .menoraMortForm__hint{ font-size:13px; line-height:1.45; color:#475569; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:12px; padding:10px 12px; }
        .menoraMortForm__block{ border:1px solid #E5EAF3; border-radius:14px; padding:12px 14px 14px; background:#fff; }
        .menoraMortForm__blockTitle{ font-size:14px; font-weight:800; color:#0B1F4B; margin-bottom:10px; }
        .menoraMortForm__grid{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px 12px; }
        .menoraMortForm__field{ display:flex; flex-direction:column; gap:4px; min-width:0; }
        .menoraMortForm__field span{ font-size:12px; font-weight:700; color:#647287; }
        .menoraMortFormPreview{ padding:12px 4px; display:flex; flex-direction:column; gap:10px; }
        .menoraMortFormPreview__row{ display:flex; justify-content:space-between; gap:12px; font-size:13.5px; }
        .menoraMortFormPreview__row span{ color:#647287; }
        .menoraMortFormPreview__row strong{ color:#0F172A; font-weight:750; }
        @media (max-width:720px){ .menoraMortForm__grid{ grid-template-columns:1fr; } }
      `;
    ...
    renderPreviewHtml(draft){
      ...
      const rows = people.map((p, idx) => {
        ...
        return `<div class="menoraMortFormPreview__row"><span>${escapeHtml(role)}</span><strong>${escapeHtml(p.fullName || "—")}</strong></div>`;
      }).join("");
      return `<div class="menoraMortFormPreview">
        <div class="menoraMortFormPreview__row"><span>חברה / מוצר</span><strong>מנורה · ריסק משכנתא</strong></div>
        <div class="menoraMortFormPreview__row"><span>תחילת ביטוח</span><strong>${escapeHtml(draft.insuranceBegin || "—")}</strong></div>
        <div class="menoraMortFormPreview__row"><span>בנק משעבד</span><strong>${escapeHtml(draft.loaner?.name || "—")}</strong></div>
        ${rows}
        ${global.CustomerDocuments?.canDownloadOfficialJoinForm?.() ? `<button class="btn btn--primary" type="button" data-menormort-open="1">פתח טופס דיגיטלי</button>` : ""}
      </div>`;
    },
    close(){
    ...
    bind(modal){
      modal.querySelectorAll("[data-menormort-close]").forEach((el) => {
        el.addEventListener("click", () => this.close());
      });
      const dl = modal.querySelector("[data-menormort-download]");
      if(dl){
        dl.addEventListener("click", async () => {
          ...
          try {
            ...
            try { console.error("MENORA_MORTGAGE_PDF_FAILED", err); } catch(_e) {}
    ...
    open(rec){
      ...
      modal.className = "giValModal menoraMortFormModal is-open giValModal--visible";
      modal.innerHTML = `
        <div class="giValModal__backdrop" data-menormort-close="1"></div>
        <div class="giValModal__card">
          <div class="giValModal__head">
            <div class="giValModal__headText">
              <div class="giValModal__title">טופס מקורי — ריסק משכנתא · מנורה</div>
              <div class="giValModal__sub">פרטים מהתיק כבר ממולאים. השלימו מה שחסר והורידו PDF רשמי.</div>
            </div>
            <button type="button" class="giValModal__closeX" data-menormort-close="1" aria-label="סגירה">✕</button>
          </div>
          <div class="giValModal__body">
            <div class="menoraMortForm__hint">ממולא אוטומטית רק מה ששמור בתיק, כולל כן/לא בהצהרת בריאות ואמצעי תשלום. פירוט רפואי, ביטול/החלפת ביטוח, סוג ריבית, Extra LIFE וחתימות לא ממולאים — אותם משלימים בטופס או ב-PDF אחרי ההורדה.</div>
            <section class="menoraMortForm__block">
              <div class="menoraMortForm__blockTitle">פרטי הצעה וסוכן</div>
              <div class="menoraMortForm__grid">
            ...
            <section class="menoraMortForm__block">
              <div class="menoraMortForm__blockTitle">בנק משעבד והלוואה — רק אם נרשם בתיק</div>
              <div class="menoraMortForm__grid">
            ...
            <section class="menoraMortForm__block">
              <div class="menoraMortForm__blockTitle">משלם והוראת קבע — רק אם נרשם בתיק</div>
              <div class="menoraMortForm__grid">
          ...
          <div class="giValModal__foot">
            <button type="button" class="btn" data-menormort-close="1">סגור</button>
            <button type="button" class="btn btn--primary" data-menormort-download="1">הורד PDF רשמי</button>
          </div>
        </div>`;
  ...
  global.MenoraMortgageForm = MenoraMortgageForm;
