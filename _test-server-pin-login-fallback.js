...
assert(app.includes("GI_VERIFY_AGENT_LOGIN_FALLBACK"), "must log fallback");
assert(app.includes('source:"local"'), "must support local fallback source");
...
assert(/Does NOT revoke agents\.pin/i.test(sql), "SQL must document no pin revoke in phase 1");
