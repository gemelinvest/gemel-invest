...
*/
const CACHE_NAME = "gemel-invest-cache-v5";
...
function normalizedRequest(req) {
  ...
  // IMPORTANT: keep query params for app.css/app.js because index.html uses ?v=BUILD for cache-busting
  const path = url.pathname || '';
  if (path.endsWith('/app.css') || path.endsWith('/app.js') || path.endsWith('/index.html')) {
    return req;
  }

  // For other assets, ignore query params (e.g., icons?x=123)
