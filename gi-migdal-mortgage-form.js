/* GEMEL INVEST — טופס מקורי ריסק משכנתא מגדל
   נטען לפי דרישה ממסמכי לקוח. ממלא רק ערכים שכבר שמורים בתיק. */
(function installMigdalMortgageForm(global){
  "use strict";
  ...
  const MigdalMortgageForm = {
    ...
    isMigdalMortgagePolicy(policy){
      if(!policy || typeof policy !== "object") return false;
      if(safeTrim(policy.company) !== "מגדל") return false;
      return /משכנתא/.test(this.policyBlob(policy));
    },
    listMigdalMortgagePolicies(payload){
      const list = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      return list.filter((p) => this.isMigdalMortgagePolicy(p));
    },
    qualifies(payload){
      return this.listMigdalMortgagePolicies(payload).length > 0;
    },
    ...
    loansFromPolicy(policy){
      const banks = this.listPledgeBanks(policy).slice(0, 5);
    ...
    applyPerson(form, person, isSpouse, font){
      ...
      this.setTextSafe(form, isSpouse ? "CitySpouse" : "City", person.city, font);
      this.setTextSafe(form, isSpouse ? "StreetNameSpouse" : "StreetName", person.street, font);
      ...
      this.setTextSafe(form, isSpouse ? "ProfessionSpouse" : "Profession", person.occupation, font);
    ...
    applyLoaner(form, loaner, font){
      if(!loaner) return;
      this.setTextSafe(form, "LoanerName", loaner.name, font);
      this.setTextSafe(form, "IrrevocableBeneficiaryName", loaner.name, font);
      this.setTextSafe(form, "IrrevocableBeneficiaryNameSpouse", loaner.name, font);
      this.setTextSafe(form, "LoanerBankNameCode", loaner.bankNo, font);
      this.setTextSafe(form, "LoanerBankBranchCode", loaner.branch, font);
      this.setTextSafe(form, "LoanerStreetName", loaner.address, font);
    },
    applyLoans(form, loans, font){
      (loans || []).slice(0, 5).forEach((loan, idx) => {
    ...
    async fillOriginalTemplate(draft){
      ...
      const templateBytes = await this.fetchFirstOk(
        this.candidateUrls("forms/migdal-mortgage/", this.TEMPLATE_FILE),
        "לא נמצא טופס משכנתא של מגדל"
      );
      ...
      if(draft.bank){
        ...
        this.setTextSafe(form, "BankBranchCode", draft.bank.branch, font);
    ...
    fileName(draft){
      ...
      return "ריסק_משכנתא_מגדל_" + name.replace(/[\\/:*?\"<>|]/g, "_") + "_" + nowISO().slice(0, 10) + ".pdf";
    },

    fieldRow(label, name, value, extra){
      return `<label class="migMortForm__field">
        <span>${escapeHtml(label)}</span>
        <input class="input" type="text" data-migmort="${escapeHtml(name)}" value="${escapeHtml(value || "")}" ${extra || ""} />
      </label>`;
    },
    personBlock(title, prefix, person){
      const p = person || {};
      return `<section class="migMortForm__block">
        <div class="migMortForm__blockTitle">${escapeHtml(title)}</div>
    ...
    collectDraftFromModal(root, draft){
      ...
      root.querySelectorAll("[data-migmort]").forEach((el) => {
        const path = safeTrim(el.getAttribute("data-migmort"));
        if(!path) return;
    ...
    ensureStyles(){
      if(document.getElementById("migMortFormStyle")) return;
      const style = document.createElement("style");
      style.id = "migMortFormStyle";
      style.textContent = `
        .migMortFormModal .giValModal__card{ max-width:min(980px,96vw); width:100%; height:min(92vh,920px); max-height:min(92vh,920px); }
        .migMortForm__hint{ font-size:13px; line-height:1.45; color:#475569; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:12px; padding:10px 12px; }
        .migMortForm__block{ border:1px solid #E5EAF3; border-radius:14px; padding:12px 14px 14px; background:#fff; }
        .migMortForm__blockTitle{ font-size:14px; font-weight:800; color:#0B1F4B; margin-bottom:10px; }
        .migMortForm__grid{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px 12px; }
        .migMortForm__field{ display:flex; flex-direction:column; gap:4px; min-width:0; }
        .migMortForm__field span{ font-size:12px; font-weight:700; color:#647287; }
        .migMortFormPreview{ padding:12px 4px; display:flex; flex-direction:column; gap:10px; }
        .migMortFormPreview__row{ display:flex; justify-content:space-between; gap:12px; font-size:13.5px; }
        .migMortFormPreview__row span{ color:#647287; }
        .migMortFormPreview__row strong{ color:#0F172A; font-weight:750; }
        @media (max-width:720px){ .migMortForm__grid{ grid-template-columns:1fr; } }
      `;
    ...
    renderPreviewHtml(draft){
      ...
      const rows = people.map((p, idx) => {
        ...
        return `<div class="migMortFormPreview__row"><span>${escapeHtml(role)}</span><strong>${escapeHtml(p.fullName || "—")}</strong></div>`;
      }).join("");
      return `<div class="migMortFormPreview">
        <div class="migMortFormPreview__row"><span>חברה / מוצר</span><strong>מגדל · ריסק משכנתא</strong></div>
        <div class="migMortFormPreview__row"><span>תחילת ביטוח</span><strong>${escapeHtml(draft.insuranceBegin || "—")}</strong></div>
        <div class="migMortFormPreview__row"><span>בנק משעבד</span><strong>${escapeHtml(draft.loaner?.name || "—")}</strong></div>
        ${rows}
        <button class="btn btn--primary" type="button" data-migmort-open="1">פתח טופס דיגיטלי</button>
      </div>`;
    },
    close(){
    ...
    bind(modal){
      modal.querySelectorAll("[data-migmort-close]").forEach((el) => {
        el.addEventListener("click", () => this.close());
      });
      const dl = modal.querySelector("[data-migmort-download]");
      if(dl){
        dl.addEventListener("click", async () => {
          ...
          try {
            ...
            try { console.error("MIGDAL_MORTGAGE_PDF_FAILED", err); } catch(_e) {}
    ...
    open(rec){
      ...
      modal.className = "giValModal migMortFormModal is-open giValModal--visible";
      modal.innerHTML = `
        <div class="giValModal__backdrop" data-migmort-close="1"></div>
        <div class="giValModal__card">
          <div class="giValModal__head">
            <div class="giValModal__headText">
              <div class="giValModal__title">טופס מקורי — ריסק משכנתא · מגדל</div>
          ...
          <div class="giValModal__body">
            ...
            <section class="migMortForm__block">
              <div class="migMortForm__blockTitle">בנק משעבד והלוואה — רק אם נרשם בתיק</div>
              <div class="migMortForm__grid">
            ...
            <section class="migMortForm__block">
              <div class="migMortForm__blockTitle">הוראת קבע — רק אם נרשמה בתיק</div>
              <div class="migMortForm__grid">
          ...
          <div class="giValModal__foot">
            <button type="button" class="btn" data-migmort-close="1">סגור</button>
            <button type="button" class="btn btn--primary" data-migmort-download="1">הורד PDF רשמי</button>
          </div>
        </div>`;
  ...
  global.MigdalMortgageForm = MigdalMortgageForm;
