...
// open health covers drawer (Health only)
    ...
    bindLocalityAutocomplete(ins){
      ...
      const refreshNativeList = () => {
        if(!nativeList) return;
        const items = this.getLocalitySuggestions(input.value || '').slice(0, 50);
        nativeList.innerHTML = items.map(item => `<option value="${escapeHtml(item)}"></option>`).join('');
      };

      this.ensureLocalitiesLoaded().then(() => {
        if(input.isConnected && menu.isConnected){
      ...
      const openMenu = () => {
        refreshNativeList();
        if(!this.getLocalitySuggestions(input.value || '').length){
      ...
      const syncLocality = () => {
        ins.data.city = safeTrim(input.value);
      ...
      on(input, 'input', () => { syncLocality(); refreshMenu(); openMenu(); });
      on(input, 'change', () => { syncLocality(); refreshMenu(); });
      on(input, 'keydown', (ev) => {
