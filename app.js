...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/XXXX/exec';
    const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyH_IdKzjFa8-y0K89BPLO2gf1Yipm86mI-BkP1Vn5Tfzeh0yPHr8lr0MdMCE3Gp-k/exec'; // default (can be overridden from Settings)
  const SERVER_URL_LS_KEY = 'gemel_server_url_v1';
  function getServerUrl_(){
    try {
      return String((state && state.server && state.server.url) || localStorage.getItem(SERVER_URL_LS_KEY) || GOOGLE_SCRIPT_URL || '').trim();
    } catch(_) {
      return String((state && state.server && state.server.url) || GOOGLE_SCRIPT_URL || '').trim();
    }
  }
  function setServerUrl_(url){
    const u = String(url||'').trim();
    if(state && state.server) state.server.url = u;
    try{ localStorage.setItem(SERVER_URL_LS_KEY, u); }catch(_){}
  }
  ...
  // ---------- Server helpers ----------
    ...
    const base = getServerUrl_();
    if(!base) return { ok:false, demo:true, error:'NO_URL' };
    const url = base + '?action=' + encodeURIComponent(action);
    const res = await fetch(url, {
      method: 'POST',
      // IMPORTANT: text/plain avoids CORS preflight on Apps Script
    ...
    if(!data){
      return { ok:false, error:'BAD_RESPONSE', status:res.status, raw:(text||'').slice(0,200) };
    }
    return data;
  }
    ...
    const base = getServerUrl_();
    if(!base) return { ok:false, demo:true };
    try{
  ...
  try{
    const savedUrl = localStorage.getItem(SERVER_URL_LS_KEY);
    if(savedUrl) state.server.url = String(savedUrl).trim();
  }catch(_){}
...
function renderKpisSettings(){
  ...
  function renderSettings(){
      ...
      <div class="grid2">
        <div class="sideCard">
          ...
          <div class="field">
            ...
            <input id="serverUrlInput" placeholder="https://script.google.com/macros/s/AKfycbyH_IdKzjFa8-y0K89BPLO2gf1Yipm86mI-BkP1Vn5Tfzeh0yPHr8lr0MdMCE3Gp-k/exec" value="${escapeAttr(state.server.url || '')}"/>
          </div>
          ...
          <div class="sideHint">הערה: כתובת השרת נשמרת כאן ונמצאת בשימוש מיידי. אפשר להדביק URL חדש ולבדוק חיבור בכל רגע.</div>
        </div>
    ...
    $('#pullBtn')?.addEventListener('click', async () => {
      if(!getServerUrl_()){ toast('דמו', 'לא הוגדרה כתובת שרת (Web App URL)'); return; }
      await syncDown();
...
function debounce(fn, wait){
  ...
  (async () => {
    if(!state.server.url) state.server.url = GOOGLE_SCRIPT_URL || '';
    if(AUTO_CONNECT && getServerUrl_()){
      const ping = await apiPing();
      state.connected = !!ping.ok;
      if(state.connected){
