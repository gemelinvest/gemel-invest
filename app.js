...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    openById(id, opts={}){
      ...
      try{
        ...
        if(this.els.dash){
          ...
          const opsReportCard = this.els.dash.querySelector('[data-open-ops-report]');
          if(opsReportCard){
            opsReportCard.onclick = (ev) => {
              ev?.preventDefault?.();
              ev?.stopPropagation?.();
              this.openOpsReportFromCurrent();
            };
    ...
    openOpsReportFromCurrent(){
      ...
      try{
        if(Wizard && typeof Wizard.openOperationalReport === 'function'){
          Wizard.openOperationalReport(payload);
          const reportEl = document.getElementById('lcReport');
          if(reportEl && !reportEl.classList.contains('is-open')){
