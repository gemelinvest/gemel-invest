...
(() => {
  ...
  const BUILD = "20260320-opsreportlogo2";
      ...
      root.innerHTML = `
        ...
        <div class="lcWelcomeLoader__panel" role="status" aria-live="polite" aria-atomic="true">
          <div class="lcWelcomeLoader__shell">
            ...
            <div class="lcWelcomeLoader__brandRow">
              <span class="lcWelcomeLoader__brandLine" aria-hidden="true"></span>
              <div class="lcWelcomeLoader__brand">GEMEL INVEST</div>
              <span class="lcWelcomeLoader__brandLine" aria-hidden="true"></span>
            </div>
...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    collectPolicies(rec){
      ...
      sourceInsureds.forEach((ins, idx) => {
        ...
        (ins?.data?.existingPolicies || []).forEach((p, pIdx) => {
          ...
          const discountYears = safeTrim(p?.discountYears);
          ...
          policies.push({
            ...
            details: {
              ...
              "הנחה": (discountPct > 0 || discountYears) ? `${discountPct}%${discountYears ? ` · ${discountYears} שנים` : ""}` : "ללא הנחה",
      ...
      return `<article class="customerPolicyRow ${this.companyClass(policy.company)} ${policy.origin === 'new' ? 'is-newRow' : 'is-existingRow'}" data-policy-id="${escapeHtml(policy.id)}">
        ...
        <div class="customerPolicyRow__main">
          ...
          <div class="customerPolicyRow__numbers">
            ...
            <div class="customerPolicyRow__afterWrap ${discountPct > 0 || safeTrim(policy.discountYears) ? 'has-discount' : ''}">
              <div class="customerPolicyRow__afterLabel">אחרי הנחה</div>
              <div class="customerPolicyRow__after">${escapeHtml(afterPremium)}</div>
              <div class="customerPolicyRow__discountMeta">${discountPct > 0 || safeTrim(policy.discountYears) ? `${escapeHtml(String(policy.discountPct || '0'))}%${safeTrim(policy.discountYears) ? ` · ${escapeHtml(policy.discountYears)} שנים` : ''}` : 'ללא הנחה'}</div>
            </div>
    ...
    renderPolicyWallet(rec, policies){
      ...
      return `<section class="customerWalletSection customerWalletSection--rows customerWalletSection--newOnly">
        ...
        ${newPolicies.length
          ...
          : `<div class="emptyState"><div class="emptyState__icon">🧾</div><div class="emptyState__title">עדיין אין פוליסות חדשות בתיק</div><div class="emptyState__text">ברגע שתישמר פוליסה חדשה, היא תוצג כאן אוטומטית.</div></div>`}
      </section>`;
    },
      ...
      try{
        ...
        if(this.els.dash){
          this.els.dash.innerHTML = stats.map(card => {
            ...
            if(card.type === "ops-report-logo") return this.renderOperationalReportLogoCard();
          ...
          const reportCardBtn = this.els.dash.querySelector('[data-open-report="1"]');
          if(reportCardBtn){
            reportCardBtn.addEventListener('click', () => {
              const currentRec = this.current();
              if(!currentRec) return;
              const payload = JSON.parse(JSON.stringify(currentRec.payload || {}));
              Wizard.openOperationalReport(payload);
            });
          }
    ...
    renderOperationalReflectionCard(state){
      ...
      return `
        <div class="customerStatCard customerStatCard--ops customerStatCard--ops-${escapeHtml(state?.tone || 'info')}" id="customerOpsReflectionCard" data-customer-id="${escapeHtml(current?.id || '')}">
          <div class="customerStatCard__icon">📡</div>
    ...
    renderOperationalReportLogoCard(){
      return `
        <button class="customerStatCard customerStatCard--reportLogo" type="button" data-open-report="1" aria-label="פתח דו״ח תפעול">
          <div class="customerStatCard__reportLogoWrap">
            <img class="customerStatCard__reportLogoImg" src="./operational-report-logo.png" alt="דו״ח תפעול" />
          </div>
        </button>`;
    },
...
init(){
      ...
      this.els.reportDownload = $("#lcReportDownload");
      this.els.reportPrint = $("#lcReportPrint");
      this.els.reportFullscreen = $("#lcReportFullscreen");
      ...
      on(this.els.reportDownload, "click", () => this.downloadOperationalReportFile());
      on(this.els.reportPrint, "click", () => this.exportOperationalPdf());
      on(this.els.reportFullscreen, "click", () => this.toggleOperationalReportFullscreen());
      ...
      this.els.policyDiscountYears = $("#lcPolicyDiscountYears");
      ...
      on(this.els.policyDiscountYears, "input", () => this.updatePolicyDiscountPreview());
...
// open health covers drawer (Health only)
    ...
    openPolicyDiscountModal(pid){
      ...
      if(this.els.policyDiscountYears) this.els.policyDiscountYears.value = String(policy.discountYears || "");
    ...
    updatePolicyDiscountPreview(){
      ...
      const years = safeTrim(this.els.policyDiscountYears?.value || "");
      const base = this.asMoneyNumber(policy.premiumMonthly);
      const after = Math.max(0, Math.round((base * (1 - pct / 100)) * 100) / 100);
      const yearsText = years ? ` · דירוג: ${escapeHtml(years)} שנים` : "";
      this.els.policyDiscountPreview.innerHTML = `פרמיה אחרי הנחה: <b>${escapeHtml(this.formatMoneyValue(after))}</b>${yearsText}`;
    },
    ...
    savePolicyDiscountModal(){
      ...
      const yearsRaw = safeTrim(this.els.policyDiscountYears?.value || "");
      ...
      if(!yearsRaw){
        if(this.els.policyDiscountError) this.els.policyDiscountError.textContent = "יש להזין דירוג הנחה בשנים.";
        return;
      }
      policy.discountPct = String(pct);
      policy.discountYears = yearsRaw;
      this.closePolicyDiscountModal();
      this.render();
      this.setHint("ההנחה נשמרה בהצלחה בפוליסה.");
    },
    ...
    renderStep5(){
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        const chips = [
          ...
          (discountPct > 0 || discountYears) ? `<span class="lcPolInfoChip lcPolInfoChip--discount"><span class="lcPolInfoChip__icon">🏷️</span><span class="lcPolInfoChip__text"><b>${escapeHtml(String(discountPct))}%${discountYears ? ` · ${escapeHtml(discountYears)} שנים` : ''}</b><small>הנחה שנשמרה</small></span></span>` : '',
...
// ---------- Step 6 ----------
      ...
      const renderNewSummaryTable = (list=[]) => {
        ...
        const rows = list.map(p => {
          ...
          return `<tr>
            ...
            <td>${escapeHtml(String(p.discountPct || "0"))}%${safeTrim(p.discountYears) ? ` · ${escapeHtml(p.discountYears)} שנים` : ""}</td>
    ...
    renderStep9(){
      ...
      const renderNewCards = () => {
        ...
        return `<div class="lcOpCards">` + (this.newPolicies || []).map((policy, idx) => {
          ...
          const discountTxt = `${escapeHtml(String(policy.discountPct || "0"))}%${safeTrim(policy.discountYears) ? ` · ${escapeHtml(policy.discountYears)} שנים` : ""}`;
    ...
    openHealthFindingsModal(insId){
      ...
      }).join('') : `<div class="emptyState"><div class="emptyState__icon">🩺</div><div class="emptyState__title">אין ממצאים להצגה</div><div class="emptyState__text">לא סומנו תשובות כן עבור המבוטח הזה.</div></div>`;
    ...
    renderReportTableNote(text){
      const note = safeTrim(text);
      return note ? `<div class="lcReportTableNote">${escapeHtml(note)}</div>` : '';
    },

    renderTable(headers, rows, opts){
      if(!rows.length) return `<div class="muted">אין נתונים להצגה.</div>`;
      const note = this.renderReportTableNote(opts?.note || '');
      return `${note}<div class="lcReportTableWrap"><table class="lcReportTable"><thead><tr>${headers.map(h => `<th>${escapeHtml(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(r => `<tr>${r.map(c => `<td>${c}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
    },

    getOperationalFieldLabel(key){
      const map = {
        firstName:'שם פרטי', lastName:'שם משפחה', idNumber:'תעודת זהות', birthDate:'תאריך לידה', gender:'מגדר', maritalStatus:'מצב משפחתי',
        phone:'טלפון', email:'אימייל', city:'יישוב', street:'רחוב', houseNumber:'מספר בית', zip:'מיקוד', clinic:'קופת חולים', shaban:'שב״ן',
        occupation:'עיסוק', heightCm:'גובה', weightKg:'משקל', bmi:'BMI', smokeStatus:'סטטוס עישון', smokeAmount:'כמות עישון',
        childrenCount:'מספר ילדים', licenseDate:'תאריך רישיון', licenseYears:'וותק רישיון', driverStatus:'סטטוס נהג',
        payerChoice:'סוג משלם', paymentMethod:'אמצעי תשלום', bankName:'שם בנק', bankNo:'מספר בנק', branch:'סניף', account:'מספר חשבון'
      };
      return map[key] || key;
    },

    buildExtraPrimaryFields(primary){
      const skip = new Set(['existingPolicies','cancellations','healthDeclaration','externalPayer','cc','ho','operationalAgentNumbers']);
      const shown = new Set(['firstName','lastName','idNumber','birthDate','gender','maritalStatus','phone','email','city','street','houseNumber','zip','clinic','shaban','occupation','heightCm','weightKg','bmi']);
      return Object.entries(primary || {}).filter(([key,val]) => {
        if(skip.has(key) || shown.has(key)) return false;
        if(val === null || val === undefined) return false;
        if(Array.isArray(val)) return false;
        if(typeof val === 'object') return false;
        return !!safeTrim(val);
      }).map(([key,val]) => [key, this.getOperationalFieldLabel(key), val]);
    },

    getOperationalAgentName(payload){
      return safeTrim(payload?.agentName) || safeTrim(payload?.assignedAgentName) || safeTrim(Auth?.current?.name) || '—';
    },

    getOperationalAgentRole(payload){
      return safeTrim(payload?.agentRole) || safeTrim(Auth?.current?.role) || '—';
    },
    ...
    renderOperationalReport(payloadOverride){
      ...
      const reportAgentName = this.getOperationalAgentName(payload);
      const reportAgentRole = this.getOperationalAgentRole(payload);
      const insuredRows = payload.insureds.map((ins, index) => {
        ...
        return [
          escapeHtml(ins?.label || `מבוטח ${index + 1}`),
          this.renderReportValue(fullName),
      ...
      payload.insureds.forEach((ins, index) => {
        ...
        existingPolicies.forEach((policy) => {
          const cancellation = (safeTrim(policy?.id) && cancellations[safeTrim(policy.id)]) || {};
          const decision = this.getPolicyStatusValue(policy, cancellation);
          const reason = safeTrim(cancellation?.reason || policy?.reason || policy?.note || '');
          existingRows.push([
            escapeHtml(ins?.label || `מבוטח ${index + 1}`),
            this.renderReportValue(policy?.company),
            this.renderReportValue(policy?.type),
            this.renderReportValue(policy?.policyNumber),
            this.renderReportValue(policy?.monthlyPremium),
            this.renderReportValue(decision),
            this.renderReportValue(reason)
          ]);
          if(safeTrim(cancellation?.status) || reason){
            cancelRows.push([
              escapeHtml(ins?.label || `מבוטח ${index + 1}`),
              this.renderReportValue(policy?.company),
              this.renderReportValue(policy?.type),
      ...
      const newRows = (payload.newPolicies || []).map(p => [
        this.renderReportValue(p?.company),
        this.renderReportValue(p?.type),
        this.renderReportValue(p?.premiumMonthly),
        this.renderReportValue(`${safeTrim(p?.discountPct || '0')}%`),
        this.renderReportValue(this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(p || {}))),
        this.renderReportValue(p?.startDate)
      ]);
      ...
      const extraPrimaryFields = this.buildExtraPrimaryFields(primary);
      const payerFields = [];
      if(primary.payerChoice === 'external'){
        ...
        if(anyEx){
          payerFields.push(`<div class="lcReportStack"><div class="lcReportMiniTitle">משלם חיצוני</div><div class="lcReportGrid">${this.compactReportFields(ex, [['relation','קרבה'],['firstName','שם פרטי'],['lastName','שם משפחה'],['idNumber','תעודת זהות'],['birthDate','תאריך לידה'],['phone','טלפון']])}</div></div>`);
        }
      }
      ...
      if(payMethod === 'cc'){
        ...
        if(['holderName','holderId','cardNumber','exp'].some(k => safeTrim(cc[k]))){
          payerFields.push(`<div class="lcReportStack"><div class="lcReportMiniTitle">כרטיס אשראי</div><div class="lcReportGrid">${this.compactReportFields(cc, [['holderName','שם מחזיק'],['holderId','תז מחזיק'],['cardNumber','מספר כרטיס'],['exp','תוקף']])}</div></div>`);
        }
      } else if(payMethod === 'ho'){
        const ho = primary.ho || {};
        if(['bankName','bankNo','branch','account'].some(k => safeTrim(ho[k]))){
          payerFields.push(`<div class="lcReportStack"><div class="lcReportMiniTitle">הוראת קבע</div><div class="lcReportGrid">${this.compactReportFields(ho, [['bankName','שם בנק'],['bankNo','מספר בנק'],['branch','סניף'],['account','מספר חשבון']])}</div></div>`);
        }
      }
      const ts = new Date(payload.createdAt).toLocaleString('he-IL');
      const primaryName = [safeTrim(primary.firstName), safeTrim(primary.lastName)].filter(Boolean).join(' ');
      return `<div class="lcReportDoc">
        <div class="lcReportTopline">
          <div class="lcReportTopline__meta">
            <span class="lcReportTopline__badge">מחלקת תפעול</span>
            <span class="lcReportTopline__badge">מיועד להדפסה / PDF</span>
          </div>
          <div class="lcReportTopline__actions">הדוח כולל את כל נתוני ההקמה, הפוליסות והחלטות הנציג.</div>
        </div>
        <section class="lcReportHeroCard">
          <div class="lcReportBrand">
            <div class="lcReportBrand__text">
              <div class="lcReportBrand__kicker">GEMEL INVEST</div>
              <div class="lcReportBrand__title">דוח תפעולי מלא למבוטח</div>
              <div class="lcReportBrand__sub">הדוח מרכז בצורה מסודרת את כל המידע שהנציג הזין בהקמת הלקוח כדי לאפשר לאנשי התפעול להקליד למערכות הביטוח בצורה מהירה, נקייה ומדויקת.</div>
            </div>
            <div class="lcReportBrand__logoWrap">
              <img alt="GEMEL INVEST" class="lcReportBrand__logo" src="./logo-login-clean.png" />
            </div>
          </div>
          <div class="lcReportSummaryGrid">
            <div class="lcReportSummaryPill"><b>מבוטח ראשי</b><span>${this.renderReportValue(primaryName)}</span></div>
            <div class="lcReportSummaryPill"><b>תעודת זהות</b><span>${this.renderReportValue(primary.idNumber)}</span></div>
            <div class="lcReportSummaryPill"><b>נציג מטפל</b><span>${this.renderReportValue(reportAgentName)}</span></div>
            <div class="lcReportSummaryPill"><b>תפקיד</b><span>${this.renderReportValue(reportAgentRole)}</span></div>
            <div class="lcReportSummaryPill"><b>מבוטחים בדוח</b><span>${payload.insureds.length}</span></div>
            <div class="lcReportSummaryPill"><b>הופק בתאריך</b><span>${escapeHtml(ts)}</span></div>
            <div class="lcReportSummaryPill"><b>פוליסות קיימות</b><span>${existingRows.length}</span></div>
            <div class="lcReportSummaryPill"><b>פוליסות חדשות</b><span>${newRows.length}</span></div>
          </div>
        </section>
        <section class="lcReportSection">
          <div class="lcReportSection__title">פרטי מבוטח ראשי</div>
          <div class="lcReportSection__sub">כל הפרטים שהוקלדו בהקמת הלקוח מוצגים כאן לשימוש ישיר של מחלקת התפעול.</div>
          <div class="lcReportGrid">${this.compactReportFields(primary, [['firstName','שם פרטי'],['lastName','שם משפחה'],['idNumber','תעודת זהות'],['birthDate','תאריך לידה'],['gender','מגדר'],['maritalStatus','מצב משפחתי'],['phone','טלפון'],['email','אימייל'],['city','יישוב'],['street','רחוב'],['houseNumber','מספר בית'],['zip','מיקוד'],['clinic','קופת חולים'],['shaban','שב״ן'],['occupation','עיסוק'],['heightCm','גובה'],['weightKg','משקל'],['bmi','BMI']])}</div>
          ${extraPrimaryFields.length ? `<div class="lcReportSubBlock"><div class="lcReportMiniTitle">פרטים נוספים מההקמה</div><div class="lcReportGrid">${extraPrimaryFields.map(([key,label,val]) => `<div class="lcReportField"><b>${escapeHtml(label)}</b><div class="lcReportValue">${this.renderReportValue(val)}</div></div>`).join('')}</div></div>` : ''}
        </section>
        <section class="lcReportSection"><div class="lcReportSection__title">כל המבוטחים בתיק</div>${this.renderTable(['סוג מבוטח','שם מלא','תעודת זהות','תאריך לידה','טלפון'], insuredRows, { note:'כולל המבוטח הראשי וכל מבוטח נוסף שנוסף בתהליך ההקמה.' })}</section>
        <section class="lcReportSection"><div class="lcReportSection__title">פוליסות קיימות + החלטת הנציג</div><div class="lcReportSection__sub">כאן רואים לכל פוליסה קיימת מה הוחלט לבצע: ביטול מלא, ביטול חלקי, מינוי סוכן או ללא שינוי.</div>${this.renderTable(['מבוטח','חברה','סוג ביטוח','מספר פוליסה','פרמיה חודשית','החלטת נציג','הערה / סיבה'], existingRows, { note:'זהו האזור המרכזי לעבודה התפעולית על הפוליסות הקיימות.' })}</section>
        <section class="lcReportSection"><div class="lcReportSection__title">ביטול / שינוי מול חברה נגדית</div>${this.renderTable(['מבוטח','חברה','סוג ביטוח','סטטוס','סיבה'], cancelRows)}</section>
        <section class="lcReportSection"><div class="lcReportSection__title">פוליסות חדשות</div>${this.renderTable(['חברה','סוג ביטוח','פרמיה חודשית','אחוז הנחה','פרמיה אחרי הנחה','תאריך תחילה'], newRows, { note:'מופיעות רק הפוליסות החדשות שנמכרו בתהליך.' })}</section>
    ...
    openOperationalReport(payloadOverride){
      ...
      this._lastOperationalReportPayload = payloadOverride ? JSON.parse(JSON.stringify(payloadOverride)) : null;
      const customerFileOpen = !!document.querySelector('#customerFull.is-open');
      this.els.report.classList.toggle('is-embedded', customerFileOpen);
      try{
      ...
      this.syncOperationalReportFullscreenButton();
    },

    closeOperationalReport(){
      if(!this.els.report) return;
      this.els.report.classList.remove('is-open', 'is-fullscreen', 'is-embedded');
      this.els.report.setAttribute('aria-hidden','true');
      this.syncOperationalReportFullscreenButton();
    },

    toggleOperationalReportFullscreen(){
      if(!this.els.report) return;
      this.els.report.classList.toggle('is-fullscreen');
      this.syncOperationalReportFullscreenButton();
    },

    syncOperationalReportFullscreenButton(){
      const btn = this.els.reportFullscreen;
      if(!btn || !this.els.report) return;
      btn.textContent = this.els.report.classList.contains('is-fullscreen') ? 'יציאה ממסך מלא' : 'מסך מלא';
    },

    getOperationalReportFileName(payloadOverride){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this._lastOperationalReportPayload || this.getOperationalPayload());
      const primary = payload?.primary && typeof payload.primary === 'object' ? payload.primary : {};
      const fullName = [safeTrim(primary.firstName), safeTrim(primary.lastName)].filter(Boolean).join('_') || 'מבוטח';
      const dateStamp = new Date().toISOString().slice(0,10);
      return `דוח_תפעול_${fullName}_${dateStamp}.html`.replace(/[\/:*?"<>|]+/g, '_');
    },

    getOperationalReportFileStyles(){
      return `
        body{margin:0;padding:28px;background:#f3f7ff;font-family:Arial,"Segoe UI",sans-serif;color:#0f172a;direction:rtl;}
        .lcReportDoc{max-width:980px;margin:0 auto;display:grid;gap:16px}
        .lcReportTopline{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}
        .lcReportTopline__meta{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
        .lcReportTopline__badge{padding:7px 12px;border-radius:999px;background:rgba(217,234,255,.52);border:1px solid rgba(43,75,138,.14);color:#0b1f4a;font-size:12px;font-weight:800}
        .lcReportTopline__actions{font-size:13px;color:rgba(15,23,42,.68);font-weight:700}
        .lcReportHeroCard,.lcReportSection{background:#fff;border:1px solid rgba(15,23,42,.08);border-radius:24px;box-shadow:0 18px 44px rgba(15,23,42,.07)}
        .lcReportHeroCard{padding:20px;background:linear-gradient(180deg,rgba(255,255,255,.98),rgba(244,249,255,.96))}
        .lcReportBrand{display:grid;grid-template-columns:1fr auto;gap:18px;align-items:start}
        .lcReportBrand__text{display:grid;gap:8px}.lcReportBrand__kicker{font-size:12px;font-weight:900;letter-spacing:.18em;color:rgba(43,75,138,.76)}
        .lcReportBrand__title{font-size:30px;font-weight:950;line-height:1.1;color:#0b1f4a}.lcReportBrand__sub{font-size:14px;line-height:1.75;color:rgba(15,23,42,.72);max-width:760px}
        .lcReportBrand__logoWrap{display:flex;align-items:flex-start;justify-content:flex-end}.lcReportBrand__logo{width:160px;max-width:32vw;object-fit:contain}
        .lcReportSummaryGrid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:18px}
        .lcReportSummaryPill{padding:12px 14px;border-radius:18px;background:rgba(243,247,255,.96);border:1px solid rgba(43,75,138,.09);display:grid;gap:6px}
        .lcReportSummaryPill b,.lcReportField b{display:block;font-size:12px;color:rgba(43,75,138,.78)}
        .lcReportSummaryPill span,.lcReportValue{font-size:14px;line-height:1.55;word-break:break-word;color:#0f172a;font-weight:700}
        .lcReportSection{padding:18px 20px}.lcReportSection__title{font-size:20px;font-weight:950;color:#0b1f4a}.lcReportSection__sub{font-size:13px;color:rgba(15,23,42,.65);margin-top:5px;margin-bottom:12px;line-height:1.65}
        .lcReportSubBlock{display:grid;gap:10px;margin-top:14px;padding-top:14px;border-top:1px dashed rgba(43,75,138,.16)}.lcReportMiniTitle{font-size:13px;font-weight:900;color:rgba(43,75,138,.86)}
        .lcReportGrid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.lcReportField,.lcReportListItem{padding:11px 13px;border-radius:16px;background:rgba(248,250,252,.98);border:1px solid rgba(15,23,42,.07)}
        .lcReportTableNote{margin-bottom:10px;padding:10px 12px;border-radius:14px;background:rgba(243,247,255,.85);border:1px solid rgba(43,75,138,.08);color:rgba(15,23,42,.7);font-size:12px;font-weight:700}
        .lcReportTableWrap{overflow:auto;border:1px solid rgba(15,23,42,.07);border-radius:18px;background:#fff}.lcReportTable{width:100%;border-collapse:separate;border-spacing:0;background:#fff}
        .lcReportTable th,.lcReportTable td{padding:12px 10px;border-bottom:1px solid rgba(15,23,42,.06);font-size:13px;vertical-align:top;text-align:right}
        .lcReportTable th{background:linear-gradient(180deg,rgba(227,238,255,.9),rgba(244,249,255,.96));color:rgba(43,75,138,.84);font-size:12px}
        .lcReportTable tbody tr:nth-child(even) td{background:rgba(248,250,252,.72)}.lcReportList{display:grid;gap:9px}.lcReportListItem strong{display:block;margin-bottom:5px;color:#0b1f4a}
      `;
    },

    downloadOperationalReportFile(payloadOverride){
      try{
        const payload = payloadOverride || this._lastOperationalReportPayload || this.getOperationalPayload();
        const html = this.renderOperationalReport(payload);
        const doc = `<!DOCTYPE html><html lang="he" dir="rtl"><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/><title>דוח תפעול</title><style>${this.getOperationalReportFileStyles()}</style></head><body>${html}</body></html>`;
        const blob = new Blob([doc], { type:'text/html;charset=utf-8' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = this.getOperationalReportFileName(payload);
        ...
        console.error('downloadOperationalReportFile failed', err);
        alert('לא הצלחנו להוריד את הקובץ כרגע. נסה שוב.');
      }
    },

    exportOperationalPdf(){
      if(!this.els.report?.classList.contains('is-open')) this.openOperationalReport(this._lastOperationalReportPayload || undefined);
      window.print();
    },
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const MirrorsUI = {
    ...
    getMirrorNewPolicyDiscountText(policy){
      const pct = safeTrim(policy?.discountPct || '0') || '0';
      const years = safeTrim(policy?.discountYears || '');
      return years ? `${pct}% · ${years} שנים` : `${pct}%`;
    },
