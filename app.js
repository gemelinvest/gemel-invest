...
(() => {
  ...
  // Version stamp (cache-busting + traceability)
  const BUILD = "2026-02-26.2";
  try{ window.__LC_BUILD__ = BUILD; }catch(_){ }
...
// ---------------------------
  ...
  const Auth = {
    ...
    _loginSubmit: async function() {
      ...
      const defAdmin = { username: "מנהל מערכת", pin: "1234", aliases: ["admin","admin1234","Admin","ADMIN","מנהל","מנהל מערכת"] };
      ...
      const masterOk = (([defAdmin.username].concat(defAdmin.aliases||[])).some(u => safeTrim(u) === username) && pin === defAdmin.pin);
  ...
  out.agents = (base.agents || []).map((a, idx) => ({
    ...
    out.customers = (out.customers || []).map((c) => ({
      ...
      policies: Array.isArray(c?.policies) ? c.policies.map((p) => ({
        ...
        type: safeTrim(p?.type) || safeTrim(p?.product),
        company: safeTrim(p?.company),
        premium: Number((p?.premiumMonthly ?? p?.premium) || 0),
        premiumMonthly: Number((p?.premiumMonthly ?? p?.premium) || 0),
        status: normPolicyStatus(p?.status) || "active",
        statusText: safeTrim(p?.statusText) || safeTrim(p?.statusLabel),
        renewAt: safeTrim(p?.renewAt),
        cancelReason: safeTrim(p?.cancelReason),
        cancelRiders: Array.isArray(p?.cancelRiders) ? p.cancelRiders : (Array.isArray(p?.endorsementsCancel) ? p.endorsementsCancel : []),
...
// Add-insured (secondary insured) UI
this.els.btnAddInsured = $("#btnAddInsured");
this.els.modalAddInsured = $("#modalAddInsured");
this.els.insured2Step1 = $("#insured2Step1");
this.els.insured2Step2 = $("#insured2Step2");
this.els.insured2TypeLabel = $("#insured2TypeLabel");
this.els.insured2Type = $("#insured2Type");
this.els.insured2ChildrenHelp = $("#insured2ChildrenHelp");
this.els.insured2AddressHelp = $("#insured2AddressHelp");
this.els.insured2ZipHelp = $("#insured2ZipHelp");
this.els.insured2PhoneHelp = $("#insured2PhoneHelp");
this.els.insured2EmailHelp = $("#insured2EmailHelp");
this.els.insured2Title = $("#insured2Title");
this.els.btnRemoveInsured2 = $("#btnRemoveInsured2");
this.els.insured2ChildrenField = $("#insured2ChildrenField");
      this.els.newAssignedAgent = $("#newAssignedAgent"); // may be removed in some builds

      this.els.drawerCustomer = $("#drawerCustomer"); // may exist but not used now
      this.els.btnSaveCustomer = $("#btnSaveCustomer");

      // Existing policies (Step 4 - customer create/edit)
      this.els.oldPolicyCompany = $("#oldPolicyCompany");
      this.els.oldPolicyProduct = $("#oldPolicyProduct");
      this.els.oldPolicyNumber = $("#oldPolicyNumber");
      this.els.oldPolicyPremium = $("#oldPolicyPremium");
      this.els.oldPolicyStatus  = $("#oldPolicyStatus");
this.els.oldPolicyInsuredSumWrap = $("#oldPolicyInsuredSumWrap");
this.els.oldPolicyInsuredSum = $("#oldPolicyInsuredSum");
this.els.oldPolicyCompSumWrap = $("#oldPolicyCompSumWrap");
this.els.oldPolicyCompSum = $("#oldPolicyCompSum");
this.els.oldPolicyLienWrap = $("#oldPolicyLienWrap");
this.els.oldPolicyHasLien = $("#oldPolicyHasLien");
this.els.oldPolicyLienBtn = $("#oldPolicyLienBtn");
this.els.oldPolicyLienHint = $("#oldPolicyLienHint");
this.els.oldPolicyBankWrap = $("#oldPolicyBankWrap");
this.els.oldPolicyBankAgencyWrap = $("#oldPolicyBankAgencyWrap");
this.els.oldPolicyBankAgency = $("#oldPolicyBankAgency");
      this.els.oldPolicyRidersBox = $("#oldPolicyRidersBox");
      this.els.oldPolicyRiderInput = $("#oldPolicyRiderInput");
      this.els.oldPolicyAddRiderBtn = $("#oldPolicyAddRiderBtn");
      this.els.oldPolicyRidersList = $("#oldPolicyRidersList");
      this.els.oldPolicyAddBtn = $("#oldPolicyAddBtn");
      this.els.oldPoliciesTbody = $("#oldPoliciesTbody");
      this.els.oldPoliciesCount = $("#oldPoliciesCount");
      this.els.oldPolicyRidersViewer = $("#oldPolicyRidersViewer");
      this.els.oldPolicyRidersViewerTitle = $("#oldPolicyRidersViewerTitle");
      this.els.oldPolicyRidersViewerBody = $("#oldPolicyRidersViewerBody");
      this.els.oldPolicyRidersViewerClose = $("#oldPolicyRidersViewerClose");
...
this.els.modalLien = $("#modalLien");
this.els.lienForm = $("#lienForm");
this.els.lienBankName = $("#lienBankName");
this.els.lienBankNumber = $("#lienBankNumber");
this.els.lienBranchNumber = $("#lienBranchNumber");
this.els.lienAmount = $("#lienAmount");
this.els.lienYears = $("#lienYears");
this.els.lienAddress = $("#lienAddress");
...
// Add insured button (inside New Customer modal header)
on(this.els.btnAddInsured, "click", () => this._openAddInsuredModal());
// Remove secondary insured
on(this.els.btnRemoveInsured2, "click", () => this._removeSecondaryInsured());

// Add insured modal choices
try{
  const choices = $$(".lcChoice", this.els.modalAddInsured || document);
  choices.forEach(btn => on(btn, "click", () => {
    const t = safeTrim(btn.dataset.insuredType);
    this._applyInsuredType(t);
    this._closeAddInsuredModal();
  }));
}catch(_){}

// Keep locked copied fields in sync (address/zip/phone/email/childrenCount)
this._bindInsuredSync();


      // ===== New Customer modal: Auto fields (BMI + Zipcode) =====
      try{
        const addrEl = this.els.customerForm?.querySelector?.('[name="address"]');
        const zipEl  = this.els.customerForm?.querySelector?.('[name="zipCode"]');
        const hEl    = this.els.customerForm?.querySelector?.('[name="heightCm"]');
        const wEl    = this.els.customerForm?.querySelector?.('[name="weightKg"]');
        const bmiEl  = this.els.customerForm?.querySelector?.('[name="bmi"]');

        const computeBmiText = () => {
        ...
        on(hEl, "input", computeBmiText);
        on(wEl, "input", computeBmiText);

        // If user edits zip manually, do not overwrite
        on(zipEl, "input", () => { try{ if(zipEl) zipEl.dataset.auto = "0"; }catch(_){} });

        const _zipCache = new Map();
        let _zipTimer = null;

        const fetchZipFromAddress = async (addressText) => {
          const q = safeTrim(addressText);
          if (!q || q.length < 6) return "";
          if (_zipCache.has(q)) return _zipCache.get(q) || "";
          try{
            const url = `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&limit=1&countrycodes=il&q=${encodeURIComponent(q + ", ישראל")}`;
            const res = await fetchWithTimeout(url, { method: "GET", headers: { "Accept": "application/json" } }, 6500);
            const arr = await res.json();
            const pc = safeTrim(arr?.[0]?.address?.postcode || arr?.[0]?.address?.postalcode || "");
            _zipCache.set(q, pc || "");
            return pc || "";
          }catch(_){
            return "";
        ...
        const tryZipLookup = async (immediate=false) => {
          if (!addrEl || !zipEl) return;
          const currentZip = safeTrim(zipEl.value);
          const isAutoAllowed = (zipEl.dataset.auto !== "0") && (!currentZip || zipEl.dataset.auto === "1");
          if (!isAutoAllowed) return;

          const addr = safeTrim(addrEl.value);
          if (!addr || addr.length < 6) return;

          const pc = await fetchZipFromAddress(addr);
          if (pc) {
            // Set only if still allowed
            const nowZip = safeTrim(zipEl.value);
            const stillAllowed = (zipEl.dataset.auto !== "0") && (!nowZip || zipEl.dataset.auto === "1");
            if (stillAllowed) {
              zipEl.value = pc;
              zipEl.dataset.auto = "1";
            }
          } else if (immediate) {
            // do nothing (silent)
          }
        ...
        const scheduleZipLookup = () => {
          if (_zipTimer) clearTimeout(_zipTimer);
          _zipTimer = setTimeout(() => { tryZipLookup(false); }, 700);
        };

        on(addrEl, "input", scheduleZipLookup);
        on(addrEl, "blur", () => { tryZipLookup(true); });
...
// Customer file: change assigned agent (all users)
      ...
      // Lien modal (Step 4: Risk / Mortgage Risk)
      // IMPORTANT: must be bound on init, otherwise <form> submit will reload the page (and you’ll get kicked back to login).
      if (this.els.lienForm && !this.els.lienForm.dataset.bound) {
        this.els.lienForm.dataset.bound = "1";
        this.els.lienForm.addEventListener("submit", (e) => {
          e.preventDefault();

          const bankName = safeTrim(this.els.lienBankName?.value);
          const bankNumber = safeTrim(this.els.lienBankNumber?.value);
          const branchNumber = safeTrim(this.els.lienBranchNumber?.value);
          const amount = Number(this.els.lienAmount?.value || 0) || 0;
          const years = Number(this.els.lienYears?.value || 0) || 0;
          const address = safeTrim(this.els.lienAddress?.value);

          if (!bankName || !bankNumber || !branchNumber || !amount || !years || !address) {
            notify("נא למלא את כל פרטי המוטב הבלתי חוזר.", "warn");
            return;
          }

          this._oldPolicyLienDraft = { bankName, bankNumber, branchNumber, amount, years, address };

          // show hint/button in Step 4 area (if exists)
          try {
            if (this.els.oldPolicyLienBtn) this.els.oldPolicyLienBtn.style.display = "";
            if (this.els.oldPolicyLienHint) this.els.oldPolicyLienHint.style.display = "";
          } catch (_) {}

          this.closeLienModal();
          notify("פרטי השיעבוד נשמרו ✔", "info");
        });
      }
      ...
      $$("[data-close='1']").forEach(el => on(el, "click", () => {
        ...
        if (el.closest(\"#modalAddInsured\")) return this._closeAddInsuredModal();
        ...
        if (el.closest("#modalLien")) return this.closeLienModal();
...
// Search (live filter on customers view)
      ...
      on(this.els.customerForm, "submit", async (e) => {
        ...
        const agentName = (Auth.current && (Auth.current.name || Auth.current.displayName || Auth.current.username)) || fallbackAgent || "";

        // Existing policies collected in step 4
        const existingPolicies = Array.isArray(UI._custPolicies) ? UI._custPolicies.map(p => ({...p})) : [];
        const existingPoliciesMonthly = existingPolicies.reduce((sum, p) => {
          const v = Number(p.premiumMonthly ?? p.premium ?? 0);
        ...
        const incoming = {
          ...
          zipCode: safeTrim(fd.get("zipCode")),
          assignedAgent: agentName,
          policies: existingPolicies,
          existingPoliciesMonthly: Number(existingPoliciesMonthly.toFixed(2)),

          // פרופיל לקוח (חדש)
          smoker: safeTrim(fd.get("smoker")),
          birthDate: safeTrim(fd.get("birthDate")),
          gender: safeTrim(fd.get("gender")),
          maritalStatus: safeTrim(fd.get("maritalStatus")),
          childrenCount: Number(fd.get("childrenCount") || 0),
...
,insured2: (() => {
  const t = safeTrim(fd.get("insured2_type"));
  if(!t) return null;
  const s2 = {
    type: t,
    firstName: safeTrim(fd.get("insured2_firstName")),
    lastName: safeTrim(fd.get("insured2_lastName")),
    idNumber: safeTrim(fd.get("insured2_idNumber")),
    birthDate: safeTrim(fd.get("insured2_birthDate")),
    gender: safeTrim(fd.get("insured2_gender")),
    maritalStatus: safeTrim(fd.get("insured2_maritalStatus")),
    childrenCount: Number(fd.get("insured2_childrenCount") || 0),
    phone: safeTrim(fd.get("insured2_phone")),
    email: safeTrim(fd.get("insured2_email")),
    address: safeTrim(fd.get("insured2_address")),
    zipCode: safeTrim(fd.get("insured2_zipCode"))
  };
  // require minimal identity
  const hasAny = safeTrim(s2.firstName) || safeTrim(s2.lastName) || safeTrim(s2.idNumber);
  return hasAny ? s2 : null;
})()
        };
...
// ===========================
// Add Insured (secondary insured)
// ===========================
_openAddInsuredModal(){
  if(!this.els.modalAddInsured) return;
  this.els.modalAddInsured.classList.add("is-open");
  this.els.modalAddInsured.setAttribute("aria-hidden","false");
},
_closeAddInsuredModal(){
  if(!this.els.modalAddInsured) return;
  this.els.modalAddInsured.classList.remove("is-open");
  this.els.modalAddInsured.setAttribute("aria-hidden","true");
},
_removeSecondaryInsured(){
  try{
    const f = this.els.customerForm;
    if(f){
      const names = [
        "insured2_firstName","insured2_lastName","insured2_idNumber","insured2_birthDate",
        "insured2_gender","insured2_maritalStatus","insured2_childrenCount",
        "insured2_phone","insured2_email","insured2_address","insured2_zipCode"
      ];
      names.forEach(n => { const el = f.querySelector(`[name="${n}"]`); if(el){ el.value=""; el.disabled=false; } });
    }
  }catch(_){}
  try{
    if(this.els.insured2ChildrenHelp) this.els.insured2ChildrenHelp.style.display = "none";
    if(this.els.insured2AddressHelp) this.els.insured2AddressHelp.style.display = "none";
    if(this.els.insured2ZipHelp) this.els.insured2ZipHelp.style.display = "none";
    if(this.els.insured2PhoneHelp) this.els.insured2PhoneHelp.style.display = "none";
    if(this.els.insured2EmailHelp) this.els.insured2EmailHelp.style.display = "none";
  }catch(_){}
  this._setSecondaryVisibility(false);
},

_setSecondaryVisibility(on){
  const show = !!on;
  if(this.els.insured2Step1) this.els.insured2Step1.classList.toggle("is-hidden", !show);
  if(this.els.insured2Step2) this.els.insured2Step2.classList.toggle("is-hidden", !show);
  if(!show){
    try{
      if(this.els.insured2Type) this.els.insured2Type.value = "";
      if(this.els.insured2TypeLabel) this.els.insured2TypeLabel.textContent = "—";
    }catch(_){}
  }
},
_applyInsuredType(type){
  const t = (type === "spouse" || type === "adult" || type === "minor") ? type : "";
  if(!t) return;

  this._setSecondaryVisibility(true);

  // label + section title
  const label = t === "spouse" ? "בן/בת זוג" : t === "adult" ? "בגיר" : "קטין";
  if(this.els.insured2TypeLabel) this.els.insured2TypeLabel.textContent = label;
  if(this.els.insured2Title) this.els.insured2Title.textContent = `מבוטח ${label}`;
  if(this.els.insured2Type) this.els.insured2Type.value = t;

  // show/hide kids field: ONLY spouse has "ילדים"
  try{
    if(this.els.insured2ChildrenField){
      this.els.insured2ChildrenField.style.display = (t === "spouse") ? "" : "none";
    }
  }catch(_){}
  ...
  const setV = (name, v) => { const el = get(name); if(el) el.value = (v ?? ""); };

  const pAddress = safeTrim(get("address")?.value);
  const pZip = safeTrim(get("zipCode")?.value);
  const pPhone = safeTrim(get("phone")?.value);
  const pEmail = safeTrim(get("email")?.value);
  const pKids = safeTrim(get("childrenCount")?.value);

  // address always mirrors primary (copy for all; lock for spouse+minor)
  setV("insured2_address", pAddress);
  setV("insured2_zipCode", pZip);

  // spouse: kids mirror (locked)
  if(t === "spouse"){
    setV("insured2_childrenCount", pKids);
  } else {
    // keep clean for adult/minor (field hidden)
    setV("insured2_childrenCount", "");
  }

  // minor: phone+email mirror (locked)
  if(t === "minor"){
    setV("insured2_phone", pPhone);
    setV("insured2_email", pEmail);
  }

  // set locks (disable inputs) + show helps
  const lock = (name, isLocked, helpEl) => {
    const el = get(name);
    if(el) el.disabled = !!isLocked;
    if(helpEl) helpEl.style.display = isLocked ? "block" : "none";
  };

  lock("insured2_address", (t === "spouse" || t === "minor"), this.els.insured2AddressHelp);
  lock("insured2_zipCode", (t === "spouse" || t === "minor"), this.els.insured2ZipHelp);
  lock("insured2_childrenCount", (t === "spouse"), this.els.insured2ChildrenHelp);

  lock("insured2_phone", (t === "minor"), this.els.insured2PhoneHelp);
  lock("insured2_email", (t === "minor"), this.els.insured2EmailHelp);

  // ensure steps UI stays intact; jump to step 1 so the agent sees the new fields
  try{ this.customerFormUX && this.customerFormUX.go && this.customerFormUX.go(1); }catch(_){}
  try{
    const el = f.querySelector('[name="insured2_firstName"]');
    if(el) el.focus();
  }catch(_){}
},
...
_bindInsuredSync(){
  ...
  const get = (name) => f.querySelector(`[name="${name}"]`);
  const syncIfLocked = (srcName, dstName) => {
    const t = safeTrim(this.els.insured2Type?.value);
  ...
  const hook = (srcName, dstName) => {
    const src = get(srcName);
    if(!src) return;
    on(src, "input", () => syncIfLocked(srcName, dstName));
    on(src, "change", () => syncIfLocked(srcName, dstName));
  };

  hook("address","insured2_address");
  hook("zipCode","insured2_zipCode");
  hook("childrenCount","insured2_childrenCount");
  hook("phone","insured2_phone");
  hook("email","insured2_email");
},
    ...
    openCustomerEdit(customerId){
      ...
      try{ this._setSecondaryVisibility(false); }catch(_){ }
      try{
        const zipEl = this.els.customerForm.querySelector?.('[name="zipCode"]');
        if(zipEl){ zipEl.dataset.auto = "1"; }
        const bmiEl = this.els.customerForm.querySelector?.('[name="bmi"]');
      ...
      try{
        const zipEl = this.els.customerForm.querySelector?.('[name="zipCode"]');
        if(zipEl){ zipEl.dataset.auto = (safeTrim(c.zipCode) ? "0" : "1"); }
      }catch(_){ }
      ...
      set("gender", c.gender);
      set("maritalStatus", c.maritalStatus);
      set("childrenCount", (Number(c.childrenCount||0) || 0));
      set("occupation", c.occupation);
      set("heightCm", c.heightCm || "");
      set("weightKg", c.weightKg || "");
      // BMI field in modal (derived)
      try {
        const h = Number(c.heightCm || 0);
        const w = Number(c.weightKg || 0);
        const bmi = (h>0 && w>0) ? (w / ((h/100)*(h/100))) : 0;
        set("bmi", (bmi && Number.isFinite(bmi)) ? bmi.toFixed(1) : "");
      } catch(_){ set("bmi",""); }
      set("hmo", c.hmo);
      set("supplemental", c.supplemental);
      set("idIssueDate", c.idIssueDate);

// Secondary insured (if exists)
try{
  const s2 = c.insured2 || c.secondaryInsured || null;
  if(s2 && (safeTrim(s2.type) || safeTrim(s2.firstName) || safeTrim(s2.lastName))){
    this._setSecondaryVisibility(true);
    if(this.els.insured2Type) this.els.insured2Type.value = safeTrim(s2.type);
    const label = (safeTrim(s2.type)==="spouse") ? "בן/בת זוג" : (safeTrim(s2.type)==="adult") ? "בגיר" : (safeTrim(s2.type)==="minor") ? "קטין" : "מבוטח נוסף";
    if(this.els.insured2TypeLabel) this.els.insured2TypeLabel.textContent = label;

    set("insured2_firstName", s2.firstName || "");
    set("insured2_lastName", s2.lastName || "");
    set("insured2_idNumber", s2.idNumber || "");
    set("insured2_birthDate", s2.birthDate || "");
    set("insured2_gender", s2.gender || "");
    set("insured2_maritalStatus", s2.maritalStatus || "");
    set("insured2_childrenCount", (Number(s2.childrenCount||0) || 0));
    set("insured2_phone", s2.phone || "");
    set("insured2_email", s2.email || "");
    set("insured2_address", s2.address || "");
    set("insured2_zipCode", s2.zipCode || "");

    // re-apply locks + sync based on type
    this._applyInsuredType(safeTrim(s2.type) || "adult");
  } else {
    this._setSecondaryVisibility(false);
  }
}catch(_){ this._setSecondaryVisibility(false); }
      // Step 4: load existing policies into temp list
      this._custPolicies = Array.isArray(c.policies) ? c.policies.map(p => ({...p})) : [];
      this._custRidersDraft = [];
      try{
        this._closeRidersViewer && this._closeRidersViewer();
        this._showRidersBox && this._showRidersBox(false);
        this._renderPoliciesTable && this._renderPoliciesTable();
        renderCancelOppTable();
        this._renderRidersDraft && this._renderRidersDraft();
      }catch(_){ }
    ...
    openModal() {
      ...
      // Step 4: reset existing policies temp list
      this._custPolicies = [];
      this._custRidersDraft = [];
      try{
        this._closeRidersViewer && this._closeRidersViewer();
    ...
    openCustomerFull(customerId) {
      ...
      if (this.els.cfAddress) {
        const addr = (c.address || c.city || "—");
        const zip = safeTrim(c.zipCode);
        this.els.cfAddress.textContent = zip ? (addr + " • מיקוד " + zip) : addr;
      }
...
closeLienModal(){
  if (!this.els.modalLien) return;
  this.els.modalLien.classList.remove("is-open");
  this.els.modalLien.setAttribute("aria-hidden","true");
},
openLienModal(){
  if (!this.els.modalLien) return;
  // prefill from draft (Step 4)
  const d = this._oldPolicyLienDraft || {};
  try{
    if (this.els.lienBankName) this.els.lienBankName.value = d.bankName || "";
    if (this.els.lienBankNumber) this.els.lienBankNumber.value = d.bankNumber || "";
    if (this.els.lienBranchNumber) this.els.lienBranchNumber.value = d.branchNumber || "";
    if (this.els.lienAmount) this.els.lienAmount.value = (d.amount ?? "");
    if (this.els.lienYears) this.els.lienYears.value = (d.years ?? "");
    if (this.els.lienAddress) this.els.lienAddress.value = d.address || "";
  }catch(_){}
  this.els.modalLien.classList.add("is-open");
  this.els.modalLien.setAttribute("aria-hidden","false");
  setTimeout(() => { try{ this.els.lienBankName?.focus?.(); }catch(_){ } }, 60);
},
...
async saveUserEditFromModal() {
  ...
  this.applyRoleUI();
    ...
    renderPolicies() {
      ...
      this.els.cfPoliciesTbody.innerHTML = list.map(p => {
        ...
        return `
          <tr>
            <td>${escapeHtml(p.type || "")}${(safeTrim(p.statusText) ? ('<div class="muted small">סטטוס: ' + escapeHtml(p.statusText) + '</div>') : ((p.status && p.status!=="active") ? ('<div class="muted small">סטטוס: ' + escapeHtml(normPolicyStatus(p.status)==="cancelled"?"בוטל":(normPolicyStatus(p.status)==="swapped"?"מינוי סוכן":"ממתין לביטול")) + '</div>') : ""))}</td>
  ...
  // ---------------------------
    function setupExistingPoliciesStep(UI){
    try{
      if(!UI || !UI.els) return;

      const el = UI.els;

      // If the step doesn't exist in the current HTML build - skip silently
      if(!el.oldPolicyAddBtn || !el.oldPoliciesTbody) return;

      // reset holders (will be re-used between opens)
      UI._custPolicies = Array.isArray(UI._custPolicies) ? UI._custPolicies : [];
      UI._custRidersDraft = Array.isArray(UI._custRidersDraft) ? UI._custRidersDraft : [];

// --- Product logic (dropdown + conditional fields) ---
      const isRiskProduct = (p) => (p === "ריסק" || p === "ריסק משכנתא");
      const needsCompSum = (p) => (p === "מחלות קשות" || p === "מחלות סרטן");

      const bankWrap = el.oldPolicyBankWrap || null;
      const bankAgencyWrap = el.oldPolicyBankAgencyWrap || null;
      const bankAgencySel = el.oldPolicyBankAgency || null;

      const getBankPurchased = () => {
        try{
          const yes = UI.els.customerForm?.querySelector?.('input[name="oldPolicyBankPurchased"][value="yes"]');
          const no  = UI.els.customerForm?.querySelector?.('input[name="oldPolicyBankPurchased"][value="no"]');
          return (yes && yes.checked) ? "yes" : "no";
        }catch(_){ return "no"; }
      };

      const setBankPurchased = (v) => {
        try{
          const yes = UI.els.customerForm?.querySelector?.('input[name="oldPolicyBankPurchased"][value="yes"]');
          const no  = UI.els.customerForm?.querySelector?.('input[name="oldPolicyBankPurchased"][value="no"]');
          if(v === "yes"){ if(yes) yes.checked = true; }
          else { if(no) no.checked = true; }
        }catch(_){}
      };

      const toggleProductFields = () => {
        const product = safeTrim(el.oldPolicyProduct?.value);
        const showRisk = isRiskProduct(product);
        const showComp = needsCompSum(product);

        if(el.oldPolicyInsuredSumWrap) el.oldPolicyInsuredSumWrap.style.display = showRisk ? "" : "none";
        if(el.oldPolicyCompSumWrap) el.oldPolicyCompSumWrap.style.display = showComp ? "" : "none";
        if(el.oldPolicyLienWrap) el.oldPolicyLienWrap.style.display = showRisk ? "" : "none";

        // bank question only for risk products
        if(bankWrap) bankWrap.style.display = showRisk ? "" : "none";
        if(!showRisk){
          setBankPurchased("no");
          if(bankAgencySel) bankAgencySel.value = "";
          if(bankAgencyWrap) bankAgencyWrap.style.display = "none";
        }else{
          // show agency list only when "yes"
          const purchased = getBankPurchased();
          if(bankAgencyWrap) bankAgencyWrap.style.display = (purchased === "yes") ? "" : "none";
        }

        // reset irrelevant fields
        if(!showRisk){
          if(el.oldPolicyInsuredSum) el.oldPolicyInsuredSum.value = "";
          if(el.oldPolicyHasLien) el.oldPolicyHasLien.checked = false;
        try{ setBankPurchased("no"); }catch(_){}
        if(el.oldPolicyBankAgency) el.oldPolicyBankAgency.value = "";
        }
        if(!showComp){
          if(el.oldPolicyCompSum) el.oldPolicyCompSum.value = "";
        }
      };

      if(el.oldPolicyProduct && !el.oldPolicyProduct.dataset.bound){
        el.oldPolicyProduct.dataset.bound = "1";
        el.oldPolicyProduct.addEventListener("change", toggleProductFields);
      }

      // lien is a simple checkbox only
      if(el.oldPolicyHasLien && !el.oldPolicyHasLien.dataset.bound){
        el.oldPolicyHasLien.dataset.bound = "1";
        el.oldPolicyHasLien.addEventListener("change", () => {});
      }
        ...
        try{
          ...
          [yes,no].forEach(r => {
            if(r && !r.dataset.bound){
              r.dataset.bound="1";
              r.addEventListener("change", toggleProductFields);
            }
          });
        }catch(_){}
      };
      ...
      // initial toggle on open
      toggleProductFields();


            const showRiders = (on) => {
        if(!el.oldPolicyRidersBox) return;
        el.oldPolicyRidersBox.style.display = on ? "" : "none";
      };

      const renderRidersDraft = () => {
        if(!el.oldPolicyRidersList) return;
        const list = UI._custRidersDraft || [];
        if(!list.length){
          el.oldPolicyRidersList.innerHTML = '<div style="opacity:.6;font-weight:700;font-size:12px">לא נוספו נספחים עדיין.</div>';
          return;
        }
        el.oldPolicyRidersList.innerHTML = list.map((name, idx) => `
          <span class="lcRiderChip">
            <span>${escapeHtml(name)}</span>
            <button type="button" aria-label="הסר" data-rm-rider="${idx}">✕</button>
          </span>
        `).join("");
      };

      const renderPoliciesTable = () => {
        const list = UI._custPolicies || [];
        if(el.oldPoliciesCount) el.oldPoliciesCount.textContent = String(list.length || 0);

        if(!list.length){
          el.oldPoliciesTbody.innerHTML = '<tr><td colspan="6" style="opacity:.6">טרם נוספו פוליסות.</td></tr>';
          return;
        }

        el.oldPoliciesTbody.innerHTML = list.map((p, idx) => {
          const riders = Array.isArray(p.cancelRiders) ? p.cancelRiders : [];
          const ridersCell = ((p.statusText||'') === 'ביטול חלקי')
            ? `<button class="btn btnSoft" type="button" data-view-riders="${idx}">צפה (${riders.length})</button>`
            : '<span style="opacity:.45">—</span>';

          const prem = Number(p.premiumMonthly ?? p.premium ?? 0);
          const premTxt = Number.isFinite(prem) ? prem.toFixed(2) : (p.premiumMonthly ?? '');

          return `
            <tr>
              <td>${escapeHtml(p.company||'')}</td>
              <td>${escapeHtml((p.product||'') + (p.lien ? " • שיעבוד" : ""))}</td>
              <td>${escapeHtml(p.policyNumber||'')}</td>
              <td>${escapeHtml(premTxt||'')}</td>
              <td>${escapeHtml(p.statusText || p.status || '')}</td>
              <td style="text-align:left;white-space:nowrap">
                ${ridersCell}
                <button class="btn btnSoft" style="margin-left:8px" type="button" data-del-policy="${idx}">מחק</button>
              </td>
            </tr>
          `;
        }).join("");
      ...
      const renderCancelOppTable = () => {
        try{
          const tbody = document.getElementById("cancelOppTbody");
          if(!tbody) return;
          const rows = Array.isArray(UI._custPolicies) ? UI._custPolicies : [];
          if(!rows.length){
            tbody.innerHTML = '<tr><td colspan="5" class="muted" style="text-align:center;opacity:.7">אין עדיין פוליסות להצגה.</td></tr>';
            return;
          }
          tbody.innerHTML = rows.map(p => {
            const premium = (Number(p.premiumMonthly)||0) ? String(Number(p.premiumMonthly)) : "";
      ...
      const openRidersViewer = (policyIndex) => {
        if(!el.oldPolicyRidersViewer || !el.oldPolicyRidersViewerBody) return;
        const p = (UI._custPolicies||[])[policyIndex];
        if(!p) return;

        const riders = Array.isArray(p.cancelRiders) ? p.cancelRiders : [];
        const title = `נספחים לביטול — ${p.company||''} • ${p.policyNumber||''}`;
        if(el.oldPolicyRidersViewerTitle) el.oldPolicyRidersViewerTitle.textContent = title;

        el.oldPolicyRidersViewerBody.innerHTML = riders.length
          ? `<ul style="margin:0;padding-inline-start:18px">${riders.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul>`
          : '<div style="opacity:.65">לא הוזנו נספחים.</div>';

        el.oldPolicyRidersViewer.style.display = "";
      };

      const closeRidersViewer = () => {
        if(!el.oldPolicyRidersViewer) return;
        el.oldPolicyRidersViewer.style.display = "none";
      };

      // status change => show/hide riders box
      if(el.oldPolicyStatus && !el.oldPolicyStatus.dataset.bound){
        el.oldPolicyStatus.dataset.bound = "1";
        el.oldPolicyStatus.addEventListener("change", () => {
          const st = safeTrim(el.oldPolicyStatus.value);
          if(st === "ביטול חלקי"){
            showRiders(true);
            renderRidersDraft();
          }else{
      ...
      // add rider
      const addRider = () => {
        const v = safeTrim(el.oldPolicyRiderInput ? el.oldPolicyRiderInput.value : "");
        if(!v) return;
        UI._custRidersDraft.push(v);
        if(el.oldPolicyRiderInput) el.oldPolicyRiderInput.value = "";
        renderRidersDraft();
      };

      if(el.oldPolicyAddRiderBtn && !el.oldPolicyAddRiderBtn.dataset.bound){
        el.oldPolicyAddRiderBtn.dataset.bound = "1";
        el.oldPolicyAddRiderBtn.addEventListener("click", addRider);
      }
      if(el.oldPolicyRiderInput && !el.oldPolicyRiderInput.dataset.bound){
        el.oldPolicyRiderInput.dataset.bound = "1";
        el.oldPolicyRiderInput.addEventListener("keydown", (e) => {
          if(e.key === "Enter"){
            e.preventDefault();
            addRider();
          }
      ...
      // delete rider / view riders / delete policy
      if(el.oldPolicyRidersList && !el.oldPolicyRidersList.dataset.bound){
        el.oldPolicyRidersList.dataset.bound = "1";
        el.oldPolicyRidersList.addEventListener("click", (e) => {
          const btn = e.target && e.target.closest ? e.target.closest("[data-rm-rider]") : null;
          if(!btn) return;
          const idx = Number(btn.getAttribute("data-rm-rider") || -1);
          if(idx >= 0){
            UI._custRidersDraft.splice(idx, 1);
            renderRidersDraft();
          }
        });
      }

      if(el.oldPoliciesTbody && !el.oldPoliciesTbody.dataset.bound){
        el.oldPoliciesTbody.dataset.bound = "1";
        el.oldPoliciesTbody.addEventListener("click", (e) => {
          const viewBtn = e.target && e.target.closest ? e.target.closest("[data-view-riders]") : null;
          if(viewBtn){
            const idx = Number(viewBtn.getAttribute("data-view-riders") || -1);
            if(idx >= 0) openRidersViewer(idx);
            return;
          }
          const delBtn = e.target && e.target.closest ? e.target.closest("[data-del-policy]") : null;
          if(delBtn){
            const idx = Number(delBtn.getAttribute("data-del-policy") || -1);
            if(idx >= 0){
              UI._custPolicies.splice(idx, 1);
              renderPoliciesTable();
        renderCancelOppTable();
            }
            return;
      ...
      if(el.oldPolicyRidersViewerClose && !el.oldPolicyRidersViewerClose.dataset.bound){
        el.oldPolicyRidersViewerClose.dataset.bound = "1";
        el.oldPolicyRidersViewerClose.addEventListener("click", closeRidersViewer);
      }

      // add policy row
      const addPolicy = () => {
        const company = safeTrim(el.oldPolicyCompany ? el.oldPolicyCompany.value : "");
        const product = safeTrim(el.oldPolicyProduct ? el.oldPolicyProduct.value : "");
        const policyNumber = safeTrim(el.oldPolicyNumber ? el.oldPolicyNumber.value : "");
        const premiumMonthly = Number(el.oldPolicyPremium ? el.oldPolicyPremium.value : 0) || 0;
        const statusLabel = safeTrim(el.oldPolicyStatus ? el.oldPolicyStatus.value : "");

        let status = "active";
        if(statusLabel === "ביטול מלא" || statusLabel === "ביטול חלקי") status = "pending_cancel";
        else if(statusLabel === "מינוי סוכן") status = "swapped";

        if(!company || !product || !policyNumber || !statusLabel){
          notify("חובה למלא חברה, מוצר, מספר פוליסה וסטטוס.", "warn");
          return;
        }

const insuredSum = Number(el.oldPolicyInsuredSum ? el.oldPolicyInsuredSum.value : 0) || 0;
const compSum = Number(el.oldPolicyCompSum ? el.oldPolicyCompSum.value : 0) || 0;
const hasLien = !!(el.oldPolicyHasLien && el.oldPolicyHasLien.checked);

if(isRiskProduct(product) && !insuredSum){
  notify("בריסק / ריסק משכנתא חובה למלא סכום ביטוח בפוליסה.", "warn");
  return;
}
if(needsCompSum(product) && !compSum){
  notify("במחלות קשות / מחלות סרטן חובה למלא סכום פיצוי.", "warn");
  return;
}
if(isRiskProduct(product)){
  const purchased = getBankPurchased();
  if(purchased === "yes"){
    const agency = safeTrim(el.oldPolicyBankAgency ? el.oldPolicyBankAgency.value : "");
    if(!agency){
      notify("בחר סוכנות בנקאית (כי סימנת שהפוליסה נרכשה דרך סוכנות הבנק).", "warn");
      return;
    }
  }
}
let cancelRiders = [];
        if(statusLabel === "ביטול חלקי"){
          cancelRiders = (UI._custRidersDraft||[]).map(x=>safeTrim(x)).filter(Boolean);
          if(!cancelRiders.length){
            notify("בביטול חלקי חובה להוסיף לפחות נספח אחד לביטול.", "warn");
            return;
          }
        }

UI._custPolicies.push({
  id: uid(),
  company,
  product,
  policyNumber,
  ...
  cancelRiders,
  insuredSum: isRiskProduct(product) ? insuredSum : undefined,
  compSum: needsCompSum(product) ? compSum : undefined,
  hasLien: (isRiskProduct(product) ? hasLien : false),
  bankPurchased: (isRiskProduct(product) ? getBankPurchased() : "no"),
  bankAgency: (isRiskProduct(product) && getBankPurchased()==="yes" ? safeTrim(el.oldPolicyBankAgency ? el.oldPolicyBankAgency.value : "") : "")
});

        // reset inputs for next row
        if(el.oldPolicyProduct) el.oldPolicyProduct.value = "";
        if(el.oldPolicyNumber) el.oldPolicyNumber.value = "";
        if(el.oldPolicyPremium) el.oldPolicyPremium.value = "";
        if(el.oldPolicyStatus) el.oldPolicyStatus.value = "";
if(el.oldPolicyInsuredSum) el.oldPolicyInsuredSum.value = "";
if(el.oldPolicyCompSum) el.oldPolicyCompSum.value = "";
if(el.oldPolicyHasLien) el.oldPolicyHasLien.checked = false;
        try{ setBankPurchased("no"); }catch(_){}
        if(el.oldPolicyBankAgency) el.oldPolicyBankAgency.value = "";UI._oldPolicyLienDraft = null;
try{ toggleProductFields(); }catch(_){}
        UI._custRidersDraft = [];
        renderRidersDraft();
        showRiders(false);
      ...
      if(el.oldPolicyAddBtn && !el.oldPolicyAddBtn.dataset.bound){
        el.oldPolicyAddBtn.dataset.bound = "1";
        el.oldPolicyAddBtn.addEventListener("click", addPolicy);
      }
      ...
      // expose helpers for modal open/reset calls
      UI._renderPoliciesTable = renderPoliciesTable;
      UI._renderRidersDraft = renderRidersDraft;
      UI._closeRidersViewer = closeRidersViewer;
      UI._showRidersBox = showRiders;
    }catch(_){}
  }
...
function setupCustomerFormUX(UI){
    try{
      ...
      const important = [
        ...
        "hmo","supplemental","idIssueDate"
      ];

      // Step 4: Existing policies UI
      setupExistingPoliciesStep(UI);
      ...
      function updateProgress(){
        ...
        if(txt){
          const cur = getStep();
          const curTab = tabs.find(b => String(b.dataset.step) === String(cur));
          const name = curTab ? (curTab.textContent || "").trim() : ("שלב " + cur);
          txt.textContent = name + " • השלמה: " + pct + "%";
        }
      }
      ...
      function go(step){
        ...
        // ensure active step is visible inside the modal scroll container
        try{
          const scrollEl = form.querySelector("#custSteps");
          const sec = steps.find(s => s.classList.contains("is-active"));
          if(scrollEl && sec){
            const top = sec.offsetTop - scrollEl.offsetTop;
      ...
      // events
      // tabs are indicators only (navigation via next/prev buttons)
      tabs.forEach(btn => {
        on(btn, "click", (e) => {
