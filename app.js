...
(() => {
  ...
  const BUILD = "20260326-ceo-dashboard-v1";
  ...
  const isLockedCeoUser = (user) => {
    ...
    return id === CEO_AGENT_ID || role === 'ceo' || name === CEO_DEFAULT_USER.name || username === CEO_DEFAULT_USER.username;
  };
  ...
  const Auth = {
    ...
    canViewAllCustomers(){
      return this.isAdmin() || this.isCeo() || this.isManager() || this.isOps();
    },
...
// nav
      $$(".nav__item").forEach(btn => {
        on(btn, "click", () => {
          ...
          if(!v) return;
          if(v === "settings" && !(Auth.isAdmin() || Auth.isCeo())) return;
...
this.applyRoleUI();
    ...
    goView(view){
      ...
      if(safe === "settings" && !(Auth.isAdmin() || Auth.isCeo())) safe = "dashboard";
      ...
      document.body.classList.toggle('ceo-dashboard-mode', safe === 'dashboard' && Auth.isCeo());
    ...
    renderAuthPill(){
      ...
      if(Auth.current) {
        pill.style.display = "";
txt.textContent = Auth.current.name + (Auth.isAdmin() ? ' (מנהל מערכת)' : Auth.isCeo() ? ' (מנכ"ל)' : Auth.isManager() ? ' (מנהל)' : Auth.isOps() ? ' (תפעול)' : '');
      } else {
...
txt.textContent = "";
  ...
  const UsersUI = {
    ...
    _filtered(){
      ...
      if(!Auth.isCeo()) arr = arr.filter(a => !isLockedCeoUser(a));
    ...
    render(){
      ...
      UI.els.usersTbody.innerHTML = rows.map(a => {
        ...
        const actions = isCeo
          ? (Auth.isCeo() ? `<button class="btn" data-act="edit" data-id="${escapeHtml(a.id)}">ערוך</button>` : '')
    ...
    async editUser(id){
      const a = (State.data.agents || []).find(x => String(x.id) === String(id));
      if(!a) return;
      if(isLockedCeoUser(a) && !Auth.isCeo()) return;
...
const SystemRepairUI = {
  ...
  const DashboardUI = {
    ...
    render(){
      ...
      const roleText = Auth.isCeo() ? 'מנהל מערכת (מנכ"ל)' : Auth.isAdmin() ? 'מנהל מערכת' : Auth.isManager() ? 'מנהל' : Auth.isOps() ? 'תפעול' : 'נציג';
      root.innerHTML = `
  ...
  const App = {
    ...
    async reloadSessionState(){
      ...
      if (r.ok) {
        ...
        if (Auth.isAdmin() || Auth.isCeo()) UsersUI.render();
        if (Auth.current) {
          DashboardUI.render();
          DashboardUI.render();
          CustomersUI.render();
