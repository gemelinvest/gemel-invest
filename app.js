...
const toggleProductFields = () => {
  ...
  if(!showRisk){
    ...
    if(el.oldPolicyHasLien) el.oldPolicyHasLien.checked = false;    UI._oldPolicyLienDraft = null;
  }else{
...
if(el.oldPolicyHasLien && !el.oldPolicyHasLien.dataset.bound){
  ...
  el.oldPolicyHasLien.addEventListener("change", () => {
    // lien is a simple checkbox (no modal)
    if(!el.oldPolicyHasLien.checked) UI._oldPolicyLienDraft = null;
  });
}
