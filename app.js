...
(() => {
  ...
  const BUILD = "20260324-operational-report-pdf-center-fit-v4";
...
// ---------- Step 6 ----------
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const style = `<style>
        ...
        .lcPdfPage{width:794px;min-height:1122px;margin:0 auto 24px;background:#ffffff;padding:34px 36px 30px;box-shadow:0 18px 46px rgba(15,23,42,.12);position:relative}
    ...
    async exportOperationalPdf(payloadOverride, sourceBtn){
      ...
      try{
        ...
        const reportBody = this.els?.reportBody || document.getElementById('lcReportBody');
        const liveDoc = reportBody?.firstElementChild || reportBody;
        if(!liveDoc) throw new Error('Operational report DOM was not found');

        const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
        const filename = this.getOperationalPdfFileName(payload);
        const clonedDoc = liveDoc.cloneNode(true);

        const A4_WIDTH_PX = 794;
        const A4_HEIGHT_PX = 1123;
        const PDF_MARGIN_PX = 18;
        const exportOuterWidth = A4_WIDTH_PX;
        const exportPageWidth = A4_WIDTH_PX - (PDF_MARGIN_PX * 2);
        ...
        host.style.width = exportOuterWidth + 'px';
        host.style.minWidth = exportOuterWidth + 'px';
        host.style.maxWidth = exportOuterWidth + 'px';
        ...
        exportFrame.style.width = exportOuterWidth + 'px';
        exportFrame.style.minWidth = exportOuterWidth + 'px';
        exportFrame.style.maxWidth = exportOuterWidth + 'px';
        exportFrame.style.minHeight = A4_HEIGHT_PX + 'px';
        exportFrame.style.margin = '0 auto';
        exportFrame.style.padding = PDF_MARGIN_PX + 'px';
        ...
        exportFrame.style.alignItems = 'flex-start';
        exportFrame.style.overflow = 'hidden';

        clonedDoc.querySelectorAll('button, .iconBtn, [data-close="1"]').forEach((el) => el.remove());
        clonedDoc.classList.add('lcPdfBody');
        clonedDoc.style.margin = '0 auto';
        clonedDoc.style.width = exportPageWidth + 'px';
        clonedDoc.style.minWidth = exportPageWidth + 'px';
        clonedDoc.style.maxWidth = exportPageWidth + 'px';
        clonedDoc.style.background = '#ffffff';
        clonedDoc.style.boxSizing = 'border-box';
        clonedDoc.style.transform = 'none';
        clonedDoc.style.transformOrigin = 'top center';
        clonedDoc.style.overflow = 'visible';

        exportFrame.appendChild(clonedDoc);
        ...
        const naturalWidth = Math.max(
          Math.ceil(clonedDoc.scrollWidth || 0),
          Math.ceil(clonedDoc.getBoundingClientRect().width || 0),
          exportPageWidth
        );
        const fitScale = Math.min(1, exportPageWidth / naturalWidth);
        if(fitScale < 0.999){
          clonedDoc.style.width = naturalWidth + 'px';
          clonedDoc.style.minWidth = naturalWidth + 'px';
          clonedDoc.style.maxWidth = naturalWidth + 'px';
          clonedDoc.style.transform = `scale(${fitScale})`;
          const scaledHeight = Math.ceil((clonedDoc.scrollHeight || clonedDoc.offsetHeight || A4_HEIGHT_PX) * fitScale);
          exportFrame.style.minHeight = Math.max(A4_HEIGHT_PX, scaledHeight + (PDF_MARGIN_PX * 2)) + 'px';
        }else{
          exportFrame.style.minHeight = Math.max(A4_HEIGHT_PX, (clonedDoc.scrollHeight || clonedDoc.offsetHeight || 0) + (PDF_MARGIN_PX * 2)) + 'px';
        }

        await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));

        const target = exportFrame;
