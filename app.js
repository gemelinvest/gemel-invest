...
// Set your GOFI module URL (GitHub Pages) here when ready
  ...
  // ---------- Boot: Local Auth (Username + Password) ----------
  // Login UI is a centered cream frosted-glass card (index.html). Users are stored locally (LS)
  // and can be moved to Google Sheets later using the existing admin Settings module.

  const SESSION_LS_KEY = 'gemel_session_v1';
  ...
  function saveSession_(safeUser){
    try{
  ...
  function setCurrentUser_(safe){
    state.currentUser = safe;
    state.agentName = safe.displayName || safe.username || 'אורח';
    try{ applyPermissions_(); }catch(_){}
    try{ updateMailBadge_(); }catch(_){}
  }

  function findUserByUsername_(username){
    const users = loadUsers_();
    return (users||[]).find(u => (u.active !== false) && (u.username === username));
  }
  ...
  function authenticate_(username, password){
    ...
    const u = (users||[]).find(x =>
      (x.active !== false) &&
      String(x.username||'') === String(username||'').trim() &&
      String(x.password||'') === String(password||'')
    );
    return u ? safeUser_(u) : null;
  }
  ...
  function bindLoginUi_(){
    ...
    if(logoutBtn) logoutBtn.style.display = ''; // visible when using local auth
    if(logoutBtn){
    ...
    if(form){
      form.addEventListener('submit', (e) => {
        ...
        if(!u || !p){
          toast('חסר', 'נא למלא שם משתמש וסיסמה');
          return;
        }
        const safe = authenticate_(u, p);
        if(!safe){
          toast('שגיאה', 'שם משתמש או סיסמה שגויים');
          return;
        }
        setCurrentUser_(safe);
        saveSession_(safe);
        showApp_();
        const r = location.hash ? location.hash.replace('#','').trim() : '';
        setRoute(r || 'customers');
        toast('ברוך הבא', 'התחברת בהצלחה');
      });
    }
  }

  (function bootAuthLocal_(){
    bindLoginUi_();

    const sess = loadSession_();
    if(sess && sess.username){
      const u = findUserByUsername_(sess.username);
      if(u){
        setCurrentUser_(safeUser_(u));
        showApp_();
        const r = location.hash ? location.hash.replace('#','').trim() : '';
        setRoute(r || 'customers');
        return;
