...
(() => {
  ...
  const BUILD = "20260324-operational-report-pdf-hide-system-actions-1";
...
// ---------- Step 6 ----------
    ...
    buildOperationalPdfMarkup(payloadOverride){
      ...
      const reportHtml = this.sanitizeOperationalReportHtmlForPdf(this.renderOperationalReport(payload));
      return {
        payload,
        html: `<div class="lcPdfSheet">
          <div class="lcPdfHeader">
            <div class="lcPdfHeader__brand">
              <img src="./logo-login-clean.png" alt="GEMEL INVEST" class="lcPdfHeader__logo">
              <div>
                <div class="lcPdfHeader__kicker">GEMEL INVEST</div>
                <div class="lcPdfHeader__title">דוח תפעולי מלא</div>
                <div class="lcPdfHeader__sub">מסמך PDF נקי ומסודר לייצוא, כולל שיקוף שיעבודי פוליסה כאשר קיימים.</div>
              </div>
            </div>
            <div class="lcPdfHeader__meta">
              <div><span>לקוח</span><strong>${escapeHtml(customerName)}</strong></div>
              <div><span>תעודת זהות</span><strong>${escapeHtml(customerId)}</strong></div>
              <div><span>תאריך ייצוא</span><strong>${escapeHtml(exportedAt)}</strong></div>
            </div>
          </div>
          <div class="lcPdfBody">${reportHtml}</div>
        </div>`
      };
    },
