...
(() => {
  ...
  const BUILD = "20260323-investo-chat-midnight-cleanup-1";
...
// 🔥 Override coverage button (Wizard)
document.addEventListener("click", function(e){
  const btn = e.target.closest("button");
  if(!btn) return;

  if(btn.textContent && btn.textContent.includes("אישור כיסויים")){
    e.preventDefault();
    e.stopPropagation();
    if(typeof openCoverageModal === "function"){
      openCoverageModal();
    }
  }
});
...
"ניתוחים ומחליפי ניתוח בישראל (שב\"ן עם השתתפות עצמית 5,000 ₪)",
...
function openCoverageModal(){
  document.getElementById("coverageModal").classList.add("is-open");
  renderCoverages();
}

function closeCoverageModal(){
  document.getElementById("coverageModal").classList.remove("is-open");
}

function renderCoverages(){
  const list=document.getElementById("coverageList");
  list.innerHTML="";
  COVERAGES.forEach(name=>{
    const el=document.createElement("div");
    el.className="coverageCard";
    el.textContent=name;
    if(selectedCoverages.includes(name)) el.classList.add("is-selected");
    el.onclick=()=>{
      if(selectedCoverages.includes(name)){
        selectedCoverages=selectedCoverages.filter(x=>x!==name);
      }else{
        selectedCoverages.push(name);
      }
      renderCoverages();
    };
    list.appendChild(el);
  });
  document.getElementById("coverageCount").textContent=
    selectedCoverages.length?`נבחרו ${selectedCoverages.length} כיסויים`:"לא נבחרו כיסויים";
}

// 🔥 override old drawer
if(window.Wizard){
  Wizard.openCoversDrawer = function(){
    openCoverageModal();
  };
}
...
function saveSelectedCoverages(){
  try{
    const payload = Wizard?.state?.payload || {};
    if(!payload.newPolicies || !payload.newPolicies.length){
      console.warn("אין פוליסה לשמירה");
      return;
    }
    const lastPolicy = payload.newPolicies[payload.newPolicies.length - 1];
    lastPolicy.coverages = [...(window.selectedCoverages||[])];
    console.log("כיסויים נשמרו:", lastPolicy.coverages);
  }catch(err){
    console.error("שגיאה בשמירת כיסויים:", err);
  }
}

document.addEventListener("click", function(e){
  const btn = e.target.closest("#coverageConfirmBtn");
  if(!btn) return;
  ...
  if(typeof closeCoverageModal==="function"){
    closeCoverageModal();
  }
});
