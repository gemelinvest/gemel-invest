...
(() => {
  ...
  const toNum = (v) => { const n = Number(String(v ?? "").replace(/[^\d.-]/g, "")); return Number.isFinite(n) ? n : 0; };
  const fmtMoney = (v) => { const n = toNum(v); return "₪" + n.toLocaleString("he-IL"); };
  const pick = (obj, path, def="") => {
    try{ return String(path.split('.').reduce((a,k)=>a?.[k], obj) ?? def); }catch(_e){ return String(def); }
  };
  ...
  function normalizeState(s){
    ...
    const out = {
      ...
      customers: Array.isArray(s?.customers) ? s.customers : (base.customers || [])
    };
...
this.els.syncDot = $("#syncDot");
      ...
      on($("#lcFinishClose"), "click", () => Wizard.closeFinish());
      on($("#lcFinishOpenCustomer"), "click", () => Wizard.openCustomerFileFromFinish());
      on($("#lcFinishDownloadOps"), "click", () => Wizard.downloadOperationalReportFromFinish());
      on($("#lcFinishTransferOps"), "click", () => Wizard.transferToOpsFromFinish());
      on($("#customerFullClose"), "click", () => Wizard.closeCustomerFile());
      on($("#customerFullDownloadOps"), "click", () => Wizard.downloadOperationalReport());
      on($("#customerFullTransferOps"), "click", () => Wizard.transferToOps());
// nav
...
init(){
    ...
    getCaseMonthlyTotals(){
      const existing = (this.insureds || []).reduce((sum, ins) => sum + ((ins.data?.existingPolicies || []).reduce((s,p)=> s + toNum(p.monthlyPremium), 0)), 0);
      const fresh = (this.newPolicies || []).reduce((sum, p) => sum + toNum(p.monthlyPremium), 0);
      return { existing, fresh, total: existing + fresh };
    },

    getCaseStatus(){
      const hasHealth = (this.insureds || []).some(ins => {
        const store = ins.data?.healthDeclaration?.categories || {};
        return Object.values(store).some(cat => Object.values(cat || {}).some(st => st?.answer === 'yes'));
      });
      return hasHealth ? 'הושלם • דורש תפעול' : 'הושלם';
    },

    buildCustomerRecord(){
      const primary = this.insureds[0] || { data:{} };
      const d = primary.data || {};
      const totals = this.getCaseMonthlyTotals();
      const insureds = (this.insureds || []).map(ins => ({
        id: ins.id,
        type: ins.type,
        label: ins.label,
        firstName: safeTrim(ins.data?.firstName),
        lastName: safeTrim(ins.data?.lastName),
        fullName: (safeTrim(ins.data?.firstName) + ' ' + safeTrim(ins.data?.lastName)).trim() || ins.label,
        idNumber: safeTrim(ins.data?.idNumber),
        birthDate: safeTrim(ins.data?.birthDate),
        gender: safeTrim(ins.data?.gender),
        existingPolicies: Array.isArray(ins.data?.existingPolicies) ? ins.data.existingPolicies : [],
        healthDeclaration: ins.data?.healthDeclaration || { categories:{} }
      }));
      return {
        customerId: uid('cust'),
        createdAt: nowISO(),
        ...
        status: 'הושלם',
        statusOps: 'טיוטה',
        agentName: Auth.current?.name || '',
        customer: {
          firstName: safeTrim(d.firstName),
          lastName: safeTrim(d.lastName),
          fullName: (safeTrim(d.firstName) + ' ' + safeTrim(d.lastName)).trim(),
        ...
        insureds,
        existingPolicies: insureds.flatMap(x => (x.existingPolicies || []).map(p => ({ ...p, insuredLabel: x.label, insuredName: x.fullName }))),
        newPolicies: Array.isArray(this.newPolicies) ? this.newPolicies.map(p => ({ ...p })) : [],
        payer: { ...(primary.data || {}) },
        totals
      };
    },
    ...
    async finishWizard(){
      try{
        const record = this.buildCustomerRecord();
        const list = Array.isArray(State.data.customers) ? State.data.customers : (State.data.customers = []);
        list.unshift({ id: record.customerId, createdAt: record.createdAt, updatedAt: record.updatedAt, status: record.status, agentName: record.agentName, data: record });
        State.data.meta.updatedAt = nowISO();
        this._lastFinishedCustomerId = record.customerId;
        await App.persist('לקוח נשמר');
        this.openFinish(record.customerId);
      }catch(e){
        console.error('finishWizard failed', e);
        this.setHint('שגיאה בסיום ההקמה');
      }
    },

    getCustomerById(id){
      return (State.data.customers || []).find(x => String(x.id) === String(id) || String(x.data?.customerId) === String(id)) || null;
    },

    openFinish(customerId){
      const rec = this.getCustomerById(customerId);
      if(!rec || !UI.els.finish) return;
      this._lastFinishedCustomerId = rec.id;
      const data = rec.data || {};
      const totals = data.totals || { existing:0, fresh:0, total:0 };
      const el = UI.els.finish;
      const name = safeTrim(data.customer?.fullName) || 'לקוח חדש';
      const customerText = `${escapeHtml(name)} · ${escapeHtml(data.customer?.idNumber || '')}`;
      const html = `
        <div class="lcFinish__hero">
          <div>
            <div class="lcFinish__title">הקמת הלקוח הושלמה</div>
            <div class="lcFinish__sub">התיק נשמר ומוכן להמשך עבודה, פתיחת תיק לקוח או הפקת דוח תפעולי.</div>
          </div>
          <div class="lcFinish__badge">✓ הושלם</div>
        </div>
        <div class="lcFinish__chips">
          <div class="lcFinish__chip"><span>לקוח</span><b>${customerText}</b></div>
          <div class="lcFinish__chip"><span>סה״כ פרמיה חודשית</span><b>${fmtMoney(totals.total)}</b></div>
          <div class="lcFinish__chip"><span>מבוטחים בתיק</span><b>${(data.insureds || []).length}</b></div>
        </div>
      `;
      const body = $('#lcFinishBody'); if(body) body.innerHTML = html;
      el.classList.add('is-open');
      el.setAttribute('aria-hidden','false');
    },

    closeFinish(){
      const el = UI.els.finish; if(!el) return;
      el.classList.remove('is-open');
      el.setAttribute('aria-hidden','true');
      this.close();
    },

    openCustomerFileFromFinish(){
      const id = this._lastFinishedCustomerId;
      this.closeFinish();
      if(id) this.openCustomerFile(id);
    },

    renderHealthSummaryLines(rec){
      const out = [];
      (rec.data?.insureds || []).forEach(ins => {
        const cats = ins.healthDeclaration?.categories || {};
        Object.entries(cats).forEach(([catKey, cat]) => {
          Object.entries(cat || {}).forEach(([qKey, st]) => {
            if(st?.answer === 'yes'){
              out.push({ insured: ins.fullName || ins.label, catKey, qKey, fields: st.fields || {} });
            }
          });
        });
      });
    ...
    openCustomerFile(customerId){
      const rec = this.getCustomerById(customerId);
      if(!rec || !UI.els.customerFull) return;
      this._lastFinishedCustomerId = rec.id;
      const data = rec.data || {};
      const totals = data.totals || { existing:0, fresh:0, total:0 };
      const insuredCards = (data.insureds || []).map(ins => `
        <div class="cfGroupCard">
          <div class="cfGroupCard__title">${escapeHtml(ins.label)} · ${escapeHtml(ins.fullName || '')}</div>
          <div class="cfGroupRows">
            <div class="cfRow"><div class="cfRow__k">ת״ז</div><div class="cfRow__v">${escapeHtml(ins.idNumber || '—')}</div></div>
            <div class="cfRow"><div class="cfRow__k">תאריך לידה</div><div class="cfRow__v">${escapeHtml(ins.birthDate || '—')}</div></div>
            <div class="cfRow"><div class="cfRow__k">פוליסות קיימות</div><div class="cfRow__v">${(ins.existingPolicies || []).length}</div></div>
          </div>
        </div>`).join('');
      const existingRows = (data.existingPolicies || []).length ? (data.existingPolicies || []).map(p => `
        <tr><td>${escapeHtml(p.insuredLabel || '')}</td><td>${escapeHtml(p.company || '')}</td><td>${escapeHtml(p.type || '')}</td><td>${fmtMoney(p.monthlyPremium)}</td></tr>`).join('') : '<tr><td colspan="4">אין פוליסות קיימות</td></tr>';
      const newRows = (data.newPolicies || []).length ? (data.newPolicies || []).map(p => `
        <tr><td>${escapeHtml(p.company || '')}</td><td>${escapeHtml(p.type || '')}</td><td>${escapeHtml(p.startDate || '')}</td><td>${fmtMoney(p.monthlyPremium)}</td></tr>`).join('') : '<tr><td colspan="4">אין פוליסות חדשות</td></tr>';
      const healthLines = this.renderHealthSummaryLines(rec).slice(0, 12).map(x => `<div class="cfRow"><div class="cfRow__k">${escapeHtml(x.insured)}</div><div class="cfRow__v">סומן ממצא רפואי</div></div>`).join('') || '<div class="cfRow"><div class="cfRow__k">הצהרת בריאות</div><div class="cfRow__v">ללא ממצאים שמורים</div></div>';
      const body = $('#customerFullBody');
      if(body) body.innerHTML = `
        <div class="cfHeaderPremium card">
          <div class="cfHeaderPremium__head">
            <div>
              <div class="cfHeaderPremium__title">תיק לקוח 360°</div>
              <div class="cfHeaderPremium__sub">תמונה מלאה של הלקוח, המבוטחים, הפוליסות והנתונים התפעוליים.</div>
            </div>
            <div class="cfHeaderPremium__chips">
              <div class="cfChip"><span class="cfChip__icon">💳</span><span>פרמיה חודשית</span><span class="cfChip__val">${fmtMoney(totals.total)}</span></div>
              <div class="cfChip"><span class="cfChip__icon">🗂️</span><span>סטטוס</span><span class="cfChip__val">${escapeHtml(rec.status || 'הושלם')}</span></div>
              <div class="cfChip"><span class="cfChip__icon">👥</span><span>מבוטחים</span><span class="cfChip__val">${(data.insureds || []).length}</span></div>
            </div>
          </div>
        </div>
        ...
        <div class="cfGroups">
          <div class="cfGroupCard">
            <div class="cfGroupCard__title">פרטי לקוח</div>
            <div class="cfGroupRows">
              <div class="cfRow"><div class="cfRow__k">שם מלא</div><div class="cfRow__v">${escapeHtml(data.customer?.fullName || '—')}</div></div>
              <div class="cfRow"><div class="cfRow__k">ת״ז</div><div class="cfRow__v">${escapeHtml(data.customer?.idNumber || '—')}</div></div>
              <div class="cfRow"><div class="cfRow__k">טלפון</div><div class="cfRow__v">${escapeHtml(data.customer?.phone || '—')}</div></div>
              <div class="cfRow"><div class="cfRow__k">מייל</div><div class="cfRow__v">${escapeHtml(data.customer?.email || '—')}</div></div>
              <div class="cfRow"><div class="cfRow__k">עיסוק</div><div class="cfRow__v">${escapeHtml(data.customer?.occupation || '—')}</div></div>
            </div>
          </div>
          <div class="cfGroupCard">
            <div class="cfGroupCard__title">סיכום פרמיה</div>
            <div class="cfGroupRows">
              <div class="cfRow"><div class="cfRow__k">קיימות</div><div class="cfRow__v">${fmtMoney(totals.existing)}</div></div>
              <div class="cfRow"><div class="cfRow__k">חדשות</div><div class="cfRow__v">${fmtMoney(totals.fresh)}</div></div>
              <div class="cfRow"><div class="cfRow__k">סה״כ חודשי</div><div class="cfRow__v">${fmtMoney(totals.total)}</div></div>
            </div>
          </div>
          <div class="cfGroupCard">
            <div class="cfGroupCard__title">תקציר בריאות</div>
            <div class="cfGroupRows">${healthLines}</div>
          </div>
        </div>

        <div class="cfGroups" style="grid-template-columns: repeat(2, minmax(0, 1fr));">
          <div class="cfGroupCard">
            <div class="cfGroupCard__title">מבוטחים בתיק</div>
            <div class="cfGroups" style="grid-template-columns:1fr; margin-top:0">${insuredCards}</div>
          </div>
          <div class="cfGroupCard">
            <div class="cfGroupCard__title">פוליסות קיימות</div>
            <div class="tableWrap" style="padding:0"><table class="table"><thead><tr><th>מבוטח</th><th>חברה</th><th>מוצר</th><th>פרמיה</th></tr></thead><tbody>${existingRows}</tbody></table></div>
          </div>
        </div>

        <div class="cfGroups" style="grid-template-columns: 1fr;">
          <div class="cfGroupCard">
            <div class="cfGroupCard__title">פוליסות חדשות שבוצעו</div>
            <div class="tableWrap" style="padding:0"><table class="table"><thead><tr><th>חברה</th><th>מוצר</th><th>תאריך התחלה</th><th>פרמיה</th></tr></thead><tbody>${newRows}</tbody></table></div>
          </div>
        </div>
      `;
      const title = $('#customerFullName'); if(title) title.textContent = data.customer?.fullName || 'תיק לקוח';
      const meta = $('#customerFullMeta'); if(meta) meta.innerHTML = `<span>ת״ז: ${escapeHtml(data.customer?.idNumber || '—')}</span><span>טלפון: ${escapeHtml(data.customer?.phone || '—')}</span><span>סוכן: ${escapeHtml(rec.agentName || '—')}</span>`;
      UI.els.customerFull.classList.add('is-open');
      UI.els.customerFull.setAttribute('aria-hidden','false');
    },

    closeCustomerFile(){
      const el = UI.els.customerFull; if(!el) return;
      el.classList.remove('is-open');
      el.setAttribute('aria-hidden','true');
    },

    _buildOperationalReportHtml(rec){
      const data = rec.data || {};
      const totals = data.totals || { existing:0, fresh:0, total:0 };
      const existingRows = (data.existingPolicies || []).map(p => `<tr><td>${escapeHtml(p.insuredLabel || '')}</td><td>${escapeHtml(p.company || '')}</td><td>${escapeHtml(p.type || '')}</td><td>${fmtMoney(p.monthlyPremium)}</td></tr>`).join('') || '<tr><td colspan="4">אין</td></tr>';
      const newRows = (data.newPolicies || []).map(p => `<tr><td>${escapeHtml(p.company || '')}</td><td>${escapeHtml(p.type || '')}</td><td>${escapeHtml(p.startDate || '')}</td><td>${fmtMoney(p.monthlyPremium)}</td></tr>`).join('') || '<tr><td colspan="4">אין</td></tr>';
      const insuredRows = (data.insureds || []).map(ins => `<tr><td>${escapeHtml(ins.label)}</td><td>${escapeHtml(ins.fullName || '')}</td><td>${escapeHtml(ins.idNumber || '')}</td><td>${escapeHtml(ins.birthDate || '')}</td></tr>`).join('');
      return `<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><title>דוח תפעולי - ${escapeHtml(data.customer?.fullName || '')}</title><style>body{font-family:Arial,sans-serif;background:#f6f8fd;color:#10203a;margin:0;padding:24px} .wrap{max-width:1000px;margin:0 auto;background:#fff;border:1px solid #d8e1f2;border-radius:18px;overflow:hidden} .head{padding:22px 24px;background:linear-gradient(135deg,#1b2d6b,#365dcb);color:#fff}.logo{font-size:28px;font-weight:900}.sub{opacity:.88;margin-top:6px}.sec{padding:18px 24px;border-top:1px solid #eef2f8}.title{font-weight:900;font-size:18px;color:#21438e;margin-bottom:12px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.card{border:1px solid #e4ebf7;border-radius:14px;padding:12px;background:#fbfcff}.k{font-size:12px;color:#5b6f97;font-weight:700}.v{font-weight:900;margin-top:4px}table{width:100%;border-collapse:collapse}th,td{padding:10px;border-bottom:1px solid #edf1f8;text-align:right;font-size:13px}th{font-size:12px;color:#4d6496}.sum{display:flex;gap:12px;flex-wrap:wrap}.chip{padding:10px 12px;border-radius:999px;background:#edf3ff;border:1px solid #d6e3ff;font-weight:800}@media print{body{background:#fff;padding:0}.wrap{border:none;border-radius:0}}</style></head><body><div class="wrap"><div class="head"><div class="logo">GEMEL INVEST</div><div class="sub">דוח תפעולי למחלקת תפעול</div></div><div class="sec"><div class="title">פרטי לקוח</div><div class="grid"><div class="card"><div class="k">שם מלא</div><div class="v">${escapeHtml(data.customer?.fullName || '—')}</div></div><div class="card"><div class="k">ת״ז</div><div class="v">${escapeHtml(data.customer?.idNumber || '—')}</div></div><div class="card"><div class="k">טלפון</div><div class="v">${escapeHtml(data.customer?.phone || '—')}</div></div><div class="card"><div class="k">סוכן מטפל</div><div class="v">${escapeHtml(rec.agentName || '—')}</div></div></div></div><div class="sec"><div class="title">מבוטחים בתיק</div><table><thead><tr><th>סוג</th><th>שם</th><th>ת״ז</th><th>תאריך לידה</th></tr></thead><tbody>${insuredRows}</tbody></table></div><div class="sec"><div class="title">פוליסות קיימות</div><table><thead><tr><th>מבוטח</th><th>חברה</th><th>מוצר</th><th>פרמיה</th></tr></thead><tbody>${existingRows}</tbody></table></div><div class="sec"><div class="title">פוליסות חדשות</div><table><thead><tr><th>חברה</th><th>מוצר</th><th>תאריך התחלה</th><th>פרמיה</th></tr></thead><tbody>${newRows}</tbody></table></div><div class="sec"><div class="title">סיכום פרמיה חודשית</div><div class="sum"><div class="chip">קיימות: ${fmtMoney(totals.existing)}</div><div class="chip">חדשות: ${fmtMoney(totals.fresh)}</div><div class="chip">סה״כ: ${fmtMoney(totals.total)}</div></div></div></div></body></html>`;
    },

    downloadOperationalReport(){
      const rec = this.getCustomerById(this._lastFinishedCustomerId);
      if(!rec) return;
      const html = this._buildOperationalReportHtml(rec);
      const blob = new Blob([html], { type:'text/html;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `operational-report-${safeTrim(rec.data?.customer?.idNumber || rec.id)}.html`;
      document.body.appendChild(a); a.click(); a.remove();
      setTimeout(()=>URL.revokeObjectURL(url), 1500);
    },

    downloadOperationalReportFromFinish(){ this.downloadOperationalReport(); },

    async transferToOps(){
      const rec = this.getCustomerById(this._lastFinishedCustomerId);
      if(!rec) return;
      rec.status = 'ממתין לתפעול';
      rec.data.status = 'ממתין לתפעול';
      rec.updatedAt = nowISO();
      await App.persist('הועבר לתפעול');
      this.openCustomerFile(rec.id);
    },

    async transferToOpsFromFinish(){
      const id = this._lastFinishedCustomerId;
      await this.transferToOps();
      this.closeFinish();
      if(id) this.openCustomerFile(id);
    },
...
// ---------- Step 6 ----------
    ...
    // ---------- Step 8 ----------
    getHealthCompanies(){
      const supported = new Set(["כלל","הפניקס","הכשרה"]);
      const found = new Set();
    ...
    getHealthStore(){
      ...
      if(!primary.data.healthDeclaration) primary.data.healthDeclaration = { categories:{} };
      if(!primary.data.healthDeclaration.categories) primary.data.healthDeclaration.categories = {};
      return primary.data.healthDeclaration;
    },
      ...
      return [
        {
          ...
          companies:["כלל"],
          questions:[
            { key:"family_history", text:"האם קיימת היסטוריה משפחתית מדרגה ראשונה של סרטן / לב / סוכרת / כליות / מחלות נוירולוגיות?", companies:["כלל"], fields:[
              { key:"details", label:"פירוט", type:"textarea" },
              { key:"familyCount", label:"כמה קרובי משפחה", type:"text" }
            ] }
          ]
        },
        {
          ...
          companies:["כלל","הכשרה"],
          questions:[
            { key:"smoking", text:"האם המועמד מעשן או הפסיק לעשן בשנתיים האחרונות?", companies:["כלל"], fields:[
              { key:"cigsPerDay", label:"כמות סיגריות ליום", type:"text" }
            ] },
            { key:"drug_use", text:"האם קיים שימוש בסמים כיום או בעבר?", companies:["כלל","הכשרה"], fields:[
              { key:"reason", label:"סיבה לשימוש", type:"text" },
              { key:"startDate", label:"מועד תחילת השימוש", type:"text" },
              { key:"type", label:"סוג / שם הסם", type:"text" },
              { key:"frequency", label:"תדירות השימוש", type:"text" },
              { key:"rehab", label:"האם עברת גמילה? פרט", type:"textarea" }
            ] }
          ]
        },
        {
          ...
          companies:["כלל"],
          questions:[
            { key:"nervous_main", text:"האם אובחנה מחלה / הפרעה במערכת העצבים, המוח או עיכוב התפתחותי?", companies:["כלל"], fields:[
              { key:"diagnosis", label:"אבחנה", type:"text" },
              { key:"lastEvent", label:"מועד אירוע / אבחון אחרון", type:"text" },
              { key:"treatment", label:"טיפול קיים", type:"textarea" }
            ] }
          ]
        },
        {
          key:"mental",
          title:"בריאות נפש",
          companies:["כלל","הפניקס"],
          questions:[
            { key:"mental_main", text:"האם קיימת בעיה / אבחנה נפשית (חרדה, דיכאון, PTSD, OCD וכו׳)?", companies:["כלל","הפניקס"], fields:[
              { key:"diagnosis", label:"אבחנה", type:"text" },
              { key:"treatment", label:"טיפול / תרופות", type:"textarea" },
              { key:"hospitalization", label:"אשפוז / נכות / ניסיון אובדני", type:"textarea" }
            ] }
          ]
        },
        {
          ...
          companies:["כלל","הפניקס"],
          questions:[
            { key:"resp_main", text:"האם קיימת מחלת נשימה / ריאות (אסטמה, COPD, דום נשימה וכו׳)?", companies:["כלל","הפניקס"], fields:[
              { key:"diagnosis", label:"אבחנה", type:"text" },
              { key:"severity", label:"חומרה / תדירות התקפים", type:"text" },
              { key:"treatment", label:"טיפול", type:"text" },
              { key:"hospitalization", label:"אשפוזים / בדיקות רלוונטיות", type:"textarea" }
            ] }
          ]
        },
        {
          ...
          companies:["כלל"],
          questions:[
            { key:"heart_main", text:"האם קיימת מחלת לב, לחץ דם או בעיה בכלי הדם?", companies:["כלל"], fields:[
              { key:"diagnosis", label:"אבחנה", type:"text" },
              { key:"bp", label:"ערכי לחץ דם / מצב נוכחי", type:"text" },
        ...
        },
        {
          ...
          companies:["כלל","הפניקס"],
          questions:[
            { key:"digestive_main", text:"האם קיימת מחלה במערכת העיכול / כבד / צהבת?", companies:["כלל","הפניקס"], fields:[
              { key:"diagnosis", label:"אבחנה", type:"text" },
              { key:"treatment", label:"טיפול", type:"text" },
        ...
        },
        {
          ...
          companies:["כלל","הפניקס"],
          questions:[
            { key:"msk_main", text:"האם קיימת בעיה בשלד, גב, מפרקים, שבר, פריקה או מחלה ראומטולוגית?", companies:["כלל","הפניקס"], fields:[
              { key:"diagnosis", label:"אבחנה", type:"text" },
              { key:"location", label:"מיקום הפגיעה", type:"text" },
              { key:"surgery", label:"ניתוח / מגבלה / כאבים", type:"textarea" }
            ] }
          ]
        },
        {
          ...
          companies:["כלל"],
          questions:[
            { key:"skin_main", text:"האם קיימת מחלת עור, נגע עור, פסוריאזיס או גידול בעור?", companies:["כלל"], fields:[
              { key:"diagnosis", label:"אבחנה", type:"text" },
              { key:"removed", label:"האם הוסר / בוצעה ביופסיה", type:"text" },
              { key:"details", label:"פירוט", type:"textarea" }
            ] }
          ]
        },
        {
          ...
          companies:["הכשרה"],
          questions:[
            { key:"accident_main", text:"האם הייתה תאונה, שבר, נכות או מגבלה עקב מחלה / תאונה?", companies:["הכשרה"], fields:[
              { key:"eventDate", label:"מתי אירע / אובחן", type:"text" },
              { key:"diagnosis", label:"אבחנה / מהות הפגיעה", type:"textarea" },
              { key:"hospitalization", label:"אשפוז / בדיקות / ניתוח", type:"textarea" },
              { key:"disability", label:"נכות / תביעה / מצב נוכחי", type:"textarea" }
            ] }
          ]
        }
      ];
    ...
    renderHealthField(qKey, field, values){
      const val = safeTrim(values?.[field.key] || "");
      if(field.type === "textarea"){
        return `<div class="field" style="grid-column:1 / -1"><label class="label">${escapeHtml(field.label)}</label><textarea class="input" rows="3" data-healthf="${escapeHtml(qKey)}.${escapeHtml(field.key)}">${escapeHtml(val)}</textarea></div>`;
      }
      return `<div class="field"><label class="label">${escapeHtml(field.label)}</label><input class="input" type="text" value="${escapeHtml(val)}" data-healthf="${escapeHtml(qKey)}.${escapeHtml(field.key)}" /></div>`;
    },

    renderStep8(){
      const companies = this.getHealthCompanies();
      const store = this.getHealthStore();
      const categories = this.getHealthSchema().filter(cat => cat.companies.some(c => companies.includes(c)));
      if(!companies.length){
        return `<div class="lcWSection"><div class="lcWTitle">הצהרת בריאות</div><div class="muted">כדי להציג את שלב 8 יש לבחור בשלב 5 לפחות פוליסה של כלל / הפניקס / הכשרה.</div></div>`;
      }
      const chips = companies.map(c => `<span class="badge">${escapeHtml(c)}</span>`).join(" ");
      const catsHtml = categories.map(cat => {
        const catState = store.categories[cat.key] || {};
        const qs = cat.questions.map(q => {
          const qState = catState[q.key] || { answer:"", fields:{} };
          const companyTags = q.companies.map(c => `<span class="badge">${escapeHtml(c)}</span>`).join(" ");
          const followup = (qState.answer === "yes") ? `<div class="formGrid" style="padding:14px 0 0">${(q.fields || []).map(f => this.renderHealthField(q.key, f, qState.fields || {})).join("")}</div>` : "";
          return `<div class="lcHealthQ">
            <div class="row row--between" style="align-items:flex-start; gap:12px; flex-wrap:wrap;">
              <div>
                <div class="lcWTitle" style="font-size:16px">${escapeHtml(q.text)}</div>
                <div class="small muted" style="margin-top:6px">מקור: ${companyTags}</div>
              </div>
              <div class="segmented">
                <button type="button" class="segmented__btn ${qState.answer === "yes" ? "is-active" : ""}" data-healthq="${escapeHtml(cat.key)}|${escapeHtml(q.key)}|yes">כן</button>
                <button type="button" class="segmented__btn ${qState.answer === "no" ? "is-active" : ""}" data-healthq="${escapeHtml(cat.key)}|${escapeHtml(q.key)}|no">לא</button>
              </div>
            </div>
            ${followup}
          </div>`;
        }).join("");
        return `<div class="lcWSection lcHealthCat">
          <div class="row row--between" style="align-items:center; gap:10px; flex-wrap:wrap;">
            <div class="lcWTitle">${escapeHtml(cat.title)}</div>
            <div>${cat.companies.filter(c => companies.includes(c)).map(c => `<span class="badge">${escapeHtml(c)}</span>`).join(" ")}</div>
          </div>
          <div class="muted small" style="margin-top:4px">ענה כן / לא. במקרה של כן ייפתח שאלון ההמשך הרלוונטי.</div>
          <div class="divider"></div>
          <div style="display:grid; gap:14px">${qs}</div>
        </div>`;
      }).join("");
      return `<div class="lcWSection"><div class="lcWTitle">הצהרת בריאות</div><div class="muted small">חברות שנמצאו בשלב 5: ${chips}</div></div>${catsHtml}`;
    },

    bindHealthInputs(){
      const store = this.getHealthStore();
      $$('[data-healthq]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const raw = btn.getAttribute('data-healthq') || '';
          const [catKey, qKey, ans] = raw.split('|');
          if(!catKey || !qKey || !ans) return;
          store.categories[catKey] = store.categories[catKey] || {};
          const prev = store.categories[catKey][qKey] || { answer:'', fields:{} };
          store.categories[catKey][qKey] = { answer: ans, fields: prev.fields || {} };
          this.render();
        });
      });
      $$('[data-healthf]', this.els.body).forEach(el => {
        const save = () => {
          const raw = el.getAttribute('data-healthf') || '';
          const [qKey, fieldKey] = raw.split('.');
          if(!qKey || !fieldKey) return;
          const schema = this.getHealthSchema();
          let catKey = '';
          for(const cat of schema){ if((cat.questions||[]).some(q => q.key === qKey)){ catKey = cat.key; break; } }
          if(!catKey) return;
          store.categories[catKey] = store.categories[catKey] || {};
          const prev = store.categories[catKey][qKey] || { answer:'yes', fields:{} };
          prev.fields = prev.fields || {};
          prev.fields[fieldKey] = safeTrim(el.value);
          store.categories[catKey][qKey] = prev;
        };
        on(el, 'input', save);
        on(el, 'change', save);
      });
