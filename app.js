...
txt.textContent = "";
  ...
  const Wizard = {
        ...
        <div class="lcWSection">
          ...
          <div class="lcWGrid">
            ...
            <div class="field">
              ...
              <input class="input" value="${escapeHtml(isChild ? inherited("zip") : d.zip)}" placeholder="ימולא אוטומטית בהמשך" ${isChild ? "disabled": ""} data-bind="zip" />
              <div class="help">בשלב הזה המיקוד יישאר ריק (אוטומציה תתווסף בהמשך).</div>
            </div>
        ...
        <div class="lcWSection">
          ...
          <div class="lcWGrid">
            ...
            <div class="field">
              ...
              <input class="input" value="${escapeHtml(bmiTxt)}" disabled />
            </div>
            <div class="field">
              <label class="label">סטטוס</label>
              <div class="lcWRow">
                <span class="lcWBadge"><span class="lcLamp ${st.lamp}"></span>${escapeHtml(st.label || "—")}</span>
                <span class="muted small">${escapeHtml(st.text || "מלא גובה ומשקל כדי לחשב")}</span>
              </div>
            </div>
          </div>
    ...
    bindInputs(ins){
      $$("[data-bind]", this.els.body).forEach(el => {
        ...
        const setVal = () => {
          ...
          if(path === "birthDate"){ this.render(); return; }
          if(path === "heightCm" || path === "weightKg"){ this.calcBmi(ins); this.render(); return; }
    ...
    fieldDate(label, bind, value){
      return `<div class="field">
        <label class="label">${escapeHtml(label)}</label>
        <input class="input" type="date" data-bind="${escapeHtml(bind)}" value="${escapeHtml(value||"")}" />
      </div>`;
    },
