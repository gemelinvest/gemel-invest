...
(() => {
  ...
  const BUILD = "20260413-vProposalSaveFix1";
  ...
  const Storage = {
    ...
    buildStateFromTableCaches(){
      try {
        ...
        const payload = normalizeState({
          meta: mappedMeta,
          agents: Array.isArray(agentsCache?.data) ? agentsCache.data.map((row, idx) => this.mapAgentRow(row, idx)) : [],
    ...
    async saveSheets(state, options = {}){
      try {
        ...
        return { ok:true, at: mergedState.meta.updatedAt || nowISO(), mode:'safe-upsert-only', conflictDetected: !!conflictInfo.detected, mergedState, proposalsSyncWarning };
      } catch(e) {
        return { ok:false, error: String(e?.message || e) };
      }
    },
  ...
  const App = {
    ...
    async persist(label){
      ...
      if (r.ok) {
        ...
        appendAuditLog({
          type: r.conflictDetected ? 'conflict_blocked_merge' : 'persist_success',
          entity: 'state',
          label: safeTrim(label) || 'נשמר',
          actorName: safeTrim(Auth?.current?.name || 'מערכת'),
          details: {
        ...
        try { Storage.saveBackup(State.data); } catch(_) {}
        UI.renderSyncStatus((label || "נשמר") + (r.conflictDetected ? " · מוזגו שינויים מהשרת" : ""), "ok", r.at);
      } else {
        appendAuditLog({
          type: 'persist_error',
          entity: 'state',
