...
(() => {
  ...
  const BUILD = "20260419-vOpsHideMirrorsMyProcesses-v1";
  ...
  function formatOpsDuration(totalSec){
  ...
  function formatOpsDate(v){
    if(!v) return '—';
    const d = new Date(v);
    if(Number.isNaN(+d)) return String(v);
    try{ return d.toLocaleString('he-IL', { day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit' }); }catch(_e){ return String(v); }
  }

  function getStoredMirrorHealthEntries(rec){
    const stepState = rec?.payload?.mirrorFlow?.healthStep || {};
    return Array.isArray(stepState.groups) ? stepState.groups : [];
  }

  function listOpsRelevantCustomers(){
  ...
  function getOpsStatePresentation(rec){
    ...
    if(call?.active && call?.startedAt){
      ...
      timerText = formatOpsDuration(sec);
      timerMeta = `בשיחה החל מ־${formatOpsClock(call.startedAt)}`;
      timerLive = true;
      ...
      timerMeta = `שיחה אחרונה בתאריך ${formatOpsFullDate(call.startedAt)}`;
    }
...
UsersGateUI.init();
    ...
    goView(view){
      ...
      if(safe === "mirrors") safe = "dashboard";
      ...
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-discountSpec-active","view-customers-active","view-proposals-active");
    ...
    renderAuthPill(){
      ...
      if(Auth.current) {
        pill.style.display = "";
        txt.textContent = Auth.current.name + (Auth.current?.role === "owner" ? " (מפתח המערכת)" : Auth.isAdmin() ? " (מנהל מערכת)" : Auth.isManager() ? " (מנהל)" : Auth.isOps() ? " (תפעול)" : "");
  ...
  const CustomersUI = {
    ...
    getMedicalGroups(rec){
      try{
        return getStoredMirrorHealthEntries(rec) || [];
      }catch(_e){}
      return [];
    },
    ...
    renderOperationalReflectionCard(state){
      ...
      const updated = safeTrim(state?.updatedText) ? formatOpsDate(state.updatedText) : '—';
...
init(){
    ...
    getCurrentSteps(){
      return this.isElementaryFlow()
        ...
        : this.steps.filter((step) => Number(step.id) !== 4);
    },
    ...
    render(){
      if(!this.els.wrap) return;
      if(!this.isElementaryFlow() && Number(this.step) === 4) this.step = 3;
      this.renderSteps();
      this.renderTabs();
...
// open health covers drawer (Health only)
    ...
    renderExistingPolicyCancellationControls(policy, cancellation = {}, options = {}){
      ...
      const statusOpts = this.getExistingPolicyCancelOptions().map((o) => `<option value="${escapeHtml(o.v)}"${status===o.v?" selected":""}>${escapeHtml(o.t)}</option>`).join("");
      ...
      const titleText = compact ? 'סטטוס הפוליסה' : 'פעולה על הפוליסה';
      const subText = compact ? 'בחר סטטוס וסיבה רק אם צריך.' : 'בחר כבר עכשיו סטטוס טיפול וסיבת ביטול במידת הצורך.';
      const pledgeBadgeCls = compact ? 'lcWBadge lcWBadge--compact' : 'lcWBadge';

      return `
        <div class="${wrapCls}">
          <div class="lcPolicyActionBox__top">
            <div>
              <div class="lcPolicyActionBox__title">${titleText}</div>
              <div class="lcPolicyActionBox__sub">${subText}</div>
            </div>
          ...
          <div class="lcPolicyActionBox__grid">
            <div class="field">
              <label class="label">סטטוס טיפול</label>
              <select class="input" data-cancel-policy="${escapeHtml(p.id || '')}" data-cancel-key="status">${statusOpts}</select>
            </div>

            <div class="field">
              <label class="label">סיבת ביטול</label>
              <select class="input" data-cancel-policy="${escapeHtml(p.id || '')}" data-cancel-key="reason" ${needReason ? "" : "disabled"}>${reasonOpts}</select>
          ...
          return `
            <tr class="lcPolImportedWrapRow">
              <td colspan="7" class="lcPolImportedWrapCell">
                <div class="lcPolImportedRow lcPolImportedRow--rich">
                  <div class="lcPolImportedRow__shine" aria-hidden="true"></div>
                  <div class="lcPolImportedRow__segment lcPolImportedRow__segment--brand">
                    <div class="lcPolImportedRow__companyBadge">
                      <div class="lcPolImportedRow__companyBadgeInner">
                        ${logo}
                        <span class="lcPolImportedRow__companyName">${escapeHtml(p.company || 'חברה לא ידועה')}</span>
                      </div>
                      <span class="lcPolImportedRow__companySub">${escapeHtml(p.company || 'חברה')}</span>
                    </div>
                  </div>

                  <div class="lcPolImportedRow__segment lcPolImportedRow__segment--summary">
                    <div class="lcPolImportedRow__metaLine">
                      ${safeTrim(p.type) ? `<span class="lcPolImportedRow__inlineMeta"><b>סוג:</b> ${escapeHtml(safeTrim(p.type))}</span>` : ''}
                      ${safeTrim(p.policyNumber) ? `<span class="lcPolImportedRow__inlineMeta"><b>מס׳:</b> ${escapeHtml(safeTrim(p.policyNumber))}</span>` : ''}
                    </div>
                    <div class="lcPolImportedRow__insuredLine">
                      ${escapeHtml(insuredName || '—')}
                    </div>
                    <div class="lcPolImportedRow__miniLine">
                      ${insuredIdNumber ? `<span>ת״ז ${escapeHtml(insuredIdNumber)}</span>` : ''}
                      ${importedIncludedText ? `<span>${escapeHtml(importedIncludedText)}</span>` : ''}
                    </div>
                    ${importedInfoBadges ? `<div class="lcPolImportedRow__tags">${importedInfoBadges}</div>` : ''}
                    ${premiumBreakdownHtml}
                  </div>

                  <div class="lcPolImportedRow__segment lcPolImportedRow__segment--price">
                    <span class="lcPolImportedRow__premiumLabel">סה״כ פרמיה חודשית</span>
                    <strong class="lcPolImportedRow__premiumValue">${monthlyPremiumTxt}</strong>
                  </div>

                  <div class="lcPolImportedRow__segment lcPolImportedRow__segment--actionShell">
                    ${cancellationBox}
                  </div>
                </div>
              </td>
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const OpsEventsUI = {
    ...
    customerOptions(){
      return listOpsRelevantCustomers().map((rec) => ({
        id: String(rec.id || ''),
