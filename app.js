...
(() => {
  ...
  const BUILD = "20260324-operational-report-direct-pdf-4";
  ...
  function ensureOperationalPdfFont(pdf){
    ...
    if(!pdfHebrewFontReady){
      pdf.addFileToVFS('NotoSansHebrew-SemiCondensed.ttf', PDF_HEBREW_FONT_BASE64);
      pdf.addFont('NotoSansHebrew-SemiCondensed.ttf', 'NotoSansHebrew', 'normal');
      pdfHebrewFontReady = true;
    }
...
// ---------- Step 6 ----------
    ...
    async exportOperationalPdf(payloadOverride){
      ...
      try{
        ...
        const textWidth = (text, size=11) => {
          pdf.setFont(fontName, 'normal');
          pdf.setFontSize(size);
          return pdf.getTextWidth(textForPdf(text) || ' ');
        };
        ...
        const drawHeader = () => {
          ...
          pdf.roundedRect(marginX, y, contentW, 90, 20, 20, 'F');
          pdf.setFillColor(255,255,255);
          if(logoData){
            try{ pdf.addImage(logoData, 'PNG', marginX + 14, y + 16, 98, 38, undefined, 'FAST'); }catch(_e){}
          }
          pdf.setTextColor(255,255,255);
          pdf.setFont(fontName, 'normal');
          pdf.setFontSize(23);
          pdf.text(textForPdf('דוח תפעולי מעודכן'), pageW - marginX - 16, y + 28, { align:'right' });
          pdf.setFontSize(11.5);
          pdf.text(textForPdf('GEMEL INVEST · מסמך פנימי למחלקת תפעול'), pageW - marginX - 16, y + 48, { align:'right' });

          const metaRows = [
            ['שם לקוח', safeTrim(pack.meta?.customerName) || '—'],
            ['נציג מטפל', safeTrim(pack.meta?.agentName) || '—'],
            ['תאריך ושעה', safeTrim(pack.meta?.exportedAt) || '—']
          ];
          let metaY = y + 62;
          pdf.setFontSize(10.5);
          metaRows.forEach(([label, value], idx) => {
            const x = pageW - marginX - 16 - (idx * 160);
            pdf.setTextColor(222, 231, 248);
            pdf.text(textForPdf(label), x, metaY, { align:'right' });
            pdf.setTextColor(255,255,255);
            pdf.text(textForPdf(value), x, metaY + 15, { align:'right' });
          });
        ...
        const drawFooter = () => {
          pdf.setDrawColor(...colors.line);
          pdf.line(marginX, pageH - bottomMargin - 10, pageW - marginX, pageH - bottomMargin - 10);
          pdf.setTextColor(...colors.muted);
          pdf.setFont(fontName, 'normal');
          pdf.setFontSize(9.5);
          pdf.text(textForPdf(`עמוד ${pageIndex}`), pageW - marginX, pageH - bottomMargin + 6, { align:'right' });
          pdf.text(textForPdf('מסמך פנימי · נוצר ישירות מתוך נתוני המערכת'), marginX, pageH - bottomMargin + 6, { align:'left' });
        };

        const ensureSpace = (heightNeeded) => {
          if(y + heightNeeded <= pageH - bottomMargin - 22) return;
          drawFooter();
          pdf.addPage('a4', 'p');
          y = topMargin;
          drawHeader();
        ...
        const splitRtl = (value, width, size=11) => {
          pdf.setFont(fontName, 'normal');
          pdf.setFontSize(size);
          const prepared = textForPdf(value) || '—';
          const lines = pdf.splitTextToSize(prepared, width, { lang:'he' });
          return Array.isArray(lines) ? lines : [prepared];
        };

        const drawSectionTitle = (title) => {
          ensureSpace(38);
          pdf.setFillColor(...colors.soft);
          pdf.roundedRect(marginX, y, contentW, 26, 10, 10, 'F');
          pdf.setTextColor(...colors.navy);
          pdf.setFont(fontName, 'normal');
          pdf.setFontSize(14);
          pdf.text(textForPdf(title), pageW - marginX - 14, y + 17, { align:'right' });
          y += 34;
        };

        const drawKvSection = (section) => {
          drawSectionTitle(section.title || 'נתונים');
          const rows = Array.isArray(section.rows) ? section.rows : [];
          rows.forEach(([label, value]) => {
            const rightColW = 132;
            const gap = 12;
            const leftColW = contentW - rightColW - gap - 24;
            const valueLines = splitRtl(value, leftColW, 11.2);
            const rowH = Math.max(28, (valueLines.length * 14) + 12);
            ensureSpace(rowH + 6);
            pdf.setDrawColor(...colors.line);
            pdf.setFillColor(255,255,255);
            pdf.roundedRect(marginX, y, contentW, rowH, 10, 10, 'FD');
            pdf.setFillColor(246, 249, 253);
            pdf.roundedRect(pageW - marginX - rightColW, y, rightColW, rowH, 10, 10, 'F');
            pdf.setTextColor(...colors.navy);
            pdf.setFont(fontName, 'normal');
            pdf.setFontSize(11.2);
            pdf.text(textForPdf(label), pageW - marginX - 12, y + 18, { align:'right' });
            pdf.setTextColor(...colors.text);
            pdf.text(valueLines, pageW - marginX - rightColW - gap, y + 18, { align:'right' });
            y += rowH + 6;
          });
          y += 6;
        };

        const drawListSection = (section) => {
          drawSectionTitle(section.title || 'רשימה');
          const items = (Array.isArray(section.items) && section.items.length ? section.items : ['אין נתונים להצגה']).map((item) => textForPdf(item) || '—');
          items.forEach((item, idx) => {
            const bulletW = 22;
            const textWAvail = contentW - bulletW - 20;
            const lines = splitRtl(item, textWAvail, 11);
            const rowH = Math.max(34, (lines.length * 14) + 14);
            ensureSpace(rowH + 6);
            pdf.setDrawColor(...colors.line);
            pdf.setFillColor(255,255,255);
            pdf.roundedRect(marginX, y, contentW, rowH, 10, 10, 'FD');
            pdf.setFillColor(...colors.soft);
            pdf.circle(pageW - marginX - 16, y + 17, 8, 'F');
            pdf.setTextColor(...colors.blue);
            pdf.setFont(fontName, 'normal');
            pdf.setFontSize(9.5);
            pdf.text(String(idx + 1), pageW - marginX - 16, y + 20, { align:'center' });
            pdf.setTextColor(...colors.text);
            pdf.setFontSize(11);
            pdf.text(lines, pageW - marginX - 34, y + 18, { align:'right' });
            y += rowH + 6;
          });
          y += 6;
        };
        ...
        drawHeader();
        (Array.isArray(pack.sections) ? pack.sections : []).forEach((section) => {
          if(!section) return;
          if(section.type === 'kv') drawKvSection(section);
          else if(section.type === 'list') drawListSection(section);
        });

        ensureSpace(80);
        pdf.setFillColor(...colors.soft);
        pdf.roundedRect(marginX, y, contentW, 56, 14, 14, 'F');
        pdf.setTextColor(...colors.navy);
        pdf.setFont(fontName, 'normal');
        pdf.setFontSize(13);
        pdf.text(textForPdf('סיכום'), pageW - marginX - 14, y + 18, { align:'right' });
        pdf.setTextColor(...colors.text);
        pdf.setFontSize(10.8);
        const summaryText = splitRtl('הדוח נוצר ישירות מתוך נתוני המערכת בפורמט PDF יציב, מסודר ובעל חלוקה בנקאית ברורה. כל הנתונים שנמסרו בשלבי ההקמה מרוכזים כאן לצורך עבודה תפעולית שוטפת.', contentW - 28, 10.8);
        pdf.text(summaryText, pageW - marginX - 14, y + 36, { align:'right' });
