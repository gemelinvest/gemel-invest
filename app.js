...
(() => {
  ...
  const BUILD = "20260330-har-inline-compact-v14";
...
// open health covers drawer (Health only)
    ...
    parseElementaryVehicleHarWorkbook(buffer, ins){
      ...
      if(!firstSheetName) throw new Error('לא נמצאה לשונית נתונים בקובץ.');
      const sheet = wb.Sheets[firstSheetName];
      const rows = window.XLSX.utils.sheet_to_json(sheet, { header:1, defval:'' });
      const headerIdx = rows.findIndex((row) => Array.isArray(row) && row.includes('תעודת זהות') && row.includes('מספר פוליסה'));
      if(headerIdx === -1) throw new Error('מבנה קובץ הר הביטוח לא זוהה.');
      const header = rows[headerIdx].map((x) => safeTrim(x));
      const idx = {
        idNumber: header.indexOf('תעודת זהות'),
        main: header.indexOf('ענף ראשי'),
        sub: header.indexOf('ענף (משני)'),
        productType: header.indexOf('סוג מוצר'),
        company: header.indexOf('חברה'),
        period: header.indexOf('תקופת ביטוח'),
        extra: header.indexOf('פרטים נוספים'),
        premium: header.indexOf('פרמיה בש"ח'),
        premiumType: header.indexOf('סוג פרמיה'),
        policyNumber: header.indexOf('מספר פוליסה')
      };
      let currentSection = '';
      const extracted = [];
      rows.slice(headerIdx + 1).forEach((row) => {
        if(!Array.isArray(row)) return;
        const mainCell = safeTrim(row[idx.main]);
        if(mainCell.startsWith('תחום - ')){
          currentSection = mainCell;
          return;
        }
        if(currentSection !== 'תחום - כללי') return;
        const productType = safeTrim(row[idx.productType]);
        const sub = safeTrim(row[idx.sub]);
        const extra = safeTrim(row[idx.extra]);
        const content = `${mainCell} ${sub} ${productType} ${extra}`;
        const isVehicle = /רכב/.test(content);
        const isHome = /דירה|מבנה|תכולה/.test(content);
        if(!isVehicle || isHome) return;
        const policyNumber = safeTrim(row[idx.policyNumber]);
        if(!policyNumber) return;
        const premiumRaw = Number(String(row[idx.premium] ?? '').replace(/[^\d.-]/g, ''));
        const premiumNum = Number.isFinite(premiumRaw) ? premiumRaw : 0;
        const premiumType = safeTrim(row[idx.premiumType]);
        extracted.push({
          id: 'veh_' + Math.random().toString(16).slice(2),
          idNumber: safeTrim(row[idx.idNumber]),
          main: mainCell,
          sub,
          productType,
          company: this.normalizeHarCompanyName(row[idx.company]) || safeTrim(row[idx.company]),
          period: safeTrim(row[idx.period]),
          extra,
          premium: premiumNum,
