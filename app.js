...
(() => {
  ...
  const PROTECTED_SYSTEM_USER = "אוריה system administrator";

  function isProtectedSystemUser(user){
    const protectedName = safeTrim(PROTECTED_SYSTEM_USER).toLowerCase();
    const byName = safeTrim(user?.name).toLowerCase();
    const byUsername = safeTrim(user?.username || user?.user).toLowerCase();
    return !!protectedName && (byName === protectedName || byUsername === protectedName);
  }
  ...
  const UsersUI = {
    ...
    async _saveFromModal(){
      ...
      if(isEdit){
        ...
        if(isProtectedSystemUser(a)){
          this._showErr(E.err, "לא ניתן לערוך את משתמש מנהל המערכת הראשי");
          return;
        }
        a.name = name;
        a.username = username;
    ...
    render(){
      ...
      UI.els.usersTbody.innerHTML = rows.map(a => {
        ...
        const isProtected = isProtectedSystemUser(a);
        return `
          <tr>
            <td>${escapeHtml(a.name)}${isProtected ? ' <span class="badge">מוגן</span>' : ''}</td>
            <td>${role}</td>
            <td><span class="badge">${status}</span></td>
            <td>
              ${isProtected
                ? '<span class="muted">משתמש מערכת מוגן</span>'
    ...
    async editUser(id){
      ...
      if(!a) return;
      if(isProtectedSystemUser(a)){
        alert("לא ניתן לערוך את משתמש מנהל המערכת הראשי");
        return;
      }
    ...
    async toggleUser(id){
      ...
      if(!a) return;
      if(isProtectedSystemUser(a)){
        alert("לא ניתן להשבית את משתמש מנהל המערכת הראשי");
        return;
      }
