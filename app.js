...
(() => {
  ...
  const Auth = {
    ...
    normalizeSessionId(name, username){
      return String((safeTrim(name) || 'user') + '__' + (safeTrim(username) || safeTrim(name) || 'user'))
        .normalize('NFKD')
        .replace(/[^\w֐-׿-]+/g, '-')
        .replace(/-+/g, '-')
        .replace(/^-|-$/g, '')
    ...
    _saveSession(cur){
      try {
        localStorage.setItem(LS_SESSION_KEY, JSON.stringify({ id: safeTrim(cur?.id), name: cur.name, role: cur.role }));
      } catch(_) {}
    },
    ...
    async _submit(){
      ...
      if (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin)) {
        this.current = { id: this.normalizeSessionId(safeTrim(adminAuth.username) || defAdmin.username, safeTrim(adminAuth.username) || defAdmin.username), name: safeTrim(adminAuth.username) || defAdmin.username, role:"admin" };
        try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      ...
      this.current = { id: safeTrim(matched.id) || this.normalizeSessionId(matched.name, matched.username), name: matched.name, role: (matched.role === "manager" ? "manager" : matched.role === "ops" ? "ops" : "agent") };
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      await App.reloadSessionState();
      this.unlock();
      UI.applyRoleUI();
...
const SystemRepairUI = {
  ...
  const ChatUI = {
    ...
    refreshCurrentUser(){
      ...
      const sourceAgent = (Array.isArray(State.data?.agents) ? State.data.agents : []).find((a) => safeTrim(a?.id) === safeTrim(Auth.current?.id) || safeTrim(a?.name) === safeTrim(Auth.current?.name) || safeTrim(a?.username) === safeTrim(Auth.current?.name));
      this.currentUser = {
        id: safeTrim(Auth.current?.id) || this.userIdFromAgent(sourceAgent || { id: Auth.current?.name, username: Auth.current?.name, name: Auth.current?.name }),
