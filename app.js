...
// ---------- Step 6 ----------
    ...
    async exportOperationalPdf(payloadOverride, sourceBtn){
      ...
      try{
        ...
        const pdfMarkup = this.buildOperationalPdfMarkup(payload, { forPreview:false });
        if(!pdfMarkup?.html) throw new Error('Operational PDF markup was not created');
        ...
        host.querySelectorAll('button, .iconBtn, [data-close="1"]').forEach((el) => el.remove());
        host.querySelectorAll('.lcPdfPage').forEach((page) => {
        ...
        host.querySelectorAll('.lcPdfPageBreak').forEach((br) => {
        ...
        const targetWidth = Math.ceil(target.getBoundingClientRect?.().width || target.scrollWidth || 794);
        const targetHeight = Math.ceil(target.scrollHeight || target.getBoundingClientRect?.().height || 2244);
        const worker = html2pdfFactory().set({
          ...
          html2canvas: {
            ...
            windowHeight: Math.max(targetHeight, 1122),
            logging: false,
