...
(() => {
  ...
  const BUILD = "20260409-vHealthReportFullDetails";
...
// ---------- Step 6 ----------
    ...
    summarizeHealthFields(fields, options = {}){
      ...
      return entries
        .slice(0, limit)
        .map((row) => includeKeys && row.key ? `${row.key}: ${row.value}` : row.value)
        .join(' • ');
    },

    formatHealthFieldsForOperationalReport(fields){
      const entries = Object.entries(fields || {})
        .map(([k, v]) => ({ key: safeTrim(k), value: safeTrim(v) }))
        .filter((row) => row.value);
    ...
    getHealthFindingsForInsured(insId){
      ...
      this.getHealthQuestionList().forEach(item => {
        ...
        if(r.answer === 'yes') findings.push({
          ...
          saved: !!r.saved,
          summary: this.summarizeHealthFields(r.fields || {}, { limit:2 }),
          fields: r.fields || {}
        });
      });
      return { ins, findings };
      ...
      (payload.insureds || []).forEach((ins, index) => {
        ...
        questionList.forEach((item) => {
          ...
          const details = answerRaw === 'yes'
            ? safeTrim(this.formatHealthFieldsForOperationalReport(answer?.fields || {})) || 'סומן כן ללא פירוט נוסף'
            : 'נענה לא';
