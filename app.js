...
(() => {
  ...
  const BUILD = "20260324-operational-report-pdf-engine-2";
...
// ---------- Step 6 ----------
    ...
    async exportOperationalPdf(payloadOverride){
      const html2canvasFn = window.html2canvas || window.html2pdf?.html2canvas;
      ...
      try{
        ...
        if(target && target.style) target.style.background = '#ffffff';
        ...
        if(!canvas || !canvas.width || !canvas.height){
          throw new Error('PDF_CANVAS_EMPTY');
        }

        const pdf = new jsPdfCtor({ unit: 'pt', format: 'a4', orientation: 'portrait', compress: true });
        const pageWidth = pdf.internal.pageSize.getWidth();
        const pageHeight = pdf.internal.pageSize.getHeight();
        const sliceHeightPx = Math.max(1, Math.floor(canvas.width * (pageHeight / pageWidth)));
        let offsetY = 0;
        let pageIndex = 0;
        ...
        while(offsetY < canvas.height){
          ...
          if(!pageCtx) throw new Error('PDF_PAGE_CONTEXT_FAILED');
          ...
          pdf.addImage(pageData, 'JPEG', 0, 0, pageWidth, renderedHeight, undefined, 'FAST');
