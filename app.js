...
txt.textContent = "";
  ...
  const Wizard = {
    ...
    // כיסויים למוצר "בריאות" (שלב 3 + שלב 5)
    healthCoverDefs: [
      { key:"surg_il", label:"ניתוחים בארץ" },
      { key:"surg_abroad", label:"ניתוחים בחו״ל" },
      { key:"transplants", label:"השתלות" },
      { key:"drugs", label:"תרופות מחוץ לסל" },
      { key:"ambulatory", label:"אמבולטורי" }
    ],
...
if(path === "birthDate"){
  ...
  return;
          ...
          if(path.includes("existingPolicies.") && path.endsWith(".type")){
            // ensure dynamic fields (health covers / risk / CI) show immediately when type changes
            this.render();
            return;
          }
          if(path.includes("existingPolicies.") && path.endsWith(".company")){
            // logo / company dependent UI
            this.render();
            return;
          }
      ...
      // covers dropdown (in-row) — Option B: stays open until click outside
      $$("[data-covers-toggle]", this.els.body).forEach(btn => {
        on(btn, "click", (e) => {
          e.preventDefault();
          e.stopPropagation();
          const id = btn.getAttribute("data-covers-toggle");
          if(!id) return;
          // close others
          this.els.body.querySelectorAll(".lcCoversDropdown.is-open").forEach(dd => {
            const ddId = dd.getAttribute("data-covers-root");
            if(ddId && ddId !== id){
              dd.classList.remove("is-open");
              const b = dd.querySelector("[data-covers-toggle]");
              if(b) b.setAttribute("aria-expanded","false");
            }
          });
          const wrap = btn.closest(".lcCoversDropdown");
          if(!wrap) return;
      ...
      // update summary live (no rerender needed)
      $$(".lcCoversDropdown input[type=checkbox]", this.els.body).forEach(chk => {
        on(chk, "change", () => {
          const wrap = chk.closest(".lcCoversDropdown");
          if(!wrap) return;
          const id = wrap.getAttribute("data-covers-root") || "";
          const total = wrap.querySelectorAll("input[type=checkbox]:checked").length;
          const summaryEl = wrap.querySelector(`[data-covers-summary="${id}"]`);
          if(summaryEl) summaryEl.textContent = total ? ("נבחרו: " + total) : "לא נבחרו";
          const item = chk.closest(".lcCoverItem");
          if(item) item.setAttribute("aria-selected", chk.checked ? "true" : "false");
        });
      });

      // global outside click closer (bind once)
      if(!this._coversDocBound){
        this._coversDocBound = true;
        document.addEventListener("click", (ev) => {
          try{
            if(!this.isOpen) return;
            const t = ev.target;
            if(t && t.closest && t.closest(".lcCoversDropdown")) return;
            const root = this.els?.body;
            if(!root) return;
            root.querySelectorAll(".lcCoversDropdown.is-open").forEach(dd => {
              dd.classList.remove("is-open");
              const b = dd.querySelector("[data-covers-toggle]");
              if(b) b.setAttribute("aria-expanded","false");
            });
    ...
    renderStep3(ins){
      ...
      const rows = (d.existingPolicies || []).map(p => {
        ...
        p.healthCovers = p.healthCovers || {};
        const healthHtml = isHealth ? this.renderHealthCoversBox({ covers: p.healthCovers, attrName:"data-bind", prefix:`existingPolicies.${p.id}.healthCovers` }) : "";
...
_coversAnySelected(covers){
  const c = covers || {};
  return Object.values(c).some(v => v === true);
},

_ensureCoversObj(obj){
  if(!obj) return {};
  if(typeof obj !== "object") return {};
  return obj;
},

// Renders the covers selector. attrName can be "data-bind" (policies) or "data-pdraft-hc" (draft)
renderHealthCoversBox({ covers, attrName, prefix }){
  // Dropdown-in-row selector (Option B: stays open until click outside)
  const defs = Array.isArray(this.healthCoverDefs) ? this.healthCoverDefs : [];
  const c = this._ensureCoversObj(covers);

  const picked = defs.filter(d => !!c[d.key]).map(d => d.label);
  const summary = picked.length ? `נבחרו: ${picked.length}` : "לא נבחרו";

  const uid = safeTrim(prefix) || ("hc_" + Math.random().toString(16).slice(2));
  const items = defs.map(def => {
    const key = def.key;
    const label = def.label;
    const checked = !!c[key];
    const attrVal = (attrName === "data-bind") ? `${prefix}.${key}` : key;
    return `
      <label class="lcCoverItem" role="option" aria-selected="${checked ? "true" : "false"}">
        <input type="checkbox" ${attrName}="${escapeHtml(attrVal)}" ${checked ? "checked" : ""}/>
        <span class="lcCoverItem__text">${escapeHtml(label)}</span>
      </label>
    `;
  }).join("");

  return `
    <div class="lcCoversDropdown" data-health-covers="1" data-covers-root="${escapeHtml(uid)}">
      <div class="lcCoversDropdown__bar">
        <button class="btn lcCoversDropdown__btn" type="button" data-covers-toggle="${escapeHtml(uid)}" aria-expanded="false">
          בחירת כיסויים <span class="lcCoversDropdown__chev" aria-hidden="true">▾</span>
        </button>
        <span class="lcCoversDropdown__summary muted small" data-covers-summary="${escapeHtml(uid)}">${escapeHtml(summary)}</span>
      </div>

      <div class="lcCoversDropdown__panel" role="listbox" aria-label="כיסויים">
        <div class="lcCoversDropdown__title">כיסויים (חובה לבחור לפחות אחד)</div>
        <div class="lcCoversDropdown__grid">${items}</div>
      </div>
    </div>
  `;
},
    ...
    addDraftPolicy(){
      ...
      const p = {
        ...
        healthCovers: Object.assign({}, (d.type === "בריאות" ? (d.healthCovers || {}) : {}))
      };
    ...
    validateStep5(){
      ...
      const bad = list.filter(p => {
        ...
        if(p.type === "בריאות"){
          const c = p.healthCovers || {};
          if(!this._coversAnySelected(c)) return true;
        }
    ...
    renderStep5(){
      ...
      const isHealthDraft = (d.type === "בריאות");
      d.healthCovers = d.healthCovers || {};
      const healthDraftBox = isHealthDraft ? this.renderHealthCoversBox({ covers: d.healthCovers, attrName:"data-pdraft-hc", prefix:"" }) : "";
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        if(p.type === "בריאות"){
          const c = p.healthCovers || {};
          const picked = (this.healthCoverDefs || []).filter(def => !!c[def.key]).map(def => def.label);
          if(picked.length){
            meta += `<div class="small muted">כיסויים: <b>${escapeHtml(picked.join(", "))}</b></div>`;
          }
        }
      ...
      this.policyDraft.healthCovers = {};
        ...
        $$("[data-pdraft]", this.els.body).forEach(el => {
          on(el, "input", () => {
            ...
            if(k === "type"){
              if(this.policyDraft.type !== "בריאות") this.policyDraft.healthCovers = {};
              else this.policyDraft.healthCovers = this.policyDraft.healthCovers || {};
              this.render();
            }else if(k === "pledge" || k === "insuredId") this.render();
          });
          on(el, "change", () => {
            this.ensurePolicyDraft();
            const k = el.getAttribute("data-pdraft");
...
// health covers (draft)
$$("[data-pdraft-hc]", this.els.body).forEach(el => {
  const sync = () => {
    this.ensurePolicyDraft();
    const k = el.getAttribute("data-pdraft-hc");
    if(!k) return;
    this.policyDraft.healthCovers = this.policyDraft.healthCovers || {};
    this.policyDraft.healthCovers[k] = !!el.checked;
  };
  on(el, "input", sync);
  on(el, "change", sync);
});
...
$$("[data-pdraft-bank]", this.els.body).forEach(el => {
    ...
    validateDraftPolicy(){
      ...
      if(d.type === "בריאות"){
        const c = d.healthCovers || {};
        if(!this._coversAnySelected(c)) return { ok:false, msg:"בבריאות חובה לבחור לפחות כיסוי אחד" };
      }
...
// ---------- Step 6 ----------
    ...
    isStepCompleteForInsured(stepId, ins){
      ...
      if(stepId === 3){
        ...
        // Health (בריאות): חובה לבחור לפחות כיסוי אחד
        for(const p of list){
          if(p.type === "בריאות"){
            const c = p.healthCovers || {};
            if(!this._coversAnySelected(c)) return false;
          }
        }
        return true;
      }
