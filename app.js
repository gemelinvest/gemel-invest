...
// ---------------------------
  ...
  const Auth = {
    ...
    _loginSubmit: async function() {
      ...
      // Backward-compat: also allow "admin" / "admin1234" as username with the default PIN 1234
      const uLower = String(username || "").trim().toLowerCase();
      const masterOk = (pin === defAdmin.pin) && (username === defAdmin.username || uLower === "admin" || uLower === "admin1234");
...
if (!out.agents.length) {
  ...
  const UI = {
    ...
    init() {
      ...
      // Add Insured (New Customer)
      this.els.btnAddInsured = $("#btnAddInsured");
      this.els.insuredAddOverlay = $("#insuredAddOverlay");
      this.els.btnCloseInsuredAdd = $("#btnCloseInsuredAdd");
      this.els.insuredsWrap = $("#insuredsWrap");
      this.els.insuredsList = $("#insuredsList");
      this._insuredsDraft = []; // additional insureds during create/edit flow
...
this.els.oldPolicyBankAgency = $("#oldPolicyBankAgency");
      ...
      this.els.cfInsuredsCard = $("#cfInsuredsCard");
      this.els.cfInsuredsList = $("#cfInsuredsList");
...
this.els.btnUserEditSave = $("#btnUserEditSave");
      ...
      // ===== New Customer modal: Add Insured (spouse/adult/minor) =====
      // (handlers are bound once; only draft changes per modal open)
      if (!this._insuredsInitBound) {
        this._insuredsInitBound = true;
        const openPick = () => {
          if (!this.els.insuredAddOverlay) return;
          this.els.insuredAddOverlay.classList.remove("hidden");
          this.els.insuredAddOverlay.setAttribute("aria-hidden", "false");
        };
        const closePick = () => {
          if (!this.els.insuredAddOverlay) return;
          this.els.insuredAddOverlay.classList.add("hidden");
          this.els.insuredAddOverlay.setAttribute("aria-hidden", "true");
        };
        const uidLocal = () => "i_" + Math.random().toString(16).slice(2) + "_" + Date.now().toString(16);
        const getPrimarySnapshot = () => {
          const f = this.els.customerForm;
          const pick = (name) => safeTrim(f.querySelector(`[name="${name}"]`)?.value);
          return { address: pick("address"), email: pick("email"), phone: pick("phone"), childrenCount: pick("childrenCount") };
        };
        const labelKind = (k) => (k === "spouse" ? "בן/בת זוג" : k === "adult" ? "בגיר" : "קטין");
        const applyCopiesForKind = (kind, primary, insured) => {
          insured._copies ||= {};
          if (kind === "spouse") {
            insured.address = primary.address || "";
            insured.childrenCount = primary.childrenCount || "";
            insured._copies.address = 1;
            insured._copies.childrenCount = 1;
          } else if (kind === "adult") {
            insured.address = primary.address || "";
            insured._copies.address = 1;
          } else if (kind === "minor") {
            insured.address = primary.address || "";
            insured.email = primary.email || "";
            insured.phone = primary.phone || "";
            insured._copies.address = 1;
            insured._copies.email = 1;
            insured._copies.phone = 1;
          }
        };
        const renderInsureds = () => {
          if (!this.els.insuredsWrap || !this.els.insuredsList) return;
          const list = Array.isArray(this._insuredsDraft) ? this._insuredsDraft : [];
          this.els.insuredsWrap.style.display = list.length ? "" : "none";
          this.els.insuredsList.innerHTML = list.map(ins => {
            const roAddr = ins._copies?.address ? "readonly" : "";
            const roMail = ins._copies?.email ? "readonly" : "";
            const roPhone = ins._copies?.phone ? "readonly" : "";
            const roKids = ins._copies?.childrenCount ? "readonly" : "";
            return `
              <div class="insuredCard" data-insured-id="${escapeHtml(ins.id)}">
                <div class="insuredCardTop">
                  <div class="insuredBadge">${escapeHtml(labelKind(ins.kind))}</div>
                  <button type="button" class="insuredRemoveBtn" data-insured-remove="1">הסר</button>
                </div>
                <div class="insuredGrid">
                  <div><label>שם פרטי</label><input class="input" type="text" data-insured-field="firstName" value="${escapeHtml(ins.firstName || "")}"></div>
                  <div><label>שם משפחה</label><input class="input" type="text" data-insured-field="lastName" value="${escapeHtml(ins.lastName || "")}"></div>
                  <div><label>ת"ז</label><input class="input" type="text" data-insured-field="idNumber" value="${escapeHtml(ins.idNumber || "")}"></div>
                  <div><label>תאריך לידה</label><input class="input" type="date" data-insured-field="birthDate" value="${escapeHtml(ins.birthDate || "")}"></div>
                  <div><label>כתובת</label><input class="input" type="text" data-insured-field="address" value="${escapeHtml(ins.address || "")}" ${roAddr}></div>
                  <div><label>טלפון</label><input class="input" type="text" data-insured-field="phone" value="${escapeHtml(ins.phone || "")}" ${roPhone}></div>
                  <div><label>מייל</label><input class="input" type="email" data-insured-field="email" value="${escapeHtml(ins.email || "")}" ${roMail}></div>
                  <div><label>מס' ילדים</label><input class="input" type="number" min="0" step="1" data-insured-field="childrenCount" value="${escapeHtml(String(ins.childrenCount ?? ""))}" ${roKids}></div>
                </div>
              </div>
            `;
          }).join("");
        };
        const syncCopiedFields = () => {
          const primary = getPrimarySnapshot();
          (this._insuredsDraft || []).forEach(ins => {
            if (!ins || !ins._copies) return;
            if (ins._copies.address) ins.address = primary.address || "";
            if (ins._copies.email) ins.email = primary.email || "";
            if (ins._copies.phone) ins.phone = primary.phone || "";
            if (ins._copies.childrenCount) ins.childrenCount = primary.childrenCount || "";
          });
          renderInsureds();
        };
        on(this.els.btnAddInsured, "click", openPick);
        on(this.els.btnCloseInsuredAdd, "click", closePick);
        on(this.els.insuredAddOverlay, "click", (e) => {
          const btn = e?.target?.closest?.("[data-insured-kind]");
          if (!btn) return;
          const kind = safeTrim(btn.dataset.insuredKind);
          if (!kind) return;
          const primary = getPrimarySnapshot();
          const ins = { id: uidLocal(), kind, firstName: "", lastName: "", idNumber: "", birthDate: "", address: "", phone: "", email: "", childrenCount: "" };
          applyCopiesForKind(kind, primary, ins);
          this._insuredsDraft ||= [];
          this._insuredsDraft.push(ins);
          closePick();
          renderInsureds();
        });
        on(this.els.insuredsList, "click", (e) => {
          const rm = e?.target?.closest?.("[data-insured-remove='1']");
          if (!rm) return;
          const card = rm.closest("[data-insured-id]");
          const id = safeTrim(card?.dataset?.insuredId);
          if (!id) return;
          this._insuredsDraft = (this._insuredsDraft || []).filter(x => x.id !== id);
          renderInsureds();
        });
        on(this.els.insuredsList, "input", (e) => {
          const fieldEl = e?.target?.closest?.("[data-insured-field]");
          if (!fieldEl) return;
          const card = fieldEl.closest("[data-insured-id]");
          const id = safeTrim(card?.dataset?.insuredId);
          const key = safeTrim(fieldEl.dataset.insuredField);
          const ins = (this._insuredsDraft || []).find(x => x.id === id);
          if (!ins || !key) return;
          if (ins._copies && ins._copies[key]) {
            delete ins._copies[key];
            try { fieldEl.removeAttribute("readonly"); } catch(_) {}
          }
          ins[key] = String(fieldEl.value || "");
        });
        ["address","email","phone","childrenCount"].forEach((n) => {
          const el = this.els.customerForm.querySelector(`[name="${n}"]`);
          on(el, "input", () => syncCopiedFields());
        });
        this._renderInsureds = renderInsureds;
      }
...
// Search (live filter on customers view)
      ...
      on(this.els.customerForm, "submit", async (e) => {
        ...
        // additional insureds from UI (if any)
        incoming.insureds = (Array.isArray(this._insuredsDraft) ? this._insuredsDraft : []).map(ins => ({
          id: safeTrim(ins.id) || uid(),
          kind: safeTrim(ins.kind),
          firstName: safeTrim(ins.firstName),
          lastName: safeTrim(ins.lastName),
          idNumber: safeTrim(ins.idNumber),
          birthDate: safeTrim(ins.birthDate),
          address: safeTrim(ins.address),
          phone: safeTrim(ins.phone),
          email: safeTrim(ins.email),
          childrenCount: (ins.childrenCount === "" || ins.childrenCount == null) ? "" : Number(ins.childrenCount)
        }));
...
this.renderPolicies();
    ...
    openCustomerEdit(customerId){
      ...
      // insureds draft (edit): load from customer if exists
      this._insuredsDraft = Array.isArray(c.insureds) ? c.insureds.map(x => ({...x, _copies: (x && x._copies) ? {...x._copies} : {}})) : [];
      try { this._renderInsureds && this._renderInsureds(); } catch(_){ }
      const set = (name, val) => {
...
if (this.els.cfIdIssueDate) this.els.cfIdIssueDate.textContent = (c.idIssueDate || "—");
      ...
      // additional insureds
      if (this.els.cfInsuredsCard && this.els.cfInsuredsList) {
        const insureds = Array.isArray(c.insureds) ? c.insureds : [];
        this.els.cfInsuredsCard.style.display = insureds.length ? "" : "none";
        this.els.cfInsuredsList.innerHTML = insureds.map((ins, idx) => {
          const kind = safeTrim(ins.kind);
          const kindLabel = (kind === "spouse" ? "בן/בת זוג" : kind === "adult" ? "בגיר" : kind === "minor" ? "קטין" : "מבוטח נוסף");
          const name = safeTrim(`${ins.firstName || ""} ${ins.lastName || ""}`) || `מבוטח נוסף ${idx+1}`;
          const metaParts = [];
          const tz = safeTrim(ins.idNumber);
          const bd = safeTrim(ins.birthDate);
          const phone = safeTrim(ins.phone);
          const email = safeTrim(ins.email);
          if (tz) metaParts.push(`ת"ז: ${escapeHtml(tz)}`);
          if (bd) metaParts.push(`לידה: ${escapeHtml(bd)}`);
          if (phone) metaParts.push(`טלפון: ${escapeHtml(phone)}`);
          if (email) metaParts.push(`מייל: ${escapeHtml(email)}`);
          return `
            <div class="cfInsuredItem">
              <div class="cfInsuredItemTop">
                <div class="cfInsuredName">${escapeHtml(name)}</div>
                <div class="insuredBadge">${escapeHtml(kindLabel)}</div>
              </div>
              ${metaParts.length ? `<div class="cfInsuredMeta">${metaParts.join(" · ")}</div>` : ``}
            </div>
          `;
        }).join("");
      }
