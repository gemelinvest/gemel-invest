...
if (!out.agents.length) {
  ...
  out.agents = (base.agents || []).map((a, idx) => ({
    ...
    out.customers = (out.customers || []).map((c) => ({
      ...
      cancelCompetitorMethod: safeTrim(c?.cancelCompetitorMethod),
      ...
      policies: Array.isArray(c?.policies) ? c.policies.map((p) => ({
        ...
        insuredSum: Number(p?.insuredSum || 0),
        compSum: Number(p?.compSum || 0),
        viaBank: (p?.viaBank === true) ? true : false,
        bankAgency: safeTrim(p?.bankAgency),
        lien: (p && p.lien) ? {
          bankName: safeTrim(p.lien.bankName),
          bankNumber: safeTrim(p.lien.bankNumber),
          branchNumber: safeTrim(p.lien.branchNumber),
          amount: safeTrim(p.lien.amount),
          years: safeTrim(p.lien.years),
          address: safeTrim(p.lien.address)
        } : null
      })) : []
    }));
...
this.els.oldPolicyLienHint = $("#oldPolicyLienHint");
      ...
      this.els.cancelPoliciesTbody = $("#cancelPoliciesTbody");
      this.els.cancelPoliciesCount = $("#cancelPoliciesCount");
...
// Search (live filter on customers view)
      ...
      on(this.els.customerForm, "submit", async (e) => {
        ...
        const incoming = {
          ...
          cancelCompetitorMethod: safeTrim(fd.get("cancelCompetitorMethod")),
...
this.renderPolicies();
    ...
    openCustomerEdit(customerId){
      ...
      // Step 5: cancellation method
      try{
        const v = safeTrim(c.cancelCompetitorMethod);
        const radios = Array.from(this.els.customerForm.querySelectorAll('input[name="cancelCompetitorMethod"]'));
        if(radios && radios.length){
          radios.forEach(r=>{ r.checked = (v && r.value === v); });
        }
      }catch(_){ }
...
const toggleProductFields = () => {
  ...
  if(el.oldPolicyBankWrap) el.oldPolicyBankWrap.style.display = showRisk ? "" : "none";

  // reset irrelevant fields
  if(!showRisk){
    if(el.oldPolicyInsuredSum) el.oldPolicyInsuredSum.value = "";
    if(el.oldPolicyHasLien) el.oldPolicyHasLien.checked = false;
    if(el.oldPolicyLienBtn) el.oldPolicyLienBtn.style.display = "none";
    if(el.oldPolicyLienHint) el.oldPolicyLienHint.style.display = "none";
    UI._oldPolicyLienDraft = null;

// reset bank-agency fields
try{
  const radios = Array.from(document.querySelectorAll('input[name="oldPolicyViaBank"]'));
  radios.forEach(r => { if(r && r.value === "no") r.checked = true; });
  if(el.oldPolicyBankAgency) el.oldPolicyBankAgency.value = "";
  if(el.oldPolicyBankAgencyWrap) el.oldPolicyBankAgencyWrap.style.display = "none";
}catch(_){}
  }else{
    // keep button visible only if checkbox on and draft exists
    const has = !!(el.oldPolicyHasLien && el.oldPolicyHasLien.checked);
    const d = UI._oldPolicyLienDraft;
    if(el.oldPolicyLienBtn) el.oldPolicyLienBtn.style.display = (has ? "" : "none");
    if(el.oldPolicyLienHint) el.oldPolicyLienHint.style.display = (has && d ? "" : "none");


// bank-agency question (only for risk products)
try{
  const getViaBank = () => {
    const r = document.querySelector('input[name="oldPolicyViaBank"]:checked');
    return r ? safeTrim(r.value) : "no";
  };
  const applyViaBankUI = () => {
    const via = getViaBank();
    const showAgency = (via === "yes");
    if(el.oldPolicyBankAgencyWrap) el.oldPolicyBankAgencyWrap.style.display = showAgency ? "" : "none";
    if(!showAgency && el.oldPolicyBankAgency) el.oldPolicyBankAgency.value = "";
  };
  applyViaBankUI();
  // bind once
  const radios = Array.from(document.querySelectorAll('input[name="oldPolicyViaBank"]'));
  radios.forEach(r=>{
    ...
    r.dataset.bound="1";
    r.addEventListener("change", applyViaBankUI);
  });
}catch(_){}
  }
...
if(el.oldPolicyHasLien && !el.oldPolicyHasLien.dataset.bound){
  ...
  el.oldPolicyHasLien.addEventListener("change", () => {
    const checked = !!el.oldPolicyHasLien.checked;
    if(!checked){
      UI._oldPolicyLienDraft = null;
      if(el.oldPolicyLienBtn) el.oldPolicyLienBtn.style.display = "none";
      if(el.oldPolicyLienHint) el.oldPolicyLienHint.style.display = "none";
    ...
    // open modal immediately if no draft yet
    if(!UI._oldPolicyLienDraft){
      try{ UI.openLienModal && UI.openLienModal(); }catch(_){}
    }
    if(el.oldPolicyLienBtn) el.oldPolicyLienBtn.style.display = "";
  });
}

if(el.oldPolicyLienBtn && !el.oldPolicyLienBtn.dataset.bound){
  el.oldPolicyLienBtn.dataset.bound = "1";
  el.oldPolicyLienBtn.addEventListener("click", () => {
    try{ UI.openLienModal && UI.openLienModal(); }catch(_){}
  });
}
      ...
      const renderPoliciesTable = () => {
        ...
        if(el.cancelPoliciesCount) el.cancelPoliciesCount.textContent = String(list.length || 0);

        if(!list.length){
          el.oldPoliciesTbody.innerHTML = '<tr><td colspan="6" style="opacity:.6">טרם נוספו פוליסות.</td></tr>';
          if(el.cancelPoliciesTbody) el.cancelPoliciesTbody.innerHTML = '<tr><td colspan="5" style="opacity:.6">טרם נוספו פוליסות.</td></tr>';
          return;
        }
...
// mirror summary table in Step 5 (cancel competitor)
if(el.cancelPoliciesTbody){
  el.cancelPoliciesTbody.innerHTML = list.map((p) => {
    const prem = Number(p.premiumMonthly ?? p.premium ?? 0);
    const premTxt = Number.isFinite(prem) ? prem.toFixed(2) : (p.premiumMonthly ?? '');
    const statusTxt = safeTrim(p.statusText) || safeTrim(p.status) || '';
    return `
      <tr>
        <td>${escapeHtml(p.company||'')}</td>
...
const viaBank = (()=>{ try{ const r=document.querySelector('input[name="oldPolicyViaBank"]:checked'); return r ? safeTrim(r.value) : "no"; }catch(_){ return "no"; } })();
const bankAgency = safeTrim(el.oldPolicyBankAgency ? el.oldPolicyBankAgency.value : "");

if(isRiskProduct(product) && viaBank === "yes" && !bankAgency){
  notify("סומן שהפוליסה נרכשה דרך סוכנות הבנק — נא לבחור סוכנות בנקאית.", "warn");
  return;
}
...
if(isRiskProduct(product) && hasLien && !UI._oldPolicyLienDraft){
  // force filling lien details
  try{ UI.openLienModal && UI.openLienModal(); }catch(_){}
  notify("סומן שיעבוד — נא למלא את פרטי המוטב הבלתי חוזר.", "warn");
  return;
}
...
UI._custPolicies.push({
  ...
  lien: (isRiskProduct(product) && hasLien && UI._oldPolicyLienDraft) ? { ...UI._oldPolicyLienDraft } : null,
  viaBank: isRiskProduct(product) ? (viaBank === "yes") : false,
  bankAgency: (isRiskProduct(product) && viaBank === "yes") ? bankAgency : ""
});
