...
init(){
      ...
      on(this.els.btnSaveDraft, "click", () => this.saveDraft({ manual:true }));

      on(this.els.wrap, "input", (e) => {
        const t = e.target;
        if(!t || !this.autoSaveEnabled || this.suppressAutoSave) return;
        if(t.closest && t.closest('#lcWizardBody')) this.scheduleAutoSave();
      }, true);
      on(this.els.wrap, "change", (e) => {
        const t = e.target;
        if(!t || !this.autoSaveEnabled || this.suppressAutoSave) return;
        if(t.closest && t.closest('#lcWizardBody')) this.scheduleAutoSave(true);
      }, true);
...
// ---------- Step 6 ----------
    ...
    hasMeaningfulDraftData(payload){
      try {
        if(this.editingDraftId) return true;
        const primary = payload?.operational?.primary || {};
        if(safeTrim(primary.firstName) || safeTrim(primary.lastName) || safeTrim(primary.idNumber) || safeTrim(primary.phone) || safeTrim(primary.email)) return true;
        if(Array.isArray(payload?.insureds) && payload.insureds.some(ins => {
          const d = ins?.data || {};
          return safeTrim(d.firstName) || safeTrim(d.lastName) || safeTrim(d.idNumber) || safeTrim(d.phone);
        })) return true;
        if(Array.isArray(payload?.newPolicies) && payload.newPolicies.length) return true;
        if(payload?.currentStep && Number(payload.currentStep) > 1) return true;
      } catch(_e) {}
      return false;
    },
    ...
    scheduleAutoSave(immediate=false){
      if(!Auth.current || !this.autoSaveEnabled || this.suppressAutoSave) return;
      this.clearAutoSaveTimer();
      const delay = immediate ? 250 : this.autoSaveDelay;
      this.autoSaveTimer = setTimeout(() => {
        this.autoSaveTimer = null;
        this.saveDraft({ silent:true, auto:true }).catch(err => {
          console.error('AUTO_SAVE_DRAFT_FAILED:', err);
        });
      }, delay);
    },
    ...
    async flushAutoSave(){
      ...
      if(this.autoSaveInFlight){
        try { await this.autoSaveInFlight; } catch(_e) {}
      }
      if(this.autoSaveEnabled && !this.suppressAutoSave){
        try { await this.saveDraft({ silent:true, auto:true, force:true }); } catch(_e) {}
      }
    },

    async saveDraft(opts={}){
      if(!Auth.current) return { ok:false, skipped:true, reason:'NO_AUTH' };
      const options = opts || {};
      ...
      if(!this.hasMeaningfulDraftData(payload) && !options.force){
        return { ok:false, skipped:true, reason:'EMPTY_DRAFT' };
      }
      ...
      const persistPromise = App.persist(options.manual ? "ההצעה נשמרה" : "ההצעה נשמרה אוטומטית");
      if(options.auto || options.silent) this.autoSaveInFlight = persistPromise;
      const persistRes = await persistPromise;
      if(this.autoSaveInFlight === persistPromise) this.autoSaveInFlight = null;
      ProposalsUI.render();
      if(options.silent) return persistRes;
      if(persistRes?.ok){
        this.setHint(options.manual ? "ההצעה נשמרה ותופיע במסך הצעות להמשך עריכה" : "ההצעה נשמרה אוטומטית ותתעדכן גם במחשבים אחרים");
      }else{
        this.setHint(options.manual ? "ההצעה נשמרה מקומית בלבד. בדוק חיבור לשרת כדי שתופיע גם ממחשב אחר." : "השינויים נשמרו מקומית בלבד. בדוק חיבור לשרת כדי שיעודכנו גם במחשבים אחרים.");
      }
      return persistRes;
    },
