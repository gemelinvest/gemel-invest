...
function updateCoverageCount(){
  const el = document.getElementById("coverageCount");
  if(!selectedCoverages.length){
    el.textContent = "לא נבחרו כיסויים";
  } else {
    el.textContent = `נבחרו ${selectedCoverages.length} כיסויים`;
  }
}

document.addEventListener("DOMContentLoaded", () => {
  const btn = document.getElementById("coverageConfirmBtn");
  if(btn){
    btn.onclick = () => {
      closeCoverageModal();
      console.log("Selected:", selectedCoverages);
    };
  }
});
