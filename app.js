...
(() => {
  ...
  const BUILD = "20260324-operational-report-pdf-engine-5";
...
// ---------- Step 6 ----------
    ...
    async exportOperationalPdf(payloadOverride){
      ...
      try{
        ...
        const worker = window.html2pdf().from(target).set({
          margin: [0, 0, 0, 0],
        ...
        const sample = ctx ? ctx.getImageData(Math.min(10, canvas.width - 1), Math.min(10, canvas.height - 1), 1, 1).data : null;
        const alpha = sample ? Number(sample[3]) : 255;
        if(alpha === 0){
          throw new Error('PDF_CANVAS_TRANSPARENT');
        }
        await worker.toPdf();
        await worker.save();
      }catch(err){
