  ...
  (function bootAuth_(){
    ...
    if(toggleBtn && passEl){
      const syncToggleLabel = () => {
        const isOn = passEl.type === 'text';
        toggleBtn.classList.toggle('isOn', isOn);
        toggleBtn.setAttribute('aria-label', isOn ? 'הסתר סיסמה' : 'הצג סיסמה');
        toggleBtn.setAttribute('title', isOn ? 'הסתר סיסמה' : 'הצג סיסמה');
      };
      syncToggleLabel();
    ...
    // Forgot password (opens existing modal, if present)
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3" style="margin-top:10px">
        ...
        ${selectField('סטטוס', 'old_decision', ['cancel_full|ביטול מלא','cancel_partial|ביטול חלקי','keep|להשאיר ללא שינוי','appoint|ביצוע מינוי סוכן'])}
      </div>
              ...
              <tbody>
                ${rows.map(x => {
                  ...
                  const dec = String(x.decision||x.action||'cancel_full');
                    const statusLabel = (dec==='keep') ? 'להשאיר ללא שינוי'
