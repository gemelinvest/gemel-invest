...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyH_IdKzjFa8-y0K89BPLO2gf1Yipm86mI-BkP1Vn5Tfzeh0yPHr8lr0MdMCE3Gp-k/exec';
...
function debounce(fn, wait){
  ...
  async function syncDown(){
    ...
    if(c && c.ok && Array.isArray(c.rows)){
      state.customers = c.rows;
      saveCustomers_();
    }
    const p = await apiCall('listProposals', {});
    if(p && p.ok && Array.isArray(p.rows)){
      state.proposals = p.rows;
      saveProposals_();
    }
  }    saveProposals_();
  }
  ...
  async function syncUpCustomer(customer){
    // Send flat customer object to Apps Script
    const r = await apiCall('upsertCustomer', customer);
    if(r && r.ok){
      // Update local (optimistic)
      const idx = state.customers.findIndex(x => x.id === customer.id);
      if(idx >= 0) state.customers[idx] = { ...state.customers[idx], ...customer };
  ...
  async function syncUpProposal(proposal){
    // Save proposal as append row in Apps Script
    const r = await apiCall('saveProposal', proposal);
    if(r && r.ok){
      const idx = state.proposals.findIndex(x => x.id === proposal.id);
      if(idx >= 0) state.proposals[idx] = { ...state.proposals[idx], ...proposal };
