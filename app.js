...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzC8KP13XkDuAFqyRwtTIMMIm1m5euWYEis-oIqITg0S2ojlN6iR8PcDys_JaoYQ13sPQ/exec';
  const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzC8KP13XkDuAFqyRwtTIMMIm1m5euWYEis-oIqITg0S2ojlN6iR8PcDys_JaoYQ13sPQ/exec'; // <-- set me (leave empty to stay in demo mode)
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  async function loginAttempt_(username, password){
    const u = String(username||'').trim();
    const p = String(password||'').trim();
    if(!u) return { ok:false, msg:'חסר שם משתמש' };
    if(!p) return { ok:false, msg:'חסרה סיסמה' };

    const resp = await apiCall('login', { username: u, password: p });
    if(!resp || !resp.ok){
      const msg = (resp && (resp.msg || resp.error)) ? String(resp.msg || resp.error) : 'שגיאת התחברות';
      return { ok:false, msg };
    }

    const user = resp.user || resp.data || resp.profile || resp.identity || resp;
    if(!user || !user.username){
      return { ok:false, msg:'שגיאת התחברות (USER)' };
    }

    setSession_({ username: user.username, at: Date.now() });
    setIdentity_(user);
    showApp_();
  ...
  (function bootAuth_(){
    ...
    // Restore session if exists (server-validated)
    (async () => {
      const sess = getSession_();
      const uname = sess && sess.username ? String(sess.username) : '';
      if(uname){
        try{
          const resp = await apiCall('getUser', { username: uname });
          const user = resp && resp.ok ? (resp.user || resp.data || resp.profile || resp.identity) : null;
          if(user && user.username && user.active !== false){
            setIdentity_(user);
            showApp_();
            const r = location.hash ? location.hash.replace('#','').trim() : '';
            setRoute(r || 'customers');
