...
(() => {
  ...
  function syncDetachedTopbarVisibility(){
    try{
      const hasWizard = !!document.body.classList.contains("lcWizardOpen");
      const hasCustomerFile = !!document.body.classList.contains("lcCustomerFileOpen");
      const hasNewCustomerType = !!document.body.classList.contains("lcNewCustomerTypeOpen");
      const shouldHide = hasWizard || hasCustomerFile || hasNewCustomerType;
      document.body.classList.toggle("lcDetachedTopbarHidden", shouldHide);
    }catch(_e){}
  }
...
init(){
    ...
    close(){
      this.isOpen = false;
      document.body.classList.remove("lcWizardOpen");
      syncDetachedTopbarVisibility();
      this.els.wrap.classList.remove("is-open");
      this.els.wrap.setAttribute("aria-hidden","true");
      try { ChatUI?.syncVisibility?.('wizard'); } catch(_e) {}
