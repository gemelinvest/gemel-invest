...
(() => {
  ...
  const BUILD = "20260319-customer-wizard-fullscreen-premium";
  ...
  const Auth = {
    ...
    async _submit(){
      ...
      if (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin)) {
        ...
        try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
        await App.reloadSessionState({ silent:true, keepOpenCustomerFile:true });
        this.unlock();
        AutoRefresh.bindLifecycle();
        AutoRefresh.start();
        UI.applyRoleUI();
        UI.renderAuthPill();
...
// nav
      ...
      on(this.els.btnCustomersRefresh, "click", async () => {
        await AutoRefresh.refreshNow({ silent:true, force:true });
        CustomersUI.render();
      });
      on(this.els.proposalsSearch, "input", () => ProposalsUI.render());
      on(this.els.btnProposalsRefresh, "click", async () => {
        await AutoRefresh.refreshNow({ silent:true, force:true });
        ProposalsUI.render();
      });
this.applyRoleUI();
    ...
    goView(view){
      ...
      if(Auth.current) {
        AutoRefresh.start();
        if(safe === "customers" || safe === "proposals") {
          void AutoRefresh.refreshNow({ silent:true, force:true });
        }
      }
    },
...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    openById(id, opts={}){
      ...
      try{
        ...
        this.els.wrap.classList.add("is-open");
        this.els.wrap.setAttribute("aria-hidden","false");
        if(!opts.preserveScroll) document.body.style.overflow = "hidden";
      }catch(err){
  ...
  const Wizard = {
    ...
    localities: [
      "אבו גוש","אבו סנאן","אבן יהודה","אום אל-פחם","אופקים","אור יהודה","אור עקיבא","אורנית","אזור","אילת","אכסאל","אליכין","אלעד","אלפי מנשה","אלקנה","אריאל","אשדוד","אשקלון","אעבלין","באקה אל-גרבייה","באר יעקב","באר שבע","בועיינה-נוג'ידאת","בוקעאתא","ביר אל-מכסור","בית ג'ן","בית דגן","בית שאן","בית שמש","ביתר עילית","בני ברק","בנימינה-גבעת עדה","בסמת טבעון","בענה","בת חפר","בת ים","ג'דיידה-מכר","ג'ולס","ג'לג'וליה","ג'סר א-זרקא","גבעת שמואל","גבעת זאב","גבעתיים","גדרה","גוש עציון","גן יבנה","גני תקווה","דאלית אל-כרמל","דבורייה","דימונה","דיר אל-אסד","הוד השרון","הר אדר","הרצליה","זכרון יעקב","זמר","זרזיר","חדרה","חולון","חורה","חורפיש","חיפה","חצור הגלילית","חריש","טבריה","טובא-זנגרייה","טורעאן","טייבה","טירה","טירת כרמל","טמרה","יבנה","יהוד-מונוסון","יסוד המעלה","יפיע","יקנעם עילית","ירוחם","ירושלים","ירכא","כאבול","כאוכב אבו אל-היג'א","כוכב יאיר צור יגאל","כסיפה","כסרא-סמיע","כעביה-טבאש-חג'אג'רה","כפר ברא","כפר ורדים","כפר יאסיף","כפר יונה","כפר כנא","כפר מנדא","כפר מצר","כפר סבא","כפר קאסם","כפר קרע","כרמיאל","לב השרון","להבים","לוד","לקיה","מאור","מבשרת ציון","מגדל העמק","מג'ד אל-כרום","מג'דל שמס","מודיעין עילית","מודיעין-מכבים-רעות","מזכרת בתיה","מזרעה","מטולה","מכבים רעות","מעלה אדומים","מעלה אפרים","מעלות-תרשיחא","מעיליא","משהד","מיתר","מצפה רמון","נהריה","נוף הגליל","נחף","נס ציונה","נצרת","נצרת עילית","נשר","נתיבות","נתניה","סאג'ור","סביון","סח'נין","ע'ג'ר","עומר","עיילבון","עילוט","עין מאהל","עכו","עספיא","עפולה","עראבה","ערד","ערערה","ערערה-בנגב","פוריידיס","פסוטה","פקיעין","פרדס חנה-כרכור","פרדסיה","פתח תקווה","צור הדסה","צפת","קדומים","קדימה-צורן","קלנסווה","קציר-חריש","קצרין","קריית אונו","קריית ארבע","קריית אתא","קריית ביאליק","קריית גת","קריית ים","קריית טבעון","קריית מוצקין","קריית מלאכי","קריית עקרון","קריית שמונה","קרני שומרון","ראמה","ראש העין","ראש פינה","ראשון לציון","רהט","רחובות","ריינה","רכסים","רמלה","רמת גן","רמת השרון","רמת ישי","רעננה","שבלי - אום אל-גנם","שגב-שלום","שדרות","שוהם","שלומי","שעב","שפרעם","תל אביב-יפו","תל מונד","תל שבע"
    ],
...
init(){
    ...
    open(){
      ...
      this.els.wrap.setAttribute("aria-hidden","false");
      document.body.style.overflow = "hidden";
      document.body.classList.add("modal-open", "lcWizardOpen");
      this.render();
      setTimeout(() => {
    ...
    nextStep(){
      ...
      const isPerInsuredStep = (this.step >= 2 && this.step <= 4);
      if(isPerInsuredStep){
        const active = this.getActive();
        const incomplete = (this.insureds || []).filter(ins => !this.isStepCompleteForInsured(this.step, ins));
        if(incomplete.length){
          const nextIns = incomplete.find(ins => String(ins.id) !== String(active?.id)) || incomplete[0];
          if(nextIns && String(nextIns.id) !== String(active?.id)){
            this.activeInsId = nextIns.id;
            const stepTitle = this.steps.find(s => s.id === this.step)?.title || "השלב הנוכחי";
            this.setHint(`לפני שממשיכים, יש להשלים את שלב "${stepTitle}" עבור: ${nextIns.label}`);
            this.render();
            return;
          }
        }
      ...
      if(!v.ok){
        if(isPerInsuredStep){
          const active = this.getActive();
          const incomplete = (this.insureds || []).filter(ins => !this.isStepCompleteForInsured(this.step, ins));
          const firstBad = incomplete[0] || active;
          if(firstBad && String(firstBad.id) !== String(active?.id)){
            this.activeInsId = firstBad.id;
            this.render();
          }
        }
      ...
      this.step += 1;
      if(this.step === 5){
        this.activeInsId = this.insureds[0]?.id || this.activeInsId || null;
      }
      this.handleStepEntry(fromStep, this.step);
      this.setHint("");
      this.render();
...
if(path === "birthDate"){
  ...
  return;
      ...
      this.bindLocalityAutocomplete(ins);
...
// open health covers drawer (Health only)
    ...
    getLocalitySuggestions(term){
      const q = this.normalizeLocalitySearch(term);
      const list = Array.isArray(this.localities) ? this.localities : [];
      if(!q) return list.slice(0, 20);
      const exact = [];
      const starts = [];
      const includes = [];
      list.forEach(item => {
        const txt = this.normalizeLocalitySearch(item);
        if(!txt.includes(q)) return;
        if(txt === q) exact.push(item);
        else if(txt.startsWith(q)) starts.push(item);
        else includes.push(item);
    ...
    renderLocalitySuggestions(term, currentValue){
      const cur = safeTrim(currentValue);
      const items = this.getLocalitySuggestions(term);
      if(!items.length){
        return `<button type="button" class="lcLocalityOption is-empty" data-locality-empty="1">לא נמצאה התאמה. אפשר להמשיך להקליד ידנית.</button>`;
      }
      return items.map(item => {
        const active = (safeTrim(item) === cur) ? " is-active" : "";
        return `<button type="button" class="lcLocalityOption${active}" data-locality-value="${escapeHtml(item)}">${escapeHtml(item)}</button>`;
      }).join("");
    },

    bindLocalityAutocomplete(ins){
      const input = $("#lcLocalityInput", this.els.body);
      const menu = $("#lcLocalityMenu", this.els.body);
      if(!input || !menu) return;
      ...
      const refreshMenu = () => {
        menu.innerHTML = this.renderLocalitySuggestions(input.value, ins.data.city || "");
      };
      const choose = (val) => {
        const picked = safeTrim(val);
        input.value = picked;
        ins.data.city = picked;
      ...
      on(input, "focus", () => { refreshMenu(); openMenu(); });
      on(input, "click", () => { refreshMenu(); openMenu(); });
      on(input, "input", () => { ins.data.city = safeTrim(input.value); this.scheduleZipLookup(ins); refreshMenu(); openMenu(); });
      on(input, "keydown", (ev) => {
        const options = $$("[data-locality-value]", menu);
        const current = menu.querySelector(".lcLocalityOption.is-hover");
        let idx = current ? options.indexOf(current) : -1;
        if(ev.key === "ArrowDown"){
          ev.preventDefault();
          if(!menu.classList.contains("is-open")){ refreshMenu(); openMenu(); }
        ...
        if(ev.key === "Enter" && menu.classList.contains("is-open")){
          const picked = menu.querySelector(".lcLocalityOption.is-hover") || menu.querySelector("[data-locality-value]");
          if(picked){
            ev.preventDefault();
            choose(picked.getAttribute("data-locality-value") || picked.textContent || "");
          }
          return;
        }
        if(ev.key === "Escape") closeMenu();
      ...
      on(menu, "mousedown", (ev) => {
        const btn = ev.target && ev.target.closest ? ev.target.closest("[data-locality-value]") : null;
        if(!btn) return;
        ev.preventDefault();
        choose(btn.getAttribute("data-locality-value") || "");
      });
      on(document, "click", (ev) => {
        if(!this.els.body || !this.els.body.contains(input)) return;
        const inside = ev.target === input || menu.contains(ev.target);
    ...
    getStep1Questions(ins){
      ...
      questions.push(
        ...
        {
          key:'city',
          title:'באיזה יישוב הלקוח גר?',
          sub: isChild ? 'היישוב נמשך אוטומטית מהמבוטח הראשי.' : 'היישוב ישמש גם לחישוב המיקוד האוטומטי.',
          required:!isChild,
          render:() => isChild
            ? `<div class="lcStep1InfoCard"><div class="lcStep1InfoCard__label">יישוב</div><div class="lcStep1InfoCard__value">${escapeHtml(inherited('city') || 'טרם מולא במבוטח הראשי')}</div><div class="lcStep1InfoCard__sub">בקטין השדה מוצג לקריאה בלבד.</div></div>`
            : `<div class="field"><label class="label">יישוב</label><div class="lcLocalityWrap"><input class="input lcLocalityInput" id="lcLocalityInput" type="text" data-bind="city" value="${escapeHtml(d.city || '')}" placeholder="התחל להקליד יישוב…" autocomplete="off" aria-autocomplete="list" aria-expanded="false" /><div class="lcLocalityMenu" id="lcLocalityMenu">${this.renderLocalitySuggestions(d.city || '', d.city || '')}</div></div><div class="help">חיפוש חכם לפי יישובים בישראל. אם לא נמצאה התאמה, אפשר להמשיך להקליד ידנית.</div></div>`
        },
        {
          key:'street',
          title:'מה שם הרחוב?',
        ...
        },
        {
          ...
          sub:'המיקוד יחושב לפי יישוב, רחוב ומספר בית.',
          required:false,
    ...
    renderStep5(){
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        const isMedicare = this.isMedicareCompany(p.company);
        const insured = this.insureds.find(x => x.id === p.insuredId);
        const insuredLabel = showCoupleBadge ? "זוג" : safeTrim((insured && insured.label) || "מבוטח");
        const policyTitle = escapeHtml(p.type || p.company || "פוליסה חדשה");
        const companyLabel = escapeHtml(p.company || "חברה");
        ...
        const subtitleBits = [
          companyLabel,
          escapeHtml(fmtDate(p.startDate)),
          escapeHtml(insuredLabel)
        ].filter(Boolean);

        return `<div class="lcPolCard lcPolCard--designer" data-pol="${p.id}">
          <div class="lcPolCard__row">
            <details class="lcPolicyActionsMenu">
              <summary class="lcPolicyActionsMenu__summary">פעולות <span>⋯</span></summary>
              <div class="lcPolicyActionsMenu__panel">
                ...
                ${p.type === "בריאות" ? `<button type="button" class="lcIconActionBtn" data-editpol="${p.id}" aria-label="כיסויים"><span class="lcIconActionBtn__icon">🩺</span><span>כיסויים</span></button>` : ``}
              </div>
            </details>
            <div class="lcPolCard__main">
              <div class="lcPolTitle">${policyTitle}${showCoupleBadge ? ` <span class="lcChip">זוגי</span>` : ``}</div>
              <div class="lcPolSub">${subtitleBits.join(" · ")}</div>
              ${p.type === "בריאות" ? `<div class="lcPolMicroNote">${escapeHtml(coverSummary)}</div>` : ``}
            </div>
            <div class="lcPolCard__logoBox">${logo}</div>
          </div>
          <div class="lcPolCard__foot">
            <div class="lcPolCard__price"><span>לפרמיה</span><strong>${fmtMoney(p.premiumMonthly)}</strong></div>
            ${(!isMedicare && (p.type === "ריסק" || p.type === "ריסק משכנתא") && p.pledge) ? `<div class="lcPolCard__flag">שיעבוד</div>` : ``}
          </div>
        </div>`;
      };

      const totalAddedPolicies = policies.length;
      ...
      const groupsHtml = this.insureds.map(ins => {
        ...
        const hasMultipleGroups = this.insureds.filter(x => (byIns[x.id] || []).length).length > 1;
        return `<div class="lcWSection lcPolAddedGroup">
          ${hasMultipleGroups ? `<div class="lcPolAddedGroup__title">${escapeHtml(ins.label)}</div>` : ``}
          <div class="lcPolList lcPolList--designer">${cards}</div>
        </div>`;
      }).join("");
      ...
      const form = `<div class="lcWSection lcPolBuilderSection">
        <div class="lcPolHero">
          <div class="lcPolHero__title">${this.editingPolicyId ? "עריכת פוליסה" : "הוספת פוליסה חדשה"}</div>
          <div class="lcPolHero__sub">כאן ניתן להוסיף פוליסה עבור מבוטח או בן משפחה.</div>
        </div>

        <div class="lcPolDesignerShell">
          <div class="lcPolDesignerTop">
            <div class="lcPolDesignerTop__title">צרף פוליסה חדשה</div>
            <div class="lcPolDesignerTop__controls">
              <select class="lcSelect lcPolDesignerTop__select" data-pdraft="insuredId"${(d.insuredMode==="couple")?" disabled":""}>
                ${insuredOpts}
              </select>
            </div>
          </div>

          ${spouse ? `<div class="lcPolModeRow">
            <button type="button" class="lcBtn lcBtn--ghost ${d.insuredMode==="couple"?"is-active":""}" data-pdraftmode="couple">פוליסה זוגית</button>
          ...
          <div class="lcPolDesignerBody">
            <div class="lcPolDesignerSide">
              ${isMedicare ? `<div class="lcField lcPolField">
                <label class="lcLabel">מוצר ביטוח</label>
                <div class="lcPolStaticValue lcPolControlShell">מדיקר</div>
              </div>` : `<div class="lcField lcPolField">
                <label class="lcLabel">מוצר ביטוח</label>
              ...
              <div class="lcPolBuilderActions lcPolBuilderActions--designer">
                ${this.editingPolicyId ? `<button type="button" class="lcBtn" data-cancel-editpol="1">ביטול עריכה</button>` : ``}
                <button type="button" class="lcBtn lcBtn--primary lcBtn--block" data-addpol="1">${this.editingPolicyId ? "שמור שינויים" : "הוסף פוליסה"}</button>
              </div>
            </div>

            <div class="lcPolDesignerCompanies">
              <div class="lcPolDesignerCompanies__label">בחר חברת ביטוח</div>
              <div class="lcCoGrid lcCoGrid--designer">${companyCards}</div>
            </div>
          </div>

          <div class="lcPolDesignerExtras">
      ...
      const res = form + `<div class="lcWSection lcPolAddedSection">
        <div class="lcPolAddedHead">
          <div>
            <div class="lcWTitle">פוליסות שנוספו</div>
            <div class="muted small">${totalAddedPolicies ? `נוספו ${totalAddedPolicies} פוליסות חדשות` : "עדיין לא נוספו פוליסות חדשות."}</div>
          </div>
        </div>
        ${emptyNote}
      </div>` + groupsHtml;
...
// ---------- Step 6 ----------
    ...
    renderOperationalReport(){
      ...
      return `<div class="lcReportDoc">
        ...
        <section class="lcReportSection"><div class="lcReportSection__title">פרטי לקוח</div><div class="lcReportGrid">${this.compactReportFields(primary, [['firstName','שם פרטי'],['lastName','שם משפחה'],['idNumber','תעודת זהות'],['birthDate','תאריך לידה'],['gender','מגדר'],['maritalStatus','מצב משפחתי'],['phone','טלפון'],['email','אימייל'],['city','יישוב'],['street','רחוב'],['houseNumber','מספר בית'],['zip','מיקוד'],['clinic','קופת חולים'],['shaban','שב״ן'],['occupation','עיסוק'],['heightCm','גובה'],['weightKg','משקל'],['bmi','BMI']])}</div></section>
  ...
  const MirrorsUI = {
    ...
    renderVerifyInfoSection(rec){
      ...
      <div class="mirrorsEditGrid">
        ...
        <label class="field"><span class="label">יישוב</span><input class="input" data-mirror-edit-field="city" type="text" value="${escapeHtml(data.city || '')}" /></label>
...
const SystemRepairUI = {
  ...
  const AutoRefresh = {
    ...
    shouldRun(){
      return !!Auth.current && document.visibilityState !== "hidden";
    },

    getCurrentView(){
      try{
        const visible = document.querySelector('.view.is-visible');
        return visible ? String(visible.id || '').replace(/^view-/, '') : '';
      }catch(_e){
        return '';
      }
    },
    ...
    isCustomerFileOpen(){
      return !!(CustomersUI?.els?.wrap && CustomersUI.els.wrap.classList.contains('is-open'));
    },

    async refreshNow(opts={}){
      if(!Auth.current) return { ok:false, error:'NO_SESSION' };
      if(this.inFlight && !opts.force) return { ok:false, error:'REFRESH_IN_FLIGHT' };
      this.inFlight = true;
      try{
        const r = await App.reloadSessionState({ silent: !!opts.silent, keepOpenCustomerFile: true });
        if(r?.ok){
          const view = this.getCurrentView();
          if(view === 'customers') CustomersUI.render();
          if(view === 'proposals') ProposalsUI.render();
          if(this.isCustomerFileOpen() && CustomersUI.currentId){
            const rec = CustomersUI.byId(CustomersUI.currentId);
            if(rec) CustomersUI.openById(CustomersUI.currentId, { preserveScroll:true, silent:true });
            else CustomersUI.close();
          }
          this.lastTickAt = Date.now();
        }
        return r;
      }catch(err){
        console.warn('AUTO_REFRESH_ERROR:', err?.message || err);
    ...
    start(opts={}){
      if(opts.delay && Number(opts.delay) > 0) this.delay = Number(opts.delay);
      this.stop();
      if(!Auth.current) return;
      this.intervalId = window.setInterval(() => {
        if(!this.shouldRun()) return;
        void this.refreshNow({ silent:true });
    ...
    bindLifecycle(){
      ...
      document.addEventListener('visibilitychange', () => {
        ...
        if(document.visibilityState === 'visible'){
          this.start();
          void this.refreshNow({ silent:true, force:true });
        }
      });
  ...
  const App = {
    ...
    async reloadSessionState(opts={}){
      if(!Auth.current) return { ok:false, error:"NO_SESSION" };
      const keepOpenCustomerFile = opts?.keepOpenCustomerFile !== false;
      const silent = !!opts?.silent;
      const currentCustomerId = keepOpenCustomerFile ? CustomersUI.currentId : null;
      const customerFileWasOpen = keepOpenCustomerFile && !!(CustomersUI?.els?.wrap && CustomersUI.els.wrap.classList.contains("is-open"));
      ...
      if (r.ok) {
        ...
        if(customerFileWasOpen && currentCustomerId){
          const refreshedRec = CustomersUI.byId(currentCustomerId);
          if(refreshedRec) CustomersUI.openById(currentCustomerId, { preserveScroll:true, silent:true });
          else CustomersUI.close();
        }
      } else {
