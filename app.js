...
(() => {
  ...
  const BUILD = "20260406-v58-goal-live-premium";
  ...
  const DashboardUI = {
    ...
    buildMetrics(){
      ...
      return {
        ...
        prevNetPremium: prevAgg.netPremium,
        topDayValue: Math.max(0, ...this.buildDailySeries(customersMonth).map((item) => item.premium || 0))
      };
    },

    animateGoalCounters(root){
      const scope = root || this.els?.root;
      if(!scope) return;
      if(!this._goalCounterCache || typeof this._goalCounterCache !== 'object') this._goalCounterCache = {};
      const formatByKind = (kind, value) => {
        const num = Number(value) || 0;
        if(kind === 'pct') return this.formatPct(num);
        if(kind === 'int') return Math.round(num).toLocaleString('he-IL');
        return this.formatMoney(num);
      };
      scope.querySelectorAll('[data-counter-value]').forEach((el) => {
        const target = Number(el.getAttribute('data-counter-value') || 0) || 0;
        const kind = safeTrim(el.getAttribute('data-counter-kind')) || 'money';
        const key = safeTrim(el.getAttribute('data-counter-key')) || '';
        const previous = key && Number.isFinite(Number(this._goalCounterCache[key])) ? Number(this._goalCounterCache[key]) : 0;
        const hasRendered = el.dataset.counterAnimated === '1';
        const from = hasRendered ? previous : 0;
        const duration = 820;
        const start = performance.now();
        const tick = (now) => {
          const progress = Math.min(1, (now - start) / duration);
          ...
          el.textContent = formatByKind(kind, value);
          if(progress < 1){
            requestAnimationFrame(tick);
          } else {
            el.textContent = formatByKind(kind, target);
            el.dataset.counterAnimated = '1';
            if(key) this._goalCounterCache[key] = target;
          }
        };
        requestAnimationFrame(tick);
      });
    ...
    animateGoalProgress(root){
      const scope = root || this.els?.root;
      if(!scope) return;
      scope.querySelectorAll('[data-goal-progress-fill]').forEach((el) => {
        const target = Math.max(0, Math.min(100, Number(el.getAttribute('data-goal-progress') || 0) || 0));
        el.style.width = '0%';
    ...
    render(){
        ...
        <section class="bankDash bankDash--cleanTop">
          ...
          <article class="bankGoal card bankGoal--${metrics.targetTone}">
            <div class="bankGoal__shell">
              <div class="bankGoal__header">
                <div class="bankGoal__titleWrap">
                  <div class="bankGoal__title">ביצועים מול יעד</div>
                  <div class="bankGoal__percentSub">${metrics.targetValue > 0 ? `מדד ביצוע חודשי / ${escapeHtml(metrics.monthLabel)}` : `ממתין להגדרת יעד חודשי`}</div>
                </div>
                <div class="bankGoal__progressHero">
                  <div class="bankGoal__percent" data-counter-kind="pct" data-counter-key="goal-pct" data-counter-value="${escapeHtml(String(metrics.targetPct))}">${escapeHtml(this.formatPct(metrics.targetPct))}</div>
                  <div class="bankGoal__percentLabel">${metrics.targetValue > 0 ? 'תכנון חודשי פעיל' : 'טרם הוגדר יעד'}</div>
                </div>
              </div>
              ...
              <div class="bankGoal__topStats">
                <div class="bankGoal__topStat">
                  <span>יעד חודשי</span>
                  <strong data-counter-kind="money" data-counter-key="goal-target" data-counter-value="${escapeHtml(String(metrics.targetValue || 0))}">${metrics.targetValue > 0 ? escapeHtml(this.formatMoney(metrics.targetValue)) : '₪0'}</strong>
                  <small>${escapeHtml(metrics.monthLabel)}</small>
                </div>
                <div class="bankGoal__topStat bankGoal__topStat--main">
                  <span>בפועל</span>
                  <strong data-counter-kind="money" data-counter-key="goal-actual" data-counter-value="${escapeHtml(String(metrics.netPremium || 0))}">${escapeHtml(this.formatMoney(metrics.netPremium))}</strong>
                  <small class="bankGoal__delta ${Number(metrics.netDelta) > 0 ? 'is-up' : Number(metrics.netDelta) < 0 ? 'is-down' : 'is-flat'}">${Number(metrics.netDelta) > 0 ? '▲' : Number(metrics.netDelta) < 0 ? '▼' : '•'} ${escapeHtml(this.formatPct(Math.abs(Number(metrics.netDelta) || 0)))} מול חודש קודם</small>
                </div>
                <div class="bankGoal__topStat">
                  <span>פער</span>
                  <strong data-counter-kind="money" data-counter-key="goal-gap" data-counter-value="${escapeHtml(String(metrics.targetRemaining || 0))}">${escapeHtml(this.formatMoney(metrics.targetRemaining || 0))}</strong>
                  <small>${metrics.targetRemaining > 0 ? 'נשאר להשלמה' : 'היעד הושלם'}</small>
                </div>
              </div>

              <div class="bankGoal__progressWrap">
                <div class="bankGoal__progressTrack">
                  <span data-goal-progress-fill data-goal-progress="${Math.min(metrics.targetPct, 100)}"></span>
                  <em class="bankGoal__progressCap">${metrics.targetValue > 0 ? escapeHtml(this.formatMoney(metrics.targetValue)) : '₪0'}</em>
                </div>
                <div class="bankGoal__metaRow">
                  <span>${escapeHtml(this.formatPct(metrics.targetPct))} מתוך היעד החודשי</span>
                  <strong>${escapeHtml(this.formatMoney(metrics.prevNetPremium || 0))} חודש קודם</strong>
                </div>
              </div>
            </div>
      ...
      this.animateGoalCounters(this.els.root);
      this.animateGoalProgress(this.els.root);
    }
  };
