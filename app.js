...
txt.textContent = "";
  ...
  const Wizard = {
    ...
    healthDeclarationQuestions: [
      { key:"smoking", title:"עישון", text:"האם המבוטח/ת מעשן/ת כיום או עישן/ה בשנתיים האחרונות?", yesLabel:"כן", noLabel:"לא", followup:"אם כן, פרט/י מה, תדירות ומתי לאחרונה." },
      { key:"medications", title:"תרופות קבועות", text:"האם המבוטח/ת נוטל/ת תרופות קבועות או נמצא/ת במעקב רפואי קבוע?", yesLabel:"כן", noLabel:"לא", followup:"אם כן, פרט/י אילו תרופות / מעקבים." },
      { key:"hospitalizations", title:"אשפוזים וניתוחים", text:"האם בוצע אשפוז, ניתוח, או קיימת המלצה לניתוח / אשפוז ב-5 השנים האחרונות?", yesLabel:"כן", noLabel:"לא", followup:"אם כן, פרט/י תאריך, סיבה ותוצאה." },
      { key:"chronic", title:"מחלות כרוניות", text:"האם קיימת או הייתה מחלה כרונית, מחלת לב, סוכרת, לחץ דם, אסתמה, מחלה אוטואימונית או כל מצב רפואי משמעותי אחר?", yesLabel:"כן", noLabel:"לא", followup:"אם כן, פרט/י אבחנה, שנה ומצב נוכחי." },
      { key:"tests", title:"בדיקות והפניות", text:"האם יש בדיקות חריגות, ממצאים רפואיים, הפניה לבירור, או תוצאות ממתינות?", yesLabel:"כן", noLabel:"לא", followup:"אם כן, פרט/י אילו בדיקות / ממצאים." },
      { key:"disability", title:"נכות / אובדן כושר", text:"האם קיימת נכות, מגבלה רפואית, אובדן כושר עבודה, או בקשה/קצבה מביטוח לאומי / גוף אחר?", yesLabel:"כן", noLabel:"לא", followup:"אם כן, פרט/י סוג מגבלה / אחוזי נכות / סטטוס." },
      { key:"pregnancy", title:"הריון", text:"למבוטחת בלבד: האם יש הריון קיים או בירור הריון בסיכון? אם לא רלוונטי, סמנו לא.", yesLabel:"כן", noLabel:"לא", followup:"אם כן, פרט/י שבוע הריון וסטטוס." },
      { key:"extra", title:"מידע נוסף", text:"האם יש מידע רפואי נוסף שלדעתך צריך לדווח עליו לחברת הביטוח?", yesLabel:"כן", noLabel:"לא", followup:"אם כן, פרט/י כאן בקצרה." }
    ],
init(){
    ...
    renderTabs(){
      ...
      // Steps 5–7 are case-level (not per-insured), step 8 returns to insured-level
    ...
    renderBody(){
      ...
      const isCaseLevel = (this.step >= 5 && this.step !== 8);
      ...
      else body = this.renderStep8(ins);

      this.els.body.innerHTML = head + body;

      // bind generic input handlers
      if(this.step < 5 || this.step === 8) this.bindInputs(ins);
...
// ---------- Step 6 ----------
    ...
    renderStep8(ins){
      const d = ins.data || {};
      if(!d.healthDeclaration || typeof d.healthDeclaration !== "object") d.healthDeclaration = {};

      const cards = (this.healthDeclarationQuestions || []).map((q, index) => {
        const ans = d.healthDeclaration[q.key] || {};
        const answer = safeTrim(ans.answer || "");
        const details = safeTrim(ans.details || "");
        return `
          <div class="lcHDCard">
            <div class="lcHDCard__top">
              <div class="lcHDCard__index">שאלה ${index + 1}</div>
              <div class="lcHDCard__title">${escapeHtml(q.title || "")}</div>
            </div>
            <div class="lcHDCard__text">${escapeHtml(q.text || "")}</div>

            <div class="lcHDChoices" role="radiogroup" aria-label="${escapeHtml(q.title || "שאלה")}">
              <label class="lcHDChoice ${answer === "yes" ? "is-active" : ""}">
                <input type="radio" name="hd_${escapeHtml(ins.id)}_${escapeHtml(q.key)}" value="yes" data-bind="healthDeclaration.${escapeHtml(q.key)}.answer" ${answer === "yes" ? "checked" : ""} />
                <span>${escapeHtml(q.yesLabel || "כן")}</span>
              </label>
              <label class="lcHDChoice ${answer === "no" ? "is-active" : ""}">
                <input type="radio" name="hd_${escapeHtml(ins.id)}_${escapeHtml(q.key)}" value="no" data-bind="healthDeclaration.${escapeHtml(q.key)}.answer" ${answer === "no" ? "checked" : ""} />
                <span>${escapeHtml(q.noLabel || "לא")}</span>
              </label>
            </div>

            ${answer === "yes" ? `
              <div class="field" style="margin-top:12px">
                <label class="label">${escapeHtml(q.followup || "פירוט")}</label>
                <textarea class="input lcHDTextarea" data-bind="healthDeclaration.${escapeHtml(q.key)}.details" placeholder="פירוט חופשי">${escapeHtml(details)}</textarea>
              </div>
            ` : ``}
          </div>
        `;
      ...
      const answered = (this.healthDeclarationQuestions || []).filter(q => {
        const ans = safeTrim(d.healthDeclaration?.[q.key]?.answer || "");
        return ans === "yes" || ans === "no";
      }).length;

      return `
        <div class="lcWSection">
          <div class="lcWTitle">הצהרת בריאות · ${escapeHtml(ins.label)}</div>
          <div class="muted small">יש לענות על כל השאלות. כאשר מסמנים "כן" נפתח שדה פירוט חובה.</div>

          <div class="lcHDSummary">
            <div class="lcHDSummary__item"><span>מבוטח</span><strong>${escapeHtml(((safeTrim(d.firstName)+" "+safeTrim(d.lastName)).trim()) || ins.label)}</strong></div>
            <div class="lcHDSummary__item"><span>ת.ז.</span><strong>${escapeHtml(d.idNumber || "—")}</strong></div>
            <div class="lcHDSummary__item"><span>השלמה</span><strong>${answered}/${(this.healthDeclarationQuestions || []).length}</strong></div>
          </div>
        </div>

        <div class="lcHDList">${cards}</div>
      `;
    },
    ...
    validateStep(stepId){
      ...
      if(stepId === 8){
        const bad = this.insureds.filter(ins => !this.isStepCompleteForInsured(8, ins));
        if(!bad.length) return { ok:true };
        const names = bad.map(x => x.label).join(", ");
        return { ok:false, msg: "חסר מילוי חובה בהצהרת הבריאות עבור: " + names };
      }
    ...
    isStepCompleteForInsured(stepId, ins){
      ...
      if(stepId === 8){
        const qlist = Array.isArray(this.healthDeclarationQuestions) ? this.healthDeclarationQuestions : [];
        const hd = d.healthDeclaration || {};
        for(const q of qlist){
          const row = hd[q.key] || {};
          const answer = safeTrim(row.answer || "");
          if(answer !== "yes" && answer !== "no") return false;
          if(answer === "yes" && !safeTrim(row.details)) return false;
        }
        return true;
      }
