...
(() => {
  ...
  const BUILD = "20260324-operational-report-direct-pdf-1a";
  ...
  const pdfScriptLoadCache = new Map();
  function loadPdfScriptOnce(src){
    const url = safeTrim(src);
    if(!url) return Promise.reject(new Error('Missing PDF script src'));
    if(window.jspdf?.jsPDF || window.jsPDF) return Promise.resolve(true);
    if(pdfScriptLoadCache.has(url)) return pdfScriptLoadCache.get(url);

    const promise = new Promise((resolve, reject) => {
      const finish = () => {
        if(window.jspdf?.jsPDF || window.jsPDF) resolve(true);
        else reject(new Error('PDF engine unavailable after loading script'));
      };

      const existing = Array.from(document.scripts || []).find((script) => safeTrim(script.src) === url);
      if(existing){
        existing.addEventListener('load', finish, { once:true });
        existing.addEventListener('error', () => reject(new Error(`Failed to load PDF script: ${url}`)), { once:true });
      ...
      script.onerror = () => reject(new Error(`Failed to load PDF script: ${url}`));
      document.head.appendChild(script);
    });

    pdfScriptLoadCache.set(url, promise);
    return promise.catch((err) => {
      pdfScriptLoadCache.delete(url);
      throw err;
    });
  }
  ...
  async function ensurePdfEngineReady(){
    ...
    for(const src of sources){
      try{
        await loadPdfScriptOnce(src);
        if(window.jspdf?.jsPDF || window.jsPDF) return true;
      }catch(err){
        lastError = err;
      }
    }
    throw lastError || new Error('PDF engine unavailable');
  }
...
// ---------- Step 6 ----------
    ...
    _getOperationalPdfSections(payloadOverride){
      ...
      const addKeyValues = (title, rows, out) => {
        const cleanRows = (Array.isArray(rows) ? rows : []).map(([label, value]) => [safeTrim(label), safeTrim(value)]).filter(([label, value]) => label && value);
        if(cleanRows.length) out.push({ type:'kv', title, rows: cleanRows });
      };
      const sections = [];
      addKeyValues('פרטי דוח', [
        ['שם לקוח', customerName],
      ...
      insureds.forEach((ins, index) => {
        const d = ins?.data || {};
        addKeyValues(`נתונים רפואיים — ${safeTrim(ins?.label) || `מבוטח ${index + 1}`}`, [
          ['גובה', safeTrim(d.height)],
          ['משקל', safeTrim(d.weight)],
          ['BMI', safeTrim(d.bmi)],
          ['עישון', safeTrim(d.smokingStatus || d.smoking)]
      ...
      insureds.forEach((ins, index) => {
        ...
        policies.forEach((policy) => {
          const statusRaw = safeTrim(policy?.status || policy?.policyStatus || policy?.actionStatus || policy?.action) || '—';
          existingRows.push(`${label} | ${safeTrim(policy?.company) || '—'} | ${safeTrim(policy?.type || policy?.product) || '—'} | מס' פוליסה: ${safeTrim(policy?.policyNumber) || '—'} | פרמיה: ${safeTrim(policy?.premium) || '—'} | סטטוס: ${statusRaw}`);
        });
      });
      sections.push({ type:'list', title:'פוליסות קיימות', items: existingRows.length ? existingRows : ['לא נוספו פוליסות קיימות'] });
      ...
      const getPolicyPledgeSummarySafe = (policy) => {
        const type = safeTrim(policy?.type || policy?.product || '');
        if(!(type === 'ריסק' || type === 'ריסק משכנתא') || !policy?.pledge) return '';
        const bank = policy?.pledgeBank && typeof policy.pledgeBank === 'object' ? policy.pledgeBank : {};
        return [
          ['סוג שיעבוד', 'מוטב בלתי חוזר'],
          ['שם בנק', safeTrim(bank.bankName)],
          ...
          ['שנות משכנתא', safeTrim(policy?.mortgageYears)],
          ['כתובת נכס', safeTrim(policy?.mortgageAddress)]
        ].filter(([,value]) => safeTrim(value)).map(([k,v]) => `${k}: ${v}`).join(' | ');
      };
      const getPolicyInsuredLabelSafe = (policy) => {
        if(policy?.insuredMode === 'couple'){
          const primaryLabel = safeTrim(insureds?.[0]?.label) || 'מבוטח ראשי';
      ...
      const newPolicyItems = newPolicies.map((policy, index) => {
        const coverItems = getPolicyCoverItemsSafe(policy);
        const pledge = getPolicyPledgeSummarySafe(policy);
        const lines = [
          `פוליסה ${index + 1}: ${safeTrim(policy?.company) || '—'} | ${safeTrim(policy?.type || policy?.product) || '—'}`,
          `מבוטח: ${getPolicyInsuredLabelSafe(policy)} | פרמיה חודשית: ${safeTrim(policy?.premium) || '—'} | תחילת ביטוח: ${safeTrim(policy?.startDate) || '—'}`
        ];
        const sumVal = safeTrim(policy?.sumInsured || policy?.coverage || policy?.compensation || '');
        if(sumVal) lines.push(`סכום/כיסוי: ${sumVal}`);
        if(coverItems.length) lines.push(`כיסויים: ${coverItems.join(', ')}`);
        if(pledge) lines.push(`שיעבוד: ${pledge}`);
        return lines.join('\n');
      });
      sections.push({ type:'list', title:'פוליסות חדשות', items: newPolicyItems.length ? newPolicyItems : ['לא נוספו פוליסות חדשות'] });

      const payer = payload?.payer || payload?.payment || {};
      addKeyValues('אמצעי תשלום', [
        ['סוג תשלום', safeTrim(payer?.method || payer?.paymentMethod)],
        ['שם משלם', safeTrim(payer?.fullName || payer?.payerName)],
        ['תעודת זהות משלם', safeTrim(payer?.idNumber || payer?.payerId)],
        ['טלפון משלם', safeTrim(payer?.phone || payer?.payerPhone)],
        ['אימייל משלם', safeTrim(payer?.email || payer?.payerEmail)],
        ['בנק', safeTrim(payer?.bankName)],
        ['מספר בנק', safeTrim(payer?.bankNo)],
        ['סניף', safeTrim(payer?.branchNo)],
        ['חשבון', safeTrim(payer?.accountNo)],
        ['כרטיס אשראי', safeTrim(payer?.cardNumberMasked || payer?.cardNumber)],
        ['תוקף', safeTrim(payer?.expiry || payer?.cardExpiry)]
      ], sections);

      const healthAnswers = [];
      const pushHealth = (q, answer, details='') => {
        const qq = safeTrim(q), aa = safeTrim(answer), dd = safeTrim(details);
        if(!qq) return;
        healthAnswers.push(dd ? `${qq} — ${aa || '—'} | פירוט: ${dd}` : `${qq} — ${aa || '—'}`);
      };
      const health = payload?.healthDeclaration || payload?.health || {};
      if(Array.isArray(health?.questions)){
        health.questions.forEach((item) => pushHealth(item?.question || item?.label, item?.answerLabel || item?.answer, item?.details || item?.notes));
      }
      if(Array.isArray(payload?.healthFindings)){
        payload.healthFindings.forEach((item) => pushHealth(item?.question || item?.title, item?.status || item?.answer, item?.details || item?.description));
      }
      sections.push({ type:'list', title:'הצהרת בריאות', items: healthAnswers.length ? healthAnswers : ['לא נמצאו תשובות הצהרת בריאות להצגה'] });
    ...
    async exportOperationalPdf(payloadOverride){
      ...
      if(typeof JsPdfCtor !== 'function'){
        try{
          ...
          console.error('PDF engine load failed', err);
        }
      }
      if(typeof JsPdfCtor !== 'function'){
      ...
      try{
        ...
        const pushPage = () => {
          ...
          ctx.fillRect(margin, y, contentWidth, 8);
          y += 28;
          ctx.direction = 'rtl';
          ctx.textAlign = 'right';
          ctx.fillStyle = '#163254';
          ctx.font = '700 44px Arial, sans-serif';
          ctx.fillText('GEMEL INVEST', pageWidth - margin, y);
          y += 52;
          ctx.font = '800 40px Arial, sans-serif';
          ctx.fillText('דוח תפעולי', pageWidth - margin, y);
          y += 42;
          ctx.fillStyle = '#5b708c';
          ctx.font = '600 22px Arial, sans-serif';
          ctx.fillText(`${pack.meta.exportedAt}  |  ${pack.meta.agentName}`, pageWidth - margin, y);
          y += 36;
          ctx.fillText(`לקוח: ${pack.meta.customerName}`, pageWidth - margin, y);
          y += 32;
        ...
        const wrapLines = (text, font) => {
          ...
          rawLines.forEach((raw) => {
            const parts = safeTrim(raw).split(/\s+/).filter(Boolean);
            if(!parts.length){ out.push(''); return; }
            let line = '';
            parts.forEach((word) => {
        ...
        const drawSectionTitle = (title) => {
          ...
          ctx.strokeRect(margin, y, contentWidth, 44);
          ctx.fillStyle = '#204167';
          ctx.font = '800 26px Arial, sans-serif';
          ctx.fillText(title, pageWidth - margin - 16, y + 30);
          y += 58;
        };

        const drawLinesBlock = (lines) => {
          (Array.isArray(lines) ? lines : []).forEach((line) => {
            const wrapped = wrapLines(line, '600 22px Arial, sans-serif');
            wrapped.forEach((part) => {
              ensureSpace(lineHeight + 4);
              ctx.fillStyle = '#22354d';
              ctx.font = '600 22px Arial, sans-serif';
              ctx.fillText(part || ' ', pageWidth - margin, y);
              y += lineHeight;
            });
            y += paragraphGap;
          });
        ...
        pack.sections.forEach((section) => {
          ...
          if(section.type === 'kv'){
            section.rows.forEach(([label, value]) => {
              const wrapped = wrapLines(`${label}: ${value}`, '600 22px Arial, sans-serif');
              wrapped.forEach((part) => {
                ensureSpace(lineHeight + 4);
                ctx.fillStyle = '#22354d';
                ctx.font = '600 22px Arial, sans-serif';
