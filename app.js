...
if (!out.agents.length) {
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.oldPolicySumInsuredWrap = $("#oldPolicySumInsuredWrap");
      this.els.oldPolicySumInsured = $("#oldPolicySumInsured");
      this.els.oldPolicyCompensationWrap = $("#oldPolicyCompensationWrap");
      this.els.oldPolicyCompensation = $("#oldPolicyCompensation");
...
// Search (live filter on customers view)
      ...
      // Policy modal: conditional fields by type
      try{
        const typeSel = this.els.policyForm ? this.els.policyForm.querySelector('select[name="type"]') : null;
        const sumWrap = this.els.policySumInsuredWrap;
        const compWrap = this.els.policyCompensationWrap;

        const togglePolicyExtras = () => {
          const t = safeTrim(typeSel ? typeSel.value : "");
          const isRisk = (t === "ריסק" || t === "ריסק משכנתא");
          const isComp = (t === "מחלות סרטן" || t === "מחלות קשות");
          if(sumWrap) sumWrap.style.display = isRisk ? "" : "none";
          if(compWrap) compWrap.style.display = isComp ? "" : "none";

          // clear hidden fields
          if(!isRisk){
            const inp = this.els.policyForm ? this.els.policyForm.querySelector('input[name="sumInsured"]') : null;
            if(inp) inp.value = "";
          }
          if(!isComp){
            const inp = this.els.policyForm ? this.els.policyForm.querySelector('input[name="compensation"]') : null;
            if(inp) inp.value = "";
          }
        };

        if(typeSel && !typeSel.dataset.boundExtras){
          typeSel.dataset.boundExtras = "1";
          typeSel.addEventListener("change", togglePolicyExtras);
        }
        togglePolicyExtras();
      }catch(_){}
      ...
      // Policy modal submit
        ...
        const sumInsured = Number(String(fd.get("sumInsured") || "").replace(/[^\d.]/g, "")) || 0;
        const compensation = Number(String(fd.get("compensation") || "").replace(/[^\d.]/g, "")) || 0;
        ...
        if(type === "ריסק" || type === "ריסק משכנתא"){
          if(!sumInsured || sumInsured <= 0){
            notify("בריסק/ריסק משכנתא חובה למלא סכום ביטוח.", "warn");
            return;
          }
        }
        if(type === "מחלות סרטן" || type === "מחלות קשות"){
          if(!compensation || compensation <= 0){
            notify("במחלות סרטן/מחלות קשות חובה למלא סכום פיצוי.", "warn");
            return;
          }
        }

        c.policies ||= [];
        c.policies.unshift({ id: "p_" + uid(), type, company, premium, status: "active", renewAt, sumInsured:(sumInsured||0), compensation:(compensation||0) });
        c.updatedAt = nowISO();
...
async saveUserEditFromModal() {
  ...
  // ---------------------------
    ...
    try{
      ...
      const toggleProductExtras = () => {
        const p = safeTrim(el.oldPolicyProduct?.value);
        const isRisk = (p === "ריסק" || p === "ריסק משכנתא");
        const isComp = (p === "מחלות סרטן" || p === "מחלות קשות");
        if(el.oldPolicySumInsuredWrap) el.oldPolicySumInsuredWrap.style.display = isRisk ? "" : "none";
        if(el.oldPolicyCompensationWrap) el.oldPolicyCompensationWrap.style.display = isComp ? "" : "none";
        // clear hidden fields so we don't accidentally save old values
        if(!isRisk && el.oldPolicySumInsured) el.oldPolicySumInsured.value = "";
        if(!isComp && el.oldPolicyCompensation) el.oldPolicyCompensation.value = "";
      };

      if(el.oldPolicyProduct && !el.oldPolicyProduct.dataset.boundExtras){
        el.oldPolicyProduct.dataset.boundExtras = "1";
        el.oldPolicyProduct.addEventListener("change", toggleProductExtras);
      }
      toggleProductExtras();
      ...
      const addPolicy = () => {
        ...
        const sumInsured = Number(el.oldPolicySumInsured ? el.oldPolicySumInsured.value : 0) || 0;
        const compensation = Number(el.oldPolicyCompensation ? el.oldPolicyCompensation.value : 0) || 0;
        ...
        // conditional required fields by product
        if(product === "ריסק" || product === "ריסק משכנתא"){
          if(!sumInsured || sumInsured <= 0){
            notify("בריסק/ריסק משכנתא חובה למלא סכום ביטוח.", "warn");
            return;
          }
        }
        if(product === "מחלות סרטן" || product === "מחלות קשות"){
          if(!compensation || compensation <= 0){
            notify("במחלות סרטן/מחלות קשות חובה למלא סכום פיצוי.", "warn");
            return;
          }
        ...
        if(el.oldPolicySumInsured) el.oldPolicySumInsured.value = "";
        if(el.oldPolicyCompensation) el.oldPolicyCompensation.value = "";
        if(typeof toggleProductExtras === "function") toggleProductExtras();
