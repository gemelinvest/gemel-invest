/* LEAD CORE • Premium CRM (Sheets-only, locked URL)
   * VERSION: 2026-02-26_LOCKED_BASELINE_MAIL_V1
...
if (!out.agents.length) {
  ...
  const Storage = {
    ...
    // ---------------------------
  // Internal Mail (localStorage + external window)
  // ---------------------------
  const LS_MAIL_KEY = "LEAD_CORE_MAIL_V1";
  const Mail = {
    win: null,
    lastUnread: 0,
    ...
    _writeAll(db) {
      try { localStorage.setItem(LS_MAIL_KEY, JSON.stringify(db || { users: {} })); } catch(_) {}
    },

    _meKey() {
      try {
        const cur = Auth && Auth.current ? Auth.current : null;
        return safeTrim(cur?.name) || "";
      } catch(_) { return ""; }
    },

    _getInbox(userKey) {
      const db = this._readAll();
      const u = safeTrim(userKey);
      if(!u) return [];
      db.users ||= {};
      db.users[u] ||= { inbox: [] };
      const inbox = Array.isArray(db.users[u].inbox) ? db.users[u].inbox : [];
      return inbox;
    },

    getUnreadCount(userKey) {
      const inbox = this._getInbox(userKey);
      return inbox.filter(m => m && m.read !== true).length;
    },
    ...
    refreshBadge() {
      try {
        ...
        const unread = this.getUnreadCount(me);
        const had = Number(this.lastUnread || 0);
        this.lastUnread = unread;

        const btn = UI?.els?.btnMail;
        if(btn) {
          if(unread > 0) btn.classList.add("has-unread");
          else btn.classList.remove("has-unread");
        }

        // play sound only when unread increases and user is logged in
        if(me && unread > had) this._beep();
      } catch(_) {}
    },

    init() {
      // Update badge on storage events (messages arriving from other window/tab)
      window.addEventListener("storage", (e) => {
        try {
          if(String(e.key) !== LS_MAIL_KEY) return;
          this.refreshBadge();
        } catch(_) {}
      });
    },
    ...
    openWindow() {
      const w = 460, h = 640;
      const left = Math.max(20, Math.floor((screen.width - w) / 2));
      const top  = Math.max(20, Math.floor((screen.height - h) / 2));
      ...
      this.win = window.open("", "LeadCoreMail", `width=${w},height=${h},left=${left},top=${top},resizable=yes,scrollbars=yes`);
      if(!this.win) {
        notify("חסימת חלונות קופצים בדפדפן. אפשר לאפשר popups כדי לפתוח דואר פנימי.", "warn");
        return;
      }
...
<title>דואר פנימי • GEMEL INVEST</title>
<style>
  body{ margin:0; font-family: system-ui,-apple-system,"Segoe UI",Arial,"Noto Sans Hebrew",sans-serif; background:#f6f8ff; color:#0f172a; }
  .top{ position:sticky; top:0; background:white; border-bottom:1px solid rgba(15,23,42,.10); padding:12px 14px; display:flex; align-items:center; justify-content:space-between; gap:10px; }
  .title{ font-weight:950; }
  .btn{ border:1px solid rgba(15,23,42,.14); background:white; padding:8px 10px; border-radius:12px; cursor:pointer; font-weight:800; }
  .btnPrimary{ background: linear-gradient(135deg,#2b4b8a,#3a6cff); color:white; border-color: rgba(43,75,138,.22); }
  .wrap{ display:grid; grid-template-rows:auto 1fr auto; height:100vh; }
  .inbox{ padding:12px 14px; overflow:auto; }
  .msg{ background:white; border:1px solid rgba(15,23,42,.10); border-radius:14px; padding:10px 10px; margin-bottom:10px; cursor:pointer; }
  .msg.unread{ border-color: rgba(239,68,68,.35); box-shadow: 0 10px 18px rgba(239,68,68,.08); }
  .row{ display:flex; justify-content:space-between; gap:10px; align-items:center; }
  .meta{ font-size:12px; color: rgba(15,23,42,.65); margin-top:4px; }
  .subject{ font-weight:900; }
  .compose{ border-top:1px solid rgba(15,23,42,.10); background:white; padding:12px 14px; }
  label{ font-size:12px; font-weight:900; color: rgba(43,75,138,.85); display:block; margin:8px 0 6px; }
  input,textarea,select{ width:100%; box-sizing:border-box; padding:10px 10px; border-radius:12px; border:1px solid rgba(15,23,42,.14); outline:0; }
...
<div class="wrap">
  <div class="top">
    <div>
      <div class="title">דואר פנימי</div>
      <div class="meta" id="meLine">—</div>
    </div>
    <div class="row">
      <button class="btn" id="markRead">סמן הכל כנקרא</button>
  ...
  <div class="compose">
    <div class="subject" style="margin-bottom:6px">שליחת הודעה</div>
    <label>אל (נמען)</label>
    <select id="toSel"></select>

    <label>נושא</label>
    <input id="subj" type="text" placeholder="נושא..." />

    <label>תוכן</label>
    <textarea id="body" placeholder="כתוב הודעה..."></textarea>

    <div class="actions">
      <button class="btn btnPrimary" id="send">שלח</button>
      <button class="btn" id="clear">נקה</button>
    </div>
  </div>
</div>
...
<script>
  const safeTrim = (v)=> String(v??"").trim();
  const LS_MAIL_KEY = "LEAD_CORE_MAIL_V1";
  const readAll = ()=> { try{ const r=localStorage.getItem(LS_MAIL_KEY); return r?JSON.parse(r):{users:{}}; }catch(e){return {users:{}};} };
  const writeAll = (db)=> { try{ localStorage.setItem(LS_MAIL_KEY, JSON.stringify(db||{users:{}})); }catch(e){} };
  const me = ()=> { try{ return safeTrim(window.opener?.Auth?.current?.name) || ""; }catch(e){ return ""; } };
  const people = ()=> {
    try{
      const st = window.opener?.State?.data;
      const admins = [];
      const adminName = safeTrim(st?.meta?.adminAuth?.username);
      if(adminName) admins.push(adminName);
      const agents = Array.isArray(st?.agents)? st.agents.map(a=>safeTrim(a?.name)).filter(Boolean):[];
      return Array.from(new Set([...admins, ...agents]));
    }catch(e){ return []; }
  };
  const getInbox = (userKey)=> {
    const db = readAll();
    db.users ||= {};
    db.users[userKey] ||= { inbox: [] };
    const inbox = Array.isArray(db.users[userKey].inbox) ? db.users[userKey].inbox : [];
    return inbox;
  };
  const setInbox = (userKey, inbox)=> {
    const db = readAll();
    db.users ||= {};
    db.users[userKey] ||= { inbox: [] };
    db.users[userKey].inbox = inbox;
    writeAll(db);
  };

  const render = ()=> {
    const u = me();
    document.getElementById("meLine").textContent = u ? ("מחובר: " + u) : "לא מחובר במערכת הראשית";
    const box = document.getElementById("inbox");
    box.innerHTML = "";
    if(!u) {
      box.innerHTML = "<div class='meta'>פתח את המערכת הראשית והתחבר כדי לראות תיבה.</div>";
      return;
    }
    const inbox = getInbox(u);
    if(!inbox.length) {
      box.innerHTML = "<div class='meta'>אין הודעות.</div>";
      return;
    }
    inbox.forEach((m, i)=> {
      const d = document.createElement("div");
      d.className = "msg" + ((m && m.read!==true) ? " unread" : "");
      d.innerHTML = \`
        <div class="row">
          <div class="subject">\${m.subject||"ללא נושא"}</div>
          <div class="meta">\${new Date(m.at||Date.now()).toLocaleString("he-IL")}</div>
        </div>
        <div class="meta">מאת: \${m.from||"—"}</div>
        <div style="margin-top:8px; white-space:pre-wrap;">\${(m.body||"")}</div>
      \`;
      d.addEventListener("click", ()=> {
        const inbox2 = getInbox(u).map((x, idx)=> idx===i ? ({...x, read:true}) : x);
        setInbox(u, inbox2);
        render();
        try{ window.opener?.Mail?.refreshBadge?.(); }catch(e){}
      });
      box.appendChild(d);
    });
  };
  ...
  const fillPeople = ()=> {
    ...
    const other = list.find(x=>x && x!==u);
    if(other) sel.value = other;
  };

  document.getElementById("refresh").addEventListener("click", ()=> { fillPeople(); render(); });
  document.getElementById("markRead").addEventListener("click", ()=> {
    const u = me(); if(!u) return;
    const inbox = getInbox(u).map(m=> ({...m, read:true}));
    setInbox(u, inbox);
    render();
    try{ window.opener?.Mail?.refreshBadge?.(); }catch(e){}
  });
  ...
  document.getElementById("send").addEventListener("click", ()=> {
    const from = me() || "מערכת";
    const to = safeTrim(document.getElementById("toSel").value);
    const subject = safeTrim(document.getElementById("subj").value) || "ללא נושא";
    const body = safeTrim(document.getElementById("body").value);
    if(!to) return alert("בחר נמען");
    const msg = { id:"m_"+Math.random().toString(16).slice(2)+"_"+Date.now().toString(16), from, subject, body, at:new Date().toISOString(), read:false, to };
    const db = readAll(); db.users ||= {};
    db.users[to] ||= { inbox: [] };
    db.users[to].inbox = [msg, ...(db.users[to].inbox||[])];
    writeAll(db);
  ...
  setInterval(()=>{ render(); }, 2500);
      ...
      try {
        ...
        notify("שגיאה בפתיחת חלון דואר: " + String(e?.message||e), "warn");
      }
    }
  };
...
this.els.oldPolicyBankAgency = $("#oldPolicyBankAgency");
      ...
      this.els.btnMail = $("#btnMail");
      this.els.mailBadge = $("#mailBadge");
