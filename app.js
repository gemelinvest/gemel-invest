...
(() => {
  ...
  const BUILD = "20260331-har-excel-header-flex-v15";
...
// open health covers drawer (Health only)
    ...
    parseElementaryVehicleHarWorkbook(buffer, ins){
      ...
      const normalizeCell = (value) => safeTrim(value)
        .replace(/[   ]/g, ' ')
        .replace(/["'׳״`´]/g, '')
        .replace(/[()\[\]{}]/g, ' ')
        .replace(/[‐‑‒–—−]/g, '-')
      ...
      const isBannerRow = (row) => {
        if(!Array.isArray(row)) return false;
        const joined = row.map(normalizeCell).filter(Boolean).join(' | ');
        if(!joined) return true;
        return /התיק הביטוחי|הופק מאתר|משרד האוצר|בתאריך/.test(joined);
      };
      ...
      sheetNames.forEach((sheetName) => {
        ...
        rows.slice(headerIdx + 1).forEach((row) => {
          if(!Array.isArray(row) || isBannerRow(row)) return;
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      const normalizeHeader = (value) => safeTrim(value)
        .replace(/[״“”]/g, '"')
        .replace(/[׳‘’]/g, "'")
        .replace(/\s+/g, " ")
        .trim();

      const normalizeCellText = (value) => normalizeHeader(value)
        .replace(/[‎‏]/g, "")
        .replace(/[:;,]+$/g, "")
        .trim();

      const ignoredBannerPatterns = [
        /התיק הביטוחי/,
        /הופק מאתר/,
        /הר הביטוח/,
        /משרד האוצר/,
      ...
      const isBannerRow = (row = []) => {
        ...
        const joined = row.map((cell) => normalizeCellText(cell)).filter(Boolean).join(" | ");
        if(!joined) return false;
        return ignoredBannerPatterns.some((pattern) => pattern.test(joined));
      };
      ...
      const scoreHeaderRow = (row = []) => {
        if(!Array.isArray(row) || !row.length || isBannerRow(row)) return 0;
        return Object.values(headerMatchers).reduce((score, patterns) => (
