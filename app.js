...
(() => {
  ...
  const BUILD = "20260323-operational-report-entry-pdf-1";
...
// ---------- Step 6 ----------
    ...
    renderOperationalReport(payloadOverride){
      ...
      const processSteps = this.steps.map((step, index) => ({
        stepNo: step.id,
        title: step.title,
        desc: [
          'פרטים אישיים, משפחה ונתוני בסיס של הלקוח',
          'בדיקת נתוני גוף וחישוב BMI',
          'מיפוי מלא של כל הפוליסות הקיימות',
          'החלטת טיפול לכל פוליסה קיימת מול החברה הנגדית',
          'הקמת הפוליסות החדשות והכיסויים שנמכרו',
          'פרטי משלם ואמצעי סליקה',
          'סיכום מכירתי לפני מעבר לתפעול',
          'הצהרת בריאות מלאה',
          'סיכום תפעולי ומספרי סוכן'
        ][index] || '',
        status: 'בוצע'
      }));

      const insuredCards = payload.insureds.map((ins, index) => {
        const d = ins?.data && typeof ins.data === 'object' ? ins.data : {};
        const existingCount = Array.isArray(d.existingPolicies) ? d.existingPolicies.length : 0;
        return `<article class="lcReportPersonCard">
          <div class="lcReportPersonCard__top">
            <div>
              <div class="lcReportPersonCard__name">${escapeHtml(safeTrim(ins?.label) || `מבוטח ${index + 1}`)}</div>
              <div class="lcReportPersonCard__meta">${this.renderReportValue(((d.firstName || '') + ' ' + (d.lastName || '')).trim())} · ${this.renderReportValue(d.idNumber)}</div>
            </div>
            <span class="lcReportChip">${escapeHtml(ins?.type === 'primary' ? 'ראשי' : ins?.type === 'spouse' ? 'בן/בת זוג' : ins?.type === 'child' ? 'קטין' : 'מבוטח')}</span>
          </div>
          <div class="lcReportPersonCard__grid">
            <div><span>תאריך לידה</span><strong>${this.renderReportValue(d.birthDate)}</strong></div>
            <div><span>טלפון</span><strong>${this.renderReportValue(d.phone)}</strong></div>
            <div><span>עיר</span><strong>${this.renderReportValue(d.city)}</strong></div>
            <div><span>קופת חולים</span><strong>${this.renderReportValue(d.clinic || d.hmo || d.kupatHolim)}</strong></div>
            <div><span>שב״ן</span><strong>${this.renderReportValue(d.shaban || d.shabanLevel)}</strong></div>
            <div><span>פוליסות קיימות</span><strong>${escapeHtml(String(existingCount))}</strong></div>
          </div>
        </article>`;
      });
      ...
      payload.insureds.forEach(ins => {
        ...
        existingPolicies.forEach(p => {
          const status = cancellations?.[p?.id] || {};
          existingRows.push([
            escapeHtml(ins?.label || ''),
            ...
            this.renderReportValue(this.getExistingPolicyStatusLabel(status?.status))
          ]);
          if(safeTrim(status?.status) || safeTrim(status?.reason)){
            cancellationRows.push([
              escapeHtml(ins?.label || ''),
              this.renderReportValue(p?.company),
              this.renderReportValue(p?.type),
              this.renderReportValue(this.getExistingPolicyStatusLabel(status?.status)),
              this.renderReportValue(status?.reason)
            ]);
          }
        });
      });
      ...
      const newRows = newPolicies.map(p => {
        const insuredLabel = this.getNewPolicyInsuredLabel(payload, p, payload.insureds || []);
        const coverageValue = this.getPolicyCoverageDisplayValue(p || {});
        return [
          this.renderReportValue(insuredLabel),
          this.renderReportValue(p?.company),
          this.renderReportValue(p?.type),
          this.renderReportValue(p?.premiumMonthly),
          this.renderReportValue(this.getPolicyDiscountDisplayText(p || {})),
          this.renderReportValue(this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(p || {}))),
          this.renderReportValue(p?.startDate),
          this.renderReportValue(coverageValue)
        ];
      });

      const coverCards = newPolicies.map((p, idx) => {
        const coverItems = this.getPolicyCoverItems(p || {});
        const coverageValue = this.getPolicyCoverageDisplayValue(p || {});
        const items = coverItems.length
          ? coverItems.map(item => `<span class="lcReportTag">${escapeHtml(item)}</span>`).join('')
          : `<span class="lcReportTag lcReportTag--muted">${escapeHtml(coverageValue || 'לא הוגדרו כיסויים')}</span>`;
        return `<article class="lcReportCoverCard">
          <div class="lcReportCoverCard__top">
            <div>
              <div class="lcReportCoverCard__title">${escapeHtml(safeTrim(p?.company) || 'חברה')} · ${escapeHtml(safeTrim(p?.type) || 'פוליסה')}</div>
              <div class="lcReportCoverCard__sub">${this.renderReportValue(this.getNewPolicyInsuredLabel(payload, p, payload.insureds || []))}</div>
            </div>
            <span class="lcReportChip">פוליסה ${idx + 1}</span>
          </div>
          <div class="lcReportTagRow">${items}</div>
        </article>`;
      });
      ...
      const reportStats = [
        { label:'שלבים שבוצעו', value:String(processSteps.length), sub:'כל הוויזארד הושלם' },
        { label:'מבוטחים', value:String(payload.insureds.length), sub:'כולל מבוטח ראשי ותלויים' },
        { label:'פוליסות קיימות', value:String(existingRows.length), sub:'מופו בתהליך והוגדר סטטוס טיפול' },
        { label:'פוליסות חדשות', value:String(newPolicies.length), sub:`חיסכון כולל ${escapeHtml(this.formatMoneyValue(totalSavings))}` }
      ];

      return `<div class="lcReportDoc">
        <section class="lcReportShowcase">
          <div class="lcReportShowcase__glow" aria-hidden="true"></div>
          <div class="lcReportShowcase__top">
            <div>
              <div class="lcReportShowcase__eyebrow">דוח פנימי למחלקת תפעול</div>
              <h2 class="lcReportShowcase__title">כל רצף ההקמה של הלקוח במקום אחד</h2>
              <div class="lcReportShowcase__sub">מסמך תפעולי מלא שמרכז את כל השלבים שהוזנו בוויזארד, כולל מבוטחים, פוליסות קיימות, פוליסות חדשות, כיסויים, תשלום והצהרת בריאות.</div>
            </div>
            <div class="lcReportChipRow">
              <span class="lcReportChip lcReportChip--soft">${escapeHtml(safeTrim(Auth?.current?.name) || 'נציג')}</span>
              <span class="lcReportChip">${escapeHtml(createdAtText)}</span>
            </div>
          </div>
          <div class="lcReportHeroGrid">
            <article class="lcReportPrimeCard">
              <div class="lcReportPrimeCard__head">
                <div>
                  <div class="lcReportPrimeCard__title">${this.renderReportValue(((primary.firstName || '') + ' ' + (primary.lastName || '')).trim())}</div>
                  <div class="lcReportPrimeCard__sub">ת.ז ${this.renderReportValue(primary.idNumber)} · ${this.renderReportValue(primary.phone)}</div>
                </div>
                <span class="lcReportChip">לקוח פעיל</span>
              </div>
              <div class="lcReportPrimeCard__grid">
                <div><span>עיר</span><strong>${this.renderReportValue(primary.city)}</strong></div>
                <div><span>מייל</span><strong>${this.renderReportValue(primary.email)}</strong></div>
                <div><span>קופת חולים</span><strong>${this.renderReportValue(primary.clinic || primary.hmo || primary.kupatHolim)}</strong></div>
                <div><span>שב״ן</span><strong>${this.renderReportValue(primary.shaban || primary.shabanLevel)}</strong></div>
              </div>
            </article>
            <article class="lcReportPrimeCard lcReportPrimeCard--accent">
              <div class="lcReportPrimeCard__head">
                <div>
                  <div class="lcReportPrimeCard__title">מדדי תפעול מהירים</div>
                  <div class="lcReportPrimeCard__sub">תמונת מצב מיידית לפני טיפול</div>
                </div>
              </div>
              <div class="lcReportMetricGrid">
                ${reportStats.map(item => `<div class="lcReportMetricCard"><span>${escapeHtml(item.label)}</span><strong>${escapeHtml(item.value)}</strong><small>${escapeHtml(item.sub)}</small></div>`).join('')}
              </div>
            </article>
          </div>
        </section>

        <section class="lcReportSection lcReportSection--timeline">
          <div class="lcReportSection__title">רצף שלבי ההקמה</div>
          <div class="lcReportSection__sub">כל השלבים שבוצעו בוויזארד, בסדר עבודה ברור למחלקת התפעול.</div>
          <div class="lcReportTimeline">
            ${processSteps.map(step => `<article class="lcReportTimelineItem"><div class="lcReportTimelineItem__step">שלב ${step.stepNo}</div><div class="lcReportTimelineItem__title">${escapeHtml(step.title)}</div><div class="lcReportTimelineItem__desc">${escapeHtml(step.desc)}</div><div class="lcReportTimelineItem__status">${escapeHtml(step.status)}</div></article>`).join('')}
          </div>
        </section>

        <section class="lcReportSection">
          <div class="lcReportSection__title">פרטי לקוח</div>
          <div class="lcReportGrid">${this.compactReportFields(primary, [['firstName','שם פרטי'],['lastName','שם משפחה'],['idNumber','תעודת זהות'],['birthDate','תאריך לידה'],['gender','מגדר'],['maritalStatus','מצב משפחתי'],['phone','טלפון'],['email','אימייל'],['city','עיר'],['street','רחוב'],['houseNumber','מספר בית'],['zip','מיקוד'],['clinic','קופת חולים'],['shaban','שב״ן'],['occupation','עיסוק'],['heightCm','גובה'],['weightKg','משקל'],['bmi','BMI']])}</div>
        </section>

        <section class="lcReportSection">
          <div class="lcReportSection__title">מבוטחים בתיק</div>
          <div class="lcReportSection__sub">כרטיס נפרד לכל מבוטח שהוזן בוויזארד.</div>
          <div class="lcReportPersonGrid">${insuredCards.length ? insuredCards.join('') : '<div class="muted">לא נמצאו מבוטחים.</div>'}</div>
        </section>

        <section class="lcReportSection">
          <div class="lcReportSection__title">פוליסות קיימות</div>
          ${this.renderTable(['מבוטח','חברה','סוג ביטוח','מספר פוליסה','פרמיה חודשית','סטטוס טיפול'], existingRows)}
        </section>

        <section class="lcReportSection">
          <div class="lcReportSection__title">ביטול / שינוי בחברה נגדית</div>
          <div class="lcReportSection__sub">רק פוליסות שסומן להן טיפול תפעולי.</div>
          ${this.renderTable(['מבוטח','חברה','סוג ביטוח','סטטוס','סיבה'], cancellationRows)}
        </section>

        <section class="lcReportSection">
          <div class="lcReportSection__title">פוליסות חדשות שנמכרו</div>
          ${this.renderTable(['מבוטח','חברה','סוג ביטוח','פרמיה חודשית','דירוג הנחה','פרמיה אחרי הנחה','תאריך תחילה','כיסוי / סכום'], newRows)}
        </section>

        <section class="lcReportSection">
          <div class="lcReportSection__title">כיסויים שנבחרו במכירה</div>
          <div class="lcReportSection__sub">פירוט חזותי של הכיסויים או סכום הביטוח בכל פוליסה חדשה.</div>
          <div class="lcReportCoverGrid">${coverCards.length ? coverCards.join('') : '<div class="muted">לא נוספו פוליסות חדשות.</div>'}</div>
        </section>
        ...
        <section class="lcReportSection">
          ...
          <div class="lcReportSection__sub">מופיעים רק ממצאים שסומנו כ"כן" ונשמרו בתהליך.</div>
          <div class="lcReportList">${healthItems.length ? healthItems.join('') : '<div class="muted">לא סומנו ממצאים רפואיים.</div>'}</div>
        </section>
      </div>`;
    },
    ...
    exportOperationalPdf(payloadOverride){
