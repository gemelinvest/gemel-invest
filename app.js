...
(() => {
  ...
  const BUILD = "20260322-investo-fab-fix-1";
  ...
  const INVESTO_FAB_STORAGE_KEY = "GI_INVESTO_FAB_POS_V1";
...
const SystemRepairUI = {
  ...
  const InvestoUI = {
    ...
    init(){
      this.els = {
        fab: $("#investoFab"), window: $("#investoWindow"), close: $("#investoClose"), minimize: $("#investoMinimize"),
        dragHandle: $("#investoDragHandle"), messages: $("#investoMessages"), input: $("#investoInput"), send: $("#investoSend")
      };
      if(!this.els.fab || !this.els.window) return;
      on(this.els.fab, 'click', () => this.toggleWindow());
      on(this.els.fab, 'keydown', (ev) => { if(ev.key === 'Enter' || ev.key === ' ') { ev.preventDefault(); this.toggleWindow(); } });
      on(this.els.close, 'click', () => this.closeWindow());
      on(this.els.minimize, 'click', () => this.closeWindow());
      on(this.els.send, 'click', () => this.handleSend());
      on(this.els.input, 'keydown', (ev) => { if(ev.key === 'Enter' && !ev.shiftKey){ ev.preventDefault(); this.handleSend(); } });
      $$('.investoQuick__btn').forEach((btn) => on(btn, 'click', () => { if(this.els.input){ this.els.input.value = btn.dataset.investoQ || ''; this.handleSend(); } }));
      this.initDrag();
      this.updateVisibility();
      setInterval(() => this.updateVisibility(), 1200);
    },
    storageKey(){
      const key = safeTrim(Auth?.current?.name) || 'guest';
      return `${INVESTO_FAB_STORAGE_KEY}__${key}`;
    },
    updateVisibility(){
      const canShow = !document.body.classList.contains('lcAuthLock');
      if(!canShow){
        this.els.fab?.classList.add('is-hidden');
        this.els.window?.classList.add('is-hidden');
        return;
      ...
      if(!(this.els.fab.style.left && this.els.fab.style.top)) this.restoreFabPosition();
    },
    toggleWindow(){
    ...
    addMessage(text, role='bot'){
      ...
      article.className = `investoMsg investoMsg--${role}`;
      const bubble = document.createElement('div');
      bubble.className = 'investoMsg__bubble';
    ...
    handleSend(){
      const q = safeTrim(this.els.input?.value);
      if(!q) return;
      this.addMessage(q, 'user');
      this.els.input.value = '';
      const answer = this.answerQuestion(q);
      setTimeout(() => this.addMessage(answer, 'bot'), 180);
    },
    answerQuestion(q){
      const s = String(q || '').toLowerCase();
      const company = ['הראל','כלל','מגדל','מנורה','הפניקס','פניקס','איילון','הכשרה'].find((name) => q.includes(name)) || '';
      if(/סוכרת/.test(q)) return company ? `${company}: לפי טבלאות החיתום שהוזנו יש לבדוק סוכרת לפי סוג, איזון ותרופות. אם תרצה כתוב גם סוג סוכרת / A1C / חברה ואחזיר תשובה ממוקדת יותר.` : 'סוכרת: ברירת המחדל היא חיפוש בכלל החברות. כתוב גם שם חברה כדי לסנן, למשל: סוכרת סוג 2 בהראל.';
      if(/לחץ.?דם|יתר לחץ/.test(q)) return company ? `${company}: לחץ דם נבחן לרוב לפי איזון, טיפול תרופתי והאם יש פגיעה נלווית. כתוב גם ערכים/תרופות אם יש.` : 'לחץ דם: אפשר לשאול בכלל החברות או לציין חברה מסוימת כדי לסנן את התשובה.';
      if(/כולסטרול|שומנים/.test(q)) return 'כולסטרול / שומנים: מומלץ לציין חברה, טיפול תרופתי וערכים עדכניים כדי להחזיר תשובת חיתום מדויקת.';
      if(/כדור|תרופה|תרופות/.test(q)) return 'תרופה: כתוב את שם התרופה ואסביר בקצרה מה היא בדרך כלל עושה, ובמידת הצורך אפשר לקשר גם לחיתום.';
      return company ? `${company}: קיבלתי את השאלה. כדי לדייק, כתוב מצב רפואי מלא, ואם יש גם חומרה/תרופות/אשפוזים.` : 'אני מוכן לשאלות על חיתום רפואי, מחלות ותרופות. כדי לדייק, כתוב מצב רפואי מלא, ואם צריך גם שם חברה.';
    },
    restoreFabPosition(){
      const fab = this.els.fab;
      if(!fab) return;
      let payload = null;
      try { payload = JSON.parse(localStorage.getItem(this.storageKey()) || 'null'); } catch(_e) {}
      if(payload && Number.isFinite(payload.left) && Number.isFinite(payload.top)){
        fab.style.left = payload.left + 'px';
        fab.style.top = payload.top + 'px';
        fab.style.bottom = 'auto';
      } else {
        fab.style.left = '24px';
        fab.style.top = Math.max(88, window.innerHeight - 92) + 'px';
        fab.style.bottom = 'auto';
      }
      this.clampFab();
    },
    saveFabPosition(){
      const fab = this.els.fab; if(!fab) return;
      const rect = fab.getBoundingClientRect();
      try { localStorage.setItem(this.storageKey(), JSON.stringify({ left: Math.round(rect.left), top: Math.round(rect.top) })); } catch(_e) {}
    },
    clampFab(){
      const fab = this.els.fab; if(!fab) return;
      const rect = fab.getBoundingClientRect();
      ...
      fab.style.left = nextLeft + 'px'; fab.style.top = nextTop + 'px'; fab.style.bottom = 'auto';
      if(!this.els.window.classList.contains('is-hidden')){ this.els.window.style.left = nextLeft + 'px'; this.els.window.style.bottom = '98px'; }
    },
    initDrag(){
      const fab = this.els.fab; if(!fab) return;
      const onMove = (ev) => { if(!this.drag) return; fab.classList.add('is-dragging'); fab.style.left = (ev.clientX - this.drag.offsetX) + 'px'; fab.style.top = (ev.clientY - this.drag.offsetY) + 'px'; fab.style.bottom = 'auto'; this.clampFab(); };
      const onUp = () => { if(!this.drag) return; this.drag = null; fab.classList.remove('is-dragging'); this.saveFabPosition(); };
      on(fab, 'pointerdown', (ev) => { this.drag = { offsetX: ev.clientX - fab.getBoundingClientRect().left, offsetY: ev.clientY - fab.getBoundingClientRect().top }; fab.setPointerCapture?.(ev.pointerId); });
      on(window, 'pointermove', onMove); on(window, 'pointerup', onUp); on(window, 'resize', () => this.clampFab());
    }
  };
