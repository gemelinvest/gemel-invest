...
(() => {
  ...
  const Wizard = (() => {
    ...
    // Update only the BMI badge inside a single medical card (no full re-render on each keystroke)
    function updateMedicalBadge(card){
      try{
        if(!card) return;
        ...
        const dob = (id === "main") ? safeTrim(els.dob?.value) : safeTrim(ref?.dob);
        const age = calcAge(dob);
        const isMinor = (type === "minor") || (age !== null && age < 18);

        const bmi = bmiFrom(ref?.heightCm, ref?.weightKg);
        const risk = riskFromBmi(bmi, isMinor);

        const badge = card.querySelector(".lcBmiBadge");
        if(!badge) return;

        const dotEl = badge.querySelector(".lcBmiDot");
        if(dotEl){
          dotEl.classList.remove("low","med","high");
          if(risk.dot) dotEl.classList.add(risk.dot);
        }
        const bmiSpan = badge.querySelector("[data-bmi-val]");
        if(bmiSpan) bmiSpan.textContent = (bmi === null ? "—" : String(bmi));
        const riskSpan = badge.querySelector("[data-risk-val]");
        if(riskSpan) riskSpan.textContent = risk.label;
      }catch(_e){}
    }
    ...
    function renderMedicalList(){
      ...
      els.medicalList.innerHTML = list.map(entry => {
        ...
        return `
          <div class="lcMedicalCard" data-med="${escapeHtml(entry.id)}" data-type="${escapeHtml(entry.type)}" data-label="${escapeHtml(entry.label)}">
            <div class="lcInsuredCard__head">
              ...
              <div class="lcBmiBadge" title="BMI + דירוג סיכון">
                ...
                <span>BMI: <span data-bmi-val>${bmi === null ? "—" : bmi}</span></span>
                <span class="muted" style="font-weight:900;">·</span>
                <span data-risk-val>${risk.label}</span>
              </div>
            </div>
            ...
            <div class="lcMedicalRow">
              <div class="field">
                ...
                <input class="input" data-f="heightCm" type="number" inputmode="decimal" step="0.1" placeholder="לדוגמה: 175" />
              </div>
              <div class="field">
                <label class="label">משקל (ק״ג) *</label>
                <input class="input" data-f="weightKg" type="number" inputmode="decimal" step="0.1" placeholder="לדוגמה: 78" />
              </div>
              <div class="field">
