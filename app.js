...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const MirrorsUI = {
    ...
    init(){
      ...
      on(this.els.verifyCard, "click", (ev) => {
...
const SystemRepairUI = {
    ...
    init(){
      ...
      on(this.els.confirm, "click", async () => { await this.run(); });
    },
    ...
    setBusy(flag){
      ...
      if(this.els.confirm) this.els.confirm.textContent = flag ? "מטפל בתקלה..." : "אישור והפעל טיפול";
    },
    ...
    safeCloseKnownLayers(){
      ...
      ["lcForgotModal", "lcUserModal", "customerFull", "customerPolicyModal", "lcArchiveCustomerModal", "systemRepairModal"].forEach((id) => {
    ...
    async run(){
      ...
      this.setBusy(true);
      try {
        this.setStatus("שלב 1/3 · משחרר שכבות חסימה, חלונות ומצבי טעינה תקועים...", "is-working");
        this.safeCloseKnownLayers();
        this.releaseUiLocks();
        await new Promise((resolve) => setTimeout(resolve, 120));

        this.setStatus("שלב 2/3 · מאפס מצב מקומי, טיימרים ורענון מסך פעיל...", "is-working");
        this.repairLocalState();
        this.rerenderCurrentView(currentView);
        await new Promise((resolve) => setTimeout(resolve, 140));

        this.setStatus("שלב 3/3 · מנסה לסנכרן מחדש את נתוני המשתמש ללא פגיעה במידע...", "is-working");
        const syncResult = await this.tryReloadSession();
        this.rerenderCurrentView(currentView);

        if(syncResult.ok || syncResult.skipped){
          this.setStatus("הטיפול הושלם. המערכת שוחררה, המסך רוענן והנתונים נטענו מחדש.", "is-ok");
        } else {
          this.setStatus("הטיפול המקומי הושלם והרוב שוחרר, אבל סנכרון הנתונים לא הצליח כעת. אפשר להמשיך לעבוד ולבדוק שוב.", "is-ok");
        ...
        this.repairLocalState();
        this.rerenderCurrentView(currentView);
        this.setStatus("בוצע טיפול חירום מקומי. אם התקלה חוזרת, כדאי לבצע רענון מלא למערכת.", "is-err");
      } finally {
        this.setBusy(false);
      }
    }
