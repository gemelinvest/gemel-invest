...
const SystemRepairUI = {
  ...
  const ChatUI = {
    ...
    listenMessages(){
      ...
      this.messageListenerOff = this.sdk.onChildAdded(messagesQuery, (snap) => {
        const msg = snap.val() || {};
        const id = snap.key;
        if(this.renderedMessageIds.has(id)) return;
        this.renderedMessageIds.add(id);
        this.lastMessageByConversation.set(this.currentConversationId, { ...msg, id });
        this.renderMessage({ ...msg, id });
        if(msg.fromId && msg.fromId !== this.userKey){
    ...
    async setTyping(flag){
      ...
      try {
        ...
        if(flag) await this.sdk.set(typingRef, true);
        else await this.sdk.remove(typingRef);
      } catch(_e) {}
    },
