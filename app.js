...
txt.textContent = "";
  ...
  const ProposalFormUI = {
    ...
    init(){
      ...
      on(this.els.close, "click", () => this.close());
      on(this.els.backdrop, "click", () => this.close());
      on(this.els.previewBtn, "click", () => this.renderPreview());
      on(this.els.downloadBtn, "click", () => this.downloadSummary());
    },
    ...
    open(id){
      ...
      this.setStatus("החלון נפתח בהצלחה. ניתן ליצור תצוגת ניסוי או להוריד סיכום נתונים.");
      this.els.wrap.classList.add("is-open");
      this.els.wrap.setAttribute("aria-hidden", "false");
    },
    ...
    getApplicants(rec){
      const payload = rec?.payload && typeof rec.payload === "object" ? rec.payload : {};
      const list = Array.isArray(payload.insureds) ? payload.insureds : [];
      if(list.length){
        return list.map((ins, index) => this.normalizeApplicant(ins, index, rec)).filter(Boolean);
      }
    ...
    normalizeApplicant(ins, index, rec){
      ...
      return {
        index: index + 1,
        label: safeTrim(ins?.label) || fullName || `מבוטח ${index + 1}`,
        fullName,
        ...
        city: safeTrim(data.city) || safeTrim(rec?.city),
        occupation: safeTrim(data.occupation),
        kupatHolim: safeTrim(data.kupatHolim || data.healthFund),
        shaban: data.hasShaban === true ? "כן" : data.hasShaban === false ? "לא" : "—"
      };
    },
    ...
    renderSummary(rec){
      ...
      const cards = applicants.map((app) => `
        <div class="proposalFormModal__card">
          <div class="proposalFormModal__cardTitle">${escapeHtml(app.label || 'מבוטח')}</div>
          <div class="proposalFormModal__grid">
            <div><span>שם מלא</span><strong>${escapeHtml(app.fullName || '—')}</strong></div>
            <div><span>ת״ז</span><strong dir="ltr">${escapeHtml(app.idNumber || '—')}</strong></div>
            <div><span>טלפון</span><strong dir="ltr">${escapeHtml(app.phone || '—')}</strong></div>
            <div><span>מייל</span><strong>${escapeHtml(app.email || '—')}</strong></div>
            <div><span>עיר</span><strong>${escapeHtml(app.city || '—')}</strong></div>
            <div><span>קופ״ח</span><strong>${escapeHtml(app.kupatHolim || '—')}</strong></div>
            <div><span>שב״ן</span><strong>${escapeHtml(app.shaban || '—')}</strong></div>
            <div><span>עיסוק</span><strong>${escapeHtml(app.occupation || '—')}</strong></div>
          </div>
        </div>`).join("");
      this.els.summary.innerHTML = `
        <div class="proposalFormModal__topMeta">
          <div><span>לקוח</span><strong>${escapeHtml(rec.fullName || '—')}</strong></div>
          <div><span>ת״ז</span><strong dir="ltr">${escapeHtml(rec.idNumber || '—')}</strong></div>
          <div><span>טלפון</span><strong dir="ltr">${escapeHtml(rec.phone || '—')}</strong></div>
          <div><span>מס׳ מבוטחים</span><strong>${applicants.length}</strong></div>
        </div>
        ${cards || '<div class="proposalFormModal__empty">לא נמצאו מבוטחים להצגה.</div>'}
      `;
    },

    renderPreview(){
      const rec = this.byId(this.currentCustomerId);
      if(!rec) return;
      this.renderSummary(rec);
      this.setStatus("תצוגת הניסוי נבנתה מחדש מתוך הנתונים המעודכנים של הלקוח.");
    },
    ...
    downloadSummary(){
      ...
      a.href = url;
      a.download = `phoenix-proposal-${cleanName}.json`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      setTimeout(() => URL.revokeObjectURL(url), 500);
      this.setStatus("סיכום הנתונים ירד בהצלחה כקובץ JSON לניסוי.");
    }
  };
