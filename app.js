...
init(){
      ...
      on(this.els.report, "click", (e) => {
        const tt = e.target;
        if(tt && tt.getAttribute && tt.getAttribute("data-close") === "1") this.closeOperationalReport();
      });
    ...
    renderFooter(){
      ...
      if(this.els.btnNext) this.els.btnNext.textContent = (this.step >= this.steps.length) ? "הפק דוח תפעולי" : "הבא";
...
// ---------- Step 6 ----------
    ...
    openOperationalReport(){
      if(!this.els.reportBody || !this.els.report) return;
    ...
    formatReportLabel(key){
      const map = {firstName:"שם פרטי", lastName:"שם משפחה", idNumber:"תעודת זהות", birthDate:"תאריך לידה", gender:"מגדר", maritalStatus:"מצב משפחתי", phone:"טלפון", email:"אימייל", city:"עיר", street:"רחוב", houseNumber:"מספר בית", zip:"מיקוד", clinic:"קופת חולים", shaban:"שב״ן", occupation:"עיסוק", heightCm:"גובה", weightKg:"משקל", bmi:"BMI", existingPolicies:"פוליסות קיימות", cancellations:"ביטול בחברה נגדית", newPolicies:"פוליסות חדשות", payerChoice:"סוג משלם", externalPayer:"משלם חריג", payAll:"משלם על הכל", policyPayers:"שיוך משלמים", paymentMethod:"אמצעי תשלום", cc:"כרטיס אשראי", ho:"הוראת קבע", holderName:"שם מחזיק", holderId:"תז מחזיק", cardNumber:"מספר כרטיס", exp:"תוקף", account:"מספר חשבון", branch:"סניף", bankName:"שם בנק", bankNo:"מספר בנק", relation:"קרבה", healthDeclaration:"הצהרת בריאות", categories:"קטגוריות", selectedPayerId:"מבוטח משלם", insuredMode:"אופן שיוך", insuredId:"מבוטח", company:"חברה", type:"סוג ביטוח", sumInsured:"סכום ביטוח", compensation:"סכום פיצוי", premiumMonthly:"פרמיה חודשית", startDate:"תאריך תחילה", pledge:"שיעבוד", pledgeBank:"פרטי שיעבוד", reason:"סיבה", status:"סטטוס", attachments:"נספחים", ui:"ממשק", responses:"תשובות", currentIndex:"אינדקס נוכחי", summary:"סיכום", diagnosis:"אבחנה", date:"תאריך", since:"ממתי", what:"מהות", details:"פירוט", meds:"תרופות", why:"למה", procedure:"פרוצדורה", result:"תוצאה", percent:"אחוז", house:"בית"};
      return map[key] || String(key || "").replace(/^att:/, 'נספח: ').replace(/([A-Z])/g,' $1').replace(/_/g,' ').trim();
    },

    renderReportValue(val){
    ...
    renderReportTree(label, value){
      if(Array.isArray(value)){
        if(!value.length) return `<div class="lcEmpty">${escapeHtml(label)}: אין נתונים</div>`;
        const items = value.map((item, idx) => `<div class="lcTreeNode"><div class="lcTreeNode__title">${escapeHtml(label)} ${idx+1}</div>${this.renderReportTreeContent(item)}</div>`).join('');
        return `<div class="lcTreeList">${items}</div>`;
      }
      if(value && typeof value === 'object'){
        const entries = Object.entries(value);
        if(!entries.length) return `<div class="lcEmpty">${escapeHtml(label)}: אין נתונים</div>`;
        return `<div class="lcTreeNode"><div class="lcTreeNode__title">${escapeHtml(label)}</div>${this.renderReportTreeContent(value)}</div>`;
      }
      return `<div class="lcTreeLeaf"><b>${escapeHtml(label)}</b><div>${this.renderReportValue(value)}</div></div>`;
    },

    renderReportTreeContent(obj){
      if(Array.isArray(obj)) return this.renderReportTree('פריט', obj);
      if(!(obj && typeof obj === 'object')) return `<div class="lcTreeLeaf"><div>${this.renderReportValue(obj)}</div></div>`;
      return `<div class="lcTreeList">${Object.entries(obj).map(([k,v]) => this.renderReportTree(this.formatReportLabel(k), v)).join('')}</div>`;
    },

    renderOperationalReport(){
      const primary = this.insureds[0] || { data:{} };
      const allExisting = this.insureds.reduce((n, ins) => n + ((ins.data?.existingPolicies || []).length), 0);
      const ts = new Date().toLocaleString('he-IL');
      const hero = `<div class="lcReportHero"><div class="lcReportCard"><div class="lcReportSection__title">דוח תפעולי מלא</div><div class="lcReportSection__sub">הדוח מרכז את כל הנתונים שהוזנו בוויזארד, כולל כל המבוטחים, פוליסות קיימות, ביטולים, פוליסות חדשות, פרטי משלם והצהרת בריאות.</div><div class="lcReportMeta"><div class="lcReportMetaItem"><b>מבוטח ראשי</b><span>${this.renderReportValue((primary.data?.firstName || '') + ' ' + (primary.data?.lastName || ''))}</span></div><div class="lcReportMetaItem"><b>ת.ז.</b><span>${this.renderReportValue(primary.data?.idNumber)}</span></div><div class="lcReportMetaItem"><b>מספר מבוטחים בתיק</b><span>${this.insureds.length}</span></div><div class="lcReportMetaItem"><b>הופק בתאריך</b><span>${escapeHtml(ts)}</span></div></div></div><div class="lcReportCard"><div class="lcReportMeta"><div class="lcReportMetaItem"><b>פוליסות קיימות</b><span>${allExisting}</span></div><div class="lcReportMetaItem"><b>פוליסות חדשות</b><span>${(this.newPolicies || []).length}</span></div><div class="lcReportMetaItem"><b>אמצעי תשלום</b><span>${this.renderReportValue(primary.data?.paymentMethod === 'cc' ? 'כרטיס אשראי' : primary.data?.paymentMethod === 'ho' ? 'הוראת קבע' : '')}</span></div><div class="lcReportMetaItem"><b>סוג משלם</b><span>${this.renderReportValue(primary.data?.payerChoice === 'insured' ? 'מבוטח קיים' : primary.data?.payerChoice === 'external' ? 'משלם חריג' : '')}</span></div></div></div></div>`;
      const insuredSections = this.insureds.map((ins, idx) => { const d = ins.data || {}; const existing = d.existingPolicies || []; return `<section class="lcReportSection"><div class="lcReportInsured"><div class="lcReportInsured__head"><div class="lcReportInsured__title">${escapeHtml(ins.label || ('מבוטח ' + (idx+1)))}</div><span class="lcReportChip">${escapeHtml(ins.type || 'insured')}</span></div><div class="lcReportGrid"><div class="lcReportField"><b>שם מלא</b><span>${this.renderReportValue((d.firstName || '') + ' ' + (d.lastName || ''))}</span></div><div class="lcReportField"><b>תעודת זהות</b><span>${this.renderReportValue(d.idNumber)}</span></div><div class="lcReportField"><b>טלפון</b><span>${this.renderReportValue(d.phone)}</span></div><div class="lcReportField"><b>אימייל</b><span>${this.renderReportValue(d.email)}</span></div><div class="lcReportField"><b>כתובת</b><span>${this.renderReportValue([d.city,d.street,d.houseNumber].filter(Boolean).join(' '))}</span></div><div class="lcReportField"><b>BMI</b><span>${this.renderReportValue(d.bmi)}</span></div></div></div><div class="lcReportSection__title">פוליסות קיימות</div><div class="lcReportSection__sub">${existing.length ? 'פירוט פוליסות קיימות של המבוטח.' : 'לא הוזנו פוליסות קיימות למבוטח זה.'}</div>${existing.length ? `<div class="lcReportPolicies">${existing.map(p => `<div class="lcReportPolicy"><div class="lcReportPolicy__title">${escapeHtml(p.company || 'ללא חברה')} · ${escapeHtml(p.type || 'ללא סוג')}</div>${this.renderReportTreeContent(p)}</div>`).join('')}</div>` : `<div class="lcEmpty">אין פוליסות קיימות</div>`}<div style="height:12px"></div><div class="lcReportSection__title">כל הנתונים שנקלטו עבור המבוטח</div><div class="lcReportSection__sub">כולל פרטי לקוח, BMI, ביטולים, שיוכי משלם והצהרת בריאות.</div><div class="lcReportTree">${this.renderReportTreeContent(d)}</div></section>`; }).join('');
      const newPoliciesSection = `<section class="lcReportSection"><div class="lcReportSection__title">פוליסות חדשות (שלב 5)</div><div class="lcReportSection__sub">פירוט מלא של כל הפוליסות החדשות שנוספו בתיק.</div>${(this.newPolicies || []).length ? `<div class="lcReportPolicies">${(this.newPolicies || []).map(p => `<div class="lcReportPolicy"><div class="lcReportPolicy__title">${escapeHtml(p.company || 'ללא חברה')} · ${escapeHtml(p.type || (this.isMedicareCompany(p.company) ? 'מדיקר' : 'ללא סוג'))}</div>${this.renderReportTreeContent(p)}</div>`).join('')}</div>` : `<div class="lcEmpty">לא נוספו פוליסות חדשות</div>`}</section>`;
      const payerSection = `<section class="lcReportSection"><div class="lcReportSection__title">פרטי משלם ותשלום</div><div class="lcReportSection__sub">פירוט כפי שנקלט בשלב 6.</div><div class="lcReportTree">${this.renderReportTreeContent({payerChoice: primary.data?.payerChoice || '', selectedPayerId: primary.data?.selectedPayerId || '', externalPayer: primary.data?.externalPayer || {}, paymentMethod: primary.data?.paymentMethod || '', cc: primary.data?.cc || {}, ho: primary.data?.ho || {}, policyPayers: primary.data?.policyPayers || {}})}</div></section>`;
      const healthSection = `<section class="lcReportSection"><div class="lcReportSection__title">הצהרת בריאות</div><div class="lcReportSection__sub">פירוט מלא של כל התשובות, הקטגוריות והשדות שנשמרו בשלב 8.</div><div class="lcReportTree">${this.renderReportTreeContent(primary.data?.healthDeclaration || {})}</div></section>`;
      return `<div class="lcReportDoc">${hero}${insuredSections}${newPoliciesSection}${payerSection}${healthSection}</div>`;
    },
