      ...
      return `<article class="customerPolicyRow ${this.companyClass(policy.company)} ${policy.origin === 'new' ? 'is-newRow' : 'is-existingRow'}" data-policy-id="${escapeHtml(policy.id)}">
        ...
        <div class="customerPolicyRow__main">
          ...
          <div class="customerPolicyRow__numbers">
            ...
            <div class="customerPolicyRow__afterWrap ${discountPct > 0 || this.getPolicyDiscountYearsLabel(policy) ? 'has-discount' : ''}">
              <div class="customerPolicyRow__afterLabel">אחרי הנחה</div>
              <div class="customerPolicyRow__after">${escapeHtml(afterPremium)}</div>
              <div class="customerPolicyRow__discountMeta">${escapeHtml(this.getPolicyDiscountDisplayText(policy))}</div>
            </div>
...
const SystemRepairUI = {
  ...
  const ChatUI = {
    ...
    notifiedConversationAt: new Map(),
    userSearchTerm: "",
    dragState: null,
    messageListenerOff: null,
    usersListenerOff: null,
    typingListenerOff: null,
    connectedListenerOff: null,
    metaListenerOff: null,
    typingTimer: null,
    notifyAudioCtx: null,
    cleanupTimer: null,
    retentionMs: Math.max(60000, Number(FIREBASE_CHAT.retentionMinutes || 5) * 60000),
    typingWindowMs: Math.max(1200, Number(FIREBASE_CHAT.typingWindowMs || 2200)),
    ...
    init(){
      ...
      on(this.els.fab, "click", () => this.toggleWindow());
      ...
      this.initDrag();
      this.syncVisibility(UI?.els?.pageTitle ? document.body.classList.contains("view-dashboard-active") ? "dashboard" : "" : "dashboard");
    },
    ...
    async ensureStarted(){
      ...
      this.renderMe();
      const hasConfig = Object.values(FIREBASE_CHAT.config || {}).every(Boolean);
      this.enabled = !!(FIREBASE_CHAT.enabled && hasConfig);
      if(!this.enabled){
        this.setConnectionStatus(hasConfig ? "Firebase chat כבוי כרגע" : "Firebase לא הוגדר עדיין", "warn");
      ...
      try {
        ...
        this.listenUsers();
        this.listenAllConversationMeta();
        this.startCleanupLoop();
      } catch(err){
        console.error("CHAT_FIREBASE_INIT_FAILED", err);
        this.enabled = false;
        this.setConnectionStatus("שגיאה בטעינת Firebase", "err");
        this.els.setupHint?.classList.remove("is-hidden");
      }
    },
    ...
    refreshCurrentUser(){
      ...
      this.currentUser = {
        id: this.normalizeKey((sourceAgent?.id || Auth.current?.name || "user") + "__" + (sourceAgent?.username || Auth.current?.name || "")),
    ...
    async loadSdk(){
      ...
      const v = encodeURIComponent(FIREBASE_CHAT.sdkVersion || "12.11.0");
      const [appSdk, dbSdk] = await Promise.all([
        import(`https://www.gstatic.com/firebasejs/${v}/firebase-app.js`),
        import(`https://www.gstatic.com/firebasejs/${v}/firebase-database.js`)
      ]);
      this.sdk = { ...appSdk, ...dbSdk };
      const apps = this.sdk.getApps();
      this.app = apps.length ? apps[0] : this.sdk.initializeApp(FIREBASE_CHAT.config);
      this.db = this.sdk.getDatabase(this.app);
      this.ready = true;
      this.setConnectionStatus("מחבר Firebase…", "warn");
    },

    basePath(path=""){
      const root = safeTrim(FIREBASE_CHAT.rootPath || "gemel_invest_chat");
      return path ? `${root}/${path}` : root;
    },

    ref(path=""){
      return this.sdk.ref(this.db, this.basePath(path));
    },
    ...
    syncVisibility(view){
      const onDashboard = view === "dashboard" && !!Auth.current;
      this.els.fab?.classList.toggle("is-hidden", !onDashboard);
      if(!onDashboard) this.closeWindow(true);
    },
    ...
    initDrag(){
      ...
      on(document, "mousemove", (ev) => {
        ...
        const nextLeft = Math.min(maxLeft, Math.max(8, this.dragState.left + (ev.clientX - this.dragState.startX)));
        const nextTop = Math.min(maxTop, Math.max(8, this.dragState.top + (ev.clientY - this.dragState.startY)));
        panel.style.left = nextLeft + "px";
        panel.style.top = nextTop + "px";
      });
      on(document, "mouseup", () => { this.dragState = null; });
    },

    connectPresence(){
      if(!this.ready || !this.currentUser) return;
      const connectedRef = this.sdk.ref(this.db, ".info/connected");
      this.connectedListenerOff?.();
      this.connectedListenerOff = this.sdk.onValue(connectedRef, async (snap) => {
        if(snap.val() !== true){
          this.setConnectionStatus("מנותק זמנית", "warn");
          return;
        }
        try {
          const statusRef = this.ref(`users/${this.userKey}`);
          await this.sdk.onDisconnect(statusRef).update({ online:false, typing:false, lastSeen:this.sdk.serverTimestamp() });
          await this.sdk.update(statusRef, {
            id: this.userKey,
            name: this.currentUser.name,
            role: this.currentUser.role,
            rawRole: this.currentUser.rawRole,
            online: true,
            typing: false,
            lastSeen: this.sdk.serverTimestamp(),
            updatedAt: this.sdk.serverTimestamp()
          });
          this.setConnectionStatus("מחובר עכשיו", "ok");
        } catch(err){
          console.error("CHAT_PRESENCE_FAILED", err);
          this.setConnectionStatus("שגיאת נוכחות", "err");
        }
      });
    },

    listenAllConversationMeta(){
      if(!this.ready || !this.userKey) return;
      this.metaListenerOff?.();
      const metaRef = this.ref('conversation_meta');
      this.metaListenerOff = this.sdk.onValue(metaRef, (snap) => {
        const meta = snap.val() || {};
        const now = Date.now();
        Object.keys(meta).forEach((conversationId) => {
          const item = meta[conversationId] || {};
          if(!item?.participants || !item.participants[this.userKey]) return;
          const expiresAt = Number(item.expiresAt || 0);
          if(expiresAt && expiresAt <= now){
            this.cleanupConversation(conversationId);
            if(this.currentConversationId === conversationId){
              this.clearActiveConversationIfExpired(conversationId);
            }
            return;
          }
          const lastFromId = safeTrim(item.lastFromId);
          this.lastMessageByConversation.set(conversationId, item);
          if(lastFromId && lastFromId !== this.userKey){
            const isCurrentOpen = this.currentConversationId === conversationId && !this.els.window?.classList.contains('is-hidden');
            const alreadySeen = Number(item.lastSeenBy?.[this.userKey] || 0);
            const updatedAt = Number(item.updatedAt || 0);
            const lastNotifiedAt = Number(this.notifiedConversationAt.get(conversationId) || 0);
            if(!isCurrentOpen && updatedAt && updatedAt > alreadySeen && updatedAt > lastNotifiedAt){
              this.unreadByConversation.set(conversationId, Math.max(1, (this.unreadByConversation.get(conversationId) || 0) + 1));
              this.notifiedConversationAt.set(conversationId, updatedAt);
              this.renderFabBadge();
              const senderId = lastFromId;
              this.pushToast(this.usersMap.get(senderId)?.name || 'הודעה חדשה', safeTrim(item.lastMessageText) || 'נשלחה אליך הודעה חדשה');
    ...
    listenUsers(){
      if(!this.ready) return;
      this.usersListenerOff?.();
      const usersRef = this.ref("users");
      this.usersListenerOff = this.sdk.onValue(usersRef, (snap) => {
        const raw = snap.val() || {};
        this.usersMap = new Map();
        Object.keys(raw).forEach((key) => {
          const item = raw[key] || {};
          this.usersMap.set(key, {
            id: key,
            name: safeTrim(item.name) || key,
            role: safeTrim(item.role) || "נציג",
            online: item.online === true,
            typing: item.typing === true,
            lastSeen: item.lastSeen || item.updatedAt || null
          });
        });
        this.renderUsers();
        this.renderSelectedPeer();
      });
      this.renderUsers();
    },
    ...
    getAvailableUsers(){
      ...
      agents.forEach((agent) => {
        ...
        const id = this.normalizeKey((agent?.id || agent?.name || agent?.username || "agent") + "__" + (agent?.username || agent?.name || ""));
        if(id === this.userKey) return;
        const fb = this.usersMap.get(id);
        fromState.push({
          id,
          name: safeTrim(agent?.name) || safeTrim(agent?.username) || "נציג",
          role: agent?.role === "manager" ? "מנהל" : agent?.role === "ops" ? "נציג תפעול" : "נציג",
          online: fb?.online === true,
          typing: fb?.typing === true,
          lastSeen: fb?.lastSeen || null
        });
      });
      const extra = [];
      this.usersMap.forEach((user, key) => {
        if(key === this.userKey) return;
        if(fromState.some((item) => item.id === key)) return;
        extra.push(user);
      });
      return [...fromState, ...extra].sort((a,b) => {
    ...
    renderUsers(){
      ...
      const users = this.getAvailableUsers().filter((user) => {
        if(!term) return true;
        return `${user.name} ${user.role}`.toLowerCase().includes(term);
      });
      if(!users.length){
      ...
      wrap.innerHTML = users.map((user) => {
        const convoId = this.conversationId(user.id);
        const unread = this.unreadByConversation.get(convoId) || 0;
        const active = this.selectedUser?.id === user.id;
        const status = user.online ? (user.typing ? "מקליד עכשיו…" : "מחובר עכשיו") : `נראה לאחרונה ${this.formatPresence(user.lastSeen)}`;
        return `
          <button class="giChatUser ${active ? "is-active" : ""}" type="button" data-chat-user="${this.escapeAttr(user.id)}">
            ...
            <div class="giChatUser__meta">
              ...
              <div class="giChatUser__status">${this.escapeHtml(status)}</div>
            </div>
            ${unread ? `<span class="giChatUser__unread">${Math.min(unread,99)}</span>` : ''}
          </button>
        `;
      }).join("");
      $$('[data-chat-user]', wrap).forEach((btn) => {
        on(btn, 'click', () => {
          const user = this.getAvailableUsers().find((item) => item.id === btn.getAttribute('data-chat-user'));
    ...
    renderSelectedPeer(){
      ...
      const live = this.usersMap.get(this.selectedUser.id);
      const merged = { ...this.selectedUser, ...(live || {}) };
      this.selectedUser = merged;
      if(this.els.peerAvatar) this.els.peerAvatar.textContent = this.initials(merged.name);
      if(this.els.peerName) this.els.peerName.textContent = merged.name;
      if(this.els.peerStatus) this.els.peerStatus.textContent = merged.online ? (merged.typing ? 'מקליד עכשיו…' : 'מחובר עכשיו') : `לא מחובר · נראה לאחרונה ${this.formatPresence(merged.lastSeen)}`;
    },

    listenMessages(){
      if(!this.ready || !this.currentConversationId) return;
      this.messageListenerOff?.();
      this.renderedMessageIds = new Set();
      const messagesQuery = this.sdk.query(this.ref(`conversations/${this.currentConversationId}/messages`), this.sdk.limitToLast(120));
      this.messageListenerOff = this.sdk.onValue(messagesQuery, (snap) => {
        const raw = snap.val() || {};
        const now = Date.now();
        const entries = Object.entries(raw)
          ...
          .filter((msg) => {
            ...
            if(expiresAt && expiresAt <= now){
              this.cleanupExpiredMessage(this.currentConversationId, msg.id);
              return false;
            }
            return true;
          })
        ...
        if(this.els.messages) this.els.messages.innerHTML = '';
        this.renderedMessageIds = new Set(entries.map((msg) => msg.id));
        entries.forEach((msg) => {
          this.lastMessageByConversation.set(this.currentConversationId, { ...msg, id: msg.id });
          this.renderMessage(msg);
        });
        const hasIncoming = entries.some((msg) => msg.fromId && msg.fromId !== this.userKey);
        if(hasIncoming) this.markConversationRead(this.currentConversationId);
      });
    },

    listenTyping(){
      this.typingListenerOff?.();
      if(!this.selectedUser || !this.ready) return;
      const typingRef = this.ref(`typing/${this.currentConversationId}/${this.selectedUser.id}`);
      this.typingListenerOff = this.sdk.onValue(typingRef, (snap) => {
        const payload = snap.val();
        const active = !!(payload && payload.active === true && Number(payload.expiresAt || payload.updatedAt || 0) > Date.now());
        this.els.typing?.classList.toggle('is-hidden', !active);
        if(this.els.typingText) this.els.typingText.textContent = `${this.selectedUser?.name || 'הנציג'} מקליד…`;
        if(!active && payload){
          this.cleanupTyping(this.currentConversationId, this.selectedUser.id);
        }
        if(this.els.peerStatus && this.selectedUser){
          const live = this.usersMap.get(this.selectedUser.id);
          const text = active
            ? 'מקליד עכשיו…'
            : (live?.online ? 'מחובר עכשיו' : `לא מחובר · נראה לאחרונה ${this.formatPresence(live?.lastSeen)}`);
          this.els.peerStatus.textContent = text;
        }
      });
    },

    renderMessage(msg){
      const wrap = this.els.messages;
      if(!wrap) return;
      const mine = msg.fromId === this.userKey;
      const row = document.createElement('article');
      row.className = `giChatMsg ${mine ? 'giChatMsg--mine' : 'giChatMsg--peer'}`;
      row.innerHTML = `
        <div class="giChatMsg__bubble">${this.escapeHtml(msg.text || '')}</div>
        <div class="giChatMsg__meta">
          <span>${this.escapeHtml(mine ? 'אתה' : (msg.fromName || this.selectedUser?.name || 'נציג'))}</span>
          <span>${this.formatClock(msg.createdAt)}</span>
        </div>
      `;
      wrap.appendChild(row);
      wrap.scrollTop = wrap.scrollHeight + 120;
    },
    ...
    async sendMessage(){
      if(!this.enabled || !this.ready){
        alert('יש להשלים הגדרת Firebase בקובץ app.js כדי שצ׳אט הלייב יעבוד.');
        return;
      }
      if(!this.selectedUser){
        alert('בחר נציג מהרשימה כדי להתחיל שיחה.');
        return;
      }
      ...
      try {
        const msgRef = this.sdk.push(this.ref(`conversations/${this.currentConversationId}/messages`));
        const now = Date.now();
        ...
        await this.sdk.set(msgRef, payload);
        await this.sdk.update(this.ref(`conversation_meta/${this.currentConversationId}`), {
          updatedAt: now,
          expiresAt,
          lastMessageText: text.slice(0, 140),
          lastFromId: this.userKey,
          participants: { [this.userKey]: true, [this.selectedUser.id]: true },
        ...
        alert('לא הצלחתי לשלוח את ההודעה כרגע.');
      } finally {
    ...
    handleTypingPulse(){
      if(!this.enabled || !this.ready || !this.currentConversationId) return;
      this.setTyping(true);
    ...
    async setTyping(flag){
      if(!this.enabled || !this.ready || !this.currentConversationId || !this.userKey) return;
      try {
        const typingRef = this.ref(`typing/${this.currentConversationId}/${this.userKey}`);
        if(flag){
          const now = Date.now();
          await this.sdk.set(typingRef, {
            active: true,
            userId: this.userKey,
            updatedAt: now,
            expiresAt: now + this.typingWindowMs
          });
        } else {
          await this.sdk.remove(typingRef);
        }
      } catch(_e) {}
    },
    ...
    async markConversationRead(conversationId){
      if(!this.enabled || !this.ready || !conversationId || !this.userKey) return;
      try {
        await this.sdk.update(this.ref(`reads/${conversationId}/${this.userKey}`), { readAt: this.sdk.serverTimestamp() });
        await this.sdk.update(this.ref(`conversation_meta/${conversationId}/lastSeenBy`), { [this.userKey]: Date.now() });
      } catch(_e) {}
    },
    ...
    notifyIncoming(message){
      ...
      if(!(this.selectedUser?.id === message.fromId && !this.els.window?.classList.contains('is-hidden'))){
    ...
    async cleanupExpiredMessage(conversationId, messageId){
      if(!this.enabled || !this.ready || !conversationId || !messageId) return;
      try { await this.sdk.remove(this.ref(`conversations/${conversationId}/messages/${messageId}`)); } catch(_e) {}
    },

    async cleanupTyping(conversationId, userId){
      if(!this.enabled || !this.ready || !conversationId || !userId) return;
      try { await this.sdk.remove(this.ref(`typing/${conversationId}/${userId}`)); } catch(_e) {}
    },

    async cleanupConversation(conversationId){
      if(!this.enabled || !this.ready || !conversationId) return;
      try {
        await Promise.all([
          this.sdk.remove(this.ref(`conversations/${conversationId}`)),
          this.sdk.remove(this.ref(`conversation_meta/${conversationId}`)),
          this.sdk.remove(this.ref(`typing/${conversationId}`)),
          this.sdk.remove(this.ref(`reads/${conversationId}`))
        ]);
      } catch(_e) {}
      this.lastMessageByConversation.delete(conversationId);
      this.unreadByConversation.delete(conversationId);
      this.notifiedConversationAt.delete(conversationId);
      this.renderFabBadge();
      this.renderUsers();
    },

    clearActiveConversationIfExpired(conversationId){
      ...
      this.els.empty?.classList.remove('is-hidden');
      this.renderUsers();
    },
    ...
    startCleanupLoop(){
      ...
      this.cleanupTimer = setInterval(run, Math.max(15000, Number(FIREBASE_CHAT.cleanupIntervalMs || 60000)));
      run();
    },
    ...
    async cleanupExpiredData(){
      ...
      this.lastMessageByConversation.forEach((_item, conversationId) => conversationIds.add(conversationId));
      if(this.currentConversationId) conversationIds.add(this.currentConversationId);
      try {
        const metaSnap = await this.sdk.get(this.ref('conversation_meta'));
        const meta = metaSnap.val() || {};
        Object.keys(meta).forEach((conversationId) => {
          conversationIds.add(conversationId);
          const item = meta[conversationId] || {};
          if(Number(item.expiresAt || 0) && Number(item.expiresAt || 0) <= now){
            this.cleanupConversation(conversationId);
          }
        });
      } catch(_e) {}
      for(const conversationId of conversationIds){
        try {
          const snap = await this.sdk.get(this.ref(`conversations/${conversationId}/messages`));
          const raw = snap.val() || {};
          const staleIds = Object.entries(raw)
            .filter(([, msg]) => Number((msg || {}).expiresAt || 0) && Number((msg || {}).expiresAt || 0) <= now)
            .map(([id]) => id);
          await Promise.all(staleIds.map((id) => this.cleanupExpiredMessage(conversationId, id)));
          const freshLeft = Object.entries(raw).some(([, msg]) => !Number((msg || {}).expiresAt || 0) || Number((msg || {}).expiresAt || 0) > now);
          if(!freshLeft){
            await this.cleanupConversation(conversationId);
          }
        } catch(_e) {}
        try {
          const typingSnap = await this.sdk.get(this.ref(`typing/${conversationId}`));
          const typing = typingSnap.val() || {};
          const staleTyping = Object.entries(typing)
            .filter(([, payload]) => Number((payload || {}).expiresAt || 0) && Number((payload || {}).expiresAt || 0) <= now)
            .map(([userId]) => userId);
          await Promise.all(staleTyping.map((userId) => this.cleanupTyping(conversationId, userId)));
        } catch(_e) {}
      }
    },
    ...
    onLogout(){
      ...
      this.notifiedConversationAt = new Map();
      this.messageListenerOff?.(); this.messageListenerOff = null;
      this.usersListenerOff?.(); this.usersListenerOff = null;
      this.typingListenerOff?.(); this.typingListenerOff = null;
      this.connectedListenerOff?.(); this.connectedListenerOff = null;
      this.metaListenerOff?.(); this.metaListenerOff = null;
      if(this.ready && this.userKey){
        try { this.sdk.update(this.ref(`users/${this.userKey}`), { online:false, typing:false, lastSeen:this.sdk.serverTimestamp() }); } catch(_e) {}
        try { if(this.currentConversationId) this.sdk.remove(this.ref(`typing/${this.currentConversationId}/${this.userKey}`)); } catch(_e) {}
      }
      this.ready = false;
