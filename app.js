...
(() => {
  ...
  // ---------------------------
  // Insurance dictionaries (for "Existing Policies" step + logos)
  // Put logo files under: ./assets/companies/<filename>
  // ---------------------------
  const INSURANCE_COMPANIES = [
    { name: "הכשרה", logo: "achshara.png" },
    { name: "הפניקס", logo: "afenix.png" },
    { name: "כלל", logo: "clal.png" },
    { name: "הראל", logo: "harel.png" },
    { name: "מגדל", logo: "megdl.png" },
    { name: "מנורה", logo: "menora.png" },
    { name: "AIG", logo: "aig.png" },
    { name: "איילון", logo: "ayalon.png" },
    { name: "ביטוח ישיר", logo: "beytuyashir.png" }
  ];
  ...
  const getCompanyLogoPath = (companyName) => {
    const n = safeTrim(companyName);
    const hit = INSURANCE_COMPANIES.find(x => x.name === n);
    return hit && hit.logo ? ("./assets/companies/" + hit.logo) : "";
  };
...
if (!out.agents.length) {
  ...
  out.agents = (base.agents || []).map((a, idx) => ({
    ...
    out.customers = (out.customers || []).map((c) => ({
      ...
      postalCode: safeTrim(c?.postalCode),
  ...
  const UI = {
    ...
    init() {
      ...
      this.els.cfPostalCode = $("#cfPostalCode");
...
// Search (live filter on customers view)
      ...
      on(this.els.customerForm, "submit", async (e) => {
        ...
        const incoming = {
          ...
          postalCode: safeTrim(fd.get("postalCode"))
        
        // Existing Policies (from step 4)
        let policiesDraft = [];
        try{
          const raw = safeTrim(fd.get("policiesJson"));
          if(raw) policiesDraft = JSON.parse(raw) || [];
          if(!Array.isArray(policiesDraft)) policiesDraft = [];
        } catch(_){
          policiesDraft = [];
        }
};
        ...
        const customer = {
          ...
          policies: (policiesDraft || []).map(p => ({
            ...
            premium: Number(p?.premium || 0),
            status: "active",
            statusAction: safeTrim(p?.statusAction),
            partialCancelText: safeTrim(p?.partialCancelText)
          })),
          createdAt: nowISO(),
          updatedAt: nowISO()
        };
...
this.renderPolicies();
    ...
    openCustomerEdit(customerId){
      ...
      // reset policies draft UI
      try{ const h = this.els.customerForm.querySelector('#lcPoliciesJson'); if(h) h.value='[]'; const t=this.els.customerForm.querySelector('#lcPolList'); if(t) t.innerHTML=''; const hint=this.els.customerForm.querySelector('#lcPolAddHint'); if(hint) hint.textContent=''; }catch(_){ }
      const set = (name, val) => {
    ...
    openCustomerFull(customerId) {
      ...
      if (this.els.cfPostalCode) this.els.cfPostalCode.textContent = (c.postalCode || "—");
      ...
      this.els.cfPoliciesTbody.innerHTML = list.map(p => {
        ...
        return `
          <tr>
            ...
            <td><span class="lcCompanyCell">${(getCompanyLogoPath(p.company) ? ('<img alt="" src="' + escapeHtml(getCompanyLogoPath(p.company)) + '" />') : "")}<span>${escapeHtml(p.company || "")}</span></span>${(p.statusAction ? ('<div class="muted small">סטטוס: ' + escapeHtml(p.statusAction) + ((p.statusAction===\"ביטול חלקי\" && p.partialCancelText)? (' • נספחים: ' + escapeHtml(p.partialCancelText)) : "") + '</div>') : "")}</td>
  ...
  function setupCustomerFormUX(UI){
    try{
      ...
      const important = [
        ...
        "hmo","supplemental","idIssueDate","assignedAgent","gender","maritalStatus","childrenCount","postalCode"
      ];
      ...
      function go(step){
        ...
        try{
          ...
          sec && sec.scrollIntoView({ block: "start", behavior: "smooth" });
            ...
            // ===== Patch: BMI auto-calc (New Customer → Profile step)
      const bmiEl = form.querySelector("#newBmi");
      const hEl = form.querySelector("input[name='heightCm']");
      const wEl = form.querySelector("input[name='weightKg']");
      const calcBmi = () => {
        try{
          if(!bmiEl) return;
      ...
      on(hEl, "input", calcBmi);
      on(wEl, "input", calcBmi);

      // ===== Patch: Postal code lookup from address (New Customer → Contact step)
      const addrEl = form.querySelector("input[name='address']");
      const postalEl = form.querySelector("#newPostalCode") || form.querySelector("input[name='postalCode']");
      let _zipT = null;
      const fetchPostal = async (addr) => {
        const q = safeTrim(addr);
        if(!q) return "";
        try{
          ...
          const res = await fetchWithTimeout(url.toString(), { method:"GET", headers: { "Accept": "application/json" } }, 8000);
          const json = await res.json();
          const a = (Array.isArray(json) && json[0] && json[0].address) ? json[0].address : null;
          const pc = a ? (a.postcode || "") : "";
          return safeTrim(pc);
        } catch(_){
          return "";
        }
      ...
      const tryZipLookup = () => {
        if(_zipT) clearTimeout(_zipT);
        _zipT = setTimeout(async () => {
          try{
            if(!addrEl || !postalEl) return;
            const addr = safeTrim(addrEl.value);
            if(!addr) return;
            if(safeTrim(postalEl.value)) return;
            const pc = await fetchPostal(addr);
            if(pc) postalEl.value = pc;
          } catch(_){}
        }, 650);
      };
      on(addrEl, "blur", tryZipLookup);

      // ===== Patch: Existing Policies step (New Customer → Step 4)
      const pol = {
        company: form.querySelector("#lcPolCompany"),
        type: form.querySelector("#lcPolType"),
        number: form.querySelector("#lcPolNumber"),
        statusAction: form.querySelector("#lcPolStatusAction"),
        partialWrap: form.querySelector("#lcPartialWrap"),
        partialText: form.querySelector("#lcPolPartialText"),
        addBtn: form.querySelector("#lcPolAddBtn"),
        hint: form.querySelector("#lcPolAddHint"),
        list: form.querySelector("#lcPolList"),
        hidden: form.querySelector("#lcPoliciesJson"),
        logo: form.querySelector("#lcPolCompanyLogo")
      };

      const policiesState = { list: [] };

      const syncPoliciesHidden = () => {
        try{ if(pol.hidden) pol.hidden.value = JSON.stringify(policiesState.list || []); } catch(_){}
      };

      const renderPoliciesList = () => {
        if(!pol.list) return;
        const rows = (policiesState.list || []).map((p, idx) => {
          const company = escapeHtml(p.company || "");
          const type = escapeHtml(p.type || "");
          const num = escapeHtml(p.policyNumber || "");
          const st = escapeHtml(p.statusAction || "");
          const extra = (p.statusAction === "ביטול חלקי" && p.partialCancelText) ? ('<div class="muted small">נספחים: ' + escapeHtml(p.partialCancelText) + '</div>') : "";
          return `
            <tr>
              <td>${company}${extra}</td>
              <td>${type}</td>
              <td>${num}</td>
              <td>${st}</td>
              <td style="text-align:left"><button class="btn btn--danger" type="button" data-pol-del="${idx}">הסר</button></td>
            </tr>
          `;
        }).join("");
        pol.list.innerHTML = rows || `<tr><td colspan="5" class="muted" style="padding:18px">לא נוספו פוליסות עדיין</td></tr>`;
        $$("button[data-pol-del]", pol.list).forEach(btn => {
          on(btn, "click", () => {
            const i = Number(btn.dataset.polDel);
            if(Number.isFinite(i)) {
              policiesState.list.splice(i,1);
              syncPoliciesHidden();
              renderPoliciesList();
            }
          });
      ...
      const fillCompanySelect = () => {
        if(!pol.company) return;
        pol.company.innerHTML = `<option value="">בחר חברה…</option>` + INSURANCE_COMPANIES.map(x =>
      ...
      const fillTypeSelect = () => {
        if(!pol.type) return;
        pol.type.innerHTML = `<option value="">בחר סוג ביטוח…</option>` + INSURANCE_TYPES.map(t =>
      ...
      const updateCompanyLogo = () => {
        if(!pol.logo) return;
        const companyName = safeTrim(pol.company?.value);
        const src = getCompanyLogoPath(companyName);
        if(src){
          pol.logo.src = src;
          pol.logo.classList.add("is-show");
        } else {
          pol.logo.removeAttribute("src");
          pol.logo.classList.remove("is-show");
        }
      };

      const updatePartialUI = () => {
        const v = safeTrim(pol.statusAction?.value);
        const show = (v === "ביטול חלקי");
        if(pol.partialWrap) pol.partialWrap.style.display = show ? "" : "none";
        if(!show && pol.partialText) pol.partialText.value = "";
      };

      const addPolicy = () => {
        if(!pol.company || !pol.type || !pol.number || !pol.statusAction) return;
        const company = safeTrim(pol.company.value);
        const type = safeTrim(pol.type.value);
        const policyNumber = safeTrim(pol.number.value);
        const statusAction = safeTrim(pol.statusAction.value);
        const partialCancelText = safeTrim(pol.partialText?.value);

        if(!company || !type || !policyNumber || !statusAction){
          if(pol.hint) pol.hint.textContent = "נא למלא חברה, סוג, מספר פוליסה וסטטוס.";
          return;
        }
        if(statusAction === "ביטול חלקי" && !partialCancelText){
          if(pol.hint) pol.hint.textContent = "בביטול חלקי חובה לרשום נספחים לביטול.";
          return;
        }
        if(pol.hint) pol.hint.textContent = "";

        policiesState.list.push({ id: "p_" + uid(), company, type, policyNumber, statusAction, partialCancelText });
        syncPoliciesHidden();
        renderPoliciesList();

        pol.number.value = "";
        pol.statusAction.value = "";
        updatePartialUI();
      };
      ...
      on(pol.company, "change", updateCompanyLogo);
      on(pol.statusAction, "change", updatePartialUI);
      on(pol.addBtn, "click", addPolicy);
