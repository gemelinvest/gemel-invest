    ...
    collectPolicies(rec){
      ...
      (payload.insureds || []).forEach((ins, idx) => {
        ...
        (ins?.data?.existingPolicies || []).forEach((p, pIdx) => {
          ...
          const coverageValue = safeTrim(p?.sumInsured || p?.compensation || p?.coverage || "");
          policies.push({
            ...
            coverageLabel: (type === "מחלות קשות" || type === "סרטן") ? "סכום פיצוי" : "סכום ביטוח",
            coverageValue,
      ...
      (payload.newPolicies || []).forEach((p, idx) => {
        ...
        const coverageValue = safeTrim(p?.sumInsured || p?.compensation || p?.coverage || (Array.isArray(p?.healthCovers) ? p.healthCovers.join(", ") : ""));
...
init(){
      ...
      this.els.coversHintText = this.els.coversDrawer?.querySelector?.('.lcCoversHint') || null;
    ...
    renderCoversDrawer(pol, opts={}){
      ...
      const selectedList = Array.isArray(pol.healthCovers) ? pol.healthCovers : (Array.isArray(pol.covers) ? pol.covers : []);
      const selected = new Set(selectedList);
      if(this.els.coversDrawerTitle) this.els.coversDrawerTitle.textContent = String(opts.title || "בחירת כיסויי בריאות");
      if(this.els.coversHintText) this.els.coversHintText.textContent = String(opts.hint || "סמן את הכיסויים הרלוונטיים לפוליסה.");
    ...
    saveCoversDrawer(){
      try{
        ...
        if(ctx.kind === "newDraft"){
          this.ensurePolicyDraft();
          if(this.policyDraft?.type !== "בריאות") return this.closeCoversDrawer();
...
// open health covers drawer (Health only)
    ...
    addDraftPolicy(){
      ...
      const p = {
        ...
        healthCovers: Array.isArray(d.healthCovers) ? d.healthCovers.slice() : [],
    ...
    renderStep5(){
      ...
      const needHealthCovers = !isMedicare && (d.type === "בריאות");
      ...
      const renderHealthCoverSummary = (covers, opts={}) => {
        const list = Array.isArray(covers) ? covers.filter(Boolean) : [];
        const max = Number(opts.max || 3);
        if(!list.length){
          return opts.emptyLabel ? `<span class="lcPolCoverSummary__empty">${escapeHtml(opts.emptyLabel)}</span>` : "";
        }
        const visible = list.slice(0, max).map(label => `<span class="lcPolCoverChip">${escapeHtml(label)}</span>`).join('');
        const more = list.length > max ? `<span class="lcPolCoverChip lcPolCoverChip--more">+${list.length - max}</span>` : '';
        return `<div class="lcPolCoverSummary">${visible}${more}</div>`;
      };
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        const isMedicare = this.isMedicareCompany(p.company);
        const isHealth = !isMedicare && p.type === "בריאות";
        ...
        const coversBlock = isHealth ? `
          <div class="lcPolCoverRow">
            <div class="lcPolCoverRow__label">כיסויים שנבחרו</div>
            ${renderHealthCoverSummary(p.healthCovers, { max: 3, emptyLabel: "לא נבחרו כיסויים" })}
          </div>` : '';
        ...
        <div class="lcPolForm lcPolForm--premium">
          <div class="lcPolBuilderCard">
            ...
            <div class="lcPolGrid lcPolGrid--money">
              ...
              ${needHealthCovers ? `<div class="lcField lcPolField lcPolField--healthCovers">
                <label class="lcLabel">כיסויי בריאות</label>
                <div class="lcPolCoversInline">
                  <div class="lcPolCoversInline__summary">
                    ${renderHealthCoverSummary(d.healthCovers, { max: 4, emptyLabel: "טרם נבחרו כיסויים" })}
                  </div>
                  <button type="button" class="lcBtn lcBtn--ghost lcPolCoversInline__btn" data-open-new-health-covers="1">${Array.isArray(d.healthCovers) && d.healthCovers.filter(Boolean).length ? "עריכת כיסויים" : "אישור כיסויים"}</button>
                </div>
                <div class="help small muted">הכיסויים יישמרו בפוליסה, יוצגו בשורת הפוליסה ויעברו גם לדוח התפעולי ולתיק הלקוח.</div>
              </div>` : ``}
        ...
        $$("[data-pdraft]", this.els.body).forEach(el => {
          on(el, "input", () => {
            ...
            if(k === "type"){
              ...
              if(this.policyDraft.type === "בריאות"){
                const needOpen = !Array.isArray(this.policyDraft.healthCovers) || !this.policyDraft.healthCovers.length;
                this.render();
                if(needOpen) setTimeout(() => this.openNewPolicyCoversDrawer(), 40);
                return;
              }
            }
...
// ---------- Step 6 ----------
    ...
    renderStep7(){
      ...
      const renderNewSummaryTable = (list=[]) => {
        ...
        const rows = list.map(p => {
          ...
          const healthCoversTxt = Array.isArray(p.healthCovers) ? p.healthCovers.filter(Boolean).join(" · ") : "";
          const coverageTxt = isRisk
            ...
            : (isCI
              ...
              : (healthCoversTxt ? escapeHtml(healthCoversTxt) : "—"));
          let pledgeTxt = "—";
          if(isRisk){
    ...
    validateHealthDetail(question, insId){
      ...
      const required = (question.fields || []);
    ...
    renderOperationalReport(){
      ...
      const newRows = (payload.newPolicies || []).map(p => [
        ...
        this.renderReportValue(Array.isArray(p.healthCovers) ? p.healthCovers.join(", ") : ""),
        this.renderReportValue(p.premiumMonthly || p.premiumBefore),
      ...
      return `<div class="lcReportDoc">
        ...
        <section class="lcReportSection"><div class="lcReportSection__title">פוליסות חדשות</div>${this.renderTable(['חברה','סוג ביטוח','כיסויים','פרמיה חודשית','תאריך תחילה'], newRows)}</section>
