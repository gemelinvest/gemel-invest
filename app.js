...
(() => {
  ...
  const BUILD = "20260414-vDataRecoveryShield1";
  ...
  function normalizeAgentRecord(a, idx=0){
    ...
    const role = (roleRaw === "manager" || roleRaw === "adminLite" || roleRaw === "admin") ? "manager" : (roleRaw === "ops" || roleRaw === "operations" || roleRaw === "תפעול") ? "ops" : "agent";
    const createdAt = safeTrim(input?.createdAt || input?.created_at);
  ...
  function normalizeCustomerRecord(c, idx=0){
    ...
    const createdAt = safeTrim(c?.createdAt);
  ...
  function normalizeProposalRecord(p, idx=0){
    ...
    const createdAt = safeTrim(p?.createdAt);
  ...
  const Storage = {
    ...
    loadFastCache(_maxAgeMs = FAST_CACHE_MAX_AGE_MS){
      return null;
    },
    loadFastCacheAnyAge(){
      return null;
    },
    loadTableCache(_tableName){
      return null;
    },
    buildStateFromTableCaches(_maxAgeMs = FAST_CACHE_MAX_AGE_MS){
      return null;
    },
    saveBackup(st){
    ...
    clearLocalSnapshots(options = {}){
      const keepBackup = options && options.keepBackup === true;
    ...
    getShadowRecoveryRows(payload, liveRows = {}){
      const pickMissing = (shadowRows, currentRows) => {
        const liveIds = new Set((Array.isArray(currentRows) ? currentRows : []).map((row) => safeTrim(row?.id)).filter(Boolean));
        return (Array.isArray(shadowRows) ? shadowRows : []).filter((row) => {
          const id = safeTrim(row?.id);
          return !!id && !liveIds.has(id);
        });
      };
      return {
        agents: pickMissing(payload?.meta?.agentsShadow, liveRows.agents),
        customers: pickMissing(payload?.meta?.customersShadow, liveRows.customers),
        proposals: pickMissing(payload?.meta?.proposalsShadow, liveRows.proposals)
      };
    },

    async recoverServerRowsFromMeta(payload, liveRows = {}){
      try {
        const recoveryRows = this.getShadowRecoveryRows(payload, liveRows);
        const summary = {
          agents: recoveryRows.agents.length,
          customers: recoveryRows.customers.length,
          proposals: recoveryRows.proposals.length
        };
        const total = summary.agents + summary.customers + summary.proposals;
        if(!total) return { ok:true, recovered:false, summary };

        if(summary.agents){
          await this.syncTable(SUPABASE_TABLES.agents, this.buildAgentRows({ agents: recoveryRows.agents }), { allowDelete:false });
        }
        if(summary.customers){
          await this.syncTable(SUPABASE_TABLES.customers, this.buildCustomerRows({ customers: recoveryRows.customers }), { allowDelete:false });
        }
        if(summary.proposals){
          await this.syncTable(SUPABASE_TABLES.proposals, this.buildProposalRows({ proposals: recoveryRows.proposals }), { allowDelete:false });
        }

        appendAuditLog({
          type: 'server_shadow_recovery',
          entity: 'state',
          label: 'שוחזרו רשומות חסרות מהמטא',
          actorName: safeTrim(Auth?.current?.name || 'מערכת'),
          details: summary
        });

        return { ok:true, recovered:true, summary };
      } catch(e) {
        return { ok:false, recovered:false, error:String(e?.message || e) };
      }
    },
      ...
      return {
        ...
        payload: {
          ...
          agentsShadow: (state?.agents || []).map((a, idx) => ({
            ...
            updatedAt: safeTrim(a?.updatedAt || a?.updated_at) || safeTrim(a?.createdAt || a?.created_at) || ""
          })),
    ...
    buildAgentRows(state){
      return (state?.agents || []).map((a, idx) => {
        const createdAt = safeTrim(a?.createdAt || a?.created_at) || nowISO();
        const updatedAt = safeTrim(a?.updatedAt || a?.updated_at) || createdAt;
        return ({
          id: safeTrim(a?.id) || ("a_" + idx),
          name: safeTrim(a?.name) || "נציג",
          username: safeTrim(a?.username) || safeTrim(a?.name) || "נציג",
          ...
          role: safeTrim(a?.role) || "agent",
          active: a?.active === false ? false : true,
          birth_date: safeTrim(a?.birthDate || a?.birth_date) || null,
    ...
    buildCustomerRows(state){
      return (state?.customers || []).map((c, idx) => {
        const createdAt = safeTrim(c?.createdAt || c?.created_at) || nowISO();
        const updatedAt = safeTrim(c?.updatedAt || c?.updated_at) || createdAt;
        return ({
    ...
    buildProposalRows(state){
      return (state?.proposals || []).map((p, idx) => {
        const createdAt = safeTrim(p?.createdAt || p?.created_at) || nowISO();
        const updatedAt = safeTrim(p?.updatedAt || p?.updated_at) || createdAt;
        return ({
    ...
    async upsertMeta(state){
      ...
      try {
        ...
        try {
          ...
          throw new Error(String(secondaryErr?.message || primaryErr?.message || secondaryErr || primaryErr || 'META_SAVE_FAILED'));
        }
      }
    },
    ...
    mapMeta(metaRow){
      ...
      return {
        updatedAt: safeTrim(payload?.updatedAt) || safeTrim(metaRow?.updated_at) || '',
    ...
    async loadSheets(options = {}){
      const useCachedFallback = options.useCachedFallback === true && options.forceLegacyCache === true;
      try {
        const connection = await this.waitForConnection();
        if(!connection.ok){
          if(useCachedFallback){
            const backupState = this.loadBackup();
            const cached = this.loadFastCache() || this.buildStateFromTableCaches() || (backupState ? { ok:true, payload:backupState, at: safeTrim(backupState?.meta?.updatedAt) || nowISO(), source:'backup-cache' } : null);
        ...
        if(!metaRes.ok) return { ok:false, error: safeTrim(metaRes.error) || 'טעינת מטא מהשרת נכשלה' };
        if(!agentsRes.ok) return { ok:false, error: safeTrim(agentsRes.error) || 'טעינת נציגים מהשרת נכשלה' };
        if(!customersRes.ok) return { ok:false, error: safeTrim(customersRes.error) || 'טעינת לקוחות מהשרת נכשלה' };
        if(!proposalsRes.ok) return { ok:false, error: safeTrim(proposalsRes.error) || 'טעינת הצעות מהשרת נכשלה' };
        ...
        const recovery = await this.recoverServerRowsFromMeta({
          meta: mappedMeta,
          agents: agentsRows,
        ...
        if(recovery?.ok && recovery.recovered){
          this.clearLocalSnapshots();
          return await this.loadSheets({ useCachedFallback:false });
        }
        if(recovery?.ok === false){
          return { ok:false, error: 'שחזור נתונים חסרים מהשרת נכשל: ' + safeTrim(recovery.error) };
        }
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const App = {
    ...
    async boot(){
      Storage.restoreUrl();
      try { Storage.clearLocalSnapshots(); } catch(_e) {}
      SaveStatusUI.init();
