...
(() => {
  ...
  // ===========================
  // HOTFIX: robust login init + visible errors (BUILD 20260226-104501)
  // ===========================
  window.addEventListener("error", (ev) => {
    try {
      console.error("GLOBAL_ERROR:", ev?.error || ev?.message || ev);
      const box = document.getElementById("lcLoginError");
      if (box && !box.textContent) {
        box.textContent = "שגיאה במערכת. פתח קונסול (F12) לפרטים.";
      }
    } catch (_) {}
  });
...
const App = {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 1) Init UI (never block login if UI init fails)
    try {
      UI.init();
    } catch (e) {
      bootErr = e;
    ...
    // 2) Always init Auth so the form works
    try {
      Auth.init();
      try { UI.renderAuthPill(); } catch (_) {}
    } catch (e) {
      ...
      try {
        const box = document.getElementById("lcLoginError");
        if (box) box.textContent = "שגיאה בהפעלת מסך התחברות. פתח קונסול (F12) לפרטים.";
      } catch (_) {}
    }

    // 3) Boot data/state in background (Auth submit already tolerates failure)
    try {
      App._bootPromise = App.boot();
      await App._bootPromise;
      try { UI.applyRoleUI(); } catch (_) {}
    } catch (e) {
      bootErr = bootErr || e;
      console.error("App.boot error:", e);
      try {
        // Show a visible, non-blocking message (keeps login usable)
        const box = document.getElementById("lcLoginError");
        if (box && !box.textContent) {
          box.textContent = "חיבור נתונים נכשל. אפשר עדיין להתחבר כמנהל (מנהל מערכת / 1234).";
        }
      } catch (_) {}
    }

    // 4) If there was a boot error and notify exists, show toast
    if (bootErr) {
      try { notify("שגיאה בעליית המערכת. פתח קונסול (F12) לפרטים.", "error"); } catch (_) {}
    }
  });
  // Debug
