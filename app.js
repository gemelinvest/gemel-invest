...
(() => {
  ...
  const BUILD = "20260328-phoenix-form-page1-v1";
...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    init(){
      ...
      on(this.els.proposalBtn, "click", (ev) => {
        ev?.preventDefault?.();
        ev?.stopPropagation?.();
        const rec = this.current();
        if(!rec) return;
        try{
          ProposalFormUI.open(rec.id);
        }catch(err){
          console.error('PROPOSAL_FORM_OPEN_FAILED', err);
          window.alert('לא ניתן היה לפתוח את טופס ההצעה כרגע.');
        }
      });
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const ProposalFormUI = {
    currentId: null,
    els: {},
    previewUrl: '',
    init(){
      this.els.wrap = $("#proposalFormModal");
      this.els.backdrop = $("#proposalFormModalBackdrop");
      this.els.close = $("#proposalFormModalClose");
      this.els.cancel = $("#proposalFormCancel");
      this.els.previewBtn = $("#proposalFormPreviewBtn");
      this.els.downloadBtn = $("#proposalFormDownloadBtn");
      this.els.summary = $("#proposalFormSummary");
      this.els.status = $("#proposalFormStatus");
      this.els.previewWrap = $("#proposalFormPreviewWrap");
      this.els.previewFrame = $("#proposalFormPreviewFrame");
      on(this.els.backdrop, 'click', () => this.close());
      on(this.els.close, 'click', () => this.close());
      on(this.els.cancel, 'click', () => this.close());
      on(this.els.previewBtn, 'click', async () => { await this.generatePreview(); });
      on(this.els.downloadBtn, 'click', async () => { await this.downloadPdf(); });
      on(document, 'keydown', (ev) => {
        if(ev.key === 'Escape' && this.isOpen()) this.close();
      });
    },
    isOpen(){
      return !!(this.els.wrap && this.els.wrap.classList.contains('is-open'));
    },
    open(id){
      const rec = CustomersUI.byId(id);
      if(!rec || !this.els.wrap) return;
      ...
      try{
        ...
        this.setStatus('מוכן להפקת עמוד הניסוי של טופס הפניקס.');
      }catch(err){
        console.error('PROPOSAL_FORM_RENDER_FAILED', err);
        this.renderFallbackSummary(rec);
        this.setStatus('חלון ההפקה נפתח במצב בטוח. אפשר להמשיך לתצוגה מקדימה ולהורדה.');
      }
      this.els.wrap.classList.add('is-open');
      this.els.wrap.setAttribute('aria-hidden', 'false');
      document.body.classList.add('modal-open');
    ...
    current(){
      return CustomersUI.byId(this.currentId);
    },
    clearPreview(){
      if(this.previewUrl){
        try { URL.revokeObjectURL(this.previewUrl); } catch(_e) {}
      }
      this.previewUrl = '';
      if(this.els.previewFrame) this.els.previewFrame.removeAttribute('src');
      if(this.els.previewWrap) this.els.previewWrap.hidden = true;
    },
    setStatus(msg){
      if(this.els.status) this.els.status.textContent = safeTrim(msg) || '';
    },
    renderFallbackSummary(rec){
      if(!this.els.summary) return;
      const fullName = safeTrim(rec?.fullName) || '—';
      const idNumber = safeTrim(rec?.idNumber) || '—';
      const phone = safeTrim(rec?.phone) || '—';
      const city = safeTrim(rec?.city) || '—';
      const requestedStartDate = this.getRequestedStartDate(rec) || '—';
      this.els.summary.innerHTML = `
        <div class="proposalFormModal__summaryCard">
          <div class="proposalFormModal__summaryTitle">פרטי הלקוח שממנו יופק הטופס</div>
          <div class="proposalFormModal__summaryList">
            ${this.summaryRow('שם לקוח', fullName)}
            ${this.summaryRow('תעודת זהות', idNumber, true)}
            ${this.summaryRow('טלפון', phone, true)}
            ${this.summaryRow('עיר', city)}
            ${this.summaryRow('תאריך תחילה', requestedStartDate)}
          </div>
        </div>
        <div class="proposalFormModal__summaryCard">
          <div class="proposalFormModal__summaryTitle">מצב פתיחה בטוח</div>
          <div class="proposalFormModal__summaryList">
            ${this.summaryRow('סטטוס', 'נפתחה תצוגת גיבוי כדי שלא לחסום את ההפקה')}
            ${this.summaryRow('טופס', 'פניקס בריאות · עמוד 1')}
            ${this.summaryRow('המשך', 'אפשר להמשיך לתצוגה מקדימה ולהורדה')}
          </div>
        </div>`;
    },
    renderSummary(rec){
      if(!this.els.summary) return;
      const applicants = this.getApplicants(rec);
      const cards = [];
      ...
      cards.push(`
        <div class="proposalFormModal__summaryCard">
          <div class="proposalFormModal__summaryTitle">פרטי הלקוח שממנו יופק הטופס</div>
          <div class="proposalFormModal__summaryList">
            ${this.summaryRow('שם לקוח', rec?.fullName || '—')}
            ${this.summaryRow('תעודת זהות', rec?.idNumber || primary.idNumber || '—', true)}
            ${this.summaryRow('טלפון', rec?.phone || primary.mobile || '—', true)}
            ${this.summaryRow('עיר', rec?.city || primary.city || '—')}
            ${this.summaryRow('תאריך תחילה', this.getRequestedStartDate(rec) || '—')}
          </div>
        </div>`);
      cards.push(`
        <div class="proposalFormModal__summaryCard">
          <div class="proposalFormModal__summaryTitle">מבוטחים שזוהו אוטומטית</div>
          <div class="proposalFormModal__summaryList">
            ${applicants.length ? applicants.map((app, idx) => this.summaryRow(`מועמד ${idx+1}`, `${app.fullName || '—'} · ${app.kupatHolim || 'קופ״ח לא הוגדרה'} · ${app.deliveryMethodLabel || 'ללא דיוור'}`)).join('') : this.summaryRow('מועמדים', 'לא זוהו מבוטחים בתיק')}
          </div>
        </div>`);
      cards.push(`
        <div class="proposalFormModal__summaryCard">
          <div class="proposalFormModal__summaryTitle">מה נכלל בהפקת הניסוי</div>
          <div class="proposalFormModal__summaryList">
            ${this.summaryRow('טופס', 'פניקס בריאות · עמוד 1')}
            ${this.summaryRow('מילוי', 'פרטי מבוטחים, סימונים, דיוור ותאריך תחילה')}
            ${this.summaryRow('הערה', 'זהו בסיס ניסוי חי לעמוד 1 לצורך כיוונון מדויק')}
          </div>
        </div>`);
      this.els.summary.innerHTML = cards.join('');
    },
    summaryRow(label, value, ltr=false){
      return `<div class="proposalFormModal__summaryItem"><span>${escapeHtml(label)}</span><strong ${ltr ? 'dir="ltr"' : ''}>${escapeHtml(value || '—')}</strong></div>`;
    },
    normalizeDate(value){
      const raw = safeTrim(value);
      if(!raw) return '';
    ...
    splitDate(value){
      ...
      if(parts.length === 3) return { day: parts[0], month: parts[1], year: parts[2], full: normalized };
    ...
    getApplicants(rec){
      const insureds = CustomersUI.getInsureds(rec) || [];
      return insureds.slice(0, 6).map((ins, index) => this.normalizeApplicant(ins, index, rec)).filter(Boolean);
    },
    normalizeApplicant(ins, index, rec){
      const data = ins?.data || {};
      const fullName = `${safeTrim(data.firstName)} ${safeTrim(data.lastName)}`.trim();
      const marital = this.mapMaritalStatus(data.maritalStatus || data.familyStatus);
      const delivery = this.mapDeliveryMethod(data.deliveryMethod || data.documentDelivery || data.deliveryPreference);
      const gender = this.mapGender(data.gender || data.sex);
      const shaban = this.mapShaban(data.shaban ?? data.hasShaban ?? data.shabanLevel);
      const address = {
        street: safeTrim(data.street),
        houseNumber: safeTrim(data.houseNumber),
        city: safeTrim(data.city || rec?.city),
        zipCode: safeTrim(data.zip || data.zipCode)
      };
      return {
        index,
        type: safeTrim(ins?.type),
        fullName,
        firstName: safeTrim(data.firstName),
        lastName: safeTrim(data.lastName),
        idNumber: this.normalizeId(data.idNumber || rec?.idNumber),
        gender,
        marital,
        birthDate: this.splitDate(data.birthDate),
        kupatHolim: safeTrim(data.hmo || data.kupatHolim || data.clinic),
        shaban,
        occupation: safeTrim(data.occupation),
        dangerousHobbies: safeTrim(data.dangerousHobbies || data.hobbiesRisk || data.dangerousSports),
        mobile: safeTrim(data.phone || data.mobile || rec?.phone),
        email: safeTrim(data.email || rec?.email),
    ...
    mapGender(value){
      ...
      return { male: ['male','m','זכר'].includes(v), female: ['female','f','נקבה'].includes(v) };
    },
    mapMaritalStatus(value){
      const v = safeTrim(value);
      return {
        single: ['single','רווק','רווקה','רווק/ה'].includes(v),
        married: ['married','נשוי','נשואה','נשוי/אה'].includes(v),
        divorced: ['divorced','גרוש','גרושה','גרוש/ה'].includes(v),
        other: ['widowed','אלמן','אלמנה','אלמן/ה','other','אחר','ידוע בציבור','ידוע/ה בציבור'].includes(v)
      };
    },
    mapShaban(value){
      if(value === true) return { yes:true, no:false };
      if(value === false) return { yes:false, no:true };
      const v = safeTrim(value).toLowerCase();
      if(!v) return { yes:false, no:false };
      if(['כן','yes','true','active','יש'].includes(v)) return { yes:true, no:false };
      if(['לא','no','false','none','אין'].includes(v)) return { yes:false, no:true };
      return { yes:true, no:false };
    },
    mapDeliveryMethod(value){
      const v = safeTrim(value).toLowerCase();
      if(['post','mail_post','דואר','דואר ישראל'].includes(v)) return { value:'post', label:'דואר ישראל' };
      if(['email','e-mail','mail','מייל','דוא"ל','דואל'].includes(v)) return { value:'email', label:'e-mail' };
      return { value:'email', label:'e-mail' };
    },
    getRequestedStartDate(rec){
      const payload = rec?.payload || {};
      const direct = payload.requestedStartDate || payload.policyStartDate || payload.coverageStartDate;
      if(safeTrim(direct)) return this.normalizeDate(direct);
      const policies = CustomersUI.getNewPolicies(rec) || [];
      const firstPolicyDate = policies.map((p) => safeTrim(p?.startDate || p?.policyStartDate || p?.beginDate)).find(Boolean);
      return this.normalizeDate(firstPolicyDate);
    },
    createExportNode(rec){
      const applicants = this.getApplicants(rec);
      const requestedStart = this.splitDate(this.getRequestedStartDate(rec));
      const page = document.createElement('div');
      page.className = 'proposalFormSheet';
      page.setAttribute('dir', 'rtl');
      const addText = (x, y, text, cls='proposalFormField proposalFormField--medium proposalFormField--center', extra='') => {
        if(!safeTrim(text)) return '';
        return `<div class="${cls}" style="left:${x}px; top:${y}px; ${extra}">${escapeHtml(String(text))}</div>`;
      };
      const addCheck = (x, y, on) => on ? `<div class="proposalFormCheck" style="left:${x}px; top:${y}px;">✓</div>` : '';
      const addBoxed = (startX, y, value, step=17, cls='proposalFormField proposalFormField--small proposalFormField--center') => {
        const digits = this.normalizeId(value);
        return digits.split('').map((digit, idx) => `<div class="${cls}" style="left:${startX + (idx*step)}px; top:${y}px;">${digit}</div>`).join('');
      };
      const addSplitDate = (startX, y, dateObj, step=17) => {
        if(!dateObj?.day) return '';
        const all = `${dateObj.day}${dateObj.month}${dateObj.year}`;
        return all.split('').map((digit, idx) => `<div class="proposalFormField proposalFormField--small proposalFormField--center" style="left:${startX + (idx*step)}px; top:${y}px;">${digit}</div>`).join('');
      };
      const colCenters = [655, 561, 468, 374, 281, 188];
      const rows = {
        lastName: 454, firstName: 486, id: 522, gender: 561, marital: 595, birth: 631, kupat: 666, shaban: 704, occupation: 741, hobbies: 776, mobile: 812, email: 848, deliveryEmail: 970, deliveryPost: 995, street: 1056, house: 1092, city: 1128, zip: 1166
      };
      const fontText = 'font-size:12px; max-width:84px; overflow:hidden;';
      const pieces = [];
      applicants.forEach((app, idx) => {
        const cx = colCenters[idx];
        pieces.push(addText(cx + 26, rows.lastName, app.lastName, 'proposalFormField proposalFormField--small proposalFormField--center', fontText));
        pieces.push(addText(cx + 26, rows.firstName, app.firstName, 'proposalFormField proposalFormField--small proposalFormField--center', fontText));
        pieces.push(addBoxed(cx - 37, rows.id, app.idNumber, 12.4));
        pieces.push(addCheck(cx + 16, rows.gender, app.gender.male));
        pieces.push(addCheck(cx + 61, rows.gender, app.gender.female));
        pieces.push(addCheck(cx - 36, rows.marital, app.marital.single));
        pieces.push(addCheck(cx - 10, rows.marital, app.marital.married));
        pieces.push(addCheck(cx + 17, rows.marital, app.marital.divorced));
        pieces.push(addCheck(cx + 44, rows.marital, app.marital.other));
        pieces.push(addSplitDate(cx - 37, rows.birth, app.birthDate, 12.2));
        pieces.push(addText(cx + 18, rows.kupat, app.kupatHolim, 'proposalFormField proposalFormField--small proposalFormField--center', 'font-size:11px; max-width:92px; overflow:hidden;'));
        pieces.push(addCheck(cx + 12, rows.shaban, app.shaban.yes));
        pieces.push(addCheck(cx + 50, rows.shaban, app.shaban.no));
        pieces.push(addText(cx + 24, rows.occupation, app.occupation, 'proposalFormField proposalFormField--tiny proposalFormField--center', 'font-size:10px; max-width:98px; overflow:hidden;'));
        pieces.push(addText(cx + 24, rows.hobbies, app.dangerousHobbies, 'proposalFormField proposalFormField--tiny proposalFormField--center', 'font-size:10px; max-width:98px; overflow:hidden;'));
        pieces.push(addText(cx + 26, rows.mobile, app.mobile, 'proposalFormField proposalFormField--tiny proposalFormField--center', 'font-size:10px; max-width:96px; overflow:hidden;'));
        pieces.push(addText(cx + 26, rows.email, app.email, 'proposalFormField proposalFormField--tiny proposalFormField--center', 'font-size:9px; max-width:104px; overflow:hidden;'));
        pieces.push(addCheck(cx + 60, rows.deliveryEmail, app.deliveryMethod === 'email'));
        pieces.push(addCheck(cx + 60, rows.deliveryPost, app.deliveryMethod === 'post'));
        pieces.push(addText(cx + 20, rows.street, app.street, 'proposalFormField proposalFormField--tiny proposalFormField--center', 'font-size:10px; max-width:96px; overflow:hidden;'));
        pieces.push(addText(cx + 24, rows.house, app.houseNumber, 'proposalFormField proposalFormField--small proposalFormField--center', 'font-size:10px; max-width:90px; overflow:hidden;'));
        pieces.push(addText(cx + 24, rows.city, app.city, 'proposalFormField proposalFormField--tiny proposalFormField--center', 'font-size:10px; max-width:92px; overflow:hidden;'));
        pieces.push(addText(cx + 24, rows.zip, app.zipCode, 'proposalFormField proposalFormField--tiny proposalFormField--center', 'font-size:10px; max-width:92px; overflow:hidden;'));
      });
      if(requestedStart?.day){
        pieces.push(addSplitDate(259, 147, requestedStart, 15.2));
      }
      page.innerHTML = pieces.join('');
      return page;
    },
    async withExportNode(rec, fn){
      const host = document.createElement('div');
      host.className = 'proposalFormExportHost';
      const node = this.createExportNode(rec);
    ...
    getFilename(rec){
      const base = safeTrim(rec?.fullName || 'לקוח').replace(/[\/:*?"<>|]/g, '').replace(/\s+/g, '_');
      return `Phoenix_Form_Page1_${base || 'customer'}.pdf`;
    },
    async buildBlobUrl(){
      const rec = this.current();
      if(!rec) throw new Error('NO_CUSTOMER');
      return await this.withExportNode(rec, async (node) => {
        const worker = window.html2pdf().set(this.pdfOptions(rec)).from(node).toPdf();
    ...
    async generatePreview(){
      const rec = this.current();
      if(!rec) return;
      this.setStatus('מייצר תצוגה מקדימה של עמוד הניסוי...');
      this.els.previewBtn.disabled = true;
      const originalText = this.els.previewBtn.textContent;
      this.els.previewBtn.textContent = 'מייצר תצוגה...';
      try{
        const blobUrl = await this.buildBlobUrl();
        this.clearPreview();
        this.previewUrl = blobUrl;
        if(this.els.previewFrame) this.els.previewFrame.src = blobUrl;
        if(this.els.previewWrap) this.els.previewWrap.hidden = false;
        this.setStatus('התצוגה המקדימה נוצרה. אפשר לבדוק ישיבה של השדות ולהוריד PDF.');
      }catch(err){
        console.error('PROPOSAL_PREVIEW_FAILED', err);
        this.setStatus('לא ניתן היה ליצור תצוגה מקדימה. רענן ונסה שוב.');
        window.alert('לא ניתן היה ליצור תצוגה מקדימה של הטופס כרגע.');
      }finally{
        this.els.previewBtn.disabled = false;
        this.els.previewBtn.textContent = originalText;
      }
    },
    async downloadPdf(){
      const rec = this.current();
      if(!rec) return;
      this.setStatus('מייצר קובץ PDF להורדה...');
      this.els.downloadBtn.disabled = true;
      const originalText = this.els.downloadBtn.textContent;
      this.els.downloadBtn.textContent = 'מייצר PDF...';
      try{
        await this.withExportNode(rec, async (node) => {
          await window.html2pdf().set(this.pdfOptions(rec)).from(node).save();
        });
        this.setStatus('הקובץ הופק והורד. זהו עמוד הניסוי הראשון לצורך כיוונון טופס פניקס.');
      }catch(err){
        console.error('PROPOSAL_DOWNLOAD_FAILED', err);
        this.setStatus('לא ניתן היה להוריד את הקובץ כרגע.');
        window.alert('לא ניתן היה להוריד את טופס ההצעה כרגע.');
      }finally{
        this.els.downloadBtn.disabled = false;
        this.els.downloadBtn.textContent = originalText;
      }
    }
  };
