      ...
      const head = isCaseLevel ? `<div class="lcWSection">
        <div class="row row--between">
          <div>
            ...
            <div class="muted small">שלב זה משותף לכל התיק (לא לפי מבוטח).</div>
          </div>
        </div>
  ...
  return;
    ...
    ensurePolicyDraft(){
      ...
      this.policyDraft = {
        ...
        sumInsured: "",
        compensation: "",
        // pledge (risk / risk mortgage)
        pledge: false,
        pledgeBank: { bankName:"", bankNo:"", branch:"", amount:"", years:"", address:"" },
        // beneficiaries (risk only)
        beneficiaryMode: "legal", // legal / id
        beneficiaries: [] // [{firstName,lastName,idNumber,birthDate,relation,percent}]
      };
      if(!spouse){
    ...
    addDraftPolicy(){
      ...
      const p = {
        ...
        discountPercent: (safeTrim(d.discountPercent) || "0"),
        // product-specific
        ...
        // beneficiaries (risk only)
        beneficiaryMode: (d.beneficiaryMode || "legal"),
        beneficiaries: Array.isArray(d.beneficiaries) ? d.beneficiaries.map(b => Object.assign({ firstName:"", lastName:"", idNumber:"", birthDate:"", relation:"", percent:"" }, b||{})) : []
      };
      ...
      this.policyDraft.premiumBefore = "";
      this.policyDraft.discountPercent = "0";
      ...
      this.policyDraft.beneficiaryMode = "legal";
    ...
    quickAddMedicare(){
      // Adds "מדיקר" instantly to the policy list (no company selection)
      this.ensurePolicyDraft();
      const d = this.policyDraft;

      // require insured linkage mode validity
      if(d.insuredMode === "couple"){
        const spouse = this.insureds.find(x => x.type === "spouse");
        if(!spouse){
          this.setHint("כדי להוסיף פוליסה זוגית יש להוסיף בן/בת זוג בשלב 1");
          return;
        }
      }else{
        if(!safeTrim(d.insuredId)){
          this.setHint("בחר למי שייכת הפוליסה");
          return;
        }
      }
      ...
      const p = {
        ...
        pledge: false,
        pledgeBank: { bankName:"", bankNo:"", branch:"", amount:"", years:"", address:"" },
        beneficiaryMode: "legal",
        beneficiaries: []
      };

      this.newPolicies = (this.newPolicies || []);
      this.newPolicies.push(p);
      this.setHint("נוספה פוליסת מדיקר — נא למלא פרמיה חודשית בשורה שלה");
      this.render();
    },
    ...
    validateStep5(){
      ...
      const bad = list.filter(p => {
        ...
        // pricing required
        if(!safeTrim(p.premiumBefore)) return true;
        const disc = Number(String(p.discountPercent||"0").replace(",","."));
        if(Number.isNaN(disc) || disc < 0 || disc > 100) return true;
        ...
        // pledge for risk/mortgage
        if((p.type === "ריסק" || p.type === "ריסק משכנתא") && p.pledge){
          const b = p.pledgeBank || {};
          const req = ["bankName","bankNo","branch","amount","years","address"];
          if(!req.every(k => safeTrim(b[k]))) return true;
        }

        // beneficiaries only for risk
        if(p.type === "ריסק"){
          const mode = safeTrim(p.beneficiaryMode || "legal");
          if(mode === "id"){
            const arr = Array.isArray(p.beneficiaries) ? p.beneficiaries : [];
            if(arr.length < 1) return true;
            for(const b of arr){
              if(!safeTrim(b.firstName) || !safeTrim(b.lastName) || !safeTrim(b.idNumber) || !safeTrim(b.birthDate) || !safeTrim(b.relation)) return true;
            }
            if(arr.length >= 2){
              const sum = arr.reduce((a,b)=> a + (Number(String(b.percent||"0").replace(",","."))||0), 0);
              if(Math.abs(sum - 100) > 0.001) return true;
              if(arr.some(b => (Number(String(b.percent||"0").replace(",","."))||0) <= 0)) return true;
            }
          }
        }
    ...
    renderStep5(){
      ...
      const calcAfter = (before, discPct) => {
        const b = Number(String(before||"").replace(",",".")); 
        const p = Number(String(discPct||"0").replace(",",".")); 
        if(!isFinite(b) || b < 0) return "";
        const pct = (isFinite(p) && p >= 0) ? Math.min(100, Math.max(0, p)) : 0;
        const a = b * (1 - pct/100);
        // keep 2 decimals if needed
        const out = (Math.round(a * 100) / 100).toString();
        return out;
      };

      const insuredOpts = this.insureds.map(ins => `<option value="${ins.id}"${d.insuredId===ins.id?" selected":""}>${escapeHtml(ins.label)}</option>`).join("");

      // company cards + special Medicare tile
      const coItems = [...this.companies, "__MEDICARE__"];
      const companyCards = coItems.map(c => {
        const isMed = (c === "__MEDICARE__");
        const name = isMed ? "מדיקר" : c;
        const src = isMed ? "medicare.png" : this.getCompanyLogoSrc(c);
        const selected = (!isMed && d.company === c);
        const cls = "lcCoCard" + (selected ? " is-selected" : "") + (isMed ? " lcCoCard--medicare" : "");
        const logo = src ? `<img class="lcCoLogo" src="${escapeHtml(src)}" alt="${escapeHtml(name)}" />`
                         : `<div class="lcCoLogo lcCoLogo--text">${escapeHtml(name)}</div>`;
        const sub = isMed ? `<div class="lcCoSub">מוצר עצמאי</div>` : ``;
        return `<button type="button" class="${cls}" data-co="${escapeHtml(c)}">${logo}<div class="lcCoName">${escapeHtml(name)}</div>${sub}</button>`;
      }).join("");
      ...
      const pledgeApplies = (d.type === "ריסק" || d.type === "ריסק משכנתא");
      const isRisk = (d.type === "ריסק");
      const isMortgage = (d.type === "ריסק משכנתא");

      const afterPreview = calcAfter(d.premiumBefore, d.discountPercent);
      ...
      const renderBeneficiariesSummary = (p) => {
        if(p.type !== "ריסק") return "";
        const mode = safeTrim(p.beneficiaryMode || "legal");
        if(mode === "legal") return `<div class="small muted">מוטבים: <b>יורשים חוקיים</b></div>`;
        const n = Array.isArray(p.beneficiaries) ? p.beneficiaries.length : 0;
        return `<div class="small muted">מוטבים: <b>לפי ת״ז (${n})</b></div>`;
      };

      const renderPolicyCard = (p, showCoupleBadge=false) => {
        const src = this.getCompanyLogoSrc(p.company) || (p.company === "מדיקר" ? "medicare.png" : "");
        ...
        const after = calcAfter(p.premiumBefore, p.discountPercent);

        // pricing (center block)
        const priceRow = `<div class="lcPolPriceRow" aria-label="מחיר והנחה">
          <div class="lcPolPriceField">
            <div class="small muted">פרמיה לפני הנחה</div>
            <input class="lcInput lcInput--sm" inputmode="decimal" data-pol-premium="${escapeHtml(p.id)}" value="${escapeHtml(p.premiumBefore||"")}" placeholder="₪" />
          </div>
          <div class="lcPolPriceField">
            <div class="small muted">הנחה (%)</div>
            <input class="lcInput lcInput--sm" inputmode="decimal" data-pol-discount="${escapeHtml(p.id)}" value="${escapeHtml((p.discountPercent ?? "0"))}" placeholder="0" />
          </div>
          <div class="lcPolPriceField">
            <div class="small muted">אחרי הנחה</div>
            <div class="lcPolAfter" data-pol-after="${escapeHtml(p.id)}">${escapeHtml(after||"")}</div>
          </div>
        </div>`;

        let metaLines = "";
        if(p.type === "סרטן" || p.type === "מחלות קשות"){
          metaLines += `<div class="small muted">סכום פיצוי: <b>${escapeHtml(p.compensation || "")}</b></div>`;
        }
        if(p.type === "ריסק" || p.type === "ריסק משכנתא"){
          metaLines += `<div class="small muted">סכום ביטוח: <b>${escapeHtml(p.sumInsured || "")}</b></div>`;
        }
        if((p.type === "ריסק" || p.type === "ריסק משכנתא") && p.pledge){
          metaLines += `<div class="small muted">שיעבוד: <b>כן</b></div>`;
        }
        metaLines += renderBeneficiariesSummary(p);
        ...
        return `<div class="lcPolCard" data-pol="${p.id}">
          ...
          <div class="lcPolMid">${priceRow}</div>
        </div>`;
      };
      ...
      const emptyNote = !list.length ? `<div class="muted small">עדיין לא נוספו פוליסות.</div>` : "";

      // beneficiaries UI for draft (risk only)
      const relOpts = ["אב","אם","אח","אחות","קרוב משפחה"];
      const benMode = safeTrim(d.beneficiaryMode || "legal");
      const benRows = Array.isArray(d.beneficiaries) ? d.beneficiaries : [];
      const benTable = (benMode === "id") ? `
        <div class="lcWSection" style="margin-top:12px">
          <div class="lcWTitle">מוטבים לפי ת״ז</div>
          <div class="muted small" style="margin-bottom:8px">מלא את פרטי המוטבים. אם יש יותר ממוטב אחד — חלק באחוזים (סה״כ 100%).</div>
          <div class="lcBenWrap">
            ${benRows.map((b, idx) => {
              const pctVisible = (benRows.length >= 2);
              const pctField = pctVisible ? `
                <div class="lcField">
                  <label class="lcLabel">אחוז</label>
                  <input class="lcInput" inputmode="decimal" data-ben-pct="${idx}" value="${escapeHtml(b.percent||"")}" placeholder="%" />
                </div>` : ``;
              return `<div class="lcBenRow" data-ben-row="${idx}">
                <div class="lcWGrid lcWGrid--3">
                  <div class="lcField"><label class="lcLabel">שם פרטי</label><input class="lcInput" data-ben-fn="${idx}" value="${escapeHtml(b.firstName||"")}" /></div>
                  <div class="lcField"><label class="lcLabel">שם משפחה</label><input class="lcInput" data-ben-ln="${idx}" value="${escapeHtml(b.lastName||"")}" /></div>
                  <div class="lcField"><label class="lcLabel">ת״ז</label><input class="lcInput" inputmode="numeric" data-ben-id="${idx}" value="${escapeHtml(b.idNumber||"")}" /></div>
                  <div class="lcField"><label class="lcLabel">תאריך לידה</label><input class="lcInput" type="date" data-ben-bd="${idx}" value="${escapeHtml(b.birthDate||"")}" /></div>
                  <div class="lcField">
                    <label class="lcLabel">קרבה</label>
                    <select class="lcSelect" data-ben-rel="${idx}">
                      <option value="">בחר</option>
                      ${relOpts.map(r => `<option value="${escapeHtml(r)}"${safeTrim(b.relation)===r?" selected":""}>${escapeHtml(r)}</option>`).join("")}
                    </select>
                  </div>
                  ${pctField}
                </div>
                <div class="row row--between" style="margin-top:8px">
                  <div class="muted small">${(benRows.length>=2 && safeTrim(d.sumInsured)) ? `` : ``}</div>
                  <button type="button" class="lcBtn lcBtn--ghost lcBtn--danger" data-ben-del="${idx}">הסר מוטב</button>
                </div>
              </div>`;
            }).join("")}
            <button type="button" class="lcBtn lcBtn--ghost" data-ben-add="1">+ הוסף מוטב</button>
          </div>
        </div>` : ``;

      const benChooser = isRisk ? `
        <div class="lcWSection" style="margin-top:12px">
          <div class="lcWTitle">מוטבים (ריסק בלבד)</div>
          <div class="lcWGrid">
            <div class="field">
              <label class="label">בחירת מוטבים</label>
              <select class="inp" data-pdraft="beneficiaryMode">
                <option value="legal"${benMode==="legal"?" selected":""}>יורשים חוקיים</option>
                <option value="id"${benMode==="id"?" selected":""}>לפי ת״ז</option>
              </select>
            </div>
          </div>
        </div>` : ``;

      // form for adding a policy (regular companies only)
      const form = `<div class="lcWSection">
        <div class="lcWTitle">הוספת פוליסה חדשה</div>

        <div class="lcWGrid">
          <div class="field">
            <label class="label">שיוך פוליסה</label>
            <div class="row" style="gap:8px;flex-wrap:wrap">
              <button type="button" class="lcBtn lcBtn--ghost ${d.insuredMode==='single'?'is-active':''}" data-pdraftmode="single">מבוטח</button>
              ${(spouse) ? `<button type="button" class="lcBtn lcBtn--ghost ${d.insuredMode==='couple'?'is-active':''}" data-pdraftmode="couple">זוגי (ראשי + בן/בת זוג)</button>` : ``}
            </div>
            ${d.insuredMode==='single' ? `<div style="margin-top:8px"><select class="inp" data-pdraft="insuredId">${insuredOpts}</select></div>` : `<div class="muted small" style="margin-top:6px">פוליסה זוגית תופיע אצל שני המבוטחים.</div>`}
          </div>
        </div>

        <div class="lcWSection" style="margin-top:12px">
          <div class="muted small">בחר חברה (או “מדיקר” להוספה מהירה).</div>
          <div class="lcCoGrid">${companyCards}</div>
        </div>

        ${(safeTrim(d.company)) ? `<div class="lcWSection" style="margin-top:12px">
          <div class="lcWGrid">
            <div class="field">
              <label class="label">מוצר ביטוח</label>
              <select class="inp" data-pdraft="type">
          ...
          ${safeTrim(d.type) ? `<div class="lcWGrid lcWGrid--3" style="margin-top:8px">
            <div class="lcField">
              <label class="lcLabel">פרמיה חודשית לפני הנחה</label>
              <input class="lcInput" inputmode="decimal" data-pdraft="premiumBefore" value="${escapeHtml(d.premiumBefore||"")}" placeholder="₪" />
            </div>
            <div class="lcField">
              <label class="lcLabel">אחוז הנחה (%)</label>
              <input class="lcInput" inputmode="decimal" data-pdraft="discountPercent" value="${escapeHtml((d.discountPercent ?? "0"))}" placeholder="0" />
            </div>
            <div class="lcField">
              <label class="lcLabel">אחרי הנחה</label>
              <div class="lcAfterBox">${escapeHtml(afterPreview||"")}</div>
            </div>
          </div>` : ``}

          ${needComp ? `<div class="lcWGrid" style="margin-top:8px">
            <div class="field">
              <label class="label">סכום פיצוי (חובה)</label>
              <input class="inp" data-pdraft="compensation" value="${escapeHtml(d.compensation||"")}" inputmode="numeric" placeholder="₪" />
            </div>
          </div>` : ``}

          ${needSum ? `<div class="lcWGrid" style="margin-top:8px">
            <div class="field">
              <label class="label">סכום ביטוח (חובה)</label>
              <input class="inp" data-pdraft="sumInsured" value="${escapeHtml(d.sumInsured||"")}" inputmode="numeric" placeholder="₪" />
            </div>
          </div>` : ``}

          ${pledgeApplies ? `<div class="lcWGrid" style="margin-top:8px">
            <div class="field">
              <label class="label"><input type="checkbox" data-pdraft="pledge" ${d.pledge?'checked':''} /> שיעבוד פוליסה</label>
              <div class="muted small">אם מסומן — יש למלא פרטי מוטב בלתי חוזר (בנק).</div>
            </div>
          </div>` : ``}

          ${(pledgeApplies && d.pledge) ? `<div class="lcWSection" style="margin-top:8px">
          ...
          ${benChooser}
          ${(isRisk && benMode==="id") ? benTable : ``}

          <div class="row row--between" style="margin-top:10px">
            <div class="muted small">קודם בוחרים חברה, ואז מוצר.</div>
            <button type="button" class="lcBtn lcBtn--premium" data-addpol="1"><span class="lcBtn__icon" aria-hidden="true">＋</span> הוסף פוליסה</button>
          </div>
        </div>` : ``}
      </div>`;

      const res = `<div class="lcNewPolicies">`+
      `<div class="lcWSection">
        <div class="muted small">חובה להוסיף לפחות פוליסה אחת. “מדיקר” מתווסף מהר ולא דורש בחירת חברה.</div>
      ...
      setTimeout(() => {
        ...
        $$("[data-pdraftmode]", this.els.body).forEach(b => {
          on(b, "click", () => {
            ...
            if(mode === "couple" && !spouse){
              this.setHint("כדי להוסיף פוליסה זוגית יש להוסיף בן/בת זוג בשלב 1");
              return;
            }
            this.policyDraft.insuredMode = mode || "single";
            if(this.policyDraft.insuredMode === "single" && !safeTrim(this.policyDraft.insuredId)){
              this.policyDraft.insuredId = this.insureds[0]?.id || "";
            }
            this.render();
          });
        });
        ...
        $$("[data-pdraft]", this.els.body).forEach(el => {
          on(el, "input", () => {
            ...
            // structural changes
            if(k === "type" || k === "pledge" || k === "insuredId" || k === "beneficiaryMode") this.render();
          });
          on(el, "change", () => {
            this.ensurePolicyDraft();
            const k = el.getAttribute("data-pdraft");
        ...
        // beneficiaries (draft) handlers
        const addBen = this.els.body.querySelector('[data-ben-add="1"]');
        if(addBen){
          on(addBen, "click", () => {
            this.ensurePolicyDraft();
            this.policyDraft.beneficiaries = Array.isArray(this.policyDraft.beneficiaries) ? this.policyDraft.beneficiaries : [];
            this.policyDraft.beneficiaries.push({ firstName:"", lastName:"", idNumber:"", birthDate:"", relation:"", percent:"" });
            this.render();
          });
        }
        $$("[data-ben-del]", this.els.body).forEach(btn => {
          on(btn, "click", () => {
            this.ensurePolicyDraft();
            const i = Number(btn.getAttribute("data-ben-del"));
            if(!Array.isArray(this.policyDraft.beneficiaries)) this.policyDraft.beneficiaries = [];
            this.policyDraft.beneficiaries.splice(i,1);
            this.render();
          });
        });
        const bindBenField = (sel, key) => {
          $$(`[${sel}]`, this.els.body).forEach(el => {
            on(el, "input", () => {
              this.ensurePolicyDraft();
              const i = Number(el.getAttribute(sel));
              if(!Array.isArray(this.policyDraft.beneficiaries)) this.policyDraft.beneficiaries = [];
              const row = this.policyDraft.beneficiaries[i] || (this.policyDraft.beneficiaries[i] = {});
              row[key] = el.value;
            });
          });
        };
        bindBenField("data-ben-fn","firstName");
        bindBenField("data-ben-ln","lastName");
        bindBenField("data-ben-id","idNumber");
        bindBenField("data-ben-bd","birthDate");
        bindBenField("data-ben-rel","relation");
        bindBenField("data-ben-pct","percent");
        ...
        // inline policy pricing edits
        $$("[data-pol-premium]", this.els.body).forEach(inp => {
          on(inp, "input", () => {
            const pid = inp.getAttribute("data-pol-premium");
            const pol = (this.newPolicies||[]).find(x => x.id === pid);
            if(!pol) return;
            pol.premiumBefore = inp.value;
            const after = calcAfter(pol.premiumBefore, pol.discountPercent);
            const el = this.els.body.querySelector(`[data-pol-after="${pid}"]`);
        ...
        $$("[data-pol-discount]", this.els.body).forEach(inp => {
          on(inp, "input", () => {
            const pid = inp.getAttribute("data-pol-discount");
            const pol = (this.newPolicies||[]).find(x => x.id === pid);
            if(!pol) return;
            pol.discountPercent = inp.value;
            const after = calcAfter(pol.premiumBefore, pol.discountPercent);
            const el = this.els.body.querySelector(`[data-pol-after="${pid}"]`);
            if(el) el.textContent = after || "";
          });
    ...
    validateDraftPolicy(){
      ...
      // pricing (required for all)
      if(!safeTrim(d.premiumBefore)) return { ok:false, msg:"חובה למלא פרמיה חודשית (לפני הנחה)" };
      const disc = Number(String(d.discountPercent||"0").replace(",",".")); 
      if(Number.isNaN(disc) || disc < 0 || disc > 100) return { ok:false, msg:"אחוז הנחה חייב להיות בין 0 ל-100" };
      ...
      // pledge (risk / mortgage)
      const pledgeApplies = (d.type === "ריסק" || d.type === "ריסק משכנתא");
      if(pledgeApplies && d.pledge){
      ...
      // beneficiaries (risk only — NOT mortgage)
      if(d.type === "ריסק"){
        const mode = safeTrim(d.beneficiaryMode || "legal");
        if(mode === "id"){
          const arr = Array.isArray(d.beneficiaries) ? d.beneficiaries : [];
          if(arr.length < 1) return { ok:false, msg:"בחרת מוטבים לפי ת״ז — חובה להוסיף לפחות מוטב אחד" };
          // validate fields
          const relReq = ["אב","אם","אח","אחות","קרוב משפחה"];
          for(const b of arr){
            if(!safeTrim(b.firstName) || !safeTrim(b.lastName) || !safeTrim(b.idNumber) || !safeTrim(b.birthDate) || !safeTrim(b.relation)){
              return { ok:false, msg:"במוטבים לפי ת״ז חובה למלא את כל השדות" };
            }
            if(relReq.length && !relReq.includes(safeTrim(b.relation))) {
              // allow custom if user typed? for now allow any
            }
          }
          if(arr.length >= 2){
            const sum = arr.reduce((a,b)=> a + (Number(String(b.percent||"0").replace(",","."))||0), 0);
            if(Math.abs(sum - 100) > 0.001) return { ok:false, msg:"בחלקים באחוזים חובה שסך הכל יהיה 100%" };
            if(arr.some(b => (Number(String(b.percent||"0").replace(",","."))||0) <= 0)) return { ok:false, msg:"אחוז לכל מוטב חייב להיות גדול מ-0" };
          }
        }
      }
