...
(() => {
  ...
  const BUILD = "20260409-vUsersGateSupabase";
  ...
  function normalizeUsersGatePinValue(value){
  ...
  function normalizeUsersManagementGate(raw){
    const input = raw && typeof raw === "object" ? raw : {};
    return {
      enabled: input.enabled === false ? false : true,
      pinHash: safeTrim(input.pinHash || input.pin_hash || input.hash),
      updatedAt: safeTrim(input.updatedAt || input.updated_at) || "",
      updatedBy: safeTrim(input.updatedBy || input.updated_by) || ""
    };
  }

  function getUsersManagementGateConfig(){
    State.data.meta = State.data.meta && typeof State.data.meta === "object" ? State.data.meta : {};
    const gate = normalizeUsersManagementGate(State.data.meta.usersManagementGate);
    State.data.meta.usersManagementGate = gate;
    return gate;
  }

  async function getUsersManagementEffectiveHash(){
    const gate = getUsersManagementGateConfig();
    if(gate.pinHash) return gate.pinHash;
    const fallbackPin = safeTrim(State.data?.meta?.adminAuth?.pin) || "1234";
    return sha256Hex(normalizeUsersGatePinValue(fallbackPin));
  }

  async function verifyUsersManagementPin(pin){
    const cleanPin = normalizeUsersGatePinValue(pin);
    if(cleanPin.length < 4) return false;
    const [expected, actual] = await Promise.all([
      getUsersManagementEffectiveHash(),
      sha256Hex(cleanPin)
    ]);
    return !!expected && expected === actual;
  }

  async function setUsersManagementPin(pin, actorName){
    const cleanPin = normalizeUsersGatePinValue(pin);
    if(cleanPin.length < 4 || cleanPin.length > 6){
      return { ok:false, error:"יש להזין קוד מורשה בן 4–6 ספרות" };
    }
    ...
    gate.updatedBy = safeTrim(actorName) || safeTrim(Auth.current?.name) || "מנהל מערכת";
    State.data.meta.usersManagementGate = gate;
    State.data.meta.updatedAt = gate.updatedAt;
    return { ok:true, gate };
  }

  function getUsersManagementGateStatusText(){
    const gate = getUsersManagementGateConfig();
    if(gate.pinHash){
      const updated = gate.updatedAt ? (" · עודכן: " + gate.updatedAt.replace("T", " ").slice(0, 16)) : "";
      const by = gate.updatedBy ? (" · ע\"י: " + gate.updatedBy) : "";
      return { badge:"קוד ייעודי פעיל", help:"קיים קוד מורשה ייעודי לניהול משתמשים" + updated + by };
    }
    return { badge:"קוד מנהל מערכת", help:"טרם הוגדר קוד ייעודי. כרגע קוד מנהל המערכת משמש כקוד מורשה." };
  }
  ...
  const defaultState = () => ({
    meta: {
      ...
      usersManagementGate: normalizeUsersManagementGate({}),
      opsEvents: [],
      chatAvatars: {}
    },
    agents: [
  ...
  function normalizeState(s){
    ...
    out.meta.usersManagementGate = normalizeUsersManagementGate(out.meta.usersManagementGate);
  ...
  const Storage = {
    ...
    buildMetaRow(state){
      return {
        ...
        payload: {
          ...
          usersManagementGate: normalizeUsersManagementGate(state?.meta?.usersManagementGate),
    ...
    mapMeta(metaRow){
      ...
      return {
        ...
        usersManagementGate: normalizeUsersManagementGate(payload?.usersManagementGate),
  ...
  const Auth = {
    ...
    logout(reason = "manual"){
      ...
      try { UsersManagementGateUI.clearAuthorization(); } catch(_e) {}
      this.lock();
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.usersGateCurrentPin = $("#usersGateCurrentPin");
      this.els.usersGateNewPin = $("#usersGateNewPin");
      this.els.usersGateConfirmPin = $("#usersGateConfirmPin");
      this.els.usersGateStatusBadge = $("#usersGateStatusBadge");
      this.els.usersGateSettingsFeedback = $("#usersGateSettingsFeedback");
...
// nav
      $$(".nav__item").forEach(btn => {
        on(btn, "click", () => {
          ...
          if(v === "users" && !Auth.canManageUsers()) return UsersManagementGateUI.requestOpen();
      ...
      on(this.els.btnSaveUsersGatePin, "click", async () => {
        await UsersManagementGateUI.saveSettingsPin();
      });
      [this.els.usersGateCurrentPin, this.els.usersGateNewPin, this.els.usersGateConfirmPin].forEach((field) => {
        on(field, "input", () => {
          if(field) field.value = normalizeUsersGatePinValue(field.value);
          UsersManagementGateUI.clearSettingsFeedback();
        });
      });
...
this.applyRoleUI();
    ...
    goView(view){
      ...
      if(safe === "users" && !UsersManagementGateUI.isAuthorized()) {
        window.setTimeout(() => UsersManagementGateUI.requestOpen(), 0);
        safe = "dashboard";
      }
      ...
      if (safe === "settings") UsersManagementGateUI.renderSettingsCard();
  ...
  const UsersManagementGateUI = {
    cacheMs: 10 * 60 * 1000,
    authorizedUntil: 0,
    pendingOpenAfterAuth: false,
    els: null,

    _ensure(){
      if(this.els) return this.els;
      this.els = {
        wrap: $("#lcUsersGateModal"),
        backdrop: $("#lcUsersGateBackdrop"),
        close: $("#lcUsersGateClose"),
        cancel: $("#lcUsersGateCancel"),
        confirm: $("#lcUsersGateConfirm"),
        pin: $("#lcUsersGatePin"),
        error: $("#lcUsersGateError")
      };
      on(this.els.close, "click", () => this.close());
      on(this.els.cancel, "click", () => this.close());
      on(this.els.backdrop, "click", () => this.close());
      on(this.els.confirm, "click", async () => { await this.confirm(); });
      on(this.els.pin, "input", () => {
        if(this.els.pin) this.els.pin.value = normalizeUsersGatePinValue(this.els.pin.value);
        this.setError("");
      });
    ...
    isAuthorized(){
      return Date.now() < Number(this.authorizedUntil || 0);
    },

    clearAuthorization(){
      this.authorizedUntil = 0;
      this.pendingOpenAfterAuth = false;
    },

    requestOpen(){
      if(!Auth.canManageUsers()) return;
      this.pendingOpenAfterAuth = true;
      this.open();
    },
    ...
    open(){
      ...
      window.setTimeout(() => {
        try { E.pin?.focus({ preventScroll:true }); } catch(_e) {}
      }, 70);
    },
    ...
    setError(msg){
      const E = this._ensure();
      if(E.error) E.error.textContent = msg ? String(msg) : "";
      if(E.pin){
        E.pin.classList.toggle("is-error", !!safeTrim(msg));
      }
    },

    async confirm(){
      const E = this._ensure();
      const pin = normalizeUsersGatePinValue(E.pin?.value);
      if(pin.length < 4 || pin.length > 6){
        this.setError("יש להזין קוד מורשה בן 4–6 ספרות");
        return;
      }
      const ok = await verifyUsersManagementPin(pin);
      if(!ok){
        this.setError("סיסמת המורשה שגויה");
        if(E.pin){
          E.pin.value = "";
          try { E.pin.focus({ preventScroll:true }); } catch(_e) {}
        }
      ...
      this.authorizedUntil = Date.now() + this.cacheMs;
      this.close();
      UI.goView("users");
    },

    clearSettingsFeedback(){
      const box = UI.els.usersGateSettingsFeedback;
      if(box){
        box.textContent = "";
    ...
    setSettingsFeedback(msg, tone="success"){
      const box = UI.els.usersGateSettingsFeedback;
      if(!box) return;
    ...
    renderSettingsCard(){
      const status = getUsersManagementGateStatusText();
      if(UI.els.usersGateStatusBadge) UI.els.usersGateStatusBadge.textContent = status.badge;
      const help = document.getElementById("usersGateCurrentPinHelp");
      if(help) help.textContent = status.help;
    },

    async saveSettingsPin(){
      if(!Auth.isAdmin()) return;
      const currentPin = normalizeUsersGatePinValue(UI.els.usersGateCurrentPin?.value);
      const newPin = normalizeUsersGatePinValue(UI.els.usersGateNewPin?.value);
      const confirmPin = normalizeUsersGatePinValue(UI.els.usersGateConfirmPin?.value);
      this.clearSettingsFeedback();

      if(newPin.length < 4 || newPin.length > 6){
        this.setSettingsFeedback("יש להזין קוד חדש בן 4–6 ספרות", "error");
        return;
      }
      if(confirmPin !== newPin){
        this.setSettingsFeedback("אימות הקוד החדש אינו תואם", "error");
        return;
      }
      const currentOk = await verifyUsersManagementPin(currentPin);
      if(!currentOk){
        this.setSettingsFeedback("הקוד הנוכחי שהוזן אינו נכון", "error");
        return;
      }
      const saved = await setUsersManagementPin(newPin, Auth.current?.name);
      if(!saved.ok){
        this.setSettingsFeedback(saved.error || "לא ניתן לשמור את הקוד המורשה", "error");
        return;
      }
      const persistRes = await App.persist("קוד מורשה לניהול משתמשים נשמר");
      if(!persistRes?.ok){
        this.setSettingsFeedback(persistRes?.error || "אירעה שגיאה בשמירה לשרת", "error");
        return;
      }
      if(UI.els.usersGateCurrentPin) UI.els.usersGateCurrentPin.value = "";
      if(UI.els.usersGateNewPin) UI.els.usersGateNewPin.value = "";
      if(UI.els.usersGateConfirmPin) UI.els.usersGateConfirmPin.value = "";
      this.renderSettingsCard();
      this.setSettingsFeedback("הקוד המורשה נשמר בהצלחה", "success");
    }
  };
