...
(() => {
  ...
  const Storage = {
    ...
    async restRequest(path, opts={}){
      const method = opts.method || "GET";
      const headers = this.restHeaders(opts.headers || {});
      const init = { method, headers };
      if(opts.body !== undefined){
        init.body = JSON.stringify(opts.body);
        if(!init.headers["Content-Type"]) init.headers["Content-Type"] = "application/json";
      }
      const res = await fetch(this.supabaseUrl + path, init);
      const raw = await res.text();
      let data = null;
      try { data = raw ? JSON.parse(raw) : null; } catch(_e) { data = raw; }
      if(!res.ok){
        const msg = data?.message || data?.error_description || data?.hint || data?.details || raw || ("HTTP_" + res.status);
    ...
    async upsertMeta(state){
      ...
      try{
        ...
        await this.restRequest(`/rest/v1/${SUPABASE_TABLES.meta}?on_conflict=key`, {
          method: "POST",
    ...
    async syncTable(tableName, rows){
      ...
      try{
        ...
        await this.restRequest(`/rest/v1/${tableName}?on_conflict=id`, {
          method: "POST",
          headers: {
    ...
    async deleteRows(tableName, ids){
      const list = Array.isArray(ids) ? ids.map(x => safeTrim(x)).filter(Boolean) : [];
      if(!list.length) return;
      const encoded = list.map(encodeURIComponent).join(",");
      try{
        const client = this.getClient();
        const { error } = await client.from(tableName).delete().in("id", list);
        if(error) throw error;
      }catch(err){
        await this.restRequest(`/rest/v1/${tableName}?id=in.(${encoded})`, {
          method: "DELETE",
    ...
    async loadSheets(){
      try {
        let metaData, agentsData, customersData, proposalsData;
        try{
          const client = this.getClient();
          const [metaRes, agentsRes, customersRes, proposalsRes] = await Promise.all([
            ...
            client.from(SUPABASE_TABLES.agents).select("*"),
            client.from(SUPABASE_TABLES.customers).select("*"),
            client.from(SUPABASE_TABLES.proposals).select("*")
          ]);
          const firstErr = metaRes.error || agentsRes.error || customersRes.error || proposalsRes.error;
          ...
          metaData = await this.restRequest(`/rest/v1/${SUPABASE_TABLES.meta}?key=eq.global&select=key,payload,updated_at`).then(r => Array.isArray(r) ? (r[0] || {}) : (r || {}));
          agentsData = await this.restRequest(`/rest/v1/${SUPABASE_TABLES.agents}?select=*`);
          customersData = await this.restRequest(`/rest/v1/${SUPABASE_TABLES.customers}?select=*`);
          proposalsData = await this.restRequest(`/rest/v1/${SUPABASE_TABLES.proposals}?select=*`);
        }

        const payload = normalizeState({
          meta: this.mapMeta(metaData || {}),
          agents: (agentsData || []).map((row, idx) => this.mapAgentRow(row, idx)),
          customers: (customersData || []).map((row, idx) => this.mapCustomerRow(row, idx)),
          proposals: (proposalsData || []).map((row, idx) => this.mapProposalRow(row, idx))
        });
    ...
    async saveSheets(state){
      try {
        ...
        const role = safeTrim(session.role);
        const canManageSystemTables = (role === "admin" || role === "manager");

        if(canManageSystemTables){
...
txt.textContent = "";
  ...
  const ArchiveCustomerUI = {
    ...
    async confirm(){
      ...
      try{
        await Storage.deleteRows(SUPABASE_TABLES.customers, [id]);
      }catch(err){
        console.error("DELETE_CUSTOMER_REMOTE_FAILED", err);
      }
  ...
  const ProposalsUI = {
    ...
    async deleteById(id){
      ...
      if(!ok) return;
      const prev = Array.isArray(State.data?.proposals) ? State.data.proposals.slice() : [];
      State.data.proposals = (State.data.proposals || []).filter(x => String(x.id) !== String(id));
      State.data.meta.updatedAt = nowISO();
      const persistRes = await App.persist("ההצעה נמחקה");
      if(!persistRes?.ok){
        State.data.proposals = prev;
        State.data.meta.updatedAt = nowISO();
        this.render();
        window.alert("מחיקת ההצעה נכשלה ב-Supabase. ההצעה נשארה במערכת.");
        return;
      }
      try{
        await Storage.deleteRows(SUPABASE_TABLES.proposals, [id]);
      }catch(err){
        console.error("DELETE_PROPOSAL_REMOTE_FAILED", err);
      }
      this.render();
    }
  };
...
init(){
      ...
      on(this.els.body, "input", (ev) => {
        const t = ev?.target;
        const tag = (t?.tagName || "").toLowerCase();
        if(tag === "input" || tag === "select" || tag === "textarea") this.scheduleAutoDraftSave();
      });
...
// ---------- Step 6 ----------
    ...
    hasDraftContent(){
      const primary = this.insureds?.[0]?.data || {};
      return !!(
        (this.insureds || []).length > 1 ||
        Object.keys(primary || {}).some(k => safeTrim(primary[k])) ||
        (this.newPolicies || []).length
      );
    },

    scheduleAutoDraftSave(){
      if(!Auth.current || !this.els?.wrap?.classList?.contains("is-open")) return;
      if(this._finishing) return;
      if(!this.hasDraftContent()) return;
      clearTimeout(this._autoDraftTimer);
      this._autoDraftTimer = setTimeout(() => {
        this.saveDraft(true);
      }, 900);
    },
    ...
    async saveDraft(silent=false){
      if(!Auth.current) return;
      if(silent && !this.hasDraftContent()) return;
    ...
    async saveCompletedCustomer(){
      ...
      State.data.proposals = Array.isArray(State.data.proposals) ? State.data.proposals : [];
      const draftIdToDelete = this.editingDraftId || null;
      if(draftIdToDelete){
        State.data.proposals = State.data.proposals.filter(x => String(x.id) !== String(draftIdToDelete));
        this.editingDraftId = null;
      }
      State.data.meta.updatedAt = nowISO();
      const saveRes = await App.persist("הלקוח נשמר");
      if(saveRes?.ok && draftIdToDelete){
        try{
          await Storage.deleteRows(SUPABASE_TABLES.proposals, [draftIdToDelete]);
        }catch(err){
          console.error("DELETE_DRAFT_AFTER_CUSTOMER_SAVE_FAILED", err);
        }
      }
      return record;
    },
...
const SystemRepairUI = {
  ...
  const App = {
    ...
    async persist(label){
      State.data = State.data || defaultState();
      State.data.meta = State.data.meta || {};
      State.data.meta.updatedAt = nowISO();
