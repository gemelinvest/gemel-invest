...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyKdPSh9jetK7sZuqgxOxWZeQlGjTwCJhKeu9JhOQ9wjdFdKynObH4H_OWPQ6CoQBA-/exec';
  const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyZ1HEzQIuA0xUP6JtvuSknONMZ3fWpl4mPLHCZ8UvEtUBwR-jXKFHLl-pllg1QRp4/exec'; // <-- connected (Web App /exec)
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  (
  // Smart login: if GOOGLE_SCRIPT_URL is set, authenticate via server; otherwise use local demo users.
  async function loginAttemptSmart_(username, password){
    const u = String(username||'').trim();
    const p = String(password||'').trim();

    // Fallback to local demo mode
    if(!GOOGLE_SCRIPT_URL){
      return loginAttempt_(u, p);
    }
    ...
    try{
      ...
      if(!r || r.ok !== true){
        ...
        const msg = (r && (r.msg || r.message)) ? String(r.msg || r.message) :
                    (r && r.error) ? ('שגיאת שרת: ' + String(r.error)) :
                    'שגיאת התחברות';
        return { ok:false, msg };
      }
      ...
      setSession_({ username: identity.username, at: Date.now() });
      setIdentity_(identity);
      showApp_();

      // Best-effort initial sync after login
      try{ await syncDown(); }catch(_){}
      render();

      return { ok:true };
    }catch(e){
      return { ok:false, msg:'שגיאת חיבור לשרת' };
    }
  }
function bootAuth_(){
    ...
    if(form){
      form.addEventListener('submit', async (e) => {
        e.preventDefault();
        showErr('');
        const res = await loginAttemptSmart_(userEl?.value, passEl?.value);
        if(!res.ok){
          showErr(res.msg);
