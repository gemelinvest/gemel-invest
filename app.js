...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    renderPolicyCard(policy){
      ...
      const coverageValue = safeTrim(policy.coverageValue) || "—";
      const insuredValue = safeTrim(policy.insuredLabel) || "—";
      const policyNumberValue = safeTrim(policy.policyNumber) || "—";
      const startDateValue = safeTrim(policy.startDate) || "—";
      const discountValue = (Number(policy.discountPct || 0) > 0 || safeTrim(policy.discountYears))
        ? `${escapeHtml(String(policy.discountPct || "0"))}%${safeTrim(policy.discountYears) ? ` · ${escapeHtml(policy.discountYears)} שנים` : ""}`
        : "ללא הנחה";
      const topMeta = [
        { label: "מבוטח", value: insuredValue },
        { label: policy.coverageLabel || "סכום ביטוח", value: coverageValue },
        { label: "מס׳ פוליסה", value: policyNumberValue },
        { label: "תחילה", value: startDateValue }
      ];
      const pricingMeta = [
        { label: "פרמיה", value: safeTrim(policy.premiumText) || "—", accent: true },
        { label: "אחרי הנחה", value: safeTrim(policy.premiumAfterDiscount) || safeTrim(policy.premiumText) || "—" },
        { label: "הנחה", value: discountValue }
      ];
      const coverChips = coverItems.length
        ? `<div class="customerPolicyCard__chips">${coverItems.map(c => `<span class="customerPolicyCard__coverChip">${escapeHtml(c)}</span>`).join("")}</div>`
        : "";
      return `<article class="customerPolicyCard ${this.companyClass(policy.company)} ${policy.origin === "new" ? "is-newCard" : "is-existingCard"}">
        <div class="customerPolicyCard__accent" aria-hidden="true"></div>
        <div class="customerPolicyCard__shine" aria-hidden="true"></div>
        <div class="customerPolicyCard__main">
          <div class="customerPolicyCard__companyCol">
          ...
          <div class="customerPolicyCard__details">
            <div class="customerPolicyCard__metaGrid customerPolicyCard__metaGrid--main">
              ${topMeta.map(item => `<div class="customerPolicyCard__metaCell"><span class="customerPolicyCard__metaLabel">${escapeHtml(item.label)}</span><strong class="customerPolicyCard__metaValue">${escapeHtml(item.value)}</strong></div>`).join("")}
            </div>
            <div class="customerPolicyCard__metaGrid customerPolicyCard__metaGrid--pricing">
              ${pricingMeta.map(item => `<div class="customerPolicyCard__metaCell ${item.accent ? "is-accent" : ""}"><span class="customerPolicyCard__metaLabel">${escapeHtml(item.label)}</span><strong class="customerPolicyCard__metaValue">${escapeHtml(item.value)}</strong></div>`).join("")}
            </div>
            ${coverChips ? `<div class="customerPolicyCard__covers"><div class="customerPolicyCard__coversLabel">כיסויים</div>${coverChips}</div>` : ``}
          </div>
    ...
    renderPolicyWallet(rec, policies){
      const existingPolicies = policies.filter(p => p.origin === "existing");
      const newPolicies = policies.filter(p => p.origin === "new");
      const renderSection = (title, sub, rows, typeClass) => `
        <section class="customerPolicySection ${typeClass}">
          <div class="customerPolicySection__head">
            <div>
              <div class="customerPolicySection__title">${escapeHtml(title)}</div>
              <div class="customerPolicySection__sub">${escapeHtml(sub)}</div>
            </div>
            <div class="customerPolicySection__count">${escapeHtml(String(rows.length || 0))}</div>
          </div>
          <div class="customerPolicyGrid">${rows.length ? rows.map(p => this.renderPolicyCard(p)).join("") : `<div class="emptyState customerPolicySection__empty"><div class="emptyState__icon">🧾</div><div class="emptyState__title">אין פוליסות להצגה</div><div class="emptyState__text">ברגע שיתווספו פוליסות לסוג הזה, הן יופיעו כאן אוטומטית.</div></div>`}</div>
        </section>`;
      const sectionsHtml = policies.length ? [
        renderSection("פוליסות שהלקוח הגיע איתן", "הפוליסות הקיימות בתיק לפני המכירה החדשה", existingPolicies, "is-existingSection"),
        renderSection("פוליסות חדשות", "הפוליסות שנמכרו ונוספו ללקוח במערכת", newPolicies, "is-newSection")
      ].join("") : `<div class="emptyState"><div class="emptyState__icon">🧾</div><div class="emptyState__title">עדיין אין פוליסות בתיק</div><div class="emptyState__text">ברגע שיישמרו פוליסות קיימות או חדשות, הן יוצגו כאן אוטומטית.</div></div>`;
        ...
        <div class="customerWalletSection__head">
          <div class="customerWalletSection__titleWrap">
            ...
            <div>
              ...
              <div class="customerWalletSection__sub">תצוגה רוחבית מסודרת עם הפרדה ברורה בין פוליסות קיימות לפוליסות חדשות</div>
            </div>
          </div>
        </div>
        <div class="customerPolicyLayout">${sectionsHtml}</div>
      </section>`;
    },
