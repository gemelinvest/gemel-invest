...
(() => {
  ...
  const BUILD = "20260319-customer-wizard-actions-modal";
...
init(){
      ...
      this.ensurePolicyActionsModal();
      this.els.policyActionsModal = $("#lcPolicyActionsModal");
      this.els.policyActionsBackdrop = $("#lcPolicyActionsBackdrop");
      this.els.policyActionsClose = $("#lcPolicyActionsClose");
      this.els.policyActionsTitle = $("#lcPolicyActionsTitle");
      this.els.policyActionsMeta = $("#lcPolicyActionsMeta");
      this._policyActionsId = null;
      ...
      on(this.els.policyActionsBackdrop, "click", () => this.closePolicyActionsModal());
      on(this.els.policyActionsClose, "click", () => this.closePolicyActionsModal());
      this.els.policyActionsModal?.querySelectorAll?.("[data-policy-action]").forEach(btn => {
        on(btn, "click", () => this.handlePolicyActionSelect(btn.getAttribute("data-policy-action")));
      });
...
// open health covers drawer (Health only)
    ...
    ensurePolicyActionsModal(){
      if(document.getElementById("lcPolicyActionsModal")) return;
      const host = document.createElement("div");
      host.innerHTML = `
        <div class="modal lcPolicyActionsModal" id="lcPolicyActionsModal" aria-hidden="true">
          <div class="modal__backdrop" id="lcPolicyActionsBackdrop"></div>
          <div class="modal__panel lcPolicyActionsModal__panel" role="dialog" aria-modal="true" aria-labelledby="lcPolicyActionsTitle">
            <div class="modal__head">
              <div>
                <div class="modal__kicker">פוליסה חדשה</div>
                <div class="modal__title" id="lcPolicyActionsTitle">פעולות על הפוליסה</div>
                <div class="lcPolicyActionsModal__meta" id="lcPolicyActionsMeta"></div>
              </div>
              <button class="iconBtn" id="lcPolicyActionsClose" type="button" aria-label="סגור">✕</button>
            </div>
            <div class="modal__body lcPolicyActionsModal__body">
              <button type="button" class="lcPolicyActionsModal__option" data-policy-action="edit">
                <span class="lcPolicyActionsModal__optionIcon">✏️</span>
                <span class="lcPolicyActionsModal__optionText">
                  <strong>עריכה</strong>
                  <small>עדכון פרטי הפוליסה</small>
                </span>
              </button>
              <button type="button" class="lcPolicyActionsModal__option" data-policy-action="discount">
                <span class="lcPolicyActionsModal__optionIcon">🏷️</span>
                <span class="lcPolicyActionsModal__optionText">
                  <strong>הנחה</strong>
                  <small>עדכון אחוז ודירוג הנחה</small>
                </span>
              </button>
              <button type="button" class="lcPolicyActionsModal__option is-danger" data-policy-action="delete">
                <span class="lcPolicyActionsModal__optionIcon">🗑️</span>
                <span class="lcPolicyActionsModal__optionText">
                  <strong>הסר</strong>
                  <small>מחיקת הפוליסה מהרשימה</small>
                </span>
              </button>
            </div>
          </div>
    ...
    openPolicyActionsModal(pid){
      const policy = (this.newPolicies || []).find(item => item.id === pid);
      if(!policy || !this.els.policyActionsModal) return;
      this._policyActionsId = pid;
      if(this.els.policyActionsTitle) this.els.policyActionsTitle.textContent = `${policy.company || "חברה"} · ${policy.type || "פוליסה"}`;
      if(this.els.policyActionsMeta) this.els.policyActionsMeta.textContent = `פרמיה: ${this.formatMoneyValue(this.asMoneyNumber(policy.premiumMonthly))} · תחילה: ${safeTrim(policy.startDate) || "—"}`;
      this.els.policyActionsModal.classList.add("is-open");
      this.els.policyActionsModal.setAttribute("aria-hidden", "false");
      setTimeout(() => this.els.policyActionsModal?.querySelector?.("[data-policy-action='edit']")?.focus?.(), 30);
    },

    closePolicyActionsModal(){
      this._policyActionsId = null;
      if(this.els.policyActionsModal){
        this.els.policyActionsModal.classList.remove("is-open");
        this.els.policyActionsModal.setAttribute("aria-hidden", "true");
      }
    },

    handlePolicyActionSelect(action){
      const pid = this._policyActionsId;
      this.closePolicyActionsModal();
      if(!pid) return;
      if(action === "edit"){
        this.startEditNewPolicy(pid);
        return;
      }
      if(action === "discount"){
    ...
    renderStep5(){
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        return `<div class="lcPolCard lcPolCard--premium lcPolCard--compact" data-pol="${p.id}">
          <div class="lcPolCard__top lcPolCard__top--compact">
            ...
            <div class="lcPolCard__sideTools">
              <div class="lcPolSummaryTag">חדש</div>
              <button type="button" class="lcPolActionsBtn" data-pol-actions="${p.id}" aria-label="פעולות">פעולות</button>
            </div>
          </div>
          <div class="lcPolInfoStrip lcPolInfoStrip--compact">${chips}</div>
          ${p.type === "בריאות" ? `<div class="lcPolCoverCompact lcPolCoverCompact--tight">
      ...
      setTimeout(() => {
        ...
        $$('[data-pol-actions]', this.els.body).forEach(btn => {
          on(btn, 'click', () => {
            const pid = btn.getAttribute('data-pol-actions');
            if(pid) this.openPolicyActionsModal(pid);
          });
        });
