...
(() => {
  ...
  const BUILD = "20260405-dashboard-agent-kpis-zero-state-v40";
...
this.applyRoleUI();
  ...
  const DashboardUI = {
    ...
    formatMonthLabel(){
      const now = new Date();
      try {
        return new Intl.DateTimeFormat('he-IL', { month:'long', year:'numeric' }).format(now);
      } catch(_e) {
    ...
    renderEmpty(){
      ...
      const agentName = this.getCurrentAgentName() || 'נציג';
      const cards = [
        {
          kicker: 'החודש',
          label: 'כמות הצעות שהוגשו',
          value: '0',
          sub: 'עדיין לא נשמרו תיקים לחודש הנוכחי',
          icon: premiumCustomerIcon('document')
        },
        {
          kicker: 'פוליסות חדשות בלבד',
          label: 'סה״כ פרמיה חודשית אחרי הנחות',
          value: this.formatMoney(0),
          sub: 'הסכום יתעדכן אוטומטית עם שמירת פוליסות חדשות',
          icon: premiumCustomerIcon('activity')
        },
        {
          kicker: 'פוליסות חדשות בלבד',
          label: 'מכירות',
          value: '0',
          sub: 'פוליסות חדשות שייסגרו החודש יופיעו כאן',
          icon: premiumCustomerIcon('briefcase')
        },
        {
          ...
          value: this.formatMoney(0),
          sub: 'מינויי סוכן מפוליסות קיימות יצטברו כאן',
          icon: premiumCustomerIcon('building')
        }
      ];
      this.els.root.innerHTML = `
        <div class="card dashboardHeroCard">
          <div class="dashboardHeroCard__body">
            <div>
              <div class="dashboardHeroCard__eyebrow">דשבורד נציג · ${escapeHtml(monthLabel)}</div>
              <div class="dashboardHeroCard__title">ביצועי ${escapeHtml(agentName)}</div>
              <div class="dashboardHeroCard__sub">כרגע עדיין אין פעילות לחודש הנוכחי, אבל המדדים כבר מוכנים ויתעדכנו אוטומטית עם כל תיק ופוליסה חדשה.</div>
            </div>
            <div class="dashboardHeroCard__badge">מוכן לקליטת נתונים</div>
          </div>
        </div>
        <div class="grid grid--kpis dashboardKpiGrid dashboardKpiGrid--empty">
          ${cards.map((card, idx) => `
            <article class="card dashboardKpiCard" style="animation-delay:${idx * 70}ms">
              <div class="dashboardKpiCard__icon">${card.icon}</div>
              <div class="dashboardKpiCard__kicker">${escapeHtml(card.kicker)}</div>
              <div class="dashboardKpiCard__value">${escapeHtml(card.value)}</div>
              <div class="dashboardKpiCard__label">${escapeHtml(card.label)}</div>
              <div class="dashboardKpiCard__sub">${escapeHtml(card.sub)}</div>
            </article>`).join('')}
        </div>
        <div class="emptyState dashboardEmptyStateNote">
          <div class="emptyState__icon">📊</div>
          <div class="emptyState__title">עדיין אין נתונים לחודש הנוכחי</div>
          <div class="emptyState__text">הכרטיסים מוצגים כבר עכשיו עם ערכי 0 כדי שתוכל לראות איך הדשבורד ייראה. ברגע שתישמר פעילות חדשה החודש, המספרים יתעדכנו כאן אוטומטית.</div>
        </div>`;
    },
      ...
      const cards = [
        {
          ...
          sub: `${metrics.customersCount || 0} תיקים פעילים החודש`,
          icon: premiumCustomerIcon('document')
        },
        {
          kicker: 'פוליסות חדשות בלבד',
          ...
          sub: 'מחושב רק לפי הפרמיה הסופית אחרי הנחה',
          icon: premiumCustomerIcon('activity')
        },
        {
          kicker: 'פוליסות חדשות בלבד',
          ...
          sub: 'נספרות רק פוליסות חדשות שנמכרו',
          icon: premiumCustomerIcon('briefcase')
        },
        {
          kicker: 'פוליסות קיימות',
          ...
          sub: 'רק פוליסות קיימות שסומנו כמינוי סוכן',
          icon: premiumCustomerIcon('building')
        }
      ];
        ...
        <div class="card dashboardHeroCard">
          <div class="dashboardHeroCard__body">
            <div>
              <div class="dashboardHeroCard__eyebrow">דשבורד נציג · ${escapeHtml(monthLabel)}</div>
              <div class="dashboardHeroCard__title">ביצועי ${escapeHtml(agentName)}</div>
              <div class="dashboardHeroCard__sub">המערכת מרכזת לך את ההצעות, המכירות, הפרמיות ומינויי הסוכן של החודש הנוכחי בלבד.</div>
            </div>
            <div class="dashboardHeroCard__badge">מתאפס אוטומטית ב־1 לכל חודש</div>
          </div>
        </div>
        <div class="grid grid--kpis dashboardKpiGrid">
          ${cards.map((card, idx) => `
            <article class="card dashboardKpiCard" style="animation-delay:${idx * 70}ms">
              <div class="dashboardKpiCard__icon">${card.icon}</div>
              <div class="dashboardKpiCard__kicker">${escapeHtml(card.kicker)}</div>
