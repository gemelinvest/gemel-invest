  ...
  const ChatUI = {
    ...
    init(){
      this.els = {
        ...
        emojiPicker: $("#giChatEmojiPicker"),
        emojiGrid: $("#giChatEmojiGrid"),
        toasts: $("#giChatToasts")
      };
      ...
      on(document, 'click', (ev) => this.handleDocumentClick(ev));
    ...
    buildEmojiPicker(){
      ...
      if(!grid) return;
      grid.innerHTML = this.emojiList.map((emoji) => `
        <button class="giChatEmojiPicker__btn" type="button" data-chat-emoji="${emoji}" aria-label="הוסף ${emoji}">${emoji}</button>
      `).join('');
      $$('[data-chat-emoji]', grid).forEach((btn) => on(btn, 'click', () => this.insertEmoji(btn.dataset.chatEmoji || '')));
    },

    toggleEmojiPicker(force){
      const picker = this.els?.emojiPicker;
      const toggle = this.els?.emojiToggle;
      if(!picker || !toggle) return;
      const shouldOpen = typeof force === 'boolean' ? force : picker.classList.contains('is-hidden');
      picker.classList.toggle('is-hidden', !shouldOpen);
      picker.setAttribute('aria-hidden', shouldOpen ? 'false' : 'true');
    ...
    handleDocumentClick(ev){
      ...
      if(!picker || !toggle) return;
      const target = ev?.target;
      if((picker && picker.contains(target)) || (toggle && toggle.contains(target))) return;
      this.toggleEmojiPicker(false);
    },
