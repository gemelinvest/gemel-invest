...
(() => {
  ...
  const BUILD = "20260326-ceo-dashboard-exclusive-v2";
  ...
  const CEO_AGENT_ID = "ceo_locked";
  const CEO_DEFAULT_USER = {
    id: CEO_AGENT_ID,
    name: 'איתי סומך',
    username: 'איתי סומך',
    pin: '2026',
  ...
  const isLockedCeoUser = (user) => {
    const id = safeTrim(user?.id);
    const name = safeTrim(user?.name);
    const username = safeTrim(user?.username || user?.user);
    return id === CEO_AGENT_ID || name === CEO_DEFAULT_USER.name || username === CEO_DEFAULT_USER.username;
  };
  const hasExclusiveCeoIdentity = (user) => {
    if(!user || typeof user !== 'object') return false;
    const id = safeTrim(user?.id);
    const role = safeTrim(user?.role || user?.type);
    const name = safeTrim(user?.name);
    const username = safeTrim(user?.username || user?.user);
    const byIdentity = (name === CEO_DEFAULT_USER.name) || (username === CEO_DEFAULT_USER.username);
    return (id === CEO_AGENT_ID || byIdentity) && role === 'ceo';
  };
  const ensureLockedCeoUser = (list) => {
    const arr = Array.isArray(list) ? list.slice() : [];
    const idx = arr.findIndex((item) => isLockedCeoUser(item));
    const base = idx >= 0 && arr[idx] && typeof arr[idx] === 'object' ? arr[idx] : {};
    const merged = {
      ...CEO_DEFAULT_USER,
      ...base,
      id: CEO_AGENT_ID,
      role: 'ceo',
      active: true,
      name: safeTrim(base.name) || CEO_DEFAULT_USER.name,
      username: safeTrim(base.username || base.user) || safeTrim(base.name) || CEO_DEFAULT_USER.username,
      pin: safeTrim(base.pin || base.pass) || CEO_DEFAULT_USER.pin
    };
  ...
  function normalizeState(s){
    ...
    out.agents = ensureLockedCeoUser((out.agents || []).map((a, idx) => {
      const isCeo = isLockedCeoUser(a);
      const fallbackName = isCeo ? CEO_DEFAULT_USER.name : "נציג";
      const name = safeTrim(a?.name) || fallbackName;
      const username = safeTrim(a?.username) || safeTrim(a?.user) || name;
      const pin = safeTrim(a?.pin) || safeTrim(a?.pass) || (isCeo ? CEO_DEFAULT_USER.pin : "0000");
      const roleRaw = safeTrim(a?.role) || safeTrim(a?.type) || "";
      const active = isCeo ? true : ((a?.active === false) ? false : true);
      const role = isCeo
        ? 'ceo'
        : (roleRaw === "manager" || roleRaw === "adminLite" || roleRaw === "admin") ? "manager" : (roleRaw === "ops" || roleRaw === "operations" || roleRaw === "תפעול") ? "ops" : "agent";
      return {
        id: isCeo ? CEO_AGENT_ID : (safeTrim(a?.id) || ("a_" + idx)),
        name, username, pin, role, active
      };
    }).filter(a => a.name));

    if (!out.agents.length) out.agents = ensureLockedCeoUser(base.agents);
  ...
  const Storage = {
    ...
    session(){
      try{
        ...
        return { id: safeTrim(s?.id), username: safeTrim(s?.username) || name, name, role };
      }catch(_e){
        return { name:"", role:"" };
      }
    },
    ...
    buildAgentRows(state){
      return ensureLockedCeoUser(state?.agents || []).map((a, idx) => {
        const isCeo = isLockedCeoUser(a);
        return {
          id: isCeo ? CEO_AGENT_ID : (safeTrim(a?.id) || ("a_" + idx)),
          name: safeTrim(a?.name) || (isCeo ? CEO_DEFAULT_USER.name : "נציג"),
          username: safeTrim(a?.username) || safeTrim(a?.name) || (isCeo ? CEO_DEFAULT_USER.username : "נציג"),
          pin: safeTrim(a?.pin) || (isCeo ? CEO_DEFAULT_USER.pin : "0000"),
          role: isCeo ? 'ceo' : (safeTrim(a?.role) || "agent"),
          active: isCeo ? true : (a?.active === false ? false : true),
  ...
  const Auth = {
    ...
    isCeo(){
      return !!(this.current && this.current.role === 'ceo');
    },

    hasExclusiveCeoAccess(){
      return hasExclusiveCeoIdentity(this.current);
    },
    ...
    canViewAllCustomers(){
      return this.isAdmin() || this.hasExclusiveCeoAccess() || this.isManager() || this.isOps();
    },

    canManageUsers(){
      return this.isAdmin() || this.hasExclusiveCeoAccess() || this.isManager();
    },
    ...
    _saveSession(cur){
      try {
        localStorage.setItem(LS_SESSION_KEY, JSON.stringify({ id: safeTrim(cur?.id), username: safeTrim(cur?.username) || safeTrim(cur?.name), name: cur.name, role: cur.role }));
      } catch(_) {}
    },
    ...
    async _submit(){
      ...
      if (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin)) {
        this.current = { name: safeTrim(adminAuth.username) || defAdmin.username, username: safeTrim(adminAuth.username) || defAdmin.username, role:"admin" };
        try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      ...
      this.current = { name: matched.name, role: (matched.role === 'ceo' ? 'ceo' : matched.role === "manager" ? "manager" : matched.role === "ops" ? "ops" : "agent") };
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      await App.reloadSessionState();
      this.unlock();
      try { InactivityGuard.start(); } catch(_e) {}
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.dashboard = $("#view-dashboard");
...
// nav
      $$(".nav__item").forEach(btn => {
        on(btn, "click", () => {
          ...
          if(!v) return;
          if(v === "settings" && !(Auth.isAdmin() || Auth.hasExclusiveCeoAccess())) return;
...
this.applyRoleUI();
    ...
    applyRoleUI(){
      ...
      const isCeo = Auth.hasExclusiveCeoAccess();
      ...
      if (settingsBtn) settingsBtn.style.display = (isAdmin || isCeo) ? "" : "none";
    ...
    goView(view){
      ...
      if(safe === "settings" && !(Auth.isAdmin() || Auth.hasExclusiveCeoAccess())) safe = "dashboard";
      ...
      document.body.classList.toggle('ceo-dashboard-mode', safe === 'dashboard' && Auth.hasExclusiveCeoAccess());
    ...
    renderAuthPill(){
      ...
      if(Auth.current) {
        pill.style.display = "";
txt.textContent = Auth.current.name + (Auth.isAdmin() ? ' (מנהל מערכת)' : Auth.hasExclusiveCeoAccess() ? ' (מנכ"ל)' : Auth.isManager() ? ' (מנהל)' : Auth.isOps() ? ' (תפעול)' : '');
      } else {
...
txt.textContent = "";
  ...
  const UsersUI = {
    ...
    openModal(mode, user){
      ...
      const isCeoUser = !!(user && isLockedCeoUser(user));
      if(E.title) E.title.textContent = isCeoUser ? 'עריכת משתמש מנכ"ל' : ((this._modalMode === "edit") ? 'עריכת משתמש' : 'הוסף נציג/סוכן');
      ...
      if(E.role) { E.role.value = user ? (isLockedCeoUser(user) ? "agent" : (user.role || "agent")) : "agent"; E.role.disabled = !!isCeoUser; }
      if(E.active) { E.active.checked = user ? (isLockedCeoUser(user) ? true : (user.active !== false)) : true; E.active.disabled = !!isCeoUser; }
    ...
    async _saveFromModal(){
      ...
      if(isEdit){
        ...
        const isCeo = isLockedCeoUser(a);
        if(isCeo && !Auth.hasExclusiveCeoAccess()){
          this._showErr(E.err, 'רק המנכ"ל יכול לערוך משתמש זה');
          return;
        }
        a.name = name;
        a.username = username;
        a.pin = pin;
        a.role = isCeo ? 'ceo' : (role === "manager" ? "manager" : role === "ops" ? "ops" : "agent");
        a.active = isCeo ? true : active;
        a.id = isCeo ? CEO_AGENT_ID : a.id;
        State.data.agents = ensureLockedCeoUser(State.data.agents);
        State.data.meta.updatedAt = nowISO();
        await App.persist(isCeo ? 'עודכנו פרטי מנכ"ל' : 'עודכן משתמש');
      } else {
    ...
    _filtered(){
      ...
      let arr = ensureLockedCeoUser(Array.isArray(State.data?.agents) ? State.data.agents.slice() : []);
      if(!Auth.hasExclusiveCeoAccess()) arr = arr.filter(a => !isLockedCeoUser(a));
    ...
    render(){
      ...
      UI.els.usersTbody.innerHTML = rows.map(a => {
        const isCeo = isLockedCeoUser(a);
        const status = isCeo ? "מוגן" : ((a.active === false) ? "מושבת" : "פעיל");
        const role = isCeo ? 'מנכ"ל' : (a.role === "manager") ? "מנהל" : (a.role === "ops") ? "תפעול" : "נציג";
        const actions = isCeo
          ? (Auth.hasExclusiveCeoAccess() ? `<button class="btn" data-act="edit" data-id="${escapeHtml(a.id)}">ערוך</button>` : '')
        ...
        return `
          <tr>
            <td>${escapeHtml(a.name)}${isCeo ? ' <span class="badge">חשבון מוגן</span>' : ''}</td>
            <td>${role}</td>
    ...
    async editUser(id){
      const a = (State.data.agents || []).find(x => String(x.id) === String(id));
      if(!a) return;
      if(isLockedCeoUser(a) && !Auth.hasExclusiveCeoAccess()) return;
      this.openModal("edit", a);
    },

    async toggleUser(id){
      const a = (State.data.agents || []).find(x => String(x.id) === String(id));
      if(!a || isLockedCeoUser(a)) return;
...
// open health covers drawer (Health only)
    ...
    async handleHarBituachFile(ins, file){
      ...
      try {
        ...
        const summary = policies.slice(0, 6).map((p) => `• ${p.company} · ${p.policyNumber} · ${p.type}`).join("\n");
        const approved = window.confirm(`זוהו ${policies.length} פוליסות רלוונטיות לייבוא.\n\n${summary}${policies.length > 6 ? "\n• ..." : ""}\n\nהקובץ לא יישמר במערכת. לייבא עכשיו לפוליסות קיימות?`);
        if(!approved){
          this.setHarImportState(ins, {
            status: "cancelled",
        ...
        this.setHarImportState(ins, {
          ...
          fileName: file.name || "",
          importedAt: nowISO(),
          message: `יובאו ${policies.length} פוליסות מתוך הקובץ.`
        });
        this.render();
      } catch(err){
        this.setHarImportState(ins, {
...
// ---------- Step 6 ----------
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const customerRows = [
        ...
        ['כתובת', [safeTrim(primary.street), safeTrim(primary.houseNo || primary.houseNumber), safeTrim(primary.apartmentNo), safeTrim(primary.zip)].filter(Boolean).join(' ')],
        ['קופת חולים', safeTrim(primary.clinic || primary.hmo || primary.kupatHolim)],
        ['שב״ן', safeTrim(primary.shaban || primary.shabanLevel)],
      ...
      const renderHealthSections = () => {
        if(!healthSections.length) return `<div class="lcPdfEmpty">לא נמצאו תשובות להצגת הצהרת הבריאות.</div>`;
      ...
      const style = `<style>
        ...
        .lcPdfHealthTable th,.lcPdfHealthTable td{border:1px solid #dbe3ed;padding:8px 9px;vertical-align:top;text-align:right}
      ...
      const pageOneSections = [
        `<section class="lcPdfSection"><div class="lcPdfSection__title">פרטי לקוח</div>${renderInfoTable(customerRows, 'לא הוזנו פרטי לקוח')}</section>`,
      ...
      return {
        ...
        html: `${style}<div class="${rootClass}">
          ...
          <div class="lcPdfPage">
            ...
            <div class="lcPdfPageFooter"><div class="lcPdfPageFooter__brand">GEMEL INVEST</div><div>עמוד 3</div></div>
          </div>
        </div>`
      };
    },
...
const SystemRepairUI = {
  ...
  const ChatUI = {
    ...
    roleLabel(raw){
      return ({ admin:'מנהל מערכת', ceo:'מנכ"ל', manager:'מנהל', ops:'נציג תפעול', agent:'נציג' })[raw] || 'נציג';
    },
  ...
  const DashboardUI = {
    ...
    ensureRoot(){
      this.root = this.root || UI.els.dashboard || document.getElementById('view-dashboard');
      return this.root;
    },
    ...
    formatPercent(value){
      const n = Number(value || 0);
      const sign = n > 0 ? '+' : '';
      return `${sign}${n.toLocaleString('he-IL', { maximumFractionDigits: 1 })}%`;
    },
    ...
    shortDate(value){
      ...
      if(!Number.isFinite(ms)) return '—';
      try {
        return new Intl.DateTimeFormat('he-IL', { day:'2-digit', month:'2-digit', year:'numeric' }).format(new Date(ms));
      } catch(_e){
    ...
    shortTime(value){
      ...
      if(Number.isFinite(ms)){
        try {
          return new Intl.DateTimeFormat('he-IL', { hour:'2-digit', minute:'2-digit' }).format(new Date(ms));
        } catch(_e){}
      }
    ...
    discountPct(policy){
      const candidates = [policy?.discountPercent, policy?.discountPct, policy?.discount, policy?.discountRate, policy?.policyDiscountPercent];
    ...
    premiumAfterDiscount(policy){
      const base = this.moneyNumber(policy?.premiumMonthly ?? policy?.monthlyPremium ?? policy?.premium ?? policy?.premiumBefore);
      const pct = this.discountPct(policy);
    ...
    getPolicyType(policy){
      return safeTrim(policy?.type || policy?.product || policy?.productType || 'פוליסה');
    },

    getPolicyCoverage(policy){
      const covers = Array.isArray(policy?.healthCovers) ? policy.healthCovers.filter(Boolean) : [];
      return safeTrim(policy?.sumInsured || policy?.compensation || policy?.coverage || (covers.length ? covers.join(' · ') : ''));
    },
    ...
    collectSales(){
      ...
      return customers
        .map((rec) => {
          ...
          const totalPremium = policies.reduce((sum, policy) => sum + this.premiumAfterDiscount(policy), 0);
          const totalCoverage = policies.reduce((sum, policy) => sum + this.moneyNumber(this.getPolicyCoverage(policy)), 0);
          const productNames = Array.from(new Set(policies.map((policy) => this.getPolicyType(policy)).filter(Boolean)));
          return {
            rec,
            policies,
            totalPremium,
            totalCoverage,
            productNames,
            displayAmount: totalPremium,
            effectiveDate: safeTrim(rec.updatedAt || rec.createdAt),
            statusText: this.getSaleStatus(rec),
            statusTone: this.getSaleTone(rec)
          };
        })
        .filter(Boolean)
    ...
    getSaleStatus(rec){
      const text = safeTrim(rec?.status);
      if(/חתימ|סיים|בוצע|הושלם|אושר/.test(text)) return 'נסגרה';
      if(/ממתין|בטיפול|שיקוף|חדש/.test(text)) return 'בתהליך';
      return 'נסגרה';
    },

    getSaleTone(rec){
      return this.getSaleStatus(rec) === 'נסגרה' ? 'success' : 'progress';
    },

    compareVsPreviousMonth(sales, field){
      const { startPrev, startCurrent } = this.previousMonthRange();
      let current = 0;
      let previous = 0;
      sales.forEach((item) => {
        const stamp = new Date(item.effectiveDate || 0);
        if(!(stamp instanceof Date) || Number.isNaN(stamp.getTime())) return;
        const value = Number(item[field] || 0) || 0;
        if(stamp >= startCurrent) current += value;
        else if(stamp >= startPrev && stamp < startCurrent) previous += value;
      });
    ...
    collectTimeline(){
      const items = [];
      const customers = Array.isArray(State.data?.customers) ? State.data.customers.slice() : [];
      const proposals = Array.isArray(State.data?.proposals) ? State.data.proposals.slice() : [];
      const opsEvents = Array.isArray(State.data?.meta?.opsEvents) ? State.data.meta.opsEvents.slice() : [];

      customers.forEach((rec) => {
        const stamp = safeTrim(rec.updatedAt || rec.createdAt);
        const created = safeTrim(rec.createdAt);
        const newPolicies = this.getNewPolicies(rec);
        const mirrorFlow = rec?.payload?.mirrorFlow && typeof rec.payload.mirrorFlow === 'object' ? rec.payload.mirrorFlow : {};
        const call = mirrorFlow.callSession && typeof mirrorFlow.callSession === 'object' ? mirrorFlow.callSession : {};
        let kind = 'customer';
        let title = `נוסף לקוח חדש: ${safeTrim(rec.fullName) || 'לקוח'}`;
        let text = safeTrim(rec.agentName) ? `הלקוח נוצר ע"י ${safeTrim(rec.agentName)}` : 'נפתח תיק חדש במערכת';
        if(call?.active){
          kind = 'call';
          title = `שיחת שיקוף פעילה עם ${safeTrim(rec.fullName) || 'לקוח'}`;
          text = safeTrim(call?.startedBy) ? `${safeTrim(call.startedBy)} החל שיחה פעילה` : 'מחלקת התפעול מטפלת בלקוח כעת';
        } else if(newPolicies.length){
          kind = 'sale';
          title = `נשמרו פוליסות חדשות עבור ${safeTrim(rec.fullName) || 'לקוח'}`;
          text = `${newPolicies.length} פוליסות חדשות · ${newPolicies.map((policy) => this.getPolicyType(policy)).filter(Boolean).slice(0,2).join(' · ') || 'מוצרי ביטוח'}`;
        } else if(created && stamp && created !== stamp){
          kind = 'update';
          title = `עודכנו נתוני לקוח: ${safeTrim(rec.fullName) || 'לקוח'}`;
          text = safeTrim(rec.status) ? `סטטוס נוכחי: ${safeTrim(rec.status)}` : 'בוצע עדכון בפרטי התיק';
        }
        items.push({ stamp, kind, title, text });
      });

      proposals.forEach((rec) => {
        const stamp = safeTrim(rec.updatedAt || rec.createdAt);
        const step = Number(rec.currentStep || rec?.payload?.currentStep || 1) || 1;
        items.push({
          stamp,
          kind: 'proposal',
          title: `נפתחה הצעה חדשה: ${safeTrim(rec.fullName) || 'הצעה'}`,
          text: `ההצעה נמצאת בשלב ${step}${safeTrim(rec.agentName) ? ` · נציג: ${safeTrim(rec.agentName)}` : ''}`
        });
      });

      opsEvents.forEach((ev) => {
        const stamp = safeTrim(ev.updatedAt || ev.createdAt || ev.scheduledAt);
        items.push({
          stamp,
          kind: 'ops',
          title: `נקבע אירוע שיקוף עבור ${safeTrim(ev.customerName) || 'לקוח'}`,
          text: `${safeTrim(ev.title) || 'שיחת שיקוף'}${safeTrim(ev.rangeStart) ? ` · ${safeTrim(ev.rangeStart)}` : ''}`
        });
      });

      return items
        .filter((item) => safeTrim(item.stamp))
        .sort((a, b) => new Date(b.stamp || 0) - new Date(a.stamp || 0))
        .slice(0, 5);
    },

    iconFor(kind){
      const map = {
        customer: { icon:'👤', tone:'is-green' },
        proposal: { icon:'📄', tone:'is-blue' },
        sale: { icon:'⭐', tone:'is-gold' },
        call: { icon:'📞', tone:'is-orange' },
        update: { icon:'✎', tone:'is-purple' },
        ops: { icon:'🗓', tone:'is-navy' }
      };
      return map[kind] || { icon:'•', tone:'is-blue' };
    },

    summary(){
      const sales = this.collectSales();
      const totalMonthlySales = sales.reduce((sum, item) => sum + (Number(item.totalPremium || 0) || 0), 0);
      const dealsClosed = sales.length;
      const avgDeal = dealsClosed ? (totalMonthlySales / dealsClosed) : 0;
      const growthSales = this.compareVsPreviousMonth(sales, 'totalPremium');
      const growthDeals = this.compareVsPreviousMonth(sales, 'displayAmount' in (sales[0] || {}) ? 'displayAmount' : 'totalPremium');
      const previousDeals = (() => {
        const { startPrev, startCurrent } = this.previousMonthRange();
        const current = sales.filter((item) => new Date(item.effectiveDate || 0) >= startCurrent).length;
        const previous = sales.filter((item) => {
          const stamp = new Date(item.effectiveDate || 0);
          return stamp >= startPrev && stamp < startCurrent;
        }).length;
        if(previous <= 0) return current > 0 ? 100 : 0;
        return ((current - previous) / previous) * 100;
      })();
      const avgGrowth = this.compareVsPreviousMonth(sales.map((item) => ({ ...item, avgField: item.totalPremium })), 'avgField');
      return {
        sales,
        totalMonthlySales,
        dealsClosed,
        avgDeal,
        growthSales,
        growthDeals: previousDeals,
        avgGrowth,
        timeline: this.collectTimeline()
      };
    },
    ...
    render(){
      ...
      if(!root) return;
      if(!Auth.current){
        root.innerHTML = `<div class="ceoDashboard ceoDashboard--empty"><div class="ceoDashboardEmpty card"><div class="emptyState"><div class="emptyState__icon">🔐</div><div class="emptyState__title">התחבר כדי לצפות בדשבורד</div><div class="emptyState__text">הדשבורד של המערכת נטען לפי המשתמש המחובר כרגע.</div></div></div></div>`;
        return;
      }

      const data = this.summary();
      const topSales = data.sales.slice(0, 5);
      const isCeo = Auth.hasExclusiveCeoAccess();
      const managerName = safeTrim(Auth.current?.name) || 'משתמש';
      const roleText = Auth.hasExclusiveCeoAccess() ? 'מנהל מערכת (מנכ"ל)' : Auth.isAdmin() ? 'מנהל מערכת' : Auth.isManager() ? 'מנהל' : Auth.isOps() ? 'תפעול' : 'נציג';
      root.innerHTML = `
        <div class="ceoDashboard${isCeo ? ' is-ceo' : ''}">
          ${isCeo ? `
            <div class="ceoHero">
              <div class="ceoHero__actions">
                <button class="ceoHeroPill ceoHeroPill--user" type="button" data-ceo-open="users">
                  <span class="ceoHeroPill__meta">התחברות</span>
                  <span class="ceoHeroPill__text">${escapeHtml(managerName)} · ${escapeHtml(roleText)}</span>
                  <span class="ceoHeroPill__status"></span>
                </button>
                <button class="ceoHeroPill ceoHeroPill--assistant" type="button" data-ceo-action="assistant">
                  <span class="ceoHeroPill__meta">INVESTO</span>
                  <span class="ceoHeroPill__text">העוזר החכם</span>
                  <span class="ceoHeroPill__plus">✦</span>
                </button>
                <button class="ceoHeroPill ceoHeroPill--new" type="button" data-ceo-action="new-customer">
                  <span class="ceoHeroPill__text">הקמת לקוח חדש</span>
                  <span class="ceoHeroPill__plus">＋</span>
                </button>
              </div>
              <div class="ceoHero__titleWrap">
                <div class="ceoHero__brand">GEMEL INVEST</div>
                <div class="ceoHero__title">דשבורד</div>
              </div>
            </div>` : ''}
          <div class="ceoDashboard__shell">
            <section class="ceoPanel ceoPanel--activity">
              <div class="ceoPanel__head">
                <div>
                  <div class="ceoPanel__title">לוג פעילות סוכנות</div>
                  <div class="ceoPanel__hint">מעקב אחר פעילויות אחרונות וצפיות במערכת</div>
                </div>
                <div class="ceoPanel__icon ceoPanel__icon--activity">⌁</div>
              </div>
              <div class="ceoTimeline">
                ${data.timeline.length ? data.timeline.map((item, idx) => {
                  const meta = this.iconFor(item.kind);
                  return `
                    <article class="ceoTimeline__item">
                      <div class="ceoTimeline__time">${escapeHtml(this.shortTime(item.stamp))}</div>
                      <div class="ceoTimeline__line${idx === data.timeline.length - 1 ? ' is-last' : ''}">
                        <span class="ceoTimeline__dot ${meta.tone}">${meta.icon}</span>
                      </div>
                      <div class="ceoTimeline__content">
                        <div class="ceoTimeline__title">${escapeHtml(item.title)}</div>
                        <div class="ceoTimeline__text">${escapeHtml(item.text)}</div>
                      </div>
                    </article>`;
                }).join('') : `<div class="ceoTimeline__empty">עדיין אין פעילות מערכת להצגה.</div>`}
              </div>
              <div class="ceoPanel__footer">
                <button class="ceoPanel__footerBtn" type="button" data-ceo-open="customers">צפייה בכל הפעילות</button>
              </div>
            </section>

            <section class="ceoPanel ceoPanel--sales">
              <div class="ceoPanel__head">
                <div>
                  <div class="ceoPanel__title">לוח מכירות סוכנות</div>
                  <div class="ceoPanel__hint">סקירת ביצועי לקוחות ופוליסות חדשות מהמערכת</div>
                </div>
                <div class="ceoPanel__icon ceoPanel__icon--sales">₪</div>
              </div>

              <div class="ceoKpis">
                <article class="ceoKpi ceoKpi--green">
                  <div class="ceoKpi__icon">💸</div>
                  <div class="ceoKpi__value">${escapeHtml(this.formatMoney(data.totalMonthlySales))}</div>
                  <div class="ceoKpi__label">סך מכירות החודש</div>
                  <div class="ceoKpi__meta">${escapeHtml(this.formatPercent(data.growthSales))} מהחודש הקודם</div>
                </article>
                <article class="ceoKpi ceoKpi--blue">
                  <div class="ceoKpi__icon">💼</div>
                  <div class="ceoKpi__value">${escapeHtml(String(data.dealsClosed))}</div>
                  <div class="ceoKpi__label">עסקאות נסגרו</div>
                  <div class="ceoKpi__meta">${escapeHtml(this.formatPercent(data.growthDeals))} מהחודש הקודם</div>
                </article>
                <article class="ceoKpi ceoKpi--gold">
                  <div class="ceoKpi__icon">⭐</div>
                  <div class="ceoKpi__value">${escapeHtml(this.formatMoney(data.avgDeal))}</div>
                  <div class="ceoKpi__label">ממוצע לעסקה</div>
                  <div class="ceoKpi__meta">${escapeHtml(this.formatPercent(data.avgGrowth))} מהחודש הקודם</div>
                </article>
              </div>

              <div class="ceoSalesTableWrap">
                <table class="ceoSalesTable">
                  <thead>
                    <tr>
                      <th>תאריך</th>
                      <th>לקוח</th>
                  ...
                  <tbody>
                    ${topSales.length ? topSales.map((item) => `
                      <tr>
                        <td>${escapeHtml(this.shortDate(item.effectiveDate))}</td>
                        <td>${escapeHtml(safeTrim(item.rec?.fullName) || 'לקוח')}</td>
                        <td>${escapeHtml(item.productNames.slice(0, 2).join(' · ') || 'פוליסות חדשות')}</td>
                        <td>${escapeHtml(this.formatMoney(item.totalPremium))}</td>
                        <td><span class="ceoStatus ${item.statusTone === 'success' ? 'is-success' : 'is-progress'}">${escapeHtml(item.statusText)}</span></td>
                      </tr>
                    `).join('') : `<tr><td colspan="5"><div class="ceoSalesTable__empty">עדיין אין עסקאות חדשות להצגה בדשבורד.</div></td></tr>`}
                  </tbody>
                </table>
              </div>

              <div class="ceoPanel__footer">
                <button class="ceoPanel__footerBtn" type="button" data-ceo-open="customers">דוח מכירות מלא</button>
              </div>
            </section>
          </div>
        </div>`;

      root.querySelectorAll('[data-ceo-open]').forEach((btn) => {
        on(btn, 'click', () => {
          const view = safeTrim(btn.getAttribute('data-ceo-open')) || 'customers';
          UI.goView(view);
        });
      });
      root.querySelectorAll('[data-ceo-action]').forEach((btn) => {
        on(btn, 'click', () => {
          const action = safeTrim(btn.getAttribute('data-ceo-action'));
          if(action === 'new-customer'){
            document.getElementById('btnNewCustomerWizard')?.click?.();
            return;
          }
          if(action === 'assistant'){
            try { document.getElementById('chatFabBtn')?.click?.(); } catch(_e) {}
          }
        });
      });
    }
  ...
  const App = {
    ...
    async reloadSessionState(){
      ...
      if (r.ok) {
        ...
        if (Auth.isAdmin() || Auth.hasExclusiveCeoAccess()) UsersUI.render();
        if (Auth.current) {
          DashboardUI.render();
          DashboardUI.render();
          CustomersUI.render();
