...
(() => {
  ...
  const BUILD = "20260324-operational-report-direct-pdf-1";
...
// ---------- Step 6 ----------
    ...
    renderHealthSummary(){
      ...
      return `<div class="lcHLayout"><div class="lcHMain"><div class="lcHFinishHero">
        ...
        <div class="lcHFinishHero__actions">
          <button type="button" class="btn btn--primary" data-health-open-operational-report="1">דוח תפעולי</button>
          <button type="button" class="btn" data-health-download-operational-report="1">הורד דוח תפעולי</button>
        </div>
    ...
    renderOperationalReport(payloadOverride){
      ...
      const exportedAt = (() => {
        const ms = Date.parse(payload.createdAt || '');
        return Number.isFinite(ms) ? new Date(ms).toLocaleString('he-IL') : new Date().toLocaleString('he-IL');
      })();

      const customerFullName = safeTrim(((primary.firstName || '') + ' ' + (primary.lastName || '')).trim()) || 'לקוח ללא שם';
      ...
      const getPolicyInsuredLabelSafe = (policy) => {
        try{
          if(typeof this.getNewPolicyInsuredLabel === 'function'){
            return this.getNewPolicyInsuredLabel(payload, policy, insureds);
          }
        }catch(_e){}
        if(policy?.insuredMode === 'couple'){
      ...
      const getPolicyCoverageDisplaySafe = (policy) => {
        try{ if(typeof this.getPolicyCoverageDisplayValue === 'function') return this.getPolicyCoverageDisplayValue(policy || {}); }catch(_e){}
        const type = safeTrim(policy?.type || policy?.product || '');
      ...
      const getPolicyPledgeSummarySafe = (policy) => {
        const type = safeTrim(policy?.type || policy?.product || '');
        const isLienType = (type === 'ריסק' || type === 'ריסק משכנתא');
        if(!isLienType) return { active:false, status:'לא רלוונטי', rows:[] };
        if(!policy?.pledge) return { active:false, status:'ללא שיעבוד', rows:[] };
        const bank = policy?.pledgeBank && typeof policy.pledgeBank === 'object' ? policy.pledgeBank : {};
        const rows = [
          ['סוג שיעבוד', 'מוטב בלתי חוזר'],
          ['שם בנק', safeTrim(bank.bankName)],
          ['מספר בנק', safeTrim(bank.bankNo)],
          ['סניף', safeTrim(bank.branch)],
          ['סכום לשיעבוד', safeTrim(bank.amount)],
          ['לכמה שנים', safeTrim(bank.years)],
          ['כתובת הבנק', safeTrim(bank.address)]
        ].filter(([, value]) => safeTrim(value));
        return { active:true, status:'יש שיעבוד', rows };
      };

      const customerRows = [
        ['שם מלא', this.renderReportValue(customerFullName)],
        ['תעודת זהות', this.renderReportValue(primary.idNumber)],
        ['תאריך לידה', this.renderReportValue(primary.birthDate)],
        ['גיל', this.renderReportValue(primary.age)],
        ['מגדר', this.renderReportValue(primary.gender)],
        ['מצב משפחתי', this.renderReportValue(primary.maritalStatus)],
        ['טלפון', this.renderReportValue(primary.phone)],
        ['אימייל', this.renderReportValue(primary.email)],
        ['עיר', this.renderReportValue(primary.city)],
        ['רחוב', this.renderReportValue(primary.street)],
        ['מספר בית', this.renderReportValue(primary.houseNumber)],
        ['מיקוד', this.renderReportValue(primary.zip)],
        ['קופת חולים', this.renderReportValue(primary.clinic || primary.hmo || primary.kupatHolim)],
        ['שב״ן', this.renderReportValue(primary.shaban || primary.shabanLevel)],
        ['עיסוק', this.renderReportValue(primary.occupation)]
      ];

      const medicalRows = [
        ['גובה', this.renderReportValue(primary.heightCm)],
        ['משקל', this.renderReportValue(primary.weightKg)],
        ['BMI', this.renderReportValue(primary.bmi)]
      ];

      const summaryRows = [
        ['לקוח', this.renderReportValue(customerFullName)],
        ['תעודת זהות', this.renderReportValue(primary.idNumber)],
        ['תאריך ייצוא', this.renderReportValue(exportedAt)],
        ['סוכן מטפל', this.renderReportValue(currentAgentName)]
      ];
      ...
      insureds.forEach((ins) => {
        ...
        existingPolicies.forEach((p) => {
          const statusMeta = this.getExistingPolicyStatusMeta(p, insData);
          existingRows.push([
            this.renderReportValue(safeTrim(ins?.label) || 'מבוטח'),
            this.renderReportValue(p?.company),
            this.renderReportValue(p?.type),
            this.renderReportValue(p?.policyNumber),
            this.renderReportValue(p?.monthlyPremium),
            `${this.renderReportStatusBadge(statusMeta.label, statusMeta.tone)}${statusMeta.reason ? `<div class="lcReportInlineNote">${escapeHtml(statusMeta.reason)}</div>` : ''}${statusMeta.partialDetails ? `<div class="lcReportInlineSubNote">${escapeHtml(statusMeta.partialDetails)}</div>` : ''}`
          ]);
        });
      });

      const newPolicyBlocks = newPolicies.map((p, idx) => {
        const pledge = getPolicyPledgeSummarySafe(p || {});
        const coverageValue = getPolicyCoverageDisplaySafe(p || {});
        const coverItems = getPolicyCoverItemsSafe(p || {});
        const policyRows = [
          ['מבוטח', this.renderReportValue(getPolicyInsuredLabelSafe(p))],
          ['חברה', this.renderReportValue(p?.company)],
          ['מוצר', this.renderReportValue(p?.type)],
          ['פרמיה חודשית', this.renderReportValue(p?.premiumMonthly)],
          ['דירוג הנחה', this.renderReportValue(this.getPolicyDiscountDisplayText(p || {}))],
          ['פרמיה אחרי הנחה', this.renderReportValue(this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(p || {})))],
          ['תאריך תחילת ביטוח', this.renderReportValue(p?.startDate)],
          ['כיסוי / סכום / פיצוי', this.renderReportValue(coverageValue)]
        ];

        const coverMarkup = coverItems.length
          ? `<div class="lcReportChips">${coverItems.map(item => `<span class="lcReportChip">${escapeHtml(item)}</span>`).join('')}</div>`
          : `<div class="lcReportInlineNote">לא הוגדרו כיסויים נוספים מעבר לנתוני המוצר.</div>`;

        const pledgeMarkup = pledge.rows.length
          ? `<div class="lcReportCard lcReportCard--sub"><div class="lcReportCard__title">שיעבוד</div>${this.renderReportRowsList(pledge.rows.map(([label, value]) => [label, this.renderReportValue(value)]), { dense:true })}</div>`
          : '';

        return `<article class="lcReportCard lcReportPolicyCard">
          <div class="lcReportPolicyCard__head">
            <div>
              <div class="lcReportPolicyCard__eyebrow">פוליסה חדשה ${idx + 1}</div>
              <div class="lcReportPolicyCard__title">${escapeHtml(safeTrim(p?.company) || 'חברה')} · ${escapeHtml(safeTrim(p?.type) || 'מוצר')}</div>
            </div>
            ${this.renderReportStatusBadge(pledge.status, pledge.active ? 'warn' : 'neutral')}
          </div>
          ${this.renderReportRowsList(policyRows, { dense:true })}
          <div class="lcReportCard lcReportCard--sub">
            <div class="lcReportCard__title">כיסויים</div>
            ${coverMarkup}
          </div>
          ${pledgeMarkup}
        </article>`;
      ...
      const payerRows = [
        ['מי משלם', this.renderReportValue(primary.payerChoice === 'external' ? 'משלם חיצוני' : 'המבוטח')],
        ['אמצעי תשלום', this.renderReportValue(primary.paymentMethod === 'cc' ? 'אשראי' : primary.paymentMethod === 'ho' ? 'הוראת קבע' : primary.paymentMethod)]
      ];
      if(primary.payerChoice === 'external'){
        const ex = primary.externalPayer || {};
        payerRows.push(
          ['קרבה למבוטח', this.renderReportValue(ex.relation)],
          ['שם פרטי', this.renderReportValue(ex.firstName)],
          ['שם משפחה', this.renderReportValue(ex.lastName)],
          ['תעודת זהות', this.renderReportValue(ex.idNumber)],
          ['תאריך לידה', this.renderReportValue(ex.birthDate)],
          ['טלפון', this.renderReportValue(ex.phone)]
        );
      }
      if(safeTrim(primary.paymentMethod) === 'cc'){
        const cc = primary.cc || {};
        payerRows.push(
          ['שם מחזיק', this.renderReportValue(cc.holderName)],
          ['ת.ז מחזיק', this.renderReportValue(cc.holderId)],
          ['מספר כרטיס', this.renderReportValue(cc.cardNumber)],
          ['תוקף', this.renderReportValue(cc.exp)]
        );
      } else if(safeTrim(primary.paymentMethod) === 'ho'){
        const ho = primary.ho || {};
        payerRows.push(
          ['שם בנק', this.renderReportValue(ho.bankName)],
          ['מספר בנק', this.renderReportValue(ho.bankNo)],
          ['סניף', this.renderReportValue(ho.branch)],
          ['מספר חשבון', this.renderReportValue(ho.account)]
        );
      }

      const healthItems = this.buildHealthItemsFromPayload(payload);
      const insuredCards = insureds.length
        ? insureds.map((ins, index) => {
            const d = ins?.data && typeof ins.data === 'object' ? ins.data : {};
            const rows = [
              ['שם מבוטח', this.renderReportValue(safeTrim(((d.firstName || '') + ' ' + (d.lastName || '')).trim()) || ins?.label)],
              ['סוג מבוטח', this.renderReportValue(ins?.type === 'primary' ? 'ראשי' : ins?.type === 'spouse' ? 'בן/בת זוג' : ins?.type === 'child' ? 'קטין' : 'מבוטח')],
              ['תעודת זהות', this.renderReportValue(d.idNumber)],
              ['תאריך לידה', this.renderReportValue(d.birthDate)],
              ['טלפון', this.renderReportValue(d.phone)],
              ['עיר', this.renderReportValue(d.city)],
              ['קופת חולים', this.renderReportValue(d.clinic || d.hmo || d.kupatHolim)],
              ['שב״ן', this.renderReportValue(d.shaban || d.shabanLevel)]
            ];
            return `<article class="lcReportCard lcReportInsuredCard">
              <div class="lcReportCard__title">${escapeHtml(safeTrim(ins?.label) || `מבוטח ${index + 1}`)}</div>
              ${this.renderReportRowsList(rows, { dense:true })}
            </article>`;
          }).join('')
        : '<div class="muted">לא נמצאו מבוטחים.</div>';

      const agentNumberRows = Object.entries(payload.companyAgentNumbers || {}).map(([company, agentNo]) => [this.renderReportValue(company), this.renderReportValue(agentNo)]);

      return `<div class="lcReportDoc lcReportDoc--modern">
        <section class="lcReportShowcase">
          <div class="lcReportShowcase__topline"></div>
          <div class="lcReportShowcase__header">
            <div class="lcReportShowcase__brand">GEMEL INVEST</div>
            <div class="lcReportShowcase__titleWrap">
              <div class="lcReportShowcase__title">דוח תפעולי</div>
            </div>
          </div>
          <div class="lcReportShowcase__line"></div>

          <div class="lcReportGrid lcReportGrid--hero">
            <section class="lcReportCard">
              <div class="lcReportCard__title">פרטי הדוח</div>
              ${this.renderReportRowsList(summaryRows, { dense:true })}
            </section>
            <section class="lcReportCard">
              <div class="lcReportCard__title">פרטי לקוח</div>
              ${this.renderReportRowsList(customerRows, { dense:true })}
            </section>
          </div>

          <div class="lcReportGrid lcReportGrid--half">
            <section class="lcReportCard">
              <div class="lcReportCard__title">נתונים רפואיים</div>
              ${this.renderReportRowsList(medicalRows, { dense:true })}
            </section>
            <section class="lcReportCard">
              <div class="lcReportCard__title">מבוטחים</div>
              <div class="lcReportStack">${insuredCards}</div>
            </section>
          </div>

          <section class="lcReportSection lcReportSection--soft">
            <div class="lcReportSection__title">פוליסות קיימות</div>
            <div class="lcReportSection__sub">הסטטוס מוצג בדיוק לפי מה שהנציג מילא בהקמת הלקוח.</div>
            ${this.renderTable(['מבוטח','חברה','סוג ביטוח','מספר פוליסה','פרמיה חודשית','סטטוס'], existingRows)}
          </section>

          <section class="lcReportSection lcReportSection--soft">
            <div class="lcReportSection__title">פוליסות חדשות</div>
            <div class="lcReportSection__sub">כל פוליסה מוצגת בשורות מסודרות, כולל כיסויים ושיעבוד כשקיים.</div>
            <div class="lcReportStack">${newPolicyBlocks.length ? newPolicyBlocks.join('') : '<div class="muted">לא נוספו פוליסות חדשות.</div>'}</div>
          </section>

          ${agentNumberRows.length ? `<section class="lcReportSection lcReportSection--soft">
            <div class="lcReportSection__title">מספרי סוכן לחברות שנמכרו</div>
            ${this.renderTable(['חברה','מספר סוכן'], agentNumberRows)}
          </section>` : ''}

          <div class="lcReportGrid lcReportGrid--half">
            <section class="lcReportCard">
              <div class="lcReportCard__title">אמצעי תשלום</div>
              ${this.renderReportRowsList(payerRows, { dense:true, emptyText:'לא הוזנו פרטי תשלום.' })}
            </section>
            <section class="lcReportCard">
              <div class="lcReportCard__title">הצהרת בריאות</div>
              <div class="lcReportHealthList">${healthItems.length ? healthItems.join('') : '<div class="muted">לא נמצאו תשובות הצהרת בריאות להצגה.</div>'}</div>
            </section>
          </div>
        </section>
      </div>`;
    ...
    buildOperationalPdfMarkup(payloadOverride){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
      const primary = payload?.primary || {};
      const insureds = Array.isArray(payload?.insureds) ? payload.insureds : [];
      const newPolicies = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      ...
      const exportedDateLabel = exportedDate.toLocaleDateString('he-IL');
      const exportedTimeLabel = exportedDate.toLocaleTimeString('he-IL', { hour:'2-digit', minute:'2-digit' });
      ...
      const customerId = safeTrim(primary.idNumber) || '—';
      const currentAgentName = safeTrim(Auth?.current?.name) || safeTrim(payload?.agentName) || 'נציג מטפל';
      ...
      const summaryRows = [
        ...
        ['תאריך דוח', exportedDateLabel],
        ['שעת ייצוא', exportedTimeLabel],
      ...
      const customerRows = [
        ...
        ['כתובת', [safeTrim(primary.street), safeTrim(primary.houseNo), safeTrim(primary.apartmentNo)].filter(Boolean).join(' ')],
      ...
      const medicalRows = insureds.map((ins, index) => {
        const d = ins?.data || {};
        return {
          label: safeTrim(ins?.label) || `מבוטח ${index + 1}`,
          values: [
            ['גובה', safeTrim(d.height)],
            ['משקל', safeTrim(d.weight)],
            ['BMI', safeTrim(d.bmi)],
            ['עישון', safeTrim(d.smokingStatus || d.smoking)]
          ].filter(([,value]) => safeTrim(value))
        };
      ...
      insureds.forEach((ins, index) => {
        ...
        policies.forEach((policy) => {
          const statusRaw = safeTrim(policy?.status || policy?.policyStatus || policy?.actionStatus || policy?.action);
          existingRows.push([
            label,
            safeTrim(policy?.company),
            safeTrim(policy?.type || policy?.product),
            safeTrim(policy?.policyNumber),
            safeTrim(policy?.premium),
            statusRaw || '—'
          ]);
        });
      });
      ...
      const getPolicyPledgeSummarySafe = (policy) => {
        ...
        return [
          ...
          ['שם בנק', safeTrim(bank.bankName)],
          ['מספר בנק', safeTrim(bank.bankNo)],
          ['סניף', safeTrim(bank.branchNo)],
          ['סכום משכנתא', safeTrim(policy?.mortgageAmount)],
          ['שנות משכנתא', safeTrim(policy?.mortgageYears)],
          ['כתובת נכס', safeTrim(policy?.mortgageAddress)]
        ].filter(([,value]) => safeTrim(value));
      };
      const getPolicyInsuredLabelSafe = (policy) => {
        if(policy?.insuredMode === 'couple'){
      ...
      const newPolicyBlocks = newPolicies.map((policy, index) => ({
        ...
        rows: [
          ...
          ['סוג מוצר', safeTrim(policy?.type || policy?.product)],
          ['פרמיה חודשית', safeTrim(policy?.premium)],
          ['תאריך תחילה', safeTrim(policy?.startDate)],
          ['כיסוי / סכום ביטוח', getPolicyCoverageDisplaySafe(policy)],
          ['אופן תשלום', safeTrim(policy?.paymentMethod)]
        ].filter(([,value]) => safeTrim(value)),
        covers: getPolicyCoverItemsSafe(policy),
        pledgeRows: getPolicyPledgeSummarySafe(policy)
      }));
      const payer = primary?.payer || {};
      const payerRows = [
        ['סוג אמצעי', safeTrim(payer?.method || primary?.paymentMethod)],
        ['בעל כרטיס / חשבון', safeTrim(payer?.holderName || primary?.payerName)],
        ['תעודת זהות משלם', safeTrim(payer?.holderId || primary?.payerId)],
        ['מספר כרטיס', safeTrim(payer?.cardNumberMasked || payer?.cardNumber || primary?.cardNumberMasked)],
        ['תוקף', safeTrim(payer?.cardExpiry || primary?.cardExpiry)],
        ['בנק', safeTrim(payer?.bankName)],
        ['מספר בנק', safeTrim(payer?.bankNo)],
        ['סניף', safeTrim(payer?.branchNo)],
        ['חשבון', safeTrim(payer?.accountNo)]
      ].filter(([,value]) => safeTrim(value));
      const healthItems = this.buildHealthItemsFromPayload(payload);
      const renderListRows = (rows, emptyText='—') => {
        ...
        return `<div class="lcPdfInfoList">${safeRows.map(([label, value]) => `<div class="lcPdfInfoRow"><div class="lcPdfInfoRow__label">${escapeHtml(label)}</div><div class="lcPdfInfoRow__value">${escapeHtml(value)}</div></div>`).join('')}</div>`;
      };
      const renderTable = (headers, rows, emptyText='אין נתונים להצגה.') => {
        ...
        if(!safeRows.length) return `<div class="lcPdfEmpty">${escapeHtml(emptyText)}</div>`;
        return `<div class="lcPdfTableWrap"><table class="lcPdfTable"><thead><tr>${headers.map((h) => `<th>${escapeHtml(h)}</th>`).join('')}</tr></thead><tbody>${safeRows.map((row) => `<tr>${row.map((cell) => `<td>${escapeHtml(cell || '—')}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
      };
      const renderHealthCards = () => {
        if(!healthItems.length) return `<div class="lcPdfEmpty">לא נמצאו תשובות הצהרת בריאות להצגה.</div>`;
        return `<div class="lcPdfHealthList">${healthItems.map((item) => `<div class="lcPdfHealthCard">${item}</div>`).join('')}</div>`;
      };
      const renderPolicyBlocks = () => {
        if(!newPolicyBlocks.length) return `<div class="lcPdfEmpty">לא נוספו פוליסות חדשות.</div>`;
        return `<div class="lcPdfPolicyStack">${newPolicyBlocks.map((block) => `<section class="lcPdfPolicyCard"><div class="lcPdfPolicyCard__head"><div><div class="lcPdfPolicyCard__title">${escapeHtml(block.title)}</div><div class="lcPdfPolicyCard__sub">${escapeHtml(block.subtitle)}</div></div></div>${renderListRows(block.rows)}${block.covers.length ? `<div class="lcPdfChips">${block.covers.map((cover) => `<span class="lcPdfChip">${escapeHtml(cover)}</span>`).join('')}</div>` : ''}${block.pledgeRows.length ? `<div class="lcPdfSubCard"><div class="lcPdfSubCard__title">שיעבוד</div>${renderListRows(block.pledgeRows)}</div>` : ''}</section>`).join('')}</div>`;
      };
      const renderMedicalCard = () => {
        if(!medicalRows.length) return `<div class="lcPdfEmpty">לא הוזנו נתונים רפואיים.</div>`;
        return `<div class="lcPdfPolicyStack">${medicalRows.map((item) => `<section class="lcPdfSubCard"><div class="lcPdfSubCard__title">${escapeHtml(item.label)}</div>${renderListRows(item.values)}</section>`).join('')}</div>`;
      };
      const style = `<style>
        .lcPdfRoot,.lcPdfRoot *{box-sizing:border-box}
        .lcPdfRoot{width:794px;background:#ffffff;color:#153050;font-family:Arial,"Noto Sans Hebrew","Segoe UI",sans-serif;direction:rtl;padding:34px 38px 40px}
        .lcPdfTopRule,.lcPdfBottomRule{height:1px;background:#d9e2ef;width:100%}
        .lcPdfHeader{padding:26px 0 18px}
        .lcPdfHeader__row{display:flex;justify-content:space-between;align-items:flex-start;gap:20px}
        .lcPdfHeader__title{font-size:44px;font-weight:800;letter-spacing:-.6px;margin:24px 0 12px;color:#112a4b}
        .lcPdfHeader__brand{display:flex;align-items:flex-start;justify-content:flex-end;gap:14px;min-width:240px}
        .lcPdfHeader__brandText{font-size:18px;font-weight:700;letter-spacing:.8px;color:#1b3555;margin-top:8px}
        .lcPdfHeader__logo{max-width:88px;max-height:88px;object-fit:contain}
        .lcPdfHeader__meta{display:flex;gap:12px;flex-wrap:wrap;margin-top:12px;color:#5a718f;font-size:13px}
        .lcPdfSection{margin-top:18px;border:1px solid #dfe6ef;border-radius:14px;overflow:hidden;background:#fff;break-inside:avoid;page-break-inside:avoid}
        .lcPdfSection__head{background:linear-gradient(180deg,#f3f6fa,#edf2f7);padding:14px 18px;border-bottom:1px solid #dde5ef;display:flex;justify-content:space-between;align-items:center;gap:12px}
        .lcPdfSection__title{font-size:18px;font-weight:800;color:#223b5a}
        .lcPdfSection__body{padding:16px 18px}
        .lcPdfGrid{display:grid;gap:18px}
        .lcPdfGrid--2{grid-template-columns:1fr 1fr}
        .lcPdfInfoList{display:grid;gap:0}
        .lcPdfInfoRow{display:grid;grid-template-columns:160px 1fr;align-items:start;gap:12px;padding:8px 0;border-bottom:1px solid #eef2f6;min-height:34px}
        .lcPdfInfoRow:last-child{border-bottom:0}
        .lcPdfInfoRow__label{font-weight:700;color:#6c8098}
        .lcPdfInfoRow__value{font-weight:600;color:#1d3552;white-space:pre-wrap;word-break:break-word}
        .lcPdfTableWrap{overflow:hidden;border:1px solid #e3e9f1;border-radius:12px}
        .lcPdfTable{width:100%;border-collapse:collapse;font-size:13px}
        .lcPdfTable th{background:#f6f8fb;color:#5e728d;font-weight:800;padding:10px 12px;border-bottom:1px solid #e1e8f0;border-left:1px solid #e8edf4;text-align:right}
        .lcPdfTable td{padding:10px 12px;border-bottom:1px solid #edf2f7;border-left:1px solid #edf2f7;color:#1f3755;vertical-align:top}
        .lcPdfTable th:last-child,.lcPdfTable td:last-child{border-left:0}
        .lcPdfTable tr:last-child td{border-bottom:0}
        .lcPdfPolicyStack{display:grid;gap:14px}
        .lcPdfPolicyCard,.lcPdfSubCard{border:1px solid #e2e8f0;border-radius:12px;padding:14px;background:#fff;break-inside:avoid;page-break-inside:avoid}
        .lcPdfPolicyCard__head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:10px}
        .lcPdfPolicyCard__title{font-size:18px;font-weight:800;color:#183252}
        .lcPdfPolicyCard__sub{margin-top:3px;font-size:13px;color:#7387a1;font-weight:700}
        .lcPdfSubCard{margin-top:12px;background:#fafcff}
        .lcPdfSubCard__title{font-size:15px;font-weight:800;color:#294361;margin-bottom:8px}
        .lcPdfChips{display:flex;flex-wrap:wrap;gap:8px;margin-top:12px}
        .lcPdfChip{display:inline-flex;align-items:center;padding:6px 10px;border-radius:999px;background:#f0f5fb;border:1px solid #dce6f1;color:#28435e;font-size:12px;font-weight:700}
        .lcPdfHealthList{display:grid;gap:12px}
        .lcPdfHealthCard .lcReportHealthItem{border:1px solid #e1e8f0;border-radius:12px;padding:12px;background:#fbfdff}
        .lcPdfHealthCard .lcReportHealthItem__top{display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap}
        .lcPdfHealthCard .lcReportHealthItem__question{margin-top:7px;font-weight:800;color:#173150;font-size:14px}
        .lcPdfHealthCard .lcReportHealthItem__answer{margin-top:6px;color:#4a607b;font-size:13px;line-height:1.65}
        .lcPdfHealthCard .lcReportStatus{display:inline-flex;align-items:center;padding:4px 10px;border-radius:999px;font-size:12px;font-weight:800;border:1px solid #d4deeb}
        .lcPdfHealthCard .lcReportStatus--success{background:#edf9f1;color:#157347;border-color:#cae8d3}
        .lcPdfHealthCard .lcReportStatus--danger{background:#fff2f2;color:#b42318;border-color:#f5d0cf}
        .lcPdfHealthCard .lcReportStatus--warn{background:#fff8e8;color:#9a6700;border-color:#f0ddb0}
        .lcPdfEmpty{padding:14px 0;color:#70839c;font-weight:600}
        .lcPdfFooterSummary{margin-top:20px;padding:18px;border:1px solid #dfe6ef;border-radius:14px;background:linear-gradient(180deg,#f8fbfe,#f1f5fa);break-inside:avoid;page-break-inside:avoid}
        .lcPdfFooterSummary__title{font-size:20px;font-weight:800;color:#173250;margin-bottom:10px}
        .lcPdfFooterSummary ul{margin:0;padding:0 18px 0 0}
        .lcPdfFooterSummary li{margin:8px 0;color:#27415f;font-weight:600}
      </style>`;
      return {
        payload,
        html: `${style}<div class="lcPdfRoot"><div class="lcPdfTopRule"></div><header class="lcPdfHeader"><div class="lcPdfHeader__row"><div><div class="lcPdfHeader__title">דוח תפעולי</div><div class="lcPdfHeader__meta"><span>${escapeHtml(exportedAt)}</span><span>•</span><span>${escapeHtml(currentAgentName)}</span></div></div><div class="lcPdfHeader__brand"><div class="lcPdfHeader__brandText">GEMEL INVEST</div><img src="${escapeHtml(logoSrc)}" alt="GEMEL INVEST" class="lcPdfHeader__logo"></div></div></header><div class="lcPdfTopRule"></div><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">פרטי הדוח</div><div class="lcPdfSection__title">פרטי לקוח</div></div><div class="lcPdfSection__body"><div class="lcPdfGrid lcPdfGrid--2"><div>${renderListRows(summaryRows)}</div><div>${renderListRows(customerRows, 'לא הוזנו פרטי לקוח')}</div></div></div></section><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">נתונים רפואיים</div><div class="lcPdfSection__title">פוליסות קיימות</div></div><div class="lcPdfSection__body"><div class="lcPdfGrid lcPdfGrid--2"><div>${renderMedicalCard()}</div><div>${renderTable(['מבוטח','חברה','סוג','מספר פוליסה','פרמיה','סטטוס'], existingRows, 'לא נוספו פוליסות קיימות')}</div></div></div></section><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">פוליסות חדשות</div></div><div class="lcPdfSection__body">${renderPolicyBlocks()}</div></section><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">אמצעי תשלום</div><div class="lcPdfSection__title">הצהרת בריאות</div></div><div class="lcPdfSection__body"><div class="lcPdfGrid lcPdfGrid--2"><div>${renderListRows(payerRows, 'לא הוזנו פרטי תשלום')}</div><div>${renderHealthCards()}</div></div></div></section><section class="lcPdfFooterSummary"><div class="lcPdfFooterSummary__title">סיכום תפעולי</div><ul><li>כל פרטי הלקוח, הפוליסות ואמצעי התשלום נכללו בדוח.</li><li>פוליסות חדשות מוצגות עם כיסויים ושיעבוד כאשר קיימים.</li><li>הצהרת הבריאות משקפת גם תשובות "כן" וגם תשובות "לא" בצורה מסודרת.</li></ul></section></div>`
      };
    },


    _getOperationalPdfFieldLabel(key){
      const map = {
        firstName:'שם פרטי', lastName:'שם משפחה', idNumber:'תעודת זהות', birthDate:'תאריך לידה', age:'גיל', gender:'מגדר',
        maritalStatus:'מצב משפחתי', phone:'טלפון', email:'אימייל', city:'עיר', street:'רחוב', houseNumber:'מספר בית',
        houseNo:'מספר בית', apartment:'דירה', apartmentNo:'דירה', zip:'מיקוד', clinic:'קופת חולים', hmo:'קופת חולים',
        kupatHolim:'קופת חולים', shaban:'שב״ן', shabanLevel:'שב״ן', occupation:'עיסוק', height:'גובה', heightCm:'גובה',
        weight:'משקל', weightKg:'משקל', bmi:'BMI', smoking:'עישון', smokingStatus:'עישון', company:'חברה', type:'סוג מוצר',
        product:'סוג מוצר', policyNumber:'מספר פוליסה', premium:'פרמיה', monthlyPremium:'פרמיה חודשית', sumInsured:'סכום ביטוח',
        compensation:'פיצוי', coverage:'כיסוי', coverages:'כיסויים', startDate:'תחילת ביטוח', status:'סטטוס', policyStatus:'סטטוס',
        actionStatus:'סטטוס', action:'סטטוס', bankAgency:'טופל דרך בנק', bankAgencyName:'שם סוכנות בנק', hasPledge:'יש שיעבוד',
        pledge:'שיעבוד', mortgageAmount:'סכום משכנתא', mortgageYears:'שנות משכנתא', mortgageAddress:'כתובת נכס', insuredMode:'אופן ביטוח',
        insuredId:'מזהה מבוטח', healthCovers:'כיסויי בריאות', covers:'כיסויים', payerChoice:'מי משלם', selectedPayerId:'משלם נבחר',
        paymentMethod:'אמצעי תשלום', relation:'קרבה למבוטח', holderName:'שם מחזיק', holderId:'תעודת זהות מחזיק', cardNumber:'מספר כרטיס',
        cardNumberMasked:'מספר כרטיס', exp:'תוקף', cardExpiry:'תוקף', bankName:'שם בנק', bankNo:'מספר בנק', branch:'סניף',
        branchNo:'סניף', account:'מספר חשבון', accountNo:'מספר חשבון', fullName:'שם מלא', payerName:'שם משלם', payerId:'תעודת זהות משלם',
        payerPhone:'טלפון משלם', payerEmail:'אימייל משלם', notes:'הערות', details:'פירוט', description:'תיאור', companyAgentNumbers:'מספרי סוכן',
        policyOwner:'בעל פוליסה', paymentDay:'יום חיוב', paymentFrequency:'תדירות תשלום', policyPayer:'משלם פוליסה', policyPayers:'משלמי פוליסה',
        creditType:'סוג כרטיס', cardBrand:'מותג כרטיס', question:'שאלה', answer:'תשובה', answerLabel:'תשובה', title:'כותרת', label:'שדה'
      };
      return map[key] || key.replace(/([A-Z])/g, ' $1').replace(/[_-]+/g, ' ').trim();
    },

    _formatOperationalPdfValue(value){
      if(value === null || value === undefined) return '';
      if(value === true) return 'כן';
      if(value === false) return 'לא';
      if(Array.isArray(value)){
        const simple = value.map((item) => this._formatOperationalPdfValue(item)).filter(Boolean);
        return simple.join(', ');
      }
      if(typeof value === 'object') return '';
      const raw = safeTrim(value);
      if(!raw) return '';
      const yesSet = new Set(['yes','true','כן']);
      const noSet = new Set(['no','false','לא']);
      const lower = raw.toLowerCase();
      if(yesSet.has(lower)) return 'כן';
      if(noSet.has(lower)) return 'לא';
      return raw;
    },

    _collectOperationalPdfPairs(input, options = {}){
      const out = [];
      const skip = new Set(Array.isArray(options.skipKeys) ? options.skipKeys : []);
      const prefix = safeTrim(options.prefix || '');
      const walk = (obj, trail='') => {
        if(!obj || typeof obj !== 'object') return;
        Object.entries(obj).forEach(([key, value]) => {
          if(skip.has(key)) return;
          if(value === null || value === undefined || value === '') return;
          const label = this._getOperationalPdfFieldLabel(key);
          const fullLabel = trail ? `${trail} · ${label}` : label;
          if(Array.isArray(value)){
            if(!value.length) return;
            const allSimple = value.every((item) => item === null || item === undefined || ['string','number','boolean'].includes(typeof item));
            if(allSimple){
              const formatted = this._formatOperationalPdfValue(value);
              if(formatted) out.push([fullLabel, formatted]);
            }
            return;
          }
          if(typeof value === 'object'){
            walk(value, fullLabel);
            return;
          }
          const formatted = this._formatOperationalPdfValue(value);
          if(formatted) out.push([fullLabel, formatted]);
    ...
    _normalizeOperationalCompanyKey(company){
      const raw = safeTrim(company).toLowerCase();
      if(!raw) return '';
      const map = [
        [/הראל|harel/, 'harel'],
        [/מגדל|migdal|magdal/, 'migdal'],
        [/כלל|clal/, 'clal'],
        [/הפניקס|פניקס|phoenix|fenix/, 'phoenix'],
        [/מנורה|מב.?טחים|menora/, 'menora'],
        [/איילון|ayalon/, 'ayalon'],
        [/הכשרה|hachshara|hakshara/, 'hachshara'],
        [/שלמה|shlomo/, 'shlomo'],
        [/ביטוח ישיר|direct/, 'direct'],
        [/ווישור|we.?sure/, 'weSure']
      ];
    ...
    async _loadOperationalCompanyLogo(company){
      this._opPdfLogoCache = this._opPdfLogoCache || new Map();
      const key = this._normalizeOperationalCompanyKey(company || '');
      if(!key) return null;
      if(this._opPdfLogoCache.has(key)) return this._opPdfLogoCache.get(key);
      const root = safeTrim(window.location?.origin || '');
      const candidates = [
        `./assets/${key}.png`, `./assets/${key}.svg`, `./assets/logos/${key}.png`, `./assets/logos/${key}.svg`,
        `./logos/${key}.png`, `./logos/${key}.svg`, `./${key}.png`, `./${key}.svg`
      ];
      const unique = Array.from(new Set(candidates));
      const promise = new Promise((resolve) => {
        let idx = 0;
        const tryNext = () => {
          if(idx >= unique.length) return resolve(null);
          const src = unique[idx++];
          const img = new Image();
          img.crossOrigin = 'anonymous';
          img.onload = () => resolve({ img, src });
          img.onerror = () => tryNext();
          img.src = src.startsWith('./') && root ? src.replace('./', `${root}/`) : src;
        };
        tryNext();
      });
      this._opPdfLogoCache.set(key, promise);
      return promise;
    },

    _getOperationalPdfSections(payloadOverride){
      const payload = this.normalizeOperationalReportPayload(payloadOverride || this.getOperationalPayload());
      const primary = payload?.primary || {};
      const insureds = Array.isArray(payload?.insureds) ? payload.insureds : [];
      const newPolicies = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      ...
      const makePairsUnique = (rows) => {
        const seen = new Set();
        return (Array.isArray(rows) ? rows : []).filter(([label, value]) => {
          const l = safeTrim(label), v = safeTrim(value);
          if(!l || !v) return false;
          const sig = `${l}__${v}`;
      ...
      const customerRows = makePairsUnique([
        ['שם מלא', customerName],
        ['תעודת זהות', safeTrim(primary.idNumber)],
        ['תאריך לידה', safeTrim(primary.birthDate)],
        ['גיל', safeTrim(primary.age)],
        ['מגדר', safeTrim(primary.gender)],
        ['מצב משפחתי', safeTrim(primary.maritalStatus)],
        ...
        ['אימייל', safeTrim(primary.email)],
        ['עיר', safeTrim(primary.city)],
        ['רחוב', safeTrim(primary.street)],
        ['מספר בית', safeTrim(primary.houseNumber || primary.houseNo)],
        ['דירה', safeTrim(primary.apartment || primary.apartmentNo)],
        ['מיקוד', safeTrim(primary.zip)],
        ['קופת חולים', safeTrim(primary.clinic || primary.hmo || primary.kupatHolim)],
        ['שב״ן', safeTrim(primary.shaban || primary.shabanLevel)],
        ['עיסוק', safeTrim(primary.occupation)],
        ...this._collectOperationalPdfPairs(primary, { skipKeys:['firstName','lastName','idNumber','birthDate','age','gender','maritalStatus','phone','email','city','street','houseNumber','houseNo','apartment','apartmentNo','zip','clinic','hmo','kupatHolim','shaban','shabanLevel','occupation','existingPolicies','newPolicies','healthDeclaration','operationalAgentNumbers','policyPayers','cancellations','cc','ho','externalPayer','payer'] })
      ]);
      ...
      const paymentSource = (() => {
        ...
        const merged = { ...payment, ...payer };
        if(!merged.method) merged.method = safeTrim(primary.paymentMethod) === 'ho' ? 'הוראת קבע' : (safeTrim(primary.paymentMethod) === 'cc' ? 'כרטיס אשראי' : safeTrim(primary.paymentMethod));
        if(primary.payerChoice === 'external'){
          const ex = primary.externalPayer || {};
          if(!merged.fullName) merged.fullName = [safeTrim(ex.firstName), safeTrim(ex.lastName)].filter(Boolean).join(' ');
          if(!merged.idNumber) merged.idNumber = safeTrim(ex.idNumber);
          if(!merged.phone) merged.phone = safeTrim(ex.phone);
          if(!merged.email) merged.email = safeTrim(ex.email);
          if(!merged.relation) merged.relation = safeTrim(ex.relation);
        } else {
          if(!merged.fullName) merged.fullName = customerName;
          if(!merged.idNumber) merged.idNumber = safeTrim(primary.idNumber);
          if(!merged.phone) merged.phone = safeTrim(primary.phone);
          if(!merged.email) merged.email = safeTrim(primary.email);
        }
        if(safeTrim(primary.paymentMethod) === 'cc') merged.cc = { ...(primary.cc || {}), ...(merged.cc || {}) };
        if(safeTrim(primary.paymentMethod) === 'ho') merged.ho = { ...(primary.ho || {}), ...(merged.ho || {}) };
        return merged;
      })();
      const paymentRows = makePairsUnique([
        ['אמצעי תשלום', safeTrim(paymentSource.method || paymentSource.paymentMethod)],
        ['מי משלם', primary.payerChoice === 'external' ? 'משלם חיצוני' : 'מבוטח'],
        ['שם משלם', safeTrim(paymentSource.fullName || paymentSource.payerName)],
        ['תעודת זהות משלם', safeTrim(paymentSource.idNumber || paymentSource.payerId)],
        ['קרבה למבוטח', safeTrim(paymentSource.relation)],
        ['טלפון משלם', safeTrim(paymentSource.phone || paymentSource.payerPhone)],
        ['אימייל משלם', safeTrim(paymentSource.email || paymentSource.payerEmail)],
        ['שם מחזיק', safeTrim(paymentSource.cc?.holderName || paymentSource.holderName)],
        ['ת.ז מחזיק', safeTrim(paymentSource.cc?.holderId || paymentSource.holderId)],
        ['מספר כרטיס', safeTrim(paymentSource.cc?.cardNumberMasked || paymentSource.cc?.cardNumber || paymentSource.cardNumberMasked || paymentSource.cardNumber)],
        ['תוקף', safeTrim(paymentSource.cc?.exp || paymentSource.cc?.expiry || paymentSource.cardExpiry || paymentSource.expiry)],
        ['שם בנק', safeTrim(paymentSource.ho?.bankName || paymentSource.bankName)],
        ['מספר בנק', safeTrim(paymentSource.ho?.bankNo || paymentSource.bankNo)],
        ['סניף', safeTrim(paymentSource.ho?.branch || paymentSource.ho?.branchNo || paymentSource.branch || paymentSource.branchNo)],
        ['מספר חשבון', safeTrim(paymentSource.ho?.account || paymentSource.ho?.accountNo || paymentSource.account || paymentSource.accountNo)],
        ...this._collectOperationalPdfPairs(paymentSource, { skipKeys:['cc','ho','method','paymentMethod','fullName','payerName','idNumber','payerId','phone','payerPhone','email','payerEmail','relation','holderName','holderId','cardNumber','cardNumberMasked','cardExpiry','expiry','bankName','bankNo','branch','branchNo','account','accountNo'] }),
        ...this._collectOperationalPdfPairs(paymentSource.cc || {}, { prefix:'כרטיס אשראי', skipKeys:['holderName','holderId','cardNumber','cardNumberMasked','exp','expiry'] }),
        ...this._collectOperationalPdfPairs(paymentSource.ho || {}, { prefix:'הוראת קבע', skipKeys:['bankName','bankNo','branch','branchNo','account','accountNo'] })
      ]);

      const getInsuredLabel = (ins, index) => safeTrim(ins?.label) || safeTrim(((ins?.data?.firstName || '') + ' ' + (ins?.data?.lastName || '')).trim()) || `מבוטח ${index + 1}`;
      const existingPolicies = [];
      insureds.forEach((ins, index) => {
        const d = ins?.data || {};
        const label = getInsuredLabel(ins, index);
        const policies = Array.isArray(d?.existingPolicies) ? d.existingPolicies : [];
        policies.forEach((policy, pIndex) => {
          const cancel = d?.cancellations?.[policy?.id] || {};
          const statusRaw = safeTrim(cancel?.status || cancel?.action || policy?.status || policy?.policyStatus || policy?.actionStatus || policy?.action);
          const rows = makePairsUnique([
            ['מבוטח', label],
            ['חברה', safeTrim(policy?.company)],
            ['סוג מוצר', safeTrim(policy?.type || policy?.product)],
            ['מספר פוליסה', safeTrim(policy?.policyNumber)],
            ['פרמיה חודשית', safeTrim(policy?.monthlyPremium || policy?.premium)],
            ['סטטוס', statusRaw],
            ['סיבת שינוי/ביטול', safeTrim(cancel?.reason)],
            ['תאריך תחילה', safeTrim(policy?.startDate)],
            ['כיסוי', safeTrim(policy?.coverage)],
            ['סכום ביטוח', safeTrim(policy?.sumInsured)],
            ['דרך סוכן בנק', this._formatOperationalPdfValue(policy?.bankAgency)],
            ['שם סוכנות בנק', safeTrim(policy?.bankAgencyName)],
            ['יש שיעבוד', this._formatOperationalPdfValue(policy?.hasPledge)],
            ['שם בנק משעבד', safeTrim(policy?.pledgeBankName)],
            ...this._collectOperationalPdfPairs(policy, { skipKeys:['id','company','type','product','policyNumber','monthlyPremium','premium','status','policyStatus','actionStatus','action','startDate','coverage','sumInsured','bankAgency','bankAgencyName','hasPledge','pledgeBankName'] }),
            ...this._collectOperationalPdfPairs(cancel, { prefix:'פרטי סטטוס', skipKeys:['status','action','reason'] })
          ]);
          existingPolicies.push({
            section:'existing',
            company: safeTrim(policy?.company),
            title: `${safeTrim(policy?.company) || 'חברה לא צוינה'} · ${safeTrim(policy?.type || policy?.product) || 'סוג מוצר לא צוין'}`,
            subtitle: `פוליסה קיימת ${pIndex + 1} · ${label}`,
            rows
          });
        });
      });

      const getPolicyInsuredLabel = (policy) => {
        if(policy?.insuredMode === 'couple'){
          const primaryLabel = getInsuredLabel(insureds?.[0], 0);
          const spouse = (insureds || []).find((item) => item?.type === 'spouse');
          const spouseLabel = spouse ? getInsuredLabel(spouse, 1) : '';
          return spouseLabel ? `${primaryLabel} + ${spouseLabel}` : `${primaryLabel} (זוגי)`;
        }
        const found = (insureds || []).find((item) => safeTrim(item?.id) === safeTrim(policy?.insuredId));
        return found ? getInsuredLabel(found, 0) : getInsuredLabel(insureds?.[0], 0);
      };

      const newPolicyEntries = newPolicies.map((policy, index) => {
        const pledgeBank = (policy?.pledgeBank && typeof policy.pledgeBank === 'object') ? policy.pledgeBank : {};
        const rows = makePairsUnique([
          ['מבוטח', getPolicyInsuredLabel(policy)],
          ['חברה', safeTrim(policy?.company)],
          ['סוג מוצר', safeTrim(policy?.type || policy?.product)],
          ['פרמיה חודשית', safeTrim(policy?.premium || policy?.premiumAfterDiscount || policy?.premiumBefore)],
          ['פרמיה לפני הנחה', safeTrim(policy?.premiumBefore)],
          ['אחוז הנחה', safeTrim(policy?.discountPct)],
          ['תחילת ביטוח', safeTrim(policy?.startDate)],
          ['סכום ביטוח', safeTrim(policy?.sumInsured)],
          ['פיצוי', safeTrim(policy?.compensation)],
          ['כיסויים', this._formatOperationalPdfValue(policy?.healthCovers || policy?.covers)],
          ['אופן תשלום', safeTrim(policy?.paymentMethod)],
          ['משלם פוליסה', safeTrim(policy?.policyOwner || policy?.policyPayer)],
          ['שיעבוד', this._formatOperationalPdfValue(policy?.pledge)],
          ['שם בנק', safeTrim(pledgeBank?.bankName)],
          ['מספר בנק', safeTrim(pledgeBank?.bankNo)],
          ['סניף', safeTrim(pledgeBank?.branch || pledgeBank?.branchNo)],
          ['סכום משכנתא', safeTrim(pledgeBank?.amount || policy?.mortgageAmount)],
          ['שנות משכנתא', safeTrim(pledgeBank?.years || policy?.mortgageYears)],
          ['כתובת נכס', safeTrim(pledgeBank?.address || policy?.mortgageAddress)],
          ...this._collectOperationalPdfPairs(policy, { skipKeys:['insuredId','insuredMode','company','type','product','premium','premiumAfterDiscount','premiumBefore','discountPct','startDate','sumInsured','compensation','healthCovers','covers','paymentMethod','policyOwner','policyPayer','pledge','pledgeBank','mortgageAmount','mortgageYears','mortgageAddress'] }),
          ...this._collectOperationalPdfPairs(pledgeBank, { prefix:'פרטי שיעבוד', skipKeys:['bankName','bankNo','branch','branchNo','amount','years','address'] })
        ]);
        return {
          section:'new',
          company: safeTrim(policy?.company),
          title: `${safeTrim(policy?.company) || 'חברה לא צוינה'} · ${safeTrim(policy?.type || policy?.product) || 'סוג מוצר לא צוין'}`,
          subtitle: `פוליסה חדשה ${index + 1} · ${getPolicyInsuredLabel(policy)}`,
          rows
        };
      });

      const healthItems = [];
      const pushHealth = (title, answer, details='') => {
        const q = safeTrim(title), a = this._formatOperationalPdfValue(answer), d = safeTrim(details);
        if(!q) return;
        healthItems.push({ question:q, answer:a || '—', details:d });
      };
      const health = payload?.healthDeclaration || payload?.health || {};
      if(Array.isArray(health?.questions)) health.questions.forEach((item) => pushHealth(item?.question || item?.label, item?.answerLabel || item?.answer, item?.details || item?.notes));
      if(Array.isArray(payload?.healthFindings)) payload.healthFindings.forEach((item) => pushHealth(item?.question || item?.title, item?.status || item?.answer, item?.details || item?.description));
      insureds.forEach((ins, index) => {
        const list = Array.isArray(ins?.data?.healthDeclaration) ? ins.data.healthDeclaration : [];
        list.forEach((item) => pushHealth(`${getInsuredLabel(ins, index)} · ${item?.question || item?.label}`, item?.answerLabel || item?.answer, item?.details || item?.notes));
      });
      ...
      return {
        ...
        meta: {
          customerName,
          customerId: safeTrim(primary.idNumber) || '—',
          agentName: currentAgentName,
          exportedAt: `${exportedDateLabel} ${exportedTimeLabel}`,
          logoSrc: safeTrim(document.querySelector('.brand__logoImg, .lcLogin__logoImg')?.src) || './logo-login-clean.png'
        },
        customerRows,
        paymentRows,
        existingPolicies,
    ...
    async exportOperationalPdf(payloadOverride){
      ...
      const triggerBtn = this.els.reportPrint || this.els.btnDownloadOpsFile || null;
      ...
      try{
        const pack = this._getOperationalPdfSections(payloadOverride);
        const brandLogo = await new Promise((resolve) => {
          const img = new Image();
          img.crossOrigin = 'anonymous';
          img.onload = () => resolve(img);
          img.onerror = () => resolve(null);
          img.src = pack.meta.logoSrc;
        });

        const logoCompanies = Array.from(new Set([
          ...pack.existingPolicies.map((item) => item.company),
          ...pack.newPolicies.map((item) => item.company)
        ].map((v) => safeTrim(v)).filter(Boolean)));
        const logoEntries = await Promise.all(logoCompanies.map(async (company) => [company, await this._loadOperationalCompanyLogo(company)]));
        const companyLogoMap = new Map(logoEntries);
        ...
        const colors = {
          navy:'#14386b', navy2:'#1f4f8f', text:'#163254', muted:'#6b7f99', line:'#dbe5f0', soft:'#f3f7fb', soft2:'#eef3f9', white:'#ffffff'
        };
        ...
        const savePage = () => {
          ...
          copy.width = pageWidth;
          copy.height = pageHeight;
          copy.getContext('2d').drawImage(canvas, 0, 0);
          pages.push(copy);
        };
        ctx.direction = 'rtl';
        const drawText = (text, x, y, font, color=colors.text, align='right') => {
          ctx.font = font;
          ctx.fillStyle = color;
          ctx.textAlign = align;
        ...
        const wrapRtl = (text, maxWidth, font) => {
          ctx.font = font;
          const lines = [];
          String(text || '').split('\n').forEach((rawLine) => {
            const words = safeTrim(rawLine).split(/\s+/).filter(Boolean);
        ...
        const drawWrappedRtl = (text, x, y, maxWidth, font, color=colors.text, lineGap=18) => {
          const lines = wrapRtl(text, maxWidth, font);
          lines.forEach((line, idx) => drawText(line, x, y + (idx * lineGap), font, color, 'right'));
          return lines.length * lineGap;
        };
        const roundRect = (x, y, w, h, r, fill=null, stroke=null, lineWidth=2) => {
          ctx.beginPath();
          ctx.moveTo(x + r, y);
        ...
        const drawPill = (label, value, x, y, w, h) => {
          roundRect(x, y, w, h, 26, colors.white, '#c8d4e5', 2);
          const labelW = Math.min(172, Math.max(132, Math.floor(w * 0.28)));
          roundRect(x + w - labelW - 12, y + 10, labelW, h - 20, 22, colors.soft, null, 0);
          drawText(label, x + w - 28, y + 38, '700 16px Arial', colors.navy, 'right');
          const valueText = safeTrim(value) || '—';
          drawWrappedRtl(valueText, x + w - labelW - 26, y + 34, w - labelW - 48, '600 14px Arial', colors.text, 16);
        };
        const pairRows = (rows) => {
          const safe = (Array.isArray(rows) ? rows : []).filter(([label, value]) => safeTrim(label) && safeTrim(value));
          const pairs = [];
          for(let i=0; i<safe.length; i += 2){
            pairs.push([safe[i], safe[i + 1] || null]);
          }
          return pairs;
        };
        const drawSectionLabel = (title, y) => {
          roundRect(margin, y, innerWidth, 42, 21, colors.soft, null, 0);
          drawText(title, pageWidth - margin - 20, y + 28, '800 22px Arial', colors.navy, 'right');
        };
        const drawInfoGrid = (title, rows, startY) => {
          let y = startY;
          drawSectionLabel(title, y);
          y += 54;
          const colGap = 18;
          const colW = (innerWidth - colGap) / 2;
          pairRows(rows).forEach((pair) => {
            const left = pair[0];
            const right = pair[1];
            const rowH = 62;
            if(right) drawPill(right[0], right[1], margin, y, colW, rowH);
            if(left) drawPill(left[0], left[1], margin + colW + colGap, y, colW, rowH);
            y += rowH + 12;
          });
          return y;
        };
        const measurePolicyCard = (entry) => {
          const detailLines = (entry.rows || []).flatMap(([label, value]) => wrapRtl(`${label}: ${value}`, innerWidth - 160, '600 14px Arial'));
          return 68 + (detailLines.length * 18);
        };
        const drawCompanyLogo = (company, x, y, size) => {
          const logo = companyLogoMap.get(company);
          if(logo && logo.img){
            try{
              roundRect(x, y, size, size, 16, colors.white, '#d7e1ec', 1.5);
              ctx.drawImage(logo.img, x + 8, y + 8, size - 16, size - 16);
              return;
            }catch(_e){}
          }
          roundRect(x, y, size, size, 16, '#edf3f9', '#d7e1ec', 1.5);
          drawText(company || 'חברה', x + size - 10, y + 24, '700 12px Arial', colors.navy, 'right');
        };
        const drawPolicySection = (title, entries, startY) => {
          let y = startY;
          drawSectionLabel(title, y);
          y += 56;
          if(!entries.length){
            roundRect(margin, y, innerWidth, 56, 24, colors.white, '#d7e1ec', 2);
            drawText('לא נמצאו נתונים להצגה', pageWidth - margin - 24, y + 36, '600 16px Arial', colors.muted, 'right');
            return y + 70;
          }
          entries.forEach((entry) => {
            const cardH = measurePolicyCard(entry);
            roundRect(margin, y, innerWidth, cardH, 24, colors.white, '#d7e1ec', 2);
            drawCompanyLogo(entry.company, margin + 16, y + 14, 72);
            drawText(entry.title, pageWidth - margin - 22, y + 34, '800 18px Arial', colors.navy, 'right');
            drawText(entry.subtitle, pageWidth - margin - 22, y + 58, '600 13px Arial', colors.muted, 'right');
            let lineY = y + 86;
            const maxTextWidth = innerWidth - 130;
            (entry.rows || []).forEach(([label, value]) => {
              const lines = wrapRtl(`${label}: ${value}`, maxTextWidth, '600 14px Arial');
              lines.forEach((line) => {
                drawText(line, pageWidth - margin - 22, lineY, '600 14px Arial', colors.text, 'right');
                lineY += 18;
              });
            });
            y += cardH + 14;
        ...
        const drawHeader = () => {
          roundRect(margin, margin, innerWidth, 156, 34, colors.navy, null, 0);
          if(brandLogo){
            try{ ctx.drawImage(brandLogo, margin + 30, margin + 34, 150, 88); }catch(_e){}
          }
          drawText('דוח תפעולי', pageWidth - margin - 34, margin + 62, '800 40px Arial', '#ffffff', 'right');
          drawText('פתרון פיננסי ללקוח', pageWidth - margin - 34, margin + 102, '600 20px Arial', '#e5eefb', 'right');
          drawText(`${pack.meta.exportedAt} · ${pack.meta.agentName}`, pageWidth - margin - 34, margin + 132, '600 16px Arial', '#d6e2f4', 'right');
        };
        const drawFooter = (pageNumber) => {
          ctx.fillStyle = '#ccd7e6';
          ctx.fillRect(margin, pageHeight - 54, innerWidth, 3);
          drawText('הופק ישירות מתוך המערכת', margin + 190, pageHeight - 20, '600 12px Arial', colors.muted, 'right');
          drawText(`עמוד ${pageNumber}`, pageWidth - margin - 8, pageHeight - 20, '600 12px Arial', colors.muted, 'right');
        };
        ...
        let y = margin + 184;
        const summaryRows = [
          ['שם לקוח', pack.meta.customerName],
          ['תעודת זהות', pack.meta.customerId],
          ['נציג מטפל', pack.meta.agentName],
          ['מועד הפקה', pack.meta.exportedAt]
        ];
        y = drawInfoGrid('פרטי לקוח', [...summaryRows, ...pack.customerRows], y) + 8;
        y = drawInfoGrid('אמצעי תשלום', pack.paymentRows, y) + 8;
        y = drawPolicySection('פוליסות קיימות', pack.existingPolicies, y) + 6;
        y = drawPolicySection('פוליסות חדשות', pack.newPolicies, y);
        drawFooter(1);
        savePage();
        ...
        drawHeader();
        y = margin + 184;
        drawSectionLabel('הצהרת בריאות', y);
        y += 56;
        const healthRows = pack.healthItems.length ? pack.healthItems : [{ question:'לא נמצאו תשובות הצהרת בריאות להצגה', answer:'—', details:'' }];
        healthRows.forEach((item) => {
          const detailText = safeTrim(item.details);
          const detailLines = detailText ? wrapRtl(detailText, innerWidth - 210, '600 14px Arial') : [];
          const cardH = 56 + (detailLines.length * 18);
          roundRect(margin, y, innerWidth, cardH, 24, colors.white, '#d7e1ec', 2);
          drawText(item.question, pageWidth - margin - 24, y + 30, '700 16px Arial', colors.text, 'right');
          drawText(`תשובה: ${item.answer || '—'}`, margin + 150, y + 30, '800 15px Arial', item.answer === 'כן' ? '#b42318' : '#157347', 'right');
          let detailY = y + 52;
          detailLines.forEach((line) => {
            drawText(line, pageWidth - margin - 24, detailY, '600 14px Arial', colors.muted, 'right');
            detailY += 18;
          });
          y += cardH + 12;
        });
        ...
        pdf.save(this.getOperationalPdfFileName(pack.payload));
      }catch(err){
