...
const SystemRepairUI = {
  ...
  const LeadShellUI = {
    ...
    init(){
      ...
      this.els.btnCloseFoot = document.getElementById("btnLeadShellCloseFoot");
      ...
      on(this.els.btnCloseFoot, "click", (ev) => { ev.preventDefault(); this.close(); });
      on(this.els.modal, "click", (ev) => {
    ...
    open(){
      ...
      try{ document.querySelectorAll('.modal.is-open').forEach((el) => { if(el !== this.els.modal) el.classList.remove('is-open'); }); }catch(_e){}
      if(!this.els.modal) return;
