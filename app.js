...
(() => {
  ...
  // "ריסק" / "ריסק משכנתא" require סכום ביטוח
  const isSumProduct = (prod) => (prod === 'ריסק' || prod === 'ריסק משכנתא');
  // Lien (שיעבוד) only relevant for those products
  const isLienProduct = (prod) => (prod === 'ריסק' || prod === 'ריסק משכנתא');
...
function isVisible(el){
  ...
  function validateNewPoliciesStep(){
    ...
    const missingSum = p.newPolicies.find((x) => x && (x.product==='ריסק' || x.product==='ריסק משכנתא') && !(parseFloat(x.sumInsured||0) > 0));
    if(missingSum){
      ...
      const btn = document.getElementById('addNewBtn');
      addReq(btn);
      return gateFail('חסר מידע','בריסק/ריסק משכנתא חובה למלא סכום ביטוח', btn || wizardMain);
    }
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3" style="margin-top:10px">
        ${selectField('סוג ביטוח', 'old_product', ['בריאות','מחלות קשות','מחלות סרטן','תאונות אישיות','ריסק','ריסק משכנתא'])}
          ...
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => {
              ...
              <tr>
                ...
                <td>${(x.product==='ריסק' || x.product==='ריסק משכנתא') ? money(x.sumInsured||0) : '—'}</td>
    ...
    $('#addOldBtn').addEventListener('click', () => {
      ...
      if(isSumProduct(product) && sumInsured <= 0){ toast('שגיאה', 'חובה למלא סכום ביטוח לריסק/ריסק משכנתא'); oldSumInput && oldSumInput.focus && oldSumInput.focus(); return; }
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3">
        ...
        ${selectField('מוצר', 'new_product', ['בריאות','מחלות קשות','מחלות סרטן','תאונות אישיות','ריסק','ריסק משכנתא'])}
          ...
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => `
              <tr>
                ...
                <td>${(x.product==='ריסק' || x.product==='ריסק משכנתא') ? money(x.sumInsured||0) : ((x.product==='מחלות קשות' || x.product==='מחלות סרטן') ? money(x.compensation||0) : '—')}</td>
                <td>
                ...
                <td>${(x.product==='ריסק' || x.product==='ריסק משכנתא') ? `<button class="btn btnSoft" type="button" data-lien-new="${idx}">שיעבוד${x.lien ? ' (✓)' : ''}</button>` : '—'}</td>
    ...
    $('#addNewBtn').addEventListener('click', () => {
      ...
      if(isLienProduct(product) && sumInsured <= 0){ toast('שגיאה', 'חובה למלא סכום ביטוח לריסק/ריסק משכנתא'); return; }
      if(isCompProduct(product) && compensation <= 0){ toast('שגיאה', 'חובה למלא סכום פיצוי למחלות קשות/סרטן'); return; }
  ...
  function openPrintView(proposal){
    ...
    try {
      ...
      const oldRows = oldPolicies.map(x => {
        ...
        const sumCell = (prod==='ריסק' || prod==='ריסק משכנתא') ? money(nnum(x.sumInsured)) : '—';
...
function openPolicyLienModal(current, onSave){
  ...
  const esc = (s)=>escapeHtml(String(s||''));
  overlay.innerHTML = `
    <div style="width:min(720px,95vw);background:rgba(255,255,255,0.98);border-radius:18px;box-shadow:0 22px 70px rgba(0,0,0,0.28);padding:14px;direction:rtl;font-family:inherit">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:10px">
        <div>
          <div style="font-weight:900">פרטי המוטב הבלתי חוזר</div>
          <div style="font-size:12px;color:rgba(18,19,25,.55);margin-top:2px">מלא את פרטי הבנק המשעבד</div>
        </div>
        <button class="btn btnSoft" type="button" data-x>סגור</button>
      </div>
  ...
  overlay.querySelector('[data-save]').addEventListener('click', () => {
    ...
    const hasAny = Object.entries(out).some(([k,v]) => k !== 'kind' && String(v||'').trim());
    close();
