  ...
  function renderSummaryStep(){
    ...
    if(markDoneBtnEl) markDoneBtnEl.addEventListener('click', () => {
      ...
      if(GOOGLE_SCRIPT_URL){
        Promise.all([
          syncUpCustomer(customer),
          syncUpProposal(p)
        ]).then(()=>toast('סנכרון', 'הלקוח וההצעה נשמרו לשיטס'))
          .catch(()=>toast('סנכרון', 'שגיאה בסנכרון לשיטס'));
      }
...
function debounce(fn, wait){
  ...
  async function syncDown(){
    ...
    const cRows = (c && c.ok) ? (c.rows || c.customers) : null;
    if(Array.isArray(cRows)){
      state.customers = cRows;
    }
    const p = await apiCall('listProposals', {});
    const pRows = (p && p.ok) ? (p.rows || p.proposals) : null;
    if(Array.isArray(pRows)){
      state.proposals = pRows;
    }
