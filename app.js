...
(() => {
  ...
  const BUILD = "20260409-vUsersGateSettingsOwnerSupabaseFix1";
...
// nav
      ...
      document.addEventListener("click", (ev) => {
        const trigger = ev.target?.closest?.('#navUsers,[data-view="users"]');
        if(!trigger) return;
        if(trigger.id === 'navUsers' || safeTrim(trigger.getAttribute?.('data-view')) === 'users'){
          ev.preventDefault();
          ev.stopPropagation();
          if(!Auth.canManageUsers()) return;
          this.openUsersManagement();
...
UsersGateUI.init();
    ...
    openUsersManagement(){
      ...
      UsersGateUI.open(() => this.goView("users"));
    },
    ...
    goView(view){
      ...
      if(safe === "users" && !UsersGateUI.isAuthorized()){ this.openUsersManagement(); return; }
  ...
  const UsersGateUI = {
    ...
    open(afterSuccess = null){
      ...
      if(!this.els?.wrap){
        const fallbackCode = window.prompt('יש להקיש סיסמת מורשה');
        if(fallbackCode == null) return;
        verifyUsersManagementPin(fallbackCode).then((result) => {
          if(!result?.ok){
            window.alert(result?.error || 'קוד מורשה שגוי');
            return;
          }
          this.rememberAuthorized();
          if(typeof this._afterSuccess === 'function') this._afterSuccess();
        }).catch(() => {
          window.alert('לא הצלחתי לאמת את הקוד המורשה');
        });
        return;
      }
