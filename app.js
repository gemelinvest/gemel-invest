...
const SystemRepairUI = {
  ...
  const LeadGeneratorUI = {
    ...
    progressTimer: null,
    running: false,
    usedKey: "GEMEL_LEAD_PHONES_V1",
    statuses: ["מאתר התאמה...", "בונה מספר ליד...", "מאמת תקינות...", "מבצע הצלבה מול המאגר...", "מכין תוצאה..."] ,
    prefixes: ["050","051","052","053","054","055","056","057","058","059"],

    init(){
      this.els.btnOpen = $("#btnCreateLead");
      this.els.wrap = $("#lcLeadModal");
      if(!this.els.btnOpen || !this.els.wrap) return;
      this.els.close = $("#lcLeadClose");
      this.els.start = $("#lcLeadStart");
      this.els.copy = $("#lcLeadCopy");
      this.els.again = $("#lcLeadGenerateAgain");
      this.els.loading = $("#lcLeadLoading");
      this.els.idle = $("#lcLeadIdle");
      this.els.result = $("#lcLeadResult");
      this.els.phone = $("#lcLeadPhone");
      this.els.digits = $("#lcLeadDigits");
      this.els.status = $("#lcLeadStatus");
      this.els.progress = $("#lcLeadProgressBar");
      this.els.panel = $("#lcLeadPanel");

      on(this.els.btnOpen, "click", (e) => {
        e.preventDefault();
        e.stopPropagation();
        if(!Auth.current) return;
        try { Wizard.close(); } catch(_e) {}
      ...
      on(this.els.start, "click", () => this.run());
      on(this.els.again, "click", () => this.run());
      on(this.els.copy, "click", async () => {
        const phone = safeTrim(this.els.phone?.textContent);
        if(!phone) return;
        try {
          await navigator.clipboard.writeText(phone);
          const txt = this.els.copy.textContent;
          this.els.copy.textContent = "הועתק";
          setTimeout(() => { this.els.copy && (this.els.copy.textContent = txt || "העתק מספר"); }, 1200);
        } catch(_e) {
          try {
    ...
    open(){
      ...
      this.els.wrap.classList.add("is-open");
      this.els.wrap.setAttribute("aria-hidden", "false");
      document.body.classList.add("lcLeadModalOpen");
    },
    ...
    close(){
      ...
      this.els.wrap.classList.remove("is-open");
      this.els.wrap.setAttribute("aria-hidden", "true");
      document.body.classList.remove("lcLeadModalOpen");
    },

    resetVisuals(){
      this.stopTimers();
      this.running = false;
      this.els.idle && (this.els.idle.hidden = false);
      this.els.loading && (this.els.loading.hidden = true);
      this.els.result && (this.els.result.hidden = true);
      this.els.copy && (this.els.copy.hidden = true);
      this.els.again && (this.els.again.hidden = true);
      this.els.start && (this.els.start.hidden = false, this.els.start.disabled = false, this.els.start.textContent = "התחל חיפוש");
      this.els.status && (this.els.status.textContent = this.statuses[0]);
      this.els.digits && (this.els.digits.textContent = "05X-XXX-XXXX");
      this.els.progress && (this.els.progress.style.width = "0%");
      this.els.panel && (this.els.panel.className = "lcLeadPanel lcLeadPanel--idle");
    },
    ...
    stopTimers(){
      ...
      this.timer = this.tickTimer = this.progressTimer = null;
    },

    run(){
      if(this.running) return;
      this.running = true;
      this.els.idle && (this.els.idle.hidden = true);
      this.els.result && (this.els.result.hidden = true);
      this.els.loading && (this.els.loading.hidden = false);
      this.els.copy && (this.els.copy.hidden = true);
      this.els.again && (this.els.again.hidden = true);
      this.els.start && (this.els.start.disabled = true, this.els.start.hidden = true);
      this.els.panel && (this.els.panel.className = "lcLeadPanel lcLeadPanel--loading");

      let progress = 0;
      let statusIdx = 0;
      this.els.status && (this.els.status.textContent = this.statuses[0]);
      this.tickTimer = setInterval(() => {
        this.els.digits && (this.els.digits.textContent = this.fakePhone());
        statusIdx = (statusIdx + 1) % this.statuses.length;
        this.els.status && (this.els.status.textContent = this.statuses[statusIdx]);
      }, 1400);
      this.progressTimer = setInterval(() => {
        progress = Math.min(96, progress + 2);
        this.els.progress && (this.els.progress.style.width = progress + "%");
      }, 160);
    ...
    finish(phone){
      ...
      this.running = false;
      this.els.loading && (this.els.loading.hidden = true);
      this.els.result && (this.els.result.hidden = false);
      this.els.phone && (this.els.phone.textContent = phone);
      this.els.copy && (this.els.copy.hidden = false, this.els.copy.textContent = "העתק מספר");
      this.els.again && (this.els.again.hidden = false);
      this.els.start && (this.els.start.hidden = true, this.els.start.disabled = false);
      this.els.progress && (this.els.progress.style.width = "100%");
      this.els.panel && (this.els.panel.className = "lcLeadPanel lcLeadPanel--result");
    },

    fakePhone(){
      const p = this.prefixes[Math.floor(Math.random() * this.prefixes.length)] || "050";
    ...
    getUsed(){
      try {
        const raw = localStorage.getItem(this.usedKey);
        const arr = JSON.parse(raw || "[]");
        return new Set(Array.isArray(arr) ? arr.map(v => safeTrim(v)).filter(Boolean) : []);
      } catch(_e){
        return new Set();
      }
    },
    ...
    saveUsed(setObj){
      try { localStorage.setItem(this.usedKey, JSON.stringify(Array.from(setObj))); } catch(_e) {}
    },
    ...
    generateUniquePhone(){
      ...
      while(tries < 5000){
        ...
        if(/(\d){6,}/.test(rest) || /1234|2345|3456|4567|5678|6789/.test(rest)) { tries++; continue; }
        if(!used.has(phone)){
          used.add(phone);
          this.saveUsed(used);
          return phone;
      ...
      const fallback = "05" + String(Date.now()).slice(-8);
      used.add(fallback);
      this.saveUsed(used);
      return fallback;
    }
