...
(() => {
  ...
  const BUILD = "20260320-ops3sec-0002";
...
// nav
      ...
      on(this.els.btnMyProcessesRefresh, "click", () => {
        ProcessesUI.refreshFromServer({ reason:"manual-button", force:true }).catch?.(() => ProcessesUI.render());
      });
...
this.applyRoleUI();
    ...
    goView(view){
      ...
      if (safe === "myProcesses") {
        ProcessesUI.render();
        ProcessesUI.refreshFromServer({ reason:"view-enter", force:true }).catch?.(() => {});
      }
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const ProcessesUI = {
    ...
    hasManualInteractionGuard(){
      if(this.isUserInteracting()) return true;
      const activeElement = document.activeElement;
      const insideMyProcesses = activeElement?.closest?.('#view-myProcesses');
      return !!insideMyProcesses && (activeElement?.tagName === 'BUTTON');
    },
    ...
    shouldAutoRefresh(){
      ...
      if(this.hasManualInteractionGuard()) return false;
      return true;
    },
