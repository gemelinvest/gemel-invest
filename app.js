...
(() => {
  ...
  const BUILD = "20260329-mfa-login-split-v10";
...
init(){
    ...
    renderSteps(){
      ...
      this.els.steps.innerHTML = steps.map(s => {
        const cls = [
          "lcStep",
      ...
      if(this.els.fill){
        ...
        const pct = steps.length === 1 ? 100 : Math.round(((this.step-1) / denom) * 100);
...
// open health covers drawer (Health only)
    ...
    normalizeHarPolicyType(row){
      ...
      const source = [section, main, sub, safeTrim(row?.productType)].join(" | ");
      ...
      if(/בריאות|השתלות|תרופות|ניתוחים|ייעוץ|בדיקות|שב"ן|שב״ן/i.test(source)) return "בריאות";
      return "בריאות";
    },
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      let currentSection = "";
      const allowedSections = new Set(["תחום - בריאות ותאונות אישיות", "תחום - חיים ואבדן כושר עבודה"]);
      const extracted = [];
      rows.slice(headerIdx + 1).forEach((row) => {
        if(!Array.isArray(row)) return;
        const mainCell = safeTrim(row[idx.main]);
        ...
        if(!allowedSections.has(currentSection)) return;
        const policyNumber = safeTrim(row[idx.policyNumber]);
        const company = this.normalizeHarCompanyName(row[idx.company]);
        if(!policyNumber || !company) return;
        const premiumRaw = Number(String(row[idx.premium] ?? "").replace(/[^\d.-]/g, ""));
        const premium = Number.isFinite(premiumRaw) ? premiumRaw : 0;
        const premiumType = safeTrim(row[idx.premiumType]);
        const monthlyPremium = /שנתי/i.test(premiumType) ? (premium / 12) : premium;
        const annualPremium = /חודש/i.test(premiumType) ? (premium * 12) : premium;
        extracted.push({
          section: currentSection,
          idNumber: safeTrim(row[idx.idNumber]),
          main: mainCell,
          sub: safeTrim(row[idx.sub]),
          productType: safeTrim(row[idx.productType]),
          company,
          period: safeTrim(row[idx.period]),
          extra: safeTrim(row[idx.extra]),
          premiumType,
          policyNumber,
          monthlyPremium,
          annualPremium,
          normalizedType: this.normalizeHarPolicyType({
            section: currentSection,
            main: mainCell,
            sub: safeTrim(row[idx.sub]),
            productType: safeTrim(row[idx.productType])
          }),
          includedLabel: this.normalizeHarIncludedLabel({
            main: mainCell,
            sub: safeTrim(row[idx.sub]),
            productType: safeTrim(row[idx.productType])
          })
      ...
      extracted.forEach((row) => {
        const key = safeTrim(row.policyNumber);
        if(!grouped.has(key)){
          grouped.set(key, {
            policyNumber: key,
            company: row.company,
      ...
      const policies = Array.from(grouped.values()).map((bucket) => {
        ...
        return {
          ...
          sourceTypes: typeList.sort((a,b) => priority.indexOf(a) - priority.indexOf(b))
        };
      }).sort((a,b) => Number(a.policyNumber || 0) - Number(b.policyNumber || 0));
    ...
    async handleHarBituachFile(ins, file){
      ...
      try {
        ...
        const approved = await showHarBituachImportModal({
          ...
          note: 'המערכת תוריד את הנתונים, תבצע עיבוד קצר ואז תציג את הפוליסות בשורות קריאה בלבד. ניתן להוסיף גם פוליסות ידנית.',
          confirmText: 'אישור',
          cancelText: 'ביטול'
        });
        if(!approved){
        ...
        this.setHarImportState(ins, {
          ...
          fileName: file.name || "",
          importedAt: nowISO(),
          message: `יובאו ${policies.length} פוליסות מתוך הקובץ והן מוצגות כעת בשורות קריאה בלבד.`
        });
        this.render();
      } catch(err){
        this.setHarImportState(ins, {
            ...
            <tr class="lcPolImportedWrapRow">
              <td colspan="7" class="lcPolImportedWrapCell">
                <div class="lcPolImportedRow">
                  ...
                  <div class="lcPolImportedRow__segment lcPolImportedRow__segment--readonly">
                    <span class="lcPolImportedRow__readonlyTitle">יובא מהר הביטוח</span>
                    <span class="lcPolImportedRow__readonlySub">שורת נתונים לקריאה בלבד</span>
                  </div>

                  <div class="lcPolImportedRow__segment lcPolImportedRow__segment--actions">
                    <span class="lcPolImportedRow__importTag">קריאה בלבד</span>
                  </div>
                </div>
              </td>
            </tr>
        ...
        return `
          <div class="lcWSection lcCancelCard">
            ...
            ${showAnnex ? `
              ...
              <div class="lcWTitle" style="margin-bottom:8px">כיסויים לביטול חלקי (מתוך הפוליסה הקיימת)</div>
              ${hasPolicyCovers ? `
                <div class="help" style="margin-bottom:10px">סמן רק את הכיסויים הרלוונטיים לביטול מתוך הפוליסה הקיימת של המבוטח.</div>
                <div class="lcWGrid">
                  ${policyCovers.map(a => `
                    <label class="row" style="gap:8px">
              ...
              ` : `
                <div class="muted small">לא הוגדרו כיסויים בפוליסה הקיימת הזו. חזור לשלב הפוליסות הקיימות והוסף את הכיסויים של הפוליסה, ואז תוכל לסמן מה לבטל חלקית.</div>
              `}
            `:""}
          </div>
        `;
...
const SystemRepairUI = {
  ...
  const SupabaseMFA = {
    ...
    extractQrMarkup(data){
      ...
      if(uri){
        html += `<details class="lcLogin__mfaManualDetails"><summary>קוד ידני למקרה שהסריקה לא עובדת</summary><div class="help">אם לא ניתן לסרוק, אפשר להשתמש גם בקוד הידני הבא:</div><code>${escapeHtml(uri)}</code></details>`;
      } else if(secret){
        html += `<details class="lcLogin__mfaManualDetails"><summary>קוד ידני למקרה שהסריקה לא עובדת</summary><div class="help">אם לא ניתן לסרוק, אפשר להשתמש גם ב-secret הבא:</div><code>${escapeHtml(secret)}</code></details>`;
      }
