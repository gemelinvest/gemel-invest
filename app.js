...
const toggleProductFields = () => {
  ...
  if(!showRisk){
    ...
    // Guard: browsers can persist checkbox state. If it is checked but there's no draft yet,
    // reset it so clicking 'הוסף ביטוח' won't unexpectedly open the lien modal.
    if(el.oldPolicyHasLien && el.oldPolicyHasLien.checked && !UI._oldPolicyLienDraft){
      try{ el.oldPolicyHasLien.checked = false; }catch(_){}
    }
...
const lienWrapVisible = !!(el.oldPolicyLienWrap && String(el.oldPolicyLienWrap.style.display) !== "none");
const hasLien = lienWrapVisible && !!(el.oldPolicyHasLien && el.oldPolicyHasLien.checked);
// Guard: if lien UI is hidden, ignore any persisted checkbox state
if(!lienWrapVisible && el.oldPolicyHasLien) { try{ el.oldPolicyHasLien.checked = false; }catch(_){} }
