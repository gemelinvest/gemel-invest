...
(() => {
  ...
  function formatLoginStamp(value){
  ...
  function syncLastLoginUi(lastLoginLabel){
    const label = safeTrim(lastLoginLabel) || "—";
  ...
  const defaultState = () => ({
    meta: {
      updatedAt: null,
      adminAuth: { username: "מנהל מערכת", pin: "1234", active: true, lastLoginAt: "" }
    },
    agents: [
      { id:"a_0", name:"יובל מנדלסון", username:"יובל מנדלסון", pin:"0000", active:true, lastLoginAt:"" }
    ],
    customers: [],
    proposals: []
  });
  ...
  function normalizeState(s){
    ...
    out.meta.adminAuth = {
      ...
      lastLoginAt: safeTrim(rawAdmin.lastLoginAt)
    };
    ...
    out.agents = (out.agents || []).map((a, idx) => {
      ...
      return {
        ...
        lastLoginAt: safeTrim(a?.lastLoginAt)
      };
    }).filter(a => a.name);
  ...
  const Storage = {
    ...
    buildMetaRow(state){
      return {
        ...
        payload: {
          adminAuth: {
            ...(defaultState().meta.adminAuth || {}),
            ...((state?.meta?.adminAuth && typeof state.meta.adminAuth === "object") ? state.meta.adminAuth : {})
          },
          updatedAt: nowISO()
        },
        updated_at: nowISO()
    ...
    buildAgentRows(state){
      return (state?.agents || []).map((a, idx) => ({
        ...
        last_login_at: safeTrim(a?.lastLoginAt),
    ...
    mapAgentRow(row, idx){
      return {
        ...
        lastLoginAt: safeTrim(row?.last_login_at),
  ...
  const Auth = {
    ...
    _touchUserLastLogin(kind, entity){
      const previous = safeTrim(entity?.lastLoginAt);
      const next = nowISO();
      if(kind === "admin"){
        State.data.meta = State.data.meta || {};
        State.data.meta.adminAuth = {
          ...(State.data.meta.adminAuth || {}),
          lastLoginAt: next
        };
      } else if(entity) {
        entity.lastLoginAt = next;
      }
      return {
        previous,
        previousLabel: previous ? formatLoginStamp(previous) : "כניסה ראשונה למערכת",
        current: next
      };
    },

    async _persistLastLoginSilently(){
      try{
        if(!State?.data) return;
        State.data.meta = State.data.meta || {};
        State.data.meta.updatedAt = nowISO();
        await App.persist("התחברות עודכנה");
      }catch(err){
        console.warn("LAST_LOGIN_PERSIST_FAILED", err);
      }
    },
    ...
    async _submit(){
      ...
      if (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin)) {
        const loginMeta = this._touchUserLastLogin("admin", adminAuth);
        this.current = {
          name: safeTrim(adminAuth.username) || defAdmin.username,
          role:"admin",
          lastLoginAt: loginMeta.previous,
          lastLoginLabel: loginMeta.previousLabel
        };
        try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
        await this._persistLastLoginSilently();
        await App.reloadSessionState();
        this.unlock();
        UI.applyRoleUI();
        UI.renderAuthPill();
        syncLastLoginUi(this.current.lastLoginLabel);
        await WelcomeLoader.play(this.current.name, 4800, this.current.lastLoginLabel);
      ...
      const loginMeta = this._touchUserLastLogin("agent", matched);
      this.current = {
        name: matched.name,
        role: (matched.role === "manager" ? "manager" : matched.role === "ops" ? "ops" : "agent"),
        lastLoginAt: loginMeta.previous,
        lastLoginLabel: loginMeta.previousLabel
      };
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
  ...
  const WelcomeLoader = {
      ...
      root.innerHTML = `
        ...
        <div class="lcWelcomeLoader__panel" role="status" aria-live="polite" aria-atomic="true">
          <div class="lcWelcomeLoader__shell">
            ...
            <div class="lcWelcomeLoader__brandRow" aria-hidden="true">
              <span class="lcWelcomeLoader__brandLine"></span>
            </div>
            ...
            <div class="lcWelcomeLoader__lastLogin" id="lcWelcomeLastLogin"></div>
    ...
    async play(name, ms=4000, lastLoginLabel=""){
      this.open(name, lastLoginLabel);
...
const SystemRepairUI = {
  ...
  const ChatUI = {
    ...
    closeWindow(forceKeepFab=false){
      this.els.window?.classList.add("is-hidden");
      if(forceKeepFab || document.body.classList.contains('view-dashboard-active')) this.els.fab?.classList.remove('is-hidden');
      this.closeEmojiPanel();
...
// login footer mirrors the authenticated user's previous login only
(function(){
  syncLastLoginUi("—");
})();
