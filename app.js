...
(() => {
  ...
  const BUILD = "20260406-live-finish-sync-v51";
  ...
  function normalizeCustomerRecord(c, idx=0){
    ...
    const wizardCompleted = c?.wizardCompleted === true || payload?.wizardCompleted === true || payload?.isCompleted === true || payload?.flowCompleted === true;
    const completedAt = safeTrim(c?.completedAt) || safeTrim(payload?.completedAt) || safeTrim(payload?.finishedAt) || "";
    const agentId = safeTrim(c?.agentId) || safeTrim(payload?.agentId) || safeTrim(payload?.agentKey) || safeTrim(payload?.createdByKey) || "";
    const monthlyPremiumAfterDiscount = Number(c?.monthlyPremiumAfterDiscount ?? payload?.monthlyPremiumAfterDiscount ?? 0) || 0;
    return {
  ...
  const Storage = {
    ...
    session(){
      try{
        ...
        return { id: safeTrim(s?.id) || name, username: safeTrim(s?.username) || name, name, role };
      }catch(_e){
        return { name:"", role:"" };
      }
    },
    ...
    buildCustomerRows(state){
      return (state?.customers || []).map((c, idx) => ({
        ...
        agent_id: safeTrim(c?.agentId || c?.payload?.agentId || c?.payload?.agentKey || c?.payload?.createdByKey),
        ...
        wizard_completed: c?.wizardCompleted === true || c?.payload?.wizardCompleted === true,
        completed_at: safeTrim(c?.completedAt || c?.payload?.completedAt) || null,
        monthly_premium_after_discount: Number(c?.monthlyPremiumAfterDiscount ?? c?.payload?.monthlyPremiumAfterDiscount ?? 0) || 0,
    ...
    mapCustomerRow(row, idx){
      return normalizeCustomerRecord({
        ...
        wizardCompleted: row?.wizard_completed,
        completedAt: row?.completed_at,
        agentId: row?.agent_id,
        monthlyPremiumAfterDiscount: row?.monthly_premium_after_discount,
  ...
  const Auth = {
    ...
    _saveSession(cur){
      try {
        localStorage.setItem(LS_SESSION_KEY, JSON.stringify({ id: safeTrim(cur?.id) || safeTrim(cur?.username) || safeTrim(cur?.name), username: safeTrim(cur?.username) || safeTrim(cur?.name), name: cur.name, role: cur.role }));
      } catch(_) {}
    },
    ...
    async _submit(){
      ...
      if (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin)) {
        this.current = { id: safeTrim(adminAuth.username) || defAdmin.username, username: safeTrim(adminAuth.username) || defAdmin.username, name: safeTrim(adminAuth.username) || defAdmin.username, role:"admin" };
        try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      ...
      this.current = { id: safeTrim(matched.id) || safeTrim(matched.username) || safeTrim(matched.name), username: safeTrim(matched.username) || safeTrim(matched.name), name: matched.name, role: (matched.role === "manager" ? "manager" : matched.role === "ops" ? "ops" : "agent") };
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      await App.reloadSessionState();
      this.unlock();
      try { InactivityGuard.start(); } catch(_e) {}
...
this.applyRoleUI();
  ...
  const DashboardUI = {
    ...
    buildMetrics(){
      const customers = this.getVisibleCustomers().filter((rec) => this.isWithinCurrentMonth(this.resolveCustomerMonthStamp(rec)));
      const metrics = {
        proposalsSubmitted: 0,
        monthlyPremiumAfterDiscount: 0,
        salesCount: 0,
      ...
      customers.forEach((rec) => {
        ...
        metrics.proposalsSubmitted += policies.length;
        metrics.salesCount += newPolicies.length;
        metrics.monthlyPremiumAfterDiscount += newPolicies.reduce((sum, p) => sum + (Number(p?.premiumAfterDiscountValue) || 0), 0);
        metrics.agentAppointmentPremium += existingPolicies.reduce((sum, p) => {
          const status = this.normalizeExistingStatus(p?.details?.['סטטוס'] || p?.status || p?.existingStatus || '');
          if(status !== 'agent_appoint') return sum;
          return sum + CustomersUI.asMoneyNumber(p?.premiumValue);
        }, 0);
      });

      metrics.monthlyPremiumAfterDiscount = Math.round(metrics.monthlyPremiumAfterDiscount * 100) / 100;
      metrics.agentAppointmentPremium = Math.round(metrics.agentAppointmentPremium * 100) / 100;
      return metrics;
    },
    ...
    renderEmpty(){
      if(!this.els.root) return;
      const monthLabel = this.formatMonthLabel();
      const cards = [
        {
          kicker: 'החודש',
          label: 'כמות הצעות שהוגשו',
          value: '0',
          icon: premiumCustomerIcon('document')
        },
        {
          kicker: 'פוליסות חדשות בלבד',
          label: 'סה״כ פרמיות לאחר הנחה',
          value: this.formatMoney(0),
          icon: premiumCustomerIcon('briefcase')
        },
        {
          kicker: 'פוליסות קיימות',
          label: 'סה״כ מינויי סוכן',
          value: this.formatMoney(0),
          icon: premiumCustomerIcon('building')
        }
      ];
      this.els.root.innerHTML = `
        <div class="grid grid--kpis dashboardKpiGrid dashboardKpiGrid--compact">
          ${cards.map((card, idx) => `
            <article class="card dashboardKpiCard" style="animation-delay:${idx * 70}ms">
              <div class="dashboardKpiCard__title">${escapeHtml(card.label)}</div>
              <div class="dashboardKpiCard__watermark" aria-hidden="true">${card.icon}</div>
              <div class="dashboardKpiCard__value">${escapeHtml(card.value)}</div>
            </article>`).join('')}
        </div>`;
    },
    ...
    render(){
      ...
      const monthLabel = this.formatMonthLabel();
      const hasData = metrics.proposalsSubmitted || metrics.monthlyPremiumAfterDiscount || metrics.salesCount || metrics.agentAppointmentPremium;
      if(!hasData && !metrics.customersCount){
        this.renderEmpty();
        return;
      }
      ...
      const cards = [
        {
          kicker: 'החודש',
          label: 'כמות הצעות שהוגשו',
          value: String(metrics.proposalsSubmitted || 0),
          icon: premiumCustomerIcon('document')
        },
        {
          kicker: 'פוליסות חדשות בלבד',
          label: 'סה״כ פרמיות לאחר הנחה',
          value: this.formatMoney(metrics.monthlyPremiumAfterDiscount),
          icon: premiumCustomerIcon('briefcase')
        },
        {
          kicker: 'פוליסות קיימות',
...
// ---------- Step 6 ----------
    ...
    async saveCompletedCustomer(){
      ...
      const currentAgentId = safeTrim(Auth?.current?.id) || safeTrim(Auth?.current?.username) || safeTrim(Auth?.current?.name);
      const monthlyPremiumAfterDiscount = Math.round(this.sumPremiumAfterDiscount(Array.isArray(payload?.newPolicies) ? payload.newPolicies : []) * 100) / 100;
      payload.wizardCompleted = true;
      payload.completedAt = completedAt;
      payload.completedBy = safeTrim(Auth?.current?.name);
      payload.agentId = currentAgentId;
      payload.monthlyPremiumAfterDiscount = monthlyPremiumAfterDiscount;
      const primary = payload?.primary || {};
