...
(() => {
  ...
  const BUILD = "20260324-customer-save-premium-no-pdf";
...
txt.textContent = "";
  ...
  const CustomerEditUI = {
    ...
    openSaveSuccess(message){
      ...
      if(this.els.successText) this.els.successText.textContent = safeTrim(message) || 'תיק הלקוח עודכן בהצלחה וכל השינויים נשמרו במערכת.';
        ...
        <section class="customerEditSection">
          <div class="customerEditSection__head">
            <div>
              ...
              <div class="customerEditSection__sub">שומר את הנתונים בדיוק לתיק הלקוח ומעדכן את כל המידע במסך.</div>
            </div>
          </div>
          <div class="customerEditGrid">
    ...
    async save(){
      ...
      this.openSaveSuccess(result?.ok
        ? 'תיק הלקוח נשמר בהצלחה וכל השינויים עודכנו בתיק הלקוח.'
        : 'השינויים עודכנו במסך הפתוח כרגע, אבל שמירת השרת לא הושלמה במלואה. מומלץ לבדוק את הנתונים לפני סגירה.');
    }
  };
...
// ---------- Step 6 ----------
    ...
    async exportOperationalPdf(payloadOverride){
      try{
        console.info('Operational PDF engine removed pending rebuild.', payloadOverride || null);
        window.alert('מנוע ייצוא ה-PDF הוסר זמנית וייבנה מחדש בשלב הבא.');
      }catch(_e){}
      return false;
    },
