...
(() => {
  ...
  const LS_FAST_CACHE_KEY = "GEMEL_FAST_CACHE_V1";
  const LS_TABLE_CACHE_PREFIX = "GEMEL_TABLE_CACHE_V1_";
  ...
  function normalizeAgentRecord(a, idx=0){
    ...
    const name = safeTrim(a?.name) || safeTrim(a?.username) || "נציג";
    const username = safeTrim(a?.username) || safeTrim(a?.user) || name;
    const roleRaw = safeTrim(a?.role) || safeTrim(a?.type) || "";
    const role = (roleRaw === "manager" || roleRaw === "adminLite" || roleRaw === "admin")
      ? "manager"
      : ((roleRaw === "ops" || roleRaw === "operations" || roleRaw === "תפעול") ? "ops" : "agent");
    return {
      id: safeTrim(a?.id) || ("a_" + idx),
      name,
      username,
      pin: safeTrim(a?.pin) || safeTrim(a?.pass) || "0000",
      birthDate: safeTrim(a?.birthDate || a?.birth_date),
      monthlySalesTarget: Number(String(a?.monthlySalesTarget ?? a?.monthly_sales_target ?? '').replace(/[^\d.-]/g, '')) || 0,
      role,
      active: a?.active === false ? false : true,
      authEmail: safeTrim(a?.authEmail || a?.email),
      createdAt: safeTrim(a?.createdAt || a?.created_at),
      updatedAt: safeTrim(a?.updatedAt || a?.updated_at || a?.createdAt || a?.created_at) || nowISO()
    };
  }
  ...
  const Storage = {
    ...
    buildFastCachePayload(st){
      const state = normalizeState(st || defaultState());
      return {
        savedAt: nowISO(),
        state,
        tables: {
          meta: this.buildMetaRow(state)?.payload || {},
          agents: this.buildAgentRows(state),
          customers: this.buildCustomerRows(state),
          proposals: this.buildProposalRows(state)
        }
      };
    },
    saveFastCache(st){
      try {
        const payload = this.buildFastCachePayload(st);
        localStorage.setItem(this.getFastCacheKey(), JSON.stringify(payload));
        Object.entries(payload.tables || {}).forEach(([name, value]) => {
          const key = this.getTableCacheKey(name);
          localStorage.setItem(key, JSON.stringify({ savedAt: payload.savedAt, data: value }));
        });
      } catch(_) {}
    },
    loadFastCache(maxAgeMs = FAST_CACHE_MAX_AGE_MS){
      try {
        const raw = localStorage.getItem(this.getFastCacheKey());
        if(!raw) return null;
        const parsed = JSON.parse(raw);
        const savedAtMs = Date.parse(parsed?.savedAt || '');
        if(Number.isFinite(maxAgeMs) && maxAgeMs > 0 && Number.isFinite(savedAtMs)){
        ...
        return {
          ok:true,
          payload: state,
          at: safeTrim(parsed?.savedAt) || safeTrim(state?.meta?.updatedAt) || nowISO(),
          source: 'fast-cache'
        };
      } catch(_) {
        return null;
    ...
    loadFastCacheAnyAge(){
      return this.loadFastCache(Number.POSITIVE_INFINITY);
    },
    loadTableCache(tableName){
      try {
        const raw = localStorage.getItem(this.getTableCacheKey(tableName));
        if(!raw) return null;
        const parsed = JSON.parse(raw);
        return { savedAt: safeTrim(parsed?.savedAt), data: parsed?.data };
      } catch(_) {
        return null;
      }
    },
    buildStateFromTableCaches(){
      try {
        const metaCache = this.loadTableCache('meta');
        const agentsCache = this.loadTableCache('agents');
        const customersCache = this.loadTableCache('customers');
        const proposalsCache = this.loadTableCache('proposals');
        if(!metaCache && !agentsCache && !customersCache && !proposalsCache) return null;
        const mappedMeta = this.mapMeta(metaCache?.data || {});
        const tableProposals = Array.isArray(proposalsCache?.data) ? proposalsCache.data.map((row, idx) => this.mapProposalRow(row, idx)) : [];
        const payload = normalizeState({
          meta: mappedMeta,
          agents: mergeEntityArraysByRecency(Array.isArray(agentsCache?.data) ? agentsCache.data.map((row, idx) => this.mapAgentRow(row, idx)) : [], mappedMeta?.agentsShadow || []),
          customers: mergeEntityArraysByRecency(Array.isArray(customersCache?.data) ? customersCache.data.map((row, idx) => this.mapCustomerRow(row, idx)) : [], mappedMeta?.customersShadow || []),
          proposals: mergeEntityArraysByRecency(tableProposals, mappedMeta?.proposalsShadow || [])
        });
        return {
          ok:true,
          payload,
          at: safeTrim(metaCache?.savedAt || agentsCache?.savedAt || customersCache?.savedAt || proposalsCache?.savedAt) || safeTrim(payload?.meta?.updatedAt) || nowISO(),
          source:'table-cache'
        };
      } catch(_) {
        return null;
    ...
    saveBackup(st){
      ...
      try { this.saveFastCache(st); } catch(_) {}
    },
    ...
    async loadSheets(options = {}){
      const useCachedFallback = options.useCachedFallback !== false;
      try {
        const connection = await this.waitForConnection();
        if(!connection.ok){
          if(useCachedFallback){
            const cached = this.loadFastCacheAnyAge() || this.buildStateFromTableCaches() || (this.loadBackup() ? { ok:true, payload:this.loadBackup(), at: safeTrim(this.loadBackup()?.meta?.updatedAt) || nowISO(), source:'backup-cache' } : null);
            if(cached?.ok) return { ok:true, payload: cached.payload, at: cached.at, source: cached.source || 'cache', stale:true, warning: connection.error || 'השרת לא זמין כרגע' };
          }
        ...
        const mappedMeta = this.mapMeta(metaRes.ok ? (metaRes.data || {}) : {});
        const agentsRows = agentsRes.ok ? (agentsRes.data || []).map((row, idx) => this.mapAgentRow(row, idx)) : [];
        const customerRows = customersRes.ok ? (customersRes.data || []).map((row, idx) => this.mapCustomerRow(row, idx)) : [];
        const proposalRows = proposalsRes.ok ? (proposalsRes.data || []).map((row, idx) => this.mapProposalRow(row, idx)) : [];
        const payload = normalizeState({
          meta: mappedMeta,
          agents: mergeEntityArraysByRecency(agentsRows, mappedMeta?.agentsShadow || []),
          customers: mergeEntityArraysByRecency(customerRows, mappedMeta?.customersShadow || []),
          proposals: mergeEntityArraysByRecency(proposalRows, mappedMeta?.proposalsShadow || [])
        });
        const partialErrors = [
        ...
        try { this.saveFastCache(payload); } catch(_e) {}
        return { ok:true, payload, at: payload?.meta?.updatedAt || nowISO(), source:'server', stale:!!partialErrors, warning:partialErrors };
      } catch(e) {
        if(useCachedFallback){
          const backupState = this.loadBackup();
          const cached = this.loadFastCacheAnyAge() || this.buildStateFromTableCaches() || (backupState ? { ok:true, payload:backupState, at: safeTrim(backupState?.meta?.updatedAt) || nowISO(), source:'backup-cache' } : null);
          if(cached?.ok) return { ok:true, payload: cached.payload, at: cached.at, source: cached.source || 'cache', stale:true, warning: String(e?.message || e) };
        }
        return { ok:false, error: String(e?.message || e) };
      }
    },
...
UsersGateUI.init();
  ...
  const CustomersUI = {
    ...
    renderPolicyRow(policy){
      ...
      const secondaryMeta = [
        ...
        (safeTrim(policy.type) === 'ריסק' && policy.umbrellaInsurance) ? `מטריה ביטוחית: נכות ${safeTrim(policy.umbrellaDisabilityAmount || '—')} · מוות ${safeTrim(policy.umbrellaDeathAmount || '—')}` : ''
            ...
            ${(this.draft.newPolicies || []).length ? this.draft.newPolicies.map((policy, idx) => {
              ...
              return `<article class="customerEditPolicyCard">
                ...
                <div class="customerEditChecks">
                  ...
                  ${isRegularRisk ? this.renderCheckbox("כולל מטריה ביטוחית",`${prefix}.umbrellaInsurance`, !!policy?.umbrellaInsurance) : ``}
                </div>
  ...
  const LiveRefresh = {
    ...
    async tick(){
      ...
      this.busy = true;
      try {
        const r = await Storage.loadSheets({ useCachedFallback:true });
        if(!r?.ok) return;
  ...
  const DashboardUI = {
    ...
    render(){
      ...
      const hasData = metrics.submittedProposals || metrics.netPremium || metrics.newClients || metrics.soldPolicies || metrics.agentAppointments;
      if(!hasData){
        this.renderEmpty();
        return;
      }
      ...
      const cards = [
        ...
        },
        {
          ...
          he: 'כמות הצעות שהוגשו',
          value: String(metrics.submittedProposals),
          delta: metrics.submittedDelta,
          ...
          <article class="bankGoal card bankGoal--${metrics.targetTone}">
            ...
            <div class="bankGoal__shell">
              <div class="bankGoal__meter">
                ...
                <div class="bankGoal__progressTrack"><span style="width:${Math.min(metrics.targetPct, 100)}%"></span></div>
  ...
  const Wizard = {
    ...
    insTypes: ["בריאות","סיעודי","מחלות קשות","סרטן","תאונות אישיות","אובדן כושר עבודה","נכויות","ריסק","ריסק משכנתא","פוליסה משולבת"],
...
init(){
    ...
    handleStepEntry(fromStep, toStep){
      ...
      if(to === 8 && from !== 8){
        ...
        store.ui = store.ui || { currentIndex: 0, summary: false };
        store.ui.summary = false;
        store.ui.currentIndex = list.length ? 0 : 0;
      }
      if(to === 9 && from !== 9){
        this._operationalGuideAccepted = false;
...
// open health covers drawer (Health only)
    ...
    getStep1Questions(ins){
      ...
      return [
        ...
        },
        {
          key:'personalDetails',
          title:'פרטים אישיים',
          sub:'כאן מרכזים תאריך לידה, גיל, מין, מצב משפחתי, קופת חולים, שב״ן ועיסוק — הכל במסך אחד.',
          ...
          render:() => {
            const clinicOptions = ['', ...this.clinics];
            const shabanOptions = d.clinic ? ['', ...(this.shabanMap[d.clinic] || [])] : [''];
            return `
            <div class="lcStep1Group">
              <div class="lcStep1Group__grid lcStep1Group__grid--personal">
                ...
                ${!isChild ? `
                  <div class="lcStep1HealthFundRow">
                    ${this.fieldSelect('קופת חולים','clinic', d.clinic, clinicOptions)}
                    <div class="field">
                      <label class="label">שב״ן</label>
                      <select class="input" data-bind="shaban" ${d.clinic ? '' : 'disabled'}>
                        ${shabanOptions.map((option) => `<option value="${escapeHtml(option)}"${String(d.shaban)===String(option)?' selected':''}>${escapeHtml(option || 'בחר שב״ן')}</option>`).join('')}
                      </select>
                    </div>
                  </div>
                  <div class="field lcOccWrap lcOccWrap--step1Wide">
                ...
                ` : `<div class="lcStep1InfoCard lcStep1InfoCard--soft"><div class="lcStep1InfoCard__label">קופת חולים / שב״ן</div><div class="lcStep1InfoCard__value">לא נדרש לקטין</div><div class="lcStep1InfoCard__sub">בקטין המערכת לא מחייבת את השדות האלו.</div></div><div class="lcStep1InfoCard lcStep1InfoCard--soft"><div class="lcStep1InfoCard__label">עיסוק</div><div class="lcStep1InfoCard__value">לא נדרש לקטין</div><div class="lcStep1InfoCard__sub">המערכת לא מחייבת עיסוק עבור ילד.</div></div>`}
              </div>
            </div>`
          }
        },
    ...
    normalizeHarPolicyType(row){
      ...
      const source = [productType, sub, main, section, safeTrim(row?.extra)].filter(Boolean).join(" | ");
      ...
      if(/תאונות אישיות/i.test(source)) return "תאונות אישיות";
      if(/מקרה מוות|ביטוח חיים/i.test(source)) return "ריסק";
      if(/בריאות|השתלות|תרופות|ניתוחים|שב"ן|שב״ן|אמבולטורי|אבחון|ייעוץ|בדיקות/i.test(source)) return "בריאות";
      if(/חיים/i.test(source)) return "ריסק";
      return "בריאות";
    },
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      sheetNames.forEach((sheetName) => {
        ...
        rows.slice(headerIdx + 1).forEach((row) => {
          ...
          const record = {
            ...
            classification: safeTrim(idx.classification >= 0 ? row[idx.classification] : "")
          };
      ...
      const policies = Array.from(grouped.values()).map((bucket) => {
        ...
        const type = typeList.length <= 1 ? (typeList[0] || "בריאות") : "פוליסה משולבת";
    ...
    renderStep3(ins){
      ...
      const rows = (d.existingPolicies || []).map(p => {
        ...
        const cancellationBox = this.renderExistingPolicyCancellationControls(p, d.cancellations?.[p.id] || {}, { compact: true, insured: ins });
              ...
              <td colspan="7" class="lcPolImportedWrapCell">
                <div class="lcPolImportedRow">
                  ...
                  <div class="lcPolImportedRow__segment lcPolImportedRow__segment--price">
                    <span class="lcPolImportedRow__premiumLabel">פרמיה חודשית</span>
    ...
    renderStep5(){
      ...
      const companyOptions = this.companies.map(c => {
        ...
        const logo = src
          ? `<img class="lcCompanyPicker__optionLogo" src="${escapeHtml(src)}" alt="${escapeHtml(c)}" />`
          : `<span class="lcCompanyPicker__optionLogo lcCompanyPicker__optionLogo--fallback">${escapeHtml((c || 'ח').slice(0,1))}</span>`;
        return `<button type="button" class="lcCompanyPicker__option${selected ? ' is-selected' : ''}" data-company-option="${escapeHtml(c)}">${logo}<span class="lcCompanyPicker__optionText">${escapeHtml(c)}</span></button>`;
      }).join("");
      const selectedCompanyLogoSrc = this.getCompanyLogoSrc(d.company);
      const selectedCompanyLogo = d.company
        ? (selectedCompanyLogoSrc
            ? `<img class="lcCompanyPicker__valueLogo" src="${escapeHtml(selectedCompanyLogoSrc)}" alt="${escapeHtml(d.company)}" />`
            : `<span class="lcCompanyPicker__valueLogo lcCompanyPicker__valueLogo--fallback">${escapeHtml((d.company || 'ח').slice(0,1))}</span>`)
        : `<span class="lcCompanyPicker__valueLogo lcCompanyPicker__valueLogo--placeholder">🏢</span>`;
      const companyPickerOpen = !!this.companyPickerOpen;
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        const chips = [
          ...
          (!isMedicare && p.type === "ריסק" && p.umbrellaInsurance) ? `<span class="lcPolInfoChip lcPolInfoChip--umbrella"><span class="lcPolInfoChip__icon">☂️</span><span class="lcPolInfoChip__text"><b>מטריה ביטוחית</b><small>${escapeHtml([safeTrim(p.umbrellaDisabilityAmount) ? `נכות: ${safeTrim(p.umbrellaDisabilityAmount)}` : '', safeTrim(p.umbrellaDeathAmount) ? `מוות: ${safeTrim(p.umbrellaDeathAmount)}` : ''].filter(Boolean).join(' · ') || 'פעיל')}</small></span></span>` : ''
        ].filter(Boolean).join('');
      ...
      const form = `<div class="lcWSection lcPolBuilderSection lcPolBuilderSection--reference">
        <div class="lcPolScreenHero lcPolScreenHero--gemelRibbon">
          <div class="lcPolScreenHero__icon" aria-hidden="true">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">
              <path d="M12 3l7 3v5c0 5-3.5 8.5-7 10-3.5-1.5-7-5-7-10V6l7-3z"></path>
              <path d="M9 12.5l2 2 4-4"></path>
            </svg>
          </div>
          <div class="lcPolScreenHero__text">
            <div class="lcPolScreenHero__kicker">GEMEL INVEST</div>
            <div class="lcPolScreenHero__title">פוליסות חדשות מבית GEMEL INVEST</div>
          </div>
        </div>

        <div class="lcPolForm lcPolForm--premium lcPolForm--reference">
          <div class="lcPolBuilderCard lcPolBuilderCard--reference">
            <div class="lcField lcInsuredGlass lcPolBuilderAssign">
              <div class="lcInsuredGlassCard lcInsuredGlassCard--reference">
                <div class="lcInsuredGlassHead lcInsuredGlassHead--reference">
                  <div class="lcInsuredGlassHead__titleWrap">
                    <label class="lcLabel">שיוך למבוטח:</label>
                    <div class="small muted">קבעו לפי מי הפוליסה תישמר בסיכום</div>
                  </div>
                  <div class="lcInsuredGlassSelectWrap">
                    <select class="lcSelect" data-pdraft="insuredId"${(d.insuredMode==="couple")?" disabled":""}>
                      ${insuredOpts}
                    </select>
                  </div>
                </div>
                <div class="lcInsuredGlassModes">
                  ${spouse ? `<button type="button" class="lcBtn lcBtn--ghost ${d.insuredMode==="couple"?"is-active":""}" data-pdraftmode="couple">פוליסה זוגית למבוטחת אחת</button>` : ``}
            ...
            <section class="lcPolReferenceBlock lcPolReferenceBlock--details lcPolReferenceBlock--detailsClean lcPolReferenceBlock--builderModern">
              <div class="lcPolReferenceBlock__head lcPolReferenceBlock__head--compact">
                <div>
                  <div class="lcPolReferenceBlock__title">פרטי הפוליסה החדשה</div>
                  <div class="lcPolReferenceBlock__sub">בחרו חברה, מוצר ופרטי בסיס בצורה נקייה ומסודרת</div>
                </div>
                <div class="lcPolReferenceBlock__icon" aria-hidden="true">✨</div>
              </div>
              <div class="lcPolGrid lcPolGrid--top lcPolGrid--mainRow lcPolGrid--reference lcPolGrid--modernStart">
                <div class="lcField lcPolField lcPolField--company lcPolField--companyPicker">
                  <label class="lcLabel">חברה</label>
                  <div class="lcCompanyPicker${companyPickerOpen ? ' is-open' : ''}">
                    <button type="button" class="lcCompanyPicker__trigger lcPolControlShell" data-toggle-company-picker="1" aria-expanded="${companyPickerOpen ? 'true' : 'false'}">
                      <span class="lcCompanyPicker__value">
                        ${selectedCompanyLogo}
                        <span class="lcCompanyPicker__valueText${d.company ? '' : ' is-placeholder'}">${escapeHtml(d.company || 'בחר חברה')}</span>
                      </span>
                      <span class="lcCompanyPicker__chevron" aria-hidden="true">▾</span>
                    </button>
                    <div class="lcCompanyPicker__menu" ${companyPickerOpen ? '' : 'hidden'}>
                      ${companyOptions}
                    </div>
                  </div>
                </div>
                ...
                <div class="lcField lcPolField lcPolField--premiumMain lcPolField--premiumReference">
                  ...
                  <div class="lcPolMoneyWrap lcPolControlShell lcPolMoneyWrap--reference">
                    ...
                    <input class="lcInput lcPolMoneyWrap__input" type="text" inputmode="numeric" data-pdraft="premiumMonthly" value="${escapeHtml(d.premiumMonthly || "")}" placeholder="250" />
                  </div>
                </div>
              </div>

              <div class="lcPolGrid lcPolGrid--money lcPolGrid--referenceExtras">
              ...
              ${isRegularRisk ? `<div class="lcRiskUmbrellaBox">
                <div class="lcRiskUmbrellaBox__head">
                  <div>
                    <div class="lcRiskUmbrellaBox__title">מטריה ביטוחית</div>
                    <div class="lcRiskUmbrellaBox__sub">אופציונלי לפוליסת ריסק רגילה בלבד</div>
                  </div>
                  <label class="lcPolToggle lcPolToggle--umbrella">
                    <input type="checkbox" data-pdraft="umbrellaInsurance" ${d.umbrellaInsurance ? "checked":""} />
                    <span>הוסף מטריה ביטוחית</span>
                  </label>
                </div>
                ${d.umbrellaInsurance ? `<div class="lcRiskUmbrellaBox__grid">
                  <div class="lcField">
                    <label class="lcLabel">פיצוי לנכות מתאונה</label>
                    <input class="lcInput" type="text" inputmode="numeric" data-pdraft="umbrellaDisabilityAmount" value="${escapeHtml(d.umbrellaDisabilityAmount || "")}" placeholder="לדוגמה: 200,000" />
                  </div>
                  <div class="lcField">
                    <label class="lcLabel">פיצוי למוות מתאונה</label>
                    <input class="lcInput" type="text" inputmode="numeric" data-pdraft="umbrellaDeathAmount" value="${escapeHtml(d.umbrellaDeathAmount || "")}" placeholder="לדוגמה: 150,000" />
                  </div>
                </div>` : ``}
              </div>` : ``}

              ${(!isMedicare && d.type === "בריאות") ? `<div class="lcPolCoverCompact lcPolCoverCompact--editor lcPolCoverCompact--reference">
              ...
              ${(canPledge && d.pledge) ? `<div class="lcWSection lcPledgeBox lcPledgeBox--reference">
            ...
            <div class="lcPolBuilderActions lcPolBuilderActions--reference">
              ${this.editingPolicyId ? `<button type="button" class="lcBtn" data-cancel-editpol="1">ביטול עריכה</button>` : ``}
              <button type="button" class="lcBtn lcBtn--primary lcBtn--referenceAdd" data-addpol="1">${this.editingPolicyId ? "שמור שינויים" : "הוסף פוליסה"}</button>
            </div>
          </div>
        </div>
      </div>`;
      ...
      setTimeout(() => {
        const syncPremiumDemo = () => {
          const premiumInput = this.els.body.querySelector('.lcPolField--premiumReference [data-pdraft="premiumMonthly"]');
          const premiumWrap = this.els.body.querySelector('.lcPolField--premiumReference .lcPolMoneyWrap--reference');
          if(!premiumInput || !premiumWrap) return;
          premiumWrap.classList.toggle('has-value', !!safeTrim(premiumInput.value));
        };

        const applySelectedCompany = (co) => {
          this.ensurePolicyDraft();
        ...
        $$('[data-toggle-company-picker]', this.els.body).forEach(btn => {
          on(btn, 'click', () => {
        ...
        $$('[data-company-option]', this.els.body).forEach(btn => {
          on(btn, 'click', () => applySelectedCompany(btn.getAttribute('data-company-option')));
        });
        ...
        $$("[data-pdraft]", this.els.body).forEach(el => {
          on(el, "input", () => {
            ...
            if(k === "premiumMonthly") syncPremiumDemo();
            if(!k) return;
            ...
            if(k === "type" || k === "pledge" || k === "insuredId" || k === "company" || k === "umbrellaInsurance") this.render();
          });
          on(el, "change", () => {
            this.ensurePolicyDraft();
            const k = el.getAttribute("data-pdraft");
    ...
    validateDraftPolicy(){
      ...
      if(!isMedicare && d.type === "ריסק" && d.umbrellaInsurance){
        if(!safeTrim(d.umbrellaDisabilityAmount) || !safeTrim(d.umbrellaDeathAmount)) return { ok:false, msg:"במטריה ביטוחית חובה למלא פיצוי לנכות מתאונה ופיצוי למוות מתאונה" };
      }
...
// ---------- Step 6 ----------
    ...
    renderStep9(){
      ...
      const getExistingStatusClass = (raw='') => {
        ...
        if(key === 'agent_appointment') return 'is-appoint';
        return '';
      };
    ...
    getHealthDerivedCache(){
      ...
      (this.newPolicies || []).forEach((policy) => {
        ...
        const type = safeTrim(policy?.type);
        if(['ריסק','ריסק משכנתא','בריאות','מחלות קשות','סרטן'].includes(type)) relevantPolicies.push(policy);
      });
    ...
    getHealthMasterSelection(){
      ...
      if(cancerShort){
        const sourcePolicies = [cancerShort.policy];
        const companies = Array.from(new Set(sourcePolicies.map(policy => safeTrim(policy?.company)).filter(Boolean)));
        const sourceLabels = sourcePolicies.map(policy => this.getPolicyLabel(policy)).filter(Boolean);
        cache.masterSelection = {
          key: 'master_cancer_short_under_600k',
          schemaKey: 'cancer_short_under_600k',
          title: 'הצהרת בריאות ראשית · סרטן עד 600,000 ₪',
          summary: 'נבחר מוצר סרטן כמוצר בודד עם סכום פיצוי של עד 600,000 ₪ — לכן נטענת ההצהרה המקוצרת של סרטן (חלק א׳ בלבד).',
          sourceLabel: 'סרטן עד 600,000 ₪',
          sourcePolicies,
          inheritedPolicies: [],
          sourceLabels,
          inheritedLabels: [],
          companies,
          shortCancerMode: true,
          amount: cancerShort.amount
        };
      ...
      if(!sourcePolicies.length) return (cache.masterSelection = null);
      cache.masterSelection = {
    ...
    getHealthQuestionsFiltered(){
      ...
      if(phoenixSchema.length){
        cache.filteredQuestions = phoenixSchema;
        return cache.filteredQuestions;
      }
      const companies = cache.companies;
      const schema = this.getHealthSchema();
      cache.filteredQuestions = (!companies.length ? schema : schema.map(cat => {
    ...
    renderStep8(){
      const companies = this.getHealthCompanies();
      const categories = this.getHealthQuestionsFiltered();
      const list = this.getHealthQuestionList();
      const store = this.getHealthStore();
      if(!list.length){
      ...
      const matrix = this.insureds.map(ins => {
        ...
        const savedBox = yes && r.saved ? `<div class="lcHSavedRow"><span class="lcHSavedRow__ok">✓ נשמר עבור ${escapeHtml(ins.label)}</span><span class="lcHSavedRow__meta">${escapeHtml(this.summarizeHealthFields(q, r.fields || {}, { limit:2 }))}</span><div class="lcHSavedRow__actions"><button type="button" class="btn" data-hedit="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">ערוך</button><button type="button" class="btn btn--danger" data-hclear="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">נקה</button></div></div>` : '';
        const editor = showEditor ? `<div class="lcHDetailCard"><div class="lcHDetailCard__head">פירוט עבור: ${escapeHtml(ins.label)}</div><div class="lcHQFields">${(q.fields || []).map(f => this.renderHealthField(q, ins.id, f)).join('')}</div><div class="lcHDetailCard__foot"><button type="button" class="btn btn--primary" data-hsave="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">שמור</button>${!valid ? `<span class="lcHInlineWarn">יש למלא לפחות שדה אחד בתת־השאלון לפני שמירה</span>` : ''}</div></div>` : '';
      ...
      const catIndex = categories.findIndex(c => c.key === item.catKey);
      return `<div class="lcHLayout">
        <div class="lcHMain">
          <div class="lcHHeroCard">
            ...
            <div class="lcHCategoryRail">${categories.map((cat, cidx) => `<button type="button" class="lcHCatPill ${cidx===catIndex ? 'is-active' : ''}" data-hgoto-cat="${cidx}">${escapeHtml(cat.title)}</button>`).join('')}</div>
          </div>
    ...
    bindHealthInputs(){
      ...
      $$('[data-hfield]', this.els.body).forEach(el => {
        ...
        const persist = () => {
          ...
          const prev = this.getHealthResponse(qKey, insId);
          const nextValue = safeTrim(el.value);
          const prevValue = safeTrim(prev.fields?.[fieldKey]);
        ...
        on(el, 'input', () => {
          ...
          writeTimer = setTimeout(persist, 90);
        });
        on(el, 'change', () => {
          clearTimeout(writeTimer);
          persist();
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      insureds.forEach((ins, index) => {
        ...
        policies.forEach((policy) => {
          ...
          const cancelExecutionLabel = this.isCancellationExecutionMethodRequired(safeTrim(statusMeta?.raw))
            ? this.getCancellationExecutionMethodLabel(this.getCancellationExecutionMethodValue(ins, safeTrim(policy?.id)))
            : '';
          const statusParts = [safeTrim(statusMeta?.reason), cancelExecutionLabel ? `אופן ביצוע הביטול: ${cancelExecutionLabel}` : '', safeTrim(statusMeta?.partialDetails)].filter(Boolean);
          existingRows.push([
            insuredLabel,
            safeTrim(policy?.company) || '—',
      ...
      const newPolicyBlocks = newPolicies.map((policy, index) => ({
        ...
        rows: [
          ...
          ['מטריה ביטוחית', policy?.type === 'ריסק' ? (policy?.umbrellaInsurance ? 'כן' : 'לא') : ''],
      ...
      const pageOneSections = [
        ...
        insuredRows.length ? `<section class="lcPdfSection"><div class="lcPdfSection__title">מבוטחים</div>${renderClassicTable(['מבוטח','שם מלא','ת״ז','תאריך לידה','טלפון','קופת חולים'], insuredRows)}</section>` : '',
      ...
      const pageTwoSections = [
        `<section class="lcPdfSection"><div class="lcPdfSection__title">פוליסות קיימות</div>${renderClassicTable(['מבוטח','חברה','סוג ביטוח','מספר פוליסה','פרמיה','סטטוס'], existingRows, 'לא נוספו פוליסות קיימות')}</section>`,
    ...
    isStepCompleteForInsured(stepId, ins){
      ...
      if(stepId === 3){
        // Existing policies: every row must include monthly premium and inline cancellation action.
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const App = {
    ...
    applyStateSnapshot(payload, options = {}){
      ...
      if (Auth.isOps()) { try { ProcessesUI.render(); } catch(_e) {} try { OpsEventsUI.renderToolbarState(); OpsEventsUI.checkReminders(); } catch(_e) {} }
      if (Auth.current) {
        try { ChatUI.onLogin(); } catch(_e) {}
    ...
    applyLoadResult(result, statusLabel = 'מחובר'){
      ...
      const label = result.stale ? 'נטען מזיכרון מקומי' : statusLabel;
    ...
    async boot(){
      ...
      const fastCache = Storage.loadFastCache() || Storage.buildStateFromTableCaches() || (Storage.loadBackup() ? { ok:true, payload:Storage.loadBackup(), at: safeTrim(Storage.loadBackup()?.meta?.updatedAt) || nowISO(), source:'backup-cache' } : null);
      if (fastCache?.ok) {
        this.applyStateSnapshot(fastCache.payload, { source: fastCache.source || 'cache' });
        UI.renderSyncStatus('טעינה מיידית מהזיכרון', 'warn', fastCache.at);
        this.warmViewCache();
      } else {
        UI.renderSyncStatus('מחבר לשרת…', 'warn');
      }

      const r = await Storage.loadSheets({ useCachedFallback:true });
      if (r.ok) {
        this.applyLoadResult(r, 'מחובר');
        Storage.startKeepAlive();
        this.warmViewCache();
      } else if (!fastCache?.ok) {
  ...
  const completeAgentLogin = async (matched, options = {}) => {
    ...
    UI.applyRoleUI();
    UI.renderAuthPill();
    await WelcomeLoader.play(Auth.current.name, loaderMs);
    BirthdaysUI.maybeCelebrateLogin();
    UI.goView('dashboard');
    try { ChatUI.onLogin(); } catch(_e) {}
  };
...
(function(){
  ...
  async function checkForSystemUpdate(){
    ...
    try{
      ...
      const build = window.__GI_BUILD || '';
      if(!build) return;
      if(!text.includes('const BUILD = "' + build + '";')){
