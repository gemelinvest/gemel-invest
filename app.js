...
(() => {
  ...
  const Wizard = {
    ...
    _focusSnapshot(){
      ...
      const snap = {
        id: ae.id || "",
        dataId: ae.getAttribute?.("data-id") || "",
        dataK: ae.getAttribute?.("data-k") || "",
        dataBmi: ae.getAttribute?.("data-bmi") || "",
        dataBmiId: ae.getAttribute?.("data-id") || "",
    ...
    _restoreFocus(snap){
      ...
      if(!el && snap.dataId && snap.dataK){
        try{
          el = this.els.wrap?.querySelector?.(`[data-id="${CSS.escape(snap.dataId)}"][data-k="${CSS.escape(snap.dataK)}"]`);
        }catch(_){}
      }

      // BMI fields
      if(!el && snap.dataBmi && snap.dataId){
        try{
          el = this.els.wrap?.querySelector?.(`[data-id="${CSS.escape(snap.dataId)}"][data-bmi="${CSS.escape(snap.dataBmi)}"]`);
        }catch(_){}
      }

      if(el && typeof el.focus === "function"){
        el.focus({ preventScroll:true });
        if(typeof el.setSelectionRange === "function" && snap.selStart !== null && snap.selEnd !== null){
    ...
    init(){
      ...
      on(this.els.fields.c_dob, "input", () => { this.updateAge(); this.updateProgress(); });
    ...
    updateProgress(){
      const __snap = this._focusSnapshot();
      try{
      ...
      if(this.els.btnNext){
        ...
        // keep caret/focus stable (prevents "kicking out" while typing)
        this._restoreFocus(__snap);
      }
