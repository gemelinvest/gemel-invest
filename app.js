...
init(){
    ...
    renderBody(){
      ...
      else if(this.step === 8) this.bindHealthDeclaration();
    },
...
// ---------- Step 6 ----------
    ...
    renderStep7(){
      ...
      return `
        <div class="lcWSection">
          ...
          <div class="divider"></div>
          <div class="help">בשלב הזה כבר יש לנו מבנה מלא של שלבים 1–8, כולל שלב הצהרת בריאות דינמי, טאבים למבוטחים ולוגיקה של מעבר לפי השלמת חובה.</div>
        </div>
    ...
    getHealthStore(){
      const primary = this.insureds[0];
      if(!primary) return {};
      if(!primary.data.healthDeclaration || typeof primary.data.healthDeclaration !== "object"){
        primary.data.healthDeclaration = { categories:{} };
      }
      if(!primary.data.healthDeclaration.categories || typeof primary.data.healthDeclaration.categories !== "object"){
        primary.data.healthDeclaration.categories = {};
      }
      return primary.data.healthDeclaration;
    },

    getSelectedHealthCompanies(){
      const map = {"כלל":"clal","הפניקס":"phoenix","הכשרה":"hachshara"};
      const out = [];
      (this.newPolicies || []).forEach(p => {
        const key = map[safeTrim(p.company)];
        if(key && !out.includes(key)) out.push(key);
      });
      return out;
    },

    getHealthCompanyLabel(key){
      return ({ clal:"כלל", phoenix:"הפניקס", hachshara:"הכשרה" })[key] || key;
    },

    getHealthDeclarationDefs(){
      return [
        {
          id:"family_history",
          title:"היסטוריה משפחתית",
          summary:"מחלות/מומים אצל קרובי משפחה מדרגה ראשונה.",
          companies:["clal"],
          question:"האם קיימת אצלך היסטוריה משפחתית רפואית מהותית בקרובי משפחה מדרגה ראשונה?",
          followups:{
            clal:[
              { key:"conditions", type:"checkboxes", label:"אילו מצבים קיימים במשפחה?", options:["סרטן המעי הגס","סרטן השד","סרטן השחלות","סרטן הערמונית","טרשת נפוצה","ALS","פרקינסון","אלצהיימר","פוליפוזיס משפחתית","מחלת לב","סוכרת","כליות פוליציסטיות","הנטינגטון","מיוטוניק דיסטרופי","צליאק"] },
              { key:"affectedCount", type:"select", label:"כמה קרובי משפחה הושפעו?", options:["1","2","3 ומעלה"] },
              { key:"youngestAge", type:"text", label:"גיל הצעיר מבין החולים" },
              { key:"details", type:"textarea", label:"פירוט נוסף / בני משפחה רלוונטיים" }
            ]
          }
        },
        {
          ...
          summary:"מחלות עצביות, הפרעות תחושה, אירועים מוחיים, מיגרנות ועוד.",
          companies:["clal"],
          question:"האם אובחנה אצל אחד המועמדים בעיה במערכת העצבים או במוח?",
          followups:{
            clal:[
              { key:"diagnoses", type:"checkboxes", label:"בחר אבחנות רלוונטיות", options:["אפילפסיה","כאבי ראש חדשים","מיגרנה","הפרעות קשב וריכוז","דלקת המוח/קרום המוח","חבלת ראש","גידול מוחי","TIA / שבץ / דימום מוחי","טרשת נפוצה / פרקינסון / ניוון שרירים","סחרחורות / התעלפויות","עיכוב התפתחותי / אוטיזם / שיתוק מוחין"] },
              { key:"lastEvent", type:"text", label:"מתי היה האירוע / האבחון האחרון?" },
              { key:"treatment", type:"textarea", label:"טיפול קיים / תרופות / תוצאות בירור" },
              { key:"residualDamage", type:"select", label:"האם נותר נזק שארי?", options:["לא","כן","לא ידוע"] }
            ]
          }
        },
        {
          id:"mental_health",
          title:"בריאות נפש",
          summary:"חרדה, דיכאון, PTSD, טיפול נפשי, אשפוזים ונכות.",
          companies:["clal","phoenix"],
          question:"האם קיימת אבחנה או טיפול בתחום בריאות הנפש?",
          followups:{
            clal:[
              { key:"diagnoses", type:"checkboxes", label:"אבחנות כלל", options:["דיכאון מג'ורי","סכיזופרניה","הפרעה דו קוטבית","חרדה","OCD","PTSD","הפרעות אכילה","לחץ נפשי / קשיי הסתגלות"] },
              { key:"treatmentType", type:"checkboxes", label:"טיפול/המלצה לטיפול", options:["תרופתי","פסיכולוגי","פסיכיאטרי"] },
              { key:"hospitalization", type:"select", label:"אשפוז / נכות נפשית", options:["לא","אשפוז בעבר","אשפוז בשנה האחרונה","נקבעה נכות"] },
              { key:"details", type:"textarea", label:"פירוט כללי / חודשים / אחוזי נכות" }
            ],
            phoenix:[
              { key:"diagnoses", type:"checkboxes", label:"אבחנות הפניקס", options:["חרדה / סטרס","דיכאון","OCD","הפרעה ביפולרית","PTSD","סכיזופרניה","הפרעת אכילה","אחר"] },
              { key:"treatmentGroup", type:"select", label:"סוג טיפול תרופתי", options:["ללא טיפול בשנתיים האחרונות","טיפול תרופתי סוג 1","טיפול תרופתי סוג 2","קנאביס רפואי","אחר"] },
              { key:"disability", type:"select", label:"נכות נפשית", options:["לא","1%-29%","30% ומעלה"] },
              { key:"riskFlags", type:"checkboxes", label:"אירועים מיוחדים", options:["אשפוז/אשפוז יום","ניסיונות אובדניים","פגישה עם פסיכיאטר בשנה האחרונה"] }
            ]
          }
        },
        {
          id:"respiratory",
          title:"נשימה וריאות",
          summary:"אסטמה, COPD, דום נשימה, ברונכיטיס, תפקודי ריאה.",
          companies:["clal","phoenix"],
          question:"האם קיימת מחלה או הפרעה בדרכי הנשימה או בריאות?",
          followups:{
            clal:[
              { key:"diagnoses", type:"checkboxes", label:"מצבים בכלל", options:["אסטמה","פנאומוטרקס","דום נשימה בשינה","ברונכיאקטזיות / אמפיזמה / ברונכיטיס כרונית / COPD","דלקות ריאה חוזרות","סרקואידוזיס","ציסטיק פיברוזיס","מושתל ריאה"] },
              { key:"attackFreq", type:"select", label:"תכיפות / חומרת הבעיה", options:["קל / לעיתים רחוקות","בינוני / עונתי","פעם בשבוע","יותר מפעם בשבוע","קשה"] },
              { key:"treatment", type:"checkboxes", label:"טיפול נוכחי", options:["משאפים","כדורים / זריקות","CPAP","מכשיר דנטלי","ניתוח","ללא טיפול"] },
              { key:"hospitalization", type:"select", label:"אשפוז / תפקודי ריאה", options:["לא","אשפוז ב-5 שנים האחרונות","אשפוז בשנה האחרונה","נדרש לצרף תפקודי ריאה"] }
            ],
            phoenix:[
              { key:"diagnosis", type:"checkboxes", label:"אבחנות הפניקס", options:["אסטמה","COPD / אמפיזמה / ברונכיט כרונית","פנאומוטורקס","סרקואידוזיס","סינוסיטיס כרונית / חוזרת","דום נשימה בשינה"] },
              { key:"asthmaTreatment", type:"select", label:"תכיפות טיפול באסטמה", options:["בעת התקף בלבד","טיפול מניעתי / יומיומי","סטרואידים ב-6 חודשים אחרונים"] },
              { key:"hospitalizationLastYear", type:"select", label:"האם היה אשפוז בשנה האחרונה?", options:["לא","כן"] },
              { key:"notes", type:"textarea", label:"פירוט רפואי / תיעוד שיש לצרף" }
            ]
          }
        },
        {
          id:"cardio",
          title:"לב וכלי דם",
          summary:"יתר לחץ דם, מחלות לב, הפרעות קצב, מסתמים וכלי דם.",
          companies:["clal"],
          question:"האם קיימת מחלה בלב, בכלי הדם או לחץ דם גבוה?",
          followups:{
            clal:[
              { key:"diagnoses", type:"checkboxes", label:"בחר מצבים רלוונטיים", options:["יתר לחץ דם","כאבי חזה","התקף לב","הפרעת קצב / הולכה","איוושה / מסתמים","מום מולד","צנתור / ניתוח מעקפים","טרומבוזה / מפרצת / PVD"] },
              { key:"bpValues", type:"text", label:"ערכי לחץ דם נוכחיים" },
              { key:"medication", type:"select", label:"האם יש טיפול תרופתי קבוע?", options:["לא","כן"] },
              { key:"details", type:"textarea", label:"פירוט בדיקות, אשפוזים, ניתוחים ומצב עדכני" }
            ]
          }
        },
        {
          id:"digestive",
          title:"מערכת העיכול",
          summary:"קרוהן, קוליטיס, כבד שומני, הפטיטיס, צרבת וניתוחי עיכול.",
          companies:["phoenix"],
          question:"האם קיימת מחלה במערכת העיכול, בכבד או בצהבת/הפטיטיס?",
          followups:{
            phoenix:[
              { key:"digestiveDiagnosis", type:"checkboxes", label:"אבחנות במערכת העיכול", options:["צרבת / החזר ושטי","כיב קיבה / תריסריון","מעי רגיז","קרוהן","אולצרטיב קוליטיס","טחורים / פיסורה","אחר"] },
              { key:"digestiveTreatment", type:"select", label:"טיפול במערכת העיכול", options:["ללא טיפול","כדורים","טיפול ביולוגי","ניתוח"] },
              { key:"extraInvolvement", type:"select", label:"מעורבות מחוץ למעי (מפרקים / כבד / אחר)", options:["לא","כן"] },
              { key:"liverDiagnosis", type:"checkboxes", label:"כבד / הפטיטיס", options:["כבד שומני","הפטיטיס A","הפטיטיס B","הפטיטיס C","הפטיטיס D","הפטיטיס E","שחמת","אחר"] }
            ]
          }
        },
        {
          ...
          summary:"שברים, פריקות, דיסק, כאבי גב, מחלות ראומטולוגיות.",
          companies:["phoenix"],
          question:"האם קיימת בעיה במערכת השלד, הגב או המפרקים?",
          followups:{
            phoenix:[
              { key:"spineDiagnosis", type:"checkboxes", label:"עמוד שדרה", options:["בלט / בקע / פריצת דיסק","עקמת / קיפוזיס / סקוליוזיס","צליפת שוט","כאבי גב / עמוד שדרה"] },
              { key:"skeletalDiagnosis", type:"checkboxes", label:"שלד / גפיים", options:["שבר","פריקה","תסמונת תעלה קרפלית","מניסקוס","רצועה","אוסטאופניה / אוסטיאופורוזיס","אחר"] },
              { key:"location", type:"text", label:"מיקום פגיעה / אזור בגוף" },
              { key:"surgeryLimit", type:"checkboxes", label:"מה נכון כרגע?", options:["עבר ניתוח","נותרה מגבלה תפקודית או כאבים","פגיעה דו צדדית"] },
              { key:"rheuma", type:"checkboxes", label:"מחלות ראומטולוגיות", options:["RA","אנקילוזינג ספונדיליטיס","אוסטיאוארטריטיס","FMF","אחר"] }
            ]
          }
        },
        {
          id:"skin",
          title:"עור",
          summary:"פסוריאזיס, נגעי עור, קונדילומה, אקזמה, מלנומה ועוד.",
          companies:["clal"],
          question:"האם קיימת מחלה, נגע או הפרעה משמעותית בעור?",
          followups:{
            clal:[
              { key:"skinIssues", type:"checkboxes", label:"בחר מצבים רלוונטיים", options:["גידול בעור / נקודת חן / שומה","פסוריאזיס","קונדילומה / פפילומה","פלונידל סינוס / מורסה","אורטיקריה / אטופיק דרמטיטיס / אקזמה","פמפיגוס / הידרדניטיס","אחר"] },
              { key:"removed", type:"select", label:"האם הנגע הוסר / טופל?", options:["לא רלוונטי","כן","לא"] },
              { key:"medication", type:"select", label:"האם קיים טיפול תרופתי קבוע?", options:["לא","משחה מקומית","טיפול מדכא חיסון","ביולוגי"] },
              { key:"details", type:"textarea", label:"פירוט פתולוגיה / מיקום / מצב עדכני" }
            ]
          }
        },
        {
          id:"smoking_drugs",
          title:"עישון / סמים",
          summary:"שימוש בסמים, קנאביס, גמילה, עישון ומסמכים תומכים.",
          companies:["clal","hachshara"],
          question:"האם קיימת צריכת סמים / קנאביס / גמילה או מידע רלוונטי בתחום זה?",
          followups:{
            clal:[
              { key:"drugTypes", type:"checkboxes", label:"שימוש לפי כלל", options:["קנביס / מריחואנה / חשיש בעבר","קנביס / מריחואנה / חשיש כיום","סמים אחרים בעבר","סמים אחרים כיום","עבר תהליך גמילה","מצוי כיום בגמילה"] },
              { key:"frequency", type:"text", label:"תכיפות שימוש / לפני כמה שנים" },
              { key:"medicalReason", type:"text", label:"רקע רפואי לשימוש (אם קיים)" },
              { key:"documents", type:"textarea", label:"מסמכים / סרולוגיה / אישור משרד הבריאות" }
            ],
            hachshara:[
              { key:"usingNow", type:"select", label:"האם יש צריכה/הייתה צריכה של סמים?", options:["לא","כן - בעבר","כן - כיום"] },
              { key:"reason", type:"select", label:"סיבת שימוש", options:["רפואי","חברתי","אחר"] },
              { key:"drugName", type:"text", label:"סוג ושם הסם" },
              { key:"rehab", type:"select", label:"האם עברת גמילה?", options:["לא","כן"] },
              { key:"details", type:"textarea", label:"מועד התחלה / תדירות / פירוט דוח גמילה" }
            ]
          }
        },
        {
          id:"accident_disability",
          title:"תאונה / נכות / שבר",
          summary:"שברים, פגיעות, אשפוזים, נכות זמנית/קבועה ותביעות.",
          companies:["hachshara"],
          question:"האם הייתה תאונה, שבר, נכות או פגיעה משמעותית ממחלה/תאונה?",
          followups:{
            hachshara:[
              { key:"eventDate", type:"text", label:"מתי אירעה התאונה / אובחנה המחלה?" },
              { key:"diagnosis", type:"textarea", label:"מה האבחנה / אילו איברים נפגעו?" },
              { key:"hospitalization", type:"text", label:"אשפוז: מתי ולכמה זמן" },
              { key:"tests", type:"textarea", label:"בדיקות, טיפולים, ניתוחים ומצב רפואי נוכחי" },
              { key:"disabilityClaim", type:"select", label:'האם הוגשה / תוגש תביעת נכות או אכ"ע?', options:["לא","כן - נכות","כן - אובדן כושר עבודה","כן - שניהם"] },
              { key:"disabilityRate", type:"text", label:"אחוזי נכות / מגבלה קיימת" }
            ]
          }
        }
      ];
    ...
    getActiveHealthCategories(){
      const selected = this.getSelectedHealthCompanies();
      if(!selected.length) return [];
      return this.getHealthDeclarationDefs().map(cat => {
        const activeCompanies = (cat.companies || []).filter(c => selected.includes(c));
        return Object.assign({}, cat, { activeCompanies });
      }).filter(cat => cat.activeCompanies.length);
    },

    getCategoryState(catId){
      const store = this.getHealthStore();
      if(!store.categories[catId] || typeof store.categories[catId] !== "object"){
        store.categories[catId] = { answer:"", notes:"", followups:{} };
      }
      const cat = store.categories[catId];
      if(!cat.followups || typeof cat.followups !== "object") cat.followups = {};
      return cat;
    },

    setHealthCategoryAnswer(catId, answer){
      const st = this.getCategoryState(catId);
      st.answer = safeTrim(answer);
    },

    toggleHealthCheckbox(catId, company, key, value){
      const st = this.getCategoryState(catId);
      if(!st.followups[company] || typeof st.followups[company] !== "object") st.followups[company] = {};
      const cur = Array.isArray(st.followups[company][key]) ? st.followups[company][key].slice() : [];
      const idx = cur.indexOf(value);
      if(idx >= 0) cur.splice(idx, 1); else cur.push(value);
      st.followups[company][key] = cur;
    },

    setHealthField(catId, company, key, value){
      const st = this.getCategoryState(catId);
      if(!st.followups[company] || typeof st.followups[company] !== "object") st.followups[company] = {};
      st.followups[company][key] = value;
    },

    renderHealthField(catId, company, field, values){
      const val = values[field.key];
      if(field.type === "textarea"){
        return `<div class="lcHdField lcHdField--full">
          <label class="lcHdLabel">${escapeHtml(field.label)}</label>
          <textarea class="lcHdTextarea" rows="3" data-hd-field="${escapeHtml(catId)}" data-hd-company="${escapeHtml(company)}" data-hd-key="${escapeHtml(field.key)}">${escapeHtml(val || "")}</textarea>
        </div>`;
      }
      if(field.type === "select"){
        const opts = [""].concat(field.options || []).map(opt => `<option value="${escapeHtml(opt)}"${String(val||"")===String(opt)?" selected":""}>${escapeHtml(opt || "בחר…")}</option>`).join("");
        return `<div class="lcHdField">
          <label class="lcHdLabel">${escapeHtml(field.label)}</label>
          <select class="lcHdSelect" data-hd-field="${escapeHtml(catId)}" data-hd-company="${escapeHtml(company)}" data-hd-key="${escapeHtml(field.key)}">${opts}</select>
        </div>`;
      }
      if(field.type === "checkboxes"){
        ...
        return `<div class="lcHdField lcHdField--full">
          <label class="lcHdLabel">${escapeHtml(field.label)}</label>
          <div class="lcHdChecks">${(field.options || []).map(opt => `
            <label class="lcHdCheck">
              <input type="checkbox" data-hd-check="${escapeHtml(catId)}" data-hd-company="${escapeHtml(company)}" data-hd-key="${escapeHtml(field.key)}" value="${escapeHtml(opt)}" ${arr.includes(opt)?"checked":""} />
      ...
      return `<div class="lcHdField">
        <label class="lcHdLabel">${escapeHtml(field.label)}</label>
        <input class="lcHdInput" type="text" value="${escapeHtml(val || "")}" data-hd-field="${escapeHtml(catId)}" data-hd-company="${escapeHtml(company)}" data-hd-key="${escapeHtml(field.key)}" />
      </div>`;
    },

    renderHealthCompanySection(cat, companyKey, values){
      const fields = ((cat.followups || {})[companyKey] || []);
      if(!fields.length) return "";
      return `<div class="lcHdFollowSection">
        <div class="lcHdFollowHead">
          <span class="lcHdTag">${escapeHtml(this.getHealthCompanyLabel(companyKey))}</span>
          <span class="muted small">שאלון משלים דינמי לפי חברת הביטוח.</span>
        </div>
        <div class="lcHdGrid">${fields.map(f => this.renderHealthField(cat.id, companyKey, f, values || {})).join("")}</div>
      </div>`;
    },

    renderStep8(){
      const categories = this.getActiveHealthCategories();
      if(!categories.length){
        return `<div class="lcWSection lcHdEmpty">
          <div class="lcWTitle">הצהרת בריאות</div>
          <div class="muted">כדי לפתוח את שלב 8 צריך להוסיף לפחות פוליסה חדשה באחת החברות: כלל, הפניקס או הכשרה בשלב 5.</div>
        </div>`;
      }

      const cards = categories.map(cat => {
        const state = this.getCategoryState(cat.id);
        const chips = cat.activeCompanies.map(c => `<span class="lcHdTag">${escapeHtml(this.getHealthCompanyLabel(c))}</span>`).join("");
        const yes = state.answer === "yes";
        const no = state.answer === "no";
        const followups = yes
          ? cat.activeCompanies.map(c => this.renderHealthCompanySection(cat, c, (state.followups || {})[c] || {})).join("")
          : "";
        return `<div class="lcHdCard ${yes?"is-yes":""} ${no?"is-no":""}">
          <div class="lcHdCard__top">
            <div>
              <div class="lcWTitle" style="margin-bottom:6px">${escapeHtml(cat.title)}</div>
              <div class="muted small">${escapeHtml(cat.summary || "")}</div>
            </div>
            <div class="lcHdTags">${chips}</div>
          </div>
          <div class="lcHdQuestion">${escapeHtml(cat.question)}</div>
          <div class="lcHdToggle" role="group" aria-label="${escapeHtml(cat.title)}">
            <button type="button" class="lcHdToggle__btn ${yes?"is-active":""}" data-hd-answer="${escapeHtml(cat.id)}" data-hd-value="yes">כן</button>
            <button type="button" class="lcHdToggle__btn ${no?"is-active":""}" data-hd-answer="${escapeHtml(cat.id)}" data-hd-value="no">לא</button>
          </div>
          ${yes ? `<div class="lcHdFollowups">${followups}</div>` : ``}
        </div>`;
      }).join("");

      const activeLabels = categories.flatMap(c => c.activeCompanies || []).filter((v, i, a) => a.indexOf(v) === i).map(c => this.getHealthCompanyLabel(c)).join(" • ");
      return `<div class="lcWSection">
        <div class="row row--between" style="align-items:flex-start; gap:12px; flex-wrap:wrap;">
          <div>
            <div class="lcWTitle">הצהרת בריאות אוניברסלית</div>
            <div class="muted small">שאלון מאוחד לפי קטגוריות רפואיות. המערכת מציגה רק את החברות שנבחרו בפוליסות החדשות.</div>
          </div>
          <div class="lcHdLegend">חברות פעילות: <b>${escapeHtml(activeLabels)}</b></div>
        </div>
      </div>
      <div class="lcHdStack">${cards}</div>`;
    },

    bindHealthDeclaration(){
      $$('[data-hd-answer]', this.els.body).forEach(btn => {
        on(btn, 'click', () => {
          const catId = btn.getAttribute('data-hd-answer');
          const value = btn.getAttribute('data-hd-value');
          if(!catId || !value) return;
          this.setHealthCategoryAnswer(catId, value);
          this.render();
        });
      });

      $$('[data-hd-field]', this.els.body).forEach(el => {
        const handler = () => {
          const catId = el.getAttribute('data-hd-field');
          const company = el.getAttribute('data-hd-company');
          const key = el.getAttribute('data-hd-key');
          if(!catId || !company || !key) return;
          this.setHealthField(catId, company, key, (el.type === 'checkbox') ? !!el.checked : el.value);
        };
        on(el, 'input', handler);
        on(el, 'change', handler);
      });

      $$('[data-hd-check]', this.els.body).forEach(el => {
        on(el, 'change', () => {
          const catId = el.getAttribute('data-hd-check');
          const company = el.getAttribute('data-hd-company');
          const key = el.getAttribute('data-hd-key');
          if(!catId || !company || !key) return;
          this.toggleHealthCheckbox(catId, company, key, el.value);
        });
      });
    },
    ...
    validateStep8(){
      const categories = this.getActiveHealthCategories();
      if(!categories.length) return { ok:false, msg:"כדי למלא הצהרת בריאות יש לבחור קודם פוליסה בכלל / הפניקס / הכשרה בשלב 5" };
      for(const cat of categories){
        const state = this.getCategoryState(cat.id);
        if(state.answer !== 'yes' && state.answer !== 'no'){
          return { ok:false, msg:`חובה לענות כן/לא בקטגוריה: ${cat.title}` };
        }
        if(state.answer === 'yes'){
          const activeCompanies = cat.activeCompanies || [];
          for(const company of activeCompanies){
            const fields = ((cat.followups || {})[company] || []);
            if(!fields.length) continue;
            const values = (state.followups || {})[company] || {};
            const hasAny = fields.some(field => {
              const v = values[field.key];
              return Array.isArray(v) ? v.length > 0 : !!safeTrim(v);
            });
            if(!hasAny){
              return { ok:false, msg:`ענית כן בקטגוריה ${cat.title}. יש להשלים לפחות פרט אחד בשאלון של ${this.getHealthCompanyLabel(company)}.` };
            }
          }
        }
      }
