...
(() => {
  ...
  const BUILD = "20260412-vAuditAntiOverwrite";
  ...
  const Storage = {
    ...
    buildStateFromTableCaches(){
      try {
        ...
        const payload = normalizeState({
          meta: this.mapMeta(metaCache?.data || {}),
          agents: Array.isArray(agentsCache?.data) ? agentsCache.data.map((row, idx) => this.mapAgentRow(row, idx)) : [],
          customers: Array.isArray(customersCache?.data) ? customersCache.data.map((row, idx) => this.mapCustomerRow(row, idx)) : [],
          proposals: Array.isArray(proposalsCache?.data) ? proposalsCache.data.map((row, idx) => this.mapProposalRow(row, idx)) : []
        });
        return {
          ok:true,
          payload,
    ...
    async loadSheets(options = {}){
      ...
      try {
        ...
        if(criticalErr) throw new Error(String(criticalErr));
        ...
        return { ok:true, payload, at: payload?.meta?.updatedAt || nowISO(), source:'server' };
      } catch(e) {
        if(useCachedFallback){
    ...
    async saveSheets(state, options = {}){
      try {
        ...
        return { ok:true, at: mergedState.meta.updatedAt || nowISO(), mode:'safe-upsert-only', conflictDetected: !!conflictInfo.detected, mergedState };
      } catch(e) {
        return { ok:false, error: String(e?.message || e) };
      }
    },
...
// open health covers drawer (Health only)
    ...
    renderStep5(){
      ...
      const companyDropdownItems = this.companies.map(c => {
        const src = this.getCompanyLogoSrc(c);
        const selected = (d.company === c);
        const logo = src
          ? `<span class="lcCompanyPicker__optionLogo"><img src="${escapeHtml(src)}" alt="${escapeHtml(c)}" /></span>`
          : `<span class="lcCompanyPicker__optionLogo lcCompanyPicker__optionLogo--fallback">${escapeHtml((c || '•').slice(0,1))}</span>`;
        return `<button type="button" class="lcCompanyPicker__option${selected ? ' is-selected' : ''}" data-co="${escapeHtml(c)}" role="option" aria-selected="${selected ? 'true' : 'false'}">${logo}<span class="lcCompanyPicker__optionText">${escapeHtml(c)}</span></button>`;
      }).join("");
      const selectedCompanyLogoSrc = this.getCompanyLogoSrc(d.company);
      const selectedCompanyLogo = d.company
        ? (selectedCompanyLogoSrc
            ? `<span class="lcCompanyPicker__valueLogo"><img src="${escapeHtml(selectedCompanyLogoSrc)}" alt="${escapeHtml(d.company)}" /></span>`
            : `<span class="lcCompanyPicker__valueLogo lcCompanyPicker__valueLogo--fallback">${escapeHtml((d.company || '•').slice(0,1))}</span>`)
      ...
      const form = `<div class="lcWSection lcPolBuilderSection lcPolBuilderSection--reference">
        ...
        <div class="lcPolForm lcPolForm--premium lcPolForm--reference">
          <div class="lcPolBuilderCard lcPolBuilderCard--reference">
            ...
            <section class="lcPolReferenceBlock lcPolReferenceBlock--details lcPolReferenceBlock--detailsClean lcPolReferenceBlock--flow">
              <div class="lcPolReferenceBlock__head lcPolReferenceBlock__head--details">
                <div class="lcPolReferenceBlock__titleWrap">
                  <div class="lcPolReferenceBlock__title">פרטי הפוליסה החדשה</div>
                  <div class="lcPolReferenceBlock__sub">בחרו חברה ולאחר מכן מלאו רק את השדות הרלוונטיים לפוליסה.</div>
                </div>
                <div class="lcPolReferenceBlock__icon lcPolReferenceBlock__icon--doc" aria-hidden="true">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M8 3.75h6.5l4 4V19a1.75 1.75 0 0 1-1.75 1.75h-8.5A1.75 1.75 0 0 1 6.5 19V5.5A1.75 1.75 0 0 1 8.25 3.75Z"></path>
                    <path d="M14.5 3.75v4h4"></path>
                    <path d="M9 12h6"></path>
              ...
              <div class="lcPolFlowIntro">
                <div class="lcPolFlowIntro__item"><span>1</span><strong>חברה</strong><small>בחירה מתוך רשימה</small></div>
                <div class="lcPolFlowIntro__item"><span>2</span><strong>מוצר</strong><small>מותאם לחברה</small></div>
                <div class="lcPolFlowIntro__item"><span>3</span><strong>פרטי פוליסה</strong><small>שדות רלוונטיים בלבד</small></div>
              </div>

              <div class="lcPolGrid lcPolGrid--top lcPolGrid--mainRow lcPolGrid--reference lcPolGrid--flowTop">
                <div class="lcField lcPolField lcPolField--companyPicker">
                  <label class="lcLabel">חברת ביטוח</label>
                  <div class="lcCompanyPicker${d.company ? ' has-value' : ''}" data-company-picker="1">
                    <button type="button" class="lcCompanyPicker__trigger" data-company-picker-trigger="1" aria-expanded="false">
                      <span class="lcCompanyPicker__triggerMain">${selectedCompanyLogo}<span class="lcCompanyPicker__valueText">${escapeHtml(d.company || 'בחרו חברה מהרשימה')}</span></span>
                      <span class="lcCompanyPicker__triggerArrow" aria-hidden="true">▾</span>
                    </button>
                    <div class="lcCompanyPicker__menu" hidden role="listbox" aria-label="בחירת חברת ביטוח">${companyDropdownItems}</div>
                  </div>
                </div>
      ...
      setTimeout(() => {
        ...
        const applySelectedCompany = (companyName) => {
          this.ensurePolicyDraft();
          const co = safeTrim(companyName);
        ...
        const companyPicker = this.els.body.querySelector('[data-company-picker="1"]');
        const companyPickerTrigger = this.els.body.querySelector('[data-company-picker-trigger="1"]');
        const companyPickerMenu = this.els.body.querySelector('.lcCompanyPicker__menu');
        const setCompanyPickerOpen = (open) => {
          if(!companyPicker || !companyPickerTrigger || !companyPickerMenu) return;
          companyPicker.classList.toggle('is-open', !!open);
          companyPickerTrigger.setAttribute('aria-expanded', open ? 'true' : 'false');
          companyPickerMenu.hidden = !open;
        };
        if(companyPickerTrigger){
          on(companyPickerTrigger, 'click', (ev) => {
            ev.preventDefault();
            ev.stopPropagation();
            setCompanyPickerOpen(!companyPicker?.classList.contains('is-open'));
          });
        }
        if(companyPickerMenu){
          $$('.lcCompanyPicker__option', companyPickerMenu).forEach(btn => {
            on(btn, 'click', (ev) => {
              ev.preventDefault();
              ev.stopPropagation();
              setCompanyPickerOpen(false);
              applySelectedCompany(btn.getAttribute('data-co'));
            });
          });
        }
        on(document, 'click', (ev) => {
          if(!companyPicker?.classList.contains('is-open')) return;
          if(companyPicker.contains(ev.target)) return;
          setCompanyPickerOpen(false);
        }, { once:true });
...
// ---------- Step 6 ----------
    ...
    async saveDraft(){
      ...
      if(!directSave?.ok){
        const msg = 'שמירת ההצעה לשרת נכשלה: ' + safeTrim(directSave?.error || 'SAVE_FAILED');
        this.setHint(msg);
      ...
      if(!verify?.ok || !verify?.data){
        const msg = 'ההצעה לא אומתה מול Supabase לאחר השמירה. לא בוצע אישור סופי. בדוק חיבור לשרת ונסה שוב.';
        this.setHint(msg);
        SaveStatusUI.error('שמירת ההצעה נכשלה', msg);
        return;
      }
      ...
      if(persistRes?.ok){
        ...
        this.setHint("ההצעה נשמרה בשרת אך סנכרון המערכת הכללי נכשל. בדוק חיבור ל-Supabase.");
        SaveStatusUI.error('ההצעה נשמרה חלקית', 'הרשומה נשמרה בשרת, אבל הסנכרון הכללי של המערכת נכשל.');
      }
    },
    ...
    async saveCompletedCustomer(){
      ...
      if(!directSave?.ok) throw new Error('שמירת הלקוח לשרת נכשלה: ' + safeTrim(directSave?.error || 'SAVE_FAILED'));
      ...
      if(!verifiedOnServer){
        ...
        throw new Error('הלקוח לא אומת מול Supabase לאחר השמירה. לא בוצע אישור סופי בשרת. בדוק חיבור ונסה שוב.');
      }
      ...
      const persistRes = { ok:true, skipped:true, at: nowISO() };
