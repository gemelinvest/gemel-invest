...
(() => {
  ...
  const BUILD = "20260325-ops-corrections-v11";
  ...
  function getOpsStatePresentation(rec){
    ...
    if(call?.active){
      ...
      tone = "info";
    } else if(liveKey === "typing"){
      liveLabel = "הלקוח בתהליך הקלדה";
      tone = "warn";
    }
    if(safeTrim(rec?.status) === 'בתהליך הקלדה'){
      liveLabel = 'הלקוח בתהליך הקלדה';
      tone = 'warn';
    }
  ...
  function getCorrectionRequest(rec){
    const ops = ensureOpsProcess(rec);
    const req = ops.correctionRequest && typeof ops.correctionRequest === "object" ? ops.correctionRequest : null;
    return req;
  }

  function isCorrectionPending(rec){
    const req = getCorrectionRequest(rec);
    return !!(req && safeTrim(req.status || 'pending') !== 'resolved' && safeTrim(req.text));
  }

  function isCustomerLockedForAgent(rec){
    if(!rec || Auth?.isOps?.()) return false;
    const call = MirrorsUI?.getCallState?.(rec);
    const ops = ensureOpsProcess(rec);
    const liveState = safeTrim(ops.liveState);
    const status = safeTrim(rec.status);
    if(status === 'בתהליך הקלדה') return true;
    if(call?.active) return true;
    return liveState === 'handling' || liveState === 'in_call';
  }
  // ---------- Config / Local keys ----------
...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    init(){
      ...
      this.els.correctionsModal = $("#customerCorrectionsModal");
      this.els.correctionsBackdrop = $("#customerCorrectionsBackdrop");
      this.els.correctionsClose = $("#customerCorrectionsClose");
      this.els.correctionsBody = $("#customerCorrectionsBody");
      this.els.correctionsFoot = $("#customerCorrectionsFoot");
      ...
      on(this.els.editBtn, "click", (ev) => {
        ...
        const rec = this.current();
        if(!rec) return;
        if(isCustomerLockedForAgent(rec)){
          this.openCorrectionsModal(rec, { mode:'agent' });
          return;
        }
      ...
      on(this.els.correctionsClose, "click", () => this.closeCorrectionsModal());
      on(this.els.correctionsBackdrop, "click", () => this.closeCorrectionsModal());
      on(this.els.correctionsModal, "click", (ev) => {
        const submitBtn = ev.target?.closest?.('[data-corrections-submit]');
        const resolveBtn = ev.target?.closest?.('[data-corrections-resolve]');
        const rec = this.byId(this._correctionsCustomerId) || this.current();
        if(submitBtn){
          if(rec) this.submitCorrectionRequest(rec);
          return;
        }
        if(resolveBtn){
          if(rec) this.resolveCorrectionRequest(rec);
        }
      });
    },

    openCorrectionsModal(rec, options = {}){
      if(!this.els.correctionsModal || !rec) return;
      const mode = safeTrim(options.mode) || (Auth.isOps() ? 'ops' : 'agent');
      const req = getCorrectionRequest(rec) || {};
      const isResolved = safeTrim(req.status || '') === 'resolved';
      const createdMeta = req.createdAt ? `${ProcessesUI.formatDate(req.createdAt)} · ${escapeHtml(req.createdBy || 'נציג')}` : 'טרם נשלחה בקשה';
      const resolvedMeta = isResolved && req.resolvedAt ? `${ProcessesUI.formatDate(req.resolvedAt)} · ${escapeHtml(req.resolvedBy || 'מחלקת תפעול')}` : '';
      const existingText = escapeHtml(req.text || '');
      const customerName = escapeHtml(rec.fullName || 'לקוח');
      const sub = mode === 'agent'
        ? 'תיק הלקוח בטיפול מחלקת תפעול. ניתן לשלוח מכאן בקשת תיקונים לנציג התפעול.'
        : 'כאן רואים את בקשת התיקונים שהנציג שלח, ומסמנים בסיום שהתיקונים בוצעו.';
      this.els.correctionsBody.innerHTML = mode === 'agent' ? `
        <div class="customerCorrectionsState customerCorrectionsState--locked">תיק הלקוח בטיפול מחלקת תפעול</div>
        <div class="customerCorrectionsIntro">הלקוח <strong>${customerName}</strong> נמצא כרגע בתהליך שיקוף/הקלדה ולכן לא ניתן לבצע עריכה ישירה מהתיק.</div>
        <label class="customerCorrectionsField">
          <span>הגש תיקונים לנציג תפעול</span>
          <textarea class="input customerCorrectionsTextarea" id="customerCorrectionsTextarea" placeholder="רשום כאן בדיוק איזה תיקון צריך לבצע בתיק הלקוח...">${existingText}</textarea>
        </label>
        ${req.text ? `<div class="customerCorrectionsMeta">בקשה אחרונה: ${createdMeta}</div>` : ''}
        ${isResolved ? `<div class="customerCorrectionsDone">תיקונים בוצעו${resolvedMeta ? ` · ${resolvedMeta}` : ''}</div>` : ''}
      ` : `
        <div class="customerCorrectionsState customerCorrectionsState--ops ${isResolved ? 'is-done' : ''}">${isResolved ? 'תיקונים בוצעו' : 'נדרש תיקונים'}</div>
        <div class="customerCorrectionsIntro">לקוח: <strong>${customerName}</strong></div>
        <div class="customerCorrectionsRequestBox">
          <div class="customerCorrectionsRequestBox__label">הטקסט שהנציג שלח</div>
          <div class="customerCorrectionsRequestBox__text">${existingText || 'לא נשלח טקסט לתיקון.'}</div>
        </div>
        <div class="customerCorrectionsMeta">נשלח: ${createdMeta}</div>
        ${resolvedMeta ? `<div class="customerCorrectionsMeta customerCorrectionsMeta--done">אושר שבוצע: ${resolvedMeta}</div>` : ''}
      `;
      this.els.correctionsFoot.innerHTML = mode === 'agent'
        ? `<button class="btn" type="button" id="customerCorrectionsCancelBtn">סגור</button><button class="btn btn--primary" type="button" data-corrections-submit="1">שלח תיקונים</button>`
        : `<button class="btn" type="button" id="customerCorrectionsOpsCloseBtn">סגור</button>${(!isResolved && req.text) ? `<button class="btn btn--primary" type="button" data-corrections-resolve="1">אשר תיקונים בוצעו</button>` : ''}`;
      const closeA = this.els.correctionsFoot.querySelector('#customerCorrectionsCancelBtn');
      const closeB = this.els.correctionsFoot.querySelector('#customerCorrectionsOpsCloseBtn');
      if(closeA) closeA.onclick = () => this.closeCorrectionsModal();
      if(closeB) closeB.onclick = () => this.closeCorrectionsModal();
      this._correctionsMode = mode;
      this._correctionsCustomerId = rec.id;
      this.els.correctionsModal.classList.add('is-open');
      this.els.correctionsModal.setAttribute('aria-hidden', 'false');
    },

    closeCorrectionsModal(){
      if(!this.els.correctionsModal) return;
      this.els.correctionsModal.classList.remove('is-open');
      this.els.correctionsModal.setAttribute('aria-hidden', 'true');
      this._correctionsCustomerId = null;
    },

    async submitCorrectionRequest(rec){
      const textarea = this.els.correctionsModal?.querySelector?.('#customerCorrectionsTextarea');
      const text = safeTrim(textarea?.value);
      if(!text){
        alert('יש לרשום את התיקונים שברצונך לשלוח למחלקת התפעול.');
        textarea?.focus?.();
        return;
      }
      setOpsTouch(rec, {
        correctionRequest: {
          ...
          status: 'pending',
          createdAt: nowISO(),
          createdBy: safeTrim(Auth?.current?.name) || 'נציג',
          createdByRole: safeTrim(Auth?.current?.role) || 'agent'
        },
        updatedBy: safeTrim(Auth?.current?.name)
      });
      await App.persist('נשלחה בקשת תיקונים למחלקת תפעול');
      ProcessesUI.render();
      this.closeCorrectionsModal();
      alert('בקשת התיקונים נשלחה למחלקת התפעול.');
    },

    async resolveCorrectionRequest(rec){
      const req = getCorrectionRequest(rec);
      if(!req || !safeTrim(req.text)) return;
      setOpsTouch(rec, {
        correctionRequest: {
          ...req,
          status: 'resolved',
          resolvedAt: nowISO(),
          resolvedBy: safeTrim(Auth?.current?.name) || 'נציג תפעול'
        },
        updatedBy: safeTrim(Auth?.current?.name)
      });
      await App.persist('בקשת תיקונים סומנה כבוצעה');
      ProcessesUI.render();
      this.openCorrectionsModal(rec, { mode:'ops' });
    },
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const ProcessesUI = {
    ...
    mapRecord(rec){
      ...
      const correctionReq = getCorrectionRequest(rec);
      ...
      const pendingCorrection = !!(correctionReq && safeTrim(correctionReq.status || 'pending') !== 'resolved' && safeTrim(correctionReq.text));
      if(pendingCorrection){
        statusText = 'נדרש תיקונים';
        stageText = 'בקשת תיקונים מנציג';
        typeText = 'תפעול / תיקונים';
        tone = 'warn';
        isUrgent = true;
        isDone = false;
      } else if(safeTrim(issuance?.savedAt)){
      ...
      if(!isCorrectionPending(rec) && safeTrim(rec.status).includes("ממתין")) isUrgent = true;
      if(!isCorrectionPending(rec) && safeTrim(rec.status).includes("בוצע")) isDone = true;
      if(!isCorrectionPending(rec) && safeTrim(rec.status).includes("חדש") && !safeTrim(verify?.savedAt) && !ops.finalLabel) tone = "info";
      if(!isCorrectionPending(rec) && safeTrim(ops.resultKey) === 'notInterested') tone = 'danger';
    ...
    render(){
      ...
      UI.els.myProcessesTbody.innerHTML = rows.length ? rows.map(row => {
        ...
        return `<tr class="lcMyProcessesRow" data-tone="${escapeHtml(row.tone)}">
          ...
          <td>${row.statusText === 'נדרש תיקונים' ? `<button class="lcMyProcessesBadge lcMyProcessesBadge--${escapeHtml(row.tone)} lcMyProcessesBadge--action" data-process-fix="${escapeHtml(rec.id)}" type="button">${escapeHtml(row.statusText)}</button>` : `<span class="lcMyProcessesBadge lcMyProcessesBadge--${escapeHtml(row.tone)}">${escapeHtml(row.statusText)}</span>`}</td>
          <td>
      ...
      UI.els.myProcessesTbody.querySelectorAll("[data-process-fix]").forEach(btn => {
        btn.onclick = () => {
          const rec = this.list().find(item => String(item.id) === String(btn.getAttribute('data-process-fix')));
          if(rec) CustomersUI.openCorrectionsModal(rec, { mode:'ops' });
        };
      });
    },
    ...
    openProcess(id){
      ...
      if(!rec) return;
      if(isCorrectionPending(rec)){
        CustomersUI.openCorrectionsModal(rec, { mode:'ops' });
        return;
      }
      UI.goView("mirrors");
      setTimeout(() => {
  ...
  const MirrorsUI = {
    ...
    init(){
      ...
      this.els.opsStatusSelect = $("#mirrorsOpsStatusSelect");
      ...
      on(this.els.opsStatusSelect, "change", () => this.applyOpsStatusSelection());
      on(document, "keydown", (ev) => {
    ...
    syncOpsStatusSelect(rec){
      if(!this.els.opsStatusSelect) return;
      const currentStatus = safeTrim(rec?.status);
      const select = this.els.opsStatusSelect;
      const values = Array.from(select.options).map(o => safeTrim(o.value));
      ...
      select.value = currentStatus || 'בטיפול מחלקת תפעול';
    },

    async applyOpsStatusSelection(){
      if(!Auth.isOps()) return;
      const rec = this.current();
      if(!rec || !this.els.opsStatusSelect) return;
      const value = safeTrim(this.els.opsStatusSelect.value);
      if(!value) return;
      rec.status = value;
      const opsPatch = {
        ownerName: safeTrim(Auth?.current?.name),
      ...
      if(value === 'בתהליך הקלדה') opsPatch.liveState = 'typing';
      else if(value === 'בטיפול מחלקת תפעול') opsPatch.liveState = 'handling';
      setOpsTouch(rec, opsPatch);
      rec.updatedAt = nowISO();
      State.data.meta.updatedAt = rec.updatedAt;
      await App.persist('עודכן סטטוס לקוח בתפעול');
      ProcessesUI.render();
      CustomersUI.refreshOperationalReflectionCard?.();
