...
(() => {
  ...
  const BUILD = "20260324-operational-report-direct-pdf-2";
  ...
  function loadExternalScriptOnce(src){
    ...
    const existingLoaded = Array.from(document.scripts || []).find((script) => safeTrim(script.src) === url && script.dataset.loaded === 'true');
    if(existingLoaded){
    ...
    const pendingExisting = Array.from(document.scripts || []).find((script) => safeTrim(script.src) === url);
    const promise = new Promise((resolve, reject) => {
      ...
      if(pendingExisting){
        pendingExisting.addEventListener('load', () => done(pendingExisting), { once:true });
        pendingExisting.addEventListener('error', fail, { once:true });
        window.setTimeout(() => {
          if(window.jspdf?.jsPDF || window.jsPDF) done(pendingExisting);
        }, 120);
        return;
      }
...
// ---------- Step 6 ----------
    ...
    async exportOperationalPdf(payloadOverride){
      ...
      if(triggerBtn){
        ...
        triggerBtn.textContent = 'טוען מנוע PDF...';
      }
      ...
      try{
        ...
        window.alert('מנוע ה-PDF לא נטען. בדוק חיבור אינטרנט ונסה שוב.');
        if(triggerBtn){
      ...
      if(typeof JsPdfCtor !== 'function'){
        window.alert('מנוע ה-PDF לא זמין כרגע. נסה שוב בעוד רגע.');
        if(triggerBtn){
          triggerBtn.disabled = false;
          triggerBtn.textContent = originalText || 'הורד PDF דוח תפעולי';
        }
