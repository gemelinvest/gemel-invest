...
(() => {
  ...
  // ---------- Login UI helpers (show password / forgot password) ----------
  function wireLoginUi_(){
    ...
    if(toggle && pass){
      toggle.addEventListener('click', () => {
        ...
        toggle.setAttribute('aria-label', isHidden ? 'הסתר סיסמה' : 'הצג סיסמה');
        toggle.title = isHidden ? 'הסתר סיסמה' : 'הצג סיסמה';
      });
    }
  ...
  // Force-check Service Worker updates (fix "stuck old login" issues)
  async function forceSwUpdate_(){
    try{
  ...
  document.addEventListener('DOMContentLoaded', () => {
    try{ wireLoginUi_(); }catch(_){}
    try{ forceSwUpdate_(); }catch(_){}
  });
