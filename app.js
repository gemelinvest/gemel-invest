    ...
    bindInputs(ins){
      ...
      $$("[data-bind]", this.els.body).forEach(el => {
        ...
        const setVal = (doRender=false) => {
          ...
          if(path === "birthDate"){
            // IMPORTANT: do not rerender on partial typing in <input type="date">.
            // Chrome fires "input" while user is still typing (e.g., after month), and rerender would replace DOM and kick focus.
            const full = /^\d{4}-\d{2}-\d{2}$/.test(String(ins.data.birthDate||""));
