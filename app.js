...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    renderPolicyWallet(rec, policies){
      const existingPolicies = (Array.isArray(policies) ? policies : []).filter(p => String(p?.origin || '') !== 'new');
      const newPolicies = this.getNewPoliciesOnly(policies);
      const renderGroup = (title, sub, rows, toneClass) => `
        <section class="customerPolicyGroup ${toneClass}">
          ...
          <div class="customerPolicyList">
            ${rows.length ? rows.map(p => this.renderPolicyRow(p)).join('') : `<div class="customerPolicyList__empty">אין כרגע פוליסות בקבוצה זו.</div>`}
          </div>
        </section>`;

      return `<section class="customerWalletSection customerWalletSection--rows">
        <div class="customerWalletSection__head customerWalletSection__head--rows">
          <div class="customerWalletSection__titleWrap">
            ...
            <div>
              ...
              <div class="customerWalletSection__sub">תצוגת פרימיום דקה ומסודרת — עם הפרדה ברורה בין פוליסות קיימות לחדשות</div>
            </div>
          </div>
        </div>
        ${(existingPolicies.length || newPolicies.length)
          ? `<div class="customerPolicyStack">
              ${renderGroup('פוליסות קיימות', 'פוליסות שהגיעו עם הלקוח', existingPolicies, 'is-existingGroup')}
              ${renderGroup('פוליסות חדשות', 'פוליסות חדשות שנבנו בתיק', newPolicies, 'is-newGroup')}
            </div>`
          : `<div class="emptyState"><div class="emptyState__icon">🧾</div><div class="emptyState__title">עדיין אין פוליסות בתיק</div><div class="emptyState__text">ברגע שיישמרו פוליסות קיימות או חדשות, הן יוצגו כאן אוטומטית.</div></div>`}
      </section>`;
    },
