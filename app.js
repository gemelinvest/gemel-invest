...
`;
  ...
  function renderKV(label, value){
    ...
    // Fix date display (input type="date" stores YYYY-MM-DD)
    ...
    return `<div class="miniStat"><span>${escapeHtml(label)}</span><b>${v}</b></div>`;
                      ...
                      if(kind==='other'){
                        if(l.businessName) bits.push(String(l.businessName).trim());
                        if(l.hp) bits.push('ח.פ. ' + String(l.hp).trim());
                        if(l.businessAddress) bits.push(String(l.businessAddress).trim());
                        if(l.years) bits.push(String(l.years).trim() + ' שנים');
                      }else{
                        if(l.bankName) bits.push(String(l.bankName).trim());
                        if(l.bankNumber) bits.push('בנק ' + String(l.bankNumber).trim());
                        const br = (l.branchNumber || l.branch);
                        if(br) bits.push('סניף ' + String(br).trim());
                        if(l.branchAddress) bits.push(String(l.branchAddress).trim());
                        const amt = asMoney(l.lienAmount);
                        if(amt) bits.push('סכום ' + amt);
                        if(l.years) bits.push(String(l.years).trim() + ' שנים');
                      }
  ...
  function openPrintView(proposal){
    ...
    try {
      ...
      function lienCell(policy){
        ...
        // Back-compat fields
        const kind = String(lien.kind || '').trim() || (lien.hp || lien.businessName ? 'other' : 'bank');
        ...
        if(kind === 'other'){
          const bits = [];
          if(lien.businessName) bits.push(String(lien.businessName).trim());
          if(lien.hp) bits.push('ח.פ. ' + String(lien.hp).trim());
          if(lien.businessAddress) bits.push(String(lien.businessAddress).trim());
          if(lien.years) bits.push(String(lien.years).trim() + ' שנים');
          return bits.length ? esc(bits.join(' • ')) : '—';
        }

        // bank
        const bits = [];
        if(lien.bankName) bits.push(String(lien.bankName).trim());
        if(lien.bankNumber) bits.push('בנק ' + String(lien.bankNumber).trim());
        const br = (lien.branchNumber || lien.branch);
        if(br) bits.push('סניף ' + String(br).trim());
        if(lien.branchAddress) bits.push(String(lien.branchAddress).trim());
        const amt = asMoney(lien.lienAmount);
        if(amt) bits.push('סכום ' + amt);
        if(lien.years) bits.push(String(lien.years).trim() + ' שנים');
        return bits.length ? esc(bits.join(' • ')) : '—';
      }
      ...
      // Logo (use the same system logo asset)
      const logoHtml = `
        <div class="logoWrap">
          <img class="brandLogo" src="assets/logo-login-clean.png" alt="GEMEL INVEST" />
      ...
      <head>
        ...
        <style>
          ...
          *{ box-sizing:border-box; }
          ...
          .logoWrap{ text-align:right; display:flex; flex-direction:column; align-items:flex-end; gap:6px; }
          .brandLogo{ height:38px; width:auto; object-fit:contain; display:block; }
          .logoSub{ font-size:11px; color:var(--muted); }
