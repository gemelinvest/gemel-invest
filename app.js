...
(() => {
  ...
  const BUILD = "20260420-vRemoveOpsNavTools-v1";
  ...
  function getOpsStatePresentation(rec){
    ...
    if(call?.active && call?.startedAt){
      ...
      timerText = `${mm}:${ss}`;
      timerMeta = `בשיחה החל מ־${call.startedAt || '—'}`;
      timerLive = true;
      ...
      timerMeta = `שיחה אחרונה בתאריך ${call.startedAt || '—'}`;
    }
...
UsersGateUI.init();
    ...
    goView(view){
      ...
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-customers-active","view-proposals-active");
    ...
    renderAuthPill(){
      ...
      if(Auth.current) {
        ...
        txt.innerHTML = `<span class="lcUserPill__avatar">${_pillInitials}</span><span class="lcUserPill__nameWrap"><span class="lcUserPill__name">${escapeHtml(_pillName)}</span><span class="lcUserPill__role">${escapeHtml(_pillRole)}</span></span>`;
  ...
  const CustomersUI = {
    ...
    renderOperationalReflectionCard(state){
      ...
      const updated = safeTrim(state?.updatedText) ? (() => { try { return new Date(state.updatedText).toLocaleString('he-IL'); } catch(_e) { return state.updatedText; } })() : '—';
    ...
    render(){
      ...
      UI.els.proposalsTbody.innerHTML = rows.length ? rows.map(rec => `
        <tr>
          ...
          <td><span class="badge lcProposalBadge lcProposalBadge--step${Number(rec.currentStep||1) >= 8 ? '-done' : Number(rec.currentStep||1) >= 5 ? '-mid' : '-open'}">שלב ${escapeHtml(String(rec.currentStep || 1))} מתוך 9</span></td>
      ...
      this.els.root.innerHTML = `
        <section class="bankDash bankDash--cleanTop">
          ...
          <article class="bankGoal card bankGoal--${metrics.targetTone}">
            ...
            <div class="bankGoal__foot">${metrics.targetValue > 0 ? `היעד מנוהל במסך ניהול המשתמשים עבור המשתמש המחובר` : `טרם הוגדר יעד חודשי על ידי מנהל המערכת`}</div>
          </article>
...
init(){
      ...
      on(this.els.btnPreviewOpsReport, "click", () => {
        if(!this.hasOperationalAgentNumbersComplete()) return;
    ...
    handleHealthDelegatedClick(ev){
      if(Number(this.step) !== 8 || !this.els.body) return;
    ...
    handleStepEntry(fromStep, toStep){
      ...
      if(to === 9 && from !== 9){
        this._operationalGuideAccepted = false;
        setTimeout(() => this.openOperationalGuideModal(), 120);
      }
    },
    ...
    openOperationalGuideModal(){
      if(!this.els.opGuideModal || !this.isOpen || Number(this.step) !== 9) return;
    ...
    renderBody(){
      ...
      const hideStepHeader = (this.step === 1 || this.step === 5 || this.step === 9);
      const head = hideStepHeader ? "" : (isCaseLevel ? `<div class="lcWSection lcSummaryHeaderSection">
        <div class="row row--between lcSummaryHeaderRow">
          ...
          <div class="lcSummaryHeaderText">
            ...
            <div class="muted small">${this.step === 9 ? 'בדיקה אחרונה לפני שמירת הלקוח והכנת הדוח התפעולי' : ''}</div>
          </div>
        </div>
...
// open health covers drawer (Health only)
      ...
      const rows = (d.existingPolicies || []).map(p => {
        ...
        if(p.importedFromHarBituach){
          ...
          const breakdownRows = premiumBreakdown.length
            ? premiumBreakdown.map(item => `<span class="lcPolCard__breakdownItem">${escapeHtml(safeTrim(item.label) || 'כיסוי')} ${escapeHtml(safeTrim(item.monthlyPremium) || '0.00')} ₪</span>`).join('')
            : '';
                ...
                <div class="lcPolCard" data-policy-id="${escapeHtml(p.id || '')}">
                  <div class="lcPolCard__top">
                    ...
                    <div class="lcPolCard__premium">
                      ...
                      ${breakdownRows ? `<div class="lcPolCard__breakdown">${breakdownRows}</div>` : ''}
                    </div>
                  </div>
    ...
    updatePolicyDiscountPreview(){
      ...
      this.els.policyDiscountPreview.innerHTML = `פרמיה אחרי הנחה: <b>${escapeHtml(this.formatMoneyValue(after))}</b>${scheduleText}${benefitLine}`;
    },
    ...
    renderStep5(){
      ...
      const body1 = `<div class="lcField">
        <select class="lcSelect lcNpCompanySelect" data-pdraft="company">
      ...
      const coverCheckboxes = allCovers.map(c => {
      ...
      setTimeout(() => {
        // company select
        const companySelect = this.els.body.querySelector('.lcNpCompanySelect');
        if(companySelect){
...
// ---------- Step 6 ----------
        ...
        <section class="lcOpSummary">
          ...
          <section class="lcOpSection lcOpSection--policies">
            <div class="lcOpSection__head">
              <div>
                ...
                <div class="lcOpSection__sub">תחילה מוצגות הפוליסות הקיימות, מקובצות לפי מבוטח, עם סטטוס הטיפול שנבחר.</div>
              </div>
            </div>
          ...
          ${(() => {
            ...
            return `<section class="lcOpSection lcOpSection--needsAnalysis">
              <div class="lcOpSection__head">
                <div>
                  <div class="lcOpSection__title">שיקולי התאמת צרכים</div>
                  <div class="lcOpSection__sub">נימוקי ושיקולי ביטול הפוליסות כפי שנרשמו על ידי הנציג בשלב התאמת הצרכים.</div>
                </div>
              </div>
          ...
          <section class="lcOpSection lcOpSection--policies lcOpSection--newPolicies">
            <div class="lcOpSection__head">
              <div>
                <div class="lcOpSection__title">פוליסות חדשות שנרכשו דרך GEMEL INVEST</div>
                <div class="lcOpSection__sub">לאחר הקיימות, מוצגות הפוליסות החדשות שנרכשו בתהליך ההקמה, גם הן מקובצות לפי מבוטח.</div>
              </div>
            </div>
    ...
    async saveDraft(){
      ...
      SaveStatusUI.show('loading', 'שומר הצעה לשרת…', 'המערכת מעדכנת עכשיו את נתוני ההצעה ב-Supabase.');
      ...
      if(persistRes?.ok){
        ...
        if(finalWarning){
          ...
          SaveStatusUI.success('ההצעה נשמרה בהצלחה בשרת', 'ההצעה אומתה מול Supabase ותופיע להמשך עריכה מכל עמדה.');
        }
      }else{
    ...
    updateOperationalPreviewButtonState(options = {}){
      ...
      if(!btn) return;
      const isStepNine = !this.isElementaryFlow() && Number(this.step) === 9;
      ...
      const ready = this.hasOperationalAgentNumbersComplete();
      btn.disabled = !ready;
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const pageTwoSections = [
        ...
        companyAgentRows.length ? `<section class="lcPdfSection"><div class="lcPdfSection__title">מספרי סוכן לחברות</div>${renderClassicTable(['חברה','מספר סוכן'], companyAgentRows)}</section>` : ''
      ].filter(Boolean).join('');
      ...
      if(stepId === 3){
        ...
        for(const p of list){
          ...
          if(c.status === "partial_health"){
            if(!safeTrim(c.partialDetails || c.partialText || c.annexText || c.partialReason || "")) return false;
          }
        }
        return true;
      }
...
(function(){
  ...
  async function checkForSystemUpdate(){
    ...
    try{
      ...
      const buildLabel = window.__GI_BUILD || '';
      if(buildLabel && !text.includes('const BUILD = "' + buildLabel + '";')){
        if(isSensitiveWorkflowOpen()) return;
        openUpdatePopup();
      }
    }catch(_err){}
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => { checkForSystemUpdate(); }, 12000);
    setInterval(checkForSystemUpdate, 60000);
  });
})();
