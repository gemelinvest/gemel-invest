  ...
  out.agents = (base.agents || []).map((a, idx) => ({
    ...
    out.customers = (out.customers || []).map((c) => ({
      ...
      policies: Array.isArray(c?.policies) ? c.policies.map((p) => ({
        ...
        // Existing policies wizard (new customer): action + partial cancel notes
        statusAction: safeTrim(p?.statusAction),
        partialCancelText: safeTrim(p?.partialCancelText) || safeTrim(p?.partialCancelAddons),
...
// Search (live filter on customers view)
      ...
      on(this.els.customerForm, "submit", async (e) => {
        ...
        // Existing Policies (wizard step)
        let policiesFromWizard = [];
        try{
          const raw = safeTrim(fd.get("policiesJson"));
          if(raw){
            const parsed = JSON.parse(raw);
            if(Array.isArray(parsed)) policiesFromWizard = parsed;
          }
        }catch(_){ policiesFromWizard = []; }

        // normalize policies to legacy schema + keep action fields
        const normPolicies = (policiesFromWizard || []).filter(Boolean).map(p => {
          const statusAction = safeTrim(p.statusAction);
          let status = normPolicyStatus(p.status) || "active";
          if(statusAction === "cancel_full") status = "cancelled";
          else if(statusAction === "cancel_partial") status = "pending_cancel";
          return {
            id: safeTrim(p.id) || ("p_" + uid()),
            company: safeTrim(p.company),
            type: safeTrim(p.type),
        ...
        if (editId) {
          ...
          // keep policies from wizard step
          c.policies = Array.isArray(normPolicies) ? normPolicies : (c.policies || []);
          c.updatedAt = nowISO();
        ...
        const customer = {
          ...
          monthlyPremium: 0,
          notes: "",
          policies: Array.isArray(normPolicies) ? normPolicies : [],
          createdAt: nowISO(),
          updatedAt: nowISO()
        };
...
this.renderPolicies();
      ...
      setupCustomerPoliciesStep(this);
    ...
    openCustomerEdit(customerId){
      ...
      try { this.customerPoliciesUX && this.customerPoliciesUX.set && this.customerPoliciesUX.set(c.policies || []); } catch(_){ }
      try { this.customerPoliciesUX && this.customerPoliciesUX.reset && this.customerPoliciesUX.reset(); } catch(_){ }
      const set = (name, val) => {
...
async saveUserEditFromModal() {
  ...
  // ---------------------------
  // New Customer Wizard: Existing Policies step (after profile)
  // ---------------------------
  function setupCustomerPoliciesStep(UI){
    try{
      const form = UI && UI.els && UI.els.customerForm;
      if(!form) return;

      const companyEl = document.querySelector("#custPolicyCompany");
      const typeEl = document.querySelector("#custPolicyType");
      const numEl = document.querySelector("#custPolicyNumber");
      const statusEl = document.querySelector("#custPolicyStatus");
      const partialWrap = document.querySelector("#custPolicyPartialWrap");
      const partialText = document.querySelector("#custPolicyPartialText");
      const addBtn = document.querySelector("#btnAddExistingPolicy");
      const listEl = document.querySelector("#custPoliciesList");
      const hidden = form.querySelector("#policiesJson") || form.querySelector('input[name="policiesJson"]');

      if(!companyEl || !typeEl || !numEl || !statusEl || !addBtn || !listEl || !hidden) return;

      const INSURERS = [
        "הראל","הפניקס","כלל","מגדל","מנורה מבטחים","איילון","AIG","הכשרה","ביטוח ישיר","ליברה","שלמה ביטוח","שירביט","אחר"
      ];
      const INS_TYPES = [
        "חיים","בריאות","פנסיה","ביטוח מנהלים","גמל","השתלמות","אובדן כושר עבודה","סיעוד","משכנתא","דירה","רכב","נסיעות לחו״ל","עסקים","אחר"
      ];
      const STATUS_ACTIONS = [
        { v:"no_change", label:"ללא שינוי" },
        { v:"agent_appointment", label:"מינוי סוכן" },
        { v:"cancel_full", label:"ביטול מלא" },
        { v:"cancel_partial", label:"ביטול חלקי" }
      ];

      const escape = (s) => escapeHtml ? escapeHtml(s) : String(s ?? "");
      const toLabel = (v) => (STATUS_ACTIONS.find(x => x.v === v)?.label) || "";

      function fillSelect(el, items, placeholder){
        el.innerHTML = [
          `<option value="">${escape(placeholder || "—")}</option>`,
          ...items.map(x => `<option value="${escape(x)}">${escape(x)}</option>`)
        ].join("");
      }
      ...
      function fillStatus(el){
        el.innerHTML = [
          ...
          ...STATUS_ACTIONS.map(x => `<option value="${escape(x.v)}">${escape(x.label)}</option>`)
        ].join("");
      }

      fillSelect(companyEl, INSURERS, "— בחר חברה —");
      fillSelect(typeEl, INS_TYPES, "— בחר סוג ביטוח —");
      fillStatus(statusEl);

      UI._custPolicies = Array.isArray(UI._custPolicies) ? UI._custPolicies : [];

      function syncHidden(){
        try{ hidden.value = JSON.stringify(UI._custPolicies || []); }catch(_){}
      }

      function render(){
        const arr = Array.isArray(UI._custPolicies) ? UI._custPolicies : [];
        if(!arr.length){
          listEl.innerHTML = `<div class="muted small">עדיין לא הוספת פוליסות.</div>`;
          syncHidden();
          return;
        }
        listEl.innerHTML = arr.map(p => {
          const title = `${escape(p.company || "")} • ${escape(p.type || "")}`;
          const num = escape(p.policyNumber || "");
          const st = escape(toLabel(p.statusAction || ""));
          const note = (p.statusAction === "cancel_partial" && p.partialCancelText) ? escape(p.partialCancelText) : "";
          return `
            <div class="lcPolicyCard" data-pid="${escape(p.id)}">
              <div class="lcPolicyCard__top">
                <div>
                  <div class="lcPolicyCard__title">${title}</div>
                  <div class="lcPolicyCard__meta">
                    <span class="lcPolicyPill">מס׳ פוליסה: <strong>${num || "—"}</strong></span>
                    <span class="lcPolicyPill">סטטוס: <strong>${st || "—"}</strong></span>
                  </div>
                </div>
                <button type="button" class="btn lcPolicyRemoveBtn" data-remove-policy="${escape(p.id)}">הסר</button>
              </div>
              ${note ? `<div class="lcPolicyCard__note"><strong>נספחים לביטול:</strong> ${note}</div>` : ``}
            </div>
          `;
        }).join("");
        syncHidden();
      ...
      function setPolicies(policies){
        ...
        UI._custPolicies = arr.map(p => ({
          ...
          statusAction: safeTrim(p?.statusAction),
          partialCancelText: safeTrim(p?.partialCancelText) || safeTrim(p?.partialCancelAddons),
          // keep canonical status for legacy screens
          status: normPolicyStatus(p?.status) || "active",
          premium: Number(p?.premium || 0),
          renewAt: safeTrim(p?.renewAt),
          cancelReason: safeTrim(p?.cancelReason),
      ...
      function getPolicies(){
        return Array.isArray(UI._custPolicies) ? UI._custPolicies : [];
      }
      ...
      function clearInputs(){
        ...
        numEl.value = "";
        statusEl.value = "";
        if(partialText) partialText.value = "";
        if(partialWrap) partialWrap.style.display = "none";
      }

      on(statusEl, "change", () => {
        const v = safeTrim(statusEl.value);
        if(partialWrap){
          partialWrap.style.display = (v === "cancel_partial") ? "" : "none";
        }
      });
      ...
      on(addBtn, "click", () => {
        ...
        const policyNumber = safeTrim(numEl.value);
        const statusAction = safeTrim(statusEl.value);

        if(!company || !type || !policyNumber || !statusAction){
          notify("נא למלא חברה, סוג ביטוח, מספר פוליסה וסטטוס.", "warn");
          return;
        }

        let partialCancelTextVal = "";
        if(statusAction === "cancel_partial"){
          partialCancelTextVal = safeTrim(partialText?.value);
          if(!partialCancelTextVal){
            notify("בביטול חלקי חובה לרשום אילו נספחים לבטל.", "warn");
            try{ partialText?.focus?.(); }catch(_){}
            return;
          }
        }

        // map action to canonical status (for existing screens)
        let status = "active";
        if(statusAction === "cancel_full") status = "cancelled";
        else if(statusAction === "cancel_partial") status = "pending_cancel";

        UI._custPolicies.unshift({
          id: "p_" + uid(),
          company,
          type,
          policyNumber,
          statusAction,
          partialCancelText: partialCancelTextVal,
          status,
          premium: 0,
          renewAt: "",
          cancelReason: (statusAction === "cancel_full") ? "ביטול מלא" : (statusAction === "cancel_partial") ? "ביטול חלקי" : "",
          cancelTemplate: "",
          pendingCancelAt: "",
          cancelledAt: "",
          swappedAt: ""
      ...
      on(listEl, "click", (e) => {
        const btn = e?.target?.closest?.("[data-remove-policy]");
        if(!btn) return;
        const id = safeTrim(btn.dataset.removePolicy);
        UI._custPolicies = (UI._custPolicies || []).filter(p => safeTrim(p.id) !== id);
        render();
      });

      // expose API (used by openModal/openCustomerEdit and submit)
      UI.customerPoliciesUX = {
        set(policies){ setPolicies(policies); },
