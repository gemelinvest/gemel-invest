...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  async function apiCall(action, payload){
    ...
    // IMPORTANT: use a "simple" request to avoid CORS preflight.
    // Apps Script Web Apps do not reliably answer OPTIONS preflight, so do NOT use application/json here.
    ...
    // Try JSON first; if Apps Script returns HTML/error, surface it.
    try{
      const data = JSON.parse(txt);
      return data;
    }catch(_){
      return { ok:false, error:'NON_JSON_RESPONSE', status: res.status, body: (txt||'').slice(0,500) };
    }
  }
