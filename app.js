...
function escapeHtml(s){
  ...
  const Wizard = {
    ...
    render(){
      ...
      // Preserve focus & caret across re-renders (prevents "one digit then blur" issues)
      let focusMeta = null;
      try{
        const ae = document.activeElement;
        if(ae && this.els.wrap.contains(ae)){
          focusMeta = {
            id: ae.id || null,
            bind: (ae.getAttribute && ae.getAttribute("data-bind")) ? ae.getAttribute("data-bind") : null,
            name: (ae.getAttribute && ae.getAttribute("name")) ? ae.getAttribute("name") : null,
            selStart: (typeof ae.selectionStart === "number") ? ae.selectionStart : null,
            selEnd: (typeof ae.selectionEnd === "number") ? ae.selectionEnd : null
          };
        }
      }catch(_e){ focusMeta = null; }
      ...
      if(focusMeta){
        try{
          ...
          if(!el && focusMeta.bind) el = this.els.wrap.querySelector('[data-bind="' + cssAttrEscape(focusMeta.bind) + '"]');
          if(!el && focusMeta.name) el = this.els.wrap.querySelector('[name="' + cssAttrEscape(focusMeta.name) + '"]');
          if(el && !el.disabled){
            el.focus({ preventScroll:true });
            if(typeof el.setSelectionRange === "function" && focusMeta.selStart !== null && focusMeta.selEnd !== null){
              const len = (el.value || "").length;
              el.setSelectionRange(Math.min(focusMeta.selStart, len), Math.min(focusMeta.selEnd, len));
            }
          }
        }catch(_e){}
      }
    ...
    bindInputs(ins){
      ...
      // new policy discount modal (Step 5)
      $$("[data-discount-open]", this.els.body).forEach(btn => {
        on(btn, "click", () => {
          const pid = btn.getAttribute("data-discount-open");
          this.openDiscountModal(pid);
        });
      });

      const discModal = $("#lcDiscountModal", this.els.body);
      if(discModal){
        const closeBtn = $("#lcDiscountClose", discModal);
        const cancelBtn = $("#lcDiscountCancel", discModal);
        const saveBtn = $("#lcDiscountSave", discModal);

        on(closeBtn, "click", () => this.closeDiscountModal());
        on(cancelBtn, "click", () => this.closeDiscountModal());
        on(saveBtn, "click", () => this.saveDiscountModal());

        // click backdrop
        on(discModal, "click", (e) => {
          const t = e.target;
          if(t && t.getAttribute && t.getAttribute("data-close") === "1") this.closeDiscountModal();
        });
      }
            ...
            <td>
              ${isRisk ? `
                ...
                ${p.hasPledge ? `
                  ...
                  ${p.bankAgency ? `
                    <input class="input" style="margin-top:6px" data-bind="existingPolicies.${p.id}.bankAgencyName" value="${escapeHtml(p.bankAgencyName||"")}" placeholder="שם סוכנות בנק" />
                  `:""}
                `:""}
        ...
        const setVal = () => {
          ...
          if(path.startsWith("existingPolicies.") || path.startsWith("newPolicies.")){
            ...
            if(ok){
              ...
              if(path.endsWith(".hasPledge") || path.endsWith(".bankAgency")) this.render();
        ...
        return `
          <div class="lcWSection">
            <div class="row row--between">
              ...
              ${pledgedBank ? `<span class="lcWBadge"><span class="lcLamp yellow"></span>שיעבוד דרך סוכנות בנק · ייווצרו 2 מכתבים</span>` : ``}
            </div>
    ...
    addNewPolicy(ins){
      const p = {
        ...
        beneficiariesMode:"heirs", // heirs/manual
        beneficiaries:[]
      };
    ...
    openDiscountModal(pid){
      this._discountPid = pid;
      const modal = $("#lcDiscountModal", this.els.body);
      if(!modal) return;
      const p = (this.getActive()?.data?.newPolicies || []).find(x => x.id === pid);
      const pctSel = $("#lcDiscountPct", modal);
      const yearsInp = $("#lcDiscountYears", modal);
      if(pctSel) pctSel.value = String(p?.discountPct || "");
      if(yearsInp) yearsInp.value = String(p?.discountYears || "");
      modal.classListrenderStep5(ins){
      const d = ins.data;
      const list = (d.newPolicies || []);
      // compute derived premiums
      list.forEach(p => this.calcPremium(p));

      const compOptsAll = this.companies;
      const typeOptsAll = this.insTypes;

      const rows = list.map(p => {
        const compOpts = compOptsAll.map(x => `<option value="${escapeHtml(x)}"${p.company===x?" selected":""}>${escapeHtml(x)}</option>`).join("");
        const typeOpts = typeOptsAll.map(x => `<option value="${escapeHtml(x)}"${p.type===x?" selected":""}>${escapeHtml(x)}</option>`).join("");
        const isRisk = (p.type === "ריסק" || p.type === "ריסק משכנתא");
        const isCI = (p.type === "מחלות קשות" || p.type === "סרטן");

        const discLabel = (p.discountPct && p.discountYears)
          ? `הנחה ${escapeHtml(String(p.discountPct))}% · ${escapeHtml(String(p.discountYears))} שנים`
          : (p.discountPct ? `הנחה ${escapeHtml(String(p.discountPct))}%` : "");

        const discBtnText = discLabel ? "ערוך הנחה" : "➕ הוסף הנחה";
        ...
        return `
          <tr>
            ...
            <td>
              <input class="input" data-bind="newPolicies.${p.id}.premiumBefore" value="${escapeHtml(p.premiumBefore||"")}" inputmode="decimal" placeholder="₪" />
            </td>
            <td>
              <div class="row" style="gap:10px; flex-wrap:wrap">
                <button class="btn lcSmallBtn" type="button" data-discount-open="${p.id}">${discBtnText}</button>
                ${discLabel ? `<span class="badge">${discLabel}</span>` : `<span class="muted small">ללא הנחה</span>`}
              </div>
            </td>
            <td><input class="input" value="${p.premiumAfter===null?"":escapeHtml(String(p.premiumAfter))}" disabled /></td>
            <td><input class="input" value="${p.annualAfter===null?"":escapeHtml(String(p.annualAfter))}" disabled /></td>
            <td>
              ${isCI ? `<input class="input" data-bind="newPolicies.${p.id}.compensation" value="${escapeHtml(p.compensation||"")}" placeholder="סכום פיצוי (₪)" />`
              : isRisk ? `<input class="input" data-bind="newPolicies.${p.id}.sumInsured" value="${escapeHtml(p.sumInsured||"")}" placeholder="סכום ביטוח (₪)" />`
              : `<span class="muted small">—</span>`}
            </td>
          ...
          <tr>
            <td colspan="8" style="padding-top:6px; padding-bottom:10px">
              <details class="lcWMore">
                <summary>פרטים נוספים</summary>
                <div class="lcWGrid" style="margin-top:10px">
                  <div class="field">
                    <label class="label">שיעבוד (מוטב בלתי חוזר)</label>
                    <label class="row" style="gap:10px">
                      <input type="checkbox" data-bind="newPolicies.${p.id}.pledge"${p.pledge ? " checked":""} />
                      <span>יש שיעבוד</span>
                    </label>
                    <div class="help">אם מסומן – ייפתח טופס פרטי הבנק המשעבד.</div>
                  </div>

                  ${p.pledge ? `
                    <div class="field">
                      <label class="label">שם בנק</label>
                      <select class="input" data-bind="newPolicies.${p.id}.pledgeBank.bankName">
                        <option value="">בחר…</option>
                        ${this.bankNames.map(x => `<option value="${escapeHtml(x)}"${p.pledgeBank?.bankName===x?" selected":""}>${escapeHtml(x)}</option>`).join("")}
                      </select>
                    </div>
                    <div class="field">
                      <label class="label">מספר בנק</label>
                      <input class="input" data-bind="newPolicies.${p.id}.pledgeBank.bankNo" value="${escapeHtml(p.pledgeBank?.bankNo||"")}" inputmode="numeric" />
                    </div>
                    <div class="field">
                      <label class="label">מספר סניף</label>
                      <input class="input" data-bind="newPolicies.${p.id}.pledgeBank.branch" value="${escapeHtml(p.pledgeBank?.branch||"")}" inputmode="numeric" />
                    </div>
                    <div class="field">
                      <label class="label">סכום לשיעבוד</label>
                      <input class="input" data-bind="newPolicies.${p.id}.pledgeBank.amount" value="${escapeHtml(p.pledgeBank?.amount||"")}" inputmode="decimal" placeholder="₪" />
                    </div>
                    <div class="field">
                      <label class="label">לכמה שנים</label>
                      <input class="input" data-bind="newPolicies.${p.id}.pledgeBank.years" value="${escapeHtml(p.pledgeBank?.years||"")}" inputmode="numeric" />
                    </div>
                    <div class="field" style="grid-column: 1 / -1;">
                      <label class="label">כתובת הבנק</label>
                      <input class="input" data-bind="newPolicies.${p.id}.pledgeBank.address" value="${escapeHtml(p.pledgeBank?.address||"")}" placeholder="עיר, רחוב, מספר" />
                    </div>
                  ` : ``}

                  <div class="field">
                    <label class="label">מוטבים</label>
                    <select class="input" data-bind="newPolicies.${p.id}.beneficiariesMode">
                      <option value="heirs"${p.beneficiariesMode==="heirs"?" selected":""}>יורשים</option>
                      <option value="manual"${p.beneficiariesMode==="manual"?" selected":""}>הוספה ידנית</option>
                    </select>
                    ${p.beneficiariesMode==="manual" ? `
                      <div class="help">הוספה ידנית תורחב בהמשך (רשימת מוטבים + אחוזים).</div>
                    ` : `<div class="help">ברירת מחדל: "יורשים".</div>`}
                  </div>
                </div>
              </details>
            </td>
      ...
      const pctOptions = [15,20,30,45,65].map(n => `<option value="${n}">${n}%</option>`).join("");
      ...
      return `
        <div class="lcWSection">
          <div class="row row--between">
            <div>
              ...
              <div class="muted small">הוספה נוחה וברורה בשורת טבלה (כמו פוליסות קיימות)</div>
            </div>
        ...
        ` : `<div class="lcWSection"><div class="muted">אין פוליסות חדשות עדיין.</div></div>`}

        <!-- Discount Modal (Step 5 only) -->
        <div class="lcMiniModal" id="lcDiscountModal" aria-hidden="true">
          <div class="lcMiniModal__backdrop" data-close="1"></div>
          <div class="lcMiniModal__panel" role="dialog" aria-modal="true" aria-label="הוסף הנחה">
            <div class="lcMiniModal__head">
              <div class="lcMiniModal__title">הוסף הנחה</div>
              <button class="iconBtn" id="lcDiscountClose" type="button" aria-label="סגור">✕</button>
            </div>
            <div class="lcMiniModal__body" style="display:grid; gap:10px">
              <div class="field">
                <label class="label">אחוז הנחה</label>
                <select class="input" id="lcDiscountPct">
                  <option value="">בחר…</option>
                  ${pctOptions}
                </select>
              </div>
              <div class="field">
                <label class="label">דירוג ההנחה בשנים</label>
                <input class="input" id="lcDiscountYears" inputmode="numeric" placeholder="לדוגמה: 3" />
              </div>
              <div class="help">לאחר שמירה — יוצגו מחיר לפני ואחרי בשורת הפוליסה.</div>
            </div>
            <div class="lcMiniModal__foot" style="display:flex; gap:10px; justify-content:flex-start">
              <button class="btn btn--primary" id="lcDiscountSave" type="button">שמור</button>
              <button class="btn" id="lcDiscountCancel" type="button">ביטול</button>
            </div>
          </div>
        </div>
      `;
    },   </div>
              <div class="help">הבנק ייחשב "מוטב בלתי חוזר". יתרה למוטבים תתווסף בשלב הבא.</div>
            ` : ""}

            ${isRisk ? `
              <div class="divider"></div>
              <div class="lcWTitle" style="margin-bottom:8px">מוטבים (ריסק רגיל)</div>
              <div class="lcWGrid">
                <div class="field">
                  <label class="label">בחירה</label>
                  <select class="input" data-bind="newPolicies.${p.id}.beneficiariesMode">
                    <option value="heirs" ${p.beneficiariesMode==="heirs"?"selected":""}>יורשים חוקיים</option>
                    <option value="manual" ${p.beneficiariesMode==="manual"?"selected":""}>הוספה ידנית</option>
                  </select>
                </div>
                <div class="field">
      ...
      return `
        <div class="lcWSection">
          <div class="row row--between">
            <div>
              <div class="lcWTitle">פוליסות חדשות</div>
              <div class="muted small">ללא מספר פוליסה וללא סטטוס ביטול</div>
            </div>
            <button class="btn" id="lcAddNewPolicy" type="button">➕ הוסף פוליסה חדשה</button>
          </div>
        </div>
    ...
    isStepCompleteForInsured(stepId, ins){
      ...
      if(stepId === 3){
        // policies optional; step complete always
        return true;
      }
    ...
    fieldDate(label, bind, value){
      // NOTE: dir=ltr prevents RTL date-input typing glitches on some browsers
      return `<div class="field">
        <label class="label">${escapeHtml(label)}</label>
        <input class="input" type="date" dir="ltr" data-bind="${escapeHtml(bind)}" value="${escapeHtml(value||"")}" />
      </div>`;
    },
