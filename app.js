...
// open health covers drawer (Health only)
    ...
    renderStep5(){
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        const pledgeText = (!isMedicare && (p.type === "ריסק" || p.type === "ריסק משכנתא") && p.pledge) ? "כן" : "לא";
        return `<div class="lcPolCard lcPolCard--premium" data-pol="${p.id}">
          <div class="lcPolCard__top">
            ...
            <div class="lcPolSummaryBar">
              <div class="lcPolSummaryBar__item">
                <span class="lcPolSummaryBar__k">פרמיה</span>
                <strong class="lcPolSummaryBar__v">${escapeHtml(p.premiumMonthly || "—")}</strong>
              </div>
              <div class="lcPolSummaryBar__item">
                <span class="lcPolSummaryBar__k">תחילת ביטוח</span>
                <strong class="lcPolSummaryBar__v">${escapeHtml(p.startDate || "—")}</strong>
              </div>
            </div>
          </div>
          <div class="lcPolMetaGrid">
            <div class="lcPolMetaItem"><span class="lcPolMetaItem__k">חברה</span><strong class="lcPolMetaItem__v">${escapeHtml(p.company || "—")}</strong></div>
            <div class="lcPolMetaItem"><span class="lcPolMetaItem__k">מוצר</span><strong class="lcPolMetaItem__v">${escapeHtml(isMedicare ? "מדיקר" : (p.type || "—"))}</strong></div>
            <div class="lcPolMetaItem"><span class="lcPolMetaItem__k">פרמיה חודשית</span><strong class="lcPolMetaItem__v">${escapeHtml(p.premiumMonthly || "—")}</strong></div>
            <div class="lcPolMetaItem"><span class="lcPolMetaItem__k">תחילת ביטוח</span><strong class="lcPolMetaItem__v">${escapeHtml(p.startDate || "—")}</strong></div>
            ${sumValue ? `<div class="lcPolMetaItem"><span class="lcPolMetaItem__k">${sumLabel}</span><strong class="lcPolMetaItem__v">${escapeHtml(sumValue)}</strong></div>` : ``}
            ${(!isMedicare && (p.type === "ריסק" || p.type === "ריסק משכנתא")) ? `<div class="lcPolMetaItem"><span class="lcPolMetaItem__k">שיעבוד</span><strong class="lcPolMetaItem__v">${pledgeText}</strong></div>` : ``}
          </div>
          ...
          <div class="lcPolBuilderCard">
            <div class="lcPolBuilderCard__head">
              <div>
                ...
                <div class="lcPolBuilderCard__title">פרטי הפוליסה החדשה</div>
              </div>
            ...
            <div class="lcPolBuilderActions">
              <button type="button" class="lcBtn lcBtn--primary" data-addpol="1">הוסף פוליסה</button>
            </div>
          </div>
        </div>
      </div>`;
