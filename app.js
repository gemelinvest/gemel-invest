...
(() => {
  ...
  const defaultState = () => ({
    ...
    // data buckets (kept even if UI screens are currently hidden)
    customers: [],
    proposals: [],
    processes: []
  });
...
const State = {
  ...
  function normalizeState(s){
    ...
    // customers/proposals/processes: preserve arrays and normalize minimally (ownership for access control)
    out.customers = (Array.isArray(out.customers) ? out.customers : []).map((c, idx) => {
      const id = safeTrim(c?.id) || ("c_" + idx);
      const agent = safeTrim(c?.agent) || safeTrim(c?.owner) || safeTrim(c?.agentName) || safeTrim(c?.createdBy) || safeTrim(c?.agentId) || "";
      return { ...(c || {}), id, agent };
    });

    out.proposals = Array.isArray(out.proposals) ? out.proposals : [];
    out.processes = Array.isArray(out.processes) ? out.processes : [];
  ...
  // ---------- Access control (Customers ownership) ----------
  const ACL = {
    // Admin sees everything; Manager (role="manager") also sees everything
    isPrivileged(){
    ...
    canSeeCustomer(c){
      ...
      const owner = safeTrim(c?.agent);
      return !!me && !!owner && me === owner;
    },

    visibleCustomers(list){
      const arr = Array.isArray(list) ? list : [];
      if (this.isPrivileged()) return arr;
      return arr.filter(c => this.canSeeCustomer(c));
    },

    // Ensure new customer objects are owned by the logged-in agent
    stampOwner(c){
      const obj = { ...(c || {}) };
      if (!safeTrim(obj.agent) && Auth.current && !this.isPrivileged()) {
        obj.agent = safeTrim(Auth.current.name);
      }
      return obj;
    }
  };

  // Expose for future customer screens (so we don't lose this rule when UI returns)
  window.GEMEL_ACL = ACL;
