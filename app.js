...
(() => {
  ...
  const BUILD = "20260404-customer-reminders-v34b";
  ...
  function buildCustomerReminderDateTime(dateStr, timeStr){
    const d = safeTrim(dateStr);
    const t = safeTrim(timeStr);
    if(!d || !t) return "";
    return `${d}T${t}:00`;
  ...
  function normalizeCustomerReminder(rem, idx=0){
    if(!rem || typeof rem !== 'object') return null;
    const type = safeTrim(rem.type) || 'call';
    const date = safeTrim(rem.date);
    const time = safeTrim(rem.time || rem.rangeStart);
    const scheduledAt = safeTrim(rem.scheduledAt) || buildCustomerReminderDateTime(date, time);
    const reminderAt = safeTrim(rem.reminderAt) || shiftIsoMinutes(scheduledAt, -5);
    return {
      id: safeTrim(rem.id) || `customer_reminder_${Date.now()}_${idx}_${Math.random().toString(16).slice(2,7)}`,
      customerId: safeTrim(rem.customerId),
      customerName: safeTrim(rem.customerName) || 'לקוח',
      type,
      title: safeTrim(rem.title) || (type === 'call' ? 'להתקשר ללקוח' : 'תזכורת ללקוח'),
      summary: safeTrim(rem.summary || rem.notes),
      date,
      time,
      scheduledAt,
      reminderAt,
      status: safeTrim(rem.status) || 'pending',
      createdAt: safeTrim(rem.createdAt) || nowISO(),
      updatedAt: safeTrim(rem.updatedAt) || safeTrim(rem.createdAt) || nowISO(),
      createdByKey: safeTrim(rem.createdByKey),
      createdByName: safeTrim(rem.createdByName) || 'נציג',
      notificationActionAt: safeTrim(rem.notificationActionAt || rem.acknowledgedAt),
      notificationActionType: safeTrim(rem.notificationActionType),
      doneAt: safeTrim(rem.doneAt),
      deletedAt: safeTrim(rem.deletedAt)
    };
  }
  ...
  const ForgotPasswordUI = {
    ...
    init(){
      ...
      on(this.els.close, "click", (ev) => { ev?.preventDefault?.(); ev?.stopPropagation?.(); this.close(); });
...
this.applyRoleUI();
  ...
  const CustomersUI = {
    ...
    init(){
      ...
      this.els.reminderBtn = $("#customerFullReminderBtn");
      ...
      const openReminderFromHero = (ev) => {
        ev?.preventDefault?.();
        ev?.stopPropagation?.();
        const targetId = safeTrim(this.els.reminderBtn?.getAttribute?.('data-customer-id'));
        const rec = this.byId(targetId) || this.current();
        if(!rec) return;
        CustomerRemindersUI.openCreate(rec.id);
      };
      on(this.els.reminderBtn, "click", openReminderFromHero);
      this.els.reminderBtn && (this.els.reminderBtn.onclick = openReminderFromHero);
    ...
    renderCurrentSection(rec){
      ...
      this.els.body.innerHTML = this.currentSection === 'medical'
        ? this.renderMedicalInfo(rec)
        : `${this.renderPolicyWallet(rec, policies)}${CustomerRemindersUI.renderSection(rec)}`;
      if(this.currentSection !== 'medical') {
        this.bindPolicyCardActions(rec, policies);
        CustomerRemindersUI.bindCustomerSection(rec);
      }
    },
    ...
    openById(id, opts={}){
      ...
      try{
        ...
        if(this.els.reminderBtn){
          this.els.reminderBtn.setAttribute('data-customer-id', safeTrim(rec.id));
          this.els.reminderBtn.setAttribute('aria-label', `צור תזכורת עבור ${safeTrim(rec.fullName || 'לקוח')}`);
        }
        if(this.els.meta){
...
// open health covers drawer (Health only)
    ...
    getStep1Questions(ins){
      ...
      return [
        ...
        },
        {
          ...
          sub: isChild ? 'לקטין פרטי הקשר נמשכים אוטומטית מהמבוטח הראשי ומוצגים כאן יחד.' : 'טלפון, עיר, כתובת, מספר בית, דירה ומיקוד אוטומטי מרוכזים כאן כדי לסיים את השלב מהר יותר.',
    ...
    renderStep5(){
      ...
      const heroSubtitle = `תצוגה ל${escapeHtml((this.insureds.find(x => x.id === d.insuredId)?.label) || safeTrim(this.insureds?.[0]?.label) || "מבוטח ראשי")}${d.insuredMode === "couple" ? " + בן/בת זוג" : ""}`;
      const form = `<div class="lcWSection lcPolBuilderSection lcPolBuilderSection--reference">
        <div class="lcPolScreenHero">
          <div class="lcPolScreenHero__text">
            <div class="lcPolScreenHero__title">${this.editingPolicyId ? "עריכת פוליסה" : "פוליסה חדשה"}</div>
            <div class="lcPolScreenHero__subWrap">
              <div class="lcPolScreenHero__sub">${heroSubtitle}</div>
              <div class="lcPolScreenHero__line" aria-hidden="true"></div>
            </div>
          </div>
        ...
        <div class="lcPolForm lcPolForm--premium lcPolForm--reference">
          <div class="lcPolBuilderCard lcPolBuilderCard--reference">
            ...
            <section class="lcPolReferenceBlock lcPolReferenceBlock--details">
              <div class="lcPolReferenceBlock__head lcPolReferenceBlock__head--details">
                <div class="lcPolReferenceBlock__titleWrap">
                  <div class="lcPolReferenceBlock__title">פרטי הפוליסה</div>
                  <div class="lcPolReferenceBlock__sub">מלאו את פרטי הפוליסה</div>
                </div>
                <div class="lcPolReferenceBlock__icon lcPolReferenceBlock__icon--doc" aria-hidden="true">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M8 3.75h6l4 4V20a1.75 1.75 0 0 1-1.75 1.75h-8.5A1.75 1.75 0 0 1 6 20V5.5A1.75 1.75 0 0 1 7.75 3.75Z"></path>
              ...
              <div class="lcPolGrid lcPolGrid--top lcPolGrid--mainRow lcPolGrid--reference">
                ...
                <div class="lcField lcPolField lcPolField--premiumMain lcPolField--premiumReference">
                  ...
                  <div class="lcPolMoneyWrap lcPolControlShell lcPolMoneyWrap--reference">
                    ...
                    <div class="lcPolMoneyWrap__demo" aria-hidden="true">
                      <span class="lcPolMoneyWrap__demoValue">250</span>
                      <span class="lcPolMoneyWrap__demoLabel">לדוגמה</span>
                    </div>
                    <input class="lcInput lcPolMoneyWrap__input" type="text" inputmode="numeric" data-pdraft="premiumMonthly" value="${escapeHtml(d.premiumMonthly || "")}" placeholder="" />
                  </div>
                </div>
              </div>
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const CustomerRemindersUI = {
    ...
    init(){
      this.els.modal = document.getElementById('customerReminderModal');
      this.els.backdrop = document.getElementById('customerReminderModalBackdrop');
      this.els.close = document.getElementById('customerReminderModalClose');
      this.els.cancel = document.getElementById('customerReminderCancel');
      this.els.save = document.getElementById('customerReminderSave');
      this.els.title = document.getElementById('customerReminderModalTitle');
      this.els.customerName = document.getElementById('customerReminderCustomerName');
      this.els.type = document.getElementById('customerReminderType');
      this.els.date = document.getElementById('customerReminderDate');
      this.els.time = document.getElementById('customerReminderTime');
      this.els.summary = document.getElementById('customerReminderSummary');
      this.els.error = document.getElementById('customerReminderError');
      this.els.preview = document.getElementById('customerReminderPreview');
      this.els.toastHost = document.getElementById('customerReminderToastHost');
      this.bindShell();
      this.startLoop();
    },
    ...
    bindShell(){
      ...
      on(this.els.cancel, 'click', () => this.close());
      on(this.els.backdrop, 'click', () => this.close());
      on(this.els.save, 'click', () => this.handleSave());
      [this.els.date, this.els.time, this.els.summary].forEach((el) => on(el, 'input', () => this.renderPreview()));
      on(document, 'keydown', (ev) => {
        if(ev.key === 'Escape' && this.els.modal?.classList.contains('is-open')) this.close();
      });
    },
    ...
    defaultTime(){
      ...
      if(rounded >= 60){ d.setHours(d.getHours() + 1); d.setMinutes(0, 0, 0); }
      else d.setMinutes(rounded, 0, 0);
      return `${String(d.getHours()).padStart(2,'0')}:${String(d.getMinutes()).padStart(2,'0')}`;
    },

    getReminders(rec){
      const source = Array.isArray(rec?.payload?.customerReminders) ? rec.payload.customerReminders : [];
      return source.map((item, idx) => normalizeCustomerReminder(item, idx)).filter(Boolean).filter((item) => !safeTrim(item.deletedAt));
    },

    setReminders(rec, reminders){
      if(!rec) return;
      rec.payload = rec.payload && typeof rec.payload === 'object' ? rec.payload : {};
      rec.payload.customerReminders = Array.isArray(reminders) ? reminders.map((item, idx) => normalizeCustomerReminder(item, idx)).filter(Boolean) : [];
      rec.updatedAt = nowISO();
      if(State.data?.meta) State.data.meta.updatedAt = rec.updatedAt;
    },

    typeLabel(type){
      return safeTrim(type) === 'call' ? 'להתקשר ללקוח' : 'תזכורת ללקוח';
    },

    statusLabel(status){
      const key = safeTrim(status);
      if(key === 'done') return 'בוצעה';
      if(key === 'ignored') return 'ההתראה הושתקה';
    ...
    statusClass(status){
      ...
      if(key === 'ignored') return 'is-ignored';
      return 'is-pending';
    },

    sortReminders(reminders){
      return reminders.slice().sort((a, b) => {
        const aDone = safeTrim(a.status) === 'done' ? 1 : 0;
        const bDone = safeTrim(b.status) === 'done' ? 1 : 0;
    ...
    renderSection(rec){
      const reminders = this.sortReminders(this.getReminders(rec));
      return `
        <section class="customerReminderSection">
          <div class="customerReminderSection__head">
            <div class="customerReminderSection__titleWrap">
              <div class="customerReminderSection__icon">${premiumCustomerIcon('activity')}</div>
              <div>
                <div class="customerReminderSection__title">תזכורות ללקוח</div>
                <div class="customerReminderSection__sub">תיעוד מסודר של שיחות חוזרות, תאריך, שעה וסיכום השיחה האחרונה</div>
              </div>
            </div>
            <div class="customerReminderSection__count">${escapeHtml(String(reminders.length))}</div>
          </div>
          <div class="customerReminderList">
            ${reminders.length ? reminders.map((item) => this.renderCard(item)).join('') : `<div class="emptyState"><div class="emptyState__icon">⏰</div><div class="emptyState__title">עדיין אין תזכורות ללקוח</div><div class="emptyState__text">לחץ על "צור תזכורת" כדי לקבוע חזרה ללקוח עם תאריך, שעה וסיכום שיחה אחרונה.</div></div>`}
          </div>
        </section>`;
    },
    ...
    renderCard(item){
      return `
        <article class="customerReminderCard ${safeTrim(item.status) === 'done' ? 'is-done' : ''}" data-reminder-id="${escapeHtml(item.id)}">
          <div class="customerReminderCard__top">
            <div>
              <div class="customerReminderCard__type">${escapeHtml(this.typeLabel(item.type))}</div>
              <div class="customerReminderCard__when">${escapeHtml(formatOpsDateTime(item.scheduledAt || buildCustomerReminderDateTime(item.date, item.time)))}</div>
            </div>
            <span class="customerReminderCard__status ${this.statusClass(item.status)}">${escapeHtml(this.statusLabel(item.status))}</span>
          </div>
          <div class="customerReminderCard__summary">${escapeHtml(item.summary || 'לא נכתב עדיין סיכום שיחה אחרונה.')}</div>
          <div class="customerReminderCard__meta">נוצרה על ידי ${escapeHtml(item.createdByName || 'נציג')} · ${escapeHtml(formatOpsDateTime(item.createdAt))}</div>
          <div class="customerReminderCard__actions">
            <button class="btn" data-reminder-open="${escapeHtml(item.id)}" type="button">פתח תזכורת</button>
            ${safeTrim(item.status) !== 'done' ? `<button class="btn btn--primary" data-reminder-done="${escapeHtml(item.id)}" type="button">סמן כבוצע</button>` : ''}
            <button class="btn" data-reminder-delete="${escapeHtml(item.id)}" type="button">מחק</button>
          </div>
        </article>`;
    },

    bindCustomerSection(rec){
      const root = CustomersUI?.els?.body;
      if(!root || !rec) return;
      root.querySelectorAll('[data-reminder-open]').forEach((btn) => {
        btn.onclick = (ev) => {
          ev.preventDefault();
          this.openCreate(rec.id, btn.getAttribute('data-reminder-open'));
        };
      });
      root.querySelectorAll('[data-reminder-done]').forEach((btn) => {
        btn.onclick = async (ev) => {
          ev.preventDefault();
          await this.markDone(rec.id, btn.getAttribute('data-reminder-done'));
        };
      });
      root.querySelectorAll('[data-reminder-delete]').forEach((btn) => {
        btn.onclick = async (ev) => {
          ev.preventDefault();
          await this.removeReminder(rec.id, btn.getAttribute('data-reminder-delete'));
        };
      });
    },

    openCreate(customerId, reminderId = ''){
      const rec = CustomersUI.byId(customerId);
      if(!rec || !this.els.modal) return;
      const reminder = this.getReminders(rec).find((item) => String(item.id) === String(reminderId)) || null;
      this.activeCustomerId = safeTrim(customerId);
      this.activeReminderId = reminder ? safeTrim(reminder.id) : '';
      if(this.els.title) this.els.title.textContent = reminder ? 'פרטי תזכורת' : 'צור תזכורת';
      if(this.els.customerName) this.els.customerName.value = safeTrim(rec.fullName || 'לקוח');
      if(this.els.type) this.els.type.value = reminder?.type || 'call';
      if(this.els.date) this.els.date.value = reminder?.date || this.defaultDate();
      if(this.els.time) this.els.time.value = reminder?.time || this.defaultTime();
      if(this.els.summary) this.els.summary.value = reminder?.summary || '';
      if(this.els.save) this.els.save.textContent = reminder ? 'עדכן תזכורת' : 'שמור תזכורת';
      if(this.els.error){ this.els.error.style.display = 'none'; this.els.error.textContent = ''; }
      this.renderPreview();
      this.els.modal.classList.add('is-open');
      this.els.modal.setAttribute('aria-hidden', 'false');
      document.body.style.overflow = 'hidden';
    ...
    close(){
      ...
      if(!CustomersUI?.els?.wrap?.classList.contains('is-open')) document.body.style.overflow = '';
    },

    validate(){
      const summary = safeTrim(this.els.summary?.value);
      const date = safeTrim(this.els.date?.value);
      const time = safeTrim(this.els.time?.value);
      if(!date) return 'יש לבחור תאריך לתזכורת.';
      if(!time) return 'יש לבחור שעה לתזכורת.';
      if(!summary || summary.length < 6) return 'יש לרשום סיכום שיחה אחרונה ברור כדי שתזכור למה לחזור ללקוח.';
      const scheduledAt = buildCustomerReminderDateTime(date, time);
      if(!scheduledAt || !Number.isFinite(Date.parse(scheduledAt))) return 'מועד התזכורת אינו תקין.';
      return '';
    },
    ...
    renderPreview(){
      ...
      const summary = safeTrim(this.els.summary?.value);
      if(!date || !time){
        this.els.preview.textContent = 'התראה תקפוץ 5 דקות לפני המועד שנקבע.';
        return;
      }
      const scheduledAt = buildCustomerReminderDateTime(date, time);
      const whenText = formatOpsDateTime(scheduledAt);
      const shortSummary = summary ? (summary.length > 110 ? `${summary.slice(0, 110)}…` : summary) : 'טרם נכתב סיכום.';
      this.els.preview.textContent = `תזכורת ל-${whenText}. ההתראה תקפוץ 5 דקות לפני הזמן. תקציר: ${shortSummary}`;
    },

    async handleSave(){
      const rec = CustomersUI.byId(this.activeCustomerId);
      if(!rec) return;
      const error = this.validate();
      if(this.els.error){
        this.els.error.textContent = error;
        this.els.error.style.display = error ? 'block' : 'none';
      }
      if(error) return;
      const creator = currentAgentIdentity();
      const reminders = this.getReminders(rec);
      const scheduledAt = buildCustomerReminderDateTime(safeTrim(this.els.date.value), safeTrim(this.els.time.value));
      const nextReminder = normalizeCustomerReminder({
        id: this.activeReminderId || `customer_reminder_${Date.now()}_${Math.random().toString(16).slice(2,7)}`,
        customerId: rec.id,
        customerName: rec.fullName || 'לקוח',
        type: safeTrim(this.els.type.value) || 'call',
        title: this.typeLabel(safeTrim(this.els.type.value) || 'call'),
        summary: safeTrim(this.els.summary.value),
        date: safeTrim(this.els.date.value),
        time: safeTrim(this.els.time.value),
        scheduledAt,
        reminderAt: shiftIsoMinutes(scheduledAt, -5),
        status: 'pending',
        createdAt: this.activeReminderId ? (reminders.find((item) => String(item.id) === String(this.activeReminderId))?.createdAt || nowISO()) : nowISO(),
        updatedAt: nowISO(),
        createdByKey: this.activeReminderId ? (reminders.find((item) => String(item.id) === String(this.activeReminderId))?.createdByKey || creator.key) : creator.key,
        createdByName: this.activeReminderId ? (reminders.find((item) => String(item.id) === String(this.activeReminderId))?.createdByName || creator.name) : creator.name,
        notificationActionAt: '',
        notificationActionType: ''
      }, reminders.length);
      const idx = reminders.findIndex((item) => String(item.id) === String(nextReminder.id));
      if(idx >= 0) reminders[idx] = nextReminder;
      else reminders.push(nextReminder);
      this.setReminders(rec, reminders);
      const result = await App.persist('תזכורת ללקוח נשמרה');
      if(!result?.ok){
        if(this.els.error){
          this.els.error.textContent = 'שמירת התזכורת נכשלה. נסה שוב.';
    ...
    async markDone(customerId, reminderId){
      const rec = CustomersUI.byId(customerId);
      if(!rec) return;
      const reminders = this.getReminders(rec);
      const idx = reminders.findIndex((item) => String(item.id) === String(reminderId));
      if(idx < 0) return;
      reminders[idx] = normalizeCustomerReminder({
        ...reminders[idx],
        status: 'done',
        doneAt: nowISO(),
        updatedAt: nowISO(),
        notificationActionAt: reminders[idx].notificationActionAt || nowISO(),
        notificationActionType: reminders[idx].notificationActionType || 'done'
      }, idx);
      this.setReminders(rec, reminders);
      const result = await App.persist('התזכורת סומנה כבוצעה');
      if(!result?.ok) return;
      CustomersUI.refreshOpenCustomerPreservingState();
      this.checkReminders();
    },

    async removeReminder(customerId, reminderId){
      const rec = CustomersUI.byId(customerId);
      if(!rec) return;
      const reminders = this.getReminders(rec).filter((item) => String(item.id) !== String(reminderId));
      this.setReminders(rec, reminders);
      const result = await App.persist('התזכורת נמחקה');
      if(!result?.ok) return;
      CustomersUI.refreshOpenCustomerPreservingState();
      this.checkReminders();
    },
    ...
    visibleCustomers(){
      return CustomersUI.list();
    },

    dueReminder(){
      const creator = currentAgentIdentity();
      const now = Date.now();
      for(const rec of this.visibleCustomers()){
        const match = this.getReminders(rec).find((item) => {
          if(safeTrim(item.status) !== 'pending') return false;
          if(safeTrim(item.createdByKey) && safeTrim(item.createdByKey) !== creator.key) return false;
          if(safeTrim(item.notificationActionAt)) return false;
          const reminderTs = Date.parse(item.reminderAt || '');
          const scheduledTs = Date.parse(item.scheduledAt || '');
          if(!Number.isFinite(reminderTs) || !Number.isFinite(scheduledTs)) return false;
          return now >= reminderTs && now <= (scheduledTs + (10 * 60000));
        });
        if(match) return { rec, reminder: match };
      }
      return null;
    },

    renderToast(payload){
      if(!this.els.toastHost || !payload?.reminder) return;
      const reminder = payload.reminder;
      const rec = payload.rec;
      const existing = this.els.toastHost.querySelector(`[data-reminder-toast-id="${CSS.escape(reminder.id)}"]`);
      if(existing) return;
      this.els.toastHost.innerHTML = '';
      const toast = document.createElement('div');
      toast.className = 'customerReminderToast';
      toast.setAttribute('data-reminder-toast-id', reminder.id);
      const shortSummary = safeTrim(reminder.summary).length > 160 ? `${safeTrim(reminder.summary).slice(0, 160)}…` : safeTrim(reminder.summary);
      toast.innerHTML = `
        <div class="customerReminderToast__kicker">תזכורת ללקוח</div>
        <div class="customerReminderToast__title">${escapeHtml(rec.fullName || reminder.customerName || 'לקוח')}</div>
        <div class="customerReminderToast__text">${escapeHtml(this.typeLabel(reminder.type))} · ${escapeHtml(formatOpsDateTime(reminder.scheduledAt))}</div>
        <div class="customerReminderToast__summary">${escapeHtml(shortSummary || 'אין סיכום שיחה אחרונה.')}</div>
        <div class="customerReminderToast__actions">
          <button class="btn" data-reminder-ignore type="button">התעלם</button>
          <button class="btn btn--primary" data-reminder-open type="button">פתח תזכורת</button>
        </div>`;
      toast.querySelector('[data-reminder-ignore]').onclick = async () => {
        await this.noteNotificationAction(rec.id, reminder.id, 'ignored');
      };
      toast.querySelector('[data-reminder-open]').onclick = async () => {
        await this.noteNotificationAction(rec.id, reminder.id, 'opened');
        CustomersUI.openById(rec.id, { section: 'wallet' });
        this.openCreate(rec.id, reminder.id);
      };
      this.els.toastHost.appendChild(toast);
    },

    async noteNotificationAction(customerId, reminderId, actionType){
      const rec = CustomersUI.byId(customerId);
      if(!rec) return;
      const reminders = this.getReminders(rec);
      const idx = reminders.findIndex((item) => String(item.id) === String(reminderId));
      ...
      reminders[idx] = normalizeCustomerReminder({
        ...reminders[idx],
        updatedAt: nowISO(),
        notificationActionAt: nowISO(),
        notificationActionType: safeTrim(actionType)
      }, idx);
      this.setReminders(rec, reminders);
      await App.persist(actionType === 'ignored' ? 'התראת תזכורת הושתקה' : 'תזכורת נפתחה');
      if(this.els.toastHost) this.els.toastHost.innerHTML = '';
      CustomersUI.refreshOpenCustomerPreservingState();
    },

    checkReminders(){
      if(!this.els.toastHost) return;
      const payload = this.dueReminder();
      if(!payload){
        this.els.toastHost.innerHTML = '';
        return;
      }
    ...
    startLoop(){
      if(this.toastTimer) window.clearInterval(this.toastTimer);
      this.toastTimer = window.setInterval(() => this.checkReminders(), 15000);
      this.checkReminders();
    }
  };
