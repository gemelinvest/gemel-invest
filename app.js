...
const SystemRepairUI = {
  ...
  const LeadGeneratorUI = {
    ...
    init(){
      ...
      if (this.els.btnOpen){
        this.els.btnOpen.onclick = (ev) => this.openFromButton(ev);
    ...
    setEngineState(state){
      if(this.els.engine) this.els.engine.setAttribute("data-state", state || "idle");
    },
    ...
    resetView(){
      ...
      if(this.els.copy) this.els.copy.disabled = true;
    },

    clearTimers(){
      try{ clearTimeout(this.activeTimer); }catch(_e){}
      try{ clearInterval(this.activeInterval); }catch(_e){}
    ...
    saveUsedNumber(num){
      const arr = this.getUsedNumbers();
      if(!arr.includes(num)) arr.push(num);
      try{ localStorage.setItem(this.storageKey, JSON.stringify(arr)); }catch(_e){}
    },

    generateUniqueNumber(){
      const used = new Set(this.getUsedNumbers());
    ...
    startSearch(){
      ...
      this.latestNumber = "";
      this.setBusy(true);
      this.setEngineState("searching");
      if(this.els.status) this.els.status.textContent = "מחפש ליד חדש";
      const statuses = [
        "סורק קידומות ישראליות…",
        "מרכיב רצף מספרים חדש…",
        "בודק כפילויות במערכת המקומית…",
        "מאמת תוצאה ומכין ליד להצגה…"
      ];
      let tick = 0;
      if(this.els.substatus) this.els.substatus.textContent = statuses[0];
      this.activeInterval = setInterval(() => {
        tick += 1;
        const preview = this.generatePreviewNumber();
        if(this.els.number) this.els.number.textContent = preview;
        if(this.els.progressBar) this.els.progressBar.style.width = Math.min(100, ((tick + 1) / 16) * 100) + "%";
        if(this.els.substatus) this.els.substatus.textContent = statuses[Math.min(statuses.length - 1, Math.floor(tick / 4))];
      }, 500);
    ...
    finishSearch(){
      ...
      if(!number){
        this.setEngineState("idle");
        if(this.els.status) this.els.status.textContent = "לא הצלחתי ליצור ליד חדש";
        if(this.els.substatus) this.els.substatus.textContent = "מאגר המספרים המקומי מלא או שלא ניתן לשמור כרגע. נסה שוב.";
        if(this.els.progressBar) this.els.progressBar.style.width = "0%";
        return;
      }
      this.latestNumber = number;
      this.saveUsedNumber(number);
      this.setEngineState("done");
      if(this.els.status) this.els.status.textContent = "הליד החדש מוכן";
      if(this.els.number) this.els.number.textContent = number;
      if(this.els.substatus) this.els.substatus.textContent = "המערכת יצרה מספר ישראלי חדש, בדקה שאין כפילות ושמרה אותו מקומית.";
      if(this.els.progressBar) this.els.progressBar.style.width = "100%";
    ...
    async copyNumber(){
      ...
      try{
        await navigator.clipboard.writeText(this.latestNumber);
        if(this.els.substatus) this.els.substatus.textContent = "המספר הועתק ללוח בהצלחה.";
      }catch(_e){
        try{
          const ta = document.createElement("textarea");
          ta.value = this.latestNumber;
