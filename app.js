...
(() => {
  ...
  const BUILD = "20260324-operational-report-redesign-1";
...
// ---------- Step 6 ----------
    ...
    buildHealthItemsFromPayload(payload){
      const rows = this.collectHealthAnswerRows(payload);
      return rows.map((item) => `<div class="lcReportHealthItem"><div class="lcReportHealthItem__top"><strong>${escapeHtml(item.insuredLabel)}</strong><span class="lcReportStatus lcReportStatus--${item.answerRaw === 'yes' ? 'danger' : 'success'}">${escapeHtml(item.answerLabel)}</span></div><div class="lcReportHealthItem__question">${escapeHtml(item.questionText)}</div><div class="lcReportHealthItem__answer">${escapeHtml(item.details)}</div></div>`);
    },

    collectHealthAnswerRows(payload){
    ...
    renderOperationalPdfRows(rows, columns = 2){
      const items = Array.isArray(rows) ? rows.filter(([label, value]) => safeTrim(label) && safeTrim(value)) : [];
      if(!items.length) return `<div class="lcPdfEmpty">אין נתונים להצגה.</div>`;
      return `<div class="lcPdfKeyGrid" style="--cols:${Math.max(1, columns)};">${items.map(([label, value]) => `<div class="lcPdfKeyItem"><div class="lcPdfKeyItem__label">${escapeHtml(label)}</div><div class="lcPdfKeyItem__value">${value}</div></div>`).join('')}</div>`;
    },

    renderOperationalPdfDocument(payloadOverride){
      ...
      const exportDate = exportDateObj.toLocaleDateString('he-IL');
      const exportTime = exportDateObj.toLocaleTimeString('he-IL', { hour: '2-digit', minute: '2-digit' });
      ...
      const customerRows = [
        ['שם מלא', this.renderReportValue(customerName)],
      ...
      const reportRows = [
        ['שם לקוח', this.renderReportValue(customerName)],
        ['מספר מבוטחים', escapeHtml(String(insureds.length || 0))],
        ['פוליסות קיימות', escapeHtml(String(insureds.reduce((sum, ins) => sum + ((Array.isArray(ins?.data?.existingPolicies) ? ins.data.existingPolicies.length : 0)), 0)))],
        ['פוליסות חדשות', escapeHtml(String(newPolicies.length || 0))],
        ...
        ['שם הנציג', this.renderReportValue(agentName)]
      ];

      const existingRows = [];
      insureds.forEach((ins) => {
        const insData = ins?.data && typeof ins.data === 'object' ? ins.data : {};
        (Array.isArray(insData.existingPolicies) ? insData.existingPolicies : []).forEach((p) => {
          const statusMeta = this.getExistingPolicyStatusMeta(p, insData);
          const statusParts = [statusMeta.label, statusMeta.reason, statusMeta.partialDetails].map(v => safeTrim(v)).filter(Boolean);
          existingRows.push([
            this.renderReportValue(ins?.label),
      ...
      const newPolicyBlocks = newPolicies.map((p, idx) => {
        const coverItems = (typeof this.getPolicyCoverItems === 'function' ? this.getPolicyCoverItems(p || {}) : []).filter(Boolean).map(v => safeTrim(v)).filter(Boolean);
        const coverageValue = (typeof this.getPolicyCoverageDisplayValue === 'function' ? this.getPolicyCoverageDisplayValue(p || {}) : safeTrim(p?.coverage || p?.sumInsured || p?.compensation));
        const pledge = (() => {
          const type = safeTrim(p?.type || p?.product || '');
          if(!(type === 'ריסק' || type === 'ריסק משכנתא')) return { active:false, rows:[] };
          if(!p?.pledge) return { active:false, rows:[] };
          const bank = p?.pledgeBank && typeof p.pledgeBank === 'object' ? p.pledgeBank : {};
          return {
            active:true,
            rows: [
        ...
        const policyRows = [
          ['מבוטח', this.renderReportValue((typeof this.getNewPolicyInsuredLabel === 'function' ? this.getNewPolicyInsuredLabel(payload, p, insureds) : 'מבוטח'))],
        ...
        return `<section class="lcPdfPolicyCard"><div class="lcPdfPolicyCard__head"><div><div class="lcPdfPolicyCard__index">פוליסה חדשה ${idx + 1}</div><div class="lcPdfPolicyCard__title">${escapeHtml(safeTrim(p?.company) || 'חברה')} · ${escapeHtml(safeTrim(p?.type) || 'מוצר')}</div></div></div>${this.renderOperationalPdfRows(policyRows, 2)}${coverItems.length ? `<div class="lcPdfInlineSection"><div class="lcPdfInlineSection__title">כיסויים</div><div class="lcPdfChips">${coverItems.map(item => `<span class="lcPdfChip">${escapeHtml(item)}</span>`).join('')}</div></div>` : ''}${pledge.active ? `<div class="lcPdfInlineSection"><div class="lcPdfInlineSection__title">פרטי שיעבוד</div>${this.renderOperationalPdfRows(pledge.rows.map(([label, value]) => [label, this.renderReportValue(value)]), 2)}</div>` : ''}</section>`;
      }).join('');
      ...
      if(primary.payerChoice === 'external'){
        ...
        payerRows.push(['קרבה למבוטח', this.renderReportValue(ex.relation)], ['שם פרטי', this.renderReportValue(ex.firstName)], ['שם משפחה', this.renderReportValue(ex.lastName)], ['תעודת זהות', this.renderReportValue(ex.idNumber)], ['תאריך לידה', this.renderReportValue(ex.birthDate)], ['טלפון', this.renderReportValue(ex.phone)]);
      }
      if(safeTrim(primary.paymentMethod) === 'cc'){
        const cc = primary.cc || {};
        payerRows.push(['שם מחזיק', this.renderReportValue(cc.holderName)], ['ת.ז מחזיק', this.renderReportValue(cc.holderId)], ['מספר כרטיס', this.renderReportValue(cc.cardNumber)], ['תוקף', this.renderReportValue(cc.exp)]);
      } else if(safeTrim(primary.paymentMethod) === 'ho'){
        const ho = primary.ho || {};
        payerRows.push(['שם בנק', this.renderReportValue(ho.bankName)], ['מספר בנק', this.renderReportValue(ho.bankNo)], ['סניף', this.renderReportValue(ho.branch)], ['מספר חשבון', this.renderReportValue(ho.account)]);
      }

      const healthRows = this.collectHealthAnswerRows(payload);
      const healthTableRows = healthRows.map(item => `<tr><td>${escapeHtml(item.insuredLabel)}</td><td>${escapeHtml(item.questionText)}</td><td><span class="lcPdfAnswerBadge lcPdfAnswerBadge--${item.answerRaw === 'yes' ? 'yes' : 'no'}">${escapeHtml(item.answerLabel)}</span></td><td>${escapeHtml(item.details)}</td></tr>`).join('');
      const summaryBlocks = [];
      if(newPolicies.length){
        const before = newPolicies.reduce((sum, p) => sum + (Number(String(p?.premiumMonthly || '').replace(/,/g,'')) || 0), 0);
        const after = newPolicies.reduce((sum, p) => sum + this.getPolicyPremiumAfterDiscount(p || {}), 0);
        summaryBlocks.push(['סה״כ פרמיה לפני הנחה', this.renderReportValue(this.formatMoneyValue(before))]);
        summaryBlocks.push(['סה״כ פרמיה אחרי הנחה', this.renderReportValue(this.formatMoneyValue(after))]);
      }

      const styles = `<style>
        .lcPdfSheet{direction:rtl;background:#fff;color:#1f2937;font-family:Arial,sans-serif;padding:18px 22px 22px;}
        .lcPdfTop{display:flex;justify-content:space-between;align-items:flex-start;gap:18px;padding-bottom:14px;border-bottom:2px solid #dde5ef;margin-bottom:18px;}
        .lcPdfBrand{display:flex;align-items:flex-start;gap:14px;}
        .lcPdfBrand__logo{width:64px;height:64px;object-fit:contain;}
        .lcPdfBrand__name{font-size:22px;font-weight:700;color:#243b53;line-height:1.2;margin-bottom:2px;}
        .lcPdfBrand__title{font-size:28px;font-weight:800;color:#0f172a;line-height:1.15;}
        .lcPdfBrand__meta{margin-top:8px;font-size:12px;color:#526071;display:grid;gap:4px;}
        .lcPdfHeaderBox{min-width:220px;background:#f5f7fb;border:1px solid #dbe3ee;border-radius:14px;padding:12px 14px;}
        .lcPdfHeaderBox__label{font-size:12px;color:#617085;margin-bottom:4px;}
        .lcPdfHeaderBox__value{font-size:15px;font-weight:700;color:#111827;}
        .lcPdfSection{margin-top:14px;border:1px solid #dde5ef;border-radius:18px;overflow:hidden;background:#fff;page-break-inside:avoid;}
        .lcPdfSection__head{background:#f5f7fb;padding:12px 16px;border-bottom:1px solid #dde5ef;}
        .lcPdfSection__title{font-size:18px;font-weight:800;color:#0f172a;}
        .lcPdfSection__sub{margin-top:4px;font-size:12px;color:#607086;}
        .lcPdfSection__body{padding:14px 16px 16px;}
        .lcPdfTwoCols{display:grid;grid-template-columns:1fr 1fr;gap:14px;}
        .lcPdfKeyGrid{display:grid;grid-template-columns:repeat(var(--cols), minmax(0,1fr));gap:10px 12px;}
        .lcPdfKeyItem{background:#fafbfd;border:1px solid #e4eaf2;border-radius:12px;padding:10px 12px;min-height:58px;}
        .lcPdfKeyItem__label{font-size:11px;color:#68778b;margin-bottom:6px;font-weight:700;}
        .lcPdfKeyItem__value{font-size:14px;color:#111827;font-weight:600;word-break:break-word;}
        .lcPdfTable{width:100%;border-collapse:collapse;font-size:12px;}
        .lcPdfTable th{background:#f5f7fb;color:#44556b;font-weight:800;padding:10px 8px;border-bottom:1px solid #dde5ef;text-align:right;}
        .lcPdfTable td{padding:10px 8px;border-bottom:1px solid #edf2f7;vertical-align:top;text-align:right;}
        .lcPdfTable tr:last-child td{border-bottom:none;}
        .lcPdfPolicyCard{border:1px solid #e3e9f2;border-radius:16px;padding:12px 14px;margin-bottom:12px;background:#fbfcfe;page-break-inside:avoid;}
        .lcPdfPolicyCard__head{display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:10px;}
        .lcPdfPolicyCard__index{font-size:12px;color:#63748a;font-weight:700;margin-bottom:4px;}
        .lcPdfPolicyCard__title{font-size:16px;font-weight:800;color:#0f172a;}
        .lcPdfInlineSection{margin-top:12px;}
        .lcPdfInlineSection__title{font-size:12px;font-weight:800;color:#44556b;margin-bottom:8px;}
        .lcPdfChips{display:flex;flex-wrap:wrap;gap:6px;}
        .lcPdfChip{background:#eef3f9;border:1px solid #d8e2ee;border-radius:999px;padding:5px 10px;font-size:11px;color:#334155;}
        .lcPdfAnswerBadge{display:inline-flex;align-items:center;justify-content:center;min-width:44px;padding:4px 10px;border-radius:999px;font-weight:800;font-size:11px;}
        .lcPdfAnswerBadge--yes{background:#fee2e2;color:#991b1b;}
        .lcPdfAnswerBadge--no{background:#e5eef7;color:#334155;}
        .lcPdfSummaryStrip{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:12px;}
        .lcPdfSummaryItem{background:#f5f7fb;border:1px solid #dde5ef;border-radius:12px;padding:10px 12px;}
        .lcPdfSummaryItem__label{font-size:11px;color:#68778b;margin-bottom:5px;font-weight:700;}
        .lcPdfSummaryItem__value{font-size:15px;font-weight:800;color:#111827;}
        .lcPdfEmpty{font-size:13px;color:#667085;padding:6px 0;}
      </style>`;

      return {
        payload,
        html: `${styles}<div class="lcPdfSheet"><div class="lcPdfTop"><div class="lcPdfBrand"><img src="./logo-login-clean.svg" alt="GEMEL INVEST" class="lcPdfBrand__logo" onerror="this.onerror=null;this.src='./logo-login-clean.png';"><div><div class="lcPdfBrand__name">GEMEL INVEST</div><div class="lcPdfBrand__title">דוח תפעולי</div><div class="lcPdfBrand__meta"><div>תאריך: ${escapeHtml(exportDate)}</div><div>שעה: ${escapeHtml(exportTime)}</div><div>נציג: ${escapeHtml(agentName)}</div></div></div></div><div class="lcPdfHeaderBox"><div class="lcPdfHeaderBox__label">לקוח</div><div class="lcPdfHeaderBox__value">${escapeHtml(customerName)}</div><div class="lcPdfHeaderBox__label" style="margin-top:10px;">תעודת זהות</div><div class="lcPdfHeaderBox__value">${escapeHtml(safeTrim(primary.idNumber) || '—')}</div></div></div><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">פרטי הדוח</div><div class="lcPdfSection__sub">סיכום עליון ממוקד לפני מעבר לפרטי הלקוח והפוליסות.</div></div><div class="lcPdfSection__body">${this.renderOperationalPdfRows(reportRows, 2)}${summaryBlocks.length ? `<div class="lcPdfSummaryStrip">${summaryBlocks.map(([label, value]) => `<div class="lcPdfSummaryItem"><div class="lcPdfSummaryItem__label">${escapeHtml(label)}</div><div class="lcPdfSummaryItem__value">${value}</div></div>`).join('')}</div>` : ''}</div></section><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">פרטי לקוח</div><div class="lcPdfSection__sub">כל פרטי הלקוח הראשי מסודרים בתצוגה נקייה ונוחה לקריאה.</div></div><div class="lcPdfSection__body">${this.renderOperationalPdfRows(customerRows, 2)}</div></section><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">פוליסות קיימות</div><div class="lcPdfSection__sub">כולל סטטוס מדויק לפי מה שנבחר בהקמת הלקוח.</div></div><div class="lcPdfSection__body">${existingRows.length ? `<table class="lcPdfTable"><thead><tr><th>מבוטח</th><th>חברה</th><th>סוג ביטוח</th><th>מספר פוליסה</th><th>פרמיה חודשית</th><th>סטטוס</th></tr></thead><tbody>${existingRows.map(r => `<tr>${r.map(c => `<td>${c}</td>`).join('')}</tr>`).join('')}</tbody></table>` : `<div class="lcPdfEmpty">אין פוליסות קיימות להצגה.</div>`}</div></section><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">פוליסות חדשות</div><div class="lcPdfSection__sub">מוצגות בשורות מסודרות, ללא דחיסה, כולל כיסויים ושיעבוד במידת הצורך.</div></div><div class="lcPdfSection__body">${newPolicyBlocks || `<div class="lcPdfEmpty">לא נוספו פוליסות חדשות.</div>`}</div></section><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">אמצעי תשלום</div><div class="lcPdfSection__sub">כולל פרטי משלם חיצוני כאשר קיים.</div></div><div class="lcPdfSection__body">${this.renderOperationalPdfRows(payerRows, 2)}</div></section><section class="lcPdfSection"><div class="lcPdfSection__head"><div class="lcPdfSection__title">הצהרת בריאות</div><div class="lcPdfSection__sub">מופיעות גם תשובות כן וגם תשובות לא, בצורה מסודרת וברורה.</div></div><div class="lcPdfSection__body">${healthRows.length ? `<table class="lcPdfTable"><thead><tr><th>מבוטח</th><th>שאלה</th><th>תשובה</th><th>פירוט</th></tr></thead><tbody>${healthTableRows}</tbody></table>` : `<div class="lcPdfEmpty">לא נמצאו תשובות הצהרת בריאות להצגה.</div>`}</div></section></div>`
      };
    },
    ...
    renderOperationalReport(payloadOverride){
      ...
      const createdAtText = (() => {
        ...
        return Number.isFinite(ms) ? new Date(ms).toLocaleString('he-IL') : '—';
      })();
      ...
      const getPolicyCoverageDisplaySafe = (policy) => {
        ...
        if(type === 'מחלות קשות' || type === 'סרטן') return safeTrim(policy?.compensation || policy?.sumInsured || policy?.coverage || (coverItems.length ? coverItems.join(', ') : '')) || '—';
        return safeTrim(policy?.sumInsured || policy?.coverage || policy?.compensation || (coverItems.length ? coverItems.join(', ') : '')) || '—';
      };
      ...
      insureds.forEach(ins => {
        ...
        const cancellations = insData?.cancellations && typeof insData.cancellations === 'object' ? insData.cancellations : {};
        existingPolicies.forEach((p) => {
          const statusMeta = this.getExistingPolicyStatusMeta(p, insData);
          const statusCell = `${this.renderReportStatusBadge(statusMeta.label, statusMeta.tone)}${statusMeta.reason ? `<div class="lcReportInlineNote">${escapeHtml(statusMeta.reason)}</div>` : ''}${statusMeta.partialDetails ? `<div class="lcReportInlineSubNote">${escapeHtml(statusMeta.partialDetails)}</div>` : ''}`;
          existingRows.push([
            this.renderReportValue(ins?.label),
            this.renderReportValue(p?.company),
            this.renderReportValue(p?.type),
      ...
      const premiumBefore = newPolicies.reduce((sum, p) => sum + (Number(String(p?.premiumMonthly || '').replace(/,/g,'')) || 0), 0);
      const premiumAfter = newPolicies.reduce((sum, p) => sum + this.getPolicyPremiumAfterDiscount(p || {}), 0);
      const totalSavings = Math.max(0, premiumBefore - premiumAfter);
      ...
      const newPolicyBlocks = newPolicies.map((p, idx) => {
        ...
        const rows = [
          ...
          ['תאריך תחילת ביטוח', this.renderReportValue(p?.startDate)],
          ['כיסוי / סכום / פיצוי', this.renderReportValue(coverageValue)],
          ['שיעבוד', this.renderReportStatusBadge(pledge.status, pledge.active ? 'warn' : 'neutral')]
        ];
        const coversMarkup = coverItems.length
          ? `<div class="lcReportInlineList">${coverItems.map(item => `<span class="lcReportInlineTag">${escapeHtml(item)}</span>`).join('')}</div>`
          : `<div class="lcReportInlineNote">לא הוגדרו כיסויים נוספים מעבר לנתוני המוצר.</div>`;
        const pledgeMarkup = pledge.rows.length
          ? `<div class="lcReportSubBlock"><div class="lcReportSubBlock__title">פרטי שיעבוד</div>${this.renderReportRowsList(pledge.rows.map(([label, value]) => [label, this.renderReportValue(value)]), { dense:true })}</div>`
          : '';
        return `<article class="lcReportPolicyBlock"><div class="lcReportPolicyBlock__head"><div><div class="lcReportPolicyBlock__title">פוליסה חדשה ${idx + 1}</div><div class="lcReportPolicyBlock__sub">${escapeHtml(safeTrim(p?.company) || 'חברה')} · ${escapeHtml(safeTrim(p?.type) || 'מוצר')}</div></div>${this.renderReportStatusBadge(pledge.status, pledge.active ? 'warn' : 'neutral')}</div>${this.renderReportRowsList(rows, { dense:true })}<div class="lcReportSubBlock"><div class="lcReportSubBlock__title">כיסויים</div>${coversMarkup}</div>${pledgeMarkup}</article>`;
      });
      ...
      const reportSummaryRows = [
        ['נציג מטפל', this.renderReportValue(safeTrim(Auth?.current?.name) || 'נציג')],
        ['תאריך הפקה', this.renderReportValue(createdAtText)],
        ['מספר מבוטחים', escapeHtml(String(insureds.length || 0))],
        ['פוליסות קיימות', escapeHtml(String(existingRows.length || 0))],
        ['פוליסות חדשות', escapeHtml(String(newPolicies.length || 0))],
        ['חיסכון חודשי כולל', this.renderReportValue(this.formatMoneyValue(totalSavings))]
      ];
      const customerRows = [
        ['שם מלא', this.renderReportValue(customerFullName)], ['תעודת זהות', this.renderReportValue(primary.idNumber)], ['תאריך לידה', this.renderReportValue(primary.birthDate)], ['מגדר', this.renderReportValue(primary.gender)], ['מצב משפחתי', this.renderReportValue(primary.maritalStatus)], ['טלפון', this.renderReportValue(primary.phone)], ['אימייל', this.renderReportValue(primary.email)], ['עיר', this.renderReportValue(primary.city)], ['רחוב', this.renderReportValue(primary.street)], ['מספר בית', this.renderReportValue(primary.houseNumber)], ['מיקוד', this.renderReportValue(primary.zip)], ['קופת חולים', this.renderReportValue(primary.clinic || primary.hmo || primary.kupatHolim)], ['שב״ן', this.renderReportValue(primary.shaban || primary.shabanLevel)], ['עיסוק', this.renderReportValue(primary.occupation)], ['גובה', this.renderReportValue(primary.heightCm)], ['משקל', this.renderReportValue(primary.weightKg)], ['BMI', this.renderReportValue(primary.bmi)]
      ];
      ...
      return `<div class="lcReportDoc lcReportDoc--linear">
        <section class="lcReportHero">
          <div class="lcReportHero__top">
            <div class="lcReportHero__main">
              <div class="lcReportHero__eyebrow">GEMEL INVEST · דוח תפעולי</div>
              <h2 class="lcReportHero__title">${escapeHtml(customerFullName)}</h2>
              <div class="lcReportHero__sub">מסמך תפעולי בפורמט מסודר ונקי, מוכן לעבודה של תפעול, חיתום ומעקב.</div>
            </div>
            <div class="lcReportHero__meta">${this.renderReportRowsList(reportSummaryRows, { dense:true })}</div>
          </div>
        </section>
        <section class="lcReportSection lcReportSection--first">
          <div class="lcReportSection__title">פרטי לקוח</div>
          <div class="lcReportSection__sub">תמצית מלאה של נתוני הלקוח הראשי, מסודרת בשורות קריאות.</div>
          ${this.renderReportRowsList(customerRows, { dense:true })}
        </section>
        <section class="lcReportSection">
          <div class="lcReportSection__title">מבוטחים בתיק</div>
          <div class="lcReportSection__sub">פירוט כל המבוטחים שהוזנו בתהליך.</div>
          <div class="lcReportStack">${insureds.length ? insureds.map((ins, index) => { const d = ins?.data && typeof ins.data === 'object' ? ins.data : {}; const rows = [['שם מבוטח', this.renderReportValue(safeTrim(((d.firstName || '') + ' ' + (d.lastName || '')).trim()) || ins?.label)], ['סוג מבוטח', this.renderReportValue(ins?.type === 'primary' ? 'ראשי' : ins?.type === 'spouse' ? 'בן/בת זוג' : ins?.type === 'child' ? 'קטין' : 'מבוטח')], ['תעודת זהות', this.renderReportValue(d.idNumber)], ['תאריך לידה', this.renderReportValue(d.birthDate)], ['טלפון', this.renderReportValue(d.phone)], ['עיר', this.renderReportValue(d.city)], ['קופת חולים', this.renderReportValue(d.clinic || d.hmo || d.kupatHolim)], ['שב״ן', this.renderReportValue(d.shaban || d.shabanLevel)]]; return `<article class="lcReportPolicyBlock lcReportPolicyBlock--insured"><div class="lcReportPolicyBlock__head"><div><div class="lcReportPolicyBlock__title">${escapeHtml(safeTrim(ins?.label) || `מבוטח ${index + 1}`)}</div></div>${this.renderReportStatusBadge(ins?.type === 'primary' ? 'ראשי' : ins?.type === 'spouse' ? 'בן/בת זוג' : ins?.type === 'child' ? 'קטין' : 'מבוטח', 'neutral')}</div>${this.renderReportRowsList(rows, { dense:true })}</article>`; }).join('') : '<div class="muted">לא נמצאו מבוטחים.</div>'}</div>
        </section>
        <section class="lcReportSection">
          <div class="lcReportSection__title">פוליסות קיימות</div>
          <div class="lcReportSection__sub">כל סטטוס מוצג בדיוק לפי מה שהנציג מילא בהקמת הלקוח.</div>
        ...
        <section class="lcReportSection">
          ...
          <div class="lcReportSection__sub">כל פוליסה מוצגת בפירוט מלא, כולל כיסויים, הנחה ושיעבוד במידת הצורך.</div>
        ...
        <section class="lcReportSection">
          <div class="lcReportSection__title">אמצעי תשלום</div>
          <div class="lcReportSection__sub">כולל פרטי משלם אם הוגדר משלם חיצוני.</div>
          ${this.renderReportRowsList(payerRows, { dense:true, emptyText:'לא הוזנו פרטי תשלום.' })}
        </section>
        <section class="lcReportSection">
          <div class="lcReportSection__title">הצהרת בריאות</div>
          <div class="lcReportSection__sub">מופיעות גם תשובות של כן וגם תשובות של לא, כפי שנענו בפועל.</div>
    ...
    buildOperationalPdfMarkup(payloadOverride){
      return this.renderOperationalPdfDocument(payloadOverride || this.getOperationalPayload());
    },
    ...
    async exportOperationalPdf(payloadOverride){
      ...
      let host = null;
      try{
        const pack = this.buildOperationalPdfMarkup(payloadOverride);
        ...
        await window.html2pdf().set({
          margin: [8, 8, 8, 8],
          filename: this.getOperationalPdfFileName(pack.payload),
