  ...
  const ChatUI = {
    ...
    async sendMessage(){
      ...
      try {
        ...
        const { error } = await this.client.from(SUPABASE_CHAT.messagesTable).insert(payload);
        if(error) throw error;
        this.lastMessageByConversation.set(this.currentConversationId, { text, at: Date.now(), fromId: this.userKey });
        ...
        alert('לא הצלחתי לשלוח את ההודעה כרגע. ודא שהרצת את קובץ ה-SQL של הצ׳אט ב-Supabase.');
      } finally {
