...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // ===== Server-only mode (Google Sheets) =====
  // We DO NOT use LocalStorage for business data.
  // All Customers / Proposals / Processes are loaded from and saved to the Apps Script Web App.

  function apiAuthed_(action, payload){
    return apiCall(action, Object.assign({}, (payload||{}), { token: state.authToken }));
  }

  async function loadAllFromServer_(){
    const r = await apiAuthed_('getAll', {});
    if(!r || !r.ok) return r || { ok:false, error:'NO_RESPONSE' };
    state.customers = Array.isArray(r.customers) ? r.customers : [];
    state.proposals = Array.isArray(r.proposals) ? r.proposals : [];
    state.processes = Array.isArray(r.processes) ? r.processes : [];
    state.users = Array.isArray(r.users) ? r.users : [];
    return { ok:true };
  }

  let __saveAllTimer = null;
  function scheduleSaveAll_(){
    if(__saveAllTimer) clearTimeout(__saveAllTimer);
    __saveAllTimer = setTimeout(() => { saveAllToServer_(); }, 650);
  }

  async function saveAllToServer_(){
    if(!state.authToken) return;
    const payload = { customers: state.customers||[], proposals: state.proposals||[], processes: state.processes||[] };
    const r = await apiAuthed_('saveAll', payload);
    if(!r || !r.ok){
      console.warn('saveAll failed', r);
      // Optional: you can show a toast here if you want
    }
  }

  // Keep legacy helpers used by the UI (now server-backed)
  function loadProcesses_(){ /* server-only: loaded after login */ }
  function saveProcesses_(){ scheduleSaveAll_(); }

  function loadProposals_(){ /* server-only: loaded after login */ }
  function saveProposals_(){ scheduleSaveAll_(); }

  function loadCustomers_(){ /* server-only: loaded after login */ }
  function saveCustomers_(){ scheduleSaveAll_(); }
  }
  ...
  // ---------- AUTH (Server-only: Google Sheets) ----------
  // Users are stored in the sheet "Users" (managed by the Apps Script Web App).
  // We keep an in-memory copy in state.users (safe users list without passwords) after login.

  function loadUsers_(){
    return Array.isArray(state.users) ? state.users : [];
  }

  let __saveUsersTimer = null;
  function scheduleSaveUsers_(){
    if(__saveUsersTimer) clearTimeout(__saveUsersTimer);
    __saveUsersTimer = setTimeout(() => { saveUsersToServer_(); }, 650);
  }

  async function saveUsersToServer_(){
    if(!state.authToken) return;
    // NOTE: Admin operations can include password fields (if provided).
    const r = await apiAuthed_('saveUsers', { users: (state.users||[]) });
    if(!r || !r.ok) console.warn('saveUsers failed', r);
    // Refresh safe users list
    if(r && r.ok && Array.isArray(r.users)) state.users = r.users;
  }

  function saveUsers_(users){
    // keep as helper; updates local cache + sync to server
  ...
  // Minimal admin CRUD (works if your UI calls it). Password changes require providing payload.password.
  function upsertUser_(payload){
    ...
    const i = users.findIndex(u => u && u.username === username);
    const base = { username, displayName, role, active, updatedAt: nowIso };
    const merged = (i >= 0) ? Object.assign({}, users[i], base) : Object.assign({ createdAt: nowIso }, base);
    if(password !== undefined) merged.password = password; // optional field for server
  ...
  function deleteUser_(username){
    ...
    if(!u) return { ok:false, msg:'חסר שם משתמש' };
    const users = (state.users||[]).filter(x => x && x.username !== u);
    state.users = users;
    scheduleSaveUsers_();
    return { ok:true };
  }
  ...
  function findUserByUsername_(username){
    const u = String(username||'').trim();
    return (state.users||[]).find(x => x && x.username === u) || null;
  }
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  function loginAttempt_(username, password){
    ...
    return (async () => {
      if(!GOOGLE_SCRIPT_URL) return { ok:false, msg:'חסר קישור לשרת (Web App URL)' };

      const r = await apiCall('login', { username:u, password:p });
      if(!r || !r.ok){
        const msg = (r && (r.msg || r.error)) ? String(r.msg || r.error) : 'שגיאת התחברות';
        return { ok:false, msg };
      }

      // Save token in sessionStorage
      state.authToken = r.token || null;
      ...
      // Load all data
      const loadRes = await loadAllFromServer_();
      if(!loadRes.ok){
        return { ok:false, msg:'התחברתי אבל לא הצלחתי לטעון נתונים מהשרת' };
      }

      // Persist session
      setSession_({ username: (r.user && r.user.username) ? r.user.username : u, token: state.authToken, at: Date.now() });

      showApp_();
      const rr = location.hash ? location.hash.replace('#','').trim() : '';
      setRoute(rr || 'customers');
      return { ok:true };
    })();
  }
  ...
  (function bootAuth_(){
    ...
    if(form){
      form.addEventListener('submit', async (e) => {
        e.preventDefault();
        showErr('');
        const res = await loginAttempt_(userEl?.value, passEl?.value);
        if(!res.ok){
          showErr(res.msg);
    ...
    // Restore session if exists (token-based)
    (async () => {
      const sess = getSession_();
      if(sess && sess.token){
        state.authToken = sess.token;
        const me = await apiCall('whoami', { token: sess.token });
        if(me && me.ok && me.user){
          setIdentity_(me.user);
          showApp_();
          await loadAllFromServer_();
