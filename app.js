...
this.els.syncDot = $("#syncDot");
      ...
      this.els.btnCreateLead = $("#btnCreateLead");
...
this.applyRoleUI();
    ...
    applyRoleUI(){
      ...
      if (createLeadBtn) createLeadBtn.style.display = isOps ? "none" : "";
    },
...
init(){
      ...
      on(this.els.btnOpen, "click", (ev) => this.openFromButton(ev));
...
const SystemRepairUI = {
  ...
  const LeadGeneratorUI = {
    els: {},
    prefixes: ["050","051","052","053","054","055","056","057","058","059"],
    storageKey: "GEMEL_LEAD_NUMBERS_V1",
    activeTimer: null,
    activeInterval: null,
    latestNumber: "",
    isBusy: false,
    ...
    init(){
      this.els.btnOpen = $("#btnCreateLead");
      this.els.modal = $("#lcLeadModal");
      this.els.backdrop = $("#lcLeadModalBackdrop");
      this.els.close = $("#lcLeadModalClose");
      this.els.cancel = $("#lcLeadCancel");
      this.els.start = $("#lcLeadStart");
      this.els.copy = $("#lcLeadCopy");
      this.els.engine = $("#lcLeadEngine");
      this.els.status = $("#lcLeadStatus");
      this.els.number = $("#lcLeadNumber");
      this.els.substatus = $("#lcLeadSubstatus");
      this.els.progressBar = $("#lcLeadProgressBar");

      on(this.els.btnOpen, "click", (ev) => this.openFromButton(ev));
      on(this.els.backdrop, "click", () => this.closeModal());
      on(this.els.close, "click", () => this.closeModal());
      on(this.els.cancel, "click", () => this.closeModal());
      on(this.els.start, "click", () => this.startSearch());
      on(this.els.copy, "click", () => this.copyNumber());
      on(document, "keydown", (ev) => {
        if(ev.key === "Escape" && this.isOpen() && !this.isBusy) this.closeModal();
      });

      this.openFromButton = (ev) => {
        try{ ev?.preventDefault?.(); ev?.stopPropagation?.(); ev?.stopImmediatePropagation?.(); }catch(_e){}
        try{ if(Wizard?.isOpen) Wizard.close(); }catch(_e){}
        this.open();
        return false;
      };
    ...
    isOpen(){
      return !!(this.els.modal && this.els.modal.getAttribute("aria-hidden") === "false");
    },
    ...
    open(){
      ...
      document.body.style.overflow = "hidden";
      if(!this.isBusy) this.resetView();
    },

    closeModal(force){
      if(!this.els.modal || (this.isBusy && !force)) return;
    ...
    setBusy(flag){
      this.isBusy = !!flag;
      if(this.els.modal) this.els.modal.classList.toggle("is-busy", this.isBusy);
      if(this.els.start) this.els.start.disabled = this.isBusy;
      if(this.els.copy) this.els.copy.disabled = this.isBusy || !this.latestNumber;
    },

    resetView(){
      this.stopAnimations();
      this.latestNumber = "";
      if(this.els.engine) this.els.engine.setAttribute("data-state", "idle");
      if(this.els.status) this.els.status.textContent = "מוכן ליצירת ליד חדש";
      if(this.els.number) this.els.number.textContent = "05X-XXX-XXXX";
      if(this.els.substatus) this.els.substatus.textContent = 'לחץ על “התחל חיפוש” כדי להתחיל סריקה חכמה.';
      if(this.els.progressBar) this.els.progressBar.style.width = "0%";
      if(this.els.start) this.els.start.textContent = "התחל חיפוש";
      this.setBusy(false);
    },
    ...
    startSearch(){
      ...
      this.latestNumber = "";
      this.setBusy(true);
      if(this.els.start) this.els.start.textContent = "מחפש...";
      if(this.els.engine) this.els.engine.setAttribute("data-state", "searching");

      const messages = [
        "אנא המתן, המערכת שולפת לך ליד...",
        "סורק קידומות ישראליות פעילות...",
        "מרכיב רצף ספרות חדש לליד...",
        "בודק שהמספר לא נוצר קודם...",
        "משלים אימות אחרון ומכין תוצאה..."
      ];
      let msgIndex = 0;
      if(this.els.status) this.els.status.textContent = "מנוע יצירת הליד הופעל";
      if(this.els.substatus) this.els.substatus.textContent = messages[0];
      this.rollDigits();
      ...
      this.activeInterval = window.setInterval(() => {
        ...
        if(this.els.progressBar) this.els.progressBar.style.width = progress.toFixed(1) + "%";
        const nextIndex = Math.min(messages.length - 1, Math.floor(elapsed / 1600));
        if(nextIndex !== msgIndex){
          msgIndex = nextIndex;
          if(this.els.substatus) this.els.substatus.textContent = messages[msgIndex];
        }
        this.rollDigits();
      }, 120);

      this.activeTimer = window.setTimeout(() => {
        const num = this.generateUniqueLeadNumber();
        this.stopAnimations();
        this.latestNumber = num;
        if(this.els.engine) this.els.engine.setAttribute("data-state", "done");
        if(this.els.status) this.els.status.textContent = "הליד מוכן לעבודה";
        if(this.els.number) this.els.number.textContent = this.formatPhone(num);
        if(this.els.substatus) this.els.substatus.textContent = "המערכת יצרה מספר חדש, שמרה אותו ברשימת המספרים שכבר הופקו ומנעה כפילות במחשב הזה.";
        if(this.els.progressBar) this.els.progressBar.style.width = "100%";
        if(this.els.start) this.els.start.textContent = "צור מספר חדש";
        this.setBusy(false);
        if(this.els.copy) this.els.copy.disabled = false;
      }, 8000);
    },

    stopAnimations(){
      if(this.activeTimer){ window.clearTimeout(this.activeTimer); this.activeTimer = null; }
      if(this.activeInterval){ window.clearInterval(this.activeInterval); this.activeInterval = null; }
    },

    rollDigits(){
      if(!this.els.number) return;
      this.els.number.textContent = this.formatPhone(this.generateCandidate(false));
    },

    formatPhone(phone){
      const clean = String(phone || "").replace(/\D+/g, "");
      if(clean.length !== 10) return clean || "05X-XXX-XXXX";
    ...
    getStoredNumbers(){
      try{
        ...
        return Array.isArray(arr) ? arr.filter(v => /^\d{10}$/.test(String(v))) : [];
      }catch(_e){
        return [];
      }
    },

    saveStoredNumbers(list){
      try{ localStorage.setItem(this.storageKey, JSON.stringify(list.slice(-50000))); }catch(_e){}
    },

    generateCandidate(checkWeird=true){
      const prefix = this.prefixes[Math.floor(Math.random() * this.prefixes.length)] || "050";
      let suffix = "";
      for(let i=0; i<7; i++) suffix += String(Math.floor(Math.random() * 10));
      const candidate = prefix + suffix;
      if(!checkWeird) return candidate;
      const tail = suffix.slice(-4);
      const repeated = /(\d)/.test(candidate);
      const sequential = ["0123","1234","2345","3456","4567","5678","6789","9876","8765","7654","6543","5432","4321","3210"].includes(tail);
      return (repeated || sequential) ? this.generateCandidate(true) : candidate;
    },

    generateUniqueLeadNumber(){
      const existing = this.getStoredNumbers();
      const used = new Set(existing);
      let attempts = 0;
      let candidate = this.generateCandidate(true);
      while(used.has(candidate) && attempts < 5000){
        candidate = this.generateCandidate(true);
        attempts++;
      }
      used.add(candidate);
      this.saveStoredNumbers(Array.from(used));
      return candidate;
    },

    async copyNumber(){
      if(!this.latestNumber) return;
      const text = this.latestNumber;
      try{
        ...
        if(this.els.status) this.els.status.textContent = "המספר הועתק";
        if(this.els.substatus) this.els.substatus.textContent = "המספר הועתק ללוח. אפשר להדביק אותו ישר בהמשך העבודה.";
      }catch(_e){
        alert("לא הצלחתי להעתיק אוטומטית. אפשר להעתיק ידנית: " + this.latestNumber);
      }
    }
  };
  ...
  window.LeadGeneratorUI = LeadGeneratorUI;
