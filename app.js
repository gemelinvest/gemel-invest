...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyZ1HEzQIuA0xUP6JtvuSknONMZ3fWpl4mPLHCZ8UvEtUBwR-jXKFHLl-pllg1QRp4/exec'; // Connected (Apps Script Web App /exec)
...
$$('.navItem').forEach(b => b.addEventListener('click', (e) => {
  ...
  async function loginAttempt_(username, password){
    ...
    // If server URL is set, prefer server-side auth (Sheets)
    if(GOOGLE_SCRIPT_URL){
      try{
        const r = await apiCall('login', { username:u, password:p });
        if(!r || !r.ok){
          return { ok:false, msg: (r && (r.msg||r.error)) ? String(r.msg||r.error) : 'שגיאת התחברות לשרת' };
        }
        const user = r.user || {};
        const safeUser = {
          username: String(user.username || u).trim(),
          role: String(user.role || 'agent').trim(),
          displayName: String(user.displayName || user.username || u).trim(),
          active: true
        };

        // store session incl. user payload (so restore works even without local users)
        setSession_({ user: { username:safeUser.username, role:safeUser.role, displayName:safeUser.displayName }, at: Date.now() });
        setIdentity_(safeUser);
        showApp_();

        // Pull latest data from Sheets right after login (best-effort)
        try{
          await syncDown();
        }catch(_){}

        // Default route after login
        const rHash = location.hash ? location.hash.replace('#','').trim() : '';
        setRoute(rHash || 'customers');
        return { ok:true };
      }catch(e){
  ...
  (function bootAuth_(){
    ...
    // Prefer server/session user payload (works even if local Users not seeded)
    if(sess && sess.user && sess.user.username){
      found = sess.user;
    }else if(sess && sess.username){
      found = findUserByUsername_(sess.username);
    }
