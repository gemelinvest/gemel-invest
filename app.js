...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // ---------- Internal Mail (in-app inbox) ----------
  const mailBtn = $('#mailBtn');
  const mailBadge = $('#mailBadge');
  const mailOverlay = $('#mailOverlay');
  const mailCloseBtn = $('#mailCloseBtn');
  const mailListEl = $('#mailList');
  const mailMainEl = $('#mailMain');

  const MESSAGES_LS_KEY = 'gemel_messages_v1';
  let mailTab = 'inbox'; // inbox | sent | compose
  let mailSelectedId = null;
  let mailComposeState = { to: [], subject: '', body: '' };

  function myUserKey_(){
    return (state.currentUser && state.currentUser.username) ? state.currentUser.username
      : (state.currentUser && state.currentUser.displayName) ? state.currentUser.displayName
      : (state.agentName || 'guest');
  }

  function myDisplayName_(){
    return (state.currentUser && state.currentUser.displayName) ? state.currentUser.displayName
      : (state.currentUser && state.currentUser.username) ? state.currentUser.username
  ...
  function saveMessages_(arr){
    try{ localStorage.setItem(MESSAGES_LS_KEY, JSON.stringify(arr || [])); }catch(_){}
    updateMailBadge_();
  }

  function isDeletedForMe_(msg, me){
    const del = msg && msg.deletedBy;
    return Array.isArray(del) && del.includes(me);
  }
  function isReadForMe_(msg, me){
    const rb = msg && msg.readBy;
    return Array.isArray(rb) && rb.includes(me);
  }
  function markRead_(id){
    const me = myUserKey_();
    const msgs = loadMessages_();
  ...
  function markDeleted_(id){
    ...
    const m = msgs.find(x => x && x.id === id);
    if(!m) return;
    if(!Array.isArray(m.deletedBy)) m.deletedBy = [];
    if(!m.deletedBy.includes(me)) m.deletedBy.push(me);
    // if deleted message was selected – clear selection
    if(mailSelectedId === id) mailSelectedId = null;
    saveMessages_(msgs);
    renderMailOverlay_();
  }

  function inboxMessages_(){
    const me = myUserKey_();
    return loadMessages_()
      .filter(m => m && Array.isArray(m.toUsers) && m.toUsers.includes(me) && !isDeletedForMe_(m, me))
      .sort((a,b) => (b.createdAt||0) - (a.createdAt||0));
  }

  function sentMessages_(){
    const me = myUserKey_();
    return loadMessages_()
      .filter(m => m && m.fromUser === me && !isDeletedForMe_(m, me))
      .sort((a,b) => (b.createdAt||0) - (a.createdAt||0));
  }

  function unreadCount_(){
    const me = myUserKey_();
    const list = inboxMessages_();
    return list.filter(m => !isReadForMe_(m, me)).length;
  }

  function updateMailBadge_(){
    if(!mailBadge) return;
    ...
    const n = unreadCount_();
    mailBadge.textContent = String(n);
    if(n > 0) mailBadge.classList.remove('hidden');
  ...
  function openMail_(){
    ...
    if(!state.currentUser){
      toast('שגיאה', 'צריך להתחבר כדי להשתמש בדואר הפנימי');
      return;
    }
    mailTab = 'inbox';
    mailSelectedId = null;
    mailComposeState = { to: [], subject: '', body: '' };
    renderMailOverlay_();
  ...
  function setMailTab_(tab){
    mailTab = tab;
    if(tab !== 'compose') mailComposeState = { to: [], subject: '', body: '' };
    if(tab === 'compose') mailSelectedId = null;
    renderMailOverlay_();
  }

  function mailHeaderHtml_(title, sub){
    return `
      <div class="mailHead">
        <div>
          <div class="mailH">${escapeHtml(title||'')}</div>
          ${sub ? `<div class="mailSub">${escapeHtml(sub)}</div>` : ``}
        </div>
      </div>
    `;
  }

  function renderMailOverlay_(){
    if(!mailOverlay || !mailListEl || !mailMainEl) return;

    // tabs active state
    mailOverlay.querySelectorAll('.mailTab').forEach(b => {
      const t = b.getAttribute('data-mailtab');
      if(t === mailTab) b.classList.add('active');
      else b.classList.remove('active');
    });

    // list
    let list = [];
    if(mailTab === 'inbox') list = inboxMessages_();
    else if(mailTab === 'sent') list = sentMessages_();
    else list = [];
    ...
    if(mailTab === 'compose'){
      mailListEl.innerHTML = `
        <div class="mailEmpty">
          <div class="mailEmptyTitle">כתיבת הודעה</div>
          <div class="mailEmptySub">בחר/י נציגים ושלח/י הודעה פנימית.</div>
        </div>
      `;
    } else if(!list.length){
      mailListEl.innerHTML = `
        <div class="mailEmpty">
          <div class="mailEmptyTitle">אין הודעות</div>
          <div class="mailEmptySub">${mailTab === 'sent' ? 'לא נשלחו הודעות עדיין.' : 'תיבת הדואר שלך ריקה.'}</div>
        </div>
      `;
    } else {
      mailListEl.innerHTML = list.map(m => {
        const isUnread = (mailTab === 'inbox') && !isReadForMe_(m, me);
        const who = (mailTab === 'sent') ? ('אל: ' + (m.toNames && m.toNames.length ? m.toNames.join(', ') : (m.toUsers||[]).join(', '))) : ('מאת: ' + (m.fromName || m.fromUser || ''));
        const dt = m.createdAt ? new Date(m.createdAt) : null;
        const when = dt ? dt.toLocaleString('he-IL') : '';
        const subject = m.subject ? m.subject : '(ללא נושא)';
        const active = (mailSelectedId === m.id) ? ' active' : '';
        const unreadCls = isUnread ? ' unread' : '';
        return `
          <button class="mailItem${active}${unreadCls}" type="button" data-mid="${escapeAttr(m.id)}">
            <div class="mailItemTop">
              <b class="mailSubject">${escapeHtml(subject)}</b>
              ${isUnread ? `<span class="mailDot" aria-hidden="true"></span>` : ``}
            </div>
            <div class="mailMeta">${escapeHtml(who)}</div>
            <div class="mailMeta2">${escapeHtml(when)}</div>
          </button>
        `;
      }).join('');
    }
    ...
    // bind list clicks
    mailListEl.querySelectorAll('[data-mid]').forEach(btn => {
      btn.addEventListener('click', () => {
        const id = btn.getAttribute('data-mid');
        mailSelectedId = id;
        if(mailTab === 'inbox') markRead_(id);
        renderMailOverlay_();
      });
    });
    ...
    if(!mailSelectedId){
      mailMainEl.innerHTML = `
        ${mailHeaderHtml_(mailTab === 'sent' ? 'נשלח' : 'נכנס', 'בחר/י הודעה מהרשימה כדי לצפות בה.')}
        <div class="mailHintCard">
          <div class="mailHintTitle">טיפ</div>
          <div class="mailHintText">אפשר ללחוץ על “כתוב הודעה” כדי לשלוח הודעה לכמה נציגים במקביל.</div>
        </div>
      `;
      return;
    }

    const all = loadMessages_();
    const msg = all.find(x => x && x.id === mailSelectedId);
    if(!msg){
      mailSelectedId = null;
      mailMainEl.innerHTML = mailHeaderHtml_('לא נמצא', 'ההודעה לא נמצאה או נמחקה.');
      return;
    }

    const dt = msg.createdAt ? new Date(msg.createdAt) : null;
    const when = dt ? dt.toLocaleString('he-IL') : '';
    const subject = msg.subject ? msg.subject : '(ללא נושא)';
    const fromLine = `מאת: ${escapeHtml(msg.fromName || msg.fromUser || '')}`;
    const toLine = `אל: ${escapeHtml((msg.toNames && msg.toNames.length ? msg.toNames.join(', ') : (msg.toUsers||[]).join(', ')))}`;

    mailMainEl.innerHTML = `
      ${mailHeaderHtml_(subject, when)}
      <div class="mailViewMeta">
        <div>${fromLine}</div>
        <div>${toLine}</div>
      </div>
      <div class="mailViewBody">${escapeHtml(msg.body || '').replace(/\n/g,'<br>')}</div>
      <div class="mailViewActions">
        <button class="btn btnSoft" type="button" id="mailDeleteBtn">מחק אצלי</button>
        <button class="btn btnPrimary" type="button" id="mailReplyBtn">השב</button>
      </div>
    `;

    $('#mailDeleteBtn')?.addEventListener('click', () => {
      markDeleted_(mailSelectedId);
      toast('עודכן', 'נמחק אצלך בלבד');
    });

    $('#mailReplyBtn')?.addEventListener('click', () => {
      // Reply opens compose with recipient set to sender (if not me)
      const meU = myUserKey_();
      const toU = msg.fromUser && msg.fromUser !== meU ? [msg.fromUser] : [];
      mailTab = 'compose';
      mailSelectedId = null;
      mailComposeState = { to: toU, subject: subject.startsWith('Re:') ? subject : ('Re: ' + subject), body: `\n\n——\n${msg.fromName || msg.fromUser || ''} כתב/ה ב-${when}:\n${msg.body||''}` };
      renderMailOverlay_();
    });
  ...
  function renderMailCompose_(){
    const users = (typeof getUsers_ === 'function') ? getUsers_() : [];
    const me = myUserKey_();

    const candidates = (users||[])
      .filter(u => u && u.username && u.username !== me)
      .map(u => ({ username: u.username, name: u.displayName || u.username }));

    const toNames = (mailComposeState.to||[]).map(u => {
      const found = candidates.find(c => c.username === u);
      return found ? found.name : u;
    });

    mailMainEl.innerHTML = `
      ${mailHeaderHtml_('כתוב הודעה', 'שליחה לכמה נציגים במקביל')}
      <div class="mailCompose">
        <label class="mailLabel">אל:</label>
        <div class="mailToBox">
          <div class="mailChips" id="mailChips">
            ${(mailComposeState.to||[]).map((u,i) => `
              <span class="mailChip">
                <span>${escapeHtml(toNames[i] || u)}</span>
                <button type="button" class="mailChipX" data-x="${escapeAttr(u)}" aria-label="הסר">×</button>
              </span>
            `).join('')}
            <input id="mailToSearch" class="mailToSearch" placeholder="חפש נציג..." autocomplete="off"/>
          </div>
          <div class="mailToResults hidden" id="mailToResults"></div>
        </div>

        <label class="mailLabel" for="mailSubject">נושא:</label>
        <input id="mailSubject" class="mailInput" type="text" value="${escapeAttr(mailComposeState.subject||'')}" placeholder="נושא ההודעה"/>

        <label class="mailLabel" for="mailBody">תוכן:</label>
        <textarea id="mailBody" class="mailTextarea" rows="10" placeholder="כתוב הודעה...">${escapeHtml(mailComposeState.body||'')}</textarea>

        <div class="mailComposeActions">
          <button class="btn btnSoft" type="button" id="mailComposeCancel">ביטול</button>
          <button class="btn btnPrimary" type="button" id="mailComposeSend">שלח</button>
        </div>
      </div>
    `;

    // chip remove
    $('#mailChips')?.querySelectorAll('[data-x]').forEach(b => b.addEventListener('click', () => {
      const u = b.getAttribute('data-x');
      mailComposeState.to = (mailComposeState.to||[]).filter(x => x !== u);
      renderMailCompose_();
    }));

    const toSearch = $('#mailToSearch');
    const results = $('#mailToResults');

    function showResults_(q){
      if(!results) return;
      const query = String(q||'').trim().toLowerCase();
      const avail = candidates
        .filter(c => !mailComposeState.to.includes(c.username))
        .filter(c => !query || c.name.toLowerCase().includes(query) || c.username.toLowerCase().includes(query))
        .slice(0, 10);

      if(!avail.length){
        results.innerHTML = `<div class="mailToEmpty">לא נמצאו נציגים</div>`;
      } else {
        results.innerHTML = avail.map(c => `
          <button type="button" class="mailToRow" data-u="${escapeAttr(c.username)}">
      ...
      results.querySelectorAll('[data-u]').forEach(b => b.addEventListener('click', () => {
        const u = b.getAttribute('data-u');
        if(u && !mailComposeState.to.includes(u)){
          mailComposeState.to.push(u);
          renderMailCompose_();
        }
      }));
    }

    toSearch?.addEventListener('input', () => showResults_(toSearch.value));
    toSearch?.addEventListener('focus', () => showResults_(toSearch.value));
    document.addEventListener('click', (e) => {
      if(!results || !toSearch) return;
      const inBox = e.target === results || results.contains(e.target) || e.target === toSearch || toSearch.contains(e.target) || $('#mailChips')?.contains(e.target);
      if(!inBox) results.classList.add('hidden');
    }, { capture:true, once:true });

    $('#mailSubject')?.addEventListener('input', (e) => mailComposeState.subject = e.target.value);
    $('#mailBody')?.addEventListener('input', (e) => mailComposeState.body = e.target.value);

    $('#mailComposeCancel')?.addEventListener('click', () => {
      setMailTab_('inbox');
    });

    $('#mailComposeSend')?.addEventListener('click', () => {
      const to = (mailComposeState.to||[]).filter(Boolean);
      const subject = String(mailComposeState.subject||'').trim();
      const body = String(mailComposeState.body||'').trim();

      if(!to.length){
        toast('שגיאה', 'בחר לפחות נציג אחד');
        return;
      }
      if(!body){
        toast('שגיאה', 'כתוב תוכן הודעה');
        return;
      }
      ...
      const msg = {
        ...
        fromUser: myUserKey_(),
        fromName: myDisplayName_(),
        toUsers: to,
        toNames: to.map(u => {
          const f = candidates.find(c => c.username === u);
          return f ? f.name : u;
        }),
        subject: subject || '(ללא נושא)',
        body,
        createdAt: now,
        readBy: [myUserKey_()], // sender considered read
        deletedBy: []
      };
      ...
      toast('נשלח', 'ההודעה נשלחה בהצלחה');
      mailComposeState = { to: [], subject: '', body: '' };
      setMailTab_('sent');
    });
  }
  ...
  // Bind mail events
  if(mailBtn) mailBtn.addEventListener('click', openMail_);
  if(mailCloseBtn) mailCloseBtn.addEventListener('click', closeMail_);
  if(mailOverlay){
    mailOverlay.addEventListener('click', (e) => {
      if(e.target === mailOverlay) closeMail_();
    });
    mailOverlay.querySelectorAll('[data-mailtab]').forEach(b => b.addEventListener('click', () => setMailTab_(b.getAttribute('data-mailtab'))));
  }
  ...
  function renderSettings(){
    ...
    viewEl.innerHTML = `
      <div class="grid2">
        ...
        <div class="sideCard">
          <div class="sideTitle">גרסה • עדכון • קאש</div>
          <div class="sideHint"><b>גרסה:</b> <span id="verLabel">—</span></div>
          <div class="sideHint"><b>BUILD:</b> <span id="buildLabel">—</span></div>
          <div class="sideHint"><b>Service Worker:</b> <span id="swLabel">—</span></div>
          <div class="sideHint"><b>Cache Storage:</b> <span id="cacheLabel">—</span></div>
          <div style="height:10px"></div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button class="btn btnPrimary" id="hardRefreshBtn">רענון נקי</button>
            <button class="btn btnSoft" id="clearCacheBtn">נקה קאש (PWA)</button>
            <button class="btn btnSoft" id="clearLocalBtn">נקה נתונים מקומיים</button>
          </div>
          <div class="sideHint" style="margin-top:10px">טיפ: אם עשית עדכון ולא רואים שינוי — לחץ “רענון נקי”.</div>
        </div>
    ...
    if(verLabel) verLabel.textContent = (window.APP_VERSION || '—');
    if(buildLabel){
      const b = window.BUILD;
      buildLabel.textContent = b ? (new Date(b).toLocaleString('he-IL') + ' • ' + b) : '—';
    }

    async function swStatus_(){
      try{
        if(!('serviceWorker' in navigator)) return { supported:false };
        const reg = await navigator.serviceWorker.getRegistration();
        return { supported:true, registered: !!reg, reg };
      }catch(_){ return { supported:false }; }
    }
    async function cacheStatus_(){
      try{
    ...
    async function refreshAbout_(){
      const sw = await swStatus_();
      if(swLabel){
        if(!sw.supported) swLabel.textContent = 'לא נתמך';
        else swLabel.textContent = sw.registered ? 'פעיל' : 'לא רשום';
      }
      const cs = await cacheStatus_();
      if(cacheLabel){
        if(!cs.supported) cacheLabel.textContent = 'לא נתמך';
        else cacheLabel.textContent = cs.keys.length ? (cs.keys.length + ' קאש(ים)') : 'ריק';
      }
    }
    refreshAbout_();

    async function clearPwaCache_(){
      const cs = await cacheStatus_();
      if(cs.supported && cs.keys?.length){
        await Promise.all(cs.keys.map(k => caches.delete(k)));
      }
      const sw = await swStatus_();
      if(sw.supported && sw.reg){
        try{ await sw.reg.unregister(); }catch(_){}
      }
    }

    $('#clearCacheBtn')?.addEventListener('click', async () => {
      if(!confirm('לנקות קאש PWA ולבטל Service Worker?')) return;
      try{
        await clearPwaCache_();
        await refreshAbout_();
        toast('✅ הקאש נוקה');
      }catch(_){
        toast('⚠️ לא הצלחתי לנקות קאש');
      }
    });

    $('#clearLocalBtn')?.addEventListener('click', () => {
      if(!confirm('למחוק נתונים מקומיים (כולל משתמשים/דמו)?')) return;
      try{
        localStorage.clear();
        ...
        toast('⚠️ לא הצלחתי למחוק נתונים');
      }
    });
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3" style="margin-top:10px">
        ...
        ${selectField('סטטוס', 'old_decision', ['cancel_full|ביטול מלא','keep|להשאיר ללא שינוי','appoint|ביצוע מינוי סוכן'])}
      </div>
        ...
        <table>
          ...
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => {
              ...
              if(dec==='cancel_partial' || dec==='ביטול חלקי') dec='cancel_full';
    ...
    $('#addOldBtn').addEventListener('click', () => {
      ...
      if(isSumProduct(product) && sumInsured <= 0){ toast('שגיאה', 'חובה למלא סכום ביטוח בריסק'); sumInput && sumInput.focus && sumInput.focus(); return; }
      p.oldPolicies.push({ insuredId, company, product, policyNo, premium, sumInsured: (isSumProduct(product)? sumInsured : undefined), compensation: (isCompProduct(product)? compensation : undefined), decision });
...
`;
  ...
  function renderSummaryStep(){
    ...
    if(markDoneBtnEl) markDoneBtnEl.addEventListener('click', () => {
      ...
      toast('נשמר', 'ההצעה נשמרה בהיסטוריה של הלקוח');
...
/* ===== Mail Toast + Sound ===== */
const MAIL_SOUND = new Audio('data:audio/wav;base64,UklGRhQAAABXQVZFZm10IBAAAAABAAEAESsAACJWAAACABAAZGF0YQAAAAA=');

function showMailToast(fromName, onOpen){
  let wrap = document.querySelector('.mailToastWrap');
  if(!wrap){
    wrap = document.createElement('div');
    wrap.className = 'mailToastWrap';
    document.body.appendChild(wrap);
  }
  const t = document.createElement('div');
  t.className = 'mailToast';
  t.innerHTML = `
    <div class="from">מאת: ${fromName}</div>
    <div class="actions">
      <button class="dismiss">דחה</button>
      <button class="open">פתח</button>
    </div>
  `;
  wrap.appendChild(t);
  try{ MAIL_SOUND.play(); }catch(e){}
  t.querySelector('.dismiss').onclick = ()=>t.remove();
  t.querySelector('.open').onclick = ()=>{
    t.remove();
...
/* Hook into message save to detect new inbox message */
const _saveMessagesOrig = saveMessages_;
saveMessages_ = function(arr){
  const before = loadMessages_();
  _saveMessagesOrig(arr);
  const after = loadMessages_();
  const me = myUserKey_();
  const newMsg = after.find(m =>
    (!before.find(b=>b.id===m.id)) &&
    Array.isArray(m.toUsers) && m.toUsers.includes(me)
  );
  if(newMsg){
    showMailToast(newMsg.fromName || newMsg.fromUser || 'נציג', ()=>openMail_());
  }
};
