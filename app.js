...
(() => {
  ...
  const BUILD = "20260409-vCustomerSaveHotfix1";
...
// ---------- Step 6 ----------
    ...
    async saveCompletedCustomer(){
      ...
      try {
        ...
        if(!verifiedOnServer){
          console.warn('CUSTOMER_SAVE_VERIFY_SKIPPED', verify?.error || 'VERIFY_EMPTY_RESULT', record.id);
        }
      } catch(verifyErr) {
        console.warn('CUSTOMER_SAVE_VERIFY_FAILED', verifyErr?.message || verifyErr, record.id);
      }
