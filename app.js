...
(() => {
  ...
  const BUILD = "20260324-operational-report-pdf-engine-3";
...
// ---------- Step 6 ----------
    ...
    async exportOperationalPdf(payloadOverride){
      ...
      try{
        ...
        const logoImg = target?.querySelector?.('.lcPdfHeader__logo');
        if(logoImg){
        ...
        const filename = this.getOperationalPdfFileName(pack.payload);
        const worker = html2pdfFactory()
          .set({
            margin: [0, 0, 0, 0],
            filename,
