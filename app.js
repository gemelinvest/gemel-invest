...
(() => {
  ...
  const BUILD = "20260412-vAgentsDataRetentionFix";
  ...
  const Storage = {
    ...
    async saveSheets(state){
      try {
        ...
        await this.syncTable(SUPABASE_TABLES.agents, this.buildAgentRows(state), { allowDelete:false });
        await this.syncTable(SUPABASE_TABLES.customers, this.buildCustomerRows(state), { allowDelete:false });
        await this.syncTable(SUPABASE_TABLES.proposals, this.buildProposalRows(state), { allowDelete:false });
        return { ok:true, at: nowISO(), mode:'safe-upsert-only' };
      } catch(e) {
        return { ok:false, error: String(e?.message || e) };
      }
    },
