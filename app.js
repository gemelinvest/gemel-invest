...
(() => {
  ...
  const BUILD = "20260423-vHarDateSkipFix-v1";
  ...
  const SUPABASE_KEEPALIVE_MS = 30000;
  ...
  const normalizePhoneValue = (value) => digitsOnly(value).slice(0, 10);
  ...
  function normalizeState(s){
    ...
    out.agents = (out.agents || []).map((a, idx) => {
      ...
      return {
        ...
        monthlySalesTarget: Number(String(a?.monthlySalesTarget ?? a?.monthly_sales_target ?? '').replace(/[^\d.-]/g, '')) || 0,
        role,
        active
      };
    }).filter(a => a.name);
  ...
  function parseBirthDateValue(value){
    ...
    return { year:y, month:m, day:d, date:birth };
  }
  ...
  const Storage = {
    ...
    async probeConnection(){
      ...
      try {
        ...
        const probes = [
          async () => client.from(SUPABASE_TABLES.meta).select('key').limit(1),
          async () => client.from(SUPABASE_TABLES.agents).select('id').limit(1),
          async () => client.from(SUPABASE_TABLES.customers).select('id').limit(1)
        ];
        let lastError = null;
        ...
        throw lastError || new Error('SUPABASE_PROBE_FAILED');
      } catch(err){
    ...
    buildMetaRow(state){
      return {
        ...
        payload: {
          ...
          agentsShadow: (state?.agents || []).map((a, idx) => ({
            id: safeTrim(a?.id) || ("a_" + idx),
          ...
          customersShadow: (state?.customers || []).map((c, idx) => normalizeCustomerRecord(c, idx)),
          proposalsShadow: (state?.proposals || []).map((p, idx) => normalizeProposalRecord(p, idx)),
          updatedAt: nowISO()
        },
        updated_at: nowISO()
      };
    ...
    buildProposalRows(state){
      return (state?.proposals || []).map((p, idx) => ({
        ...
        current_step: Math.max(1, Math.min(8, Number(p?.currentStep || 1) || 1)),
    ...
    async upsertSingleRow(tableName, row){
    ...
    async loadSingleRow(tableName, id, selectExpr='*'){
      ...
      try {
        const client = this.getClient();
        const { data, error } = await client.from(tableName).select(selectExpr).eq('id', safeId).maybeSingle();
        if(error) throw error;
    ...
    async deleteRows(tableName, ids = []){
      ...
      try {
        const client = this.getClient();
        const { error } = await client.from(tableName).delete().in('id', normalizedIds);
        if(error) throw error;
    ...
    async loadSheets(options = {}){
      ...
      try {
        ...
        const customerRows = (customersRes.data || []).map((row, idx) => this.mapCustomerRow(row, idx));
        const proposalRows = (proposalsRes.data || []).map((row, idx) => this.mapProposalRow(row, idx));
    ...
    async saveSheets(state, options = {}){
      try {
        ...
        const localState = normalizeState(state || defaultState());
        ...
        if(options.skipServerMerge !== true){
          const latest = await this.loadSheets({ useCachedFallback:false });
          if(latest?.ok && latest.payload){
            serverState = normalizeState(latest.payload);
            conflictInfo.serverAt = safeTrim(serverState?.meta?.updatedAt) || "";
            const serverRevision = normalizeRevisionMeta(serverState?.meta?.revision, conflictInfo.serverAt);
        ...
        const mergedState = buildSafeMergedState(localState, serverState || defaultState(), { bumpRevision:true });
        mergedState.meta.updatedAt = nowISO();
        ...
        await this.upsertMeta(mergedState);
        let agentsSyncWarning = '';
        let customersSyncWarning = '';
        let proposalsSyncWarning = '';
        try {
          await this.syncTable(SUPABASE_TABLES.agents, this.buildAgentRows(mergedState), { allowDelete:false });
        } catch(agentSyncErr) {
          agentsSyncWarning = String(agentSyncErr?.message || agentSyncErr || 'AGENTS_SYNC_FAILED');
          console.warn('AGENTS_SYNC_WARNING:', agentsSyncWarning);
        }
        try {
          await this.syncTable(SUPABASE_TABLES.customers, this.buildCustomerRows(mergedState), { allowDelete:false });
        } catch(customerSyncErr) {
          customersSyncWarning = String(customerSyncErr?.message || customerSyncErr || 'CUSTOMERS_SYNC_FAILED');
          console.warn('CUSTOMERS_SYNC_WARNING:', customersSyncWarning);
        }
        try {
          await this.syncTable(SUPABASE_TABLES.proposals, this.buildProposalRows(mergedState), { allowDelete:false });
        } catch(proposalSyncErr) {
          proposalsSyncWarning = String(proposalSyncErr?.message || proposalSyncErr || 'PROPOSALS_SYNC_FAILED');
        ...
        return {
          ...
          syncWarnings: [agentsSyncWarning, customersSyncWarning, proposalsSyncWarning].filter(Boolean)
        };
      } catch(e) {
        return { ok:false, error: String(e?.message || e) };
      }
...
// nav
      ...
      on(this.els.myProcessesSearch, "input", () => void 0 /* ProcessesUI removed */);
      on(this.els.btnMyProcessesRefresh, "click", () => void 0 /* ProcessesUI removed */);
...
UsersGateUI.init();
    ...
    goView(view){
      ...
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-myTools-active","view-customers-active","view-proposals-active","view-myProcesses-active","view-mirrors-active");
      document.body.classList.add("view-" + safe + "-active");

      // render view data
      if (safe === "settings") this.renderSettingsView();
      // render view data
      if (safe === "dashboard") DashboardUI.render();
      if (safe === "users") UsersUI.render();
      if (safe === "customers") CustomersUI.render();
      if (safe === "proposals") ProposalsUI.render();
      if (safe === "myProcesses") void 0 /* ProcessesUI removed */;
      // mirrors view removed
    },
    ...
    async publishVersion(){
      ...
      try {
        ...
        const res = await fetch(SUPABASE_URL + '/rest/v1/' + SUPABASE_TABLES.meta, {
          method: 'POST',
          cache: 'no-store',
          headers,
          body: JSON.stringify({ key: 'app_version', payload: newVersion })
        });
        if(!res.ok){
          let msg = 'HTTP_' + res.status;
          try { const j = await res.json(); msg = j?.message || j?.error || msg; } catch(_e){}
          throw new Error(msg);
        }
        if(statusEl){ statusEl.style.color = 'var(--brandC)'; statusEl.textContent = '✓ גרסה ' + newVersion + ' פורסמה — הנציגים יקבלו הודעה תוך 3 דקות'; }
      } catch(e){
    ...
    renderAuthPill(){
      ...
      if(Auth.current) {
        ...
        const _pillRole = Auth.current?.role === "owner" ? "מפתח המערכת" : Auth.isAdmin() ? "מנהל מערכת" : Auth.isManager() ? "מנהל" : Auth.isOps() ? "תפעול" : "נציג";
  ...
  const UsersUI = {
    ...
    openModal(mode, user){
      ...
      if(E.monthlyTarget) E.monthlyTarget.value = user ? String(Number(user.monthlySalesTarget || 0) || '') : '';
    ...
    async _saveFromModal(){
      ...
      const monthlySalesTarget = Number(String(E.monthlyTarget?.value || '').replace(/[^\d.-]/g, '')) || 0;
      ...
      if(isEdit){
        ...
        a.active = active;
        State.data.meta.updatedAt = nowISO();
        appendAuditLog({ type:"agent_updated", entity:"agent", entityId:a.id, label:"עודכן משתמש", actorName:safeTrim(Auth?.current?.name), details:{ name, username, role:a.role, active:a.active } });
        refreshStateShadows();
        ...
        State.data.meta.updatedAt = nowISO();
        appendAuditLog({ type:"agent_created", entity:"agent", entityId:newId, label:"נשמר משתמש חדש", actorName:safeTrim(Auth?.current?.name), details:{ name, username, role, active:true } });
        refreshStateShadows();
          ...
          <tr>
            ...
            <td>
              <div class="lcUsers__rowActions">
                ...
                <button class="btn" data-act="security" data-id="${escapeHtml(a.id)}">אבטחה</button>
  ...
  const AppToast = {
    ...
    normalizePayload(payload, opts = {}){
      ...
      const signature = safeTrim(merged.signature) || JSON.stringify({ title, text, items, singletonKey, variant });
      return { title, text, items, singletonKey, variant, durationMs, signature };
    },
  ...
  const CustomersUI = {
    ...
    render(){
      ...
      }).join("") : `<tr><td colspan="5"><div class="emptyState"><div class="emptyState__icon">🗂️</div><div class="emptyState__title">עדיין אין לקוחות</div><div class="emptyState__text">ברגע שמסיימים הקמת לקוח, הלקוח יישמר כאן אוטומטית ויהיה אפשר לפתוח את תיק הלקוח המלא.</div></div></td></tr>`;
    ...
    buildUniformDiscountSchedule(years, pct){
      const totalYears = Math.max(0, Math.min(10, Number(years) || 0));
    ...
    getPolicyPremiumAfterDiscount(policy){
      ...
      // אם יש ממוצע אפקטיבי מהכיסויים — משתמש בו; אחרת ב-discountPct הרגיל
      const effectiveRaw = policy?.discountPctEffective ?? policy?.discountPct ?? 0;
      const pct = Math.max(0, Number(String(effectiveRaw).replace(/[^\d.\-]/g, '')) || 0);
    ...
    renderHeroMeta(rec){
      ...
      const idIcon = `
        <span class="customerHero__metaBadge" aria-hidden="true">
      ...
      const phoneIcon = `
        <span class="customerHero__metaBadge" aria-hidden="true">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
            <path d="M22 16.92v2a2 2 0 0 1-2.18 2 19.86 19.86 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6A19.86 19.86 0 0 1 2.12 3.18 2 2 0 0 1 4.11 1h2a2 2 0 0 1 2 1.72c.12.9.33 1.78.63 2.62a2 2 0 0 1-.45 2.11L7.1 8.91a16 16 0 0 0 8 8l1.46-1.19a2 2 0 0 1 2.11-.45c.84.3 1.72.51 2.62.63A2 2 0 0 1 22 16.92z"></path>
          </svg>
        </span>`;
      const agentIcon = `
        <span class="customerHero__metaBadge" aria-hidden="true">
      ...
      const topRow = [
        idNumber ? `<div class="customerHero__metaInfo customerHero__metaInfo--id">${idIcon}<span class="customerHero__metaLabel">ת״ז</span><strong class="customerHero__metaValue" dir="ltr">${escapeHtml(idNumber)}</strong></div>` : '',
        phone ? `<div class="customerHero__metaInfo customerHero__metaInfo--phone">${phoneIcon}<span class="customerHero__metaLabel">טלפון</span><strong class="customerHero__metaValue" dir="ltr">${escapeHtml(phone)}</strong></div>` : ''
      ].filter(Boolean).join('<span class="customerHero__metaInlineSep" aria-hidden="true"></span>');
      const bottomRow = agentName ? `<div class="customerHero__metaInfo customerHero__metaInfo--agent">${agentIcon}<span class="customerHero__metaLabel">נציג מטפל</span><strong class="customerHero__metaValue">${escapeHtml(agentName)}</strong></div>` : '';
      return `
        <div class="customerHero__metaRow customerHero__metaRow--top">${topRow}</div>
        ${bottomRow ? `<div class="customerHero__metaRow customerHero__metaRow--bottom">${bottomRow}</div>` : ''}
      `;
    },
    ...
    openById(id, opts={}){
      ...
      try{
        ...
        if(this.els.meta){
          this.els.meta.innerHTML = this.renderHeroMeta(rec);
        }
        if(this.els.dash){
    ...
    renderOperationalReflectionCard(state){
      ...
      const updated = safeTrim(state?.updatedText) ? '—' : '—';
  ...
  const ArchiveCustomerUI = {
    ...
    async confirm(){
      ...
      State.data.meta.updatedAt = nowISO();
      appendAuditLog({ type:"customer_deleted", entity:"customer", entityId:safeTrim(id), label:"הלקוח נגנז ונמחק", actorName:safeTrim(Auth?.current?.name), details:{ previousCount: prevCustomers.length } });

      const r = await Storage.deleteRows(SUPABASE_TABLES.customers, [id]);
      if(!r?.ok){
      ...
      UI.renderSyncStatus("הלקוח נגנז ונמחק", "ok", r.at);
  ...
  const CustomerEditUI = {
    ...
    handleFieldEvent(ev){
      ...
      if(path === "primary.clinic"){
        const allowed = this.shabanMap?.[safeTrim(value)] || [];
        const currentShaban = safeTrim(this.field("primary.shaban"));
        if(!allowed.includes(currentShaban)) this.setByPath(this.draft, "primary.shaban", "");
        this.render();
        return;
      }
      ...
      return `
        <section class="customerEditSection">
          ...
          <div class="customerEditGrid">
            ...
            ${this.renderSelect("קופת חולים","primary.clinic", p.clinic, [''].concat(this.clinics || []).map(opt => `<option value="${this.esc(opt)}">${this.esc(opt || 'בחר…')}</option>`).join(''))}
            ${this.renderSelect("שב״ן","primary.shaban", p.shaban, [''].concat(this.shabanMap?.[p.clinic] || []).map(opt => `<option value="${this.esc(opt)}">${this.esc(opt || 'בחר…')}</option>`).join(''))}
    ...
    getHealthEditorContext(){
      ...
      ctx.getHealthStore = Wizard.getHealthStore;
    ...
    getDraftHealthQuestionList(){
      ...
      return typeof ctx.getHealthQuestionList === 'function' ? (ctx.getHealthQuestionList() || []) : [];
    },

    getDraftHealthResponse(qKey, insId){
      const ctx = this.getHealthEditorContext();
      return typeof ctx.getHealthResponse === 'function' ? ctx.getHealthResponse(qKey, insId) : { answer:'', fields:{}, saved:false };
    },

    setDraftHealthResponse(qKey, insId, patch){
      const ctx = this.getHealthEditorContext();
      if(typeof ctx.setHealthResponse === 'function') ctx.setHealthResponse(qKey, insId, patch || {});
    },

    validateDraftHealthDetail(question, insId){
      const ctx = this.getHealthEditorContext();
      return typeof ctx.validateHealthDetail === 'function' ? !!ctx.validateHealthDetail(question, insId) : true;
    },
    ...
    renderDraftHealthField(question, insId, field){
      ...
      const response = this.getDraftHealthResponse(question.key, insId);
      const val = safeTrim(response?.fields?.[field.key] || '');
    ...
    renderHealthEditSection(){
      ...
      const companies = typeof ctx.getHealthCompanies === 'function' ? (ctx.getHealthCompanies() || []) : [];
      if(!list.length){
      ...
      const grouped = [];
      list.forEach(item => {
        let group = grouped.find(g => g.key === item.catKey);
        if(!group){
          group = { key:item.catKey, title:item.catTitle, summary:item.catSummary || '', items:[] };
          grouped.push(group);
        }
        ...
        <div class="customerEditHealth__stack">${grouped.map(group => `
          <div class="customerEditHealthGroup">
            ...
            <div class="customerEditHealthQuestionList">${group.items.map((item, index) => {
              ...
              const reqs = typeof ctx.getHealthQuestionRequirements === 'function' ? (ctx.getHealthQuestionRequirements(q) || []) : [];
              return `<article class="customerEditHealthQuestion">
                <div class="customerEditHealthQuestion__top">
                  ...
                  <div class="customerEditHealthQuestion__companies">${(q.companies || companies || []).map(c => `<span class="customerEditHealth__chip customerEditHealth__chip--soft">${this.esc(c)}</span>`).join('')}</div>
                </div>
                ${reqs.length ? `<div class="customerEditHealthReqs">${reqs.map(req => `<div class="customerEditHealthReq"><strong>${this.esc(req.company || 'כללי')}</strong><span>${this.esc((req.items || []).join(' • '))}</span></div>`).join('')}</div>` : ''}
                <div class="customerEditHealthMatrix">${(this.draft.insureds || []).map(ins => {
                  const response = this.getDraftHealthResponse(q.key, ins.id);
                  const yes = response.answer === 'yes';
                  const no = response.answer === 'no';
                  const valid = this.validateDraftHealthDetail(q, ins.id);
                  return `<div class="customerEditHealthRow ${yes ? 'is-yes' : no ? 'is-no' : ''}">
                    <div class="customerEditHealthRow__who">${this.esc(ins.label || 'מבוטח')}</div>
                    <div class="customerEditHealthRow__answers">
                      <button type="button" class="customerEditHealthAnswer ${yes ? 'is-active' : ''}" data-ce-health-answer="${this.esc(`${q.key}|${ins.id}|yes`)}">כן</button>
                      <button type="button" class="customerEditHealthAnswer ${no ? 'is-active' : ''}" data-ce-health-answer="${this.esc(`${q.key}|${ins.id}|no`)}">לא</button>
                    </div>
                    <div class="customerEditHealthRow__body">${yes ? `
                      <div class="customerEditHealthEditor ${response.saved ? 'is-saved' : ''}">
                        ${(q.fields || []).length ? `<div class="customerEditHealthFields">${(q.fields || []).map(field => this.renderDraftHealthField(q, ins.id, field)).join('')}</div>` : `<div class="customerEditHealth__noDetails">לשאלה הזו אין שדות פירוט נוספים.</div>`}
                        <div class="customerEditHealthEditor__actions">
                          <button type="button" class="btn btn--primary" data-ce-health-save="${this.esc(`${q.key}|${ins.id}`)}">שמור פירוט</button>
                          <button type="button" class="btn" data-ce-health-edit="${this.esc(`${q.key}|${ins.id}`)}">ערוך מחדש</button>
                          <button type="button" class="btn" data-ce-health-clear="${this.esc(`${q.key}|${ins.id}`)}">נקה</button>
                      ...
                      </div>` : `<div class="customerEditHealthRow__placeholder">בחר תשובה כדי לערוך את פירוט ההצהרה עבור ${this.esc(ins.label || 'המבוטח')}.</div>`}
                    </div>
                  </div>`;
                }).join('')}</div>
              </article>`;
  ...
  const ProposalsUI = {
    ...
    render(){
      ...
      UI.els.proposalsTbody.innerHTML = rows.length ? rows.map(rec => `
        <tr>
          ...
          <td><span class="badge lcProposalBadge lcProposalBadge--step${Number(rec.currentStep||1) >= 7 ? '-done' : Number(rec.currentStep||1) >= 5 ? '-mid' : '-open'}">שלב ${escapeHtml(String(rec.currentStep || 1))} מתוך 8</span></td>
    ...
    deleteById(id){
      ...
      DeleteProposalModal.open(rec.fullName || "הלקוח", async () => {
        ...
        const r = await Storage.deleteRows(SUPABASE_TABLES.proposals, [id]);
        if(!r?.ok){
          State.data.proposals = prevProposals;
          State.data.meta.updatedAt = nowISO();
          UI.renderSyncStatus("שגיאה במחיקת ההצעה", "err", null, r.error);
          return;
        }
        try { Storage.saveBackup(State.data); } catch(_e) {}
  ...
  const DashboardUI = {
    ...
    getCurrentAgentRecord(){
      const currentName = this.getCurrentAgentName();
      const agents = Array.isArray(State.data?.agents) ? State.data.agents : [];
      return agents.find((a) => safeTrim(a?.name) === currentName || safeTrim(a?.username) === currentName) || null;
    },

    isCustomerOwnedByCurrentAgent(rec){
      const currentName = this.getCurrentAgentName();
      if(!currentName) return false;
      const agentName = safeTrim(rec?.agentName) || safeTrim(rec?.createdBy) || safeTrim(rec?.payload?.createdBy) || '';
      if(agentName && agentName === currentName) return true;
      const currentIdentity = currentAgentIdentity();
      const candidateKeys = [
        safeTrim(rec?.agentId),
        safeTrim(rec?.createdByKey),
        safeTrim(rec?.payload?.createdByKey),
        safeTrim(rec?.payload?.agentKey),
        safeTrim(rec?.payload?.agentId)
      ].filter(Boolean).map((v) => String(v).toLowerCase());
      return !!(currentIdentity?.key && candidateKeys.includes(String(currentIdentity.key).toLowerCase()));
    },
    ...
    buildDailySeries(customersMonth){
      ...
      customersMonth.forEach((rec) => {
        ...
        entry.premium += policies.reduce((sum, p) => sum + (Number(p?.premiumAfterDiscountValue) || 0), 0);
        entry.clients += 1;
      });
      return daily.map((item) => ({ ...item, premium: Math.round(item.premium * 100) / 100 }));
    },

    buildRecentActivity(customersMonth, proposalsMonth){
      const items = [];
      customersMonth.forEach((rec) => {
        const policies = CustomersUI.collectPolicies(rec).filter((p) => String(p?.origin || '') === 'new');
        items.push({
          type: 'customer',
          stamp: this.resolveCustomerMonthStamp(rec),
          title: safeTrim(rec?.fullName) || 'לקוח חדש',
          subtitle: `נפתח תיק לקוח · ${safeTrim(rec?.status) || 'חדש'}`,
          amount: policies.reduce((sum, p) => sum + (Number(p?.premiumAfterDiscountValue) || 0), 0),
          status: safeTrim(rec?.status) || 'חדש'
        });
      });
      proposalsMonth.forEach((rec) => {
        items.push({
          type: 'proposal',
          stamp: this.resolveProposalMonthStamp(rec),
          title: safeTrim(rec?.fullName) || 'הצעה חדשה',
          subtitle: `הצעה בשלב ${Number(rec?.currentStep || 1) || 1}`,
          amount: 0,
          status: safeTrim(rec?.status) || 'פתוחה'
        });
      });
      return items
        .filter((item) => item.stamp)
        .sort((a, b) => Date.parse(b.stamp || '') - Date.parse(a.stamp || ''))
        .slice(0, 5);
    },
    ...
    buildMetrics(){
      ...
      const currentRange = this.getMonthRange();
      ...
      const accumulate = (records) => {
        ...
        records.forEach((rec) => {
          ...
          const newPolicies = policies.filter((p) => String(p?.origin || '') === 'new');
          const existingPolicies = policies.filter((p) => String(p?.origin || '') === 'existing');
          out.soldPolicies += newPolicies.length;
          out.grossPremium += newPolicies.reduce((sum, p) => sum + CustomersUI.asMoneyNumber(p?.premiumValue), 0);
          out.netPremium += newPolicies.reduce((sum, p) => sum + (Number(p?.premiumAfterDiscountValue) || 0), 0);
          existingPolicies.forEach((p) => {
            const status = this.normalizeExistingStatus(p?.details?.['סטטוס'] || p?.status || p?.existingStatus || '');
            if(status === 'agent_appoint'){
              out.agentAppointments += 1;
              out.agentAppointmentPremium += CustomersUI.asMoneyNumber(p?.premiumValue);
            }
          });
        });
        out.grossPremium = Math.round(out.grossPremium * 100) / 100;
        out.netPremium = Math.round(out.netPremium * 100) / 100;
        out.agentAppointmentPremium = Math.round(out.agentAppointmentPremium * 100) / 100;
        return out;
      };

      const currentAgg = accumulate(customersMonth);
      const prevAgg = accumulate(customersPrev);
      const currentAgent = this.getCurrentAgentRecord();
      const targetValue = getAgentMonthlyTarget(currentAgent?.id, currentAgent);
      ...
      return {
        ...
        customersMonth,
        proposalsMonth,
        dailySeries: this.buildDailySeries(customersMonth),
        recentActivity: this.buildRecentActivity(customersMonth, proposalsMonth),
        ...
        topDayValue: Math.max(0, ...this.buildDailySeries(customersMonth).map((item) => item.premium || 0))
      };
    },
      ...
      this.els.root.innerHTML = `
        <section class="bankDash bankDash--empty">
          <div class="bankDash__topStats bankDash__topStats--empty">
            <span>עודכן לאחרונה כעת</span>
            <span>0% עמידה ביעד</span>
            <span>0 לקוחות חדשים</span>
            <span>0 פוליסות חדשות</span>
            <span>₪0 ממוצע ללקוח</span>
          </div>
    ...
    render(){
      ...
      const hasData = metrics.submittedProposals || metrics.netPremium || metrics.newClients || metrics.soldPolicies || metrics.agentAppointmentPremium || metrics.agentAppointments;
      if(!hasData){
        this.renderEmpty();
        return;
      }
      const topBarItems = [
        `עודכן לאחרונה ${new Date(metrics.updatedAt).toLocaleTimeString('he-IL', { hour:'2-digit', minute:'2-digit' })}`,
        `${this.formatPct(metrics.targetPct)} עמידה ביעד`,
        `${metrics.newClients} לקוחות חדשים`,
        `${metrics.soldPolicies} פוליסות חדשות`,
        `${this.formatMoney(metrics.avgPremium)} ממוצע ללקוח`
      ];
      const cards = [
        {
          en: 'Monthly Sales',
        ...
        },
        {
          en: 'Net Premium',
          he: 'פרמיה לפני הנחה',
          value: this.formatMoney(metrics.grossPremium),
          delta: metrics.netDelta,
          icon: premiumCustomerIcon('building')
        },
        {
          en: 'Agent Appointment Premium',
      ...
      const maxBar = Math.max(1, ...metrics.dailySeries.map((item) => item.premium || 0));
      this.els.root.innerHTML = `
        <section class="bankDash bankDash--cleanTop">
          <div class="bankDash__topStats">
            ${topBarItems.map((item) => `<span>${escapeHtml(item)}</span>`).join('')}
          </div>
          ...
          <div class="bankDash__kpis">
            ${cards.map((card) => {
              ...
              return `
                <article class="bankKpi card bankKpi--compact">
                  ...
                  <div class="bankKpi__delta ${deltaClass}">${deltaPrefix} ${escapeHtml(this.formatPct(Math.abs(Number(card.delta) || 0)))} מול חודש קודם</div>
            ...
            <div class="bankGoal__shell">
              ...
              <div class="bankGoal__details">
                <div class="bankGoal__topStats">
                  ...
                  <div class="bankGoal__topStat">
                    <span>יעד ביניים:</span>
                    <strong>${escapeHtml(this.formatMoney(metrics.targetSoFar || 0))}</strong>
                    <small>${Number(metrics.netDelta) >= 0 ? `כפחות ${escapeHtml(this.formatPct(Math.abs(Number(metrics.netDelta) || 0)))} שר` : `פחות ${escapeHtml(this.formatPct(Math.abs(Number(metrics.netDelta) || 0)))} שר`}</small>
                  </div>
                  <div class="bankGoal__topStat">
                    <span>יעד חודשי:</span>
          ...
          <div class="bankDash__body">
            <article class="bankChart card">
              ...
              <div class="bankChart__plot">
                ${metrics.dailySeries.map((item) => {
                  const height = Math.max(8, Math.round(((item.premium || 0) / maxBar) * 100));
                  return `<div class="bankChart__barWrap"><div class="bankChart__bar" style="height:${height}%"></div><span>${item.day}</span></div>`;
                }).join('')}
              </div>
            </article>

            <article class="bankSide card">
              <div class="bankSide__title">הפעילות האחרונה</div>
              <div class="bankSide__list">
                ${metrics.recentActivity.map((item) => `
                  <div class="bankSide__item">
                    <div>
                      <div class="bankSide__itemTitle">${escapeHtml(item.title)}</div>
                      <div class="bankSide__itemSub">${escapeHtml(item.subtitle)}</div>
                    </div>
                    <div class="bankSide__itemMeta">
                      <strong>${item.amount > 0 ? escapeHtml(this.formatMoney(item.amount)) : escapeHtml(item.status)}</strong>
                      <span>${escapeHtml(new Date(item.stamp).toLocaleTimeString('he-IL', { hour:'2-digit', minute:'2-digit' }))}</span>
                    </div>
                  </div>`).join('')}
              </div>
            </article>
          ...
          <article class="bankTable card">
            <div class="bankTable__title">סיכום פעילות אחרונה</div>
            <div class="tableWrap">
              <table class="table">
                <thead>
                  <tr>
                ...
                <tbody>
                  ${metrics.recentActivity.map((item) => `
                    <tr>
                      <td>${item.type === 'customer' ? 'לקוח' : 'הצעה'}</td>
                      <td>${escapeHtml(item.title)}</td>
                      <td>${escapeHtml(item.subtitle)}</td>
                      <td>${item.amount > 0 ? escapeHtml(this.formatMoney(item.amount)) : '—'}</td>
                      <td>${escapeHtml(new Date(item.stamp).toLocaleTimeString('he-IL', { hour:'2-digit', minute:'2-digit' }))}</td>
                    </tr>`).join('')}
                </tbody>
              </table>
            </div>
    ...
    healthCoversByCompany: {
      ...
      "כלל": [
        ...
        },
        {
          ...
          radio: true,
          radioKey: "healthCoversConsultGroup",
          items: [
            { k:"ייעוץ רפואי מהיר" },
            { k:"אבחון רפואי מהיר" },
...
init(){
      ...
      on(this.els.btnPreviewOpsReport, "click", () => {
        const sched = this.getMirrorSchedule();
        if(!safeTrim(sched?.date) || !safeTrim(sched?.time)) return;
    ...
    getCurrentSteps(){
      return this.isElementaryFlow()
        ? [{ id:1, title:"פרטי מבוטח" }, { id:2, title:"ייבוא מהר הביטוח" }]
    ...
    handleHealthDelegatedClick(ev){
      ...
      if(target.hasAttribute('data-hans')){
        ...
        if(ans === 'no') {
          ...
          const hasAnyFilledField = !!Object.values(prev.fields || {}).some(v => safeTrim(v));
          this.setHealthResponse(qKey, insId, { answer:'yes', saved: hasDetailFields ? false : true, editing: hasDetailFields ? true : false, fields: prev.fields || {} });
          if(!hasDetailFields) this.setHint('התשובה נשמרה — אין צורך בפירוט נוסף לשאלה הזו');
          this.renderHealthStepUi({ focusDetail: hasDetailFields, qKey, insId, restoreValues: hasAnyFilledField });
          return;
        }
      }
      ...
      if(target.hasAttribute('data-hedit')){
        ...
        this.renderHealthStepUi({ focusDetail:true, qKey, insId, restoreValues:true });
        return;
      }
    ...
    renderCoversDrawer(pol, opts={}){
        ...
        companyGroups.forEach(group => {
          ...
          if(group.radio){
            ...
            group.items.forEach(item => {
              ...
              const hasAmount = !!item.amountField;
              const amountVal = escapeHtml(String(amountData[item.amountField] || ""));
              html += `
                ...
                ${hasAmount ? `<div class="lcCoverAmountRow${checked ? '' : ' lcCoverAmountRow--hidden'}" data-cover-amount-row="${escapeHtml(item.amountField)}">
                  <label class="lcCoverAmountRow__label">${escapeHtml(item.amountLabel || "סכום פיצוי")}</label>
                  <div class="lcCoverAmountRow__wrap">
                    <span class="lcCoverAmountRow__sym">₪</span>
                    <input type="text" inputmode="numeric" class="lcInput lcCoverAmountRow__input" data-cover-amount-field="${escapeHtml(item.amountField)}" value="${amountVal}" placeholder="לדוגמה: 200,000" />
                  </div>
                </div>` : ""}`;
            });
          }
      ...
      this.els.coversList.querySelectorAll('input[type="checkbox"]').forEach(cb => {
        ...
        cb.addEventListener('change', () => {
          ...
          // מצא את שורת הסכום הבאה
          const amountRow = label?.nextElementSibling;
          if(amountRow && amountRow.hasAttribute('data-cover-amount-row')){
            amountRow.classList.toggle('lcCoverAmountRow--hidden', !cb.checked);
            if(cb.checked) amountRow.querySelector('input')?.focus();
          }
        });
      });
    ...
    saveCoversDrawer(){
      try{
        ...
        this.els.coversList?.querySelectorAll?.('[data-cover-amount-field]')?.forEach?.(inp => {
          ...
          if(field && val) amounts[field] = val;
        });
        ...
        if(ctx.kind === "newDraft"){
          ...
          if(this.policyDraft){
            ...
            if(Object.keys(amounts).length) this.policyDraft.healthCoversAmounts = amounts;
          }
          this.closeCoversDrawer();
          this.render();
        ...
        pol.covers = filtered;
        if(Object.keys(amounts).length) pol.healthCoversAmounts = amounts;
    ...
    nextStep(){
      ...
      if(this.isElementaryFlow()){
        ...
        if(this.step >= this.getCurrentSteps().length){
          this.setHint("השלבים הבאים של אלמנטרי יפותחו בשלב הבא.");
          return;
        }
        this.step += 1;
        this.setHint("");
      ...
      if(this.step === 4){
        ...
        $$('[data-na-reason]', this.els.body || document).forEach((el) => {
          const pid = safeTrim(el.getAttribute('data-na-reason'));
          const card = el.closest('[data-na-insid]');
          if(!pid || !card) return;
          const insId = safeTrim(card.getAttribute('data-na-insid'));
          const ins = self.insureds.find(x => x.id === insId) || self.insureds[0];
          if(!ins) return;
          if(!ins.data.cancellations) ins.data.cancellations = {};
          if(!ins.data.cancellations[pid]) ins.data.cancellations[pid] = {};
          ins.data.cancellations[pid].needsAnalysisReason = safeTrim(el.value);
        });
      }
    ...
    renderSteps(){
      ...
      if(this.isElementaryFlow() && !this.elementaryProduct){
        this.els.steps.innerHTML = `<div class="lcStep is-active"><span class="lcStep__num">•</span><span>בחירת תחום אלמנטרי</span></div>`;
    ...
    renderFooter(){
      if(this.isElementaryFlow()){
        if(this.els.btnPrev) this.els.btnPrev.disabled = (this.step <= 0);
        if(this.els.btnSaveDraft) this.els.btnSaveDraft.style.display = "none";
        if(this.els.btnNext){
          this.els.btnNext.disabled = !this.elementaryProduct || this.step >= this.getCurrentSteps().length;
          this.els.btnNext.textContent = this.step <= 0 ? "בחירת מסלול" : (this.step === 1 ? "לשלב הבא" : "המשך יפותח בהמשך");
        }
      ...
      if(this.step === 1){
        ...
        if(this.els.btnNext){
          ...
          this.els.btnNext.textContent = isLast ? "לשלב הבא" : "לשאלה הבאה";
          const isComplete = this.isStep1QuestionComplete(ins, questions[idx]);
          this.els.btnNext.classList.toggle('lcBtnNext--ready', !!isComplete);
        }
        return;
      }
      ...
      if(this.els.btnNext) this.els.btnNext.textContent = (this.step >= this.getCurrentSteps().length) ? "סיום הקמת לקוח" : "הבא";
    },
    ...
    renderBody(){
      ...
      if(this.isElementaryFlow()){
        ...
        else body = this.renderElementaryStep2(ins);
      ...
      const hideStepHeader = (this.step === 1 || this.step === 5 || this.step === 8);
      ...
      else if(this.step === 4) this.bindStep4NeedsAnalysis();
      ...
      $$("[data-bind]", this.els.body).forEach(el => {
        ...
        const setVal = (doRender=false) => {
          ...
          if(path === "clinic"){
            ...
            else if(!this.shabanMap[ins.data.clinic]?.includes(ins.data.shaban)) ins.data.shaban = "";
...
// open health covers drawer (Health only)
    ...
    renderElementaryCategory(){
      ...
      const makeCard = (key, title, options = {}) => {
        ...
        return `<button type="button" class="${cls}" data-elementary-product="${escapeHtml(key)}" ${disabled ? 'disabled' : ''}>
          ${isVehicle ? `<span class="lcElementaryCard__vehicleMedia" aria-hidden="true"><img class="lcElementaryCard__vehicleImg" src="./mercedes-progress-car.png" alt="" loading="lazy" decoding="async" /></span>` : ''}
    ...
    renderElementaryProgress(){
      const steps = [
        { id:1, title:'פרטי מבוטח' },
        { id:2, title:'הפקת פוליסות' }
      ];
      const currentStep = Math.max(1, Math.min(2, Number(this.step) || 1));
        ...
        <div class="lcElementaryRoad">
          ...
          <div class="lcElementaryRoad__car" aria-hidden="true">
            <div class="lcElementaryRoad__carMotion">
              ...
              <img class="lcElementaryRoad__carImg" src="./mercedes-progress-car.png" alt="" loading="lazy" decoding="async" />
            </div>
          </div>
          <div class="lcElementaryRoad__stops">
            ${steps.map((step) => {
              const cls = ['lcElementaryRoad__stop', step.id === currentStep ? 'is-active' : '', step.id < currentStep ? 'is-done' : ''].join(' ').trim();
              return `<div class="${cls}"><span class="lcElementaryRoad__dot">${step.id < currentStep ? '✓' : step.id}</span><span class="lcElementaryRoad__label">${escapeHtml(step.title)}</span></div>`;
            }).join('')}
          </div>
        </div>
      </div>`;
    ...
    renderElementaryStep1(ins){
      ...
      return `<div class="lcElementaryWrap">
        ...
        <div class="formGrid lcElementaryFormGrid lcElementaryFormGrid--clean">
          ...
          <div class="field lcElementaryDateField">
            ...
            <input class="input lcElementaryDateInput" type="text" dir="ltr" inputmode="numeric" autocomplete="off" placeholder="DD/MM/YYYY" maxlength="10" data-datefmt="dmy" data-bind="birthDate" value="${escapeHtml(d.birthDate || '')}" />
    ...
    bindElementaryInputs(ins){
      ...
      $$('[data-elementary-product]', this.els.body).forEach((btn) => {
        on(btn, 'click', () => {
          const product = safeTrim(btn.getAttribute('data-elementary-product'));
          if(!product) return;
          this.elementaryProduct = product;
          this.step = product === 'vehicle' ? 1 : 0;
          this.setHint(product === 'vehicle' ? '' : 'תחום זה עדיין בפיתוח');
          this.render();
        });
      });
      $$('[data-bind]', this.els.body).forEach((el) => {
        ...
        const apply = (opts = {}) => {
          let v = (el.type === 'checkbox') ? !!el.checked : safeTrim(el.value);
          if(path === 'birthDate' && el.getAttribute('data-datefmt') === 'dmy'){
        ...
        const isElementaryBirthDate = this.isElementaryFlow() && path === 'birthDate' && el.type === 'date';
        if(isElementaryBirthDate){
          on(el, 'input', () => apply({ render:false }));
          on(el, 'change', () => apply({ render:false }));
          on(el, 'blur', () => apply({ render:false }));
        } else {
          on(el, 'input', () => apply({ render:false }));
          on(el, 'change', () => apply({ render:true }));
        }
      });
    ...
    updateElementaryAgeUi(age){
      if(!this.els.body) return;
      const chip = this.els.body.querySelector('[data-elementary-age-inline]');
      const valueEl = this.els.body.querySelector('[data-elementary-age-value]');
    ...
    getStep1Questions(ins){
      ...
      return [
        ...
        },
        {
          ...
          fields: isChild ? ['birthDate','gender'] : ['birthDate','gender','maritalStatus','occupation'],
          requiredMsg: isChild ? 'נא להשלים תאריך לידה ומין לפני שממשיכים' : 'נא להשלים תאריך לידה, מין, מצב משפחתי ועיסוק לפני שממשיכים',
          render:() => `
            <div class="lcStep1Group">
              <div class="lcStep1Group__grid lcStep1Group__grid--personal">
                ...
                ${this.fieldSelect('קופת חולים','clinic', d.clinic, ['', ...this.clinics])}
                ${this.fieldSelect('שב״ן','shaban', d.shaban, ['', ...((this.shabanMap[d.clinic] || []))])}
          ...
          <div class="lcStep1Premium__main">
            <section class="lcStep1QuestionCard lcStep1QuestionCard--compact">
              <div class="lcStep1QuestionCard__top lcStep1QuestionCard__top--single">
                <div class="lcStep1QuestionCard__tag" data-insured-live-label="1">${escapeHtml(liveLabel)}</div>
              </div>
              <h3 class="lcStep1QuestionCard__title">${escapeHtml(q?.title || '')}</h3>
              <div class="lcStep1QuestionCard__sub">${escapeHtml(q?.sub || '')}</div>
              <div class="lcStep1QuestionCard__body lcStep1QuestionCard__body--grouped" data-step1-body="1">${q?.render ? q.render() : ''}</div>
            </section>
          </div>
    ...
    getHarImportState(ins){
      ...
      return this._harImportState[key];
    },

    setHarImportState(ins, patch={}){
      const next = Object.assign(this.getHarImportState(ins), patch || {});
      next.status = safeTrim(next.status) || "idle";
      return next;
    },
    ...
    normalizeHarPolicyType(row){
      ...
      if(/אבדן כושר|אובדן כושר/i.test(source)) return "אובדן כושר עבודה";
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      sheetNames.forEach((sheetName) => {
        ...
        if(headerIdx === -1 || headerMeta.requiredHits < 3 || headerScore < 4){
          debug.sheets.push({ sheetName, headerIdx, headerScore, requiredHits: headerMeta.requiredHits || 0, skipped: true, reason: 'header_not_found' });
          return;
        }
        ...
        debug.sheets.push({ sheetName, headerIdx, headerScore, skipped: false, rowsAccepted, idx });
      });
      try { console.log('HAR BITUACH DEBUG', debug); } catch(_e) {}

      if(!extracted.length) return { policies: [], fileName: "" };
      ...
      try {
        ...
        if(!policies.length){
          this.setHarImportState(ins, {
            ...
            count: 0,
            fileName: file.name || "",
            message: "לא נמצאו פוליסות מתאימות לייבוא מתוך תחומי בריאות / חיים."
          });
          this.render();
          return;
        }
    ...
    renderStep3(ins){
      ...
      const rows = (d.existingPolicies || []).map(p => {
        ...
        const importedInfoBadges = [classificationBadge, collectiveBadge, importedBadge].filter(Boolean).join('');
        ...
        if(p.importedFromHarBituach){
          ...
          const logoInitials = (p.company || '?').replace(/[^א-תa-zA-Z0-9]/g, '').slice(0, 2).toUpperCase();
          const logoHtml = logoSrc
            ? `<img class="lcPolCardLogo__img" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(p.company || '')}" />`
            : `<span class="lcPolCardLogo__initials">${escapeHtml(logoInitials)}</span>`;
          const insuredDots = [
      ...
      return `
        <div class="lcWSection">
          <div class="lcPolImportBar">
            ...
            <div class="lcPolImportBar__text">
    ...
    renderStep4NeedsAnalysis(){
      // Collect all policies marked for full or partial cancellation across all insureds
      ...
      this.insureds.forEach((ins) => {
        ...
        policies.forEach((policy) => {
          ...
          const status = safeTrim(c.status);
          if(status === 'full' || status === 'partial_health'){
            cancelledPolicies.push({ policy, cancellation: c, ins });
          }
        });
      });
      ...
      const rows = cancelledPolicies.map((item, idx) => {
        const { policy, cancellation, ins } = item;
        const pid = safeTrim(policy.id);
        const logoSrc = this.getCompanyLogoSrc(policy.company);
        const logo = logoSrc
          ? `<img class="lcOpPolicyRow__logoImg" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(policy.company)}" style="width:32px;height:32px;object-fit:contain;" />`
          : `<div class="lcOpPolicyRow__logoFallback">${escapeHtml((policy.company || '•').slice(0,1))}</div>`;
        const existingReason = safeTrim(cancellation.needsAnalysisReason || '');
        return `
          <div class="lcNaCard" data-na-insid="${escapeHtml(ins.id)}" data-na-pid="${escapeHtml(pid)}" style="animation-delay:${idx * 60}ms">
            <div class="lcNaCard__head">
              <div class="lcNaCard__brand">
                <div class="lcNaCard__logo">${logo}</div>
                <div class="lcNaCard__titles">
                  <div class="lcNaCard__company">${escapeHtml(policy.company || '—')}</div>
                  <div class="lcNaCard__meta">${escapeHtml(policy.type || '—')} · ${escapeHtml(ins.label || 'מבוטח')}</div>
                </div>
              </div>
              <span class="lcOpPolicyBadge ${statusClass(cancellation.status)}">${escapeHtml(statusLabel(cancellation.status))}</span>
            </div>
            <div class="lcNaCard__body">
              <label class="label" for="lcNaReason_${escapeHtml(pid)}">שיקולים ונימוקים להמלצת הביטול</label>
              <textarea
                class="input lcNaCard__textarea"
                id="lcNaReason_${escapeHtml(pid)}"
                data-na-reason="${escapeHtml(pid)}"
                placeholder="פרט את שיקולי ההמלצה לביטול פוליסה זו... (למשל: כפל כיסויים, עלות גבוהה יחסית לכיסוי, אי התאמה לצרכי המבוטח)"
                rows="3"
              >${escapeHtml(existingReason)}</textarea>
            </div>
          </div>`;
      }).join('');

      const emptyState = `
        <div class="emptyState" style="padding:48px 0;">
          <div class="emptyState__icon">${premiumCustomerIcon('document')}</div>
          <div class="emptyState__title">לא סומנו פוליסות לביטול</div>
          <div class="emptyState__text">לא נבחרו פוליסות לביטול מלא או ביטול חלקי בשלב הפוליסות הקיימות.</div>
        </div>`;

      return `
        <div class="lcWSection">
          <div class="lcNaHeader">
            <div class="lcNaHeader__title">התאמת צרכים</div>
            <div class="lcNaHeader__sub">פרט את שיקולי ההמלצה לביטול עבור כל פוליסה שסומנה לביטול מלא או חלקי. ההסברים יישמרו בהצעה וישתקפו בדוח התפעולי.</div>
          </div>
          <div class="lcNaList" id="lcNaList">
            ${cancelledPolicies.length ? rows : emptyState}
          </div>
        </div>`;
    },
    ...
    bindStep4NeedsAnalysis(){
      $$('[data-na-reason]', this.els.body).forEach((el) => {
        ...
        if(!pid || !card) return;
        const insId = safeTrim(card.getAttribute('data-na-insid'));
        const ins = this.insureds.find(x => x.id === insId) || this.insureds[0];
        if(!ins) return;
        const save = () => {
          if(!ins.data.cancellations) ins.data.cancellations = {};
          if(!ins.data.cancellations[pid]) ins.data.cancellations[pid] = {};
    ...
    getPolicyDiscountDisplayText(policy, options = {}){
      ...
      if(benefitText){
        return baseText === 'ללא הנחה' ? benefitText : `${baseText} · ${benefitText}`;
      }
      return baseText;
    },
    ...
    ensurePolicyDraft(){
      ...
      this.policyDraft = {
        ...
        premiumPerInsured: {}, // insuredId -> premiumMonthly
        umbrellaInsurance: false,
        umbrellaDisabilityAmount: "",
        umbrellaDeathAmount: "",
        premiumMonthly: "",
    ...
    openPolicyAddedModal(pid){
      ...
      // store insuredIds for sequential discount flow
      const policy = (this.newPolicies || []).find(p => p.id === pid);
      this._lastAddedInsuredIds = Array.isArray(policy?.insuredIds) && policy.insuredIds.length > 1
        ? policy.insuredIds.slice() : null;
    ...
    goToLastAddedPolicyDiscount(){
      ...
      this.closePolicyAddedModal();
      if(!pid) return;
      // if multi-insured, open discount sequentially per insured
      if(this._lastAddedInsuredIds && this._lastAddedInsuredIds.length > 1){
        this._multiDiscountPolicyId = pid;
        this._multiDiscountInsuredIds = this._lastAddedInsuredIds.slice();
    ...
    _openNextMultiDiscount(){
      ...
      const ids = this._multiDiscountInsuredIds || [];
      const idx = this._multiDiscountIdx || 0;
      if(!pid || idx >= ids.length) return;
      const iid = ids[idx];
      const ins = this.insureds.find(x => x.id === iid);
      const insLabel = ins ? ins.label : `מבוטח ${idx+1}`;
      this._multiDiscountCurrentInsuredId = iid;
      this._multiDiscountCurrentInsuredLabel = insLabel;
      this._multiDiscountTotal = ids.length;
      this._multiDiscountIdx = idx;
      this.openPolicyDiscountModal(pid);
    },

    _advanceMultiDiscount(){
      if(!this._multiDiscountInsuredIds) return false;
      this._multiDiscountIdx = (this._multiDiscountIdx || 0) + 1;
      if(this._multiDiscountIdx < this._multiDiscountInsuredIds.length){
        this._openNextMultiDiscount();
        return true;
      }
    ...
    openPolicyDiscountModal(pid){
      ...
      const discountMode = this.getPolicyDiscountMode(policy);
      if(this.els.policyDiscountModeInputs){
        this.els.policyDiscountModeInputs.forEach((el) => {
          el.checked = safeTrim(el.value) === discountMode;
        });
      }
      const multiLabel = this._multiDiscountCurrentInsuredLabel
        ? ` · ${this._multiDiscountCurrentInsuredLabel} (${(this._multiDiscountIdx||0)+1}/${this._multiDiscountTotal||1})`
        : "";
      if(this.els.policyDiscountName) this.els.policyDiscountName.textContent = `${policy.company || "חברה"} · ${policy.type || "פוליסה"}${multiLabel}`;
      // use per-insured premium if available
      const discountPremiumBase = (() => {
        const iid = this._multiDiscountCurrentInsuredId;
        if(iid && policy.premiumPerInsured && policy.premiumPerInsured[iid]){
          return this.asMoneyNumber(policy.premiumPerInsured[iid]);
        }
      ...
      const introBenefitKey = this.getPolicyIntroBenefitKey(policy);
      ...
      this.renderPolicyCoverDiscounts(policy);
      this.renderPolicyDiscountScheduleSummary();
      this.updatePolicyDiscountPreview();
    ...
    renderPolicyDiscountScheduleSummary(){
      ...
      if(titleEl) titleEl.textContent = isTenYears ? 'הנחה אוטומטית ל־10 שנים' : (count ? 'דירוג הנחה שנשמר' : 'דירוג ההנחה בשנים');
      if(subEl) subEl.textContent = isTenYears
        ? 'האחוז שנבחר יוחל אוטומטית על כל 10 השנים ויוצג בשורת הפוליסה.'
        : (count ? 'הדירוג יוצג בשורת הפוליסה ובסיכומי התהליך בצורה מסודרת' : 'טרם הוגדר דירוג הנחה לפי שנים');
      if(badgeEl) badgeEl.textContent = `${count}/10`;
      if(!count){
        this.els.policyDiscountScheduleList.innerHTML = `<div class="lcPolicyDiscountModal__scheduleEmpty">${isTenYears ? 'בחר אחוז הנחה כדי להחיל אותו על כל 10 השנים.' : 'לא הוזנו עדיין שנים לדירוג הנחה.'}</div>`;
    ...
    calcHealthCoverAvgDiscount(coverDiscounts, fallbackPct){
      // כיסוי עם הנחה ספציפית → משתמש בה
      // כיסוי עם 0% → יורש את הנחה הכללית (fallbackPct)
      // ממוצע כל הכיסויים הוא ההנחה האפקטיבית
      if(!coverDiscounts || typeof coverDiscounts !== 'object') return fallbackPct;
      const vals = Object.values(coverDiscounts).map(v => {
        const n = Number(v);
        return Number.isFinite(n) ? (n > 0 ? n : fallbackPct) : fallbackPct;
      });
      if(!vals.length) return fallbackPct;
      const avg = vals.reduce((s, v) => s + v, 0) / vals.length;
      return Math.round(avg * 100) / 100;
    },

    renderPolicyCoverDiscounts(policy){
      const wrap = document.getElementById('lcPolicyCoverDiscounts');
      const table = document.getElementById('lcPolicyCoverDiscountsTable');
      if(!wrap || !table) return;
      const isHealth = safeTrim(policy?.type) === 'בריאות';
      wrap.hidden = !isHealth;
      if(!isHealth){ table.innerHTML = ''; return; }
      const covers = Array.isArray(policy.healthCovers) ? policy.healthCovers.filter(Boolean) : [];
      if(!covers.length){ wrap.hidden = true; return; }
      const saved = policy.healthCoverDiscounts || {};
      const pctOptions = [0,2,5,10,15,20,25,30,35,40,45,50,55,60,65,70];
      table.innerHTML = covers.map(cover => {
        const val = saved[cover] ?? '';
        const opts = pctOptions.map(p =>
          `<option value="${p}" ${String(val)===String(p)?'selected':''}>${p}%</option>`
        ).join('');
        return `<div class="lcCoverDiscountRow">
          <div class="lcCoverDiscountRow__label">${escapeHtml(cover)}</div>
          <select class="input lcCoverDiscountRow__select" data-cover-discount="${escapeHtml(cover)}">${opts}</select>
        </div>`;
      }).join('');
      // עדכון preview בזמן אמת כשמשנים הנחה לכיסוי
      table.querySelectorAll('[data-cover-discount]').forEach(sel => {
        sel.addEventListener('change', () => this.updatePolicyDiscountPreview());
      });
    },

    getHealthCoverDiscountDraft(){
      const table = document.getElementById('lcPolicyCoverDiscountsTable');
      if(!table) return {};
      const result = {};
      table.querySelectorAll('[data-cover-discount]').forEach(el => {
        const cover = el.getAttribute('data-cover-discount');
    ...
    updatePolicyDiscountPreview(){
      ...
      // for health policies: use average of per-cover discounts for live preview
      const isHealthPreview = safeTrim(policy?.type) === 'בריאות';
      const coverDiscountsPreview = isHealthPreview ? this.getHealthCoverDiscountDraft() : null;
      const effectivePctPreview = coverDiscountsPreview ? this.calcHealthCoverAvgDiscount(coverDiscountsPreview, pct) : pct;
      const after = Math.max(0, Math.round((base * (1 - effectivePctPreview / 100)) * 100) / 100);
      ...
      const scheduleText = scheduleSummary
        ? `<div class="lcPolicyDiscountModal__previewSub">${mode === 'tenYears' ? 'מסלול 10 שנים: ' : 'דירוג בשנים: '}${escapeHtml(scheduleSummary)}</div>`
        : `<div class="lcPolicyDiscountModal__previewSub">${mode === 'tenYears' ? 'בחר אחוז הנחה כדי להחיל אותו על כל 10 השנים' : 'טרם נשמר דירוג הנחה בשנים'}</div>`;
      ...
      const coverNote = (coverDiscountsPreview && effectivePctPreview !== pct) ? `<div class="lcPolicyDiscountModal__previewSub">הנחה ממוצעת מהכיסויים: <b>${effectivePctPreview}%</b></div>` : '';
      this.els.policyDiscountPreview.innerHTML = `פרמיה אחרי הנחה: <b>${escapeHtml(this.formatMoneyValue(after))}</b>${coverNote}${scheduleText}${benefitLine}`;
    },
    ...
    savePolicyDiscountModal(){
      ...
      const mode = this.getSelectedPolicyDiscountMode();
      if(mode === 'tenYears' && pct <= 0){
        if(this.els.policyDiscountError) this.els.policyDiscountError.textContent = 'בחר אחוז הנחה למסלול 10 שנים.';
        return;
      }
      const introBenefit = (this.els.policyDiscountBenefitInputs || []).find((el) => el.checked)?.value || 'none';
      const discountSchedule = mode === 'tenYears'
        ? this.buildUniformDiscountSchedule(10, pct)
        : (Array.isArray(this._discountScheduleDraft) ? JSON.parse(JSON.stringify(this._discountScheduleDraft)) : []);
      // save discount — per insured if multi
      const multiIid = this._multiDiscountCurrentInsuredId;
      if(multiIid && Array.isArray(policy.insuredIds) && policy.insuredIds.length > 1){
        if(!policy.discountPerInsured) policy.discountPerInsured = {};
        policy.discountPerInsured[multiIid] = {
          discountPct: String(pct),
          discountMode: mode,
          discountSchedule: discountSchedule,
          discountYears: discountSchedule.length ? String(discountSchedule.length) : "",
          introBenefit
        };
        // also set top-level from first insured
        const firstIid = policy.insuredIds[0];
        if(multiIid === firstIid || !policy.discountPct){
          const coverDiscountsM = safeTrim(policy.type) === 'בריאות' ? this.getHealthCoverDiscountDraft() : null;
          const effectivePctM = coverDiscountsM ? this.calcHealthCoverAvgDiscount(coverDiscountsM, pct) : pct;
          policy.discountPct = String(pct); // תמיד שומר את ההנחה הכללית שנבחרה
          policy.discountPctEffective = String(effectivePctM); // הממוצע האפקטיבי מהכיסויים
          policy.discountMode = mode;
          policy.discountSchedule = discountSchedule;
          policy.discountYears = discountSchedule.length ? String(discountSchedule.length) : "";
          policy.introBenefit = introBenefit;
          if(coverDiscountsM) policy.healthCoverDiscounts = coverDiscountsM;
        }
      } else {
        const coverDiscounts = safeTrim(policy.type) === 'בריאות' ? this.getHealthCoverDiscountDraft() : null;
        const effectivePct = coverDiscounts ? this.calcHealthCoverAvgDiscount(coverDiscounts, pct) : pct;
        policy.discountPct = String(pct); // תמיד שומר את ההנחה הכללית שנבחרה
        policy.discountPctEffective = String(effectivePct); // הממוצע האפקטיבי מהכיסויים
        policy.discountMode = mode;
        policy.discountSchedule = discountSchedule;
        policy.discountYears = discountSchedule.length ? String(discountSchedule.length) : "";
        policy.introBenefit = introBenefit;
        if(coverDiscounts) policy.healthCoverDiscounts = coverDiscounts;
      }

      this.closePolicyDiscountModal();

      // if multi-insured, advance to next insured discount
      if(this._multiDiscountInsuredIds && this._advanceMultiDiscount()){
        return; // next insured discount will open
      }

      this.render();
      this.setHint(mode === 'tenYears' ? "ההנחה ל־10 שנים והטבת ההצטרפות נשמרו בהצלחה בפוליסה." : "ההנחה, דירוג השנים והטבת ההצטרפות נשמרו בהצלחה בפוליסה.");
    },
    ...
    validateStep5(){
      ...
      const bad = list.filter(p => {
        ...
        if(!isMedicare && (p.type === "סרטן" || p.type === "מחלות קשות")){
          const pIds2 = Array.isArray(p.insuredIds) && p.insuredIds.length ? p.insuredIds : (p.insuredId ? [p.insuredId] : []);
          const allComp = pIds2.every(id => safeTrim((p.compensationPerInsured || {})[id]) || safeTrim(p.compensation));
          if(!allComp) return true;
        }
        if(!isMedicare && (p.type === "ריסק" || p.type === "ריסק משכנתא")){
          const pIds2 = Array.isArray(p.insuredIds) && p.insuredIds.length ? p.insuredIds : (p.insuredId ? [p.insuredId] : []);
          const allSum = pIds2.every(id => safeTrim((p.sumInsuredPerInsured || {})[id]) || safeTrim(p.sumInsured));
          if(!allSum) return true;
        }
        if(!isMedicare && p.type === "ריסק" && p.umbrellaInsurance){
          if(!safeTrim(p.umbrellaDisabilityAmount) || !safeTrim(p.umbrellaDeathAmount)) return true;
        }
        ...
        // verify all insuredIds actually exist
        const knownIds = new Set(this.insureds.map(x => x.id));
        if(!pIds.every(id => knownIds.has(id))) return true;
        return false;
      });
    ...
    renderStep5(){
      ...
      const getLogoSrc = (c) => {
        const logoMap = {
          "הפניקס":"afenix.png","הראל":"harel.png","כלל":"clal.png",
          "מגדל":"megdl.png","מנורה":"menora.png","איילון":"ayalon.png",
          "הכשרה":"achshara.png","AIG":"aig.png","ביטוח ישיר":"beytuyashir.png",
          "9 מיליון":"9milyon.png","מדיקר":"medicare.png"
        };
        return logoMap[c] || "";
      };
      const selectedCompany = safeTrim(d.company || "");
      const selectedLogoSrc = selectedCompany ? getLogoSrc(selectedCompany) : "";
      const triggerLogoHtml = selectedLogoSrc
        ? `<img class="lcNpCoDd__logo" src="${escapeHtml(selectedLogoSrc)}" alt="${escapeHtml(selectedCompany)}" />`
      ...
      const dropdownItems = [
        ...
        ...activeCompanyOptions.map(c => {
          const logo = getLogoSrc(c);
          const logoEl = logo
            ? `<img class="lcNpCoDd__itemLogo" src="${escapeHtml(logo)}" alt="${escapeHtml(c)}" />`
            : `<span class="lcNpCoDd__itemLogoPlaceholder"></span>`;
        ...
        companyGroups.forEach(group => {
          ...
          if(group.radio){
            ...
            group.items.forEach(item => {
              ...
              ${hasAmount ? `<div class="lcNpCoverAmountRow${checked ? '' : ' lcNpCoverAmountRow--hidden'}" data-np-cover-amount-row="${escapeHtml(item.amountField)}">
                <label class="lcNpCoverAmountRow__label">${escapeHtml(item.amountLabel || "סכום פיצוי")}</label>
                <div class="lcNpCoverAmountRow__wrap">
                  <span class="lcNpCoverAmountRow__sym">₪</span>
                  <input type="text" inputmode="numeric" class="lcInput lcNpCoverAmountRow__input" data-np-cover-amount="${escapeHtml(item.amountField)}" value="${amountVal}" placeholder="לדוגמה: 200,000" />
                </div>
              </div>` : ""}`;
            });
          }
      ...
      const body4 = `
        ...
        <div class="lcNpDetailsGrid">
          <div class="lcField lcNpDetailField lcNpDetailField--date">
            ...
            <div class="lcNpDetailField__hint">מועד תחילת הכיסוי בפוליסה החדשה</div>
          </div>
          ${insuredIds.map(iid => {
            const ins = this.insureds.find(x => x.id === iid);
            const label = ins ? escapeHtml(ins.label) : escapeHtml(iid);
            const val = escapeHtml((d.premiumPerInsured && d.premiumPerInsured[iid]) || "");
            return `<div class="lcField lcNpDetailField lcNpDetailField--money">
              <div class="lcNpDetailField__meta">פרמיה חודשית</div>
              <label class="lcLabel">פרמיה חודשית${insuredIds.length > 1 ? ` — ${label}` : ''} (חובה)</label>
              <div class="lcNpDetailField__shell">
                <span class="lcNpDetailField__icon" aria-hidden="true">₪</span>
                <span class="lcNpDetailField__currency">₪</span>
                <input class="lcInput lcPolMoneyWrap__input" type="text" inputmode="numeric" data-pdraft-per-insured-premium="${escapeHtml(iid)}" value="${val}" placeholder="250" />
              </div>
              <div class="lcNpDetailField__hint">הפרמיה החודשית שתיגבה עבור הפוליסה</div>
            </div>`;
          }).join("")}
        ...
        ${!isMedicare && d.type === "בריאות" ? `<div class="lcNpCoversSection">
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        const chips = [
          ...
          (() => { const ss = this.getPolicyDiscountScheduleSummary(p); return (discountPct > 0 || discountYears) ? `<span class="lcPolInfoChip lcPolInfoChip--discount lcPolInfoChip--discountWide"><span class="lcPolInfoChip__icon">🏷️</span><span class="lcPolInfoChip__text"><b>${escapeHtml(String(discountPct))}%</b><small>${ss ? escapeHtml(ss) : (discountYears ? `מדורג ל־${escapeHtml(discountYears)} שנים` : 'הנחה שנשמרה')}</small></span></span>` : ''; })(),
          introBenefitText ? `<span class="lcPolInfoChip lcPolInfoChip--success"><span class="lcPolInfoChip__icon">🎁</span><span class="lcPolInfoChip__text"><b>${escapeHtml(introBenefitMonths === 2 ? '2 חודשים' : 'חודש 1')}</b><small>${escapeHtml(introBenefitText)}</small></span></span>` : '',
          ...
          (!isMed && (p.type === "ריסק" || p.type === "ריסק משכנתא")) ? `<span class="lcPolInfoChip"><span class="lcPolInfoChip__icon">🏦</span><span class="lcPolInfoChip__text"><b>${escapeHtml(pledgeText)}</b><small>סטטוס שיעבוד</small></span></span>` : '',
          (!isMed && p.type === "ריסק" && p.umbrellaInsurance) ? `<span class="lcPolInfoChip lcPolInfoChip--umbrella"><span class="lcPolInfoChip__icon">☂️</span><span class="lcPolInfoChip__text"><b>הרחבות לפוליסה</b><small>${escapeHtml([safeTrim(p.umbrellaDisabilityAmount)?`נכות: ${safeTrim(p.umbrellaDisabilityAmount)}`:'', safeTrim(p.umbrellaDeathAmount)?`מוות: ${safeTrim(p.umbrellaDeathAmount)}`:''].filter(Boolean).join(' · ') || 'פעיל')}</small></span></span>` : ''
        ].filter(Boolean).join('');
        return `<div class="lcPolCard lcPolCard--premium" data-pol="${p.id}">
          <div class="lcPolCard__top">
            <div class="lcPolCard__brand">${logo}<div class="lcPolCard__brandText"><div class="lcPolTitle">${policyTitle} ${badge}</div><div class="lcPolSub">פוליסה חדשה${showCoupleBadge ? " · משויכת לשני מבוטחים" : ""}</div></div></div>
            <div class="lcPolSummaryTag">חדש</div>
          </div>
          <div class="lcPolInfoStrip">${chips}</div>
          ${p.type === "בריאות" ? `<div class="lcPolCoverCompact"><div class="lcPolCoverCompact__text"><span class="lcPolCoverCompact__count">${polCovers.length||0}</span><span>${escapeHtml(coverSummary)}</span></div><button type="button" class="lcPolCoverCompact__btn" data-editpol="${p.id}">ערוך כיסויים</button></div>` : ''}
      ...
      const groupsHtml = this.insureds.map(ins => {
        const items = (this.newPolicies || []).filter(p => {
      ...
      const res = `
        <div class="lcNpWrapper">
          ...
          ${step4Open ? `<div class="lcNpAddBar">
            ${this.editingPolicyId ? `<button type="button" class="lcBtn" data-cancel-editpol="1">ביטול עריכה</button>` : ''}
      ...
      setTimeout(() => {
        ...
        if(coDdTrigger && coDdMenu){
          ...
          document.addEventListener('click', function closeDdOutside(ev){
        ...
        $$("[data-pdraft-per-insured-premium]", this.els.body).forEach(el => {
          const handler = () => {
            this.ensurePolicyDraft();
            ...
            // also update premiumMonthly as sum of all
            const total = Object.values(this.policyDraft.premiumPerInsured).reduce((s, v) => {
              const n = Number(String(v||'').replace(/[^0-9.]/g,''));
              return s + (Number.isFinite(n) ? n : 0);
            }, 0);
            this.policyDraft.premiumMonthly = total > 0 ? String(total) : "";
          };
        ...
        if(addBtn){
          on(addBtn, "click", () => {
            ...
            if(!chk.ok){ this.setHint(chk.msg); return; }
            this.setHint("");
        ...
        $$('[data-discountpol]', this.els.body).forEach(btn => {
          on(btn, 'click', () => { const pid = btn.getAttribute('data-discountpol'); if(pid) this.openPolicyDiscountModal(pid); });
        });
    ...
    validateDraftPolicy(){
      ...
      const vInsuredIds = Array.isArray(d.insuredIds) && d.insuredIds.length ? d.insuredIds : (d.insuredId ? [d.insuredId] : []);
      if(!vInsuredIds.length) return { ok:false, msg:"בחר לפחות מבוטח אחד" };
      ...
      if(!isMedicare && (d.type === "סרטן" || d.type === "מחלות קשות")){
        const allComp = vInsuredIds.every(iid => safeTrim((d.compensationPerInsured || {})[iid]));
        if(!allComp) return { ok:false, msg:"במוצר זה חובה למלא סכום פיצוי לכל מבוטח" };
      }
      if(!isMedicare && (d.type === "ריסק" || d.type === "ריסק משכנתא")){
        const allSum = vInsuredIds.every(iid => safeTrim((d.sumInsuredPerInsured || {})[iid]));
        if(!allSum) return { ok:false, msg:"בריסק/ריסק משכנתא חובה למלא סכום ביטוח לכל מבוטח" };
      }
      if(!isMedicare && d.type === "ריסק" && d.umbrellaInsurance){
        if(!safeTrim(d.umbrellaDisabilityAmount) || !safeTrim(d.umbrellaDeathAmount)) return { ok:false, msg:"בהרחבות לפוליסה חובה למלא פיצוי לנכות מתאונה ופיצוי למוות מתאונה" };
      }
...
// ---------- Step 6 ----------
    ...
    renderStep9(){
      ...
      const renderPolicyRows = (list=[], kind='existing') => {
        ...
        return `<div class="lcOpPolicyGroups">` + groups.map((group, groupIdx) => {
          const rowsHtml = group.items.map((policy, idx) => {
            const logoSrc = this.getCompanyLogoSrc(policy.company);
            const logo = logoSrc
              ? `<img class="lcOpPolicyRow__logoImg" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(policy.company)}" />`
              : `<div class="lcOpPolicyRow__logoFallback">${escapeHtml((policy.company || '•').slice(0,1))}</div>`;
            ...
            if(kind === 'existing'){
              ...
              return `<article class="lcOpPolicyRow" style="animation-delay:${(groupIdx * 60) + (idx * 45)}ms">
                ...
                <div class="lcOpPolicyRow__side"><span class="lcOpPolicyBadge ${statusClass}">${escapeHtml(statusLabel)}</span></div>
      ...
      const normalizedNewPolicies = (this.newPolicies || []).map((policy, idx) => ({
        ...policy,
        ...
        <section class="lcOpSummary">
          <div class="lcOpHero">
            ...
            <div class="lcOpHero__stats">
              ...
              <div class="lcOpStat"><span>פוליסות חדשות</span><strong>${normalizedNewPolicies.length}</strong></div>
              <div class="lcOpStat"><span>סה"כ פרמיה אחרי הנחה</span><strong>${escapeHtml(this.formatMoneyValue(totalPremium))}</strong></div>
            </div>
          </div>

          <section class="lcOpSection lcOpSection--policies">
            <div class="lcOpSectionBanner">
              <div class="lcOpSectionBanner__icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="22" height="22"><path d="M3.75 8.75a2 2 0 0 1 2-2h4.15l1.5 1.7h6.85a2 2 0 0 1 2 2v6.8a2 2 0 0 1-2 2H5.75a2 2 0 0 1-2-2v-8.5Z"/><path d="M3.75 10.25h16.5"/></svg>
              </div>
              <div class="lcOpSectionBanner__text">
                <div class="lcOpSectionBanner__title">פוליסות קיימות</div>
                <div class="lcOpSectionBanner__sub">מוצגות הפוליסות הקיימות, מקובצות לפי מבוטח, עם סטטוס הטיפול שנבחר</div>
              </div>
            </div>
            ${renderPolicyRows(existingPolicies, 'existing')}
          </section>
          ...
          ${(() => {
            ...
            this.insureds.forEach((ins) => {
              ...
              policies.forEach((policy) => {
                ...
                if((status === 'full' || status === 'partial_health') && reason){
                  cancellationReasons.push({ policy, c, ins, reason, status });
                }
              });
            });
            if(!cancellationReasons.length) return '';
            const statusLabel = (s) => s === 'full' ? 'ביטול מלא' : 'ביטול חלקי';
            const statusCls = (s) => s === 'full' ? 'is-full' : 'is-partial';
            const items = cancellationReasons.map((item, idx) => {
              const logoSrc = this.getCompanyLogoSrc(item.policy.company);
              const logo = logoSrc
                ? `<img class="lcOpPolicyRow__logoImg" src="${escapeHtml(logoSrc)}" alt="${escapeHtml(item.policy.company)}" style="width:28px;height:28px;object-fit:contain;" />`
                : `<div class="lcOpPolicyRow__logoFallback">${escapeHtml((item.policy.company || '•').slice(0,1))}</div>`;
              return `<article class="lcNaOpItem" style="animation-delay:${idx * 50}ms">
                <div class="lcNaOpItem__head">
                  <div class="lcOpPolicyRow__brand">${logo}<div class="lcOpPolicyRow__titles"><div class="lcOpPolicyRow__company">${escapeHtml(item.policy.company || '—')}</div><div class="lcOpPolicyRow__meta">${escapeHtml(item.policy.type || '—')} · ${escapeHtml(item.ins.label || 'מבוטח')}</div></div></div>
                  <span class="lcOpPolicyBadge ${statusCls(item.status)}">${escapeHtml(statusLabel(item.status))}</span>
                </div>
                <div class="lcNaOpItem__reason">${escapeHtml(item.reason)}</div>
              </article>`;
            }).join('');
            return `<section class="lcOpSection lcOpSection--needsAnalysis">
              <div class="lcOpSectionBanner">
                <div class="lcOpSectionBanner__icon" aria-hidden="true">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="22" height="22"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg>
                </div>
                <div class="lcOpSectionBanner__text">
                  <div class="lcOpSectionBanner__title">שיקולי התאמת צרכים</div>
                  <div class="lcOpSectionBanner__sub">נימוקי ושיקולי ביטול הפוליסות כפי שנרשמו על ידי הנציג בשלב התאמת הצרכים</div>
                </div>
              </div>
              <div class="lcNaOpList">${items}</div>
            </section>`;
          })()}

          <section class="lcOpSection lcOpSection--policies lcOpSection--newPolicies">
            <div class="lcOpSectionBanner">
              <div class="lcOpSectionBanner__icon" aria-hidden="true">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" width="22" height="22"><path d="M8 4.75h6.5l4 4V18a2 2 0 0 1-2 2h-8a2 2 0 0 1-2-2V6.75a2 2 0 0 1 2-2Z"/><path d="M14.5 4.75v4h4"/><path d="M9 12h6"/><path d="M9 15.5h6"/></svg>
              </div>
              <div class="lcOpSectionBanner__text">
                <div class="lcOpSectionBanner__title">פוליסות חדשות שנרכשו דרך GEMEL INVEST</div>
                <div class="lcOpSectionBanner__sub">הפוליסות החדשות שנרכשו בתהליך ההקמה, מקובצות לפי מבוטח</div>
              </div>
            </div>
            ${renderPolicyRows(normalizedNewPolicies, 'new')}
          </section>
    ...
    getHealthCacheKey(){
      ...
      const policyKey = (this.newPolicies || []).map((policy) => [
        safeTrim(policy?.id),
      ...
      return {
        ...
        cancer_short_under_600k: {
          ...
          steps: [
            ...
            { key:'cancer_short_family', text:'האם למיטב ידיעתך התגלו בקרב קרוב משפחה מדרגה ראשונה בלבד (אב / אם / אח / אחות), עד גיל 60, אחת או יותר מהמחלות המפורטות בשאלון?', questionnaireNos:['22'], fields:[{ key:'familyRelative', label:'איזה קרוב משפחה', type:'text' },{ key:'familyDisease', label:'איזו מחלה', type:'text' },{ key:'familyAge', label:'גיל גילוי', type:'text' },{ key:'familyNotes', label:'פירוט נוסף', type:'textarea' }] },
            { key:'cancer_short_tests', text:'האם עברת או הומלץ לך לעבור בדיקות פולשניות ו/או בדיקות הדמיה ו/או בדיקות לגילוי מוקדם של סרטן בחמש השנים האחרונות?', questionnaireNos:['18'], fields:[{ key:'testType', label:'סוג הבדיקה', type:'text' },{ key:'testDate', label:'מתי', type:'text' },{ key:'testResult', label:'תוצאה / ממצא', type:'textarea' },{ key:'testFollowup', label:'המשך בירור / המלצה', type:'textarea' }] },
            { key:'cancer_short_conditions', text:'האם חלית ו/או אובחנת ו/או סבלת במהלך חייך באחת מהמחלות או מהמצבים הבאים: מחלה / גידול ממאיר, קרוהן / קוליטיס / מחלות כבד או דיכוי חיסוני / HIV / השתלת איברים?', questionnaireNos:['17','9','10'], fields:[{ key:'conditionGroup', label:'איזה תחום רלוונטי', type:'text' },{ key:'diagnosis', label:'אבחנה / מצב רפואי', type:'text' },{ key:'diagnosisDate', label:'מועד אבחון', type:'text' },{ key:'treatment', label:'טיפול / הליך רפואי', type:'textarea' },{ key:'currentStatus', label:'מצב כיום', type:'textarea' }] }
          ]
        }
      };
    },
    ...
    getMagdalHealthSchema(){
      ...
      if(!selection) return [];
      const detailField = [{ key:'details', label:'פירוט', type:'textarea' }];
      return [{
        key: 'master_magdal_health',
        title: 'הצהרת בריאות מלאה · מגדל',
        summary: 'הצהרת בריאות על פי תבנית מגדל לביטוח בריאות (טופס 170), עם שאלות כן/לא לכל מבוטח.',
        policyId: selection.sourcePolicies[0]?.id || '',
        questions: [
          // קטע א' — אורח חיים
          { key:'magdal__smoking', text:'א. האם הנך מעשן כיום? אם כן, המשאלה מתייחסת לכל מוצרי הסיגריות אלקטרוניות (מכשיר אידוי אלקטרוני), נרגילה. אם תשובתך היא כן, ציין מס׳ סיגריות / פעמים שמעשן ביום: ___', companies: selection.companies, fields: detailField },
          { key:'magdal__past_smoking', text:'ב. אם עניית כי אינך מעשן כיום, האם עישנת בשנתיים האחרונות? (לילדים החל מגיל 16)', companies: selection.companies, fields: detailField },
          { key:'magdal__alcohol', text:'האם הנך שותה באופן קבוע יותר מ-2 כוסות אלכוהול ליום? (לילדים החל מגיל 16) (22)', companies: selection.companies, fields: detailField },
          { key:'magdal__drugs', text:'האם הנך צורך או צרכת סמים כעת או בעבר? (לילדים החל מגיל 16)? (3)', companies: selection.companies, fields: detailField },
          // קטע ג' — רקע רפואי
          { key:'magdal__medications', text:'האם הנך נוטל תרופות באופן קבוע או הומלץ לך ליטול תרופות או הינך נמצא כעת במעקב רופא אחד מהשנה הרפואית? (21)', companies: selection.companies, fields: detailField },
          { key:'magdal__hospitalization', text:'האם ב-5 השנים האחרונות אושפזת ו/או עברת ניתוח לרבות ניתוח קוסמטי/אסתטי ו/או יצאת לך לעבור ניתוח? (3)', companies: selection.companies, fields: detailField },
          { key:'magdal__tests', text:'האם ב-5 השנים האחרונות עברת בדיקה רפואית אבחנתית לרבות בדיקת הדמיה ו/או נמצא בה ממצא שטרם הצריך בירור רפואי ו/או הינך נמצא במהלך של בדיקות רפואיות אלו שטרם הסתיימו ותוצאות אלו סופיות? אם כן, אנא פרט וצרף את תוצאות הבדיקות.', companies: selection.companies, fields: detailField },
          { key:'magdal__disability', text:'האם נקבעה לך ו/או היך נמצא בתהליך לקבלת אחוזי נכות או הומלץ לך לסייע בשימוש באביזר המסמכים? (23)', companies: selection.companies, fields: detailField },
          // מחלות קשות — היסטוריה משפחתית
          { key:'magdal__family', text:'לרכוש מחלות קשות (מזור מורחב/מזור לסרטן) - היסטוריה משפחתית: האם למיטב ידיעתך התגלו אחד מהמחלות הבאות בקרב אחד מקרובי משפחה מדרגה ראשונה (אם/אב/אח/אחות) אחת מהמחלות הבאות: מחלת לב, מות פתאומי, סרטן (כגון): סרטן שד, סרטן המעי, סרטן העצמות, סרטן הכבד, סוכרת, כליות פוליציסטיות, טרשת נפוצה? (20)', companies: selection.companies, fields: detailField },
          // שאלות מפורטות לפי מערכות גוף
          { key:'magdal__neuro', text:'מערכת העצבים, המוח, ו/או הפרעות התפתחותיות: האם אובחנת אי פעם עם אפילפסיה (4), אירוע מוחי, גידול מוחי, טרשת נפוצה, רעד, פרקינסון, שיתוק, בעיות הכוונה ו/או גדילה (מגיל 5), הפרעות התפתחות הרשב (כגון) ASPERGER, אוטיזם, קשב וריכוז, וכד׳, הפרעות להתפתחות הגמולי הקשות ב-3 השנים האחרונות?', companies: selection.companies, fields: detailField },
          { key:'magdal__mental', text:'נפש: לרבות מחלות נפש שאובחנו ע"י פסיכיאטר או פסיכולוג, פסיכיאטר או רופא משפחה ו/או הפרעות אכילה.', companies: selection.companies, fields: detailField },
          { key:'magdal__cancer', text:'מחלות ממאירות, גידולים: לרבות גידול שפיר, סרטני טרום סרטני (24)', companies: selection.companies, fields: detailField },
          { key:'magdal__respiratory', text:'מערכת ריאות ודרכי נשימה (1): לרבות אסטמה, COPD ו/או ברונכיטיס, מחלות ריאות חסימתיות, אמפיזמה, סרקואידוזיס, דום נשימה בשינה, ציסטיק פיברוזיס, דלקת ריאות חוזרת, שפת.', companies: selection.companies, fields: detailField },
          { key:'magdal__eyes', text:'מערכת הראייה (11): לרבות ליקויי ראייה מעל מס׳ 8, גלאוקומה, רשתית, אובאיטיס, אוסת.', companies: selection.companies, fields: detailField },
          { key:'magdal__ent', text:'מערכת אף אוזן גרון: לרבות בעיות פגיעה בשמיעה (12), דלקות אוזניים חוזרות, סינוסיטיס חוזרת, סטיית מחיצת האף, פוליפים, שקדים, פגיעה במיתרי הקול, דום נשימה.', companies: selection.companies, fields: detailField },
          { key:'magdal__heart', text:'מערכת הלב/לחץ דם (2) (28): לרבות כאבים בחזה, התקף לב, אי ספיקת לב, הפרעות קצב, דום מולד, הפרעות במסתמים, טרומבוזה, תחלואי ריאות, מרוצות אורטות, ורידים מורחבים, ו/או לחץ דם גבוה / יתר לחץ תוך עורקי, טרשת צנתרות.', companies: selection.companies, fields: detailField },
          { key:'magdal__digestive', text:'בקע מערכת העיכול (6) (29): לרבות בקע בקש/שרה/הרניה מכל סוג שהוא, אפליקוס (החזר ושטפי), קוליטיס, רקטוצלה, הפוליפיס/צרבת, כבד מנגדל אן שומני, שחמת (צירוזיס), אבנים בדרכי המרה ו/או כיס מרה, דלקת בלבלב.', companies: selection.companies, fields: detailField },
          { key:'magdal__kidneys', text:'מערכת כליות ודרכי שתן (15): לרבות אי ספיקת כליות, ריפלוקס דרכי שתן, צניחת שלפוחית השתן, דלקת, טיפת שתן, ערמונית מגדלת, בעיה בשתן, שחתה אחרת בשתן (16).', companies: selection.companies, fields: detailField },
          { key:'magdal__endocrine', text:'מחלות מטבוליות, הורמונאלית (אנדוקרינולוגיות): לרבות כבדים כאובים, שיכון הרגיס/המגן (8), יותרת הכבד (9), גוש (18) FMF, בלוטת תריס (9), הפרעות המין, יותרת הכבד, הזעת יתר, הפרעות גדילה, הזעת יתר.', companies: selection.companies, fields: detailField },
          { key:'magdal__blood', text:'מערכת דם, החיסון, מחלות זיהומיות: לרבות דלקת מפרקים, טחול מוגדל, הפרעה בקרישת הדם, הפרעה במערכת החיסון, איידס/ נשאות נגיף האיידס, מחלות תוך חום מעל 3 חודשים.', companies: selection.companies, fields: detailField },
          { key:'magdal__musculoskeletal', text:'מערכת השלד, השרירים, מפרקים (19) מחלות המפרקים (17) בעיות הרצועות: לרבות גב שגרה, פציעה גידידים, ראוונות, כאבי גב (5), בעיה הלולית, דלקות מפרקים ניוון, לופוס, ברית המין, פורס, מחלת מפרקים גיוניות, תסמונת השגרונות הכרונית, תסמינות ההרגשות.', companies: selection.companies, fields: detailField },
          { key:'magdal__skin', text:'מערכת עור וציפורן: לרבות פסוריאזיס, אקזמה, אלקסות, אבנות, גע ו/או גידול דלקתי (26).', companies: selection.companies, fields: detailField },
          { key:'magdal__child_pregnancy', text:'מערכת הריה לכל המועמדים לביטוח - לילדים החל מגיל 12 (27): בנים: בעיות גניקולוגיות ו/או פריון חוזר, הגדלה או הקטנה של השדיים, ניתוחים קיסריים בעבר, וכד׳. בנות: בעיות גניקולוגיות בעבר, יחסים קיסריים בעבר, עקמת הפין, אשר תמיר, הירודרולה/אורינקולה.', companies: selection.companies, fields: detailField },
          { key:'magdal__general', text:'האם הינך מקבל ו/או קיבלת גמלת מביטוח לאומי או מגוף אחר, ו/או מועמד בביצוע אחת מהפעולות הבאות: לאכול, לשתות, להתרחץ, להתלבש, לשכב, לקום, ללכת סל המיוחד ו/או להשתמש במאזנת אחד בביצוע פעולות אלה. (למעט ילד - אם המגבלות הינה טבעית לאור גילו).', companies: selection.companies, fields: detailField }
        ]
      }];
    },
    ...
    getPhoenixHealthSchema(){
      ...
      return [{
        ...
        policyId: selection.sourcePolicies[0]?.id || '',
        questions: [
          { key:'phoenix_full__smoking', text:'למילוי מגיל 16 האם הנך מעשן או עישנת בשנתיים האחרונות לרבות סיגריה אלקטרונית ו/או נרגילה?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__family', text:'האם למיטב ידיעתך התגלו בקרב אחד מקרובי משפחתך מדרגה ראשונה בלבד (אב/אם/אח/אחות), אחת או יותר מהמחלות הבאות, עד גיל 60 (כולל): מחלות לב, סרטן המעי הגס, סרטן שחלות, סרטן השד, סרטן הערמונית, סרטן אחר, סוכרת, כליות פוליציסטיות, טרשת נפוצה, ניוון שרירים, פרקינסון, אלצהיימר?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__drugs', text:'למילוי מגיל 16 האם הינך צורך כעת או צרכת בעבר סמים מסוג כלשהו (אין להצהיר על שימוש חד פעמי)?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__alcohol', text:'למילוי מגיל 18 האם הנך צורך או צרכת בעבר באופן קבוע משקאות אלכוהוליים בכמות של יותר מ- 2 כוסות משקה ליום לרבות יין בירה או כל משקה אלכוהולי אחר?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },

          { key:'phoenix_full__heart', text:'מחלות לב, כלי דם ודם, לרבות, יתר לחץ דם בעשר השנים האחרונות, דליות ברגליים?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__neuro', text:'מערכת העצבים והמח, לרבות אירוע מוחי כגון CVA, TIA, שבץ מוחי, הפרעות קשב וריכוז, אוטיזם, תסמונת דאון, פיגור שכלי, שיתוק מוחין, פוליו, אפילפסיה, רעד, כאבי ראש כרוניים בחמש השנים האחרונות, מיגרנות, התעלפויות חוזרות, הפרעה בשיווי משקל, סחרחורות, פרקינסון, ניוון שרירים, ניוון במערכת העצבים, טרשת נפוצה, שיתוק או חולשה בגפיים, ליקויי קואורדינציה, הפרעות התפתחות וגדילה?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__digestive', text:'מחלות מערכת העיכול, לרבות ושט, תריסריון, קיבה, מעיים, כבד, גושה, כיס המרה, ניתוח בריאטרי - קיצור קיבה, דם סמוי, פוליפים במעי, טחורים, פיסורה?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__endocrine', text:'מחלות והפרעות במערכות ההפרשה הפנימית, לרבות בלבלב, בבלוטת התריס (תירואיד), בחילוף חומרים, סוכרת, יתר שומנים בדם, הזעת יתר, הפרעות הורמונלית?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__eyes', text:'מחלות עיניים והפרעות בראייה, לרבות משקפיים (מספר עדשות 8 ומעלה)?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__ent', text:'מחלות והפרעות במערכת אף, אוזן, גרון, לרבות פגיעה בשמיעה, דלקות אוזניים חוזרות, טינטון, סינוסיטיס חוזרת, סטיית מחיצת האף, פוליפים, שקדים, אדנואידים, פגיעה במיתרי הקול, דום נשימה?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__musculoskeletal', text:'מחלות/כאבים במערכת השלד, לרבות פיברומיאלגיה, עמוד השדרה והמפרקים/ראומטולוגיה, כאבי גב, דלקות פרקים, ארטריטיס, ירידה בצפיפות העצם, אוסטאופורוזיס, הלוקס וולגוס, האם היו שברים ב3- השנים האחרונות?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__respiratory', text:'מחלות והפרעות במערכת הנשימה והריאות, לרבות אסטמה, COPD/אמפיזמה, שיעול דמי?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__kidneys', text:'מחלות והפרעות במערכת הכליות, לרבות דרכי השתן, אבנים בכליות ו/ או בדרכי השתן, אי ספיקת כליות, דם או חלבון בשתן, צניחת שלפוחית השתן?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__cancer', text:'מחלה ממארת (סרטן), גידול שפיר או ממאיר, לרבות גידול טרום סרטני, BCC, מלנומה?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__blood', text:'מחלות מערכת הדם, לרבות הפרעות בקרישת הדם כגון קרישיות יתר, תת קרישיות, APLA?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__skin', text:'מחלות ותופעות בעור, לרבות כתמי לידה, שומות במעקב?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__immune', text:'מחלות במערכת החיסון (אוטואימוניות), לרבות פגיעה במערכת החיסונית, FMF, לופוס-זאבת, איידס לרבות, נשאות נגיף האיידס HIV?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__hernia', text:'בקע/הרניה, לרבות מפשעתי, בטני, טבורי, סרעפתי?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__mental', text:'מחלות נפש שאובחנו ע"י פסיכולוג, פסיכיאטר או רופא משפחה ו/או הפרעות אכילה?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__child_premature', text:'ילדים עד גיל שנה - האם נולד פג? במקרה וכן יש לצרף דו"ח אבחנות וטיפולים עדכני מהרופא המטפל', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__child_growth', text:'ילדים עד גיל 18 - האם קיימות הפרעות התפתחות וגדילה, האם הומלץ או הינך במעקב גובה/ קומה נמוכה?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__child_undescended', text:'בנים עד גיל 18 - האם קיימת אבחנה לאשך טמיר?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__male', text:'למילוי מגיל 18 מחלות והפרעות במערכת המין הזכרית, לרבות הגדלה של ערמונית, עלייה בערכי PSA, בעיית פריון?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__female', text:'מחלות והפרעות במערכת המין הנשית לרבות אנדומטריוזיס, ממצאים בצוואר הרחם, CIN, הפלות חוזרות, הגדלה/הקטנה של השדיים, ציסטות בשד, קונדילומה, פפילומה, ניתוחים קיסריים בעבר, בעיית פריון, האם הינך בהריון?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },

          { key:'phoenix_full__tests', text:'האם עברת או הומלץ לך לעבור בדיקות פולשניות ו/או בדיקות הדמיה ב5- השנים האחרונות?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__hospitalization', text:'האם ב 5 שנים האחרונות אושפזת ו/או עברת ניתוח ו/או הומלץ לך לעבור ניתוח עתידי לרבות, ניתוח קוסמטי ו/או עברת השתלת איבר ב10- שנים האחרונות?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__medications', text:'האם הינך נוטל ו/או הומלץ לך ליטול תרופות באופן קבוע ב3- שנים האחרונות?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField },
          { key:'phoenix_full__disability', text:'האם נקבעה לך נכות זמנית או צמיתה ו/או בתהליך קביעת נכות לרבות, נכות ממחלה או תאונה?', companies:(selection?.companies?.length ? selection.companies : ['כלל','הפניקס','הכשרה','הראל','מגדל','מנורה','איילון']), fields: detailField }
        ]
      }];
    },
    ...
    getMenoraHealthSchema(){
      // הצהרת בריאות מנורה — טופס 227 — לבריאות ומחלות קשות
      const detailField = [{ key:'details', label:'פירוט', type:'textarea' }];
      const companies = ['מנורה'];
      return [{
        key: 'master_menora_health',
        title: 'הצהרת בריאות · מנורה מבטחים',
        summary: 'שאלון בריאות למועמדים לביטוח — מנורה מבטחים (טופס 227), לבריאות ומחלות קשות.',
        questions: [
          { key:'menora__smoking', text:'האם הינך מעשן או עישנת בשנתיים האחרונות? (מגיל 15) ציין כמות סיגריות: סיגריות / נרגילה / סיגריה אלקטרונית / גראס/מריחואנה בעישון (2)', companies, fields: detailField },
          { key:'menora__alcohol', text:'האם הינך שותה כיום או שתית בעבר יותר מ-2 מנות משקה אלכוהולי ליום? (שאלון אלכוהול-1)', companies, fields: detailField },
          { key:'menora__drugs', text:'האם הינך צורך כעת או צרכת בעבר סמים מכל סוג שהוא (אין להצהיר על שימוש חד פעמי)? (שאלון סמים-2)', companies, fields: detailField },
          { key:'menora__clarification', text:'מהלך בירור או מחלה שהסתיימה: האם הופנה לבדיקה בגוף? מהלך הינו בירור רופא שסיים. כל מהלך הינו בירור שטרם הסתיים בגוף? בבדיקה שסיים בבירור. בבדיקת אולטרסאונד אם כן הלך לבדיקה: מגנטומדה, היסטוסקופיה, קולונוסקופיה, PSA (שלא כחלק ממעקב קרין), היסטוסקופיה, EEG, דופלר קרינה לגנום ולאחר קבלת אחת מהן חד שמיעה.', companies, fields: detailField },
          { key:'menora__neuro', text:'מערכת העצבים: לרבות אירוע מוחי• מוח○ טרשת נפוצה○ אפילפסיה (3)○ פרקינסון○ רעד○○ שיתוק מוחין○ פוליו○ ניוון שרירים○ אוטיזם○ תסמונת דאון○ מיגרנה ○ פיגור שכלי○ האם פנה לרופא בשל ירידה בזיכרון ב-3 השנים האחרונות? (שאלון אפילפסיה-3 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__heart', text:'לב, מחלות דם (4): מחלת לב ○ אנמיה ○ מפרצת / AVM ○ הפרעות קרישה (5) ○ DVT (5) ○ PVD ○ AI/ספיקה ○ בדרכי הרגליים ○ חסימה בעורקי הצואר (קרוטיס)○ ITP/ תרומבוציטופניה ○ טלנגיאסיה מינור ○ טלנגיאסיה מייג׳ור (שאלון לב-4 / שאלון כלי דם-5 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__mental', text:'בעיה/מחלה נפשית לרבות דיכאון וחרדה (7): טיפול תרופתי / ניתוח ו/או / דיאטה לרבות: לחץ דם (6) ○ סוכרת (8) ○ כולסטרול (9) ○ טריגליצרידים ○ זריקות לטיפול בעודף משקל (שאלון דם-6 / שאלון סוכרת-8 / שאלון שומנים-9 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__endocrine', text:'אנדוקרינולוגיה ובלוטות: בלוטת תריס/מגן (10) ○ פרתירואיד (10) ○ פגיעה בגדילה ○ בלוטת יותרת המוח (10) ○ בלוטת יותרת הכליה ○ אדרנל ○ פרולקטין ○ טיפול הורמונלי שלא בגלל גיל המעבר ○ הזעת יתר ○ פרולקטין (שאלון אנדוקרינולוגיה-10 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__cancer', text:'סרטן ומחלות ממאירות (11): גידולים ממאירים ○ גידולים שפירים ○ סרטן/מחלה ממארת (שאלון שפירים-11 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__digestive', text:'מע׳ עיכול: מעיים (12) ○ ושט (12) ○ קיבה (12) ○ טחול○ אבנים בכיס (12) ○ כבד (12) ○ צהבת (13) ○ כבד שומני (13) ○ ניתוח/טיפול להשמנת יתר (קיצור קיבה/מאזנת קיבה/לון ז׳ד׳) ○ חסרים (במידה וכן יש לענות על השאלה הבאה) (שאלון מע׳ העיכול-12 / שאלון כבד וצהבת-13 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__respiratory', text:'ריאות ונשימה: אסטמה/ברונכיטיס (14) ○ COPD/אמפיזמה ○ מחלות ריאות חסימתית ○ ציסטיק פיברוזיס ○ דום נשימה בשינה (שאלון אסטמה-14 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__immune', text:'מחלות זיהומיות/דלקתיות: HIV ○ שפקת ○ סרקואידוזיס ○ איידס/נשאות HIV ○ סקלרודרמה ○ מכתב רופא', companies, fields: detailField },
          { key:'menora__kidneys', text:'כליות ודרכי שתן: כליות (15) ○ מערכת/דרכי השתן (15) ○ שלפוחית השתן צניחת (15) ○ טרפת ○ מפשעה ○ שמאל ○ ימין ○ ציסטה אחר (שאלון כליות ודרכי השתן-15)', companies, fields: detailField },
          { key:'menora__hernia', text:'הרניה/בקע: ○ טבורי ○ מפשעתי ○ בטני ○ סרעפתי ○ האם בוצע ניתוח? ○ האם נקבע מצב תקין?', companies, fields: detailField },
          { key:'menora__rheum', text:'ראומטולוגיה ומחלות אוטואימוניות: מחלות מפרקים ○ פיברומיאלגיה ○ מחלות אוטואימוניות/ראומטו ○ FMF -16 ○ לופוס/זאבת ○ גאוטה/פסוריאזיס ○ ארתריט-ראומטואיד ○ תסמונת יובש ○ FMF-16 / מחלת קרון ○ תסמונת הכרונית (שאלון FMF-16 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__ortho', text:'אורטופדיה: גב ועמוד שדרה (17) ○ ירידה בצפיפות העצם ○ שברים (19) ○ ברכיים (18) ○ מרפקים (18) ○ כתפיים (18) ○ מחלת עצמות (18) ○ קרסוליים (18) ○ CTS ○ קרוטיס ○ דלק׳ יריכי/קרסולי ○ CTS (שאלון עמוד שדרה-17 / שאלון מפרקים-18 / שאלון שברים-19 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__eyes', text:'עיניים: מחלה/בעיה בשדה הראיה (8 ומעלה) ○ אובאיטיס ○ עיוורון (21) (שאלון עיניים-21 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__ent', text:'אף/אוזן/גרון: אוזניים (22) ○ ירידה בשמיעה/אטימות (22) ○ טינטון (22) ○ גרון (22) ○ אף (22) ○ בלוטת הרוק (22) ○ מחלת מניייר ○ ורטיגו (שאלון אאג-22)', companies, fields: detailField },
          { key:'menora__skin', text:'עור ורקמות ומיניות: מחלות עור (20) ○ ניוון עור/נגע (20) ○ בדיקות רפואיות הנדרשות עקב ○ פפילומה ○ קונדילומה (20) ○ גידול/נגע בעור (20) ○ פסוריאזיס (20) ○ לופוס עור ○ פמפיגוס ○ אקזמה (20) ○ מיקוזיס פונגואידס ○ פילנדרוסינוס ○ מיקוזיס (שאלון עור ורקמות ומיניות-20 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__male', text:'שאלון גברים (מגיל 18): בלוטת ערמונית (23) ○ בעיות פריון ○ ניתוק המין (23) ○ אשכים (23) ○ הידרוצלה (23) ○ ואריקוצלה (23) ○ גינקומסטיה (קטנת ההד) ○ שאלון בלוטות גברים-23 / מכתב רופא', companies, fields: detailField },
          { key:'menora__female', text:'שאלון נשים (מגיל 14): רחם (24) ○ שלחופית/תכנות (24) ○ גינקולוגית (24) ○ אנדומטריוזיס (24) ○ תכיות דם ○ הריון קיסרי ○ הפלות חוזרות (3 ומעלה) ○ גידולים בשדיים (24) ○ ניתוח הגדלה/הקטנה (24) עם שימוש בשתלים בהתרחבות הריון שיפרוי קיסרי / שאלון נשים-24 / מכתב רופא', companies, fields: detailField },
          { key:'menora__child_dev', text:'שאלון ילדים (עד גיל 18): ○ שפה שסועה ○ חיך שסוע ○ עיכוב בהתפתחות ○ הפרעות גדילה ○ מולד פג* ○ עקמת פין ○ הידרוצלה ○ האם הבעיה נפתרה? ○ כן ○ לא ○ האם רואה כיום? ○ כן ○ לא ○ אשך טמיר: האם הבעיה נפתרה? ○ כן ○ לא ○ האם גדיה כיום? ○ לא ○ כן ○ שאלון גברים ובלוטות הרמונית-23 / מכתב רופא', companies, fields: detailField },
          { key:'menora__sport', text:'האם הנך עוסק בספורט מקצועי בשכר? (מגיל 16) פרט:', companies, fields: detailField },
          { key:'menora__family_critical', text:'היסטוריה משפחתית — שאלה נוספת לרוכשי כיסוי קרן אור טופ/קרן לחיים: האם למיטב ידיעתך אחד מקרוביך המקרובים (אב/אם/אח/אחות) אובחנו באחת או יותר מהמחלות הבאות: ○ מחלת לב ○ מחלת המעי הגס ○ סרטן הגוס ○ כליות פוליציסטיות ○ טרשת נפוצה ○ ניון שרירים ○ פרקינסון ○ סכרת ○ סרטן השד ○ סרטן ערמונית ○ אלצהיימר (שאלון בני משפחה-26)', companies, fields: detailField },
          { key:'menora__family_child', text:'היסטוריה משפחתית לילדים מתחת לגיל שנה הרוכשים כיסוי קרן אור טופ/קרן לחיים: האם למיטב ידיעתך, הורים, בקרב אחד מהמחלות הבאות: קרדיומיופתיה תורשתית ○ כליות פוליציסטיות ○ FMF ○ ציסטיק פיברוזיס ○ SMA ○ מחלה תורשתית אחרת (שאלון בני משפחה-26 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__child_birth', text:'האם התינוק נולד פג או שהייתה לידה אחר שחרורו בגלל פגיעה ו/או נמצאו לטיפול או ממצא בביקורים הראשונים (שאלון בני משפחה-26 / מכתב רופא)', companies, fields: detailField },
          { key:'menora__child_followup', text:'האם התינוק ממתין לתוצאות בדיקות שבוצעו ו/או הומלץ לו לבצע אחר שחרורו מביה"ח מרחשורו מביה"ח ע"י רופא ילדים או ע"י ד"י רופא מומחה אחר?', companies, fields: detailField },
          { key:'menora__child_specialist', text:'האם התינוק נמצא כעת ו/או הומלץ לו לעבור בירור רפואי/מעקב רפואי/טיפול תרופתי ○ קבוע אצל רופא מומחה/מרפאת פגים/התפתחות הילד?', companies, fields: detailField }
        ]
      }];
    },

    getMagdalLifeHealthSchema(){
      // הצהרת בריאות מגדל חיים — טופס 481 — לריסק וריסק משכנתא
      const detailField = [{ key:'details', label:'פירוט', type:'textarea' }];
      const companies = ['מגדל'];
      return [{
        key: 'master_magdal_life_health',
        title: 'הצהרת בריאות · מגדל חיים (ריסק)',
        summary: 'הצהרת בריאות על פי תבנית מגדל חיים לביטוח ריסק וריסק משכנתא (טופס 481).',
        questions: [
          // חלק ב' — שאלות כלליות
          { key:'magdal_life__work_risk', text:'האם יש לך עיסוקים/תחביבים/אורח חיים עם סיכון מיוחד? אם כן, צרף שאלון תחביבים/מקצועי. ברכישת כיסוי לאובדן כושר עבודה, אנו מבקשים להסב את תשומת לבך לרשימת פעיליות של ספורט אתגרי אשר מוחרגות מתכנית הביטוח ואינן מכוסות. הרשימה מופיעה באתר החברה בדף מוצר הביטוח ומתעדכנת מעת לעת. ההחרגה נכונה לגבי מועד קרות מקרה הביטוח. הנגנו מחליזים לבדוק את הרשימה ההחרגות לספורט אתגרי מעת לעת.', companies, fields: detailField },
          { key:'magdal_life__smoking', text:'האם הנך מעשן כיום? (השאלה מתייחסת לכל מוצרי הטבק לרבות סיגריות אלקטרוניות) אם כן ציין מס׳ סיגריות/פעמים שמעשן ביום: בן/בת זוג ___ מבוטח ראשי ___', companies, fields: detailField },
          { key:'magdal_life__past_smoking', text:'אם עניית כי אינך מעשן כיום, האם עישנת בשנתיים האחרונות?', companies, fields: detailField },
          { key:'magdal_life__alcohol', text:'האם הנך שותה באופן קבוע יותר מ-2 כוסות אלכוהול ליום? (22)', companies, fields: detailField },
          { key:'magdal_life__drugs', text:'האם הנך צורך או צרכת סמים כעת או בעבר? (22)', companies, fields: detailField },
          { key:'magdal_life__medications', text:'האם הנך נוטל תרופות באופן קבוע? (21)', companies, fields: detailField },
          { key:'magdal_life__hospitalization', text:'האם ב-5 השנים האחרונות אושפזת ו/או עברת ניתוח ו/או יצאו לך לעבור ניתוח? (3)', companies, fields: detailField },
          { key:'magdal_life__tests', text:'האם ב-5 השנים האחרונות עברת בדיקה רפואית אבחנתית ו/או נמצא בה ממצא שטרם הצריך בירור/מעקב רופא? אם כן פרט וצרף את תוצאות הבדיקות.', companies, fields: detailField },
          { key:'magdal_life__disability', text:'האם הנך נמצא בתהליך לקבלת אחוזי נכות ו/או הומלץ לך לקבלת אחוזי נכות מכל סוג? (23)', companies, fields: detailField },
          { key:'magdal_life__family', text:'האם נקבעה אצל קרובי משפחה מדרגה ראשונה רפואית אצל קרוב משפחה מדרגה הראשונה מתחת לגיל 50, הקשורים ללב, קולון, סרטן, כליות, סוכרת, מערכת העצבים? (20)', companies, fields: detailField },
          // חלק ג' — שאלות מפורטות
          { key:'magdal_life__neuro', text:'מערכת העצבים, המוח ו/או הפרעות התפתחותיות: לרבות אפילפסיה (4), אירוע מוחי, גידול מוחי, טרשת נפוצה, רעד, שיתוק, פרקינסון, אוטיזם, אספרגר, הפרעות קשב וריכוז, סחרחורות/כהות הכרה, הפרעות בזיכרון ב-3 השנים האחרונות, לירידה בזיכרון ב-3 השנים האחרונות?', companies, fields: detailField },
          { key:'magdal_life__mental', text:'נפש: לרבות מחלות דיכאון (13), חרדות, תסמונת פוסט טראומטית. האם אובחנת כחולה ממחלת נפש?', companies, fields: detailField },
          { key:'magdal_life__cancer', text:'מחלות ממאירות, גידולים: לרבות גידול שפיר, סרטני טרום סרטני (24).', companies, fields: detailField },
          { key:'magdal_life__respiratory', text:'מערכת ריאות ודרכי נשימה (1): לרבות אסטמה, COPD, ברונכיטיס, מחלת ריאות חסימתית, אמפיזמה, סרקואידוזיס, דום נשימה בשינה, ציסטיק פיברוזיס, דלקת ריאות חוזרת, שפת.', companies, fields: detailField },
          { key:'magdal_life__eyes', text:'מערכת הראייה (11): לרבות ליקויי ראייה מעל מס׳ 7, הפרעות רשתית, גלאוקומה, לחץ תוך עיני, קטרקט, אובאיטיס, קרטוקונוס.', companies, fields: detailField },
          { key:'magdal_life__ent', text:'מערכת אף אוזן גרון: לרבות ירידה בשמיעה (12), דלקות אוזניים חוזרות, מנייר, טינטון, פוליפים, פגיעה במיתרי הקול.', companies, fields: detailField },
          { key:'magdal_life__heart', text:'מערכת הלב/לחץ דם (2), (28): לרבות יתר לחץ דם, כאבים בחזה, התקף לב, אי ספיקת לב, הפרעות קצב, איוושה, פגיעה במסתמים, טרומבוזה, תחלואי ריאות, מרוצות אורטה, ורידים מורחבים, יתר לחץ תוך ריאתי, טרשת עורקים. האם עברת צנתרות?', companies, fields: detailField },
          { key:'magdal_life__digestive', text:'בקע ומערכת העיכול (6): לרבות בקע בקש/שרה/הרניה מכל סוג שהוא, אפליקוס (החזר ושטפי), קוליטיס, פרוקטוסיגמוס, דימום רקטלי, רקטוצלה, הפטיטיס/צהבת, כבד מוגדל, כבד שומני, שחמת (צירוזיס), אבנים בדרכי המרה ו/או כיס מרה, דלקת בלבלב (פנקריאטיסיס).', companies, fields: detailField },
          { key:'magdal_life__kidneys', text:'מערכת כליות ודרכי שתן (15): לרבות אי ספיקת כליות, כליה פוליציסטית, מום מולד, דלקות חוזרות, ריפלוקס דרכי שתן, צניחת שלפוחית השתן, דם ו/או חלבון בשתן, ערמונית מוגדלת (16).', companies, fields: detailField },
          { key:'magdal_life__endocrine', text:'מחלות מטבוליות, הורמונאליות (אנדוקרינולוגיה): לרבות כבדים כאובים, שיגדון (גאוט), גושה, (18) FMF, בלוטת התריס/המגן (8), יותרת המגן/התריס, הפרעות בגדילה (פולקלטינומיה/פרוקלטינמיה), הפרעות בגדילה (8).', companies, fields: detailField },
          { key:'magdal_life__blood', text:'מערכת הדם, החיסון, מחלות זיהומיות (3): לרבות דלקת מפרקים, ערכים חריגים בספירת דם, הפרעה בקרישת הדם, טחול מוגדל, אנמיה, איידס/נשאות, מחלת חום ממושכת מעל 3 חודשים.', companies, fields: detailField },
          { key:'magdal_life__musculoskeletal', text:'מערכת השלד, השרירים, מפרקים (19), בעיות הרצועות (17): לרבות גב שגרה, פציעה גידידים, ראוונות, כאבי גב (5), פגיעה גידידים/רצועות, בריחת סיד (אוסטיאופורוזיס)/ אוסטיאופניה, פרס, מחלת מפרקים גיוניות (ראומטיד ארטריטיס), לופוס (זאבת), פיברומיאלגיה, תסמונת הכרוניות, סקלרודרמה.', companies, fields: detailField },
          { key:'magdal_life__skin', text:'מערכת עור וציפורן (26): לרבות פסוריאזיס, אקזמה, אלקסות, אבנות, נגע ו/או גידול דלקתי, מחלת עור דלקתית, פפילומה/קונדילומה.', companies, fields: detailField },
          { key:'magdal_life__reproductive', text:'מערכת הריבייה (27): נשים: לרבות גוש בשד/אבצס בשד, ד׳׳ימומים בלא קשר לווסת החודשית, רחם שריוני, אנדומטריוזיס, ציסטות שחלתיות, פלות חוזרות, צניחת רחם ו/או הרנרטיק, בעיות פריון, האם את כעת בהריון? גברים: גוש בשד/אבצס בשד, בעיות פריון, גידול בשד, אשך טמיר, גידול באשך, הידרוצלה, ואריקוצלה.', companies, fields: detailField }
        ]
      }];
    },

    getShortHealthSchema(){
      // הצהרת בריאות מקוצרת לריסק, ריסק משכנתא ומחלות קשות
      const detailField = [{ key:'details', label:'פירוט', type:'textarea' }];
      return [{
        key: 'short_health',
        title: 'הצהרת בריאות מקוצרת',
        summary: 'הצהרת בריאות מקוצרת לריסק ומחלות קשות — הפניקס',
        policyId: '',
        questions: [
          { key:'short__smoking',
            text:'האם הינך מעשן או עישנת במהלך השנתיים האחרונות? אם כן: כמות סיגריות____ תאריך הפסקת עישון___ לרבות סיגריה אלקטרונית',
            fields: detailField },
          { key:'short__tests',
            text:'האם בשנתיים האחרונה עברת או הומלץ לך או שהינך מועמד לביצוע בדיקות פולשניות, בדיקות הדמיה (לרבות CT ,US ו-MRI) או ניתוח/אשפוז? אם כן: סוג הבדיקה/ניתוח: ________ תאריך:________________ משך אשפוז:________________',
            fields: detailField },
          { key:'short__heart',
            text:'מחלות לב, כלי דם ודם?',
            fields: detailField },
          { key:'short__neuro',
            text:'מחלות מערכת העצבים והמוח, אירוע מוחי, אוטיזם, תסמונת דאון, פרקינסון, רעד, ניוון שרירים, טרשת נפוצה?',
            fields: detailField },
          { key:'short__cancer',
            text:'מחלה או גידול ממאיר (סרטן), גידול שפיר או טרום סרטני, CIN?',
            fields: detailField },
          { key:'short__kidneys',
            text:'מחלות מערכת כליות ושתן, דם או חלבון בשתן, PSA גבוה?',
            fields: detailField },
          { key:'short__digestive',
            text:'מחלות מערכת העיכול, דם בצואה, כבד?',
            fields: detailField },
          { key:'short__respiratory',
            text:'מחלות מערכת הנשימה, ריאות?',
            fields: detailField },
          { key:'short__diabetes',
            text:'מחלת הסוכרת?',
            fields: detailField },
          { key:'short__musculoskeletal',
            text:'מחלות ראומטולוגיות, מפרקים/אורטופדי או שלד לרבות לופוס/זאבת, בריחת סידן?',
            fields: detailField },
          { key:'short__mental',
            text:'מחלה נפשית לרבות דיכאון, הפרעה דו קוטבית, סכיזופרניה או פוסט טראומה PTSD?',
            fields: detailField },
          { key:'short__senses',
            text:'מחלות חושים (ראייה, שמיעה) לרבות משקפיים (מספר העדשות 8 ומעלה)?',
            fields: detailField },
          { key:'short__family',
            text:'האם למיטב ידיעתך התגלו בקרב אחד מקרובי משפחתך מדרגה ראשונה בלבד (אב/אם/אח/אחות), אחת או יותר מהמחלות הבאות עד גיל 60 (כולל): מחלות לב, סרטן, סוכרת נעורים, כליות פוליציסטיות, טרשת נפוצה, ניוון שרירים, פרקינסון, אלצהיימר?',
            fields: detailField }
        ]
      }];
    },
    ...
    getAyalonHealthSchema(){
      // הצהרת בריאות איילון — טופס 001 — לבריאות ומחלות קשות
      const selection = this.getHealthMasterSelection();
      ...
      return [{
        ...
        summary: 'הצהרת בריאות על פי טופס 001 של איילון חברה לביטוח, לביטוח בריאות ומחלות קשות.',
        policyId: selection?.sourcePolicies?.[0]?.id || '',
        questions: [
          // אורח חיים / עישון
          { key:'ayalon__alcohol', text:'אורח חיים 1: האם אתה שותה או שתית בעבר ב-5 השנים האחרונות באופן קבוע יותר מ-3 כוסות אלכוהול ביום (7)?', companies, fields: detailField },
          { key:'ayalon__drugs', text:'אורח חיים 2: האם אתה צורך או צרכת בעבר סמים (7)?', companies, fields: detailField },
          { key:'ayalon__smoking', text:'אורח חיים 3: האם אתה מעשן כיום? נא ציין כמה שנים אתה/ה מעשן _____ וכמות סיגריות ביום _____. במידה והפסקת לעשן — נא ציין מתי (חודש ושנה) _____. (השאלה מתייחסת לכל מוצרי הטבק, לרבות סיגריות אלקטרוניות, מכשיר אידוי אלקטרוני).', companies, fields: detailField },
          // רקע רפואי
          { key:'ayalon__medications', text:'רקע רפואי 1: האם אתה נוטל או הומלץ לך לטיפול בתרופות באופן קבוע?', companies, fields: detailField },
          { key:'ayalon__hospitalization', text:'רקע רפואי 2: האם ב-5 השנים האחרונות אושפזת או עברת ניתוח ו/או יצאו לך לעבור ניתוח (9)? אם כן, נא פרט.', companies, fields: detailField },
          { key:'ayalon__tests', text:'רקע רפואי 3: האם ב-5 השנים האחרונות עברת או הופנית לביצוע בדיקה רפואית אבחנתית לרבות בדיקות הדמיה (כגון CT, MRI), אקו לב, מיפוי, פולשנית (כגון ביופסיה), קולונוסקופיה, דם סמוי, שאינן בדיקות שגרתיות ו/או שנמצא בהן ממצא שהצריך בירור ו/או מעקב, או שהנך כיום בהליך של אחת או יותר מבדיקות אלה?', companies, fields: detailField },
          { key:'ayalon__disability', text:'רקע רפואי 4: האם נקבעה לך נכות ו/או אתה נמצא בתהליך לקבלת אחוזי נכות (16)?', companies, fields: detailField },
          { key:'ayalon__family_history', text:'רקע רפואי 5 — היסטוריה משפחתית (32): האם למיטב ידיעתך התגלו בקרב אחד מקרוביך מדרגה ראשונה בלבד (אם, אב, אח, אחות), אחת מהמחלות הבאות: סרטן גס המעיים, סרטן שד, סרטן ערמונית, מחלת לב, סוכרת, טרשת נפוצה, כליות פוליציסטיות, סרטן שחלות, מחלת אלצהיימר, ניוון שרירים, מחלת פרקינסון?', companies, fields: detailField },
          // פירוט מחלות לפי מערכות
          { key:'ayalon__neuro', text:'מערכת עצבים, המוח ו/או הפרעות התפתחותיות (1): אירוע מוחי, מיגרנה, אפילפסיה (3), טרשת נפוצה, פרקינסון, ניוון שרירים, שיתוקים, קשב וריכוז (3), בעיות בהתפתחות. האם פנית לרופא בתלונות הקשורות לירידה בזיכרון ב-3 השנים האחרונות?', companies, fields: detailField },
          { key:'ayalon__mental', text:'נפש (17): מחלה שאובחנה ע"י פסיכולוג, פסיכיאטר או רופא משפחה ו/או הפרעות אכילה. נא פרט.', companies, fields: detailField },
          { key:'ayalon__cancer', text:'מחלות ממאירות, גידולים (3): לרבות גידול שפיר, סרטני, טרום סרטני. נא פרט.', companies, fields: detailField },
          { key:'ayalon__respiratory', text:'מערכת ריאות ודרכי נשימה (2): לרבות אסטמה, מחלה חסימתית, COPD, סרקואידוזיס, דום נשימה בשינה, ציסטיק פיברוזיס, דלקות חוזרות, אלרגיות, תסחיף.', companies, fields: detailField },
          { key:'ayalon__eyes', text:'מערכת הראייה (8): לרבות ליקויי ראייה מעל מס\' 8, הפרדות רשתית, גלאוקומה, קטרקט, אובאיטיס, קרטוקונוס.', companies, fields: detailField },
          { key:'ayalon__ent', text:'מערכת אף אוזן גרון (20): לרבות פגיעה בשמיעה, דלקות אוזניים חוזרות, מניייר, טינטון, סינוסיטיס חוזרת, סטיית מחיצת האף, פוליפים, שקדים, פגיעה במיתרי הקול.', companies, fields: detailField },
          { key:'ayalon__heart', text:'מערכת הלב (11), כלי דם (22): כאבים בחזה, התקף לב, מחלות לב, מחלת לב כרונית, מפרצת, מסתמים, בעיות בקרישת דם, מחלות לב ב-3 שנים האחרונות. האם אתה סובל כיום מיתר לחץ דם או האם סבלת ב-3 שנים האחרונות מיתר לחץ דם (12)?', companies, fields: detailField },
          { key:'ayalon__digestive', text:'מערכת העיכול (6): לרבות בקע/שבר/הרניה מכל סוגו שהוא, קרוהן, קוליטיס, מחלות כבד/מרה/לבלב (19), טחורים, פיסורה.', companies, fields: detailField },
          { key:'ayalon__kidneys', text:'מערכת כליות ודרכי שתן (13): לרבות אי ספיקת כליות, כליה פוליציסטית, אבנים בכליות, דלקות, צניחת שלפוחית השתן, דם ו/או חלבון בשתן, ערמונית (23).', companies, fields: detailField },
          { key:'ayalon__endocrine', text:'מחלה מטבולית, הורמונאלית, אנדוקרינולוגיה (10): לרבות סוכרת, סוכרת הריון (15), בלוטת התריס (4), טרום סוכרת, הזעת יתר.', companies, fields: detailField },
          { key:'ayalon__musculoskeletal', text:'מערכת השלד (5): שריר, פרקים ורקמת חיבור (21): לרבות בעיות בגב, ברכיים, עמוד שדרה, בריחת סיד, לופוס, פיברומיאלגיה, גאוט, FMF (אף.אם.אף) (1).', companies, fields: detailField },
          { key:'ayalon__skin', text:'מחלות עור ומין (18): לרבות פסוריאזיס, גומיפגוס, פפילומה / קונדילומה.', companies, fields: detailField },
          { key:'ayalon__infectious', text:'מחלות זיהומיות (13): לרבות איידס, נשא HIV.', companies, fields: detailField },
          { key:'ayalon__female', text:'לנשים (14) (מגיל 12 בלבד): בעיות גינקולוגיות ו/או בעיות פריון, ניתוח קיסרי, הגדלת/הקטנת שדיים.', companies, fields: detailField }
        ]
      }];
    },

    getAyalonCriticalHealthSchema(){
      // הצהרת בריאות איילון — מחלות קשות בלבד
      const companies = ['איילון'];
      const detailField = [{ key:'details', label:'פירוט', type:'textarea' }];
      return [{
        key: 'master_ayalon_critical',
        title: 'הצהרת בריאות מחלות קשות · איילון חברה לביטוח',
        summary: 'הצהרת בריאות על פי טופס 001 של איילון חברה לביטוח, למוצר מחלות קשות.',
        policyId: '',
        questions: [
          { key:'ayalon_crit__alcohol', text:'אורח חיים 1: האם אתה שותה או שתית בעבר ב-5 השנים האחרונות באופן קבוע יותר מ-3 כוסות אלכוהול ביום (7)?', companies, fields: detailField },
          { key:'ayalon_crit__drugs', text:'אורח חיים 2: האם אתה צורך או צרכת בעבר סמים (7)?', companies, fields: detailField },
          { key:'ayalon_crit__smoking', text:'אורח חיים 3: האם אתה מעשן כיום? נא ציין כמה שנים אתה/ה מעשן _____ וכמות סיגריות ביום _____. במידה והפסקת לעשן — נא ציין מתי (חודש ושנה) _____. (השאלה מתייחסת לכל מוצרי הטבק, לרבות סיגריות אלקטרוניות, מכשיר אידוי אלקטרוני).', companies, fields: detailField },
          { key:'ayalon_crit__medications', text:'רקע רפואי 1: האם אתה נוטל או הומלץ לך לטיפול בתרופות באופן קבוע?', companies, fields: detailField },
          { key:'ayalon_crit__hospitalization', text:'רקע רפואי 2: האם ב-5 השנים האחרונות אושפזת או עברת ניתוח ו/או יצאו לך לעבור ניתוח (9)? אם כן, נא פרט.', companies, fields: detailField },
          { key:'ayalon_crit__tests', text:'רקע רפואי 3: האם ב-5 השנים האחרונות עברת או הופנית לביצוע בדיקה רפואית אבחנתית לרבות בדיקות הדמיה (כגון CT, MRI), אקו לב, מיפוי, פולשנית (כגון ביופסיה), קולונוסקופיה, דם סמוי, שאינן בדיקות שגרתיות ו/או שנמצא בהן ממצא שהצריך בירור ו/או מעקב, או שהנך כיום בהליך של אחת או יותר מבדיקות אלה?', companies, fields: detailField },
          { key:'ayalon_crit__disability', text:'רקע רפואי 4: האם נקבעה לך נכות ו/או אתה נמצא בתהליך לקבלת אחוזי נכות (16)?', companies, fields: detailField },
          { key:'ayalon_crit__family_history', text:'רקע רפואי 5 — היסטוריה משפחתית (32): האם למיטב ידיעתך התגלו בקרב אחד מקרוביך מדרגה ראשונה בלבד (אם, אב, אח, אחות), אחת מהמחלות הבאות: סרטן גס המעיים, סרטן שד, סרטן ערמונית, מחלת לב, סוכרת, טרשת נפוצה, כליות פוליציסטיות, סרטן שחלות, מחלת אלצהיימר, ניוון שרירים, מחלת פרקינסון?', companies, fields: detailField },
          { key:'ayalon_crit__neuro', text:'מערכת עצבים, המוח ו/או הפרעות התפתחותיות (1): אירוע מוחי, מיגרנה, אפילפסיה (3), טרשת נפוצה, פרקינסון, ניוון שרירים, שיתוקים, קשב וריכוז (3), בעיות בהתפתחות. האם פנית לרופא בתלונות הקשורות לירידה בזיכרון ב-3 השנים האחרונות?', companies, fields: detailField },
          { key:'ayalon_crit__mental', text:'נפש (17): מחלה שאובחנה ע"י פסיכולוג, פסיכיאטר או רופא משפחה ו/או הפרעות אכילה. נא פרט.', companies, fields: detailField },
          { key:'ayalon_crit__cancer', text:'מחלות ממאירות, גידולים (3): לרבות גידול שפיר, סרטני, טרום סרטני. נא פרט.', companies, fields: detailField },
          { key:'ayalon_crit__respiratory', text:'מערכת ריאות ודרכי נשימה (2): לרבות אסטמה, מחלה חסימתית, COPD, סרקואידוזיס, דום נשימה בשינה, ציסטיק פיברוזיס, דלקות חוזרות, אלרגיות, תסחיף.', companies, fields: detailField },
          { key:'ayalon_crit__eyes', text:'מערכת הראייה (8): לרבות ליקויי ראייה מעל מס\' 8, הפרדות רשתית, גלאוקומה, קטרקט, אובאיטיס, קרטוקונוס.', companies, fields: detailField },
          { key:'ayalon_crit__ent', text:'מערכת אף אוזן גרון (20): לרבות פגיעה בשמיעה, דלקות אוזניים חוזרות, מניייר, טינטון, סינוסיטיס חוזרת, סטיית מחיצת האף, פוליפים, שקדים, פגיעה במיתרי הקול.', companies, fields: detailField },
          { key:'ayalon_crit__heart', text:'מערכת הלב (11), כלי דם (22): כאבים בחזה, התקף לב, מחלות לב, מחלת לב כרונית, מפרצת, מסתמים, בעיות בקרישת דם. האם אתה סובל כיום מיתר לחץ דם או האם סבלת ב-3 שנים האחרונות מיתר לחץ דם (12)?', companies, fields: detailField },
          { key:'ayalon_crit__digestive', text:'מערכת העיכול (6): לרבות בקע/שבר/הרניה מכל סוגו שהוא, קרוהן, קוליטיס, מחלות כבד/מרה/לבלב (19), טחורים, פיסורה.', companies, fields: detailField },
          { key:'ayalon_crit__kidneys', text:'מערכת כליות ודרכי שתן (13): לרבות אי ספיקת כליות, כליה פוליציסטית, אבנים בכליות, דלקות, צניחת שלפוחית השתן, דם ו/או חלבון בשתן, ערמונית (23).', companies, fields: detailField },
          { key:'ayalon_crit__endocrine', text:'מחלה מטבולית, הורמונאלית, אנדוקרינולוגיה (10): לרבות סוכרת, סוכרת הריון (15), בלוטת התריס (4), טרום סוכרת, הזעת יתר.', companies, fields: detailField },
          { key:'ayalon_crit__musculoskeletal', text:'מערכת השלד (5): שריר, פרקים ורקמת חיבור (21): לרבות בעיות בגב, ברכיים, עמוד שדרה, בריחת סיד, לופוס, פיברומיאלגיה, גאוט, FMF (אף.אם.אף) (1).', companies, fields: detailField },
          { key:'ayalon_crit__skin', text:'מחלות עור ומין (18): לרבות פסוריאזיס, גומיפגוס, פפילומה / קונדילומה.', companies, fields: detailField },
          { key:'ayalon_crit__infectious', text:'מחלות זיהומיות (13): לרבות איידס, נשא HIV.', companies, fields: detailField },
          { key:'ayalon_crit__female', text:'לנשים (14) (מגיל 12 בלבד): בעיות גינקולוגיות ו/או בעיות פריון, ניתוח קיסרי, הגדלת/הקטנת שדיים.', companies, fields: detailField }
        ]
      }];
    },

    getHachsharaHealthSchema(){
      // הצהרת בריאות הכשרה — טופס 2498 — לבריאות ומחלות קשות
      const selection = this.getHealthMasterSelection();
      ...
      return [{
        ...
        summary: 'הצהרת בריאות על פי טופס 2498 של הכשרה חברה לביטוח, לביטוח בריאות ומחלות קשות.',
        policyId: selection?.sourcePolicies?.[0]?.id || '',
        questions: [
          // א. שאלות כלליות
          { key:'hachshara__hospitalization', text:'א.1. האם בעשר השנים האחרונות אושפזת (ניתחת (28)) / "יצוי" לך לעבור ניתוח (28)?', companies, fields: detailField },
          { key:'hachshara__tests_5y', text:'א.2. האם עברת בדיקות רפואיות או הופנת לבדיקות ב-5 שנים האחרונות לגילוי/אבחון של: סרטון / ניוון גפיים (26) / בעיות לב / בדיקות הדמיה כגון US, MRI, CT / מחלות ניוון/כרוניות / תרשת נפוצה / הנסינגטון / בליות / פוליו / ציסטות / פרקינסון / מיפוי עצמות / בדיקה עם ממצא שאחרי בירור/מעקב רפואי / בדיקה לגלוי איידס?', companies, fields: detailField },
          { key:'hachshara__treatment_5y', text:'א.3. האם בחמש השנים האחרונות: הנך בטיפול רפואי או תרופתי / הוגדרת נדרשת לטיפול רפואי / הומלך לך על טיפול רפואי או תרופתי?', companies, fields: detailField },
          { key:'hachshara__chronic', text:'א.4. האם בחמש השנים האחרונות סבלת מ: סכרת (6) / יתר לחץ הדם (8) / יתר שומנים בדם (7)? אם כן, נא צרף שאלון מתאים.', companies, fields: detailField },
          { key:'hachshara__memory', text:'א.5. האם פנית לרופא בתלונות הקשורות בירידה בזיכרון?', companies, fields: detailField },
          { key:'hachshara__disability', text:'א.6. האם נגבעו לך אחוזי נכות ממחלה/תאונה/מום או הגשת בקשה לקביעתם? (3) / האם נפעת/סבלת משהר ב-5 השנים האחרונות? (3)', companies, fields: detailField },
          { key:'hachshara__mental', text:'א.7. האם אובחנת כלוקה ב: מחלת נפש / הפרעות נפשיות / תרדה / דיכאון / הפרעות אכילה? אם כן, נא למלא שאלון ביעות נפשיות (13) ולצרף מכתב רופא מפורט.', companies, fields: detailField },
          { key:'hachshara__substances', text:'א.8. האם השתמשת או שהנך משתמש ב: סמים (4) / אלכוהול (5)? נא לפרט סוג, כמות ותדירות.', companies, fields: detailField },
          // ב. האם הנך סובל או סבלת אי פעם מסימני/מחלות
          { key:'hachshara__neuro', text:'ב.1. בעיות ומחלות מערכת העצבים והמוח: ארוע מוחי / שיתוק / הפרעות תחושה/תנועה / אפילפסיה (9) / שיתוק מוחין / פרקינסון / טרשת נפוצה / הפרעות קשב וריכוז (10) / אוטיזם / גידול (גם שפיר)?', companies, fields: detailField },
          { key:'hachshara__respiratory', text:'ב.2. בעיות ומחלות ריאות ודרכי הנשימה: ברונכיטיס / אסטמה / אמפיזמה / מחלת ריאות חסימתית / שחפת?', companies, fields: detailField },
          { key:'hachshara__heart', text:'ב.3. בעיות ומחלות לב וכלי דם: קרדיומיופתיה / סהומחזה / מחלת לב איסכמית / אי ספיקת לב / הפרעות קצב הלב / בעיות מסתמי הלב / מומי לב / דלקת כלי הדם/וסקוליטיס (27)?', companies, fields: detailField },
          { key:'hachshara__blood', text:'ב.4. בעיות דם וקרישה: אנמיה (12)?', companies, fields: detailField },
          { key:'hachshara__liver', text:'ב.5. בעיות ומחלות כבד: הפטיטיס/צהבת / שחמת הכבד / כבד שמוני / הפרעה בתפקוד הכבד?', companies, fields: detailField },
          { key:'hachshara__digestive', text:'ב.6. בעיות מפים ודרכי העיכול: קרוהן / קוליטיס / אולקוס/כיב קיבה / מעי רגיז / פיסורה / טחורים (15)?', companies, fields: detailField },
          { key:'hachshara__kidneys', text:'ב.7. בעיות ומחלות כליות: אי ספיקת כליות / בליות פוליציסטיות / דם או חלבון בשתן (16)?', companies, fields: detailField },
          { key:'hachshara__glands', text:'ב.8. בעיות ומחלות בלוטות: ערמונית/פרוסטטה (20) / בלוטת המגן/תירוס (21) / יותרת הכליה / בלוטת הרוק / הלבלב?', companies, fields: detailField },
          { key:'hachshara__skin', text:'ב.9. בעיות מין ועור: איידס / פסוריאזיס (25) / שומות במעקב (26)?', companies, fields: detailField },
          { key:'hachshara__aids', text:'ב.10. מחלת איידס / נשאות איידס?', companies, fields: detailField },
          { key:'hachshara__musculoskeletal', text:'ב.11. מחלות ובעיות פרקים ועצמות: בעיות גב, צוואר, עמוד השדרה (24) / פיברומיאלגיה / גאוט (18) / פוליו / בריחת סיד / אנקילוזינג ספונדיליטיס?', companies, fields: detailField },
          { key:'hachshara__cancer', text:'ב.12. סרטון / מחלות ממאירות / גידולים (ממאירים)? כל שסימנת אחת מהתשובות יש למלא שאלון (26).', companies, fields: detailField },
          { key:'hachshara__autoimmune', text:'ב.13. מחלות אוטואימוניות: מחלות ניוון / מחלת כרוניות / פוליו / לופוס/זאבת / ניוון שרירים / סרקואידוזיס / מחלות ו/או הפרעות אנדוקרינולוגיות (19) / F.M.F?', companies, fields: detailField },
          { key:'hachshara__eyes', text:'ב.14. בעיות ומחלות עיניים וראיה: גלאוקומה / רטיניטיס פיגמנטוזה / קוצר ראיה (מעל 8) / קרטוקונוס (22)?', companies, fields: detailField },
          { key:'hachshara__ent', text:'ב.15. בעיות אזניים ושמיעה: דום נשימה / סחרחורות / הפרעות בשיווי משקל / מיגרנה/כאבי ראש / בעיות אף ואוזן (23)?', companies, fields: detailField },
          { key:'hachshara__hernia', text:'ב.16. האם בחמש השנים האחרונות סבלת מאי אשש כל סיטמין ב: בקע / הרניה מפשעתי, בגני, סבורי, סרעפתי?', companies, fields: detailField },
          { key:'hachshara__female', text:'ב.17. לנשים מגיל 18 בלבד: בעיות ומחלות בגינקולוגיות / אנדומטריוזיס / דימומים / מחלות שדיים / בעיות פריון / מפלות חוזרות / ניתוח קיסרי / הגדלת חזה (29)? האם כעת את בהריון?', companies, fields: detailField },
          { key:'hachshara__child_dev', text:'ב.18. לילדים עד גיל 18 בלבד: הפרעה/עיכוב בהתפתחות / בגידה בגדילה / נולד/ה כפג / הפרעה או בעיה רגשית / ח"ש שפה שסועה? האם נותח/ה? האם הבעיה עדיין קיימת?', companies, fields: detailField },
          // ג. רכישת כיסוי מחלות קשות
          { key:'hachshara__family_critical', text:'ג. לרכישת כיסוי מחלות קשות — תולדות משפחה (ילדים מגיל 18): האם, למיטב ידיעתך, קרוב/ה מדרגה ראשונה (הורים/אחים) לקה באחת ממחלות הבאות: סרטן / גידול שפיר / מחלות ניוון גפיים / הנגטינגטון / פרקינסון / מחלות ניוון שרירים/מוטורי / טרשת נפוצה / מחלות לב, צנחו, קרדיומיופתיה / כליות פוליציסטיות / אלצהיימר / פרקינסון (27)?', companies, fields: detailField },
          // ד. תינוקות עד גיל חצי שנה
          { key:'hachshara__infant_1', text:'ד.1. הצהרת בריאות לתינוק עד גיל חצי שנה (למילוי הורים בלבד): האם שבוע נולד התינוק? האם סמה בבגינה ו/או כל מחלה אחרת שאינה מחלת ילדים כלשהי לאחר שחרורו?', companies, fields: detailField },
          { key:'hachshara__infant_2', text:'ד.2. הצהרת בריאות לתינוק עד גיל חצי שנה (למילוי הורים בלבד): האם המולך בעת השחרור ו/או לאחר השחרור על המשך מעקב ספציפי אצל רופא מומחה/מרפאת פגים/התפתחות הילד ו/או ממתין לתוצאות בדיקות ו/או הומלך לבצע בדיקות לאחר השחרור?', companies, fields: detailField }
        ]
      }];
    },

    getHachsharaCriticalHealthSchema(){
      // הצהרת בריאות הכשרה — מחלות קשות בלבד (ללא בריאות)
      const companies = ['הכשרה'];
      const detailField = [{ key:'details', label:'פירוט', type:'textarea' }];
      return [{
        key: 'master_hachshara_critical',
        title: 'הצהרת בריאות מחלות קשות · הכשרה חברה לביטוח',
        summary: 'הצהרת בריאות על פי טופס 2498 של הכשרה חברה לביטוח, למוצר מחלות קשות.',
        policyId: '',
        questions: [
          { key:'hachshara_crit__hospitalization', text:'א.1. האם בעשר השנים האחרונות אושפזת (ניתחת (28)) / "יצוי" לך לעבור ניתוח (28)?', companies, fields: detailField },
          { key:'hachshara_crit__tests_5y', text:'א.2. האם עברת בדיקות רפואיות או הופנת לבדיקות ב-5 שנים האחרונות לגילוי/אבחון של: סרטון / ניוון גפיים (26) / בעיות לב / בדיקות הדמיה כגון US, MRI, CT / מחלות ניוון/כרוניות / תרשת נפוצה / הנסינגטון / בליות / פוליו / ציסטות / פרקינסון / מיפוי עצמות / בדיקה עם ממצא שאחרי בירור/מעקב רפואי / בדיקה לגלוי איידס?', companies, fields: detailField },
          { key:'hachshara_crit__treatment_5y', text:'א.3. האם בחמש השנים האחרונות: הנך בטיפול רפואי או תרופתי / הוגדרת נדרשת לטיפול רפואי / הומלך לך על טיפול רפואי או תרופתי?', companies, fields: detailField },
          { key:'hachshara_crit__chronic', text:'א.4. האם בחמש השנים האחרונות סבלת מ: סכרת (6) / יתר לחץ הדם (8) / יתר שומנים בדם (7)? אם כן, נא צרף שאלון מתאים.', companies, fields: detailField },
          { key:'hachshara_crit__memory', text:'א.5. האם פנית לרופא בתלונות הקשורות בירידה בזיכרון?', companies, fields: detailField },
          { key:'hachshara_crit__disability', text:'א.6. האם נגבעו לך אחוזי נכות ממחלה/תאונה/מום או הגשת בקשה לקביעתם? (3) / האם נפעת/סבלת משהר ב-5 השנים האחרונות? (3)', companies, fields: detailField },
          { key:'hachshara_crit__mental', text:'א.7. האם אובחנת כלוקה ב: מחלת נפש / הפרעות נפשיות / תרדה / דיכאון / הפרעות אכילה? אם כן, נא למלא שאלון ביעות נפשיות (13) ולצרף מכתב רופא מפורט.', companies, fields: detailField },
          { key:'hachshara_crit__substances', text:'א.8. האם השתמשת או שהנך משתמש ב: סמים (4) / אלכוהול (5)? נא לפרט סוג, כמות ותדירות.', companies, fields: detailField },
          { key:'hachshara_crit__neuro', text:'ב.1. בעיות ומחלות מערכת העצבים והמוח: ארוע מוחי / שיתוק / הפרעות תחושה/תנועה / אפילפסיה (9) / שיתוק מוחין / פרקינסון / טרשת נפוצה / הפרעות קשב וריכוז (10) / אוטיזם / גידול (גם שפיר)?', companies, fields: detailField },
          { key:'hachshara_crit__respiratory', text:'ב.2. בעיות ומחלות ריאות ודרכי הנשימה: ברונכיטיס / אסטמה / אמפיזמה / מחלת ריאות חסימתית / שחפת?', companies, fields: detailField },
          { key:'hachshara_crit__heart', text:'ב.3. בעיות ומחלות לב וכלי דם: קרדיומיופתיה / סהומחזה / מחלת לב איסכמית / אי ספיקת לב / הפרעות קצב הלב / בעיות מסתמי הלב / מומי לב / דלקת כלי הדם/וסקוליטיס (27)?', companies, fields: detailField },
          { key:'hachshara_crit__blood', text:'ב.4. בעיות דם וקרישה: אנמיה (12)?', companies, fields: detailField },
          { key:'hachshara_crit__liver', text:'ב.5. בעיות ומחלות כבד: הפטיטיס/צהבת / שחמת הכבד / כבד שמוני / הפרעה בתפקוד הכבד?', companies, fields: detailField },
          { key:'hachshara_crit__digestive', text:'ב.6. בעיות מפים ודרכי העיכול: קרוהן / קוליטיס / אולקוס/כיב קיבה / מעי רגיז / פיסורה / טחורים (15)?', companies, fields: detailField },
          { key:'hachshara_crit__kidneys', text:'ב.7. בעיות ומחלות כליות: אי ספיקת כליות / בליות פוליציסטיות / דם או חלבון בשתן (16)?', companies, fields: detailField },
          { key:'hachshara_crit__glands', text:'ב.8. בעיות ומחלות בלוטות: ערמונית/פרוסטטה (20) / בלוטת המגן/תירוס (21) / יותרת הכליה / בלוטת הרוק / הלבלב?', companies, fields: detailField },
          { key:'hachshara_crit__skin', text:'ב.9. בעיות מין ועור: איידס / פסוריאזיס (25) / שומות במעקב (26)?', companies, fields: detailField },
          { key:'hachshara_crit__aids', text:'ב.10. מחלת איידס / נשאות איידס?', companies, fields: detailField },
          { key:'hachshara_crit__musculoskeletal', text:'ב.11. מחלות ובעיות פרקים ועצמות: בעיות גב, צוואר, עמוד השדרה (24) / פיברומיאלגיה / גאוט (18) / פוליו / בריחת סיד / אנקילוזינג ספונדיליטיס?', companies, fields: detailField },
          { key:'hachshara_crit__cancer', text:'ב.12. סרטון / מחלות ממאירות / גידולים (ממאירים)? כל שסימנת אחת מהתשובות יש למלא שאלון (26).', companies, fields: detailField },
          { key:'hachshara_crit__autoimmune', text:'ב.13. מחלות אוטואימוניות: מחלות ניוון / מחלת כרוניות / פוליו / לופוס/זאבת / ניוון שרירים / סרקואידוזיס / מחלות ו/או הפרעות אנדוקרינולוגיות (19) / F.M.F?', companies, fields: detailField },
          { key:'hachshara_crit__eyes', text:'ב.14. בעיות ומחלות עיניים וראיה: גלאוקומה / רטיניטיס פיגמנטוזה / קוצר ראיה (מעל 8) / קרטוקונוס (22)?', companies, fields: detailField },
          { key:'hachshara_crit__ent', text:'ב.15. בעיות אזניים ושמיעה: דום נשימה / סחרחורות / הפרעות בשיווי משקל / מיגרנה/כאבי ראש / בעיות אף ואוזן (23)?', companies, fields: detailField },
          { key:'hachshara_crit__hernia', text:'ב.16. האם בחמש השנים האחרונות סבלת מבקע / הרניה מפשעתי, בגני, סבורי, סרעפתי?', companies, fields: detailField },
          { key:'hachshara_crit__female', text:'ב.17. לנשים מגיל 18 בלבד: בעיות ומחלות בגינקולוגיות / אנדומטריוזיס / דימומים / מחלות שדיים / בעיות פריון / מפלות חוזרות / ניתוח קיסרי / הגדלת חזה (29)? האם כעת את בהריון?', companies, fields: detailField },
          { key:'hachshara_crit__child_dev', text:'ב.18. לילדים עד גיל 18 בלבד: הפרעה/עיכוב בהתפתחות / בגידה בגדילה / נולד/ה כפג / הפרעה או בעיה רגשית / שפה שסועה? האם נותח/ה? האם הבעיה עדיין קיימת?', companies, fields: detailField },
          { key:'hachshara_crit__family_critical', text:'ג. תולדות משפחה (ילדים מגיל 18): האם, למיטב ידיעתך, קרוב/ה מדרגה ראשונה (הורים/אחים) לקה באחת ממחלות הבאות: סרטן / גידול שפיר / מחלות ניוון גפיים / הנגטינגטון / פרקינסון / מחלות ניוון שרירים/מוטורי / טרשת נפוצה / מחלות לב, קרדיומיופתיה / כליות פוליציסטיות / אלצהיימר (27)?', companies, fields: detailField }
        ]
      }];
    },
    ...
    getHealthQuestionsFiltered(){
      ...
      const hasHealth = allPolicies.some(p => safeTrim(p?.type) === 'בריאות');
      const hasShortTypes = allPolicies.some(p => ['ריסק','ריסק משכנתא','מחלות קשות','סרטן','אובדן כושר עבודה'].includes(safeTrim(p?.type)));

      if(hasHealth){
        const healthPolicies = allPolicies.filter(p => safeTrim(p?.type) === 'בריאות');
        const allMigdal    = healthPolicies.length > 0 && healthPolicies.every(p => safeTrim(p?.company) === 'מגדל');
        const allMenora    = healthPolicies.length > 0 && healthPolicies.every(p => safeTrim(p?.company) === 'מנורה');
        const allHachshara = healthPolicies.length > 0 && healthPolicies.every(p => safeTrim(p?.company) === 'הכשרה');
        const allAyalon    = healthPolicies.length > 0 && healthPolicies.every(p => safeTrim(p?.company) === 'איילון');
        if(allMigdal){
          const s = this.getMagdalHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        } else if(allMenora){
          const s = this.getMenoraHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        } else if(allHachshara){
          const s = this.getHachsharaHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        } else if(allAyalon){
          const s = this.getAyalonHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        } else {
          const s = this.getPhoenixHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        }
      } else if(hasShortTypes){
        const shortPolicies = allPolicies.filter(p => ['ריסק','ריסק משכנתא','מחלות קשות'].includes(safeTrim(p?.type)));
        const allShortMigdal = shortPolicies.length > 0 && shortPolicies.every(p => safeTrim(p?.company) === 'מגדל')
          && !allPolicies.some(p => ['סרטן','אובדן כושר עבודה'].includes(safeTrim(p?.type)));
        const criticalPolicies = allPolicies.filter(p => ['מחלות קשות','בריאות'].includes(safeTrim(p?.type)));
        const allShortMenora = shortPolicies.length > 0 && [...shortPolicies, ...criticalPolicies].every(p => safeTrim(p?.company) === 'מנורה')
          && !allPolicies.some(p => ['סרטן','אובדן כושר עבודה','ריסק','ריסק משכנתא'].includes(safeTrim(p?.type)));
        const allShortHachshara = shortPolicies.length > 0 && shortPolicies.every(p => safeTrim(p?.company) === 'הכשרה')
          && !allPolicies.some(p => ['סרטן','אובדן כושר עבודה','ריסק','ריסק משכנתא'].includes(safeTrim(p?.type)));
        const allShortAyalon = shortPolicies.length > 0 && shortPolicies.every(p => safeTrim(p?.company) === 'איילון')
          && !allPolicies.some(p => ['סרטן','אובדן כושר עבודה','ריסק','ריסק משכנתא'].includes(safeTrim(p?.type)));
        if(allShortMigdal){
          const s = this.getMagdalLifeHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        } else if(allShortMenora){
          const s = this.getMenoraHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        } else if(allShortHachshara){
          const s = this.getHachsharaCriticalHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        } else if(allShortAyalon){
          const s = this.getAyalonCriticalHealthSchema();
          cache.filteredQuestions = s.length ? s : [];
        } else {
          cache.filteredQuestions = this.getShortHealthSchema();
        }
      } else {
    ...
    formatHealthFieldsForOperationalReport(question, fields){
      ...
      return entries.map((row) => row.display).join(' • ');
    },
    ...
    focusHealthDetailCard(qKey, insId, options = {}){
      ...
      const tryFocus = () => {
        ...
        this.setHint(options?.restoreValues ? 'נפתח שאלון ההמשך — ניתן להמשיך לעדכן את הפירוט הקיים' : 'נפתח שאלון ההמשך — יש למלא לפחות שדה אחד ואז לשמור');
        return true;
      };
      if(tryFocus()) return;
      requestAnimationFrame(() => {
    ...
    renderStep8(){
      ...
      // show only insureds that have relevant policies
      ...
      const rows = list.map((item) => {
        ...
        const insuredCols = relevantInsureds.map((ins) => {
          ...
          const no = answer === 'no';
          const saved = !!r.saved;
          const detailValue = safeTrim(r.fields?.details || '');
          return `<div class="lcPhxHealth__insuredCol">
            ...
            <div class="lcPhxHealth__detailWrap ${yes ? 'is-open' : ''}" data-hdetail-wrap="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">
              <label class="lcPhxHealth__detailLabel">פירוט</label>
              <textarea class="input lcPhxHealth__detailInput" rows="4" data-hfield="${escapeHtml(q.key)}|${escapeHtml(ins.id)}|details" placeholder="פירוט">${escapeHtml(detailValue)}</textarea>
              <div class="lcPhxHealth__detailActions">
                <button type="button" class="btn btn--primary" data-hsave="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">שמור</button>
                <button type="button" class="btn" data-hedit="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">עריכה</button>
                <button type="button" class="btn btn--danger" data-hclear="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">נקה</button>
              </div>
              <div class="lcPhxHealth__saved ${saved ? 'is-visible' : ''}" data-hsaved="${escapeHtml(q.key)}|${escapeHtml(ins.id)}">הפירוט נשמר</div>
            </div>
          </div>`;
        }).join('');
      ...
      return `<div class="lcPhxHealth">
        <div class="lcPhxHealth__hero">
          <div>
            ...
            <div class="lcPhxHealth__summary">${escapeHtml(section.catSummary || 'סימון כן מחייב פירוט ושמירה.')}</div>
          </div>
        </div>
    ...
    bindHealthInputs(){
      ...
      $$('[data-hans]', body).forEach((btn) => {
        on(btn, 'click', () => {
          ...
          const prev = this.getHealthResponse(qKey, insId);
          this.setHealthResponse(qKey, insId, { answer:'yes', editing:true, saved: !!safeTrim(prev.fields?.details), fields: { ...(prev.fields || {}) } });
          this.render();
          this.focusHealthDetailCard(qKey, insId, { restoreValues:true });
        });
      });
    ...
    getHealthBlockingIssue(){
      ...
      const allNewPolicies = this.newPolicies || [];
    ...
    getHealthQuestionListFromPayload(payload){
      try{
        ...
        ctx.newPolicies = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const getInsuredHeading = (ins, index) => {
        ...
        const roleLabel = index === 0 ? 'מבוטח ראשי' : 'מבוטח משני';
      ...
      insureds.forEach((ins, index) => {
        ...
        policies.forEach((policy) => {
          ...
          const statusParts = [isAgentAppointment ? 'בוצע מינוי סוכן בפוליסה זו' : '', safeTrim(statusMeta?.reason), cancelExecutionLabel ? `אופן ביצוע הביטול: ${cancelExecutionLabel}` : '', safeTrim(statusMeta?.partialDetails)].filter(Boolean);
          existingRows.push([
            insuredLabel,
            safeTrim(policy?.company) || '—',
            safeTrim(policy?.type || policy?.product) || '—',
      ...
      const newPolicyBlocks = newPolicies.map((policy, index) => {
        ...
        // build per-insured rows
        const perInsuredRows = isMulti ? pIds.map(iid => {
          const ins = (insureds || []).find(x => safeTrim(x?.id) === safeTrim(iid) || safeTrim(x?.data?.id) === safeTrim(iid));
          const insLabel = ins ? (safeTrim(ins?.label) || safeTrim(`${ins?.data?.firstName || ''} ${ins?.data?.lastName || ''}`).trim() || iid) : iid;
          const premium = (policy?.premiumPerInsured && policy.premiumPerInsured[iid]) ? this.formatMoneyValue(this.asMoneyNumber(policy.premiumPerInsured[iid])) : '—';
          // per-insured discount
          const discountData = (policy?.discountPerInsured && policy.discountPerInsured[iid]) || {};
          const discountPct = Number(safeTrim(discountData?.discountPct || policy?.discountPct || '0')) || 0;
          const premiumBase = (policy?.premiumPerInsured && policy.premiumPerInsured[iid]) ? this.asMoneyNumber(policy.premiumPerInsured[iid]) : this.asMoneyNumber(policy?.premiumMonthly);
          const premiumAfter = premiumBase > 0 && discountPct > 0 ? this.formatMoneyValue(Math.round(premiumBase * (1 - discountPct/100) * 100)/100) : premium;
          const sumVal = (policy?.sumInsuredPerInsured && policy.sumInsuredPerInsured[iid]) || (policy?.compensationPerInsured && policy.compensationPerInsured[iid]) || '';
          const sumLabel = (policy?.type === 'מחלות קשות' || policy?.type === 'סרטן') ? 'סכום פיצוי' : 'סכום ביטוח';
          return [
            `— ${insLabel}`,
            `${premium}${discountPct > 0 ? ` ← אחרי הנחה: ${premiumAfter}` : ''}${sumVal ? ` | ${sumLabel}: ${this.formatMoneyValue(this.asMoneyNumber(sumVal))}` : ''}`
          ];
        }) : [];
        ...
        const rows = [
          ...
          ...(!isMulti ? [
            ['פרמיה', safeTrim(policy?.premiumMonthly || policy?.premium) ? this.formatMoneyValue(safeTrim(policy?.premiumMonthly || policy?.premium)) : '—'],
            ['פרמיה אחרי הנחה', this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(policy || {}))],
            ['דירוג הנחה', safeTrim(this.getPolicyDiscountDisplayText(policy || {}))],
            ...(() => {
              const hcd = policy?.healthCoverDiscounts;
              if(!hcd || typeof hcd !== 'object') return [];
              const generalPct = this.getPolicyDiscountPct(policy);
              const lines = Object.entries(hcd).map(([cover, pct]) => {
                const n = Number(pct);
                const effective = (Number.isFinite(n) && n > 0) ? n : generalPct;
                const suffix = (Number.isFinite(n) && n === 0 && generalPct > 0) ? ' (הנחה כללית)' : '';
                return [`↳ ${cover}`, `${effective}%${suffix}`];
              });
              return lines;
            })(),
          ...
          ...perInsuredRows,
          ...(isMulti ? [
            ['סה"כ פרמיה חודשית', this.formatMoneyValue(this.asMoneyNumber(policy?.premiumMonthly))],
            ['דירוג הנחה', safeTrim(this.getPolicyDiscountDisplayText(policy || {}))],
            ['הטבת הצטרפות', safeTrim(this.getPolicyIntroBenefitText(policy || {}))],
          ] : []),
          ['הרחבות לפוליסה', policy?.type === 'ריסק' ? (policy?.umbrellaInsurance ? 'כן' : 'לא') : ''],
          ['פיצוי לנכות מתאונה', policy?.type === 'ריסק' && policy?.umbrellaInsurance ? safeTrim(policy?.umbrellaDisabilityAmount) : ''],
          ['פיצוי למוות מתאונה', policy?.type === 'ריסק' && policy?.umbrellaInsurance ? safeTrim(policy?.umbrellaDeathAmount) : ''],
          ['אופן תשלום', safeTrim(policy?.paymentMethod)]
        ].filter(([,value]) => safeTrim(value));

        const isRiskType = (policy?.type === 'ריסק' || policy?.type === 'ריסק משכנתא');
        const beneficiaryRows = isRiskType && Array.isArray(policy?.beneficiaries) && policy.beneficiaries.length
          ? policy.beneficiaries.map((b, bi) => [
              `מוטב ${bi + 1}`,
              [
                safeTrim(b.firstName) && safeTrim(b.lastName) ? `${safeTrim(b.firstName)} ${safeTrim(b.lastName)}` : (safeTrim(b.firstName) || safeTrim(b.lastName) || ''),
                safeTrim(b.idNumber) ? `ת"ז: ${safeTrim(b.idNumber)}` : '',
                safeTrim(b.birthDate) ? `ת. לידה: ${safeTrim(b.birthDate)}` : '',
                safeTrim(b.phone) ? `טל': ${safeTrim(b.phone)}` : '',
                safeTrim(b.relationship) ? `קרבה: ${safeTrim(b.relationship)}` : '',
                safeTrim(String(b.sharePct || '')) ? `חלק: ${safeTrim(String(b.sharePct))}%` : ''
              ].filter(Boolean).join(' · ')
            ])
          : [];
        ...
        return {
          ...
          subtitle: (isMulti ? `פוליסה משותפת · ` : '') + (safeTrim(policy?.type || policy?.product) || 'פוליסה חדשה'),
          rows,
      ...
      const mirrorSchedulePayload = payload.mirrorSchedule || payload.operational?.mirrorSchedule || {};
      ...
      const healthSections = (insureds || []).map((ins, index) => {
        const insLabel = safeTrim(ins?.label) || `מבוטח ${index + 1}`;
        const insId = safeTrim(ins?.id) || safeTrim(ins?.data?.id) || `payload_ins_${index}`;
        const rows = [];
      ...
      const renderClassicTable = (headers, rows, emptyText='אין נתונים להצגה.') => {
        const safeRows = Array.isArray(rows) ? rows.filter((row) => Array.isArray(row) && row.some((cell) => safeTrim(cell))) : [];
        if(!safeRows.length) return `<div class="lcPdfEmpty">${escapeHtml(emptyText)}</div>`;
        return `<table class="lcPdfTable"><thead><tr>${headers.map((h) => `<th>${escapeHtml(h)}</th>`).join('')}</tr></thead><tbody>${safeRows.map((row) => `<tr>${row.map((cell) => `<td>${escapeHtml(cell || '—')}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
      };
      ...
      const renderHealthSections = (sections) => {
        ...
        if(!safeSections.length) return `<div class="lcPdfEmpty">לא נמצאו תשובות להצגת הצהרת הבריאות.</div>`;
        return safeSections.map((section) => `
          <section class="lcPdfHealthSection">
            <div class="lcPdfHealthSection__title">הצהרת בריאות — ${escapeHtml(section.title)}</div>
            <table class="lcPdfHealthTable">
              <thead>
                <tr>
                  <th class="lcPdfHealthTable__yes">כן</th>
                  <th class="lcPdfHealthTable__no">לא</th>
                  <th>נימוק / פירוט</th>
                  <th>סעיף</th>
                </tr>
              </thead>
              <tbody>
                ${section.rows.map((row) => `
                  <tr>
                    <td class="lcPdfHealthTable__mark">${row.yes ? '✕' : ''}</td>
                    <td class="lcPdfHealthTable__mark">${row.noMark ? '✕' : ''}</td>
                    <td>${escapeHtml(row.details)}</td>
                    <td><span class="lcPdfHealthQNo">${row.no}.</span> ${escapeHtml(row.question)}</td>
                  </tr>`).join('')}
              </tbody>
            </table>
          </section>`).join('');
      ...
      const style = `<style>
        ...
        .lcPdfRoot{direction:rtl;font-family:Arial,"Noto Sans Hebrew","Segoe UI",sans-serif;color:#20324a;background:#eef1f5;padding:20px}
        .lcPdfRoot--preview{padding:0;background:transparent}
        .lcPdfPage{width:720px;min-height:1046px;margin:0 auto 22px;background:#ffffff;padding:26px 28px 24px;box-shadow:0 18px 46px rgba(15,23,42,.12);position:relative}
        .lcPdfPage--health{min-height:1046px}
        .lcPdfRoot--preview .lcPdfPage{width:100%;min-height:auto;padding:24px 22px 26px;box-shadow:none;border:1px solid #e1e7ef;margin:0 0 18px;border-radius:18px}
        ...
        .lcPdfHeader{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding-bottom:14px;border-bottom:1px solid #d8e0ea;margin-bottom:14px}
        .lcPdfHeader__title{font-size:26px;font-weight:800;color:#1b3555;line-height:1.1;margin:0}
        .lcPdfHeader__sub{margin-top:6px;font-size:14px;color:#536983}
        .lcPdfHeader__meta{margin-top:14px;display:grid;gap:4px;font-size:13px;color:#415874}
        .lcPdfHeader__brand{display:flex;align-items:flex-start;justify-content:flex-end}
        .lcPdfHeader__logo{max-width:140px;max-height:68px;object-fit:contain}
        .lcPdfSection{margin-top:16px}
        .lcPdfSection__title{font-size:16px;font-weight:800;color:#1b3555;background:#eef2f7;border:1px solid #dde4ee;border-radius:999px;padding:6px 14px;display:inline-block;margin-bottom:8px}
        .lcPdfInsuredBlock{margin-top:12px;border:1px solid #dbe3ed;border-radius:14px;padding:12px;background:#fff;break-inside:avoid;page-break-inside:avoid}
        .lcPdfInsuredBlock__title{font-size:15px;font-weight:800;color:#1b3555;margin-bottom:8px}
        ...
        .lcPdfInfoTable th,.lcPdfInfoTable td{border:1px solid #dbe3ed;padding:8px 10px;vertical-align:top}
        ...
        .lcPdfTable{width:100%;border-collapse:collapse;font-size:13px}
        .lcPdfTable th,.lcPdfTable td{border:1px solid #dbe3ed;padding:8px 10px;vertical-align:top;text-align:right}
        ...
        .lcPdfPolicyCard{border:1px solid #dbe3ed;border-radius:14px;padding:12px 12px 10px;background:#fff;break-inside:avoid;page-break-inside:avoid}
        ...
        .lcPdfPageFooter{margin-top:18px;padding-top:10px;border-top:1px solid #d8e0ea;display:flex;justify-content:space-between;align-items:center;color:#7a8ca3;font-size:11px}
        .lcPdfPageFooter__brand{font-weight:800;color:#1b3555}
        .lcPdfEmpty{padding:16px 8px;color:#71839a;font-weight:600}
        .lcPdfHealthLead{font-size:14px;color:#4f647c;margin-bottom:10px}
        .lcPdfHealthSection{margin-top:18px}
        .lcPdfHealthSection__title{font-size:24px;font-weight:800;color:#1b3555;text-align:right;margin-bottom:10px}
        .lcPdfHealthTable{width:100%;border-collapse:collapse;font-size:13.5px}
        .lcPdfHealthTable th,.lcPdfHealthTable td{border:1px solid #dbe3ed;padding:10px 10px;vertical-align:top;text-align:right}
      ...
      const pageOneSections = [
        `<section class="lcPdfSection"><div class="lcPdfSection__title">פרטי לקוח</div>${insuredDetailBlocks.length ? insuredDetailBlocks.map((block) => `<div class="lcPdfInsuredBlock"><div class="lcPdfInsuredBlock__title">${escapeHtml(block.heading)}</div>${renderInfoTable(block.rows, 'לא הוזנו פרטי לקוח')}</div>`).join('') : `<div class="lcPdfEmpty">לא הוזנו פרטי לקוח</div>`}</section>`,
        insuredRows.length ? `<section class="lcPdfSection"><div class="lcPdfSection__title">מבוטחים</div>${renderClassicTable(['מבוטח','שם מלא','ת״ז','תאריך לידה','טלפון','קופת חולים','שב״ן'], insuredRows)}</section>` : '',
        bmiRows.length ? `<section class="lcPdfSection"><div class="lcPdfSection__title">גובה, משקל ו־BMI</div>${renderClassicTable(['מבוטח','גובה','משקל','BMI'], bmiRows, 'לא נמצאו נתוני גובה / משקל / BMI')}</section>` : ''
      ].filter(Boolean).join('');

      const pageTwoSections = [
        `<section class="lcPdfSection"><div class="lcPdfSection__title">פוליסות קיימות</div>${renderClassicTable(['מבוטח','חברה','סוג ביטוח','מספר פוליסה','פרמיה','סטטוס','פירוט טיפול'], existingRows, 'לא נוספו פוליסות קיימות')}</section>`,
        `<section class="lcPdfSection"><div class="lcPdfSection__title">פוליסות חדשות</div>${renderPolicyBlocks()}</section>`,
        payerRows.length ? `<section class="lcPdfSection"><div class="lcPdfSection__title">אמצעי תשלום</div>${renderInfoTable(payerRows, 'לא הוזנו פרטי תשלום')}</section>` : '',
        mirrorScheduleRows.length ? `<section class="lcPdfSection"><div class="lcPdfSection__title">תזמון שיחת שיקוף</div>${renderInfoTable(mirrorScheduleRows, 'לא נקבע תזמון שיחת שיקוף')}</section>` : ''
      ].filter(Boolean).join('');
        ...
        html: `${style}<div class="${rootClass}">
          <div class="lcPdfPage">
            <header class="lcPdfHeader">
              <div>
                <h1 class="lcPdfHeader__title">דוח תפעולי</h1>

                <div class="lcPdfHeader__meta">
                  <div>${escapeHtml(exportedAt)}</div>
                  <div>הצעה לביטוח: ${escapeHtml(customerName)}</div>
                  <div>${escapeHtml(currentAgentName)} — סוכן מטפל</div>
                </div>
              </div>
              <div class="lcPdfHeader__brand"><img src="${escapeHtml(logoSrc)}" alt="GEMEL INVEST" class="lcPdfHeader__logo"></div>
            </header>
            <section class="lcPdfSection"><div class="lcPdfSection__title">סיכום הדוח</div>${renderInfoTable(summaryRows, 'אין נתוני סיכום')}</section>
            ${pageOneSections}
            <div class="lcPdfPageFooter"><div class="lcPdfPageFooter__brand">GEMEL INVEST</div><div>עמוד 1</div></div>
          </div>
          <div class="lcPdfPageBreak"></div>
          <div class="lcPdfPage">
            <header class="lcPdfHeader">
              <div>
                <h1 class="lcPdfHeader__title">דוח תפעולי — המשך</h1>
                <div class="lcPdfHeader__sub">פוליסות, אמצעי תשלום ומספרי סוכן.</div>
                <div class="lcPdfHeader__meta">
            ...
            ${pageTwoSections}
            <div class="lcPdfPageFooter"><div class="lcPdfPageFooter__brand">GEMEL INVEST</div><div>עמוד 2</div></div>
          </div>
          ${healthPages.map((pageSections, pageIndex) => `
          <div class="lcPdfPageBreak"></div>
          <div class="lcPdfPage lcPdfPage--health">
            <header class="lcPdfHeader">
              <div>
                <h1 class="lcPdfHeader__title">הצהרת בריאות</h1>
                <div class="lcPdfHeader__sub">עמוד זה מרכז את תשובות הצהרת הבריאות בלבד.</div>
                <div class="lcPdfHeader__meta">
                  <div>${escapeHtml(customerName)}</div>
                  <div>${escapeHtml(exportedAt)}</div>
                </div>
            ...
            <div class="lcPdfHealthLead">התשובות מוצגות בצורה ברורה עם סימון כן / לא ופירוט בעת הצורך.</div>
            ${renderHealthSections(pageSections)}
            <div class="lcPdfPageFooter"><div class="lcPdfPageFooter__brand">GEMEL INVEST</div><div>עמוד ${pageIndex + 3}</div></div>
          </div>`).join('')}
        </div>`
      };
    },
    ...
    async exportOperationalPdfPageByPage(payloadOverride, sourceBtn){
      ...
      try{
        ...
        root.querySelectorAll('.lcPdfPage').forEach((page) => {
          ...
          page.style.minHeight = PDF_CONTENT_MIN_HEIGHT_PX + 'px';
          page.style.margin = '0 auto 8px';
          page.style.padding = '16px 18px 16px';
        ...
        for(let i = 0; i < pages.length; i += 1){
          ...
          const canvas = await html2canvasFn(pageEl, {
            ...
            windowHeight: Math.max(pageEl.scrollHeight || pageEl.offsetHeight || 1123, 1123)
          });
          ...
          const drawY = (pdfPageHeight - drawHeight) / 2;
          if(i > 0) pdf.addPage();
    ...
    async exportOperationalPdf(payloadOverride, sourceBtn){
      ...
      try{
        ...
        const PDF_CONTENT_WIDTH_PX = 770;
        const PDF_CONTENT_MIN_HEIGHT_PX = 1084;
        host = document.createElement('div');
        host.className = 'lcPdfExportHost';
        host.setAttribute('dir','rtl');
        host.setAttribute('aria-hidden', 'true');
    ...
    focusPremiumValidationIssue(issue){
      ...
      window.setTimeout(() => {
        this.refreshVisibleValidation();
        const selector = issue.path ? `[data-bind="${String(issue.path).replace(/"/g, '\"')}"]` : '';
        const field = selector ? this.els.body?.querySelector(selector) : null;
        field?.focus?.();
    ...
    validateStep(stepId){
      if(this.isElementaryFlow()){
        ...
        if(Number(stepId) === 0) return this.elementaryProduct ? { ok:true } : { ok:false, msg:'יש לבחור רכב או דירה' };
        if(Number(stepId) === 1){
          const req = ['firstName','lastName','idNumber','birthDate'];
          return req.every((k) => safeTrim(d[k])) ? { ok:true } : { ok:false, msg:'חסר מילוי חובה בפרטי המבוטח' };
        }
        if(Number(stepId) === 2){
          const rows = Array.isArray(d.elementaryVehiclePolicies) ? d.elementaryVehiclePolicies : [];
          return rows.length ? { ok:true } : { ok:false, msg:'יש לייבא לפחות פוליסת רכב אחת מהר הביטוח' };
        }
        return { ok:true };
      }
    ...
    isStepCompleteForInsured(stepId, ins){
      const d = ins.data || {};
      if(this.isElementaryFlow()){
        if(Number(stepId) === 1) return ['firstName','lastName','idNumber','birthDate'].every((k) => safeTrim(d[k]));
        if(Number(stepId) === 2) return Array.isArray(d.elementaryVehiclePolicies) && d.elementaryVehiclePolicies.length > 0;
        return true;
      }
      if(stepId === 1){
        const baseReq = ["firstName","lastName","idNumber","birthDate","gender"];
        const childReq = baseReq;
        const adultReq = baseReq.concat(["maritalStatus","phone","city","street"]);
        const req = (ins.type === "child") ? childReq : adultReq;

        if(ins.type === "child"){
          const p = this.insureds[0]?.data || {};
          const inh = ["phone","city","street"];
      ...
      if(stepId === 4){
        ...
        this.insureds.forEach((ins) => {
          ...
          policies.forEach((policy) => {
            ...
            const status = safeTrim(c.status);
            if(status === 'full' || status === 'partial_health'){
              if(!safeTrim(c.needsAnalysisReason || '')) missing = true;
            }
          });
        });
        if(missing) return { ok:false, msg:'חובה למלא הסבר על כל פוליסה שנבחרה' };
        return { ok:true };
      }
...
calcAge(dateStr){
  ...
  const OpsEventsUI = {
    ...
    customerOptions(){
      return [].map((rec) => ({  /* ProcessesUI removed */
        id: String(rec.id || ''),
  ...
  const SaveStatusUI = {
    ...
    show(state='loading', title='', text=''){
      ...
      this.els.host.hidden = false;
      window.clearTimeout(this.timer);
      this.els.host.classList.remove('is-success','is-error','is-loading','is-visible');
      this.els.host.classList.add('is-visible', state === 'error' ? 'is-error' : state === 'success' ? 'is-success' : 'is-loading');
    ...
    hide(){
      if(!this.els) this.init();
      if(!this.els?.host) return;
      this.els.host.classList.remove('is-visible','is-success','is-error','is-loading');
      this.els.host.hidden = true;
    }
  };
  ...
  const App = {
    ...
    applyStateSnapshot(payload, options = {}){
      const state = normalizeState(payload || defaultState());
      State.data = state;
      refreshStateShadows();
      try { Storage.saveBackup(state); } catch(_e) {}
      ...
      if (Auth.isOps()) { try { void 0 /* ProcessesUI removed */; } catch(_e) {} try { OpsEventsUI.renderToolbarState(); OpsEventsUI.checkReminders(); } catch(_e) {} }
      if (Auth.current) {
        try { ChatUI.onLogin(); } catch(_e) {}
    ...
    applyLoadResult(result, statusLabel = 'מחובר'){
      if(!result?.ok || !result.payload) return result;
      this.applyStateSnapshot(result.payload);
      if(!result.stale){
    ...
    warmViewCache(){
      try {
        window.setTimeout(() => { try { CustomersUI.render(); } catch(_e) {} }, 60);
        window.setTimeout(() => { try { ProposalsUI.render(); } catch(_e) {} }, 120);
        window.setTimeout(() => { try { DashboardUI.render(); } catch(_e) {} }, 180);
      } catch(_e) {}
    },
    ...
    async persist(label){
      ...
      if (r.ok) {
        if(r.mergedState) State.data = normalizeState(r.mergedState);
    ...
    async reloadSessionState(){
      ...
      const r = await Storage.loadSheets({ useCachedFallback:false });
      if (r.ok) {
        this.applyLoadResult(r, 'נתוני משתמש נטענו');
      } else {
  ...
  const getAgentMonthlyTarget = (agentId, fallbackAgent = null) => {
    ...
    return normalizeAgentTargetValue(fallbackAgent?.monthlySalesTarget);
  };
  ...
  const SupabaseMFA = {
    ...
    extractQrMarkup(data){
      ...
      const secret = safeTrim(data?.totp?.secret || data?.secret || '');
      const uri = safeTrim(data?.totp?.uri || data?.uri || '');
      let html='';
      if(qr){
        const src = qr.startsWith('data:') ? qr : 'data:image/svg+xml;utf8,' + encodeURIComponent(qr);
        html += `<div class="lcLogin__mfaQrMedia"><img alt="QR Google Authenticator" src="${src}"/></div>`;
      }
      if(secret || uri){
        html += `<div class="lcLogin__mfaManualKey">`;
        html += `<div class="lcLogin__mfaManualLabel">קוד ידני לחיבור</div>`;
        if(secret) html += `<code dir="ltr">${escapeHtml(secret)}</code>`;
        else html += `<code dir="ltr">${escapeHtml(uri)}</code>`;
        html += `<span class="lcLogin__mfaManualHint">ב־Google Authenticator עדיף לבחור הזנה ידנית ולהדביק את הקוד הידני אם הסריקה לא נתפסת.</span>`;
        html += `</div>`;
      }
  ...
  const completeAgentLogin = async (matched, options = {}) => {
    const loaderMs = Math.max(1200, Number(options?.loaderMs) || 1200);
    Auth.current = { name: matched.name, role: (isOwnerIdentity(matched) ? 'owner' : matched.role === 'manager' ? 'manager' : matched.role === 'ops' ? 'ops' : 'agent') };
    ...
    try {
      ...
      UI.applyRoleUI();
      UI.renderAuthPill();
      await new Promise((resolve) => window.setTimeout(resolve, loaderMs));
      BirthdaysUI.maybeCelebrateLogin();
      UI.goView('dashboard');
      try { ChatUI.onLogin(); } catch(_e) {}
    } finally {
  ...
  UsersUI._saveFromModal = async function(){
    ...
    const monthlySalesTarget = normalizeAgentTargetValue(E.monthlyTarget?.value);
    if(this._modalMode === 'edit' && safeTrim(E.id?.value)){
      setAgentSecurity(safeTrim(E.id.value), { authEmail, mfaRequired: !!authEmail });
      setAgentMonthlyTarget(safeTrim(E.id.value), monthlySalesTarget);
    }
    ...
    if(this._modalMode !== 'edit'){
      ...
      if(last?.id) {
        setAgentSecurity(last.id, { authEmail, mfaRequired: !!authEmail });
        setAgentMonthlyTarget(last.id, monthlySalesTarget);
        await App.persist('עודכנו פרטי משתמש');
      }
    } else {
      await App.persist('עודכנו פרטי משתמש');
    }
  ...
  Auth._setMfaModeUi = function(mode, qrHtml=''){
    ...
    const { step, title, setupBox, qrBox, verifyBtn } = this._getMfaElements();
  ...
  Auth._verifyPendingMfa = async function(options = {}){
    ...
    try {
      ...
      await new Promise((resolve) => window.setTimeout(resolve, 220));
      await completeAgentLogin(pending.agent, { loaderMs:7000 });
  ...
  const SecurityUI = {
    ...
    open(agentId=''){
      ...
      this.els.wrap?.classList.add('is-open');
      this.els.wrap?.setAttribute('aria-hidden','false');
      window.setTimeout(() => { try { this.els.password?.focus({ preventScroll:true }); } catch(_e) {} }, 80);
    },
    close(){ this.els.wrap?.classList.remove('is-open'); this.els.wrap?.setAttribute('aria-hidden','true'); this.setError(''); this.targetAgentId=''; },
...
(function(){
  ...
  function getAcknowledgedVersion(){
    try {
      const raw = localStorage.getItem(GI_VC_ACKNOWLEDGED_KEY);
      if(!raw) return '';
      const parsed = JSON.parse(raw);
      // תוקף של 30 שניות בלבד — מספיק לחסום את הקפיצה אחרי reload
      if(Date.now() - (parsed.ts || 0) > 30000) { localStorage.removeItem(GI_VC_ACKNOWLEDGED_KEY); return ''; }
      return parsed.ver || '';
    } catch(_e){ return ''; }
  }
  function setAcknowledgedVersion(ver){
    try { localStorage.setItem(GI_VC_ACKNOWLEDGED_KEY, JSON.stringify({ ver, ts: Date.now() })); } catch(_e){}
  }
  ...
  async function pollVersionFromSupabase(){
    try{
      ...
      const url = (typeof SUPABASE_URL !== 'undefined' ? SUPABASE_URL : '') + '/rest/v1/app_meta?key=eq.' + GI_VC_META_KEY + '&select=payload&limit=1';
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // נקה תמיד בטעינה — ה-acknowledged משמש רק לחסום את הקפיצה המיידית אחרי לחיצת "עדכן"
    // ברגע שהדף נטען מחדש בהצלחה, אין סיבה לחסום יותר
    ...
    if(dismissBtn){
      dismissBtn.addEventListener('click', () => {
        // dismiss — רק מסתיר לסשן הנוכחי, לא שומר בlocalStorage
        // בפעם הבאה שיש גרסה חדשה (אחרי reload ידני) — יקפוץ שוב
    ...
    setTimeout(() => {
      startRealtimeVersionWatch();
      // polling כגיבוי — כל 60 שניות, למקרה שה-Realtime לא עובד
      pollVersionFromSupabase();
      setInterval(() => pollVersionFromSupabase(), 60000);
    }, 3000);
  });
})();
