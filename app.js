...
(() => {
  ...
  const BUILD = "20260322-investo-local-url-bundle-1";
...
const SystemRepairUI = {
  ...
  const LeadShellUI = {
    INVESTO_URL: "./investo-assistant/index.html",
    els: {},
    loaded: false,
    ...
    getInvestoUrl(){
      const fromWindow = safeTrim(window.INVESTO_ASSISTANT_URL);
      const fromStorage = safeTrim(localStorage.getItem("INVESTO_ASSISTANT_URL") || "");
      return fromWindow || fromStorage || safeTrim(this.INVESTO_URL);
    },
    buildFallbackSrcDoc(){
      const who = safeTrim(Auth?.current?.name) || "נציג";
      return `<!DOCTYPE html><html lang="he" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>INVESTO</title><style>body{margin:0;font-family:system-ui,-apple-system,Segoe UI,Arial,sans-serif;background:linear-gradient(180deg,#f5f8ff,#fffaf2);color:#0f172a;display:grid;place-items:center;min-height:100vh;padding:24px;box-sizing:border-box}.card{width:min(760px,100%);background:rgba(255,255,255,.92);border:1px solid rgba(15,23,42,.10);border-radius:28px;box-shadow:0 20px 50px rgba(15,23,42,.12);padding:28px}.kicker{font-size:12px;font-weight:800;color:#2b4b8a}.title{font-size:28px;font-weight:900;margin:6px 0 12px}.text{font-size:16px;line-height:1.7;color:rgba(15,23,42,.82)}.chip{display:inline-flex;margin-top:18px;padding:8px 12px;border-radius:999px;background:rgba(217,234,255,.45);border:1px solid rgba(43,75,138,.12);font-size:12px;font-weight:800;color:#17306f}.box{margin-top:18px;padding:14px 16px;border-radius:18px;background:rgba(245,248,255,.92);border:1px solid rgba(15,23,42,.08)}code{direction:ltr;unicode-bidi:plaintext;display:block;margin-top:8px;padding:10px 12px;border-radius:14px;background:#fff;border:1px solid rgba(15,23,42,.08);font-size:13px;overflow:auto}</style></head><body><div class="card"><div class="kicker">GEMEL INVEST</div><div class="title">INVESTO העוזר החכם</div><div class="text">שלום ${escapeHtml(who)}. החלון החכם מוכן במערכת, אבל עדיין לא הוגדרה כתובת הריפו של INVESTO. ברגע שתעדכן את הכתובת, החלון הזה יטען את הצ'אט החדש אוטומטית.</div><div class="chip">מוכן לחיבור חיצוני</div><div class="box"><div class="text">כדי לחבר את הריפו החדש, צריך להגדיר URL ב־<strong>window.INVESTO_ASSISTANT_URL</strong> או ב־localStorage תחת המפתח:</div><code>INVESTO_ASSISTANT_URL</code></div></div></body></html>`;
    },
    buildFrameUrl(){
      const baseUrl = this.getInvestoUrl();
      if(!baseUrl) return "";
      try{
        const url = new URL(baseUrl, window.location.href);
        const agentName = safeTrim(Auth?.current?.name);
        const agentRole = safeTrim(Auth?.current?.role);
        if(agentName) url.searchParams.set("agentName", agentName);
        if(agentRole) url.searchParams.set("agentRole", agentRole);
        return url.toString();
      }catch(_e){
        return baseUrl;
      }
    ...
    ensureFrameLoaded(){
      ...
      const targetUrl = this.buildFrameUrl();
      this.loaded = false;
      if(this.els.placeholder) this.els.placeholder.style.display = "flex";
      if(targetUrl){
        this.fallbackActive = false;
        try{ this.els.frame.removeAttribute("srcdoc"); }catch(_e){}
        if(this.els.frame.getAttribute("src") !== targetUrl){
          this.els.frame.setAttribute("src", targetUrl);
        }
        return;
      }
      this.fallbackActive = true;
      this.els.frame.setAttribute("src", "about:blank");
      this.els.frame.setAttribute("srcdoc", this.buildFallbackSrcDoc());
    },
    open(){
