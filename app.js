...
(() => {
  ...
  const BUILD = "20260409-vUmbrellaRisk";
...
// open health covers drawer (Health only)
    ...
    isUmbrellaRiskPolicy(policy){
      const type = safeTrim(policy?.type || policy?.product || "");
      if(!(type === "ריסק" || type === "ריסק משכנתא")) return false;
      return !!policy?.umbrellaEnabled;
    },

    getUmbrellaRiskRows(policy){
      if(!this.isUmbrellaRiskPolicy(policy)) return [];
      return [
        ['מטריה ביטוחית', 'כן'],
        ['נכות מתאונה', safeTrim(policy?.accidentDisabilitySum)],
        ['מוות מתאונה', safeTrim(policy?.accidentDeathSum)]
      ].filter(([, value]) => safeTrim(value));
    },

    renderUmbrellaRiskNotes(policy){
      const rows = this.getUmbrellaRiskRows(policy);
      if(!rows.length) return '';
      return rows.map(([label, value]) => `<div class="lcOpPolicyRow__note"><b>${escapeHtml(label)}:</b> ${escapeHtml(label === 'מטריה ביטוחית' ? value : this.formatMoneyValue(value))}</div>`).join('');
    },
    ...
    addDraftPolicy(){
      ...
      const p = {
        ...
        umbrellaEnabled: !!d.umbrellaEnabled,
        accidentDisabilitySum: (d.accidentDisabilitySum || ""),
        accidentDeathSum: (d.accidentDeathSum || ""),
        pledge: !!d.pledge,
      ...
      this.policyDraft.umbrellaEnabled = false;
      this.policyDraft.accidentDisabilitySum = "";
      this.policyDraft.accidentDeathSum = "";
    ...
    startEditNewPolicy(pid){
      ...
      this.policyDraft = {
        ...
        umbrellaEnabled: !!p.umbrellaEnabled,
        accidentDisabilitySum: p.accidentDisabilitySum || "",
        accidentDeathSum: p.accidentDeathSum || "",
        pledge: !!p.pledge,
    ...
    renderStep5(){
      ...
      const renderPolicyCard = (p, showCoupleBadge=false) => {
        ...
        const chips = [
          ...
          this.isUmbrellaRiskPolicy(p) ? `<span class="lcPolInfoChip lcPolInfoChip--success"><span class="lcPolInfoChip__icon">☂️</span><span class="lcPolInfoChip__text"><b>מטריה ביטוחית</b><small>נכות: ${escapeHtml(fmtMoney(p.accidentDisabilitySum))} · מוות: ${escapeHtml(fmtMoney(p.accidentDeathSum))}</small></span></span>` : '',
        ...
        <div class="lcPolForm lcPolForm--premium lcPolForm--reference">
          <div class="lcPolBuilderCard lcPolBuilderCard--reference">
            ...
            <section class="lcPolReferenceBlock lcPolReferenceBlock--details lcPolReferenceBlock--detailsClean">
              ...
              ${isRisk ? `<div class="lcWSection lcPledgeBox lcPledgeBox--reference">
                <div class="lcWTitle">מטריה ביטוחית</div>
                <div class="lcField">
                  <label class="lcCheckRow lcCheckRow--reference">
                    <input type="checkbox" data-pdraft="umbrellaEnabled" ${d.umbrellaEnabled ? "checked":""} />
                    <span>כולל מטריה ביטוחית</span>
                  </label>
                  <div class="help small muted">אופציונלי. מאפשר להזין גם סכום פיצוי לנכות מתאונה וגם סכום פיצוי למוות מתאונה.</div>
                </div>
                ${d.umbrellaEnabled ? `<div class="lcGrid2">
                  <div class="lcField">
                    <label class="lcLabel">סכום פיצוי לנכות מתאונה</label>
                    <input class="lcInput" data-pdraft="accidentDisabilitySum" value="${escapeHtml(d.accidentDisabilitySum || "")}" inputmode="numeric" placeholder="למשל 200000" />
                  </div>
                  <div class="lcField">
                    <label class="lcLabel">סכום פיצוי למוות מתאונה</label>
                    <input class="lcInput" data-pdraft="accidentDeathSum" value="${escapeHtml(d.accidentDeathSum || "")}" inputmode="numeric" placeholder="למשל 150000" />
                  </div>
                </div>` : ``}
              </div>` : ``}
      ...
      setTimeout(() => {
        ...
        $$("[data-pdraft]", this.els.body).forEach(el => {
          on(el, "input", () => {
            ...
            if(k === "type" && !(this.policyDraft[k] === "ריסק" || this.policyDraft[k] === "ריסק משכנתא")){
              this.policyDraft.umbrellaEnabled = false;
              this.policyDraft.accidentDisabilitySum = "";
              this.policyDraft.accidentDeathSum = "";
            }
            // Re-render only when the change affects visible structure
            if(k === "type" || k === "pledge" || k === "insuredId" || k === "company" || k === "umbrellaEnabled") this.render();
          });
          on(el, "change", () => {
            this.ensurePolicyDraft();
            const k = el.getAttribute("data-pdraft");
    ...
    validateDraftPolicy(){
      ...
      if(!isMedicare && (d.type === "ריסק" || d.type === "ריסק משכנתא")){
        ...
        if(d.umbrellaEnabled){
          if(!safeTrim(d.accidentDisabilitySum)) return { ok:false, msg:"במטריה ביטוחית חובה למלא סכום פיצוי לנכות מתאונה" };
          if(!safeTrim(d.accidentDeathSum)) return { ok:false, msg:"במטריה ביטוחית חובה למלא סכום פיצוי למוות מתאונה" };
        }
      }
      ...
      const renderPolicyRows = (list=[], kind='existing') => {
        ...
        return `<div class="lcOpPolicyGroups">` + groups.map((group, groupIdx) => {
          const rowsHtml = group.items.map((policy, idx) => {
            ...
            const umbrellaNotes = this.renderUmbrellaRiskNotes(policy);
            return `<article class="lcOpPolicyRow lcOpPolicyRow--new" style="animation-delay:${(groupIdx * 60) + (idx * 45)}ms">
              ...
              ${umbrellaNotes ? `<div class="lcOpPolicyRow__notes">${umbrellaNotes}</div>` : ``}
            </article>`;
          }).join('');
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const newPolicyBlocks = newPolicies.map((policy, index) => ({
        ...
        rows: [
          ...
          ...this.getUmbrellaRiskRows(policy).map(([label, value]) => [label, label === 'מטריה ביטוחית' ? value : this.formatMoneyValue(value)]),
