...
(() => {
  ...
  const BUILD = "20260410-vFix-UsersGate-Real";
  ...
  function syncDetachedTopbarVisibility(){
    try{
      const body = document.body;
      if(!body) return;
      const hasWizard = !!(body.classList.contains("lcWizardOpen") || document.querySelector("#lcWizard.is-open"));
      const hasCustomerFile = !!(body.classList.contains("lcCustomerFileOpen") || document.querySelector("#customerFull.is-open"));
      const hasNewCustomerType = !!(body.classList.contains("lcNewCustomerTypeOpen") || document.querySelector("#lcNewCustomerTypeModal.is-open"));
      const shouldHide = hasWizard || hasCustomerFile || hasNewCustomerType;
      body.classList.toggle("lcDetachedTopbarHidden", shouldHide);
      if(!hasWizard) body.classList.remove("lcWizardOpen");
      if(!hasCustomerFile) body.classList.remove("lcCustomerFileOpen");
      if(!hasNewCustomerType) body.classList.remove("lcNewCustomerTypeOpen");
    }catch(_e){}
  }
...
UsersGateUI.init();
  ...
  function openUsersManagementFlow(){
    ...
    try {
      ...
      console.error('USERS_GATE_OPEN_FAILED', _e);
      window.alert('לא הצלחתי לפתוח את חלון הקוד המורשה');
      return false;
    }
  }
  ...
  const UsersGateUI = {
    ...
    els: null,
    _bound: false,
    _rootId: 'usersGateModal',
    _createMarkup(){
      const existing = document.getElementById(this._rootId);
      if(existing) existing.remove();
      const root = document.createElement('div');
      root.id = this._rootId;
      root.className = 'modal usersGateModal';
      root.setAttribute('aria-hidden', 'true');
      root.innerHTML = `
        <div class="modal__backdrop" data-close="1"></div>
        <div aria-label="קוד מורשה לניהול משתמשים" aria-modal="true" class="modal__panel usersGateModal__panel" role="dialog">
          <div class="modal__head">
            <div>
              <div class="modal__kicker">GEMEL INVEST</div>
              <div class="modal__title">יש להקיש סיסמת מורשה</div>
            </div>
            <button aria-label="סגור" class="iconBtn" id="usersGateModalClose" type="button">✕</button>
          </div>
          <div class="modal__body">
            <div class="formGrid usersGateModal__grid">
              <div class="field" style="grid-column:1 / -1;">
                <label class="label" for="usersGateCode">קוד מורשה</label>
                <input class="input" id="usersGateCode" inputmode="numeric" maxlength="6" type="password" placeholder="הקש קוד מורשה" />
                <div class="help">פתיחת חלון ניהול משתמשים דורשת הרשאה נוספת.</div>
              </div>
              <div aria-live="polite" class="lcLogin__error usersGateModal__error" id="usersGateError"></div>
            </div>
          </div>
          <div class="modal__foot">
            <button class="btn" id="usersGateModalCancel" type="button">ביטול</button>
            <button class="btn btn--primary" id="usersGateModalConfirm" type="button">אישור</button>
          </div>
        </div>
      `;
      document.body.appendChild(root);
    ...
    init(force = false){
      ...
      let wrap = document.getElementById(this._rootId);
      if(!wrap || !wrap.querySelector('#usersGateCode') || !wrap.querySelector('#usersGateModalConfirm')){
        wrap = this._createMarkup();
      }
      this.els = {
        wrap,
        close: $('#usersGateModalClose', wrap),
        cancel: $('#usersGateModalCancel', wrap),
        confirm: $('#usersGateModalConfirm', wrap),
        code: $('#usersGateCode', wrap),
        error: $('#usersGateError', wrap)
      };
      if(!this.els.wrap || !this.els.code || !this.els.confirm) return false;
      ...
      on(this.els.code, 'input', () => {
        if(this.els.code) this.els.code.value = String(this.els.code.value || '').replace(/\D+/g,'').slice(0,6);
        if(this.els.error) this.els.error.textContent = '';
      });
      on(this.els.code, 'keydown', (ev) => {
    ...
    ensureReady(){
      if(this.els?.wrap && this._bound && this.els.code && this.els.confirm) return true;
    ...
    open(afterSuccess = null){
      ...
      try {
        document.body.classList.remove('lcDetachedTopbarHidden');
        this.els.wrap.style.display = 'block';
      } catch(_e) {}
      this.els.wrap?.classList.add('is-open');
      this.els.wrap?.setAttribute('aria-hidden','false');
      window.setTimeout(() => this.els.code?.focus?.(), 40);
    },
    close(){
      if(!this.ensureReady()) return;
      this.els.wrap?.classList.remove('is-open');
      this.els.wrap?.setAttribute('aria-hidden','true');
      this.els.wrap && (this.els.wrap.style.display = '');
      if(this.els.error) this.els.error.textContent = '';
      if(this.els.code) this.els.code.value = '';
  ...
  const CustomersUI = {
    ...
    openById(id, opts={}){
      ...
      try{
        ...
        this.els.wrap.classList.add("is-open");
        this.els.wrap.setAttribute("aria-hidden","false");
        document.body.classList.add("lcCustomerFileOpen");
        syncDetachedTopbarVisibility();
        document.body.style.overflow = "hidden";
        if(this.els.body){
    ...
    close(){
      ...
      this.els.wrap.classList.remove("is-open");
      this.els.wrap.setAttribute("aria-hidden","true");
      document.body.classList.remove("lcCustomerFileOpen");
      syncDetachedTopbarVisibility();
...
init(){
    ...
    open(){
      ...
      this.isOpen = true;
      document.body.classList.add("lcWizardOpen");
      syncDetachedTopbarVisibility();
      this.els.wrap.classList.add("is-open");
      ...
      document.body.style.overflow = "hidden";
      document.body.classList.add("modal-open", "lcNewCustomerTypeOpen");
      syncDetachedTopbarVisibility();
      this.render();
    ...
    close(){
      this.isOpen = false;
      document.body.classList.remove("lcWizardOpen", "lcNewCustomerTypeOpen");
      syncDetachedTopbarVisibility();
      this.els.wrap.classList.remove("is-open");
      this.els.wrap.setAttribute("aria-hidden","true");
      try { ChatUI?.syncVisibility?.('wizard'); } catch(_e) {}
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      insureds.forEach((ins, index) => {
        ...
        policies.forEach((policy) => {
          ...
          const statusParts = [safeTrim(statusMeta?.reason), safeTrim(statusMeta?.partialDetails)].filter(Boolean);
          existingRows.push([
            insuredLabel,
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const NewCustomerEntryUI = {
    ...
    close(){
      ...
      this.els.modal.setAttribute("aria-hidden", "true");
      const hasOtherOpenModal = !!document.querySelector('.modal.is-open, .drawer.is-open, .lcWizard.is-open');
      document.body.classList.remove("lcNewCustomerTypeOpen");
      syncDetachedTopbarVisibility();
