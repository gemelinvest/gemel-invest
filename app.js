...
txt.textContent = "";
  ...
  const Wizard = {
    ...
    // כיסויי בריאות (שלב 3 + 5)
    healthCovers: [
      { id: 'surg_il', label: 'ניתוחים בארץ' },
      { id: 'surg_abroad', label: 'ניתוחים בחו"ל' },
      { id: 'transplants', label: 'השתלות' },
      { id: 'drugs', label: 'תרופות מחוץ לסל' },
      { id: 'ambulatory', label: 'אמבולטורי' }
    ],
...
if(path === "birthDate"){
  ...
  return;
      ...
      const rows = (d.existingPolicies || []).map(p => {
        ...
        return `
          <tr>
            ...
            <td>
              <div class="lcTypeCell">
                <select class="input" data-bind="existingPolicies.${p.id}.type">
                  <option value="">בחר…</option>${typeOpts}
                </select>
                ${p.type === "בריאות" ? this.renderHealthCoversControl(`existingPolicies.${p.id}`, p) : ``}
              </div>
            </td>
    ...
    resolvePolicyBind(ins, path, value, kind){
      ...
      if(!row) return false;
      if(field.indexOf('.') !== -1){
        try { this.setPath(row, field, value); } catch(e){ row[field] = value; }
      } else {
        row[field] = value;
      }
      return true;
      ...
      $$("[data-bind]", this.els.body).forEach(el => {
        ...
        const setVal = (doRender=false) => {
          ...
          if(path.startsWith("existingPolicies.") || path.startsWith("newPolicies.")){
            ...
            if(ok){
              if(path.includes(".healthCovers.")){
                const base = path.split(".healthCovers.")[0];
                const partsB = base.split(".");
                const listNameB = partsB[0];
                const pidB = partsB[1];
                const listB = (listNameB === "existingPolicies") ? (ins.data.existingPolicies||[]) : (ins.data.newPolicies||[]);
                const rowB = listB.find(x => x.id === pidB) || {};
                this.updateCoversSummary(base, rowB);
                this.setHint("");
                return;
              }
    ...
    // ---------- Health covers (בריאות) ----------
    _ensureHealthCoversObj(obj){
      if(!obj) return;
      if(!obj.healthCovers || typeof obj.healthCovers !== 'object') obj.healthCovers = {};
    },

    getHealthCoverSummary(obj, maxItems=2){
      const hc = (obj && obj.healthCovers) ? obj.healthCovers : {};
      const picked = (this.healthCovers || []).filter(c => hc && hc[c.id]).map(c => c.label);
      if(!picked.length) return { text:'בחר כיסויים', count:0, labels:[] };
      if(picked.length <= maxItems) return { text: picked.join(' · '), count:picked.length, labels:picked };
      const head = picked.slice(0, maxItems).join(' · ');
      return { text: head + ' +' + (picked.length-maxItems), count:picked.length, labels:picked };
    },

    renderHealthCoversControl(bindBase, obj){
      this._ensureHealthCoversObj(obj);
      const sum = this.getHealthCoverSummary(obj);
      const summaryCls = 'lcCoversSummary' + (sum.count ? '' : ' is-empty');
      const items = (this.healthCovers || []).map(c => {
        const checked = obj.healthCovers && obj.healthCovers[c.id];
        return `
          <label class="lcCoverItem">
            <input type="checkbox" data-bind="${bindBase}.healthCovers.${c.id}" ${checked ? 'checked' : ''} />
            <span>${escapeHtml(c.label)}</span>
          </label>`;
      }).join('');

      return `
        <div class="lcCoversWrap" data-covers-wrap="${bindBase}">
          <button class="btn lcSmallBtn lcCoversBtn" type="button" data-covers-btn="${bindBase}">כיסויים ▾</button>
          <div class="${summaryCls}" title="${escapeHtml((sum.labels||[]).join(', '))}" data-covers-summary="${bindBase}">${escapeHtml(sum.text)}</div>
          <div class="lcCoversPop" data-covers-pop="${bindBase}" aria-hidden="true">
            <div class="lcCoversPop__title">כיסויים (חובה לבחור לפחות אחד)</div>
            <div class="lcCoversGrid">${items}</div>
            <div class="lcCoversPop__foot">
              <button class="btn lcSmallBtn" type="button" data-covers-close="1">סגור</button>
            </div>
          </div>
        </div>`;
    },

    updateCoversSummary(bindBase, obj){
      const el = this.els.body && this.els.body.querySelector(`[data-covers-summary="${bindBase}"]`);
      if(!el) return;
      const sum = this.getHealthCoverSummary(obj);
      el.textContent = sum.text;
      el.classList.toggle('is-empty', !sum.count);
      el.setAttribute('title', (sum.labels||[]).join(', '));
    },

    bindCoversUI(){
      const closeAll = () => {
        $$(".lcCoversWrap.is-open", this.els.body).forEach(w => {
          w.classList.remove('is-open');
          const pop = w.querySelector('.lcCoversPop');
          if(pop) pop.setAttribute('aria-hidden','true');
        });
      };

      if(!this._coversOutsideBound){
        this._coversOutsideBound = true;
        on(this.els.body, 'click', (e) => {
          const t = e.target;
          if(!t) return;
          const inWrap = t.closest && t.closest('.lcCoversWrap');
          if(!inWrap) closeAll();
        });
      }

      $$('[data-covers-btn]', this.els.body).forEach(btn => {
        on(btn, 'click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          const wrap = btn.closest('.lcCoversWrap');
          if(!wrap) return;
          const isOpen = wrap.classList.contains('is-open');
          closeAll();
          if(!isOpen){
            wrap.classList.add('is-open');
            const pop = wrap.querySelector('.lcCoversPop');
            if(pop) pop.setAttribute('aria-hidden','false');
          }
        });
      });

      $$('[data-covers-close="1"]', this.els.body).forEach(btn => {
        on(btn, 'click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          closeAll();
      ...
      $$('.lcCoversPop', this.els.body).forEach(pop => on(pop, 'click', (e) => e.stopPropagation()));
    },
    ...
    addDraftPolicy(){
      ...
      const p = {
        ...
        healthCovers: Object.assign({}, d.healthCovers || {})
      };
// push + reset draft (keep insured selection)
    ...
    validateStep5(){
      ...
      const bad = list.filter(p => {
        ...
        if(p.type === "בריאות"){
          const hc = (p.healthCovers || {});
          const any = Object.keys(hc).some(k => hc[k]);
          if(!any) return true;
        }
    ...
    renderStep5(){
      ...
      const form = `<div class="lcWSection">
        ...
        <div class="lcPolForm">
          <div class="lcField">
            ...
            ${(!spouse) ? `<div class="muted small" style="margin-top:6px">כדי ליצור פוליסה זוגית יש להוסיף בן/בת זוג בשלב 1.</div>` : ``}
          </div>
          ...
          ${(d.type === "בריאות") ? `
          <div class="lcField">
            <label class="lcLabel">כיסויים</label>
            <div class="lcCoversRow" data-pdraft-covers-row="1">
              <button type="button" class="lcBtn lcBtn--ghost lcCoversBtn" data-pdraft-covers-btn="1">בחר כיסויים ▾</button>
              <div class="lcCoversSummary ${this.getHealthCoverSummary(d).count ? '' : 'is-empty'}" data-pdraft-covers-sum="1" title="${escapeHtml((this.getHealthCoverSummary(d).labels||[]).join(', '))}">${escapeHtml(this.getHealthCoverSummary(d).text)}</div>
              <div class="lcCoversPop" data-pdraft-covers-pop="1" aria-hidden="true">
                <div class="lcCoversPop__title">כיסויים (חובה לבחור לפחות אחד)</div>
                <div class="lcCoversGrid">
                  ${(this.healthCovers||[]).map(c=>`<label class=\"lcCoverItem\"><input type=\"checkbox\" data-pdraft-cover=\"${c.id}\" ${(d.healthCovers&&d.healthCovers[c.id])?'checked':''} /><span>${escapeHtml(c.label)}</span></label>`).join('')}
                </div>
                <div class="lcCoversPop__foot"><button class="btn lcSmallBtn" type="button" data-pdraft-covers-close="1">סגור</button></div>
              </div>
            </div>
          </div>` : ``}
      ...
      const res = `<div class="lcWSection">
        <div class="muted small">חובה להוסיף לפחות פוליסה אחת. הפוליסות יופיעו לפי המבוטחים.</div>
      ...
      setTimeout(() => {
        ...
        // health covers (draft)
        const coversRow = this.els.body.querySelector('[data-pdraft-covers-row="1"]');
        const closeDraftCovers = () => {
          if(!coversRow) return;
          coversRow.classList.remove('is-open');
          const pop = coversRow.querySelector('[data-pdraft-covers-pop="1"]');
          if(pop) pop.setAttribute('aria-hidden','true');
        };

        const openDraftBtn = this.els.body.querySelector('[data-pdraft-covers-btn="1"]');
        if(openDraftBtn){
          on(openDraftBtn, 'click', (e) => {
            e.preventDefault();
            e.stopPropagation();
            if(!coversRow) return;
            const isOpen = coversRow.classList.contains('is-open');
            closeDraftCovers();
            if(!isOpen){
              coversRow.classList.add('is-open');
              const pop = coversRow.querySelector('[data-pdraft-covers-pop="1"]');
              if(pop) pop.setAttribute('aria-hidden','false');
            }
          });
        ...
        const closeDraftBtn = this.els.body.querySelector('[data-pdraft-covers-close="1"]');
        if(closeDraftBtn){
          on(closeDraftBtn, 'click', (e) => { e.preventDefault(); e.stopPropagation(); closeDraftCovers(); });
        }

        $$('[data-pdraft-cover]', this.els.body).forEach(chk => {
          on(chk, 'change', () => {
            this.ensurePolicyDraft();
            const id = chk.getAttribute('data-pdraft-cover');
            if(!id) return;
            if(!this.policyDraft.healthCovers) this.policyDraft.healthCovers = {};
            this.policyDraft.healthCovers[id] = !!chk.checked;
            const sumEl = this.els.body.querySelector('[data-pdraft-covers-sum="1"]');
            if(sumEl){
              const sum = this.getHealthCoverSummary(this.policyDraft);
              sumEl.textContent = sum.text;
              sumEl.classList.toggle('is-empty', !sum.count);
              sumEl.setAttribute('title', (sum.labels||[]).join(', '));
            }
          });
        });
        ...
        on(this.els.body, 'click', (e) => {
          const t = e.target;
          if(t && t.closest && t.closest('.lcCoversRow')) return;
          closeDraftCovers();
        });
        $$('.lcCoversPop', this.els.body).forEach(pop => on(pop, 'click', (e)=>e.stopPropagation()));
