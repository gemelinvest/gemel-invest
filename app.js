...
(() => {
  ...
  const Wizard = {
    els: {},
    state: { step: 1, insured: [], mainMedical: { heightCm: "", weightKg: "" } },
    ...
    init(){
      ...
      this.els.btnNext = $("#btnStepNext") || $("#btnWizardNext");
      this.els.btnBack = $("#btnStepBack") || $("#btnWizardBack");
      this.els.btnFinish = $("#btnStepFinish") || $("#btnWizardFinish");
      ...
      on(this.els.btnBack, "click", () => this.prev());
      ...
      $$(".lcPickBtn", this.els.picker).forEach(btn => {
        on(btn, "click", () => {
          const t = btn.getAttribute("data-pick") || btn.getAttribute("data-type");
          this.addInsured(t);
          this.closePicker();
        });
      });
      ...
      on(this.els.btnFinish, "click", () => { this.finish(); });
    ...
    reset(){
      ...
      this.state.step = 1;
      this.state.insured = [];
      this.state.mainMedical = { heightCm: "", weightKg: "" };
      this.renderInsured();
      this.renderMedical();
      this.syncStepUI();
      this.updateProgress();
    ...
    isStepValid(){
      if(this.state.step === 2) return this.isMedicalValid();
    ...
    insuredShort(type){
      // Short labels for medical UI
      if(type === "main") return { title:"מבוטח ראשי", bmi:"BMI" };
      if(type === "spouse") return { title:"מבוטח משני", bmi:"BMI משני" };
      if(type === "adult") return { title:"מבוטח בגיר", bmi:"BMI בגיר" };
      return { title:"מבוטח קטין", bmi:"BMI קטין" };
    },
    ...
    completionPctStep2(){
      const people = this.getAllPeopleForMedical();
      const total = people.length * 2;
      if(total <= 0) return 0;
      let done = 0;
      people.forEach(p => {
        const m = this.getMedical(p.id);
        if(safeTrim(m.heightCm)) done++;
        if(safeTrim(m.weightKg)) done++;
      });
      return Math.max(0, Math.min(100, Math.round((done / total) * 100)));
    },

    completionPctOverall(){
      const s1 = this.completionPctStep1() / 100;
      const s2 = this.completionPctStep2() / 100;
      return Math.max(0, Math.min(100, Math.round(((s1 + s2) / 2) * 100)));
    },
    updateProgress(){
      this.updateAge();
      const pct = this.completionPctOverall();
    ...
    syncStepUI(){
      ...
      if(this.els.btnBack){
        this.els.btnBack.style.display = (step === 2) ? "" : "none";
      }

      if(this.els.title) this.els.title.textContent = (step === 1) ? "הקמת לקוח חדש · שלב 1" : "הקמת לקוח חדש · שלב 2";
      if(this.els.sub) this.els.sub.textContent = (step === 1) ? "פרטי לקוח" : "נתונים רפואיים";

      if(this.els.progressMeta) this.els.progressMeta.textContent = (step === 1) ? "השלמת תהליך (שלב 1/2)" : "השלמת תהליך (שלב 2/2)";

      if(this.els.footHint){
        this.els.footHint.textContent = (step === 1)
          ? "כפתור “המשך” יופעל לאחר מילוי כל שדות החובה."
          : "כפתור “סיום” יופעל לאחר מילוי גובה ומשקל לכל המבוטחים.";
      }

      if(this.els.btnNext){
        this.els.btnNext.textContent = (step === 1) ? "המשך" : "סיום";
      }
    },
    ...
    next(){
      ...
      if(this.state.step === 1){
        ...
        this.updateProgress();
        setTimeout(() => {
          const first = this.els.medList?.querySelector?.('input[data-med-k="heightCm"]');
          first?.focus?.();
        }, 60);
        return;
      }

      alert("✔️ שלבים 1-2 הושלמו!\nבשלב הבא נחבר את זה לשמירה/תהליך הבא.");
    },
    ...
    prev(){
      ...
      this.syncStepUI();
      this.updateProgress();
      setTimeout(() => this.els.fields.c_firstName?.focus?.(), 60);
    }

finish(){
  // validate step 2 before finishing
  ...
  // TODO: connect to save/next process later
  alert("✔️ שלבים 1-2 הושלמו!\nבשלב הבא נחבר את זה לשמירה/תהליך הבא.");
  this.close();
  this.state.step = 1;
},
,
    ...
    getAllPeopleForMedical(){
      const main = { id: "main", label: "מבוטח ראשי", type: "main" };
      const others = (this.state.insured || []).map(it => ({ id: it.id, label: it.label, type: it.type }));
      return [main, ...others];
    },

    getMedical(id){
      if(id === "main") return this.state.mainMedical || { heightCm:"", weightKg:"" };
      const it = (this.state.insured || []).find(x => x.id === id);
      const d = (it && it.data) ? it.data : {};
      return { heightCm: d.heightCm || "", weightKg: d.weightKg || "" };
    },

    setMedical(id, k, v){
      if(id === "main"){
        this.state.mainMedical = this.state.mainMedical || { heightCm:"", weightKg:"" };
        this.state.mainMedical[k] = v;
        return;
      }
      const it = (this.state.insured || []).find(x => x.id === id);
      if(!it) return;
    ...
    bmiRisk(bmi, age){
      ...
      if(Number.isFinite(age) && age < 18) return { label:"בסיסי", cls:"is-med" };
      if(bmi >= 30) return { label:"גבוה", cls:"is-high" };
      if(bmi >= 25) return { label:"בינוני", cls:"is-med" };
      if(bmi >= 18.5) return { label:"נמוך", cls:"is-low" };
      return { label:"בינוני", cls:"is-med" };
    },

    renderMedical(){
      const host = this.els.medList;
      if(!host) return;

      const mainName = [safeTrim(this.els.fields.c_firstName?.value), safeTrim(this.els.fields.c_lastName?.value)].filter(Boolean).join(" ");
      const mainDob = safeTrim(this.els.fields.c_dob?.value);
      const mainAge = this.ageFromDob(mainDob);

      const people = this.getAllPeopleForMedical();
      host.innerHTML = people.map(p => {
        const m = this.getMedical(p.id);
        const dob = (p.id === "main") ? mainDob : safeTrim((this.state.insured.find(x => x.id === p.id)?.data || {}).dob);
        const age = this.ageFromDob(dob);
        const bmi = this.bmiFrom(m.heightCm, m.weightKg);
        const risk = this.bmiRisk(bmi, age);

        const short = this.insuredShort(p.type);
        const title = (p.id === "main")
          ? (short.title + (mainName ? " · " + escapeHtml(mainName) : ""))
          : (short.title);

        const meta = (age != null) ? `גיל: ${age}` : "גיל: —";

        return `
          <div class="lcMedCard" data-med-card="${p.id}">
            <div class="lcMedCard__head">
              <div>
                <div class="lcMedCard__title">${title}</div>
                <div class="lcMedCard__meta">${meta}</div>
              </div>

              <div class="lcMedCard__badges">
                <span class="lcBmiPill" title="BMI">${escapeHtml(short.bmi)}: <span data-med-bmi="${p.id}">${bmi != null ? bmi : "—"}</span></span>
                <span class="lcRiskPill ${risk.cls}" data-med-risk="${p.id}">סיכון: ${escapeHtml(risk.label)}</span>
              </div>
            </div>
            ...
            <div class="lcMedGrid">
              <div class="lcFormGrid lcFormGrid--2">
                <div class="field">
                  ...
                  <input class="input lcNum" type="number" step="0.1" inputmode="decimal" data-med-id="${p.id}" data-med-k="heightCm" placeholder="לדוגמה: 170" value="${escapeAttr(m.heightCm)}" />
                </div>

                <div class="field">
                  <label class="label">משקל (ק״ג) *</label>
                  <input class="input lcNum" type="number" step="0.1" inputmode="decimal" data-med-id="${p.id}" data-med-k="weightKg" placeholder="לדוגמה: 72" value="${escapeAttr(m.weightKg)}" />
                </div>
              </div>
            </div>
          </div>
      ...
      $$('[data-med-k]', host).forEach(inp => {
        const id = inp.getAttribute("data-med-id");
        const k = inp.getAttribute("data-med-k");
        const handler = () => {
          this.setMedical(id, k, inp.value);
          this.updateMedicalCard(id);
          this.updateProgress();
        };
      ...
      people.forEach(p => this.updateMedicalCard(p.id));
    },

    updateMedicalCard(id){
      const host = this.els.medList;
      if(!host) return;

      const card = host.querySelector(`[data-med-card="${CSS.escape(id)}"]`);
      if(!card) return;

      const m = this.getMedical(id);
      const dob = (id === "main")
        ? safeTrim(this.els.fields.c_dob?.value)
        : safeTrim((this.state.insured.find(x => x.id === id)?.data || {}).dob);

      const age = this.ageFromDob(dob);
      const bmi = this.bmiFrom(m.heightCm, m.weightKg);
      const risk = this.bmiRisk(bmi, age);

      const bmiEl = card.querySelector(`[data-med-bmi="${CSS.escape(id)}"]`);
      if(bmiEl) bmiEl.textContent = (bmi != null ? bmi : "—");

      const riskEl = card.querySelector(`[data-med-risk="${CSS.escape(id)}"]`);
      if(riskEl){
        riskEl.classList.remove("is-low","is-med","is-high");
        if(risk.cls) riskEl.classList.add(risk.cls);
        riskEl.textContent = "סיכון: " + risk.label;
      }
    },
    ...
    isMedicalValid(){
      ...
      return people.every(p => {
        const m = this.getMedical(p.id);
        const h = parseFloat(String(m.heightCm).replace(",", "."));
        const w = parseFloat(String(m.weightKg).replace(",", "."));
        if(!Number.isFinite(h) || !Number.isFinite(w)) return false;
        if(h < 40 || h > 250) return false;
        if(w < 10 || w > 300) return false;
        return true;
      });
    },
    ...
    addInsured(type){
      ...
      this.renderInsured();
      if(this.state.step === 2) this.renderMedical();
      this.updateProgress();
    },
