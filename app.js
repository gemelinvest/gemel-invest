...
(() => {
  ...
  const Wizard = {
    ...
    init(){
      ...
      this.showStep(1);
      this.updateProgress({ keepFocus:false });
    },
    ...
    _captureFocus(){
      ...
      if(!ae) return null;
      // Only keep focus inside wizard
      if(!this.els.wrap || !this.els.wrap.contains(ae)) return null;

      const token = {
        id: ae.id || null,
        name: ae.getAttribute?.("name") || null,
        // We try to uniquely identify inputs in lists
        bmiId: ae.getAttribute?.("data-id") || null,
        bmiKey: ae.getAttribute?.("data-bmi") || null,
        k: ae.getAttribute?.("data-k") || null,
        insuredId: ae.getAttribute?.("data-id") || null,
    ...
    _restoreFocus(token){
      ...
      // Prefer exact id
      if(token.id){
        el = this.els.wrap.querySelector(`#${CSS.escape(token.id)}`);
      }

      // BMI fields
      if(!el && token.bmiId && token.bmiKey){
        el = this.els.wrap.querySelector(`input[data-bmi="${CSS.escape(token.bmiKey)}"][data-id="${CSS.escape(token.bmiId)}"]`);
      }

      // Insured dynamic fields
      if(!el && token.k && token.insuredId){
        el = this.els.wrap.querySelector(`input[data-k="${CSS.escape(token.k)}"][data-id="${CSS.escape(token.insuredId)}"]`);
      }

      if(!el) return;

      // If something (like step change) intentionally moved focus, don't steal it
      const ae = document.activeElement;
      if(ae && ae !== document.body && this.els.wrap.contains(ae) && ae !== el) return;

      try{
        el.focus({ preventScroll: true });
        if(typeof el.setSelectionRange === "function" && token.selStart !== null && token.selEnd !== null){
          el.setSelectionRange(token.selStart, token.selEnd);
        }
      }catch(_){}
    },
updateProgress(opts={}){
      const keepFocus = (opts && opts.keepFocus === false) ? false : true;
      const __focus = keepFocus ? this._captureFocus() : null;
      this.updateAge();
      ...
      if(keepFocus){
        requestAnimationFrame(() => this._restoreFocus(__focus));
      }
},
