...
txt.textContent = "";
  ...
  const Wizard = {
    ...
    localities: [
      "אבו גוש","אבו סנאן","אבן יהודה","אום אל-פחם","אופקים","אור יהודה","אור עקיבא","אזור","אילת","איתמר","אלעד","אליכין","אלפי מנשה","אריאל","אשדוד","אשקלון","באקה אל-גרבייה","באר יעקב","באר שבע","בית דגן","בית שאן","בית שמש","ביתר עילית","בני ברק","בנימינה","בת חפר","בת ים","ג'דיידה-מכר","ג'סר א-זרקא","גבעת אבני","גבעת זאב","גבעת שמואל","גבעתיים","גדרה","גן יבנה","דאלית אל-כרמל","דבוריה","דימונה","הוד השרון","הושעיה","הרצליה","זכרון יעקב","זמר","זמרת","חדרה","חולון","חורה","חורפיש","חיפה","חצור הגלילית","טבריה","טייבה","טירה","טירת כרמל","יבנה","יהוד-מונוסון","יפיע","יקנעם עילית","ירוחם","ירושלים","כאוכב אבו אל-היג'א","כוכב יאיר","כסיפה","כפר ברא","כפר ורדים","כפר יאסיף","כפר יונה","כפר כנא","כפר מנדא","כפר סבא","כפר קאסם","כרמיאל","להבים","לוד","לקיה","מאור","מגדל העמק","מודיעין-מכבים-רעות","מודיעין עילית","מזכרת בתיה","מזרעה","מטולה","מיתר","מעלה אדומים","מעלות-תרשיחא","מצפה רמון","משגב","נהריה","נווה זיו","נוף הגליל","נחף","נס ציונה","נצרת","נצרת עילית","נשר","נתיבות","נתניה","סאג'ור","סביון","סח'נין","עאבלין","עומר","עיילבון","עכו","עילוט","עין מאהל","עין קנייא","עפולה","עראבה","ערד","ערערה","ערערה-בנגב","פוריידיס","פסוטה","פתח תקווה","פרדס חנה-כרכור","פרדסיה","צור הדסה","צפת","קדימה-צורן","קיסריה","קלנסווה","קצרין","קריית אונו","קריית ארבע","קריית אתא","קריית ביאליק","קריית גת","קריית טבעון","קריית ים","קריית מלאכי","קריית מוצקין","קריית עקרון","קריית שמונה","קרני שומרון","ראש העין","ראש פינה","ראשון לציון","רהט","רחובות","ריינה","רכסים","רמלה","רמת גן","רמת השרון","רמת ישי","רעננה","שדרות","שוהם","שפרעם","תל אביב-יפו","תל מונד","תל שבע","תמרת"
    ],
    occupations: [
...
// open health covers drawer (Health only)
    ...
    bindLocalityAutocomplete(ins){
      ...
      const openMenu = () => {
        if(!safeTrim(input.value)) return closeMenu();
        const hasOptions = !!menu.querySelector(".lcLocalityOption:not(.is-empty)");
        if(!hasOptions && !menu.querySelector(".lcLocalityOption.is-empty")) return closeMenu();
      ...
      refreshMenu();
      closeMenu();
      on(input, "focus", () => { if(!safeTrim(input.value)) closeMenu(); });
      on(input, "click", () => { if(!safeTrim(input.value)) closeMenu(); });
      on(input, "input", () => {
        ins.data.city = safeTrim(input.value);
        this.scheduleZipLookup(ins);
        refreshMenu();
        if(safeTrim(input.value)) openMenu(); else closeMenu();
      });
      on(input, "change", () => {
        ins.data.city = safeTrim(input.value);
        this.scheduleZipLookup(ins);
      ...
      on(input, "keydown", (ev) => {
        ...
        if(ev.key === "ArrowDown"){
          if(!safeTrim(input.value)) return;
          ev.preventDefault();
    ...
    getStep1Questions(ins){
      ...
      questions.push(
        ...
        },
        {
          ...
          sub: isChild ? 'היישוב נמשך אוטומטית מהמבוטח הראשי.' : 'הקלד יישוב והרשימה תישלף מתחת לשדה רק בזמן הקלדה.',
          required:!isChild,
          render:() => isChild
            ...
            : `<div class="field"><label class="label">יישוב</label><div class="lcLocalityWrap"><input class="input lcLocalityInput" id="lcLocalityInput" type="text" data-bind="city" value="${escapeHtml(d.city || '')}" placeholder="התחל להקליד יישוב…" autocomplete="off" aria-autocomplete="list" aria-expanded="false" /><div class="lcLocalityMenu" id="lcLocalityMenu"></div></div><div class="help">הרשימה נפתחת רק בזמן הקלדה. אם אין התאמה, אפשר להמשיך להקליד ידנית.</div></div>`
        },
        {
          key:'street',
          title:'מה שם הרחוב?',
