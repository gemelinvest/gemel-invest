...
(() => {
  ...
  const BUILD = "20260409-vCustomerSaveHardening1";
  ...
  const SUPABASE_VERIFY_RETRIES = 4;
  const SUPABASE_VERIFY_DELAY_MS = 700;
  ...
  const Storage = {
    ...
    async upsertSingleRow(tableName, row){
      ...
      const safeId = safeTrim(payload?.id);
      const selectExpr = safeId ? 'id,updated_at,created_at' : 'id,updated_at';
      try {
        const client = this.getClient();
        const { data, error } = await client.from(tableName).upsert([payload], { onConflict: 'id' }).select(selectExpr).maybeSingle();
        if(error) throw error;
        return { ok:true, at: nowISO(), data: data || null };
      } catch(primaryErr) {
        try {
          const data = await this.restRequest(tableName, {
            method: 'POST',
            body: payload,
            headers: { Prefer: 'resolution=merge-duplicates,return=representation' }
          });
          const rowData = Array.isArray(data) ? (data[0] || null) : (data || null);
          return { ok:true, at: nowISO(), data: rowData };
        } catch(restErr) {
    ...
    async verifyRowPresence(tableName, id, options = {}){
      const safeId = safeTrim(id);
      if(!safeId) return { ok:false, error:'MISSING_ID' };
      const selectExpr = safeTrim(options.selectExpr) || '*';
      const retries = Math.max(1, Number(options.retries || SUPABASE_VERIFY_RETRIES));
      const delayMs = Math.max(150, Number(options.delayMs || SUPABASE_VERIFY_DELAY_MS));
      let lastError = '';
      for(let attempt = 1; attempt <= retries; attempt += 1){
        const res = await this.loadSingleRow(tableName, safeId, selectExpr);
        if(res?.ok && res?.data){
          return { ok:true, data: res.data, attempt };
        }
        lastError = safeTrim(res?.error) || 'VERIFY_EMPTY_RESULT';
        if(attempt < retries) await new Promise((resolve) => setTimeout(resolve, delayMs));
      }
      return { ok:false, error:lastError || 'VERIFY_FAILED' };
    },

    async saveVerifiedRow(tableName, row, options = {}){
      const payload = row && typeof row === 'object' ? row : {};
      const safeId = safeTrim(payload?.id);
      if(!safeId) return { ok:false, error:'MISSING_ID' };
      ...
      const connection = await this.waitForConnection({
        ...
        delayMs: options.connectionDelayMs
      });
      if(!connection?.ok){
        return { ok:false, error: safeTrim(connection?.error) || 'NO_SERVER_CONNECTION' };
      }

      const saveRes = await this.upsertSingleRow(tableName, payload);
      if(!saveRes?.ok){
        return { ok:false, error: safeTrim(saveRes?.error) || 'SAVE_FAILED' };
      }

      const verifyRes = await this.verifyRowPresence(tableName, safeId, {
        selectExpr: safeTrim(options.verifySelectExpr) || '*',
        retries: options.verifyRetries,
        delayMs: options.verifyDelayMs
      });
      if(!verifyRes?.ok){
        return { ok:false, error: safeTrim(verifyRes?.error) || 'VERIFY_FAILED' };
      }

      return {
        ok:true,
        at: safeTrim(saveRes?.at) || nowISO(),
        data: verifyRes.data,
        verifyAttempt: verifyRes.attempt || 1
      };
    },
...
// ---------- Step 6 ----------
    ...
    async saveDraft(){
      ...
      if(proposalIssues.length){
        ...
        this.showValidationToast(issueMsg);
      ...
      const directSave = await Storage.saveVerifiedRow(SUPABASE_TABLES.proposals, directRow, {
        verifySelectExpr: 'id,updated_at'
      });
      if(!directSave?.ok){
    ...
    showValidationToast(message){
      ...
      if(!msg || typeof window.showToast !== 'function') return;
      try { window.showToast(msg); } catch(_e) {}
    },
    ...
    async saveCompletedCustomer(){
      ...
      const directSave = await Storage.saveVerifiedRow(SUPABASE_TABLES.customers, directRow, {
        verifySelectExpr: 'id,updated_at'
      });
      if(!directSave?.ok) throw new Error('שמירת הלקוח לשרת נכשלה: ' + safeTrim(directSave?.error || 'SAVE_FAILED'));

      const verifiedOnServer = true;
