  ...
  const CustomersUI = {
    ...
    init(){
      ...
      on(this.els.dash, "click", async (ev) => {
        const reportBtn = ev.target?.closest?.('[data-open-ops-report]');
        if(reportBtn){
          ev.preventDefault?.();
          ev.stopPropagation?.();
          this.openOpsReportFromCurrent();
    ...
    getStats(rec, policies){
      ...
      return [
        ...
        },
        {
          type: "ops-report",
          label: "דו״ח תפעול",
          sub: "לחץ לפתיחת הדוח מתוך תיק הלקוח",
          image: "./operational-report-logo.png"
        },
      ...
      try{
        ...
        if(this.els.dash){
          this.els.dash.innerHTML = stats.map(card => {
            ...
            if(card.type === "ops-report"){
              return `
              <button class="customerStatCard customerStatCard--report" type="button" data-open-ops-report="1" aria-label="פתיחת דוח תפעול">
                <div class="customerStatCard__content customerStatCard__content--report">
                  <img class="customerStatCard__reportImg" src="${escapeHtml(card.image || './operational-report-logo.png')}" alt="דו״ח תפעול"/>
                  <div class="customerStatCard__label customerStatCard__label--report">${escapeHtml(card.label)}</div>
                  <div class="customerStatCard__sub customerStatCard__sub--report">${escapeHtml(card.sub)}</div>
                </div>
              </button>`;
            }
            return `
    ...
    openOpsReportFromCurrent(){
      ...
      try{
        ...
        console.error('openOpsReportFromCurrent failed', err);
      }
    },
...
// ---------- Step 6 ----------
    ...
    openOperationalReport(payloadOverride){
      ...
      const sourcePayload = payloadOverride || this._lastOperationalReportPayload || this.getOperationalPayload();
      this._lastOperationalReportPayload = JSON.parse(JSON.stringify(sourcePayload || {}));
      try{
        this.els.reportBody.innerHTML = this.renderOperationalReport(this._lastOperationalReportPayload);
      }catch(err){
      ...
      this.els.report.style.display = 'block';
      this.els.report.style.pointerEvents = 'auto';
      try { document.body.classList.add('lcReportOpen'); } catch(_e) {}
    },
    ...
    closeOperationalReport(){
      ...
      this.els.report.style.display = '';
      this.els.report.style.pointerEvents = '';
      try { document.body.classList.remove('lcReportOpen'); } catch(_e) {}
    },

    exportOperationalPdf(){
      if(!this.els.report?.classList?.contains('is-open')){
        this.openOperationalReport(this._lastOperationalReportPayload || this.getOperationalPayload());
      }
      window.print();
    },
