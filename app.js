...
(() => {
  ...
  const BUILD = "20260331-har-excel-skip-banner-v16";
  ...
  const HAR_EXCEL_BANNER_PATTERNS = [
    /התיק\s*הביטוחי/,
    /הופק\s*מאתר/,
    /הר\s*הביטוח/,
    /משרד\s*האוצר/,
  ...
  function isHarWorkbookBannerRow(row){
    if(!Array.isArray(row)) return false;
    const joined = row.map((cell) => normalizeHarWorkbookCell(cell)).filter(Boolean).join(' | ');
    if(!joined) return false;
    return HAR_EXCEL_BANNER_PATTERNS.some((pattern) => pattern.test(joined));
  }

  function stripHarWorkbookBannerRows(rows){
    if(!Array.isArray(rows)) return [];
    return rows.filter((row) => !isHarWorkbookBannerRow(row));
  }
...
// open health covers drawer (Health only)
    ...
    parseElementaryVehicleHarWorkbook(buffer, ins){
      ...
      sheetNames.forEach((sheetName) => {
        ...
        const rows = stripHarWorkbookBannerRows(rawRows);
        if(!Array.isArray(rows) || !rows.length) return;

        const headerIdx = rows.reduce((bestIdx, row, idx) => {
          const score = scoreHeaderRow(row);
          const bestScore = bestIdx >= 0 ? scoreHeaderRow(rows[bestIdx]) : 0;
          return score > bestScore ? idx : bestIdx;
        }, -1);
        const headerScore = headerIdx >= 0 ? scoreHeaderRow(rows[headerIdx]) : 0;
        if(headerIdx === -1 || headerScore < 4) return;
    ...
    parseHarBituachWorkbook(buffer, ins){
      ...
      const scoreHeaderRow = (row = []) => {
        if(!Array.isArray(row) || !row.length || isHarWorkbookBannerRow(row)) return 0;
        return Object.values(headerMatchers).reduce((score, patterns) => (
      ...
      sheetNames.forEach((sheetName) => {
        ...
        const headerScore = headerIdx >= 0 ? scoreHeaderRow(rows[headerIdx]) : 0;
        if(headerIdx === -1 || headerScore < 4){
          debug.sheets.push({ sheetName, headerIdx, headerScore, skipped: true, reason: 'header_not_found' });
          return;
        }
