...
(() => {
  ...
  const BUILD = "20260413-vUpdateNotify2";
...
(function(){
  ...
  function getBuild(){
    const fromWindow = String(window.__GI_BUILD || '').trim();
    if(fromWindow) return fromWindow;
    const fromScript = String(document.querySelector('script[src*="app.js"]')?.getAttribute('src') || '');
    const match = fromScript.match(/v=([^&]+)/);
  ...
  function setSystemVersionLabel(){
    ...
    if(el && build) el.textContent = 'גרסה: ' + build;
  }
  ...
  function initSystemUpdateUi(){
    try { window.__GI_BUILD = getBuild() || window.__GI_BUILD || ''; } catch(_e) {}
    setSystemVersionLabel();
    bindUpdatePopup();
    if(updateTimerId) clearInterval(updateTimerId);
    setTimeout(() => { checkForSystemUpdate(); }, 12000);
    updateTimerId = setInterval(checkForSystemUpdate, 60000);
  }

  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', initSystemUpdateUi, { once:true });
  } else {
    initSystemUpdateUi();
  }
})();
