...
if (!out.agents.length) {
  ...
  const UI = {
    els: {},
    // ===========================
    // Permissions: Agent sees only own customers (Admin sees all)
    // ===========================
    _ownerOfCustomer(c){ return String(c?.assignedAgent || "").trim(); },
    canSeeCustomer(c){
      if (!Auth.current) return false;
      if (Auth.isAdmin()) return true;
      const owner = UI._ownerOfCustomer(c);
      const me = String(Auth.current?.name || "").trim();
      return !!owner && owner === me;
    },
    getVisibleCustomers(){
      const all = Array.isArray(State.data?.customers) ? State.data.customers : [];
      if (Auth.isAdmin()) return all;
      return all.filter(c => UI.canSeeCustomer(c));
    },
...
this.els.btnUserEditSave = $("#btnUserEditSave");
      ...
      const getOpenCustomer = () => {
        ...
        if (!id) return null;
        return (this.getVisibleCustomers()).find(x => x.id === id) || null;
      };
...
this.renderPolicies();
    ...
    openCustomerEdit(customerId){
      const c = State.data.customers.find(x => x.id === customerId);
      if(!c) return;
            if(!this.canSeeCustomer(c)){
        notify(\"אין הרשאה לערוך לקוח זה.\", \"warn\");
        return;
      }
if(!this.els.modalCustomer || !this.els.customerForm) return;
    ...
    openCustomerFull(customerId) {
      ...
      if(!this.canSeeCustomer(c)){
        notify(\"אין הרשאה לצפות בלקוח זה.\", \"warn\");
        return;
      }
...
if (this.els.cfIdIssueDate) this.els.cfIdIssueDate.textContent = (c.idIssueDate || "—");
      ...
      if (this.els.cfAgentSelect) {
        ...
        this.els.cfAgentSelect.disabled = (!Auth.current || (Auth.current && Auth.current.role === \"agent\"));
      }
this.renderPolicies();
...
openPolicyActionModal(action, policyId, policyLabel) {
    ...
    renderCustomers() {
      ...
      const scored = (this.getVisibleCustomers()).map((c, idx) => {
