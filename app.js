...
// open health covers drawer (Health only)
    ...
    renderStep3(ins){
      ...
      const rows = (d.existingPolicies || []).map(p => {
        ...
        const cancellationBox = this.renderExistingPolicyCancellationControls(p, d.cancellations?.[p.id] || {}, { compact: true });
    ...
    bindInputs(ins){
      ...
      $$("[data-cancel-policy]", this.els.body).forEach(el => {
        const commitCancel = (doRender = true) => {
          ...
          if(key === 'status') this.syncCancellationExecutionMethodState(ins);
          if(doRender) this.render();
        };
        on(el, "input", () => {
...
// ---------- Step 6 ----------
    ...
    getCancellationExecutionMethodOptions(){
      return [
        { value: 'agent', label: 'על ידי הנציג' },
        { value: 'company', label: 'על ידי החברה המבטחת' },
        { value: 'client', label: 'הלקוח מבטל באופן עצמאי' }
      ];
    },
    ...
    insuredHasRelevantCancellation(insured = null){
      const ins = insured || this.getActive();
      const data = ins?.data && typeof ins.data === 'object' ? ins.data : {};
      const rows = data?.cancellations && typeof data.cancellations === 'object' ? Object.values(data.cancellations) : [];
      return rows.some((row) => {
        const status = safeTrim(row?.status);
        return status === 'full' || status === 'partial_health';
      });
    },

    getCancellationExecutionMethodValue(insured = null){
      const ins = insured || this.getActive();
      return safeTrim(ins?.data?.cancellationExecutionMethod);
    },

    setCancellationExecutionMethodValue(value, insured = null){
      const ins = insured || this.getActive();
      if(!ins) return '';
      ins.data = ins.data && typeof ins.data === 'object' ? ins.data : {};
      const clean = safeTrim(value);
      if(clean) ins.data.cancellationExecutionMethod = clean;
      else delete ins.data.cancellationExecutionMethod;
      return clean;
    },

    syncCancellationExecutionMethodState(insured = null){
      const ins = insured || this.getActive();
      if(!ins) return '';
      if(!this.insuredHasRelevantCancellation(ins)){
        this.setCancellationExecutionMethodValue('', ins);
        return '';
      }
      return this.getCancellationExecutionMethodValue(ins);
    },

    ensureCancellationExecutionMethodBeforeStep3Exit(){
      const ins = this.getActive();
      if(!ins) return false;
      const hasRelevant = this.insuredHasRelevantCancellation(ins);
      if(!hasRelevant){
        this.setCancellationExecutionMethodValue('', ins);
        return false;
      }
      const existing = this.getCancellationExecutionMethodValue(ins);
      if(existing) return false;
      this.openCancellationExecutionMethodModal(ins);
      return true;
    },
      ...
      this.insureds.forEach((ins, insuredIdx) => {
        (ins.data?.existingPolicies || []).forEach((policy, policyIdx) => {
          ...
          existingPolicies.push({
            ...
            cancellationExecutionMethod: (safeTrim(statusMeta?.raw) === 'full' || safeTrim(statusMeta?.raw) === 'partial_health')
              ? this.getCancellationExecutionMethodLabel(this.getCancellationExecutionMethodValue(ins))
              : ''
          });
        });
      });
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const cancellationExecutionSummary = (insureds || []).map((ins) => {
        const label = this.insuredHasRelevantCancellation({ data: ins?.data || {} }) ? this.getCancellationExecutionMethodLabel(safeTrim(ins?.data?.cancellationExecutionMethod)) : '';
        return label ? `${safeTrim(ins?.label) || 'מבוטח'}: ${label}` : '';
      }).filter(Boolean).join(' | ');
      const summaryRows = [
      ...
      insureds.forEach((ins, index) => {
        ...
        policies.forEach((policy) => {
          ...
          const cancelExecutionLabel = (safeTrim(statusMeta?.raw) === 'full' || safeTrim(statusMeta?.raw) === 'partial_health')
            ? this.getCancellationExecutionMethodLabel(safeTrim(d?.cancellationExecutionMethod))
            : '';
