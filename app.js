...
(() => {
  ...
  function normalizeState(s){
    ...
    // attachments (e.g., Har Bituach PDF)
    const rawAtt = out.meta.attachments || {};
    out.meta.attachments = {
      harBituach: (rawAtt.harBituach && typeof rawAtt.harBituach === "object") ? rawAtt.harBituach : null
    };
...
init(){
      ...
      // har bituach modal (step 3)
      this.els.harWrap = $("#lcHarBituach");
      this.els.harClose = $("#lcHarBituachClose");
      this.els.harFile = $("#lcHarBituachFile");
      this.els.harPick = $("#lcHarPickBtn");
      this.els.harStatus = $("#lcHarBituachStatus");
      this.els.harSave = $("#lcHarBituachSave");
      this.els.harDownload = $("#lcHarBituachDownload");
      this.els.harRemove = $("#lcHarBituachRemove");
      this._harPendingFile = null;
...
on(this.els.btnOpen, "click", () => {
      ...
      on(this.els.picker, "click", (e) => {
        ...
        if(t && t.matches && t.matches("[data-ins-type]")){
          ...
          this.addInsured(typ);
          this.closePicker();
      this.closeHarBituach();
        }
      });


      // Har Bituach modal events
      on(this.els.harClose, "click", () => this.closeHarBituach());
      on(this.els.harWrap, "click", (e) => {
        const t = e.target;
        if(t && t.getAttribute// Har Bituach modal events
      on(this.els.harClose, "click", () => this.closeHarBituach());
      on(this.els.harWrap, "click", (e) => {
        const t = e.target;
        if(t && t.getAttribute && t.getAttribute("data-close") === "1") this.closeHarBituach();
      });

      on(this.els.harPick, "click", () => {
        try{ this.els.harFile?.click?.(); }catch(_e){}
      });

      on(this.els.harFile, "change", () => {
        const f = this.els.harFile?.files?.[0];
        this._harPendingFile = f || null;
        this.renderHarBituachUI();
      });

      on(this.els.harSave, "click", async () => {
        if(!this._harPendingFile) return;
        await this.saveHarBituachFile(this._harPendingFile);
        this._harPendingFile = null;
        this.closeHarBituach();
      });

      on(this.els.harDownload, "click", () => this.downloadHarBituach());
      on(this.els.harRemove, "click", async () => { await this.removeHarBituach(); });
...
ype, label) => ({
      ...
      // attachments (case-level)
      this.harBituach = null;
      this.step = 1;
      this.render();
    },
    ...
    openHarBituach(){
      if(!this.els.harWrap) return;
      this.els.harWrap.classList.add("is-open");
      this.els.harWrap.setAttribute("aria-hidden","false");
      this.renderHarBituachUI();
    },
    closeHarBituach(){
      if(!this.els.harWrap) return;
      this.els.harWrap.classList.remove("is-open");
      this.els.harWrap.setAttribute("aria-hidden","true");
      // reset file input so selecting same file again triggers change
      if (this.els.harFile) this.els.harFile.value = "";
      this._harPendingFile = null;
    },


    async saveHarBituachFile(file){
      try{
        if(!file) return;
        const maxMB = 12;
        const sizeMB = (file.size || 0) / (1024*1024);
        if(sizeMB > maxMB){
          this.setHint("הקובץ גדול מדי (מעל " + maxMB + "MB).");
          if(this.els.harStatus) this.els.harStatus.textContent = "הקובץ גדול מדי.";
          return;
        }
        ...
        const b64 = await new Promise((resolve, reject) => {
          ...
          fr.onload = () => {
            ...
            // res is like: data:application/pdf;base64,XXXX
        ...
        this.harBituach = {
          name: safeTrim(file.name) || "har-bituach.pdf",
        ...
        // also mirror to State meta as infrastructure for saving now
        State.data.meta = State.data.meta || {};
        State.data.meta.attachments = State.data.meta.attachments || {};
        State.data.meta.attachments.harBituach = { ...this.harBituach };

        await App.persist("נשמר קובץ הר ביטוח");
        this.renderHarBituachUI();
        this.setHint("קובץ הר ביטוח נשמר");
      }catch(e){
        console.error("HAR_UPLOAD_ERR:", e);
        this.setHint("שגיאה בהעלאת הקובץ");
        if(this.els.harStatus) this.els.harStatus.textContent = "שגיאה בהעלאה: " + String(e?.message || e);
      }
    },

    downloadHarBituach(){
      try{
        const a = this.harBituach || State.data?.meta?.attachments?.harBituach || null;
        if(!a || !a.b64) return;
        const bin = atob(a.b64);
        const bytes = new Uint8Array(bin.length);
        for(let i=0;i<bin.length;i++) bytes[i] = bin.charCodeAt(i);
        const blob = new Blob([bytes], { type: a.mime || "application/pdf" });
        const url = URL.createObjectURL(blob);
        const link = document.createElement("a");
        link.href = url;
        link.download = a.name || "har-bituach.pdf";
        document.body.appendChild(link);
        link.click();
        link.remove();
        setTimeout(() => URL.revokeObjectURL(url), 800);
      }catch(e){
        console.error("HAR_DL_ERR:", e);
      }
    },

    async removeHarBituach(){
      this.harBituach = null;
      try{
        if(State.data?.meta?.attachments) delete State.data.meta.attachments.harBituach;
        await App.persist("הוסר קובץ הר ביטוח");
      }catch(_e){}
      this.renderHarBituachUI();
      this.setHint("קובץ הר ביטוח הוסר");
    },

    renderHarBituachUI(){
      const saved = this.harBituach || State.data?.meta?.attachments?.harBituach || null;
      const pending = this._harPendingFile;

      // status text
      if(this.els.harStatus){
        if(pending){
          const kb = Math.round((pending.size||0)/1024);
          this.els.harStatus.textContent = "נבחר לשמירה: " + (pending.name||"") + " (" + kb + "KB)";
        } else if(saved && saved.name){
          const kb = Math.round((saved.size||0)/1024);
          this.els.harStatus.textContent = "נשמר: " + saved.name + " (" + kb + "KB)";
        } else {
          this.els.harStatus.textContent = "לא נבחר קובץ";
        }
      }

      // save enabled only when pending exists
      if(this.els.harSave){
        this.els.harSave.disabled = !pending;
      }

      // download/remove visible when saved exists
      if(this.els.harDownload) this.els.harDownload.style.display = (saved && saved.b64) ? "" : "none";
      if(this.els.harRemove) this.els.harRemove.style.display = (saved && saved.b64) ? "" : "none";
    },
    ...
    renderBody(){
      ...
      const addBtn = (this.step === 3) ? `<div class="row" style="gap:10px; flex-wrap:wrap;"><button class="btn" id="lcAddExistingPolicy" type="button">➕ הוסף פוליסה</button><button class="btn" id="lcHarBituachBtn" type="button">הר ביטוח</button></div>` : "";
...
if(path === "birthDate"){
  ...
  return;
      ...
      const hb = $("#lcHarBituachBtn", this.els.body);
      on(hb, "click", () => { this.openHarBituach(); });
