...
(() => {
  ...
  const BUILD = "20260319-autor-refresh-hardening";
...
txt.textContent = "";
  ...
  const UsersUI = {
    ...
    async _saveFromModal(){
      ...
      this.closeModal();
      this.render();
      requestAnimationFrame(() => this.syncViewportLayout());
    },
    ...
    syncViewportLayout(){
      if(!this.isOpen || !this.els?.wrap) return;
      const panel = this.els.wrap.querySelector('.lcWizard__panel');
      const body = this.els.body;
      if(!panel || !body) return;
      const head = panel.querySelector('.lcWizard__head');
      const tabs = panel.querySelector('.lcWizard__tabs');
      const foot = panel.querySelector('.lcWizard__foot');
      const panelHeight = Math.max(0, panel.clientHeight || panel.getBoundingClientRect().height || 0);
      const used = (head?.offsetHeight || 0) + (tabs?.offsetHeight || 0) + (foot?.offsetHeight || 0);
      const free = Math.max(180, Math.floor(panelHeight - used));
      body.style.maxHeight = free + 'px';
      body.style.height = free + 'px';
...
init(){
      ...
      this._syncViewportLayoutBound = () => this.syncViewportLayout();
      window.addEventListener("resize", this._syncViewportLayoutBound, { passive:true });
    ...
    close(){
      ...
      this.closeHealthFindingsModal();
      document.body.style.overflow = "";
      if(this.els?.body){ this.els.body.style.maxHeight = ""; this.els.body.style.height = ""; }
      this.closePicker();
    },
