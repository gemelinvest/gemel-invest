...
(() => {
  ...
  const BUILD = "20260326-operational-report-all-insureds-v1";
...
// ---------- Step 6 ----------
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const formatExistingPolicyStatus = (policy, data) => {
        const statusMeta = (typeof this.getExistingPolicyStatusMeta === 'function')
          ? this.getExistingPolicyStatusMeta(policy, data)
        ...
        return {
          label: safeTrim(statusMeta?.label) || '—',
          note: statusParts.join(' · ') || ''
        };
      };
      const policyBelongsToInsured = (policy, ins, index) => {
        const insId = safeTrim(ins?.id) || safeTrim(ins?.data?.id) || `payload_ins_${index}`;
        const policyInsuredId = safeTrim(policy?.insuredId);
        const type = safeTrim(ins?.type);
        if(policy?.insuredMode === 'couple'){
          return index === 0 || type === 'spouse';
        }
        if(policyInsuredId) return policyInsuredId === insId;
        return index === 0;
      };
      const buildHealthRowsForInsured = (ins, index) => {
        const insId = safeTrim(ins?.id) || safeTrim(ins?.data?.id) || `payload_ins_${index}`;
        return (questionList || []).map((item, qIndex) => {
          const q = item?.question || {};
          const answer = responses?.[q.key]?.[insId] || null;
          const answerRaw = safeTrim(answer?.answer);
          ...
          return {
            no: qIndex + 1,
            question: safeTrim(q.text || q.label || q.key) || `שאלה ${qIndex + 1}`,
            yes: answerRaw === 'yes',
            noMark: answerRaw === 'no',
            details
          };
      ...
      const buildExistingPolicyCardsForInsured = (ins, index) => {
        const data = ins?.data || {};
        const policies = Array.isArray(data?.existingPolicies) ? data.existingPolicies : [];
        return policies.map((policy, policyIndex) => {
          const status = formatExistingPolicyStatus(policy, data);
      ...
      const buildNewPolicyCardsForInsured = (ins, index) => {
        return (newPolicies || []).filter((policy) => policyBelongsToInsured(policy, ins, index)).map((policy, policyIndex) => ({
      ...
      const insuredBundles = insureds.map((ins, index) => {
        const d = ins?.data || {};
        const label = safeTrim(ins?.label) || `מבוטח ${index + 1}`;
        const displayName = safeTrim(((d.firstName || '') + ' ' + (d.lastName || '')).trim()) || label;
        return {
          index,
          ...
          existingCards: buildExistingPolicyCardsForInsured(ins, index),
          newCards: buildNewPolicyCardsForInsured(ins, index),
          healthRows: buildHealthRowsForInsured(ins, index)
        };
      });
      ...
      const renderInfoTable = (rows, emptyText) => {
        const list = Array.isArray(rows) ? rows.filter(([label, value]) => safeTrim(label) && safeTrim(value)) : [];
        if(!list.length) return `<div class="lcPdfEmpty">${escapeHtml(emptyText || 'אין נתונים להצגה')}</div>`;
        return `<table class="lcPdfInfoTable"><tbody>${list.map(([label, value]) => `<tr><th>${escapeHtml(label)}</th><td>${escapeHtml(value)}</td></tr>`).join('')}</tbody></table>`;
      };
      const renderClassicTable = (headers, rows, emptyText) => {
        const list = Array.isArray(rows) ? rows : [];
        if(!list.length) return `<div class="lcPdfEmpty">${escapeHtml(emptyText || 'אין נתונים להצגה')}</div>`;
        return `<table class="lcPdfTable"><thead><tr>${headers.map(h => `<th>${escapeHtml(h)}</th>`).join('')}</tr></thead><tbody>${list.map((row) => `<tr>${row.map(cell => `<td>${escapeHtml(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table>`;
      };
      const renderPolicyCards = (cards, emptyText) => {
        if(!cards.length) return `<div class="lcPdfEmpty">${escapeHtml(emptyText || 'אין נתונים להצגה')}</div>`;
        return `<div class="lcPdfPolicyStack">${cards.map((card) => `
          <article class="lcPdfPolicyCard lcPdfPolicyCard--${escapeHtml(card.tone || 'neutral')}">
            ...
            ${renderInfoTable(card.rows, 'לא נמצאו פרטי פוליסה')}
            ${card.covers?.length ? `<div class="lcPdfSubBox"><div class="lcPdfSubBox__title">כיסויים</div><div class="lcPdfChips">${card.covers.map((cover) => `<span class="lcPdfChip">${escapeHtml(cover)}</span>`).join('')}</div></div>` : ''}
            ${card.pledgeRows?.length ? `<div class="lcPdfSubBox"><div class="lcPdfSubBox__title">פרטי שיעבוד</div>${renderInfoTable(card.pledgeRows, 'לא הוזנו פרטי שיעבוד')}</div>` : ''}
          </article>`).join('')}</div>`;
      };
      const renderHealthSectionForBundle = (bundle) => {
      ...
      const renderInsuredBlock = (bundle) => {
        return `
          <section class="lcPdfSection lcPdfSection--insured">
            <div class="lcPdfInsuredHeader">
              ...
              <div class="lcPdfInsuredMeta">קיימות: ${bundle.existingCards.length} · חדשות: ${bundle.newCards.length} · בריאות: ${bundle.healthRows.length}</div>
            </div>
            <div class="lcPdfInsuredGrid">
              <div class="lcPdfInsuredCol">
                <div class="lcPdfMiniTitle">פרטים אישיים</div>
                ${renderInfoTable(bundle.detailsRows, 'לא הוזנו פרטי מבוטח')}
              </div>
              <div class="lcPdfInsuredCol">
                <div class="lcPdfMiniTitle">פוליסות קיימות</div>
                ${renderPolicyCards(bundle.existingCards, 'לא נוספו פוליסות קיימות למבוטח זה')}
              </div>
            </div>
            <div class="lcPdfInsuredSingle">
              <div class="lcPdfMiniTitle">פוליסות חדשות</div>
              ${renderPolicyCards(bundle.newCards, 'לא נוספו פוליסות חדשות למבוטח זה')}
            </div>
            <div class="lcPdfInsuredSingle">
              <div class="lcPdfMiniTitle">הצהרת בריאות</div>
              ${renderHealthSectionForBundle(bundle)}
            </div>
          </section>`;
      };

      const insuredOverviewRows = insuredBundles.map((bundle) => {
        const d = insureds[bundle.index]?.data || {};
        return [
          bundle.label,
          bundle.displayName,
          safeTrim(d.idNumber) || '—',
      ...
      const style = `<style>
        ...
        .lcPdfInsuredGrid{display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:start}
        .lcPdfInsuredCol,.lcPdfInsuredSingle{margin-top:8px}
        ...
        .lcPdfPolicyCard--existing{border-color:#d8e1eb;background:#ffffff}
        ...
        .lcPdfHealthSection{margin-top:8px}
        .lcPdfHealthSection__title{font-size:18px;font-weight:800;color:#1b3555;text-align:right;margin-bottom:10px}
      ...
      const overviewPage = `
        <div class="lcPdfPage">
          ...
          <section class="lcPdfSection"><div class="lcPdfSection__title">סקירת מבוטחים</div>${renderClassicTable(['סוג מבוטח','שם מלא','ת״ז','תאריך לידה','טלפון','קיימות','חדשות','בריאות'], insuredOverviewRows, 'לא נמצאו מבוטחים')}</section>
      ...
      const insuredPages = insuredBundles.map((bundle, pageIndex) => `
        <div class="lcPdfPage">
          <header class="lcPdfHeader">
            <div>
              <h1 class="lcPdfHeader__title">דוח תפעולי — ${escapeHtml(bundle.displayName)}</h1>
              <div class="lcPdfHeader__sub">שיקוף מלא של המבוטח, כולל פרטים אישיים, פוליסות והצהרת בריאות.</div>
              <div class="lcPdfHeader__meta">
          ...
          ${renderInsuredBlock(bundle)}
          <div class="lcPdfPageFooter"><div class="lcPdfPageFooter__brand">GEMEL INVEST</div><div>עמוד ${pageIndex + 2}</div></div>
        </div>`);

      return {
        payload,
        html: `${style}<div class="${rootClass}">${[overviewPage].concat(insuredPages).join('<div class="lcPdfPageBreak"></div>')}</div>`
      };
    },
