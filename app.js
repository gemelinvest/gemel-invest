...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbyH_IdKzjFa8-y0K89BPLO2gf1Yipm86mI-BkP1Vn5Tfzeh0yPHr8lr0MdMCE3Gp-k/exec'; // connected Web App (/exec)
  ...
  // Option 4: sync on save + input + blur + next/back (best coverage)
  ...
  const SYNC_STATE = {
    ...
    debounceMs: 900,
    pullEveryMs: 10000,
    pending: { customers:false, proposals:false, processes:false, users:false },
    lastPullIso: ''
  };

  function scheduleAutoSync_(reason){
    if(!AUTO_SYNC || !GOOGLE_SCRIPT_URL) return;
    // mark everything as potentially changed (safe + simple)
    SYNC_STATE.pending.customers = true;
    SYNC_STATE.pending.proposals = true;
    SYNC_STATE.pending.processes = true;
    // users usually change rarely, but keep it true for full coverage
    // SYNC_STATE.pending.users = true;

    if(SYNC_STATE.timer) clearTimeout(SYNC_STATE.timer);
    SYNC_STATE.timer = setTimeout(()=>flushAutoSync_(), SYNC_STATE.debounceMs);
  }

  async function flushAutoSync_(){
    if(!AUTO_SYNC || !GOOGLE_SCRIPT_URL) return;

    const doCustomers = SYNC_STATE.pending.customers;
    const doProposals = SYNC_STATE.pending.proposals;
    const doProcesses = SYNC_STATE.pending.processes;
    const doUsers     = SYNC_STATE.pending.users;

    SYNC_STATE.pending.customers = false;
    SYNC_STATE.pending.proposals = false;
    SYNC_STATE.pending.processes = false;
    SYNC_STATE.pending.users = false;

    try{
      // push changes (best effort)
      if(doCustomers) await syncUpAllCustomers_();
      if(doProposals) await syncUpAllProposals_();
      if(doProcesses) await syncUpAllProcesses_();
      if(doUsers)     await syncUpAllUsers_();
    }catch(_){}

    // pull changes from server (multi-user)
    try{
      await syncDownSince_();
    }catch(_){}
  }

  async function syncUpAllCustomers_(){
    const arr = Array.isArray(state.customers) ? state.customers : [];
    for(let i=0;i<arr.length;i++){
      const c = arr[i];
      if(!c || !c.id) continue;
      await apiCall('upsertCustomer', { customer: c });
    }
  }

  async function syncUpAllProposals_(){
    const arr = Array.isArray(state.proposals) ? state.proposals : [];
    for(let i=0;i<arr.length;i++){
      const p = arr[i];
      if(!p || !p.id) continue;
      await apiCall('upsertProposal', { proposal: p });
    }
  }

  async function syncUpAllProcesses_(){
    const arr = Array.isArray(state.processes) ? state.processes : [];
    for(let i=0;i<arr.length;i++){
      const pr = arr[i];
      if(!pr || !pr.id) continue;
      await apiCall('upsertProcess', { process: pr });
    }
  }

  async function syncUpAllUsers_(){
    const arr = Array.isArray(state.users) ? state.users : [];
    for(let i=0;i<arr.length;i++){
      const u = arr[i];
      if(!u || !u.username) continue;
      await apiCall('upsertUser', { user: u });
    }
  }

  async function syncDownSince_(){
    // If server supports incremental, use it; otherwise fall back to full list.
    const sinceIso = SYNC_STATE.lastPullIso || '';
    let c=null, p=null, pr=null;
    c = await apiCall('listCustomers', sinceIso ? { sinceIso } : {});
    if(c && c.ok && Array.isArray(c.customers)) state.customers = c.customers;

    p = await apiCall('listProposals', sinceIso ? { sinceIso } : {});
    if(p && p.ok && Array.isArray(p.proposals)) state.proposals = p.proposals;

    pr = await apiCall('listProcesses', sinceIso ? { sinceIso } : {});
    if(pr && pr.ok && Array.isArray(pr.processes)) state.processes = pr.processes;

    saveCustomers_();
    saveProposals_();
    saveProcesses_();

    SYNC_STATE.lastPullIso = new Date().toISOString();
  }

  function startAutoSync_(){
    if(!AUTO_SYNC || !GOOGLE_SCRIPT_URL) return;
    if(SYNC_STATE.pullTimer) clearInterval(SYNC_STATE.pullTimer);
    SYNC_STATE.pullTimer = setInterval(()=>scheduleAutoSync_('pull'), SYNC_STATE.pullEveryMs);

    // global listeners: input/change/blur/click -> schedule sync
    try{
      document.addEventListener('input', ()=>scheduleAutoSync_('input'), true);
      document.addEventListener('change', ()=>scheduleAutoSync_('change'), true);
      document.addEventListener('blur', ()=>scheduleAutoSync_('blur'), true);
      document.addEventListener('click', ()=>scheduleAutoSync_('click'), true);
    }catch(_){}
  }
  ...
  function saveProcesses_(){
    try{
      ...
      scheduleAutoSync_('saveProcesses');
    }catch(_){}
  }
  ...
  function saveProposals_(){
    try{
      ...
      scheduleAutoSync_('saveProposals');
    }catch(_){}
  }
  ...
  function saveCustomers_(){
    try{
      ...
      scheduleAutoSync_('saveCustomers');
    }catch(_){}
  }
...
function debounce(fn, wait){
  ...
  async function syncDown(){
    // Pull data from server (if available)
