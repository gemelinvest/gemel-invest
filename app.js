...
(() => {
  ...
  const BUILD = "20260327-ops-workbench-v1";
  ...
  function ensureOpsProcess(rec){
    ...
    if(!store.workbench || typeof store.workbench !== "object") store.workbench = {};
    if(!store.workbench.copyMap || typeof store.workbench.copyMap !== "object") store.workbench.copyMap = {};
    if(!Array.isArray(store.workbench.corrections)) store.workbench.corrections = [];
    if(typeof store.workbench.drawerOpen !== "boolean") store.workbench.drawerOpen = false;
    return store;
  }

  function getOpsWorkbench(rec){
    const store = ensureOpsProcess(rec);
    const wb = store.workbench && typeof store.workbench === "object" ? store.workbench : (store.workbench = {});
    if(!wb.copyMap || typeof wb.copyMap !== "object") wb.copyMap = {};
    if(!Array.isArray(wb.corrections)) wb.corrections = [];
    return wb;
  }

  function isOpsWorkbenchLocked(rec){
    const wb = getOpsWorkbench(rec);
    return !!wb.active;
  }

  function getUnreadCorrections(rec){
    const wb = getOpsWorkbench(rec);
    return (wb.corrections || []).filter(item => !item?.readAt).length;
  }
...
txt.textContent = "";
  ...
  const CustomersUI = {
    ...
    init(){
      ...
      this.els.correctionsBtn = $("#customerFullCorrectionsBtn");
      ...
      on(this.els.editBtn, "click", (ev) => {
        ...
        const rec = this.current();
        if(!rec) return;
        if(isOpsWorkbenchLocked(rec) && !Auth.isOps()){
          window.alert('התיק בתהליך הקמה במחלקת התפעול. אפשר לשלוח תיקונים דרך לחצן "הגש תיקונים".');
          return;
        }
        CustomerEditUI.open(rec.id);
      });
      on(this.els.opsBtn, "click", (ev) => {
        ev?.preventDefault?.();
        const rec = this.current();
        if(!rec || !Auth.isOps()) return;
        OpsWorkbenchUI.open(rec.id);
      });
      on(this.els.correctionsBtn, "click", (ev) => {
        ev?.preventDefault?.();
        const rec = this.current();
        if(!rec) return;
        CorrectionsRequestUI.open(rec.id);
      });
    ...
    openById(id, opts={}){
      ...
      try{
        ...
        const workbenchLocked = isOpsWorkbenchLocked(rec);
        if(this.els.opsBtn) this.els.opsBtn.style.display = Auth.isOps() ? '' : 'none';
        if(this.els.editBtn) this.els.editBtn.style.display = (!Auth.isOps() && workbenchLocked) ? 'none' : '';
        if(this.els.correctionsBtn) this.els.correctionsBtn.style.display = (!Auth.isOps() && workbenchLocked) ? '' : 'none';
        if(this.els.dash){
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const ProcessesUI = {
    ...
    render(){
        ...
        return `<tr class="lcMyProcessesRow" data-tone="${escapeHtml(row.tone)}">
          ...
          <td>
            <div class="lcMyProcessesActions">
              ...
              <button class="btn btn--primary btn--softBlue" data-process-workbench="${escapeHtml(rec.id)}" type="button">מסך תפעול</button>
      ...
      UI.els.myProcessesTbody.querySelectorAll("[data-process-workbench]").forEach(btn => {
        btn.onclick = () => OpsWorkbenchUI.open(btn.getAttribute("data-process-workbench"));
      });
