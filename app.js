...
(() => {
  ...
  const BUILD = "20260326-support-tickets-owner-v1";
  ...
  const SUPPORT_OWNER_IDENTIFIERS = [
    "איתי",
    "איתי סומך",
    "איתי סומך - מנכ\"ל גמל אינווסט",
    "איתי /איתי סומך - מנכ\"ל גמל אינווסט"
  ];
  const SUPPORT_OWNER_LABEL = "איתי /איתי סומך - מנכ\"ל גמל אינווסט";
  ...
  function isSupportOwnerIdentity(...values){
    const normalizedTargets = SUPPORT_OWNER_IDENTIFIERS.map(normalizeSupportIdentity).filter(Boolean);
    return values.some((value) => {
      const normalized = normalizeSupportIdentity(value);
      return normalized && normalizedTargets.includes(normalized);
    });
  }
  ...
  const Storage = {
    ...
    async createSupportTicket(ticket){
      const row = {
        user_name: safeTrim(ticket?.user_name),
        user_role: safeTrim(ticket?.user_role) || "agent",
        type: safeTrim(ticket?.type) || "תקלה במערכת",
        message: safeTrim(ticket?.message),
        status: safeTrim(ticket?.status) || "new",
        assigned_to: safeTrim(ticket?.assigned_to) || SUPPORT_OWNER_LABEL,
        created_at: safeTrim(ticket?.created_at) || nowISO()
      };
      try {
        const client = this.getClient();
        const { data, error } = await client.from(SUPABASE_TABLES.supportTickets).insert([row]).select("*").single();
        if(error) throw error;
        return { ok:true, data };
      } catch(primaryErr) {
        try {
          const data = await this.restRequest(SUPABASE_TABLES.supportTickets, { method:"POST", body: row });
          return { ok:true, data: Array.isArray(data) ? (data[0] || row) : (data || row) };
        } catch(restErr) {
          return { ok:false, error: String(restErr?.message || primaryErr?.message || restErr || primaryErr) };
        }
      }
    ...
    async listSupportTickets(){
      try {
        ...
        const { data, error } = await client
          .from(SUPABASE_TABLES.supportTickets)
          .select("*")
          .eq("assigned_to", SUPPORT_OWNER_LABEL)
        ...
        try {
          const data = await this.restRequest(
            SUPABASE_TABLES.supportTickets +
            "?assigned_to=eq." + encodeURIComponent(SUPPORT_OWNER_LABEL) +
    ...
    async updateSupportTicket(id, patch){
      ...
      try {
        ...
        try {
          const data = await this.restRequest(SUPABASE_TABLES.supportTickets + "?id=eq." + encodeURIComponent(id), {
            method:"PATCH",
            body: payload
          });
    ...
    async deleteSupportTicket(id){
      try {
        const client = this.getClient();
        const { error } = await client.from(SUPABASE_TABLES.supportTickets).delete().eq("id", id);
        if(error) throw error;
        return { ok:true };
      } catch(primaryErr) {
        try {
          await this.restRequest(SUPABASE_TABLES.supportTickets + "?id=eq." + encodeURIComponent(id), {
            method:"DELETE",
            headers: { Prefer: "return=minimal" }
          });
          return { ok:true };
  ...
  const Auth = {
    ...
    isSupportOwner(){
      return !!(this.current && isSupportOwnerIdentity(this.current.name, this.current.username));
    },
    ...
    _restoreSession(){
      try {
        ...
        const username = safeTrim(s?.username) || name;
        if(!name) return null;
    ...
    _saveSession(cur){
      try {
        localStorage.setItem(LS_SESSION_KEY, JSON.stringify({ name: cur.name, username: cur.username || cur.name, role: cur.role }));
      } catch(_) {}
    },
    ...
    async _submit(){
      ...
      this.current = { name: matched.name, username: safeTrim(matched.username) || matched.name, role: (matched.role === "manager" ? "manager" : matched.role === "ops" ? "ops" : "agent") };
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      await App.reloadSessionState();
      this.unlock();
      try { InactivityGuard.start(); } catch(_e) {}
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.navSupportCenter = $("#navSupportCenter");
      ...
      this.els.supportTicketType = $("#supportTicketType");
      this.els.supportTicketMessage = $("#supportTicketMessage");
      this.els.supportTicketSubmit = $("#supportTicketSubmit");
      this.els.supportTicketClear = $("#supportTicketClear");
      this.els.supportTicketFeedback = $("#supportTicketFeedback");
      this.els.supportTicketsTbody = $("#supportTicketsTbody");
      this.els.supportInboxCard = $("#supportInboxCard");
      this.els.supportInboxLocked = $("#supportInboxLocked");
      this.els.supportTicketsTableWrap = $("#supportTicketsTableWrap");
      this.els.supportTicketsCountBadge = $("#supportTicketsCountBadge");
      this.els.supportInboxHint = $("#supportInboxHint");
      this.els.supportOwnerPill = $("#supportOwnerPill");

      on(this.els.btnLogout, "click", () => Auth.logout());
      on(this.els.supportTicketSubmit, "click", () => SupportUI.submit());
      on(this.els.supportTicketClear, "click", () => SupportUI.clearForm());
// nav
...
this.applyRoleUI();
    ...
    applyRoleUI(){
      ...
      if (this.els.navSupportCenter) this.els.navSupportCenter.style.display = Auth.current ? "" : "none";
    ...
    goView(view){
      ...
      if(safe === "supportCenter" && !Auth.current) safe = "dashboard";
      // hide all views
      ...
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-discountSpec-active","view-customers-active","view-proposals-active","view-myProcesses-active","view-mirrors-active","view-supportCenter-active");
      ...
      if (safe === "supportCenter") SupportUI.render();
    },
          ...
          <div class="lcPdfPage">
            <header class="lcPdfHeader">
              <div>
                ...
                <div class="lcPdfHeader__sub">מסודר עבור נציג התפעול לצפייה מהירה וברורה.</div>
                <div class="lcPdfHeader__meta">
                  <div>${escapeHtml(exportedAt)}</div>
                  <div>בכל הנוגע: ${escapeHtml(customerName)}</div>
                  <div>בכל הנוגע: ${escapeHtml(currentAgentName)} — סוכן מטפל</div>
                </div>
              </div>
...
const SystemRepairUI = {
  ...
  const SupportUI = {
    ...
    renderOwnerLabel(){
      const el = UI?.els?.supportOwnerPill || $("#supportOwnerPill");
      if(el) el.textContent = SUPPORT_OWNER_LABEL;
    },

    canViewInbox(){
      return Auth.isSupportOwner();
    },

    setFeedback(msg, kind=""){
      const el = UI.els.supportTicketFeedback;
      if(!el) return;
    ...
    clearForm(){
      if(UI.els.supportTicketType) UI.els.supportTicketType.value = "תקלה במערכת";
      if(UI.els.supportTicketMessage) UI.els.supportTicketMessage.value = "";
      this.setFeedback("");
    },

    async submit(){
      if(!Auth.current) return;
      const type = safeTrim(UI.els.supportTicketType?.value) || "תקלה במערכת";
      const message = safeTrim(UI.els.supportTicketMessage?.value);
      if(!message){
        this.setFeedback("יש למלא את תוכן הפנייה לפני שליחה.", "error");
        UI.els.supportTicketMessage?.focus?.();
        return;
      }
      this.setFeedback("שולח פנייה…");
      const res = await Storage.createSupportTicket({
        user_name: safeTrim(Auth.current?.name),
        user_role: safeTrim(Auth.current?.role),
        type,
        message,
        status: "new",
        assigned_to: SUPPORT_OWNER_LABEL,
        created_at: nowISO()
      });
      if(!res.ok){
        this.setFeedback("לא הצלחנו לשמור את הפנייה. ודא שטבלת support_tickets קיימת ב-Supabase.", "error");
        console.error("SUPPORT_TICKET_CREATE_FAILED", res.error || res);
        return;
      }
      this.clearForm();
      this.setFeedback("הפנייה נשלחה בהצלחה ונשמרה במערכת.", "success");
      if(this.canViewInbox()) await this.loadTickets();
    },

    async loadTickets(){
      if(!this.canViewInbox()) return;
      this.loading = true;
      const res = await Storage.listSupportTickets();
      this.loading = false;
      if(!res.ok){
        this.tickets = [];
        this.renderTableError("לא הצלחנו לטעון את פניות המערכת. ודא שטבלת support_tickets קיימת ב-Supabase.");
        console.error("SUPPORT_TICKETS_LOAD_FAILED", res.error || res);
        return;
      }
    ...
    renderTableError(message){
      const tbody = UI.els.supportTicketsTbody;
      if(!tbody) return;
      tbody.innerHTML = `<tr><td colspan="6"><div class="emptyState"><div class="emptyState__icon">⚠️</div><div class="emptyState__title">שגיאה בטעינת הפניות</div><div class="emptyState__text">${escapeHtml(message || "אירעה שגיאה בטעינת פניות המערכת.")}</div></div></td></tr>`;
      if(UI.els.supportTicketsCountBadge) UI.els.supportTicketsCountBadge.textContent = "0 פניות";
    },

    renderTable(){
      const tbody = UI.els.supportTicketsTbody;
      if(!tbody) return;
      const items = Array.isArray(this.tickets) ? this.tickets : [];
      if(UI.els.supportTicketsCountBadge){
        UI.els.supportTicketsCountBadge.textContent = `${items.length} פניות`;
      }
      if(!items.length){
        tbody.innerHTML = `<tr><td colspan="6"><div class="emptyState"><div class="emptyState__icon">📭</div><div class="emptyState__title">אין פניות מערכת</div><div class="emptyState__text">ברגע שנציגים ישלחו פנייה מתוך המערכת, היא תופיע כאן ליוזר המורשה בלבד.</div></div></td></tr>`;
        return;
      }
      tbody.innerHTML = items.map((ticket) => {
        const statusKey = safeTrim(ticket?.status) || "new";
        const statusLabel = SUPPORT_TICKET_STATUSES[statusKey] || statusKey || "חדש";
        const createdLabel = this.formatDate(ticket?.created_at);
        return `
          <tr data-ticket-id="${escapeHtml(ticket?.id || "")}">
            <td>
              <div class="supportTicketMetaCell">
                <strong>${escapeHtml(ticket?.user_name || "—")}</strong>
                <span class="muted small">${escapeHtml(this.roleLabel(ticket?.user_role || ""))}</span>
              </div>
            </td>
            <td>${escapeHtml(ticket?.type || "—")}</td>
            <td><div class="supportTicketMessageCell">${escapeHtml(ticket?.message || "—")}</div></td>
            <td><span class="supportTicketStatus is-${escapeHtml(statusKey)}">${escapeHtml(statusLabel)}</span></td>
            <td>${escapeHtml(createdLabel)}</td>
            <td>
              <div class="supportTicketActions">
                <select class="input" data-support-status="${escapeHtml(ticket?.id || "")}">
                  ${Object.entries(SUPPORT_TICKET_STATUSES).map(([value, label]) => `<option value="${escapeHtml(value)}" ${value === statusKey ? "selected" : ""}>${escapeHtml(label)}</option>`).join("")}
                </select>
                <button class="btn supportTicketDelete" data-support-delete="${escapeHtml(ticket?.id || "")}" type="button">מחק</button>
              </div>
            </td>
          </tr>
        `;
    ...
    roleLabel(role){
      const key = safeTrim(role);
      return key === "admin" ? "מנהל מערכת" : key === "manager" ? "מנהל" : key === "ops" ? "תפעול" : "נציג";
    },
    ...
    async handleTableClick(ev){
      if(!this.canViewInbox()) return;
      const deleteBtn = ev.target?.closest?.("[data-support-delete]");
      if(deleteBtn){
        const id = safeTrim(deleteBtn.getAttribute("data-support-delete"));
        if(!id) return;
        const ok = window.confirm("למחוק את הפנייה הזו?");
        if(!ok) return;
        const res = await Storage.deleteSupportTicket(id);
        if(!res.ok){
          window.alert("לא הצלחנו למחוק את הפנייה.");
          console.error("SUPPORT_TICKET_DELETE_FAILED", res.error || res);
          return;
        }
    ...
    async handleTableChange(ev){
      if(!this.canViewInbox()) return;
      const select = ev.target?.closest?.("[data-support-status]");
      if(!select) return;
      const id = safeTrim(select.getAttribute("data-support-status"));
      const status = safeTrim(select.value) || "new";
      const res = await Storage.updateSupportTicket(id, { status });
      if(!res.ok){
        window.alert("לא הצלחנו לעדכן את סטטוס הפנייה.");
        console.error("SUPPORT_TICKET_UPDATE_FAILED", res.error || res);
        await this.loadTickets();
        return;
      }
      await this.loadTickets();
    ...
    async render(){
      this.renderOwnerLabel();
      const canView = this.canViewInbox();
      if(UI.els.supportInboxLocked) UI.els.supportInboxLocked.hidden = !!canView;
      if(UI.els.supportTicketsTableWrap) UI.els.supportTicketsTableWrap.hidden = !canView;
      if(UI.els.supportInboxHint){
        UI.els.supportInboxHint.textContent = canView
          ? "אזור זה פתוח רק ליוזר המורשה ורק הוא יכול לראות ולטפל בכלל פניות המערכת."
          : "אזור זה זמין רק ליוזר המורשה לצפייה בפניות.";
      }
      if(!canView){
        if(UI.els.supportTicketsCountBadge) UI.els.supportTicketsCountBadge.textContent = "גישה חסומה";
        return;
      }
      await this.loadTickets();
    }
  ...
  const App = {
    ...
    async boot(){
      ...
      if (Auth.current) {
        ...
        try { SupportUI.renderOwnerLabel(); } catch(_e) {}
      } else {
        UI.goView("dashboard");
      }
    },
    ...
    async reloadSessionState(){
      ...
      if (r.ok) {
        ...
        if (Auth.current) {
          ...
          if (document.getElementById("view-supportCenter")?.classList.contains("is-visible")) {
            try { await SupportUI.render(); } catch(_e) {}
          }
        }
      } else {
  ...
  LeadShellUI.init();
  on(document, "click", (ev) => { SupportUI.handleTableClick(ev); });
  on(document, "change", (ev) => { SupportUI.handleTableChange(ev); });
  ChatUI.init();
