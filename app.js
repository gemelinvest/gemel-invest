...
init(){
      ...
      on(this.els.harFile, "change", async () => { await this.handleHarBituachFile(); });
      on(this.els.harDownload, "click", () => this.downloadHarBituach());
      on(this.els.harRemove, "click", async () => { await this.removeHarBituach(); });


      // har bituach modal bindings
      on(this.els.harClose, "click", () => this.closeHarBituach());
      on(this.els.har, "click", (e) => {
        const t = e.target;
        if(t && t.getAttribute && t.getAttribute("data-close") === "1") this.closeHarBituach();
      });

      on(this.els.harFile, "change", async () => {
        const f = this.els.harFile?.files?.[0];
        if(!f) return;
        await this.saveHarBituachFile(f);
      });
      on(this.els.harDownload, "click", () => this.downloadHarBituach());
      on(this.els.harRemove, "click", () => this.removeHarBituach());
    ...
    openHarBituach(){
      if(!this.els.har) return;
      this.els.har.classList.add("is-open");
      this.els.har.setAttribute("aria-hidden","false");
      this.renderHarBituachUI();
    },
    closeHarBituach(){
      if(!this.els.har) return;
      this.els.har.classList.remove("is-open");
      this.els.har.setAttribute("aria-hidden","true");
    ...
    renderHarBituachUI(){
      const a = this.harBituach || State.data?.meta?.attachments?.harBituach || null;
      const st = this.els.harStatus;
      const dl = this.els.harDownload;
      const rm = this.els.harRemove;
      if(st){
        if(a && a.name){
          const kb = Math.round((a.size||0)/1024);
          st.textContent = "נשמר: " + a.name + " (" + kb + "KB)";
        }else{
          st.textContent = "לא נבחר קובץ עדיין.";
        }
      }
      if(dl) dl.style.display = (a && a.b64) ? "" : "none";
      if(rm) rm.style.display = (a && a.b64) ? "" : "none";
    },
