...
(() => {
  ...
  const BUILD = "20260322-chat-fab-position-stable-2";
...
const SystemRepairUI = {
  ...
  const ChatUI = {
    ...
    syncVisibility(view){
      const onDashboard = view === "dashboard" && !!Auth.current && !document.body.classList.contains('lcAuthLock');
      if(!onDashboard) this.hideFab();
      else if(this.els.window?.classList.contains("is-hidden")) this.showFab();
      if(!onDashboard) this.closeWindow(false, true);
    },
    ...
    showFab(){
      ...
      if(hasSavedPosition || !hadInlinePosition) this.restoreFabPosition();
      fab.classList.remove('is-hidden');
      if(!hasSavedPosition && !hadInlinePosition){
        requestAnimationFrame(() => this.captureFabPosition());
      }
    },
    ...
    closeWindow(forceKeepFab=false, skipSync=false){
      ...
      const shouldShowFab = !!Auth.current && !document.body.classList.contains('lcAuthLock') && (forceKeepFab || document.body.classList.contains('view-dashboard-active'));
    ...
    onLogin(){
      ...
      this.syncVisibility(document.body.classList.contains('view-dashboard-active') ? 'dashboard' : '');
      this.ensureStarted();
    },
