...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // ===== Users -> Google Sheets (Option A: never send password) =====
  function sanitizeUserForSheets_(u){
    if(!u) return null;
    return {
      ...
      // keep a safe json without password
      json: (typeof u.json === 'string' && u.json.trim()) ? u.json : JSON.stringify({
        username: String(u.username||'').trim(),
        displayName: String(u.displayName||'').trim(),
        role: String(u.role||'agent'),
        active: u.active !== false,
  ...
  async function pushUserToSheets_(user){
    const u = sanitizeUserForSheets_(user);
    if(!u || !u.username) return { ok:false, error:'NO_USER' };

    const url = getGoogleScriptUrl();
    if(!url) return { ok:false, demo:true, error:'NO_URL' };

    // Try several server action names / payload shapes (for compatibility with previous server versions)
    const attempts = [
      () => apiCall('upsertUser',   { user: u }),                 // payload.user
      () => apiCall('usersUpsert',  u ),                          // payload fields flat
      () => apiCall('upsert',       { table:'Users', row: u }),   // generic upsert
      () => apiCall('saveUser',     { user: u }),
      () => apiCall('userUpsert',   { user: u }),
    ];
  ...
  function loadUsers_(){
    try{
      ...
      if(Array.isArray(arr) && arr.length){
        ...
        // Default accounts: keep stored passwords as-is (do not force-change).
        // If you want to reset a password, use the admin user management UI.
  ...
  function upsertUser_(payload){
    ...
    if(!isEdit){
      ...
      return { ok:true, user: {
        ...
        role,
        active,
        createdAt: (users[idx] && users[idx].createdAt) ? users[idx].createdAt : '',
        updatedAt: nowIso,
        json: JSON.stringify({ username, displayName, role, active, createdAt: (users[idx] && users[idx].createdAt) ? users[idx].createdAt : '', updatedAt: nowIso })
      } };
    }
  }
...
function renderKpisSettings(){
  ...
  function openUserModal_(opt){
    ...
    overlay.querySelector('[data-save]')?.addEventListener('click', () => {
      ...
      if(res.user){
        const syncRes = await pushUserToSheets_(res.user);
        if(syncRes && syncRes.ok){
          toast('סנכרון משתמש', 'המשתמש נשמר גם בגיליון');
        }else if(syncRes && syncRes.demo){
          // demo/no url - ignore
        }else{
          toast('שגיאה בסנכרון', 'המשתמש לא נשמר בגיליון');
        }
      }
      close();
      renderUsers();
