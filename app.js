...
(() => {
  ...
  const Wizard = {
    ...
    init(){
      ...
      const ids = ["c_firstName","c_lastName","c_id","c_dob","c_gender","c_marital","c_phone","c_email","c_height","c_weight",
                   "c_bmi","c_idealWeight",
                   "c_city","c_street","c_house","c_apt","c_zip","c_job"];
      this.els.fields = Object.fromEntries(ids.map(id => [id, $("#"+id)]));
      this.els.age = $("#c_age");
      this.els.bmiLight = $("#c_bmiLight");
    ...
    updateHealthMetrics(){
      const hRaw = safeTrim(this.els.fields.c_height?.value).replace(",", ".");
      const wRaw = safeTrim(this.els.fields.c_weight?.value).replace(",", ".");
      const gender = safeTrim(this.els.fields.c_gender?.value);

      const h = Number(hRaw);
      const w = Number(wRaw);

      // defaults
      const bmiEl = this.els.fields.c_bmi;
      const idealEl = this.els.fields.c_idealWeight;
      const light = this.els.bmiLight;
      ...
      // If missing/invalid: clear outputs
      if(!(h > 0) || !(w > 0)){
        bmiEl.value = "";
        if(idealEl) idealEl.value = "";
        setLight(null);
        return;
      }
      ...
      // BMI traffic-light (standard adult BMI categories)
      let cls = null;
      if(bmiRounded < 18.5) cls = "is-amber";
      else if(bmiRounded < 25) cls = "is-green";
      else if(bmiRounded < 30) cls = "is-amber";
      else cls = "is-red";

      bmiEl.value = Number.isFinite(bmiRounded) ? String(bmiRounded) : "";
      setLight(cls);

      // Ideal weight estimation by sex (Devine) + ±10% range
      if(idealEl){
        const inches = h / 2.54;
        const base = (gender === "נקבה") ? 45.5 : 50; // kg at 5ft
        const ideal = base + 2.3 * (inches - 60);
        const idealRounded = Math.round(ideal * 10) / 10;
        const lo = Math.round((ideal * 0.9) * 10) / 10;
        const hi = Math.round((ideal * 1.1) * 10) / 10;
        if(Number.isFinite(idealRounded)){
          idealEl.value = `${idealRounded} ק"ג (טווח ${lo}–${hi})`;
        } else {
          idealEl.value = "";
        }
      }
    ...
    requiredMain(){
      return ["c_firstName","c_lastName","c_id","c_dob","c_gender","c_marital","c_phone","c_height","c_weight","c_city","c_street","c_house","c_job"];
    },
    ...
    isStepValid(){
      ...
      // Height/Weight sanity checks (required)
      const h = Number(safeTrim(this.els.fields.c_height?.value).replace(",", "."));
      const w = Number(safeTrim(this.els.fields.c_weight?.value).replace(",", "."));
      if(!(h >= 80 && h <= 250)) missing.push("c_height");
      if(!(w >= 20 && w <= 350)) missing.push("c_weight");
