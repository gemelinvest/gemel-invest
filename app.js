...
(() => {
  ...
  const BUILD = "20260319-customer-wizard-step4-nextfix";
...
init(){
    ...
    nextStep(){
      ...
      if(this.step === 4){
        ...
        const currentComplete = !!active && this.isStepCompleteForInsured(4, active);
        const incomplete = (this.insureds || []).filter(ins => !this.isStepCompleteForInsured(4, ins));
        if(currentComplete && incomplete.length){
          ...
          if(nextIns && String(nextIns.id) !== String(active?.id)){
            ...
            this.setHint(`נותרו פוליסות לעדכון עבור: ${nextIns.label}`);
            this.render();
            return;
          }
        }
