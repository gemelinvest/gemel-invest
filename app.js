...
(() => {
  ...
  const BUILD = "20260404-insured-name-tabs-v32";
...
init(){
    ...
    getInsuredRoleTitle(ins, index){
      const type = safeTrim(ins?.type);
      if(type === "primary" || index === 0) return "מבוטח ראשי";
      if(type === "spouse" || type === "secondary") return "בן/בת זוג";
      if(type === "adult") return "בגיר";
      if(type === "child") return "קטין";
      return "מבוטח נוסף";
    },

    getInsuredTabLabel(ins, index){
      const role = this.getInsuredRoleTitle(ins, index);
    ...
    syncInsuredLabels(){
      if(!Array.isArray(this.insureds)) return;
      this.insureds.forEach((ins, index) => {
        if(!ins || typeof ins !== "object") return;
        ins.label = this.getInsuredTabLabel(ins, index);
      });
    },
...
// open health covers drawer (Health only)
    ...
    getStep1Questions(ins){
      ...
      return [
        ...
        },
        {
          key:'personalDetails',
          title:'פרטים אישיים',
          sub:'כאן מרכזים תאריך לידה, גיל, מין ומצב משפחתי — הכל במסך אחד.',
          fields: isChild ? ['birthDate','gender'] : ['birthDate','gender','maritalStatus'],
          requiredMsg: isChild ? 'נא להשלים תאריך לידה ומין לפני שממשיכים' : 'נא להשלים תאריך לידה, מין ומצב משפחתי לפני שממשיכים',
          render:() => `
            <div class="lcStep1Group">
        ...
        },
        {
          key:'contactDetails',
          title:'פרטי קשר',
          sub: isChild ? 'לקטין פרטי הקשר נמשכים אוטומטית מהמבוטח הראשי ומוצגים כאן יחד.' : 'טלפון, עיר וכתובת מרוכזים כאן כדי לסיים את השלב מהר יותר.',
          fields: isChild ? [] : ['phone','city','street'],
          requiredMsg:'נא להשלים טלפון, עיר וכתובת לפני שממשיכים',
          render:() => isChild
            ? `
              <div class="lcStep1Group">
                <div class="lcStep1Group__grid lcStep1Group__grid--3">
                  ...
                  <div class="lcStep1InfoCard"><div class="lcStep1InfoCard__label">כתובת</div><div class="lcStep1InfoCard__value">${escapeHtml(inherited('street') || 'טרם מולא במבוטח הראשי')}</div><div class="lcStep1InfoCard__sub">הכתובת מוצגת כאן לקריאה בלבד.</div></div>
                </div>
              </div>`
            : `
              <div class="lcStep1Group">
                <div class="lcStep1Group__grid lcStep1Group__grid--3">
                  ...
                  ${this.fieldText('כתובת','street', d.street)}
                </div>
              </div>`
        }
      ].map((q, i) => ({
    ...
    getStep1SummaryValue(ins, q){
      ...
      switch(q?.key){
        ...
        case 'contactDetails':
          if(ins.type === 'child') return [safeTrim(primary.phone), safeTrim(primary.city), safeTrim(primary.street)].filter(Boolean).join(' · ');
          return [safeTrim(d.phone), safeTrim(d.city), safeTrim(d.street)].filter(Boolean).join(' · ');
        default:
          return '';
      }
    },
