...
(() => {
  ...
  const Wizard = {
    ...
    bmiRisk(bmi, age){
      ...
      if(Number.isFinite(age) && age < 18) return { label:"בסיסי (קטין)", cls:"is-med" };
    ...
    renderMedical(){
      ...
      host.innerHTML = people.map(p => {
        ...
        const title = (p.id === "main")
          ? ("מבוטח ראשי" + (mainName ? " · " + escapeHtml(mainName) : ""))
          : escapeHtml(p.label);
          ...
          <div class="lcMedCard" data-med-card="${p.id}">
            <div class="lcMedCard__head">
              ...
              <div class="lcMedCard__badges">
                <span class="lcBmiPill" title="BMI">BMI: <span data-med-bmi="${p.id}">${bmi != null ? bmi : "—"}</span></span>
            ...
            <div class="lcMedGrid">
              <div class="lcFormGrid lcFormGrid--2">
                <div class="field">
                  ...
                  <input class="input lcNum" inputmode="decimal" data-med-id="${p.id}" data-med-k="heightCm" placeholder="לדוגמה: 170" value="${escapeAttr(m.heightCm)}" />
                </div>

                <div class="field">
                  <label class="label">משקל (ק״ג) *</label>
                  <input class="input lcNum" inputmode="decimal" data-med-id="${p.id}" data-med-k="weightKg" placeholder="לדוגמה: 72" value="${escapeAttr(m.weightKg)}" />
                </div>
              </div>
            </div>
          </div>
