...
(() => {
  ...
  const BUILD = "20260409-vTopbarButtonsAppleCompact";
...
UsersGateUI.init();
  ...
  const CustomersUI = {
    ...
    renderPolicyRow(policy){
      ...
      return `<article class="customerPolicyRow customerPolicyRow--compact ${this.companyClass(policy.company)} ${policy.origin === 'new' ? 'is-newRow' : 'is-existingRow'}" data-policy-id="${escapeHtml(policy.id)}" tabindex="0" role="button" aria-label="פרטי פוליסה עבור ${escapeHtml(policy.company || 'חברה')} ${escapeHtml(policy.type || 'פוליסה')}">
