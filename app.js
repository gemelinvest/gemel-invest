  ...
  const CustomersUI = {
    ...
    init(){
      ...
      on(this.els.opsBtn, "click", () => {
        ...
        const prevRecord = Wizard.getReportSourceRecord ? Wizard.getReportSourceRecord() : null;
        try{
          if(Wizard.setReportSourceRecord) Wizard.setReportSourceRecord(rec);
          ...
          if(Wizard.setReportSourceRecord) Wizard.setReportSourceRecord(prevRecord);
        }
      });
...
init(){
      ...
      this.els.reportRefresh = $("#lcReportRefresh");
      ...
      on(this.els.reportRefresh, "click", () => this.openOperationalReport());
...
// ---------- Step 6 ----------
    ...
    setReportSourceRecord(rec){
      this._reportSourceRecord = rec || null;
    },

    getReportSourceRecord(){
      return this._reportSourceRecord || null;
    },
    ...
    formatReportDateTime(value){
      ...
      return escapeHtml(d.toLocaleString('he-IL'));
    },

    getReportFieldLabel(key){
      const map = {
        firstName:'שם פרטי', lastName:'שם משפחה', idNumber:'תעודת זהות', birthDate:'תאריך לידה', phone:'טלפון', email:'אימייל',
        city:'יישוב', street:'רחוב', houseNumber:'מספר בית', zip:'מיקוד', gender:'מגדר', maritalStatus:'מצב משפחתי',
        clinic:'קופת חולים', shaban:'שב״ן', occupation:'עיסוק', heightCm:'גובה', weightKg:'משקל', bmi:'BMI',
        smoker:'עישון', smokingStatus:'סטטוס עישון', payerChoice:'סוג משלם', paymentMethod:'אמצעי תשלום',
        relation:'קרבה', holderName:'שם מחזיק', holderId:'ת.ז מחזיק', cardNumber:'מספר כרטיס', exp:'תוקף',
        bankName:'שם בנק', bankNo:'מספר בנק', branch:'סניף', account:'מספר חשבון',
        agentName:'שם נציג', agentRole:'תפקיד נציג', createdAt:'נוצר בתאריך', updatedAt:'עודכן בתאריך',
        status:'סטטוס', source:'מקור', notes:'הערות', resultStatus:'תוצאת תהליך', liveState:'מצב תפעולי',
        policyNumber:'מספר פוליסה', monthlyPremium:'פרמיה חודשית', premiumMonthly:'פרמיה חודשית', startDate:'תאריך תחילה',
        company:'חברה', type:'סוג ביטוח', insuredMode:'שיוך מבוטח', insuredId:'מזהה מבוטח', discountPct:'אחוז הנחה'
      };
      return map[key] || key;
    },

    collectScalarReportFields(obj, options = {}){
      const source = obj && typeof obj === 'object' ? obj : {};
      const omit = new Set(Array.isArray(options.omit) ? options.omit : []);
      const preferred = Array.isArray(options.preferred) ? options.preferred : [];
      const entries = [];
      const used = new Set();
      const pushEntry = (key) => {
        if(!key || used.has(key) || omit.has(key)) return;
        const value = source[key];
        if(value === null || value === undefined) return;
        if(Array.isArray(value)) return;
        if(typeof value === 'object') return;
        const trimmed = (typeof value === 'boolean') ? (value ? 'כן' : 'לא') : safeTrim(value);
        if(trimmed === '') return;
        used.add(key);
        entries.push([this.getReportFieldLabel(key), this.renderReportValue(trimmed)]);
      };
      preferred.forEach(pushEntry);
      Object.keys(source).forEach(pushEntry);
      return entries;
    },

    renderPairsGrid(entries, cols = 2){
      if(!entries.length) return `<div class="muted">אין נתונים להצגה.</div>`;
      return `<div class="lcReportGrid${cols === 3 ? ' lcReportGrid--3' : ''}">${entries.map(([label, value]) => `<div class="lcReportField"><b>${escapeHtml(label)}</b><div class="lcReportValue">${value}</div></div>`).join('')}</div>`;
    },

    renderSectionHead(title, sub = '', badge = ''){
      return `<div class="lcReportSection__head"><div><div class="lcReportSection__title">${escapeHtml(title)}</div>${sub ? `<div class="lcReportSection__sub">${escapeHtml(sub)}</div>` : ''}</div>${badge ? `<span class="lcReportSection__badge">${escapeHtml(badge)}</span>` : ''}</div>`;
    },

    renderReportBrand(primary, rec, payload){
      const fullName = safeTrim(rec?.fullName || ((primary?.firstName || '') + ' ' + (primary?.lastName || '')).trim()) || 'לקוח ללא שם';
      return `<div class="lcReportBrandRow">
        <div class="lcReportBrand">
          <div class="lcReportBrand__logo">GI</div>
          <div>
            <div class="lcReportBrand__title">GEMEL INVEST · סוכנות לביטוח</div>
            <div class="lcReportBrand__name">${escapeHtml(fullName)}</div>
            <div class="lcReportBrand__sub">קובץ עבודה תפעולי מלא להזנה מול חברת הביטוח</div>
          </div>
        </div>
        <div class="lcReportStamp"><b>הוכן עבור נציג התפעול</b><span>${this.formatReportDateTime(payload?.createdAt || rec?.updatedAt || nowISO())}</span></div>
      </div>`;
    },

    renderOpsSummarySection(rec){
      const state = getOpsStatePresentation(rec || {});
      const rows = [
        ['סטטוס תפעולי חי', `<span class="lcReportStateBadge lcReportStateBadge--${escapeHtml(state.tone || 'info')}">${escapeHtml(state.liveLabel || '—')}</span>`],
        ['תוצאה סופית', this.renderReportValue(state.finalLabel || 'טרם נבחרה')],
        ['טיימר שיחה', this.renderReportValue(state.timerText)],
        ['פירוט טיימר', this.renderReportValue(state.timerMeta)],
        ['גורם מטפל', this.renderReportValue(state.ownerText || 'מחלקת תפעול')],
        ['עודכן לאחרונה', this.formatReportDateTime(state.updatedText)]
      ];
      return `<section class="lcReportSection">${this.renderSectionHead('סטטוס תפעולי ושיקוף', 'החלק הזה נועד לאפשר לנציג התפעול להבין מיד איפה הלקוח עומד בתהליך.', state.finalLabel || state.liveLabel || 'תמונה תפעולית')}<div class="lcReportStateHero"><div class="lcReportStateCard"><div class="lcReportStateCard__label">מצב לקוח נוכחי</div><div class="lcReportStateCard__value">${escapeHtml(state.timerText || '00:00')}</div><div class="lcReportStateCard__meta">${escapeHtml(state.timerMeta || 'הטיימר יופיע כאן כאשר שיחת השיקוף תופעל.')}</div></div><div>${this.renderPairsGrid(rows, 2)}</div></div></section>`;
    },

    renderMirrorHealthReport(rec){
      if(!rec || !MirrorsUI?.getMirrorHealthEntries) return `<div class="muted">לא נמצאו נתוני הצהרת בריאות.</div>`;
      const groups = MirrorsUI.getMirrorHealthEntries(rec) || [];
      if(!groups.length) return `<div class="muted">לא נמצאו תשובות שמורות בהצהרת הבריאות.</div>`;
      return `<div class="lcReportStack">${groups.map(group => {
        const items = (group.items || []).map(item => {
          const resp = item.response || {};
          const fields = resp.fields && typeof resp.fields === 'object' ? Object.entries(resp.fields).filter(([k,v]) => safeTrim(v)) : [];
          return `<div class="lcReportListItem"><strong>${escapeHtml(item.meta?.title || 'הצהרת בריאות')} · ${escapeHtml(item.meta?.text || item.qKey || '')}</strong><span>תשובה: ${escapeHtml(resp.answer === 'yes' ? 'כן' : resp.answer === 'no' ? 'לא' : resp.answer || '—')}</span>${fields.length ? `<div class="lcReportChipRow">${fields.map(([k,v]) => `<span class="lcReportChip">${escapeHtml(this.getReportFieldLabel(k))}: ${escapeHtml(v)}</span>`).join('')}</div>` : ''}</div>`;
        }).join('');
        return `<div class="lcReportCard"><div class="lcReportSection__title">${escapeHtml(group.insured?.label || 'מבוטח')}</div><div class="lcReportSection__sub">כל התשובות שנשמרו עבור ההצהרה הרפואית של מבוטח זה.</div><div class="lcReportStack lcReportStack--tight">${items}</div></div>`;
      }).join('')}</div>`;
    },

    renderOperationalReport(){
      const payload = this.getOperationalPayload();
      const rec = this.getReportSourceRecord();
      const sourcePayload = payload && typeof payload === 'object' ? payload : {};
      const primary = sourcePayload.primary || sourcePayload.insureds?.[0]?.data || {};
      const insureds = Array.isArray(sourcePayload.insureds) ? sourcePayload.insureds : [];
      const newPolicies = Array.isArray(sourcePayload.newPolicies) ? sourcePayload.newPolicies : [];
      const insuredRows = insureds.map(ins => {
        const d = ins.data || {};
        return [
          escapeHtml(ins.label || ''),
          this.renderReportValue(((d.firstName || '') + ' ' + (d.lastName || '')).trim()),
      ...
      insureds.forEach(ins => {
        const data = ins.data || {};
        (data.existingPolicies || []).forEach(p => existingRows.push([
        ...
        const canc = data.cancellations || {};
        (data.existingPolicies || []).forEach(p => {
          const c = canc[p.id] || {};
      ...
      const newRows = newPolicies.map(p => [
      ...
      const agentNumberRows = Object.entries(sourcePayload.companyAgentNumbers || {}).map(([company, agentNo]) => [this.renderReportValue(company), this.renderReportValue(agentNo)]);
      const primaryFields = this.collectScalarReportFields(primary, {
        preferred:['firstName','lastName','idNumber','birthDate','gender','maritalStatus','phone','email','city','street','houseNumber','zip','clinic','shaban','occupation','heightCm','weightKg','bmi'],
        omit:['existingPolicies','cancellations','healthDeclaration','externalPayer','cc','ho','operationalAgentNumbers']
      });
      const insuredSections = insureds.map((ins, idx) => {
        const data = ins.data || {};
        const rows = this.collectScalarReportFields(data, {
          preferred:['firstName','lastName','idNumber','birthDate','gender','maritalStatus','phone','email','city','occupation','heightCm','weightKg','bmi'],
          omit:['existingPolicies','cancellations','healthDeclaration','externalPayer','cc','ho','operationalAgentNumbers']
        });
        const policies = Array.isArray(data.existingPolicies) ? data.existingPolicies : [];
        return `<div class="lcReportCard"><div class="lcReportSection__head"><div><div class="lcReportSection__title">${escapeHtml(ins.label || `מבוטח ${idx+1}`)}</div><div class="lcReportSection__sub">כל הפרטים שהוקלדו עבור המבוטח, אחד אחרי השני, בצורה מוכנה לעבודה.</div></div><span class="lcReportSection__badge">${policies.length} פוליסות קיימות</span></div>${this.renderPairsGrid(rows, 2)}</div>`;
      }).join('');

      const payerSections = [];
      if(primary.payerChoice === 'external' && primary.externalPayer && typeof primary.externalPayer === 'object'){
        const extRows = this.collectScalarReportFields(primary.externalPayer, { preferred:['relation','firstName','lastName','idNumber','birthDate','phone'] });
        if(extRows.length) payerSections.push(`<div class="lcReportCard"><div class="lcReportSection__title">משלם חיצוני</div><div class="lcReportSection__sub">הפרטים שיוזנו במערכת חברת הביטוח כאשר המשלם אינו המבוטח הראשי.</div>${this.renderPairsGrid(extRows, 2)}</div>`);
      }
      if(primary.paymentMethod === 'cc' && primary.cc && typeof primary.cc === 'object'){
        const ccRows = this.collectScalarReportFields(primary.cc, { preferred:['holderName','holderId','cardNumber','exp'] });
        if(ccRows.length) payerSections.push(`<div class="lcReportCard"><div class="lcReportSection__title">אמצעי תשלום · כרטיס אשראי</div>${this.renderPairsGrid(ccRows, 2)}</div>`);
      }
      if(primary.paymentMethod === 'ho' && primary.ho && typeof primary.ho === 'object'){
        const hoRows = this.collectScalarReportFields(primary.ho, { preferred:['bankName','bankNo','branch','account'] });
        if(hoRows.length) payerSections.push(`<div class="lcReportCard"><div class="lcReportSection__title">אמצעי תשלום · הוראת קבע</div>${this.renderPairsGrid(hoRows, 2)}</div>`);
      }

      const timelineEntries = [
        ['שם נציג מטפל', this.renderReportValue(rec?.agentName || Auth?.current?.name || '—')],
        ['תפקיד נציג', this.renderReportValue(rec?.agentRole || Auth?.current?.role || '—')],
        ['נוצר במערכת', this.formatReportDateTime(rec?.createdAt || sourcePayload.createdAt)],
        ['עודכן לאחרונה', this.formatReportDateTime(rec?.updatedAt || sourcePayload.createdAt)],
        ['סטטוס לקוח', this.renderReportValue(rec?.status || 'חדש')],
        ['מספר מבוטחים', this.renderReportValue(String(insureds.length || 0))]
      ];

      return `<div class="lcReportDoc">
        <div class="lcReportCard">
          ${this.renderReportBrand(primary, rec, sourcePayload)}
          <div class="lcReportHero">
            <div class="lcReportCard">
              ${this.renderSectionHead('תקציר לקוח ראשי', 'הנתונים המרכזיים שהנציג יקליד במערכת חברת הביטוח.', safeTrim(rec?.status || 'מוכן לתפעול'))}
              ${this.renderPairsGrid(primaryFields, 2)}
            </div>
            <div class="lcReportCard">
              ${this.renderSectionHead('סיכום על', 'מדדים מהירים לנציג התפעול לפני הזנה.', 'תמונה מהירה')}
              <div class="lcReportMeta">
                <div class="lcReportMetaItem"><b>מבוטחים בתיק</b><span>${insureds.length}</span></div>
                <div class="lcReportMetaItem"><b>פוליסות קיימות</b><span>${existingRows.length}</span></div>
                <div class="lcReportMetaItem"><b>פוליסות חדשות</b><span>${newRows.length}</span></div>
                <div class="lcReportMetaItem"><b>אמצעי תשלום</b><span>${this.renderReportValue(primary.paymentMethod === 'cc' ? 'כרטיס אשראי' : primary.paymentMethod === 'ho' ? 'הוראת קבע' : '—')}</span></div>
                <div class="lcReportMetaItem"><b>סוג משלם</b><span>${this.renderReportValue(primary.payerChoice === 'external' ? 'משלם חיצוני' : primary.payerChoice === 'insured' ? 'המבוטח הראשי' : '—')}</span></div>
                <div class="lcReportMetaItem"><b>תאריך הפקת דוח</b><span>${this.formatReportDateTime(sourcePayload.createdAt || rec?.updatedAt || nowISO())}</span></div>
              </div>
            </div>
          </div>
        </div>

        ${rec ? this.renderOpsSummarySection(rec) : ''}

        <section class="lcReportSection">${this.renderSectionHead('פרטי כל המבוטחים', 'כרטיס נפרד לכל מבוטח עם כל הפרטים שהוקלדו בפועל.', `${insureds.length} כרטיסים`)}<div class="lcReportStack">${insuredSections || '<div class="muted">אין מבוטחים להצגה.</div>'}</div></section>

        <section class="lcReportSection">${this.renderSectionHead('טבלת מבוטחים מרוכזת', 'לבדיקה מהירה של המבוטחים בתיק.', 'מבט טבלאי')}${this.renderTable(['סוג מבוטח','שם מלא','תעודת זהות','תאריך לידה','טלפון'], insuredRows)}</section>

        <section class="lcReportSection">${this.renderSectionHead('פוליסות קיימות', 'כל הפוליסות שהגיעו עם הלקוח ונשמרו בתיק.', `${existingRows.length} שורות`)}${this.renderTable(['מבוטח','חברה','סוג ביטוח','מספר פוליסה','פרמיה חודשית'], existingRows)}</section>

        <section class="lcReportSection">${this.renderSectionHead('ביטול בחברה נגדית', 'שיקוף מלא של סטטוס הביטולים והסיבות שסומנו.', `${cancelRows.length} פעולות`)}${this.renderTable(['מבוטח','חברה','סוג ביטוח','סטטוס','סיבה'], cancelRows)}</section>

        <section class="lcReportSection">${this.renderSectionHead('פוליסות חדשות למכירה/הנפקה', 'החלק התפעולי העיקרי להזנה למערכת חברת הביטוח.', `${newRows.length} פוליסות חדשות`)}${this.renderTable(['חברה','סוג ביטוח','פרמיה חודשית','אחוז הנחה','פרמיה אחרי הנחה','תאריך תחילה'], newRows)}</section>

        <section class="lcReportSection">${this.renderSectionHead('מספרי סוכן לפי חברה', 'מוצג רק עבור חברות שנשמר עבורן מספר סוכן בפועל.', `${agentNumberRows.length} התאמות`)}${this.renderTable(['חברה','מספר סוכן'], agentNumberRows)}</section>

        ${payerSections.length ? `<section class="lcReportSection">${this.renderSectionHead('משלם ואמצעי תשלום', 'כל הפרטים הנדרשים לחיוב במערכת חברת הביטוח.', 'גבייה')}${payerSections.join('')}</section>` : ''}

        <section class="lcReportSection">${this.renderSectionHead('הצהרת בריאות מלאה', 'כל תשובה שנשמרה עבור ההצהרה הרפואית, מסודרת לפי מבוטח.', 'בריאות')} ${rec ? this.renderMirrorHealthReport(rec) : '<div class="muted">הצהרת הבריאות תוצג כאן כאשר הדוח נפתח מתוך תיק לקוח.</div>'}</section>

        <section class="lcReportSection">${this.renderSectionHead('נתוני טיפול וזמני מערכת', 'מידע מסכם על הנציג, הלקוח והפקת הדוח.', 'לוג עבודה')}<div class="lcReportTimeline">${timelineEntries.map(([label, value]) => `<div class="lcReportTimelineItem"><b>${escapeHtml(label)}</b><span>${value}</span></div>`).join('')}</div></section>
      </div>`;
    },

    openOperationalReport(){
      if(!this.els.report || !this.els.reportBody) return;
      const payloadSnapshot = JSON.parse(JSON.stringify(this.getOperationalPayload() || {}));
      const prevPayload = this.getOperationalPayload;
      try{
        this.getOperationalPayload = () => JSON.parse(JSON.stringify(payloadSnapshot || {}));
        this.els.reportBody.innerHTML = this.renderOperationalReport();
      } finally {
        this.getOperationalPayload = prevPayload;
      }
      this.els.report.classList.add('is-open');
      this.els.report.setAttribute('aria-hidden','false');
      document.body.style.overflow = 'hidden';
      this._lastOperationalPayloadSnapshot = payloadSnapshot;
    },
    ...
    closeOperationalReport(){
      ...
      if(!document.getElementById('customerFull')?.classList.contains('is-open')) document.body.style.overflow = '';
    },

    exportOperationalPdf(){
      const body = this.els.reportBody;
      const target = body?.querySelector?.('.lcReportDoc');
      if(!target) return;
      const rec = this.getReportSourceRecord();
      const customerName = safeTrim(rec?.fullName || `${this._lastOperationalPayloadSnapshot?.primary?.firstName || ''} ${this._lastOperationalPayloadSnapshot?.primary?.lastName || ''}`.trim()) || 'client';
      const fileName = `דוח-תפעולי-${customerName.replace(/[\/:*?"<>|]/g, '-')}.pdf`;
      if(window.html2pdf){
        const opt = {
          margin: [8, 8, 8, 8],
          filename: fileName,
