...
this.els.picker = $("#lcInsuredPicker");
      ...
      on(this.els.btnNext, "click", () => {
        ...
        if(this.currentStep === 2){
         const people = this.bmiPeople();
         const map = this.state.bmiById || {};
         const total = people.length * 2;
         let done = 0;
         for(const p of people){
           const rec = map[p.id] || {};
           if(safeTrim(rec.height)) done++;
           if(safeTrim(rec.weight)) done++;
         }
         if(total <= 0) return 0;
    ...
    updateProgress(){
      this.updateAge();
      if(this.currentStep === 2) this.updateBMI();
