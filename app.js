...
(() => {
  ...
  function normalizeState(s){
    ...
    const out = {
      ...
      activityLogs: Array.isArray(s?.activityLogs) ? s.activityLogs : []
    };
    ...
    out.activityLogs = (out.activityLogs || []).map((log, idx) => normalizeActivityLogRecord(log, idx)).filter(Boolean);
  ...
  function normalizeActivityLogRecord(log, idx=0){
    const createdAt = safeTrim(log?.createdAt || log?.created_at) || nowISO();
    return {
      id: safeTrim(log?.id) || ("log_" + idx + "_" + Math.random().toString(16).slice(2)),
      createdAt,
      userName: safeTrim(log?.userName || log?.user_name) || "מערכת",
      role: safeTrim(log?.role) || "system",
      action: safeTrim(log?.action) || "אירוע",
      type: safeTrim(log?.type) || "system",
      entityId: safeTrim(log?.entityId || log?.entity_id),
      description: safeTrim(log?.description) || "",
      dayKey: safeTrim(log?.dayKey || log?.day_key) || getLocalDayKey(createdAt)
    };
  }
  ...
  function toLogActionLabel(action){
    const key = safeTrim(action).toLowerCase();
    if(key.includes('delete') || key.includes('remove') || key.includes('מחיקה') || key.includes('נמחק')) return 'מחיקה';
    if(key.includes('create') || key.includes('new') || key.includes('יצירה') || key.includes('נוצר') || key.includes('נשמר')) return 'יצירה';
    if(key.includes('login') || key.includes('logout') || key.includes('start') || key.includes('finish') || key.includes('התחבר') || key.includes('התנתק') || key.includes('התחילה') || key.includes('הסתיימה') || key.includes('שיקוף') || key.includes('תהליך')) return 'תהליך';
    return 'עדכון';
  }

  function toLogTypeLabel(type){
    const key = safeTrim(type).toLowerCase();
    if(key === 'customer') return 'לקוח';
    if(key === 'proposal') return 'הצעה';
    if(key === 'mirror') return 'שיקוף';
    if(key === 'user') return 'משתמש';
    return 'מערכת';
  }
  ...
  const Storage = {
    ...
    buildActivityLogRows(state){
      return (state?.activityLogs || []).map((log, idx) => ({
        id: safeTrim(log?.id) || ("log_" + idx),
        created_at: safeTrim(log?.createdAt) || nowISO(),
        user_name: safeTrim(log?.userName) || "מערכת",
        role: safeTrim(log?.role) || "system",
        action: safeTrim(log?.action) || "אירוע",
        type: safeTrim(log?.type) || "system",
        entity_id: safeTrim(log?.entityId),
        description: safeTrim(log?.description),
        day_key: safeTrim(log?.dayKey) || getLocalDayKey(log?.createdAt)
      }));
    },

    mapActivityLogRow(row, idx){
      return normalizeActivityLogRecord({
        id: row?.id,
        createdAt: row?.created_at,
    ...
    async loadActivityLogs(){
      try {
        const client = this.getClient();
        const { data, error } = await client.from(SUPABASE_TABLES.activityLogs).select('*').order('created_at', { ascending:false }).limit(400);
        if(error) throw error;
        return { ok:true, data: (data || []).map((row, idx) => this.mapActivityLogRow(row, idx)) };
      } catch(primaryErr) {
        try {
          const data = await this.restRequest(SUPABASE_TABLES.activityLogs + '?select=*&order=created_at.desc&limit=400', { method:'GET' });
          return { ok:true, data: (data || []).map((row, idx) => this.mapActivityLogRow(row, idx)) };
        } catch(restErr) {
          return { ok:false, error: String(restErr?.message || primaryErr?.message || restErr || primaryErr) };
        }
    ...
    async insertActivityLog(entry){
      const row = this.buildActivityLogRows({ activityLogs:[entry] })[0];
      try {
        const client = this.getClient();
        const { error } = await client.from(SUPABASE_TABLES.activityLogs).insert([row]);
        if(error) throw error;
        return { ok:true };
      } catch(primaryErr) {
        try {
          await this.restRequest(SUPABASE_TABLES.activityLogs, { method:'POST', body: row, headers:{ Prefer:'return=minimal' } });
          return { ok:true };
        } catch(restErr) {
          return { ok:false, error: String(restErr?.message || primaryErr?.message || restErr || primaryErr) };
        }
    ...
    async pruneActivityLogs(dayKey){
      const targetDay = safeTrim(dayKey) || getLocalDayKey();
      try {
        const client = this.getClient();
        const { error } = await client.from(SUPABASE_TABLES.activityLogs).delete().neq('day_key', targetDay);
        if(error) throw error;
        return { ok:true };
      } catch(primaryErr) {
        try {
          await this.restRequest(SUPABASE_TABLES.activityLogs + '?day_key=neq.' + encodeURIComponent(targetDay), { method:'DELETE', headers:{ Prefer:'return=minimal' } });
          return { ok:true };
        } catch(restErr) {
          return { ok:false, error: String(restErr?.message || primaryErr?.message || restErr || primaryErr) };
        }
  ...
  const Auth = {
    ...
    logout(){
      const leavingUser = this.current ? { ...this.current } : null;
      if(leavingUser){
        try { ActivityLog.add({ action:'logout', type:'system', description:`${leavingUser.name} התנתק מהמערכת`, userName:leavingUser.name, role:leavingUser.role }); } catch(_e) {}
      }
      this.current = null;
      try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
      try { AutoRefresh.stop(); } catch(_e) {}
      this.lock();
      UI.applyRoleUI();
      UI.goView("dashboard");
    },
    ...
    async _submit(){
      ...
      if (adminAuth.active !== false && username === safeTrim(adminAuth.username) && pin === safeTrim(adminAuth.pin)) {
        ...
        try { AutoRefresh.start(); } catch(_e) {}
        try { await ActivityLog.cleanupIfNeeded(); await ActivityLog.add({ action:'login', type:'system', description:`${this.current.name} התחבר למערכת`, userName:this.current.name, role:this.current.role }); await ActivityLog.load(); } catch(_e) {}
        UI.goView("settings");
        return;
      }
      ...
      await WelcomeLoader.play(this.current.name, 4000);
      try { AutoRefresh.start(); } catch(_e) {}
      try { await ActivityLog.cleanupIfNeeded(); await ActivityLog.add({ action:'login', type:'system', description:`${this.current.name} התחבר למערכת`, userName:this.current.name, role:this.current.role }); } catch(_e) {}
      UI.goView("dashboard");
    }
  };
  ...
  const ActivityLog = {
    ...
    normalize(entry={}){
      return normalizeActivityLogRecord({
        id: safeTrim(entry?.id) || ('log_' + Date.now().toString(16) + '_' + Math.random().toString(16).slice(2,8)),
        createdAt: entry?.createdAt || nowISO(),
        userName: safeTrim(entry?.userName || Auth?.current?.name) || 'מערכת',
        role: safeTrim(entry?.role || Auth?.current?.role) || 'system',
        action: safeTrim(entry?.action) || 'אירוע',
        type: safeTrim(entry?.type) || 'system',
        entityId: safeTrim(entry?.entityId),
        description: safeTrim(entry?.description),
        dayKey: safeTrim(entry?.dayKey) || getLocalDayKey(entry?.createdAt)
      });
    },
    async add(entry={}){
      const record = this.normalize(entry);
      State.data.activityLogs = Array.isArray(State.data?.activityLogs) ? State.data.activityLogs : [];
      State.data.activityLogs.unshift(record);
      State.data.activityLogs = State.data.activityLogs
        .filter(log => safeTrim(log?.dayKey) === getLocalDayKey())
        .sort((a,b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
        .slice(0,400);
      if(Auth.isAdmin() && UI.getCurrentView?.() === 'activityLogs') ActivityLogsUI.render();
      const result = await Storage.insertActivityLog(record);
      if(!result?.ok) console.warn('ACTIVITY_LOG_INSERT_FAILED:', result?.error || result);
      await this.cleanupIfNeeded();
      return result;
    },
    async load(){
      const result = await Storage.loadActivityLogs();
      if(result?.ok){
        State.data.activityLogs = (result.data || []).filter(log => safeTrim(log?.dayKey) === getLocalDayKey());
      }
      return result;
    },
    async cleanupIfNeeded(force=false){
      const todayKey = getLocalDayKey();
      if(!force && this.lastCleanupDayKey === todayKey) return { ok:true, skipped:true };
      this.lastCleanupDayKey = todayKey;
      const result = await Storage.pruneActivityLogs(todayKey);
      State.data.activityLogs = (Array.isArray(State.data.activityLogs) ? State.data.activityLogs : []).filter(log => safeTrim(log?.dayKey) === todayKey);
      if(Auth.isAdmin() && UI.getCurrentView?.() === 'activityLogs') ActivityLogsUI.render();
      return result;
    },
    scheduleMidnightCleanup(){
      try {
        ...
        this.cleanupTimer = window.setTimeout(async () => {
          try { await this.cleanupIfNeeded(true); } catch(_e) {}
          this.scheduleMidnightCleanup();
        }, ms);
      } catch(_e) {}
    }
  ...
  const ActivityLogsUI = {
    list(){
      const rows = Array.isArray(State.data?.activityLogs) ? State.data.activityLogs.slice() : [];
      rows.sort((a,b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
      return rows;
    },
    filtered(){
      const q = safeTrim(UI.els.activityLogsSearch?.value).toLowerCase();
      const typeFilter = safeTrim(UI.els.activityLogsFilter?.value) || 'all';
      return this.list().filter(rec => {
        if(typeFilter !== 'all' && safeTrim(rec.type) !== typeFilter) return false;
        if(!q) return true;
        return [rec.userName, rec.role, rec.action, rec.type, rec.entityId, rec.description].some(v => safeTrim(v).toLowerCase().includes(q));
      });
    },
    formatTime(value){
      try {
    ...
    roleLabel(role){
      ...
      if(key === 'admin') return 'מנהל מערכת';
      if(key === 'manager') return 'מנהל';
      if(key === 'ops') return 'תפעול';
      if(key === 'agent') return 'נציג';
      return key || 'מערכת';
    },
    actionClass(action){
      const label = toLogActionLabel(action);
      if(label === 'יצירה') return 'is-create';
      if(label === 'מחיקה') return 'is-delete';
      if(label === 'תהליך') return 'is-process';
      return 'is-update';
    },
    render(){
      if(!UI.els.activityLogsTbody) return;
      const rows = this.filtered();
      if(UI.els.activityLogsCountBadge) UI.els.activityLogsCountBadge.textContent = rows.length + ' אירועים';
      UI.els.activityLogsTbody.innerHTML = rows.length ? rows.map(rec => `
        <tr>
          <td>${escapeHtml(this.formatTime(rec.createdAt))}</td>
          <td>${escapeHtml(rec.userName || 'מערכת')}</td>
          <td>${escapeHtml(this.roleLabel(rec.role))}</td>
          <td><span class="lcLogActionBadge ${this.actionClass(rec.action)}">${escapeHtml(toLogActionLabel(rec.action))}</span></td>
          <td><span class="lcLogTypeBadge">${escapeHtml(toLogTypeLabel(rec.type))}</span></td>
          <td>${escapeHtml(rec.entityId || '—')}</td>
          <td><div class="lcLogDesc">${escapeHtml(rec.description || '—')}</div></td>
        </tr>`).join('') : `<tr><td colspan="7"><div class="lcLogs__empty">אין אירועים להצגה כרגע.</div></td></tr>`;
    },
    async refresh(){
      const result = await ActivityLog.load();
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.navActivityLogs = $("#navActivityLogs");
      ...
      this.els.activityLogsTbody = $("#activityLogsTbody");
      this.els.activityLogsSearch = $("#activityLogsSearch");
      this.els.activityLogsFilter = $("#activityLogsFilter");
      this.els.activityLogsCountBadge = $("#activityLogsCountBadge");
      this.els.btnActivityLogsRefresh = $("#btnActivityLogsRefresh");
...
// nav
      $$(".nav__item").forEach(btn => {
        on(btn, "click", () => {
          ...
          if(v === "activityLogs" && !Auth.isAdmin()) return;
      ...
      on(this.els.activityLogsSearch, "input", () => ActivityLogsUI.render());
      on(this.els.activityLogsFilter, "change", () => ActivityLogsUI.render());
      on(this.els.btnActivityLogsRefresh, "click", async () => { await ActivityLogsUI.refresh(); });
this.applyRoleUI();
      this.renderAuthPill();
    },
    ...
    applyRoleUI(){
      ...
      if (this.els.navActivityLogs) this.els.navActivityLogs.style.display = isAdmin ? "" : "none";
    ...
    getCurrentView(){
      const active = document.querySelector('.view.is-visible');
      return active ? String(active.id || '').replace(/^view-/, '') || 'dashboard' : 'dashboard';
    },
    ...
    goView(view){
      ...
      if(safe === "activityLogs" && !Auth.isAdmin()) safe = "dashboard";
      ...
      if (this.els.pageTitle) {
        const map = {
          ...
          users: "ניהול משתמשים",
          activityLogs: "לוג פעילות מערכת"
        };
        this.els.pageTitle.textContent = map[safe] || "דשבורד";
      }

      this.setActiveNav(safe);
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-discountSpec-active","view-customers-active","view-proposals-active","view-mirrors-active","view-activityLogs-active");
      document.body.classList.add("view-" + safe + "-active");

      // render view data
      if (safe === "users") UsersUI.render();
      if (safe === "activityLogs") ActivityLogsUI.render();
...
txt.textContent = "";
  ...
  const UsersUI = {
    ...
    async _saveFromModal(){
      ...
      if(isEdit){
        ...
        await ActivityLog.add({ action:"user_updated", type:"user", entityId: user.id, description:`עודכן משתמש: ${user.name || "ללא שם"}` });
      } else {
        ...
        await ActivityLog.add({ action:"user_created", type:"user", entityId: user.id, description:`נוצר משתמש חדש: ${user.name || "ללא שם"}` });
      }
    ...
    async toggleUser(id){
      ...
      await ActivityLog.add({ action: a.active ? "user_enabled" : "user_disabled", type:"user", entityId: a.id, description: `${a.active ? "הופעל" : "הושבת"} משתמש: ${a.name || "ללא שם"}` });
      this.render();
    }
  };
  ...
  const CustomersUI = {
    ...
    collectPolicies(rec){
      ...
      sourceInsureds.forEach((ins, idx) => {
        ...
        (ins?.data?.existingPolicies || []).forEach((p, pIdx) => {
          ...
          const premiumAfterDiscount = this.formatMoneyValue(this.getPolicyPremiumAfterDiscount(p));
          policies.push({
    ...
    renderPolicyCard(policy){
      ...
      const discountLine = (Number(policy.discountPct || 0) > 0 || safeTrim(policy.discountYears))
        ? `<div class="customerPolicyCard__metaLine"><span>הנחה:</span> <strong>${escapeHtml(String(policy.discountPct || "0"))}%${safeTrim(policy.discountYears) ? ` · ${escapeHtml(policy.discountYears)} שנים` : ""}</strong></div><div class="customerPolicyCard__metaLine"><span>פרמיה אחרי הנחה:</span> <strong>${escapeHtml(policy.premiumAfterDiscount || "—")}</strong></div>`
        : `<div class="customerPolicyCard__metaLine"><span>פרמיה אחרי הנחה:</span> <strong>${escapeHtml(policy.premiumAfterDiscount || policy.premiumText || "—")}</strong></div>`;
      const bottomLine = safeTrim(policy.startDate)
        ? `${discountLine}<div class="customerPolicyCard__metaLine"><span>תחילה:</span> <strong>${escapeHtml(policy.startDate)}</strong></div>`
        : (safeTrim(policy.policyNumber) ? `${discountLine}<div class="customerPolicyCard__metaLine"><span>מס׳ פוליסה:</span> <strong>${escapeHtml(policy.policyNumber)}</strong></div>` : `${discountLine}<div class="customerPolicyCard__metaLine customerPolicyCard__metaLine--muted">${escapeHtml(policy.insuredLabel || "")}</div>`);
    ...
    getUiState(){
      const policyModalOpen = !!(this.policyModal?.wrap && this.policyModal.wrap.classList.contains("is-open"));
      const policyTitle = safeTrim(this.policyModal?.title?.textContent || "");
      return {
        currentId: safeTrim(this.currentId),
        currentSection: safeTrim(this.currentSection || "wallet") || "wallet",
        bodyScrollTop: this.els.body ? this.els.body.scrollTop : 0,
        policyModalOpen,
        policyTitle
      };
    },

    restoreUiState(rec, snapshot={}){
      if(!rec) return;
      const state = snapshot && typeof snapshot === 'object' ? snapshot : {};
      const wantPolicyModal = !!state.policyModalOpen;
      const policyTitle = safeTrim(state.policyTitle || "");
      if(wantPolicyModal){
        const policies = this.collectPolicies(rec);
        const policy = policies.find(x => {
          const title = `${safeTrim(x.company || "חברה")} · ${safeTrim(x.type || "פוליסה")}`;
          return title === policyTitle;
        }) || null;
        if(policy) this.openPolicyModal(rec, policy);
      }
      if(this.els.body && Number.isFinite(Number(state.bodyScrollTop))){
        this.els.body.scrollTop = Number(state.bodyScrollTop) || 0;
      }
    },
    ...
    openById(id, opts={}){
      ...
      const preserveSection = options.preserveSection === true;
      const preserveScroll = options.preserveScroll === true;
      const preserveUiState = options.preserveUiState && typeof options.preserveUiState === 'object' ? options.preserveUiState : null;
      const prevSection = preserveSection ? safeTrim(this.currentSection || 'wallet') || 'wallet' : 'wallet';
      const prevScrollTop = preserveScroll && this.els.body ? this.els.body.scrollTop : 0;
      const uiSnapshot = preserveUiState || {
        currentSection: prevSection,
        bodyScrollTop: prevScrollTop,
      ...
      try{
        if(Wizard?.isOpen && !options.silentRefresh) Wizard.close();
        this.currentId = rec.id;
        const policies = this.collectPolicies(rec);
        const stats = this.getStats(rec, policies);
        this.currentSection = uiSnapshot.currentSection || 'wallet';
        ...
        this.els.wrap.setAttribute("aria-hidden","false");
        document.body.style.overflow = "hidden";
        this.restoreUiState(rec, uiSnapshot);
      }catch(err){
  ...
  const ArchiveCustomerUI = {
    ...
    async confirm(){
      ...
      if(r?.ok){ await ActivityLog.add({ action:"customer_deleted", type:"customer", entityId: id, description:`לקוח נמחק מהמערכת: ${rec.fullName || "ללא שם"}` }); }
      if(!r?.ok){
  ...
  const ProposalsUI = {
    ...
    async deleteById(id){
      ...
      await ActivityLog.add({ action:"proposal_deleted", type:"proposal", entityId: id, description:`הצעה נמחקה: ${rec.fullName || "ללא שם"}` });
      this.render();
    }
  };
...
init(){
    ...
    async startCall(){
      ...
      this.render();
      this.startTimerLoop();
      const __mirrorStartPersist = await App.persist('שיחת שיקוף התחילה');
      if(__mirrorStartPersist?.ok){ await ActivityLog.add({ action:'mirror_started', type:'mirror', entityId: rec.id, description:`התחילה שיחת שיקוף ללקוח: ${rec.fullName || 'ללא שם'}` }); }
    },
    ...
    async finishCall(){
      ...
      this.stopTimerLoop();
      this.render();
      const __mirrorFinishPersist = await App.persist('שיחת שיקוף הסתיימה');
      if(__mirrorFinishPersist?.ok){ await ActivityLog.add({ action:'mirror_finished', type:'mirror', entityId: rec.id, description:`הסתיימה שיחת שיקוף ללקוח: ${rec.fullName || 'ללא שם'}` }); }
...
// ---------- Step 6 ----------
    ...
    async saveDraft(){
      ...
      if(persistRes?.ok){
        await ActivityLog.add({ action: idx >= 0 ? "proposal_updated" : "proposal_created", type:"proposal", entityId: record.id, description: `${idx >= 0 ? "עודכנה" : "נשמרה"} הצעה: ${record.fullName || "ללא שם"} · שלב ${record.currentStep || 1}` });
      }
      ProposalsUI.render();
      if(persistRes?.ok){
    ...
    async saveCompletedCustomer(){
      ...
      State.data.meta.updatedAt = nowISO();
      const __persistCustomer = await App.persist("הלקוח נשמר");
      if(__persistCustomer?.ok){
        await ActivityLog.add({ action:"customer_created", type:"customer", entityId: record.id, description:`נוצר לקוח חדש: ${record.fullName || "ללא שם"}` });
      }
      return record;
    },
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const MirrorsUI = {
    ...
    init(){
      ...
      on(this.els.verifyCard, "click", async (ev) => {
        ...
        if(saveEditBtn){
          ...
          if(result?.ok){ await ActivityLog.add({ action:'customer_updated', type:'customer', entityId: rec.id, description:`עודכנו נתוני לקוח בשיקוף: ${rec.fullName || 'ללא שם'}` }); }
          this.render();
    ...
    async startCall(){
      ...
      this.render();
      this.startTimerLoop();
      const __mirrorStartPersist2 = await App.persist('שיחת שיקוף התחילה');
      if(__mirrorStartPersist2?.ok){ await ActivityLog.add({ action:'mirror_started', type:'mirror', entityId: rec.id, description:`התחילה שיחת שיקוף ללקוח: ${rec.fullName || 'ללא שם'}` }); }
    },
    ...
    async finishCall(){
      ...
      this.stopTimerLoop();
      this.render();
      const __mirrorFinishPersist2 = await App.persist('שיחת שיקוף הסתיימה');
      if(__mirrorFinishPersist2?.ok){ await ActivityLog.add({ action:'mirror_finished', type:'mirror', entityId: rec.id, description:`הסתיימה שיחת שיקוף ללקוח: ${rec.fullName || 'ללא שם'}` }); }
      alert(`שיחת השיקוף נשמרה. תאריך: ${store.dateFull} · התחלה: ${store.startTime} · משך: ${store.durationText}`);
    },
...
const SystemRepairUI = {
    ...
    safeCloseKnownLayers(){
      ...
      ["lcForgotModal", "lcUserModal", "customerFull", "customerPolicyModal", "lcArchiveCustomerModal"].forEach((id) => {
        const el = document.getElementById(id);
        if(!el) return;
        el.classList.remove("is-open");
        el.setAttribute("aria-hidden", "true");
      });
      ["mirrorsSearchModal", "mirrorsStartModal"].forEach((id) => {
        const el = document.getElementById(id);
        if(!el) return;
        el.hidden = true;
        el.classList.remove("is-open");
    ...
    releaseUiLocks(){
      ...
      try { document.body.classList.remove("is-loading", "is-busy", "modal-open", "lcBusy", "appBusy"); } catch(_e) {}
  ...
  const App = {
    ...
    async boot(){
      ...
      try { await ActivityLog.cleanupIfNeeded(); } catch(_e) {}
      if(Auth.isAdmin()){
        try { await ActivityLog.load(); } catch(_e) {}
      }
    ...
    async reloadSessionState(opts={}){
      ...
      const preserveCustomerOpen = options.preserveCustomerOpen !== false;
      const currentView = keepView ? UI.getCurrentView?.() : "";
      const openCustomerId = preserveCustomerOpen ? safeTrim(CustomersUI.currentId) : "";
      const wasCustomerOpen = !!(preserveCustomerOpen && CustomersUI.els?.wrap && CustomersUI.els.wrap.classList.contains('is-open') && openCustomerId);
      const customerUiState = wasCustomerOpen ? CustomersUI.getUiState() : null;
      ...
      if (r.ok) {
        ...
        if(keepView && currentView) {
          try { UI.goView(currentView); } catch(_e) {}
        }
        if(wasCustomerOpen){
          const updatedRec = CustomersUI.byId(openCustomerId);
          if(updatedRec){
            CustomersUI.openById(openCustomerId, {
              skipLoader:true,
              preserveSection:true,
              preserveScroll:true,
              preserveUiState: customerUiState,
              silentRefresh:true
            });
    ...
    async syncNow(){
      ...
      const r = await this.reloadSessionState({ preserveCustomerOpen:true, keepView:true });
      if (r.ok) {
