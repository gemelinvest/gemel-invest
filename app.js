    ...
    renderOperationalReport(payloadOverride){
      ...
      insureds.forEach(ins => {
        ...
        existingPolicies.forEach((p) => {
          const statusKey = safeTrim(cancellations?.[p?.id]?.status);
          const statusLabel = this.getExistingPolicyStatusLabel(statusKey);
          const tone = statusKey === 'cancel' || statusKey === 'full' ? 'danger' : statusKey === 'partial_health' || statusKey === 'replace' ? 'warn' : 'success';
          existingRows.push([
            ...
            this.renderReportStatusBadge(statusLabel, tone)
          ]);
        });
      });
      ...
      return `<div class="lcReportDoc lcReportDoc--linear"><section class="lcReportHero"><div class="lcReportHero__top"><div><div class="lcReportHero__eyebrow">GEMEL INVEST · דוח תפעולי</div><h2 class="lcReportHero__title">${escapeHtml(customerFullName)}</h2><div class="lcReportHero__sub">מסמך תפעולי מסודר לפי סדר עבודה: פרטי לקוח, פוליסות קיימות, פוליסות חדשות, אמצעי תשלום והצהרת בריאות.</div></div><div class="lcReportHero__meta">${this.renderReportRowsList(reportSummaryRows, { dense:true })}</div></div></section>
        <section class="lcReportSection"><div class="lcReportSection__title">פרטי לקוח</div><div class="lcReportSection__sub">כל פרטי הלקוח בצורה נקייה וממוקדת, ללא קוביות.</div>${this.renderReportRowsList(customerRows, { dense:true })}</section>
        <section class="lcReportSection"><div class="lcReportSection__title">מבוטחים בתיק</div><div class="lcReportSection__sub">פירוט כל המבוטחים שהוזנו בתהליך.</div><div class="lcReportStack">${insureds.length ? insureds.map((ins, index) => { const d = ins?.data && typeof ins.data === 'object' ? ins.data : {}; const rows = [['שם מבוטח', this.renderReportValue(safeTrim(((d.firstName || '') + ' ' + (d.lastName || '')).trim()) || ins?.label)], ['סוג מבוטח', this.renderReportValue(ins?.type === 'primary' ? 'ראשי' : ins?.type === 'spouse' ? 'בן/בת זוג' : ins?.type === 'child' ? 'קטין' : 'מבוטח')], ['תעודת זהות', this.renderReportValue(d.idNumber)], ['תאריך לידה', this.renderReportValue(d.birthDate)], ['טלפון', this.renderReportValue(d.phone)], ['עיר', this.renderReportValue(d.city)], ['קופת חולים', this.renderReportValue(d.clinic || d.hmo || d.kupatHolim)], ['שב״ן', this.renderReportValue(d.shaban || d.shabanLevel)]]; return `<article class="lcReportPolicyBlock"><div class="lcReportPolicyBlock__head"><div><div class="lcReportPolicyBlock__title">${escapeHtml(safeTrim(ins?.label) || `מבוטח ${index + 1}`)}</div></div>${this.renderReportStatusBadge(ins?.type === 'primary' ? 'ראשי' : ins?.type === 'spouse' ? 'בן/בת זוג' : ins?.type === 'child' ? 'קטין' : 'מבוטח', 'neutral')}</div>${this.renderReportRowsList(rows, { dense:true })}</article>`; }).join('') : '<div class="muted">לא נמצאו מבוטחים.</div>'}</div></section>
        <section class="lcReportSection"><div class="lcReportSection__title">פוליסות קיימות</div><div class="lcReportSection__sub">כולל סטטוס הטיפול שנבחר לכל פוליסה.</div>${this.renderTable(['מבוטח','חברה','סוג ביטוח','מספר פוליסה','פרמיה חודשית','סטטוס'], existingRows)}</section>
        <section class="lcReportSection"><div class="lcReportSection__title">פוליסות חדשות</div><div class="lcReportSection__sub">כל פוליסה מוצגת בפירוט מלא, כולל כיסויים, הנחה ושיעבוד במידת הצורך.</div><div class="lcReportStack">${newPolicyBlocks.length ? newPolicyBlocks.join('') : '<div class="muted">לא נוספו פוליסות חדשות.</div>'}</div></section>
        ...
        <section class="lcReportSection"><div class="lcReportSection__title">אמצעי תשלום</div><div class="lcReportSection__sub">כולל פרטי משלם אם הוגדר משלם חיצוני.</div>${this.renderReportRowsList(payerRows, { dense:true, emptyText:'לא הוזנו פרטי תשלום.' })}</section>
        <section class="lcReportSection"><div class="lcReportSection__title">הצהרת בריאות</div><div class="lcReportSection__sub">מופיעות גם תשובות של כן וגם תשובות של לא, כפי שנענו בפועל.</div><div class="lcReportHealthList">${healthItems.length ? healthItems.join('') : '<div class="muted">לא נמצאו תשובות הצהרת בריאות להצגה.</div>'}</div></section></div>`;
    },
