...
// open health covers drawer (Health only)
    ...
    uniqSortedStrings(list){
      return Array.from(new Set((Array.isArray(list) ? list : []).map(x => safeTrim(x)).filter(Boolean)))
        .sort((a,b) => a.localeCompare(b, 'he'));
    },

    async ensureLocalitiesLoaded(){
      if(Array.isArray(this.localities) && this.localities.length) return this.localities;
      this.localities = this.uniqSortedStrings(this.localityFallback || []);
      if(this.localitiesPromise) return this.localitiesPromise;
      const refreshCityUI = () => {
        try{
          const input = this.els?.body ? $('#lcLocalityInput', this.els.body) : null;
          const menu = this.els?.body ? $('#lcLocalityMenu', this.els.body) : null;
          if(input && menu){
            menu.innerHTML = this.renderLocalitySuggestions(input.value, input.value);
          }
        }catch(_e){}
      };
      this.localitiesPromise = (async () => {
        try{
          const apiUrl = 'https://data.gov.il/api/3/action/package_search?q=' + encodeURIComponent('רשימת ישובים בישראל') + '&rows=5';
          const metaRes = await fetch(apiUrl, { cache:'no-store' });
          if(!metaRes.ok) throw new Error('LOCALITIES_META_' + metaRes.status);
          const meta = await metaRes.json();
          const results = Array.isArray(meta?.result?.results) ? meta.result.results : [];
          const resources = results.flatMap(r => Array.isArray(r?.resources) ? r.resources : []);
          const csvRes = resources.find(r => /csv/i.test(String(r?.format || '')) && /^https?:\/\//i.test(String(r?.url || '')));
          const csvUrl = String(csvRes?.url || '');
          if(!csvUrl) throw new Error('LOCALITIES_RESOURCE_NOT_FOUND');
          const csvText = await fetch(csvUrl, { cache:'no-store' }).then(r => {
            if(!r.ok) throw new Error('LOCALITIES_CSV_' + r.status);
            return r.text();
          });
          const lines = csvText.split(/\r?\n/).map(x => x.trim()).filter(Boolean);
          if(!lines.length) throw new Error('LOCALITIES_EMPTY_CSV');
          const header = this.parseCsvLine(lines[0]).map(h => this.normalizeLocalitySearch(h));
          const headerCandidates = ['שם ישוב','שם יישוב','ישוב','יישוב','locality name','locality','name'];
          let idx = -1;
          headerCandidates.some(c => {
            idx = header.indexOf(this.normalizeLocalitySearch(c));
            return idx >= 0;
          });
          if(idx < 0) idx = Math.max(header.findIndex(h => h.includes('ישוב') || h.includes('יישוב')), 0);
          const rows = [];
          for(let i=1;i<lines.length;i++){
            const cols = this.parseCsvLine(lines[i]);
            const value = safeTrim(cols[idx]);
            if(!value) continue;
            if(/^[A-Za-z0-9 _-]+$/.test(value)) continue;
            if(value.length < 2) continue;
            rows.push(value.replace(/\s+/g, ' '));
          }
          const merged = this.uniqSortedStrings([...(this.localityFallback || []), ...rows]);
          if(merged.length){
            this.localities = merged;
            this.localitiesLoadedAt = nowISO();
            refreshCityUI();
          }
        }catch(err){
          console.warn('LOCALITIES_LOAD_FAILED:', err?.message || err);
        }
    ...
    getLocalitySuggestions(term){
      ...
      const list = Array.isArray(this.localities) && this.localities.length ? this.localities : this.uniqSortedStrings(this.localityFallback || []);
    ...
    renderLocalitySuggestions(term, currentValue){
      ...
      if(!items.length){
        return `<button type="button" class="lcLocalityOption is-empty" data-locality-empty="1">לא נמצאה התאמה. אפשר להקליד יישוב ידנית.</button>`;
      }
    ...
    bindLocalityAutocomplete(ins){
      ...
      const nativeList = $('#lcLocalityNativeList', this.els.body);
      if(!input || !menu) return;
      if(input.dataset.localityBound === '1') return;
      input.dataset.localityBound = '1';

      const refreshNativeList = (items) => {
        if(!nativeList) return;
        const rows = Array.isArray(items) ? items : this.getLocalitySuggestions(input.value || '');
        nativeList.innerHTML = rows.slice(0, 80).map(item => `<option value="${escapeHtml(item)}"></option>`).join('');
      };
      ...
      const renderMenu = () => {
        ...
        const items = this.getLocalitySuggestions(term);
        menu.innerHTML = this.renderLocalitySuggestions(term, ins.data.city || '');
        refreshNativeList(items);
        const shouldOpen = !!items.length && (document.activeElement === input || safeTrim(term));
      ...
      this.ensureLocalitiesLoaded().then(() => {
        if(input.isConnected && menu.isConnected) renderMenu();
      }).catch(() => {
        renderMenu();
      });

      renderMenu();
      on(input, 'focus', () => { requestAnimationFrame(() => renderMenu()); });
      on(input, 'click', () => { requestAnimationFrame(() => renderMenu()); });
      on(input, 'input', () => { syncLocality(); renderMenu(); });
      on(input, 'change', () => { syncLocality(); renderMenu(); });
      on(input, 'blur', () => {
        setTimeout(() => {
          if(document.activeElement !== input && !menu.contains(document.activeElement)) closeMenu();
        }, 120);
      });
      on(input, 'keydown', (ev) => {
        let options = $$('[data-locality-value]', menu);
        let current = menu.querySelector('.lcLocalityOption.is-hover');
        let idx = current ? options.indexOf(current) : -1;
        if(ev.key === 'ArrowDown'){
          ev.preventDefault();
          if(!menu.classList.contains('is-open')) renderMenu();
          options = $$('[data-locality-value]', menu);
        ...
        if(ev.key === 'Tab'){
          const picked = menu.querySelector('.lcLocalityOption.is-hover') || menu.querySelector('[data-locality-value]');
          if(picked && safeTrim(input.value)) choose(picked.getAttribute('data-locality-value') || picked.textContent || '');
          return;
        }
      ...
      on(document, 'pointerdown', (ev) => {
        ...
        const inside = ev.target === input || input.contains(ev.target) || menu.contains(ev.target);
        if(!inside) closeMenu();
      });
    },
      ...
      questions.push(
        ...
        },
        {
          ...
          render:() => isChild
            ...
            : `<div class="field"><label class="label">יישוב</label><div class="lcLocalityWrap"><input class="input lcLocalityInput" id="lcLocalityInput" type="text" data-bind="city" value="${escapeHtml(d.city || '')}" placeholder="התחל להקליד יישוב…" autocomplete="off" list="lcLocalityNativeList" aria-autocomplete="list" aria-expanded="false" /><datalist id="lcLocalityNativeList"></datalist><div class="lcLocalityMenu" id="lcLocalityMenu">${this.renderLocalitySuggestions(d.city || '', d.city || '')}</div></div><div class="help">חיפוש חכם לפי יישובים בישראל. אם לא נמצאה התאמה, אפשר להמשיך להקליד ידנית.</div></div>`
        },
        {
          key:'street',
          title:'מה שם הרחוב?',
...
const SystemRepairUI = {
  ...
  const AutoRefresh = {
    ...
    init(){
      ...
      window.addEventListener("focus", () => this.tick(true));
    },

    start(){
      this.stop();
      this.timer = window.setInterval(() => this.tick(false), this.intervalMs);
    },
    ...
    shouldRun(){
      ...
      const currentView = this.getCurrentView();
      if(currentView === "customers" || currentView === "proposals") return true;
      if(CustomersUI?.els?.wrap?.classList?.contains("is-open")) return true;
      return false;
    },
    ...
    async tick(force=false){
      ...
      this.busy = true;
      try{
        const previousMeta = safeTrim(State?.data?.meta?.updatedAt || "");
        const currentView = this.getCurrentView();
        const openCustomerId = safeTrim(CustomersUI?.currentId || "");
        const customerOpen = !!CustomersUI?.els?.wrap?.classList?.contains("is-open");

        const res = await Storage.loadSheets();
        if(!res?.ok) return;

        const nextState = normalizeState(res.payload || {});
        const nextMeta = safeTrim(nextState?.meta?.updatedAt || "");
        const changed = force || !previousMeta || (nextMeta && nextMeta !== previousMeta);
        if(!changed) return;

        State.data = nextState;
        Storage.saveBackup(State.data);
        this.lastPulledAt = nextMeta || nowISO();
        UI.renderSyncStatus("התעדכן אוטומטית", "ok", this.lastPulledAt);

        if(Auth.isAdmin()) UsersUI.render();
        if(currentView === "customers") CustomersUI.render();
        if(currentView === "proposals") ProposalsUI.render();
        if(currentView === "mirrors") MirrorsUI.render();
        if(customerOpen && openCustomerId){
          const stillExists = CustomersUI.byId(openCustomerId);
          if(stillExists) CustomersUI.openById(openCustomerId, { silentRefresh:true });
          else CustomersUI.close();
        }
      }catch(err){
        console.error("AUTO_REFRESH_FAILED:", err);
      }finally{
        this.busy = false;
      }
    }
  ...
  LeadShellUI.init();
  AutoRefresh.init();
  App._bootPromise = App.boot().then((res) => {
    try { AutoRefresh.tick(true); } catch(_e) {}
    return res;
  });
