...
(() => {
  ...
  const Storage = {
    ...
    async syncTable(tableName, rows){
      ...
      const nextIds = new Set((rows || []).map(r => safeTrim(r?.id)).filter(Boolean));
    ...
    async loadSheets(){
      try {
        ...
        if(firstErr) return { ok:false, error: String(firstErr.message || firstErr) };

        const payload = normalizeState({
          meta: this.mapMeta(metaRes.data || {}),
