...
(() => {
  ...
  const BUILD = "20260406-bankgoal-premium-v52";
  ...
  const Storage = {
    ...
    buildAgentRows(state){
      return (state?.agents || []).map((a, idx) => ({
        ...
        monthly_sales_target: Number(String(a?.monthlySalesTarget ?? a?.monthly_sales_target ?? '').replace(/[^\d.-]/g, '')) || 0,
...
this.els.syncDot = $("#syncDot");
      ...
      this.els.navSimulators = $("#navSimulators");
...
this.applyRoleUI();
    ...
    applyRoleUI(){
      ...
      if (this.els.navSimulators) this.els.navSimulators.style.display = Auth.current ? "" : "none";
    ...
    goView(view){
      ...
      if(safe === "simulators" && !Auth.current) safe = "dashboard";
      // hide all views
      ...
      document.body.classList.remove("view-users-active","view-dashboard-active","view-settings-active","view-discountSpec-active","view-simulators-active","view-customers-active","view-proposals-active","view-myProcesses-active","view-mirrors-active");
      document.body.classList.add("view-" + safe + "-active");

      // render view data
      if (safe === "dashboard") DashboardUI.render();
      if (safe === "users") UsersUI.render();
      if (safe === "simulators") SimHub.render();
  ...
  const DashboardUI = {
    ...
    renderLoading(){
      ...
      this.els.root.innerHTML = `
        <section class="bankDash bankDash--empty">
          <div class="bankDash__hero card">
            <div class="bankDash__heroTitle">טוען נתוני דשבורד מהשרת…</div>
            <div class="bankDash__heroMeta">המערכת מושכת כעת מ־Supabase את נתוני המכירות, הלקוחות, ההצעות והיעד החודשי של המשתמש המחובר בלבד.</div>
          </div>
        </section>`;
    },

    async refreshLiveState(options = {}){
      const minIntervalMs = Number(options.minIntervalMs || 15000);
      const now = Date.now();
      if(!options.force && this._lastLiveRefreshAt && (now - this._lastLiveRefreshAt) < minIntervalMs){
        return { ok:true, skipped:true };
      }
      if(this._liveRefreshPromise) return this._liveRefreshPromise;
      this._liveRefreshPromise = (async () => {
        ...
        return { ok:false, error: safeTrim(res?.error || 'LIVE_REFRESH_FAILED') };
      })();
      try {
        return await this._liveRefreshPromise;
      } finally {
        this._liveRefreshPromise = null;
      }
    },
    ...
    buildMetrics(){
      ...
      const completedCustomerPredicate = (rec) => {
        const payload = rec?.payload && typeof rec.payload === 'object' ? rec.payload : {};
        const policies = CustomersUI.collectPolicies(rec).filter((p) => String(p?.origin || '') === 'new');
        const hasNewPolicies = policies.length > 0;
        const netPremium = policies.reduce((sum, p) => sum + (Number(p?.premiumAfterDiscountValue) || 0), 0);
        const finishCompleted = payload?.wizardCompleted === true || payload?.isCompleted === true || payload?.flowCompleted === true || !!payload?.completedAt || !!payload?.finishedAt;
        return !!(hasNewPolicies && (netPremium > 0 || finishCompleted));
      };

      const customersMonth = customersAll.filter((rec) => this.isWithinRange(this.resolveCustomerMonthStamp(rec), currentRange) && completedCustomerPredicate(rec));
      const customersPrev = customersAll.filter((rec) => this.isWithinRange(this.resolveCustomerMonthStamp(rec), previousRange) && completedCustomerPredicate(rec));
    ...
    renderEmpty(){
      ...
      this.els.root.innerHTML = `
        <section class="bankDash bankDash--empty">
          <div class="bankDash__hero card">
            <div class="bankDash__heroTitle">ביצועי החודש — ${escapeHtml(this.formatMonthTitle())}</div>
            <div class="bankDash__heroMeta">הדשבורד הבנקאי החדש מוכן. ברגע שיתווספו נתונים יוצגו כאן KPI, יעד חודשי, גרף פעילות ופעילות אחרונה.</div>
          </div>
    ...
    async render(options = {}){
      ...
      if(!options.skipLive){
        this.renderLoading();
        const liveRes = await this.refreshLiveState({ force: !!options.forceLive });
        if(!liveRes?.ok && !liveRes?.skipped){
          console.warn('DASHBOARD_LIVE_REFRESH_FAILED:', liveRes?.error || liveRes);
        }
      }
      ...
      this.els.root.innerHTML = `
        <section class="bankDash">
          <div class="bankDash__hero card">
            <div class="bankDash__heroTop">
              <div>
                <div class="bankDash__heroKicker">🔥 ${escapeHtml(metrics.monthLabel)}</div>
                <div class="bankDash__heroTitle">ביצועי החודש — ${escapeHtml(metrics.monthLabel)}</div>
              </div>
              <div class="bankDash__heroBadge">GEMEL INVEST • FINANCIAL BOARD</div>
            </div>
            <div class="bankDash__heroMeta">
          ...
          <div class="bankDash__kpis">
            ${cards.map((card) => {
              ...
              return `
                <article class="bankKpi card">
                  <div class="bankKpi__top">
                    <div class="bankKpi__en">${escapeHtml(card.en)}</div>
          ...
          <article class="bankGoal card bankGoal--${metrics.targetTone}">
            <div class="bankGoal__titleRow">
              <div class="bankGoal__title">ביצועים מול יעד</div>
            </div>
            <div class="bankGoal__shell">
              <div class="bankGoal__details">
                <div class="bankGoal__mainNumbers">
                  <div class="bankGoal__mainStat bankGoal__mainStat--primary">
                    <span>החודש</span>
                    <strong>${escapeHtml(this.formatMoney(metrics.netPremium))}</strong>
                    <small>${Number(metrics.netDelta) >= 0 ? `▲ ${escapeHtml(this.formatPct(Math.abs(Number(metrics.netDelta) || 0)))} מול חודש קודם` : `▼ ${escapeHtml(this.formatPct(Math.abs(Number(metrics.netDelta) || 0)))} מול חודש קודם`}</small>
                  </div>
                  <div class="bankGoal__mainStat">
                    <span>יעד חודשי:</span>
                    <strong>${metrics.targetValue > 0 ? escapeHtml(this.formatMoney(metrics.targetValue)) : '₪0'}</strong>
                    <small>${metrics.targetValue > 0 ? `חסר ${escapeHtml(this.formatMoney(metrics.targetRemaining))}` : `טרם הוגדר יעד`}</small>
                  </div>
                </div>
                <div class="bankGoal__miniCols">
                  <div class="bankGoal__miniStat">
                    <span>ברוטו כולל:</span>
                    <strong>${escapeHtml(this.formatMoney(metrics.grossPremium))}</strong>
                    <small>${Number(metrics.netDelta) >= 0 ? `בכמות ${escapeHtml(this.formatPct(Math.abs(Number(metrics.netDelta) || 0)))}+` : `בכמות ${escapeHtml(this.formatPct(Math.abs(Number(metrics.netDelta) || 0)))}-`}</small>
                  </div>
                  <div class="bankGoal__miniStat">
                    <span>נטו חודשי:</span>
                    <strong>${escapeHtml(this.formatMoney(metrics.netPremium))}</strong>
              ...
              <div class="bankGoal__divider" aria-hidden="true"></div>
              <div class="bankGoal__meter">
                <div class="bankGoal__meterTop">
                  <div class="bankGoal__percentLabel">מול היעד</div>
                  <div class="bankGoal__percent">${escapeHtml(this.formatPct(metrics.targetPct))}</div>
                </div>
                <div class="bankGoal__progressTrack">
                  <span style="width:${Math.min(metrics.targetPct, 100)}%"></span>
                  <i class="bankGoal__targetGlow" aria-hidden="true"></i>
                </div>
                <div class="bankGoal__metaRow">
                  <span>₪${escapeHtml(this.numberFmt(Math.round(Number(metrics.targetValue || 0))))}</span>
                  <strong>${escapeHtml(this.formatPct(metrics.targetPct))}</strong>
                </div>
              </div>
            </div>
            <div class="bankGoal__foot">${metrics.targetValue > 0 ? `היעד מוגדר עבור המשתמש המחובר על ידי מנהל המערכת` : `טרם הוגדר יעד חודשי על ידי מנהל המערכת`}</div>
          </article>
...
// open health covers drawer (Health only)
      ...
      return `<div class="lcElementaryWrap">
        ...
        <div class="formGrid lcElementaryFormGrid lcElementaryFormGrid--clean">
          ...
          <div class="field lcElementaryDateField">
            ...
            <input class="input lcElementaryDateInput" type="date" data-bind="birthDate" value="${escapeHtml(d.birthDate || '')}" max="${this.getTodayIsoDate()}" />
            ...
            <div class="lcStep1Group">
              <div class="lcStep1Group__grid lcStep1Group__grid--personal">
                <div class="field lcStep1DateField">
                  <label class="label">תאריך לידה</label>
                  <input class="input lcStep1DateInput" type="date" data-bind="birthDate" value="${escapeHtml(d.birthDate || '')}" max="${this.getTodayIsoDate()}" />
    ...
    renderExistingPolicyCancellationControls(policy, cancellation = {}, options = {}){
      ...
      const policyCovers = Array.isArray(p.covers) ? p.covers.map(v => safeTrim(v)).filter(Boolean) : [];
      const hasPolicyCovers = policyCovers.length > 0;
      ...
      const partialDetails = safeTrim(c.partialDetails || c.partialText || "");
        ...
        <div class="${wrapCls}">
          ...
          ${showAnnex ? `
            <div class="lcPolicyActionBox__annex">
              <div class="lcPolicyActionBox__annexTitle">פירוט ביטול חלקי</div>
              ${hasPolicyCovers ? `
                <div class="help" style="margin-bottom:10px">סמן רק את הכיסויים הרלוונטיים לביטול מתוך הפוליסה הקיימת.</div>
                <div class="lcPolicyActionBox__checks">
                  ${policyCovers.map((a) => `
                    <label class="row" style="gap:8px">
                      <input type="checkbox" data-cancel-policy="${escapeHtml(p.id || '')}" data-cancel-key="att:${escapeHtml(a)}" ${(c.attachments && c.attachments[a]) ? "checked" : ""} />
              ...
              ` : `
                <div class="field">
                  <label class="label">מה מבוטל בפוליסה</label>
                  <textarea class="input lcPolicyActionBox__textarea" data-cancel-policy="${escapeHtml(p.id || '')}" data-cancel-key="partialDetails" placeholder="לדוגמה: נספח אמבולטורי / ריידר מסוים">${escapeHtml(partialDetails)}</textarea>
                  <div class="help">כשאין כיסויים מזוהים מהקובץ, כתוב כאן ידנית מה מבוטל חלקית.</div>
                </div>
              `}
            </div>
          ` : ``}
    ...
    savePolicyDiscountModal(){
      ...
      this.closePolicyDiscountModal();
      this.render();
      this.setHint("ההנחה, דירוג השנים והטבת ההצטרפות נשמרו בהצלחה בפוליסה.");
    },
...
// ---------- Step 6 ----------
    ...
    getExistingPolicyStatusMeta(policy, insuredData){
      ...
      const attachmentMap = cancel.attachments && typeof cancel.attachments === 'object' ? cancel.attachments : {};
      const selectedAttachments = Object.keys(attachmentMap).filter((key) => attachmentMap[key]).map(v => safeTrim(v)).filter(Boolean);
      const annexes = Array.isArray(cancel.annexes) ? cancel.annexes.filter(Boolean).map(v => safeTrim(v)).filter(Boolean) : [];
      const partialItems = selectedAttachments.length ? selectedAttachments : annexes;
      if(partialItems.length) partialDetails.push(`כיסויים לביטול: ${partialItems.join(', ')}`);
    ...
    async saveDraft(){
      ...
      const verify = await Storage.loadSingleRow(SUPABASE_TABLES.proposals, record.id);
      if(!verify?.ok || !verify?.data){
        const msg = 'ההצעה לא אומתה מול Supabase לאחר השמירה.';
        this.setHint(msg);
        SaveStatusUI.error('שמירת ההצעה נכשלה', msg);
        return;
      }
    ...
    async saveCompletedCustomer(){
      ...
      payload.completedBy = safeTrim(Auth?.current?.name);
      const primary = payload?.primary || {};
      ...
      const verify = await Storage.loadSingleRow(SUPABASE_TABLES.customers, record.id);
      if(!verify?.ok || !verify?.data) throw new Error('הלקוח לא אומת מול Supabase לאחר השמירה.');
      ...
      if(!persistRes?.ok) throw new Error('הלקוח נשמר בשרת אך סנכרון המערכת הכללי נכשל. בדוק חיבור ונסה שוב.');
      return record;
    },
      ...
      if(stepId === 3){
        ...
        for(const p of list){
          ...
          if(c.status === "partial_health"){
            const selected = Object.values(c.attachments || {}).some(Boolean);
            if(Array.isArray(p.covers) && p.covers.length && !selected) return false;
            if(!Array.isArray(p.covers) || !p.covers.length){
              if(!safeTrim(c.partialDetails || c.partialText || "")) return false;
            }
          }
        }
        return true;
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const SimHub = {
    ...
    simulators: [
      {
        ...
        category: 'risk',
        tags: ['סיכונים','רשמי'],
        description: 'בדיקת צורך בביטוח חיים למשפחה לפי נתוני לקוח ומבנה משק בית.',
        url: 'https://www.clalbit.co.il/riskcalc/',
        internalAvailable: false,
        internalStatus: 'זמין רק באתר החברה'
      },
      {
        id: 'migdal-risk',
        company: 'מגדל',
        ...
        category: 'risk',
        tags: ['סיכונים','רשמי'],
        description: 'מחשבון לצורך הערכת סכום ביטוח חיים נדרש למשפחה.',
        url: 'https://www.migdal.co.il/macro/risk-calculator',
        internalAvailable: false,
        internalStatus: 'זמין רק באתר החברה'
      },
      {
        id: 'menora-health-benefits',
        company: 'מנורה מבטחים',
        name: 'סימולטור תגמולי בריאות',
        category: 'health',
        tags: ['בריאות','תגמולים'],
        description: 'בדיקת תגמול או החזר פוטנציאלי לפי הליך רפואי וכיסוי קיים.',
        url: 'https://www.menoramivt.co.il/health-benefits-calculator/',
        internalAvailable: false,
        internalStatus: 'זמין רק באתר החברה'
      },
      {
        id: 'gov-health-compare',
        company: 'רשות שוק ההון',
        name: 'השוואת ביטוח בריאות',
        category: 'market',
        tags: ['השוואה','בריאות'],
        description: 'השוואת ביטוחי בריאות בין חברות הביטוח לפי נתוני לקוח.',
        url: 'https://briut.cma.gov.il/',
        internalAvailable: false,
        internalStatus: 'זמין רק באתר הרשמי'
      },
      {
        id: 'gov-life-compare',
        company: 'רשות שוק ההון',
        name: 'השוואת ביטוח חיים',
        category: 'market',
        tags: ['השוואה','סיכונים'],
        description: 'בדיקת מחירי ביטוח חיים והשוואה בין חברות הביטוח.',
        url: 'https://life.cma.gov.il/',
        internalAvailable: false,
        internalStatus: 'זמין רק באתר הרשמי'
      },
      {
        ...
        category: 'mortgage',
        tags: ['משכנתא','מחיר'],
        description: 'בדיקת נתוני ביטוח משכנתא ומעבר להצעת מחיר באתר החברה.',
        url: 'https://www.aig.co.il/mortgage-insurance-aig/',
        internalAvailable: false,
        internalStatus: 'זמין רק באתר החברה'
      }
    ],
    ...
    init(){
      this.els.wrap = $('#view-simulators');
      this.els.cards = $('#simCards');
      this.els.filters = $('#simFilters');
      this.els.customerBox = $('#simCustomerBox');
      this.els.count = $('#simHubCountBadge');
      this.els.iframeModal = $('#simIframeModal');
      this.els.iframeBackdrop = $('#simIframeModalBackdrop');
      this.els.iframeClose = $('#simIframeModalClose');
      this.els.iframeDone = $('#simIframeModalDone');
      this.els.iframeTitle = $('#simIframeModalTitle');
      this.els.iframeNotice = $('#simIframeModalNotice');
      this.els.iframeEmpty = $('#simIframeModalEmpty');
      this.els.iframeWrap = $('#simIframeFrameWrap');
      this.els.iframe = $('#simIframeFrame');
      this.els.iframeExternal = $('#simIframeOpenExternalBtn');
      this.els.insightModal = $('#simInsightModal');
      this.els.insightBackdrop = $('#simInsightModalBackdrop');
      this.els.insightClose = $('#simInsightModalClose');
      this.els.insightCancel = $('#simInsightCancel');
      this.els.insightSave = $('#simInsightSave');
      this.els.insightTitle = $('#simInsightModalTitle');
      this.els.insightStatus = $('#simInsightStatus');
      this.els.insightText = $('#simInsightText');
      this.els.insightHelp = $('#simInsightHelp');

      on(this.els.filters, 'click', (ev) => {
        const btn = ev.target?.closest?.('[data-filter]');
        if(!btn) return;
        this.filter = safeTrim(btn.getAttribute('data-filter')) || 'all';
        $$('.segmented__btn', this.els.filters).forEach((el) => el.classList.toggle('is-active', el === btn));
        this.renderCards();
      });

      on(this.els.cards, 'click', (ev) => {
        const openInternal = ev.target?.closest?.('[data-sim-open-internal]');
        if(openInternal){
          this.openInternal(openInternal.getAttribute('data-sim-open-internal'));
          return;
        }
        const openExternal = ev.target?.closest?.('[data-sim-open-external]');
        if(openExternal){
          this.openExternal(openExternal.getAttribute('data-sim-open-external'));
          return;
        }
        const saveInsight = ev.target?.closest?.('[data-sim-save-insight]');
        if(saveInsight){
          this.openInsight(saveInsight.getAttribute('data-sim-save-insight'));
        }
      });

      [this.els.iframeBackdrop, this.els.iframeClose, this.els.iframeDone].forEach((el) => on(el, 'click', () => this.closeInternal()));
      on(this.els.iframeExternal, 'click', () => {
        const sim = this.byId(this.activeSimulatorId);
        if(sim?.url) window.open(sim.url, '_blank', 'noopener');
      });

      [this.els.insightBackdrop, this.els.insightClose, this.els.insightCancel].forEach((el) => on(el, 'click', () => this.closeInsight()));
      on(this.els.insightSave, 'click', () => this.saveInsight());

      document.addEventListener('keydown', (ev) => {
        if(ev.key !== 'Escape') return;
        if(this.els.iframeModal?.classList.contains('is-open')) this.closeInternal();
        if(this.els.insightModal?.classList.contains('is-open')) this.closeInsight();
      });
    },

    byId(id){
      return this.simulators.find((item) => String(item.id) === String(id)) || null;
    },

    getActiveCustomer(){
      try {
        const rec = CustomersUI?.current?.();
        if(rec) return rec;
      } catch(_e) {}
      return null;
    },

    getCustomerDisplay(rec){
      if(!rec) return null;
      const marital = safeTrim(rec.maritalStatus || rec.familyStatus || rec.marital);
      const childrenCount = Number(rec.childrenCount || rec.children || rec.kidsCount || 0) || 0;
      const hasMortgage = !!(rec.mortgageAmount || rec.hasMortgage || rec.mortgage || rec.payload?.mortgageAmount);
      const age = Number(rec.age) || '';
      return {
        name: safeTrim(rec.fullName),
        age: age ? `${age}` : '—',
        marital: marital || '—',
        phone: safeTrim(rec.phone) || '—',
        children: childrenCount > 0 ? `${childrenCount} ילדים` : 'ללא אינדיקציה',
        mortgage: hasMortgage ? 'כן' : 'לא'
      };
    },
    ...
    renderCustomer(){
      if(!this.els.customerBox) return;
      const rec = this.getActiveCustomer();
      const info = this.getCustomerDisplay(rec);
      if(!info){
        this.els.customerBox.innerHTML = `
          <div class="simHub__customerCard simHub__customerCard--empty">
            <div class="simHub__customerItem">
              <div class="simHub__customerLabel">לקוח פעיל</div>
              <div class="simHub__customerValue">לא נבחר לקוח</div>
              <div class="simHub__customerHint">ניתן להשתמש בסימולטורים גם בלי לקוח פתוח. כדי לשמור תובנה, פתח תיק לקוח ואז חזור לכאן.</div>
            </div>
          </div>`;
        return;
      }
      this.els.customerBox.innerHTML = `
        <div class="simHub__customerCard">
          <div class="simHub__customerItem"><div class="simHub__customerLabel">לקוח</div><div class="simHub__customerValue">${escapeHtml(info.name || '—')}</div></div>
          <div class="simHub__customerItem"><div class="simHub__customerLabel">גיל</div><div class="simHub__customerValue">${escapeHtml(info.age)}</div></div>
          <div class="simHub__customerItem"><div class="simHub__customerLabel">מצב משפחתי</div><div class="simHub__customerValue">${escapeHtml(info.marital)}</div></div>
          <div class="simHub__customerItem"><div class="simHub__customerLabel">טלפון</div><div class="simHub__customerValue">${escapeHtml(info.phone)}</div></div>
          <div class="simHub__customerItem"><div class="simHub__customerLabel">ילדים</div><div class="simHub__customerValue">${escapeHtml(info.children)}</div></div>
          <div class="simHub__customerItem"><div class="simHub__customerLabel">משכנתא</div><div class="simHub__customerValue">${escapeHtml(info.mortgage)}</div></div>
        </div>`;
    },

    filtered(){
      return this.simulators.filter((item) => this.filter === 'all' || item.category === this.filter);
    },

    renderCards(){
      if(!this.els.cards) return;
      const rows = this.filtered();
      if(this.els.count) this.els.count.textContent = `${rows.length} סימולטורים`;
      if(!rows.length){
        this.els.cards.innerHTML = `<div class="card emptyState simCard__empty"><div class="emptyState__icon">🧭</div><div class="emptyState__title">אין סימולטורים בקטגוריה הזו</div><div class="emptyState__text">נסה לעבור לקטגוריה אחרת או לחזור להצגת כל הסימולטורים.</div></div>`;
        return;
      }
      this.els.cards.innerHTML = rows.map((sim) => `
        <article class="simCard">
          <div class="simCard__top">
            <div>
              <div class="simCard__company">${escapeHtml(sim.company)}</div>
              <div class="simCard__title">${escapeHtml(sim.name)}</div>
            </div>
            <div class="simCard__status"><span class="simCard__statusDot"></span>${escapeHtml(sim.internalStatus || (sim.internalAvailable ? 'זמין בתוך המערכת' : 'זמין רק באתר החברה'))}</div>
          </div>
          <div class="simCard__desc">${escapeHtml(sim.description || '')}</div>
          <div class="simCard__tags">${(sim.tags || []).map((tag) => `<span class="badge">${escapeHtml(tag)}</span>`).join('')}</div>
          <div class="simCard__actions">
            <button class="btn simCard__btn" type="button" data-sim-open-internal="${escapeHtml(sim.id)}" ${sim.internalAvailable ? '' : 'disabled'}>פתח במערכת</button>
            <button class="btn btn--primary simCard__btn" type="button" data-sim-open-external="${escapeHtml(sim.id)}">היכנס לאתר החברה</button>
            <button class="btn simCard__btn" type="button" data-sim-save-insight="${escapeHtml(sim.id)}">שמור תובנה</button>
          </div>
        </article>`).join('');
    },
    ...
    openExternal(id){
      ...
      if(!sim?.url) return;
      window.open(sim.url, '_blank', 'noopener');
    },

    openInternal(id){
      const sim = this.byId(id);
      if(!sim) return;
      this.activeSimulatorId = sim.id;
      if(this.els.iframeTitle) this.els.iframeTitle.textContent = sim.name;
      if(this.els.iframeNotice) this.els.iframeNotice.textContent = sim.internalAvailable ? 'הסימולטור נטען בתוך המערכת. במידה ויחסם, ניתן להמשיך דרך אתר החברה.' : 'הסימולטור הזה לא תומך בפתיחה פנימית כרגע. השארתי לך גישה מהירה לאתר החברה בלחיצה אחת.';
      if(this.els.iframeExternal) this.els.iframeExternal.disabled = !sim.url;

      if(sim.internalAvailable && sim.url){
        this.els.iframeWrap.hidden = false;
        this.els.iframeEmpty.hidden = true;
        this.els.iframe.src = sim.url;
      } else {
        this.els.iframeWrap.hidden = true;
        this.els.iframeEmpty.hidden = false;
        this.els.iframe.src = 'about:blank';
        this.els.iframeEmpty.innerHTML = '<div><strong>הפתיחה בתוך המערכת לא זמינה כרגע.</strong><div style="margin-top:8px">השתמש בלחצן "היכנס לאתר החברה" כדי לעבוד מול הסימולטור הרשמי בטאב חדש.</div></div>';
      }
      this.els.iframeModal?.classList.add('is-open');
      this.els.iframeModal?.setAttribute('aria-hidden', 'false');
      document.body.style.overflow = 'hidden';
    },

    closeInternal(){
      this.activeSimulatorId = '';
      if(this.els.iframe) this.els.iframe.src = 'about:blank';
      this.els.iframeModal?.classList.remove('is-open');
      this.els.iframeModal?.setAttribute('aria-hidden', 'true');
      document.body.style.overflow = '';
    },
    ...
    openInsight(id){
      const sim = this.byId(id);
      if(!sim) return;
      this.insightSimulatorId = sim.id;
      const rec = this.getActiveCustomer();
      if(this.els.insightTitle) this.els.insightTitle.textContent = `שמור תובנה · ${sim.name}`;
      if(this.els.insightStatus) this.els.insightStatus.value = 'תקין';
      if(this.els.insightText) this.els.insightText.value = '';
      if(this.els.insightHelp) this.els.insightHelp.textContent = rec ? `התובנה תישמר על הלקוח ${rec.fullName || ''}.` : 'לא נבחר לקוח פתוח כרגע. ניתן לפתוח תיק לקוח ואז לשמור תובנה.';
      if(this.els.insightSave) this.els.insightSave.disabled = !rec;
      this.els.insightModal?.classList.add('is-open');
      this.els.insightModal?.setAttribute('aria-hidden', 'false');
      document.body.style.overflow = 'hidden';
    },

    closeInsight(){
      this.insightSimulatorId = '';
      this.els.insightModal?.classList.remove('is-open');
      this.els.insightModal?.setAttribute('aria-hidden', 'true');
      document.body.style.overflow = '';
    },

    async saveInsight(){
      const rec = this.getActiveCustomer();
      const sim = this.byId(this.insightSimulatorId);
      if(!rec || !sim) return;
      const status = safeTrim(this.els.insightStatus?.value) || 'תקין';
      const note = safeTrim(this.els.insightText?.value);
      const payload = rec.payload && typeof rec.payload === 'object' ? rec.payload : (rec.payload = {});
      const list = Array.isArray(payload.simulatorInsights) ? payload.simulatorInsights : (payload.simulatorInsights = []);
      list.unshift({
        id: `simInsight_${Date.now()}`,
        simulatorId: sim.id,
        ...
        note,
        createdAt: nowISO(),
        createdBy: safeTrim(Auth?.current?.name) || ''
      });
      payload.simulatorInsights = list.slice(0, 30);
      rec.updatedAt = nowISO();
      if(State?.data?.meta) State.data.meta.updatedAt = rec.updatedAt;
      const result = await App.persist('תובנת סימולטור נשמרה');
      if(result?.ok && typeof window.showToast === 'function'){
        try { window.showToast('תובנת הסימולטור נשמרה בהצלחה'); } catch(_e) {}
      }
      this.closeInsight();
      this.renderCustomer();
    }
  ...
  const SecureCoreUI = {
    ...
    busy: false,
    lastScan: null,
    checksOrder: ["session", "permissions", "server", "ui", "state", "activity"],

    init(){
      this.els.wrap = $("#secureCoreModal");
      this.els.backdrop = $("#secureCoreBackdrop");
      this.els.close = $("#secureCoreClose");
      this.els.cancel = $("#secureCoreCancel");
      this.els.run = $("#secureCoreRun");
      this.els.autoFix = $("#secureCoreAutoFix");
      this.els.toggleDetails = $("#secureCoreToggleDetails");
      this.els.summaryStatus = $("#secureCoreSummaryStatus");
      this.els.summaryMeta = $("#secureCoreSummaryMeta");
      this.els.progress = $("#secureCoreProgress");
      this.els.progressBar = $("#secureCoreProgressBar");
      this.els.progressSteps = Array.from(document.querySelectorAll("#secureCoreProgressSteps .secureCoreProgress__step"));
      this.els.details = $("#secureCoreDetails");
      this.els.btn = $("#btnSecureCoreScan");
      this.els.state = $("#secureBadgeState");
      on(this.els.btn, 'click', () => this.open());
      on(this.els.close, 'click', () => this.close());
      on(this.els.cancel, 'click', () => this.close());
      on(this.els.backdrop, 'click', () => this.close());
      on(this.els.wrap, 'click', (ev) => { if(ev.target?.getAttribute?.('data-close') === '1') this.close(); });
      on(this.els.run, 'click', async () => { await this.runScan(); });
      on(this.els.autoFix, 'click', async () => { await this.runAutoFix(); });
      on(this.els.toggleDetails, 'click', () => this.toggleDetails());
      on(document, 'keydown', (ev) => { if(ev.key === 'Escape' && this.isOpen() && !this.busy) this.close(); });
      this.resetUi();
      this.paintBadge('idle');
    },

    isOpen(){ return !!this.els.wrap && this.els.wrap.getAttribute('aria-hidden') === 'false'; },
    open(){ if(!this.els.wrap) return; this.els.wrap.classList.add('is-open'); this.els.wrap.setAttribute('aria-hidden', 'false'); },
    close(){ if(!this.els.wrap || this.busy) return; this.els.wrap.classList.remove('is-open','is-scanning'); this.els.wrap.setAttribute('aria-hidden', 'true'); },

    resetUi(){
      this.setSummary('המערכת מוכנה להריץ בדיקה חכמה.', 'הבדיקות כוללות session, הרשאות, חיבור שרת, יציבות מסך, state וחריגות פעילות.', 'idle');
      if(this.els.progress) this.els.progress.setAttribute('aria-hidden', 'true');
      ...
      this.checksOrder.forEach((key) => this.setCheckState(key, { tone:'idle', label:'מוכן', text:'טרם בוצעה בדיקה' }));
      if(this.els.details){ this.els.details.hidden = true; this.els.details.innerHTML=''; }
      if(this.els.toggleDetails) this.els.toggleDetails.textContent = 'הצג פירוט בדיקה';
      if(this.els.autoFix) this.els.autoFix.disabled = false;
      if(this.els.run){ this.els.run.disabled = false; this.els.run.textContent = 'התחלת בדיקה'; }
      this.els.wrap?.classList.remove('is-ok','is-warn','is-danger');
    },

    setBusy(flag){
      this.busy = !!flag;
      [this.els.close, this.els.cancel, this.els.run, this.els.autoFix].forEach((el) => { if(el) el.disabled = !!flag; });
      if(this.els.run) this.els.run.textContent = flag ? 'מריץ בדיקה...' : 'בדיקה מחדש';
      this.els.wrap?.classList.toggle('is-scanning', !!flag);
      this.els.btn?.classList.toggle('is-scanning', !!flag);
    },

    setSummary(title, meta, tone='idle'){
      if(this.els.summaryStatus){ this.els.summaryStatus.textContent = title || ''; this.els.summaryStatus.className = 'secureCoreSummary__status' + (tone && tone !== 'idle' ? ' is-' + tone : ' is-idle'); }
      if(this.els.summaryMeta) this.els.summaryMeta.textContent = meta || '';
      this.els.wrap?.classList.remove('is-ok','is-warn','is-danger');
      if(['ok','warn','danger'].includes(tone)) this.els.wrap?.classList.add('is-' + tone);
    },

    setCheckState(key, cfg={}){
      const pill = document.getElementById('secureCorePill-' + key);
      const text = document.getElementById('secureCoreText-' + key);
      const tone = safeTrim(cfg.tone || 'idle');
      if(pill){ pill.textContent = cfg.label || 'מוכן'; pill.className = 'secureCoreCheck__pill' + (tone ? ' is-' + tone : ''); }
      if(text) text.textContent = cfg.text || '';
    },

    updateProgress(index){
      const total = this.checksOrder.length;
      const percent = index <= 0 ? 0 : Math.round((index / total) * 100);
      if(this.els.progressBar) this.els.progressBar.style.width = percent + '%';
      (this.els.progressSteps || []).forEach((el, idx) => {
        el.classList.remove('is-active','is-done');
        if(idx + 1 < index) el.classList.add('is-done');
        else if(idx + 1 === index) el.classList.add('is-active');
      });
    },

    toggleDetails(){
      if(!this.els.details) return;
      const nextHidden = !this.els.details.hidden;
      this.els.details.hidden = nextHidden;
      if(this.els.toggleDetails) this.els.toggleDetails.textContent = nextHidden ? 'הצג פירוט בדיקה' : 'הסתר פירוט';
    },

    paintBadge(level='idle'){
      const btn = this.els.btn;
      if(!btn) return;
      btn.classList.remove('is-ok','is-warn','is-danger');
      if(level === 'ok') btn.classList.add('is-ok');
      else if(level === 'warn') btn.classList.add('is-warn');
      else if(level === 'danger') btn.classList.add('is-danger');
      const titleMap = { idle:'SecureCore Scan • בדיקת תקינות ואבטחת מערכת', ok:'SecureCore Scan • הבדיקה האחרונה עברה בהצלחה', warn:'SecureCore Scan • זוהתה חריגה הדורשת בדיקה', danger:'SecureCore Scan • זוהה מצב המחייב טיפול' };
      btn.title = titleMap[level] || titleMap.idle;
      btn.setAttribute('aria-label', btn.title);
    },

    buildDetails(results=[]){
      if(!this.els.details) return;
      this.els.details.innerHTML = results.map((item) => {
        const tone = item.severity === 'danger' ? 'danger' : item.severity === 'warn' ? 'warn' : 'ok';
        return `<div class="secureCoreDetail"><div class="secureCoreDetail__head"><div class="secureCoreDetail__title">${escapeHtml(item.title || '')}</div><div class="secureCoreCheck__pill is-${tone}">${tone === 'ok' ? 'תקין' : tone === 'warn' ? 'אזהרה' : 'כשל'}</div></div><div class="secureCoreDetail__text">${escapeHtml(item.message || '')}</div><div class="secureCoreDetail__time">${escapeHtml(item.detail || '')}</div></div>`;
      }).join('');
    },
    ...
    async runScan(trigger='manual'){
      if(this.busy) return;
      this.resetUi();
      if(this.els.progress) this.els.progress.setAttribute('aria-hidden', 'false');
      this.setBusy(true);
      this.paintBadge('idle');
      this.setSummary('בודק את תקינות המערכת', 'מריץ בדיקות אבטחה, הרשאות, חיבור ויציבות. נא להמתין...', 'idle');
      const results = [];
      let warns = 0;
      let dangers = 0;
      for(let i=0;i<this.checksOrder.length;i++){
        const key = this.checksOrder[i];
        this.updateProgress(i + 1);
        this.setCheckState(key, { tone:'active', label:'בבדיקה', text:'המערכת מריצה בדיקה חכמה עבור רכיב זה.' });
        await this.wait(220);
        const result = await this.runSingleCheck(key);
        results.push(result);
        if(result.severity === 'danger') dangers += 1;
        else if(result.severity === 'warn') warns += 1;
        this.setCheckState(key, { tone: result.severity === 'danger' ? 'danger' : result.severity === 'warn' ? 'warn' : 'ok', label: result.severity === 'danger' ? 'כשל' : result.severity === 'warn' ? 'אזהרה' : 'תקין', text: result.message });
      }
      this.updateProgress(this.checksOrder.length);
      const tone = dangers ? 'danger' : warns ? 'warn' : 'ok';
      const statusTitle = tone === 'ok' ? 'המערכת תקינה ומאובטחת' : tone === 'warn' ? 'נמצאה חריגה הדורשת תשומת לב' : 'זוהה מצב המחייב טיפול מיידי';
      const meta = tone === 'ok'
        ? 'לא זוהו חריגות פעילות. החיבור לשרת תקין, ההרשאות תואמות והמסך יציב.'
        : tone === 'warn'
          ? `זוהו ${warns} חריגות קלות${dangers ? ' ו-' + dangers + ' כשלים' : ''}. מומלץ לעבור על הפירוט או להפעיל תיקון אוטומטי.`
          : `זוהו ${dangers} כשלים מהותיים${warns ? ' ו-' + warns + ' אזהרות נוספות' : ''}. מומלץ לבצע תיקון אוטומטי או נעילה ובדיקה מחדש.`;
      this.setSummary(statusTitle, meta, tone);
      this.buildDetails(results);
      this.paintBadge(tone);
      this.lastScan = { tone, trigger, results, scannedAt: nowISO() };
      this.setBusy(false);
      if(tone === 'ok' && this.els.autoFix) this.els.autoFix.disabled = true;
      if(typeof window.showToast === 'function'){
        const msg = tone === 'ok' ? 'SecureCore: המערכת תקינה' : tone === 'warn' ? 'SecureCore: נמצאו חריגות קלות' : 'SecureCore: זוהתה חריגה המחייבת טיפול';
        try { window.showToast(msg); } catch(_e) {}
      }
    },
    ...
    async runAutoFix(){
      if(this.busy) return;
      this.setBusy(true);
      this.setSummary('מבצע תיקון אוטומטי בטוח', 'משחרר שכבות תקועות, מרענן state ומבצע בדיקת יציבות מחדש.', 'warn');
      try {
        forceCloseUiLayers({ keepIds:['secureCoreModal'] });
      } catch(_e) {}
      try { releaseGlobalUiLocks(); } catch(_e) {}
      try { MirrorsUI?.stopTimerLoop?.(); } catch(_e) {}
      try { LiveRefresh?.stop?.(); } catch(_e) {}
      await this.wait(260);
      try { LiveRefresh?.start?.(); } catch(_e) {}
      await this.wait(220);
      this.setBusy(false);
      await this.runScan('autofix');
    },

    async runSingleCheck(key){
      const startedAt = performance.now();
      const titleMap = { session:'Session ואימות', permissions:'הרשאות ותפריטים', server:'חיבור שרת', ui:'יציבות מסך', state:'מצב נתונים', activity:'חריגות פעילות' };
      let severity = 'ok';
      let message = 'הבדיקה עברה בהצלחה.';
      try {
        if(key === 'session'){
          const current = Auth?.current || null;
          if(!current || !safeTrim(current.name)) { severity = 'danger'; message = 'לא זוהה משתמש מחובר או שה-session המקומי חסר.'; }
          else if(current.active === false) { severity = 'warn'; message = 'המשתמש מזוהה אך מסומן כלא פעיל. מומלץ לרענן הרשאות.'; }
          else message = `מחובר כעת: ${safeTrim(current.name)} · session פעיל.`;
        }
        if(key === 'permissions'){
          const current = Auth?.current || {};
          const role = safeTrim(current.role || 'agent');
          const navUsers = $('#navUsers');
          const navProcesses = $('#navMyProcesses');
          const badUsers = role === 'agent' && navUsers && getComputedStyle(navUsers).display !== 'none';
          const badProcesses = role === 'agent' && navProcesses && getComputedStyle(navProcesses).display !== 'none';
          if(badUsers || badProcesses){ severity = 'danger'; message = 'זוהתה אי-תאימות בין תפקיד המשתמש לבין התפריטים המוצגים במסך.'; }
          else if(!role){ severity = 'warn'; message = 'לא אותר role למשתמש המחובר. מומלץ לבצע רענון הרשאות.'; }
          else message = `הרשאות נטענו בצורה תקינה עבור התפקיד: ${role}.`;
        }
        if(key === 'server'){
          ...
          try {
            if(window.supabase?.createClient && SUPABASE_URL && SUPABASE_PUBLISHABLE_KEY){
              ...
              const res = await client.from(SUPABASE_TABLES.meta).select('key', { head:true, count:'exact' }).limit(1);
              ok = !res.error;
              if(res.error){ severity = 'warn'; message = 'חיבור השרת זמין אך התקבלה שגיאה בבדיקת meta.'; }
            } else { severity = 'warn'; message = 'לא נמצאה תשתית שרת זמינה לבדיקת ping.'; }
          } catch(_e) { severity = 'danger'; message = 'בדיקת חיבור השרת נכשלה או שהחיבור אינו יציב.'; }
          const took = Math.round(performance.now() - started);
          if(ok){
            if(took > 1500){ severity = 'warn'; message = `החיבור לשרת תקין אך איטי מהרגיל (${took}ms).`; }
            else message = `החיבור לשרת תקין וזמן התגובה הוא ${took}ms.`;
          }
        }
        if(key === 'ui'){
          const stuckModals = $$('.modal.is-open,[aria-busy="true"],.drawer.is-open,.lcWizard.is-open').filter((el) => el.id !== 'secureCoreModal');
          const hasBlockedBody = document.body.hasAttribute('inert') || document.documentElement.hasAttribute('inert');
          if(hasBlockedBody || stuckModals.length > 2){ severity = 'warn'; message = 'זוהו שכבות/נעילות מסך חריגות. ניתן להפעיל תיקון אוטומטי בטוח.'; }
          else message = 'לא זוהו שכבות תקועות, חסימות לחיצה או modal חריג.';
        }
        if(key === 'state'){
          const hasMeta = !!State?.data?.meta;
          const hasAgents = Array.isArray(State?.data?.agents) && State.data.agents.length > 0;
          const hasCustomers = Array.isArray(State?.data?.customers);
          if(!hasMeta || !hasAgents || !hasCustomers){ severity = 'danger'; message = 'State מקומי חלקי או חסר ברכיבים קריטיים.'; }
          else if(!safeTrim(State?.data?.meta?.updatedAt)){ severity = 'warn'; message = 'State קיים אך חסרה חותמת עדכון אחרונה ב-meta.'; }
          else message = 'State מקומי שלם והרכיבים הקריטיים זמינים.';
        }
        if(key === 'activity'){
          const globals = (window.__GI_RUNTIME_ERRORS__ || []).length;
          const loginErr = safeTrim($('#lcLoginError')?.textContent);
          if(globals >= 3){ severity = 'danger'; message = `זוהו ${globals} שגיאות runtime אחרונות. מומלץ לבצע תיקון אוטומטי.`; }
          else if(globals > 0 || loginErr){ severity = 'warn'; message = 'זוהו שגיאות מקומיות/אזהרות פעילות אחרונות. מומלץ לעבור על פירוט הבדיקה.'; }
          else message = 'לא זוהתה פעילות מקומית חריגה או ריבוי שגיאות אחרונות.';
        }
      } catch(_e) {
        severity = 'danger';
        message = 'הבדיקה נכשלה בעקבות שגיאת מערכת לא צפויה.';
      }
      const tookMs = Math.max(1, Math.round(performance.now() - startedAt));
      return { key, title: titleMap[key] || key, severity, message, detail: `זמן בדיקה: ${tookMs}ms · ${new Date().toLocaleTimeString('he-IL', { hour:'2-digit', minute:'2-digit', second:'2-digit' })}` };
    },
