...
(() => {
  ...
  const BUILD = "20260409-vOwnerRestoreSystemSettings";
...
this.applyRoleUI();
    ...
    renderAuthPill(){
      ...
      if(Auth.current) {
        pill.style.display = "";
        txt.textContent = Auth.current.name + (Auth.current?.isSystemOwner ? " (מפתח המערכת)" : Auth.isAdmin() ? " (מנהל מערכת)" : Auth.isManager() ? " (מנהל)" : Auth.isOps() ? " (תפעול)" : "");
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  function isSystemOwnerAgent(agent){
    if(!agent || typeof agent !== 'object') return false;
    const sec = getAgentSecurity(safeTrim(agent.id));
    const ownerName = safeTrim(SYSTEM_OWNER_DEFAULT.name).toLowerCase();
    const ownerUsername = safeTrim(SYSTEM_OWNER_DEFAULT.username).toLowerCase();
    const ownerEmail = safeTrim(SYSTEM_OWNER_DEFAULT.email).toLowerCase();
    const agentName = safeTrim(agent.name).toLowerCase();
    const agentUsername = safeTrim(agent.username || agent.user).toLowerCase();
    const agentEmail = safeTrim(sec?.authEmail).toLowerCase();
    return !!(
      (ownerName && agentName === ownerName) ||
      (ownerUsername && agentUsername === ownerUsername) ||
      (ownerEmail && agentEmail === ownerEmail)
    );
  }

  const completeAgentLogin = async (matched, options = {}) => {
    const loaderMs = Math.max(1200, Number(options?.loaderMs) || 1200);
    const resolvedRole = isSystemOwnerAgent(matched)
      ? 'admin'
      : (matched.role === 'manager' ? 'manager' : matched.role === 'ops' ? 'ops' : 'agent');
    Auth.current = { name: matched.name, role: resolvedRole, isSystemOwner: resolvedRole === 'admin' && isSystemOwnerAgent(matched) };
    try { localStorage.removeItem(LS_SESSION_KEY); } catch(_) {}
    await App.reloadSessionState();
    Auth.unlock();
    try { InactivityGuard.start(); } catch(_e) {}
