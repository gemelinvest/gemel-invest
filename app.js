...
(() => {
  ...
  const BUILD = "20260411-vPerformanceShieldUltra";
  ...
  function isRuntimeBackgrounded(){
    try {
    ...
    startKeepAlive(){
      ...
      const tick = async () => {
        try {
          ...
          if(isRuntimeBackgrounded()) return;
          await this.probeConnection();
        } catch(_e) {}
      };
      this.keepAliveTimer = setInterval(tick, Math.max(45000, SUPABASE_KEEPALIVE_MS));
    },
...
UsersGateUI.init();
  ...
  const UsersGateUI = {
    ...
    open(afterSuccess = null){
      ...
      try { document.body.classList.remove('lcDetachedTopbarHidden'); } catch(_e) {}
      this.els.wrap?.classList.add('is-open');
      this.els.wrap?.setAttribute('aria-hidden','false');
      this.els.wrap && (this.els.wrap.style.display = 'block');
  ...
  const CustomersUI = {
    ...
    startOpsCardLoop(){
      ...
      this._opsCardTimer = window.setInterval(() => {
        if(isRuntimeBackgrounded()) return;
        this.refreshOperationalReflectionCard();
      }, 1000);
    },
...
ההכנסה מעבודה שאתה צפוי להרוויח תעמוד על סך של ______ ₪ לחודש. ידוע לך כי בקרות מקרה הביטוח יוגבלו תגמולי הפיצוי החודשי להם תהיה זכאי לגובה שלא יעלה על 75% מהכנסתך מעבודה בפועל ב־12 החודשים שקדמו למועד קרות מקרה הביטוח, או בתקופה ממועד תחילת הביטוח ועד מועד קרות מקרה הביטוח, אם חלפו פחות מ־12 חודשים.`
  ...
  const ChatUI = {
    ...
    startCleanupLoop(){
      ...
      this.cleanupTimer = setInterval(run, Math.max(90000, Number(SUPABASE_CHAT.cleanupIntervalMs || 60000)));
      if(!isRuntimeBackgrounded()) run();
    },
  ...
  // ---------- Start ----------
  try { document.body.classList.add('giPerfLite'); } catch(_e) {}
