...
init(){
    ...
    reset(){
      ...
      // Har Bituach attachment (case-level)
      this.harBituach = null;
      try { sessionStorage.removeItem("GEMEL_HAR_FILE_V1"); } catch(_) {}
    ...
    close(){
      ...
      this.closePicker();      this.closeHarBituachModal();
    },
    ...
    // ---------- Har Bituach (Step 3 attachment) ----------
    ensureHarBituachModal(){
      if (this.els.harModal) return;
      const wrap = document.createElement("div");
      wrap.className = "lcMiniModal";
      wrap.id = "lcHarBituachModal";
      wrap.setAttribute("aria-hidden","true");
      wrap.innerHTML = `
        <div class="lcMiniModal__backdrop" data-har-close="1"></div>
        <div class="lcMiniModal__card" role="dialog" aria-modal="true" aria-label="הר ביטוח">
          <div class="lcMiniModal__head">
            <div class="lcMiniModal__title">הר ביטוח</div>
            <button class="btn lcSmallBtn" type="button" data-har-close="1">סגור</button>
          </div>
          <div class="lcMiniModal__body">
            <div class="muted small">העלה קובץ מהר הביטוח. הקובץ נשמר זמנית עד לסיום תהליך הקמת הלקוח.</div>
            <input class="input" id="lcHarFileInput" type="file" />
            <div class="lcHarFileMeta" id="lcHarFileMeta" style="display:none"></div>
            <div class="row" style="gap:10px; justify-content:flex-end; flex-wrap:wrap">
              <button class="btn" id="lcHarSaveBtn" type="button">שמור</button>
              <button class="btn btn--ghost" id="lcHarClearBtn" type="button">נקה</button>
            </div>
          </div>
        </div>
      `;
      ...
      this.els.harModal = wrap;
      this.els.harFileInput = $("#lcHarFileInput", wrap);
      this.els.harFileMeta = $("#lcHarFileMeta", wrap);
      this.els.harSaveBtn = $("#lcHarSaveBtn", wrap);
      this.els.harClearBtn = $("#lcHarClearBtn", wrap);

      on(wrap, "click", (e) => {
        const t = e.target;
        if (t && t.getAttribute && t.getAttribute("data-har-close") === "1") this.closeHarBituachModal();
      });

      on(this.els.harSaveBtn, "click", async () => {
        const f = this.els.harFileInput?.files?.[0];
        if(!f) { this.setHint("בחר קובץ להר הביטוח ואז לחץ שמור"); return; }

        // soft size guard (stores in sessionStorage)
        const maxBytes = 10 * 1024 * 1024; // 10MB
        if (f.size > maxBytes) {
          this.setHint("הקובץ גדול מדי לשמירה זמנית במערכת (מעל 10MB). נא להעלות קובץ קטן יותר.");
          return;
        }

        const dataUrl = await this.readFileAsDataURL(f);
        this.harBituach = { name: f.name, type: f.type || "", size: f.size, dataUrl, savedAt: nowISO() };

        try { sessionStorage.setItem("GEMEL_HAR_FILE_V1", JSON.stringify(this.harBituach)); } catch(_) {}
        this.setHint("נשמר קובץ הר הביטוח: " + f.name);
        this.closeHarBituachModal();
      });

      on(this.els.harClearBtn, "click", () => {
        this.harBituach = null;
        try { sessionStorage.removeItem("GEMEL_HAR_FILE_V1"); } catch(_) {}
        if (this.els.harFileInput) this.els.harFileInput.value = "";
        this.setHint("הקובץ נוקה");
        this.render();
      });

      // restore if exists
      try {
        const raw = sessionStorage.getItem("GEMEL_HAR_FILE_V1");
        if(raw) this.harBituach = JSON.parse(raw);
      } catch(_) {}
    },

    openHarBituachModal(){
      this.ensureHarBituachModal();
      if(!this.els.harModal) return;
      this.els.harModal.classList.add("is-open");
      this.els.harModal.setAttribute("aria-hidden","false");

      // show current file meta
      const m = this.harBituach;
      if (this.els.harFileMeta){
        if(m && m.name){
          const kb = Math.round((m.size || 0) / 1024);
          this.els.harFileMeta.style.display = "";
          this.els.harFileMeta.innerHTML = `<div class="badge">קיים קובץ שמור</div><div class="small" style="margin-top:6px"><b>${escapeHtml(m.name)}</b> · ${kb}KB</div>`;
        } else {
          this.els.harFileMeta.style.display = "none";
          this.els.harFileMeta.innerHTML = "";
        }
      }
    },

    closeHarBituachModal(){
      if(!this.els.harModal) return;
      this.els.harModal.classList.remove("is-open");
      this.els.harModal.setAttribute("aria-hidden","true");
    },
    ...
    renderBody(){
      ...
      const harBtn = (this.step === 3) ? `<button class="btn" id="lcHarBituachBtn" type="button">🏔️ הר ביטוח</button>` : "";
      const actions = (this.step === 3) ? `<div class="lcPolActions">${addBtn}${harBtn}</div>` : "";
...
if(path === "birthDate"){
  ...
  return;
    ...
    bindInputs(ins){
      ...
      const harBtnEl = $("#lcHarBituachBtn", this.els.body);
      on(harBtnEl, "click", () => { this.openHarBituachModal(); });
