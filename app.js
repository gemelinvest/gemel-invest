...
(() => {
  ...
  const BUILD = "20260327-operational-report-insureds-health-pages-v1";
...
txt.textContent = "";
  ...
  const ProposalFormUI = {
    ...
    init(){
      ...
      this.els.customerBtn = $("#customerFullProposalBtn");

      on(this.els.customerBtn, "click", (ev) => {
        ev?.preventDefault?.();
        const rec = CustomersUI.current?.();
        if(!rec){
          this.setStatus("לא נמצא כרגע לקוח פתוח בתיק הלקוח.", true);
          return;
        }
        this.open(rec.id);
      });
      on(this.els.close, "click", () => this.close());
      on(this.els.backdrop, "click", () => this.close());
      on(this.els.previewBtn, "click", async () => { await this.openPreview(); });
    ...
    byId(id){
      return CustomersUI?.byId?.(id) || null;
    },

    open(id){
      const rec = this.byId(id);
      if(!rec || !this.els.wrap) return;
      this.currentCustomerId = rec.id;
      this.renderSummary(rec);
      this.setStatus("המערכת מוכנה להפיק את טופס ההצעה המקורי של הפניקס כ־PDF editable לעמוד 1.");
      this.els.wrap.classList.add("is-open");
      this.els.wrap.setAttribute("aria-hidden", "false");
    ...
    setStatus(msg, isError=false){
      ...
      this.els.status.textContent = String(msg || "");
      this.els.status.classList.toggle("is-error", !!isError);
    },

    current(){
      return this.byId(this.currentCustomerId);
    },

    getApplicants(rec){
      const insureds = CustomersUI?.getInsureds?.(rec) || [];
      if(Array.isArray(insureds) && insureds.length){
        return insureds.slice(0, 6).map((ins, index) => this.normalizeApplicant(ins, index, rec)).filter(Boolean);
      }
      const payload = rec?.payload && typeof rec.payload === "object" ? rec.payload : {};
      return [this.normalizeApplicant({ data: payload.primary || {} }, 0, rec)].filter(Boolean);
    },

    normalizeApplicant(ins, index, rec){
      const data = ins?.data && typeof ins.data === "object" ? ins.data : (ins || {});
      const firstName = safeTrim(data.firstName);
      const lastName = safeTrim(data.lastName);
      const fullName = safeTrim((firstName + " " + lastName).trim()) || safeTrim(rec?.fullName) || `מבוטח ${index + 1}`;
      const address = {
        street: safeTrim(data.street),
        houseNumber: safeTrim(data.houseNumber),
        city: safeTrim(data.city || rec?.city),
        zip: safeTrim(data.zip || data.postalCode || data.zipCode)
      };
      ...
      const shaban = this.mapShaban(data.hasShaban ?? data.shaban ?? data.shabanLevel);
      return {
        index: index + 1,
        label: safeTrim(ins?.label) || this.getApplicantLabel(ins, index),
        firstName,
        lastName,
        fullName,
        idNumber: this.normalizeId(safeTrim(data.idNumber) || safeTrim(rec?.idNumber)),
        rawIdNumber: safeTrim(data.idNumber) || safeTrim(rec?.idNumber),
        phone: safeTrim(data.phone) || safeTrim(rec?.phone),
        email: safeTrim(data.email) || safeTrim(rec?.email),
        city: address.city,
        street: address.street,
        houseNumber: address.houseNumber,
        zip: address.zip,
        occupation: safeTrim(data.occupation),
        kupatHolim: safeTrim(data.kupatHolim || data.healthFund || data.clinic || data.hmo),
        dangerousHobbies: safeTrim(data.dangerousHobbies || data.hobbiesDangerous || data.hobbies),
        birthDate: this.normalizeDate(data.birthDate || data.dob),
        gender,
        marital,
        delivery,
        deliveryMethodLabel: delivery.email ? 'מייל' : delivery.post ? 'דואר ישראל' : '—',
        shaban,
        shabanLabel: shaban.yes ? 'כן' : shaban.no ? 'לא' : '—'
      };
    },

    getApplicantLabel(ins, index){
      ...
      cards.push(`
        <div class="proposalFormModal__summaryCard">
          ...
          <div class="proposalFormModal__summaryList">
            ...
            ${this.summaryRow('תעודת זהות', rec?.idNumber || applicants[0]?.rawIdNumber || '—', true)}
            ${this.summaryRow('טלפון', rec?.phone || applicants[0]?.phone || '—', true)}
            ${this.summaryRow('עיר', rec?.city || applicants[0]?.city || '—')}
            ${this.summaryRow('תאריך תחילה', this.getRequestedStartDate(rec) || '—')}
          </div>
        </div>`);
      cards.push(`
        <div class="proposalFormModal__summaryCard">
          <div class="proposalFormModal__summaryTitle">מבוטחים שימולאו בעמוד 1</div>
          <div class="proposalFormModal__summaryList">
            ${applicants.length ? applicants.map((app, idx) => this.summaryRow(`מועמד ${idx+1}`, `${app.fullName || '—'} · ${app.kupatHolim || 'קופ״ח לא הוגדרה'} · ${app.deliveryMethodLabel || 'ללא דיוור'}`)).join('') : this.summaryRow('מועמדים', 'לא נמצאו מבוטחים להצגה')}
          </div>
        </div>`);
      cards.push(`
        <div class="proposalFormModal__summaryCard">
          <div class="proposalFormModal__summaryTitle">מה יופק כעת</div>
          <div class="proposalFormModal__summaryList">
            ${this.summaryRow('טופס', 'פניקס בריאות · PDF מקורי')}
            ${this.summaryRow('עמוד', 'עמוד 1')}
            ${this.summaryRow('עריכה בהמשך', 'כן · שדות הטקסט נשארים editable ככל שניתן')}
          </div>
        </div>`);
    ...
    mapGender(value){
      ...
      return {
        male: ['male','m','זכר','בן'].includes(v),
        female: ['female','f','נקבה','בת'].includes(v)
      };
    },
    ...
    mapMaritalStatus(value){
      const v = safeTrim(value).toLowerCase();
      return {
        single: ['single','רווק','רווקה'].includes(v),
        married: ['married','נשוי','נשואה'].includes(v),
        divorced: ['divorced','גרוש','גרושה'].includes(v),
        other: ['widowed','אלמן','אלמנה','other','אחר','ידוע בציבור','ידועה בציבור'].includes(v)
      };
    },
    ...
    mapDeliveryMethod(value){
      const v = safeTrim(value).toLowerCase();
      return {
        email: ['email','e-mail','mail','מייל','דוא"ל'].includes(v),
        post: ['post','mail_post','דואר','דואר ישראל','home'].includes(v)
      };
    },
    ...
    mapShaban(value){
      ...
      const v = safeTrim(value).toLowerCase();
      if(['yes','כן','יש','true'].includes(v)) return { yes:true, no:false };
      if(['no','לא','false','אין'].includes(v)) return { yes:false, no:true };
      return { yes:false, no:false };
    },

    getNewPolicies(rec){
      return CustomersUI?.getNewPolicies?.(rec) || [];
    },

    getRequestedStartDate(rec){
      const payload = rec?.payload && typeof rec.payload === 'object' ? rec.payload : {};
      const fromPayload = safeTrim(payload.requestedStartDate || payload.startDate || payload.operational?.requestedStartDate);
      if(fromPayload) return this.normalizeDate(fromPayload);
      const policy = (this.getNewPolicies(rec) || []).find(p => safeTrim(p?.startDate));
      if(policy?.startDate) return this.normalizeDate(policy.startDate);
      return '';
    },

    async loadPdfBytes(){
      const response = await fetch(`${this.pdfAssetPath}?v=${encodeURIComponent(BUILD)}`, { cache:'no-store' });
      if(!response.ok){
        const err = new Error(`PDF_FETCH_${response.status}`);
    ...
    toPdfRect(page, top, left, width, height){
    ...
    buildPage1Layout(page){
      const cols = [455, 390, 325, 260, 195, 130];
      return {
        page,
        applicants: cols.map((left, idx) => ({
          index: idx,
          left,
          width: 56,
          lastNameTop: 212,
        ...
        requestedStartDate: { left: 108, top: 785, width: 74, height: 15 }
      };
    },

    addTextField(form, page, name, value, top, left, width, height, options = {}){
      const PDFLib = window.PDFLib;
      const { rgb } = PDFLib;
      const rect = this.toPdfRect(page, top, left, width, height);
      const field = form.createTextField(name);
      if(options.maxLength) field.setMaxLength(options.maxLength);
      field.setText(String(value || ''));
      try { field.setFontSize(options.fontSize || 9); } catch(_e) {}
      try { field.setAlignment(options.align || 0); } catch(_e) {}
      field.addToPage(page, {
        ...rect,
        borderWidth: options.borderWidth ?? 0,
        textColor: rgb(0.07, 0.11, 0.23),
        borderColor: rgb(1,1,1)
      });
      return field;
    },

    addCheckbox(form, page, name, checked, top, left, size = 8){
      const PDFLib = window.PDFLib;
      const { rgb } = PDFLib;
      const rect = this.toPdfRect(page, top, left, size, size);
      const field = form.createCheckBox(name);
      field.addToPage(page, {
        ...rect,
        borderWidth: 0.6,
        borderColor: rgb(0.18, 0.33, 0.62)
      });
      if(checked) field.check();
      return field;
    },

    fillIdBoxes(form, page, prefix, value, top, left, boxWidth=5, gap=1){
      const digits = this.normalizeId(value).split('');
      digits.forEach((digit, idx) => {
        this.addTextField(form, page, `${prefix}_id_${idx}`, digit || '', top, left + (idx * (boxWidth + gap)), boxWidth, 11, { fontSize: 7.5, align: 1, maxLength: 1 });
      });
    },

    populateApplicant(form, page, applicant, layout){
      const prefix = `phoenix_p1_app_${applicant.index}`;
      this.addTextField(form, page, `${prefix}_last`, applicant.lastName, layout.lastNameTop, layout.left, layout.width, 14, { fontSize: 8.5 });
      this.addTextField(form, page, `${prefix}_first`, applicant.firstName, layout.firstNameTop, layout.left, layout.width, 14, { fontSize: 8.5 });
      this.fillIdBoxes(form, page, prefix, applicant.idNumber, layout.idTop, layout.left + 1, 5, 1);
      this.addCheckbox(form, page, `${prefix}_gender_m`, applicant.gender.male, layout.genderTop, layout.left + 7, 7);
      this.addCheckbox(form, page, `${prefix}_gender_f`, applicant.gender.female, layout.genderTop, layout.left + 29, 7);
      this.addCheckbox(form, page, `${prefix}_marital_s`, applicant.marital.single, layout.maritalTop, layout.left + 2, 6.8);
      this.addCheckbox(form, page, `${prefix}_marital_m`, applicant.marital.married, layout.maritalTop, layout.left + 16, 6.8);
      this.addCheckbox(form, page, `${prefix}_marital_d`, applicant.marital.divorced, layout.maritalTop, layout.left + 30, 6.8);
      this.addCheckbox(form, page, `${prefix}_marital_o`, applicant.marital.other, layout.maritalTop, layout.left + 44, 6.8);
      this.addTextField(form, page, `${prefix}_birth`, this.normalizeDate(applicant.birthDate), layout.birthTop, layout.left, layout.width, 13, { fontSize: 8.5, align: 1 });
      this.addTextField(form, page, `${prefix}_kupa`, applicant.kupatHolim, layout.kupaTop, layout.left, layout.width, 13, { fontSize: 8.2 });
      this.addCheckbox(form, page, `${prefix}_shaban_y`, applicant.shaban.yes, layout.shabanTop, layout.left + 7, 7);
      this.addCheckbox(form, page, `${prefix}_shaban_n`, applicant.shaban.no, layout.shabanTop, layout.left + 28, 7);
      this.addTextField(form, page, `${prefix}_occupation`, applicant.occupation, layout.occupationTop, layout.left - 1, layout.width + 3, 14, { fontSize: 8.1 });
      this.addTextField(form, page, `${prefix}_hobbies`, applicant.dangerousHobbies, layout.hobbiesTop, layout.left - 1, layout.width + 3, 14, { fontSize: 7.2 });
      this.addTextField(form, page, `${prefix}_mobile`, applicant.phone, layout.mobileTop, layout.left - 1, layout.width + 3, 13, { fontSize: 8.1, align: 1 });
      this.addTextField(form, page, `${prefix}_email`, applicant.email, layout.emailTop, layout.left - 4, layout.width + 10, 14, { fontSize: 6.4, align: 1 });
      this.addCheckbox(form, page, `${prefix}_delivery_email`, applicant.delivery.email, layout.deliveryTop, layout.left + 8, 7);
      this.addCheckbox(form, page, `${prefix}_delivery_post`, applicant.delivery.post, layout.deliveryTop + 10, layout.left + 8, 7);
      this.addTextField(form, page, `${prefix}_street`, applicant.street, layout.streetTop, layout.left - 1, layout.width + 3, 13, { fontSize: 8.0 });
      this.addTextField(form, page, `${prefix}_house`, applicant.houseNumber, layout.houseTop, layout.left + 15, 20, 13, { fontSize: 8.0, align: 1 });
      this.addTextField(form, page, `${prefix}_city`, applicant.city, layout.cityTop, layout.left - 2, layout.width + 4, 13, { fontSize: 8.0 });
      this.addTextField(form, page, `${prefix}_zip`, applicant.zip, layout.zipTop, layout.left - 1, layout.width + 3, 13, { fontSize: 8.0, align: 1 });
    },

    async buildPdfBytes(){
      if(!window.PDFLib) throw new Error('PDFLIB_MISSING');
      const rec = this.current();
      if(!rec) throw new Error('CUSTOMER_NOT_FOUND');
      const PDFLib = window.PDFLib;
      const { PDFDocument } = PDFLib;
      const source = await this.loadPdfBytes();
      const pdfDoc = await PDFDocument.load(source, { ignoreEncryption: true });
      const form = pdfDoc.getForm();
      try { form.deleteXFA?.(); } catch(_e) {}
      const page = pdfDoc.getPages()[0];
      const applicants = this.getApplicants(rec).slice(0, 6);
      const layout = this.buildPage1Layout(page);
      applicants.forEach((applicant, idx) => {
        const column = layout.applicants[idx];
        if(column) this.populateApplicant(form, page, applicant, column);
      });
      this.addTextField(form, page, 'phoenix_requested_start', this.getRequestedStartDate(rec), layout.requestedStartDate.top, layout.requestedStartDate.left, layout.requestedStartDate.width, layout.requestedStartDate.height, { fontSize: 8.5, align: 1 });
      pdfDoc.setTitle(`טופס הצעה פניקס - ${safeTrim(rec.fullName) || 'לקוח'}`);
      pdfDoc.setSubject('טופס הצעה לביטוח בריאות - פניקס');
      pdfDoc.setProducer('GEMEL INVEST CRM');
      pdfDoc.setCreator('GEMEL INVEST CRM');
      return await pdfDoc.save();
    },

    async openPreview(){
      try{
        this.setStatus('מייצר תצוגה מקדימה של הטופס המקורי...', false);
        const pdfBytes = await this.buildPdfBytes();
        this.revokePreviewUrl();
        const blob = new Blob([pdfBytes], { type: 'application/pdf' });
        const url = URL.createObjectURL(blob);
        this.previewUrl = url;
        const win = window.open(url, '_blank', 'noopener,noreferrer');
        if(!win) throw new Error('PREVIEW_POPUP_BLOCKED');
        this.setStatus('תצוגת PDF נפתחה בחלון חדש. כעת אפשר גם להוריד את הטופס.', false);
      }catch(err){
        console.error('PHOENIX_PREVIEW_FAILED', err);
        const missingPdf = err?.code === 'PDF_FETCH_FAILED' || String(err?.message || '').startsWith('PDF_FETCH_');
        const msg = missingPdf
          ? 'קובץ הטופס GetPDF.pdf לא נמצא לצד קבצי המערכת. יש להעלות גם אותו לריפו.'
          : 'לא ניתן היה ליצור תצוגה מקדימה של טופס הפניקס כרגע.';
    ...
    async downloadPdf(){
      try{
        const rec = this.current();
        if(!rec) throw new Error('CUSTOMER_NOT_FOUND');
        this.setStatus('מייצר עכשיו את טופס הפניקס המקורי להורדה...', false);
        const pdfBytes = await this.buildPdfBytes();
        const blob = new Blob([pdfBytes], { type: 'application/pdf' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        const cleanName = safeTrim(rec.fullName || 'customer').replace(/[\\/:*?"<>|]+/g, '-');
        a.href = url;
        a.download = `Phoenix_Proposal_${cleanName}.pdf`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        setTimeout(() => URL.revokeObjectURL(url), 2000);
        this.setStatus('טופס הפניקס המקורי ירד בהצלחה. ניתן לערוך את שדות הטקסט בקובץ PDF לאחר ההורדה.', false);
      }catch(err){
        console.error('PHOENIX_DOWNLOAD_FAILED', err);
        const missingPdf = err?.code === 'PDF_FETCH_FAILED' || String(err?.message || '').startsWith('PDF_FETCH_');
        const msg = missingPdf
          ? 'קובץ הטופס GetPDF.pdf לא נמצא לצד קבצי המערכת. יש להעלות גם אותו לריפו.'
          : 'לא ניתן היה להפיק כעת את טופס הפניקס המקורי.';
        this.setStatus(msg, true);
        alert(msg);
      }
    }
