...
(() => {
  ...
  const BUILD = "20260326-existing-policies-toolbar-v2";
...
// ---------- Step 6 ----------
    ...
    buildOperationalPdfMarkup(payloadOverride, options = {}){
      ...
      const insuredBundles = insureds.map((ins, index) => {
        const d = ins?.data || {};
        const insuredLabel = safeTrim(ins?.label) || `מבוטח ${index + 1}`;
        const insuredName = safeTrim(((d.firstName || '') + ' ' + (d.lastName || '')).trim()) || insuredLabel;
        const existingPolicies = Array.isArray(d?.existingPolicies) ? d.existingPolicies : [];
        const healthRows = [];
        return {
          index,
          id: safeTrim(ins?.id) || safeTrim(d?.id) || `payload_ins_${index}`,
          label: insuredLabel,
          displayName: insuredName,
          detailsRows: [
            ['שם מלא', insuredName],
            ['סוג מבוטח', insuredLabel],
            ...
            ['גובה', formatHeightValue(d.heightCm)],
            ['משקל', formatWeightValue(d.weightKg)],
            ['BMI', formatBmiValue(d.bmi)]
          ].filter(([,value]) => safeTrim(value)),
          existingCards: existingPolicies.map((policy, policyIndex) => {
            ...
            return {
              key: `existing_${index}_${policyIndex}`,
              title: safeTrim(policy?.company) || `פוליסה קיימת ${policyIndex + 1}`,
              subtitle: safeTrim(policy?.type || policy?.product) || 'פוליסה קיימת',
              rows: [
                ['מבוטח', insuredName],
                ['חברה', safeTrim(policy?.company)],
                ['סוג ביטוח', safeTrim(policy?.type || policy?.product)],
                ['מספר פוליסה', safeTrim(policy?.policyNumber)],
                ['פרמיה חודשית', premiumValue ? this.formatMoneyValue(premiumValue) : '—'],
                ['סטטוס', safeTrim(statusMeta?.label) || '—'],
                ['פירוט פעולה', statusParts.join(' · ') || '—'],
                ['כיסויים / סכום', getPolicyCoverageDisplaySafe(policy)]
              ].filter(([,value]) => safeTrim(value)),
          ...
          newCards: newPolicies.filter((policy) => {
            const insId = safeTrim(ins?.id) || safeTrim(d?.id) || `payload_ins_${index}`;
            const policyInsuredId = safeTrim(policy?.insuredId);
            if(policy?.insuredMode === 'couple') return index === 0 || safeTrim(ins?.type) === 'spouse';
            if(policyInsuredId) return policyInsuredId === insId;
            return index === 0;
          }).map((policy, policyIndex) => ({
            key: `new_${index}_${policyIndex}`,
            title: safeTrim(policy?.company) || `פוליסה חדשה ${policyIndex + 1}`,
            ...
            rows: [
              ...
              ['פרמיה חודשית', safeTrim(policy?.premiumMonthly || policy?.premium) ? this.formatMoneyValue(safeTrim(policy?.premiumMonthly || policy?.premium)) : '—'],
      ...
      const insuredOverviewRows = insuredBundles.map((bundle) => [
        bundle.label,
        bundle.displayName,
        safeTrim((insureds[bundle.index]?.data || {}).idNumber) || '—',
        safeTrim((insureds[bundle.index]?.data || {}).birthDate) || '—',
        safeTrim((insureds[bundle.index]?.data || {}).phone) || '—',
        String(bundle.existingCards.length || 0),
        String(bundle.newCards.length || 0),
        String(bundle.healthRows.length || 0)
      ]);
      ...
      insuredBundles.forEach((bundle) => {
        questionList.forEach((item, qIndex) => {
          const q = item?.question || {};
          const answer = responses?.[q.key]?.[bundle.id] || null;
          const answerRaw = safeTrim(answer?.answer);
          if(!answerRaw) return;
          const details = answerRaw === 'yes'
            ? safeTrim(this.summarizeHealthFields(answer?.fields || {})) || 'סומן כן ללא פירוט נוסף'
            : 'נענה לא';
          bundle.healthRows.push({
            no: qIndex + 1,
      ...
      const summaryRows = [
        ...
        ['פוליסות קיימות', String(insuredBundles.reduce((acc, bundle) => acc + bundle.existingCards.length, 0))],
        ['פוליסות חדשות', String(insuredBundles.reduce((acc, bundle) => acc + bundle.newCards.length, 0))]
      ];
      ...
      const renderPolicyCards = (cards, emptyText='אין נתונים להצגה.') => {
        if(!cards.length) return `<div class="lcPdfEmpty">${escapeHtml(emptyText)}</div>`;
        return `<div class="lcPdfPolicyStack">${cards.map((card) => `
          <section class="lcPdfPolicyCard lcPdfPolicyCard--${escapeHtml(card.tone || 'neutral')}">
            <div class="lcPdfPolicyCard__head">
              <div>
                <div class="lcPdfPolicyCard__title">${escapeHtml(card.title)}</div>
                <div class="lcPdfPolicyCard__sub">${escapeHtml(card.subtitle)}</div>
              </div>
            </div>
            ${renderInfoTable(card.rows, 'לא הוזנו פרטי פוליסה')}
            ${card.covers.length ? `<div class="lcPdfChips">${card.covers.map((cover) => `<span class="lcPdfChip">${escapeHtml(cover)}</span>`).join('')}</div>` : ''}
            ${card.pledgeRows.length ? `<div class="lcPdfSubBox"><div class="lcPdfSubBox__title">שיעבוד</div>${renderInfoTable(card.pledgeRows, 'לא הוזנו פרטי שיעבוד')}</div>` : ''}
          </section>`).join('')}</div>`;
      };
      const renderHealthSection = (bundle) => {
        if(!bundle.healthRows.length) return `<div class="lcPdfEmpty">לא נמצאו תשובות להצגת הצהרת הבריאות עבור מבוטח זה.</div>`;
        return `
          <section class="lcPdfHealthSection">
            <div class="lcPdfHealthSection__title">הצהרת בריאות — ${escapeHtml(bundle.displayName)}</div>
      ...
      const renderBundlePanel = (bundle, title, bodyHtml, metaText='') => `
        <section class="lcPdfSection lcPdfSection--insured">
          <div class="lcPdfInsuredHeader">
            <div>
              <div class="lcPdfSection__title">${escapeHtml(bundle.label)}</div>
              <div class="lcPdfInsuredHeader__name">${escapeHtml(bundle.displayName)}</div>
            </div>
            <div class="lcPdfInsuredMeta">${escapeHtml(metaText)}</div>
          </div>
          <div class="lcPdfMiniTitle">${escapeHtml(title)}</div>
          ${bodyHtml}
        </section>`;
      ...
      const style = `<style>
        ...
        .lcPdfPolicyCard--existing{border-color:#d8e1eb}
        .lcPdfPolicyCard--new{border-color:#cfe0ff;background:#fbfdff}
        ...
        .lcPdfSection--insured{border:1px solid #dde4ee;border-radius:18px;padding:16px 16px 14px;background:#fcfdff;break-inside:avoid;page-break-inside:avoid}
        .lcPdfInsuredHeader{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px;flex-wrap:wrap}
        .lcPdfInsuredHeader__name{font-size:22px;font-weight:800;color:#1b3555;margin-top:6px}
        .lcPdfInsuredMeta{font-size:12px;color:#5f738b;font-weight:800;background:#f5f8fc;border:1px solid #dde4ee;border-radius:999px;padding:7px 12px}
        .lcPdfMiniTitle{font-size:13px;font-weight:800;color:#415874;margin-bottom:8px}
      </style>`;

      const wrapPage = (title, sub, innerHtml, pageNumber) => `
        <div class="lcPdfPage">
          <header class="lcPdfHeader">
            <div>
              <h1 class="lcPdfHeader__title">${escapeHtml(title)}</h1>
              <div class="lcPdfHeader__sub">${escapeHtml(sub)}</div>
      ...
      const overviewPage = wrapPage(
        'דוח תפעולי',
        'שיקוף מלא של כל הפרטים שהוקלדו בתהליך ההקמה.',
        `<section class="lcPdfSection"><div class="lcPdfSection__title">סיכום הדוח</div>${renderInfoTable(summaryRows, 'אין נתוני סיכום')}</section><section class="lcPdfSection"><div class="lcPdfSection__title">סקירת מבוטחים</div>${renderClassicTable(['סוג מבוטח','שם מלא','ת״ז','תאריך לידה','טלפון','קיימות','חדשות','בריאות'], insuredOverviewRows, 'לא נמצאו מבוטחים')}</section>`,
        1
      );

      const detailsPage = wrapPage(
        'פרטי המבוטחים',
        'כל פרטי המבוטחים שהוזנו במערכת, כולל BMI.',
        insuredBundles.map((bundle) => renderBundlePanel(bundle, 'פרטים אישיים', renderInfoTable(bundle.detailsRows, 'לא הוזנו פרטי מבוטח'), `BMI: ${String((bundle.detailsRows.find(([label]) => label === 'BMI') || [null, 'לא חושב'])[1] || 'לא חושב')}`)).join(''),
        2
      );

      const existingPage = wrapPage(
        'פוליסות קיימות',
        'כל הפוליסות הקיימות לפי מבוטח.',
        insuredBundles.map((bundle) => renderBundlePanel(bundle, 'פוליסות קיימות', renderPolicyCards(bundle.existingCards, 'לא נוספו פוליסות קיימות למבוטח זה'), `סה״כ פוליסות קיימות: ${bundle.existingCards.length}`)).join(''),
        3
      );

      const newPage = wrapPage(
        'פוליסות חדשות',
        'כל הפוליסות החדשות לפי מבוטח.',
        insuredBundles.map((bundle) => renderBundlePanel(bundle, 'פוליסות חדשות', renderPolicyCards(bundle.newCards, 'לא נוספו פוליסות חדשות למבוטח זה'), `סה״כ פוליסות חדשות: ${bundle.newCards.length}`)).join(''),
        4
      );

      const paymentPage = wrapPage(
        'פרטי תשלום',
        'אמצעי תשלום ומספרי סוכן כפי שהוזנו בתהליך.',
        `<section class="lcPdfSection lcPdfSection--insured"><div class="lcPdfMiniTitle">אמצעי תשלום</div>${renderInfoTable(payerRows, 'לא הוזנו פרטי תשלום')}${companyAgentRows.length ? `<div class="lcPdfSubBox"><div class="lcPdfSubBox__title">מספרי סוכן לחברות</div>${renderClassicTable(['חברה','מספר סוכן'], companyAgentRows, 'לא הוזנו מספרי סוכן')}</div>` : ''}</section>`,
        5
      );

      const healthPage = wrapPage(
        'הצהרות בריאות',
        'כל תשובות ההצהרה בנפרד לכל מבוטח.',
        insuredBundles.map((bundle) => renderBundlePanel(bundle, 'הצהרת בריאות', renderHealthSection(bundle), `סה״כ תשובות משוקפות: ${bundle.healthRows.length}`)).join(''),
        6
      );
      return {
        payload,
        html: `${style}<div class="${rootClass}">${[overviewPage, detailsPage, existingPage, newPage, paymentPage, healthPage].join('<div class="lcPdfPageBreak"></div>')}</div>`
      };
    },
