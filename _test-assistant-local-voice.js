...
const TAG = "20260829-assistant-session-ux-v1";
let failed = 0;
let passed = 0;
...
assert(openNow.type === "open_customer" && openNow.query === "רחל" && !openNow.customerId, "open customer sends the name as query only");
assert(api.commandFromLocalTool("search_customer", { query: "דוד" }) === null, "search does not auto-open a file");
assert(api.parseLocalCommand("שם פרטי אוריה").tool === "fill_wizard", "later commands are not stuck on open-file");
