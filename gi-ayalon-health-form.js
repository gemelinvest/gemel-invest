/* GEMEL INVEST — טופס מקורי בריאות איילון (3.2026, 001)
   נטען לפי דרישה ממסמכי לקוח. ממלא רק ערכים שכבר שמורים בתיק. */
(function installAyalonHealthForm(global){
  "use strict";
  ...
  const AyalonHealthForm = {
    ...
    isAyalonHealthPolicy(policy){
      ...
      if(/סרטן/.test(blob) && !/בריאות/.test(blob) && !/בשביל\s*החוסן/.test(blob)) return false;
      return /בריאות/.test(blob);
    },
    listAyalonHealthPolicies(payload){
      const list = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      return list.filter((p) => this.isAyalonHealthPolicy(p));
    },
    qualifies(payload){
    ...
    coverLetters(covers){
      ...
      const letters = [];
      const add = (letter, re) => {
        if(re.test(blob) && letters.indexOf(letter) < 0) letters.push(letter);
      };
      add("C", /השתל/);
      add("D", /תרופ/);
      add("E", /ניתוח.*חו.?ל|מחליפי\s*ניתוח\s*בחו/);
      add("B", /מהשקל/);
      if(/5000|5,000|5\.000/.test(blob)) add("A", /./);
      else add("R", /משלים\s*שב/);
      add("F", /אמבולטורי\s*מורחב|ייעוץ\s*ובדיקות|ייעוצים\s*ובדיקות/);
      add("N", /טיפולים\s*אמבולטוריים|אמבולטוריים\s*טיפול/);
      add("Q", /אבחון/);
      add("L", /טכנולוגי/);
      add("K", /רפואה\s*משלימה/);
      add("J", /אונליין|און[\s\-]*ליין|בקליק/);
      add("S", /עד\s*הבית/);
      add("I", /ספורטיבי/);
      add("P", /משבר|בר\s*גפן|בר\s*גוף/);
      add("M", /התפתחות\s*הילד/);
      return letters;
    },
    ...
    applyPolicyToPerson(person, insured, policy, letters){
      ...
      if(!person.criticalAmount){
        person.criticalAmount = this.fmtMoneyPlain(policy.ayalonCriticalAmount)
          || this.amountFromMap(policy, ["בשביל החוסן — מחלות קשות", "בשביל החוסן - מחלות קשות", "מחלות קשות"]);
      }
      if(!person.cancerAmount){
        person.cancerAmount = this.fmtMoneyPlain(policy.ayalonCancerAmount)
          || this.amountFromMap(policy, ["בשביל החוסן — סרטן", "בשביל החוסן - סרטן", "סרטן"]);
      }
    },
    ...
    applyPerson(form, person, role, font){
      ...
      this.setTextSafe(form, isChild ? ("StreetNameChild" + childIdx) : (isSpouse ? "StreetCodeSpouse" : "StreetCode"), person.street, font);
    ...
    async fillOriginalTemplate(draft){
      ...
      const templateBytes = await this.fetchFirstOk(
        this.candidateUrls("forms/ayalon-health/", this.TEMPLATE_FILE),
        "לא נמצא טופס בריאות של איילון"
      );
      ...
      this.setExport(form, "InsuranceType", "1");
      ...
      if(draft.payer){
        this.setTextSafe(form, "FirstNamePayer", draft.payer.firstName, font);
        this.setTextSafe(form, "LastNamePayer", draft.payer.lastName, font);
        this.setTextSafe(form, "FullNamePayer", draft.payer.fullName, font);
        this.setTextSafe(form, "PIDPayer", draft.payer.idNumber, font);
        this.setTextSafe(form, "CellPhoneNumberPayer", draft.payer.phone, font);
        this.setTextSafe(form, "EmailPayer", draft.payer.email, font);
        this.setTextSafe(form, "BirthDatePayer", draft.payer.birthDate, font);
        this.setTextSafe(form, "CityPayer", draft.payer.city, font);
        this.setTextSafe(form, "StreetNamePayer", draft.payer.street, font);
    ...
    fileName(draft){
      ...
      return "בריאות_איילון_" + name.replace(/[\\/:*?\"<>|]/g, "_") + "_" + nowISO().slice(0, 10) + ".pdf";
    },

    fieldRow(label, name, value, extra){
      return `<label class="ayalHealthForm__field">
        <span>${escapeHtml(label)}</span>
        <input class="input" type="text" data-ayalhealth="${escapeHtml(name)}" value="${escapeHtml(value || "")}" ${extra || ""} />
      </label>`;
    },
    personBlock(title, prefix, person){
      const p = person || {};
      return `<section class="ayalHealthForm__block">
        <div class="ayalHealthForm__blockTitle">${escapeHtml(title)}</div>
    ...
    collectDraftFromModal(root, draft){
      ...
      root.querySelectorAll("[data-ayalhealth]").forEach((el) => {
        const path = safeTrim(el.getAttribute("data-ayalhealth"));
        if(!path) return;
    ...
    ensureStyles(){
      if(document.getElementById("ayalHealthFormStyle")) return;
      const style = document.createElement("style");
      style.id = "ayalHealthFormStyle";
      style.textContent = `
        .ayalHealthFormModal .giValModal__card{ max-width:min(980px,96vw); width:100%; height:min(92vh,920px); max-height:min(92vh,920px); }
        .ayalHealthForm__hint{ font-size:13px; line-height:1.45; color:#475569; background:#F8FAFC; border:1px solid #E2E8F0; border-radius:12px; padding:10px 12px; }
        .ayalHealthForm__block{ border:1px solid #E5EAF3; border-radius:14px; padding:12px 14px 14px; background:#fff; }
        .ayalHealthForm__blockTitle{ font-size:14px; font-weight:800; color:#0B1F4B; margin-bottom:10px; }
        .ayalHealthForm__grid{ display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px 12px; }
        .ayalHealthForm__field{ display:flex; flex-direction:column; gap:4px; min-width:0; }
        .ayalHealthForm__field span{ font-size:12px; font-weight:700; color:#647287; }
        .ayalHealthFormPreview{ padding:12px 4px; display:flex; flex-direction:column; gap:10px; }
        .ayalHealthFormPreview__row{ display:flex; justify-content:space-between; gap:12px; font-size:13.5px; }
        .ayalHealthFormPreview__row span{ color:#647287; }
        .ayalHealthFormPreview__row strong{ color:#0F172A; font-weight:750; }
        @media (max-width:720px){ .ayalHealthForm__grid{ grid-template-columns:1fr; } }
      `;
    ...
    renderPreviewHtml(draft){
      ...
      const rows = people.map((p, idx) => {
        ...
        return `<div class="ayalHealthFormPreview__row"><span>${escapeHtml(role)}</span><strong>${escapeHtml(p.fullName || "—")}</strong></div>`;
      }).join("");
      return `<div class="ayalHealthFormPreview">
        <div class="ayalHealthFormPreview__row"><span>חברה / מוצר</span><strong>איילון · בריאות</strong></div>
        <div class="ayalHealthFormPreview__row"><span>תחילת ביטוח</span><strong>${escapeHtml(draft.insuranceBegin || "—")}</strong></div>
        ${rows}
        ${global.CustomerDocuments?.canDownloadOfficialJoinForm?.() ? `<button class="btn btn--primary" type="button" data-ayalhealth-open="1">פתח טופס דיגיטלי</button>` : ""}
      </div>`;
    },
    close(){
    ...
    bind(modal){
      modal.querySelectorAll("[data-ayalhealth-close]").forEach((el) => {
        el.addEventListener("click", () => this.close());
      });
      const dl = modal.querySelector("[data-ayalhealth-download]");
      if(dl){
        dl.addEventListener("click", async () => {
          ...
          try {
            ...
            try { console.error("AYALON_HEALTH_PDF_FAILED", err); } catch(_e) {}
    ...
    open(rec){
      ...
      modal.className = "giValModal ayalHealthFormModal is-open giValModal--visible";
      modal.innerHTML = `
        <div class="giValModal__backdrop" data-ayalhealth-close="1"></div>
        <div class="giValModal__card">
          <div class="giValModal__head">
            <div class="giValModal__headText">
              <div class="giValModal__title">טופס מקורי — בריאות · איילון</div>
          ...
          <div class="giValModal__body">
            ...
            ${childBlocks}
            <section class="ayalHealthForm__block">
              <div class="ayalHealthForm__blockTitle">הוראת קבע — רק אם נרשמה בתיק</div>
              <div class="ayalHealthForm__grid">
          ...
          <div class="giValModal__foot">
            <button type="button" class="btn" data-ayalhealth-close="1">סגור</button>
            <button type="button" class="btn btn--primary" data-ayalhealth-download="1">הורד PDF רשמי</button>
          </div>
        </div>`;
  ...
  global.AyalonHealthForm = AyalonHealthForm;
