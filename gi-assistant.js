...
(() => {
  ...
  (() => {
    ...
    function formatWizardDate(day, month, year) {
      if (!Number.isFinite(day) || !Number.isFinite(month) || !Number.isFinite(year)) return null;
      if (year < 100) year += year >= 30 ? 1900 : 2e3;
    ...
    function normalizeWizardDate(raw) {
      let s = trim(raw).replace(/\bטקסט\b/gi, " ").replace(/[!,?״"']/g, " ").replace(/\s+/g, " ").trim();
      if (!s) return null;
      s = s.replace(/^(?:ביום|ביום\s+ה|היום\s+ה|בתאריך|תאריך)\s+/u, "").replace(/\b(?:ביום|בתאריך|של|לשנת|לשנה)\b/g, " ").replace(/\s+/g, " ").trim();
    ...
    function wizardDateToIso(ddmmyyyy) {
      const hit = /^(\d{2})\/(\d{2})\/(\d{4})$/.exec(trim(ddmmyyyy));
      if (!hit) return "";
      return hit[3] + "-" + hit[2] + "-" + hit[1];
    }
    function extractSpokenReportDate(text) {
      var _a, _b, _c;
      const raw = trim(text);
      if (!raw) return "";
      const slice = ((_a = raw.match(/(?:של|בתאריך|לתאריך|מ)\s+(.+?)(?:\s+(?:ותראה|תראה|מה\s+נמכר|בבקשה|תודה)|$)/u)) == null ? void 0 : _a[1]) || ((_b = raw.match(/(\d{1,2}\s*(?:ל)?(?:ינואר|פברואר|מרץ|מרס|אפריל|מאי|יוני|יולי|אוגוסט|ספטמבר|אוקטובר|נובמבר|דצמבר)(?:\s+\d{2,4})?)/u)) == null ? void 0 : _b[1]) || ((_c = raw.match(/(\d{1,2}[\/.\-]\d{1,2}[\/.\-]\d{2,4})/)) == null ? void 0 : _c[1]) || "";
      const wizard = normalizeWizardDate(slice || raw);
      return wizard ? wizardDateToIso(wizard) : "";
    }
    function isChatSpeech(text) {
      return /צאט|צ׳אט|צ'אט|chat/i.test(text);
    }
    function extractChatComposeText(text) {
      const raw = trim(text);
      const hit = raw.match(/^(?:כתוב|תכתוב|תרשום|רשום|תקליד|הקלד|ההודעה(?:\s+היא)?)\s+(.+)$/u);
      return trim((hit == null ? void 0 : hit[1]) || "");
    }
    function extractChatUserName(text) {
      const raw = trim(text);
      let name = "";
      ...
      name = name.replace(/^(?:את\s+)?(?:ה)?(?:שיחה|הודעה|צאט)\s+/u, "").replace(/\s+(?:בבקשה|תודה)$/u, "").trim();
      if (/^(?:צאט|צ׳אט|צ'אט|שיחה|הודעה)$/u.test(name)) return "";
      return name;
    }
    function extractSmokingType(raw) {
    ...
    function extractSmokingAmount(raw) {
      const patterns = [
        /כמות(?:\s*ליום)?\s+(\d{1,3})/,
        /(?:ליום|ביום)\s+(\d{1,3})/,
    ...
    function hasFillPayload(fields, raw) {
      return !!(fields.firstName || fields.lastName || fields.idNumber || fields.street || fields.houseNumber || fields.apartment || fields.zip || fields.phone || fields.city || fields.email || fields.birthDate || fields.idIssueDate || fields.maritalStatus || fields.clinic || fields.occupation || fields.gender || fields.smokingType || fields.smokingAmount || fields.smoker === true || fields.smoker === false || fields.age != null || fields.shaban || /(מלא|רשום|עדכן|באשף|בהצעה|מעשן)/.test(raw) || extractCompany(raw) || extractProduct(raw));
    }
    ...
    function parseLocalCommand(text) {
      ...
      if (/(?:פתח|תפתח|תפתחי|תיכנסי?|היכנסי?|כנסי)\s+(?:את\s+)?(?:ה)?(?:צאט|צ׳אט|צ'אט)(?!\S)/.test(raw) || /^(?:צאט|צ׳אט|צ'אט)$/.test(raw)) {
      ...
      }
      {
        ...
        try {
          if (chatName && ((_b = (_a = readBridge()).isChatOpen) == null ? void 0 : _b.call(_a))) {
            return { tool: "chat_select_user", args: { name: chatName } };
          }
          if (!isOpenNavSpeech(raw) && !extractFillFields(raw) && ((_d = (_c = readBridge()).isChatOpen) == null ? void 0 : _d.call(_c))) {
            const maybeName = raw.replace(/^(?:את\s+)/, "").trim();
            if (maybeName && maybeName.length >= 2 && maybeName.length <= 40 && !/\d/.test(maybeName) && !/(?:דוח|אשף|לקוח|תיק|שלב|מכיר|ביטול|סימול|הצעה)/.test(maybeName)) {
      ...
      if (/(?:דוח\s*)?מכירות.+(?:של|בתאריך|לתאריך|\d)|(?:תראה|הראה|הציג).*(?:מה\s+נמכר|מכירות)|נמכר\s+ב(?:תאריך|יום)/.test(raw) || /(?:דוח\s*)?מכירות/.test(raw) && extractSpokenReportDate(raw)) {
        const date = extractSpokenReportDate(raw);
        return { tool: "open_daily_sales", args: date ? { date } : {} };
      }
      if (/(?:כנס|תיכנסי?|היכנסי?|כנסי|עבור|תעבור|עברי|פתח|תפתח)\s+(?:ל)?דוח\s*מכירות|(?:דוח\s*)מכירות(?!\s+של)/.test(raw) && !extractSpokenReportDate(raw)) {
    ...
    function speechHoldMs(text) {
      if (isPhonePage()) {
        return Math.min(9e3, Math.max(750, Math.round(text.length * 65 + 350)));
      }
    ...
    function replyFromTool(tool, data) {
      if (isPhonePage() && !data.forceVerbose) {
        ...
        if (tool === "go_view" || tool === "click_topbar" || tool === "fill_wizard" || tool === "wizard_next" || tool === "open_har_import" || tool === "open_simulator" || tool === "create_proposal" || tool === "open_customer" || tool === "find_customer_by_id" || tool === "dismiss_validation_modal" || tool === "open_chat" || tool === "close_chat" || tool === "chat_select_user" || tool === "chat_set_draft" || tool === "chat_send" || tool === "open_sales_report" || tool === "open_cancellations_report" || tool === "open_daily_sales") {
    ...
    function commandFromLocalTool(tool, args) {
      ...
      if (tool === "open_daily_sales") {
        const out = { type: "open_daily_sales" };
    ...
    function executeClientCommand(cmd) {
      ...
      if (type === "open_simulator") {
        void ((_d = active.openSimulator) == null ? void 0 : _d.call(active, trim(cmd.company), trim(cmd.product)));
        return { ok: true };
      }
      if (type === "quote_simulator") {
        void ((_e = active.quoteSimulator) == null ? void 0 : _e.call(active, trim(cmd.company), trim(cmd.product), cmd.input && typeof cmd.input === "object" ? cmd.input : {}));
        return { ok: true };
      }
      if (type === "open_wizard") {
      ...
      if (type === "wizard_next") {
        const res = (_h = active.wizardNext) == null ? void 0 : _h.call(active);
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "open_har_import") {
        const res = (_i = active.openHarImport) == null ? void 0 : _i.call(active);
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "dismiss_validation_modal") {
        const res = (_j = active.dismissValidationModal) == null ? void 0 : _j.call(active);
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "open_chat") {
        const res = (_k = active.openChat) == null ? void 0 : _k.call(active);
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "close_chat") {
        const res = (_l = active.closeChat) == null ? void 0 : _l.call(active);
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "chat_select_user") {
        const res = (_m = active.selectChatUserByName) == null ? void 0 : _m.call(active, trim(cmd.name));
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "chat_set_draft") {
        const res = (_n = active.setChatDraft) == null ? void 0 : _n.call(active, trim(cmd.text));
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "chat_send") {
        const res = (_o = active.sendChatMessage) == null ? void 0 : _o.call(active);
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "open_sales_report") {
        const res = (_p = active.openSalesReport) == null ? void 0 : _p.call(active);
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "open_cancellations_report") {
        const res = (_q = active.openCancellationsReport) == null ? void 0 : _q.call(active);
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "open_daily_sales") {
        const res = (_r = active.openDailySalesReport) == null ? void 0 : _r.call(active, trim(cmd.date));
        return res && typeof res === "object" ? res : { ok: true };
      }
      if (type === "click_topbar") {
        (_s = active.clickTopbar) == null ? void 0 : _s.call(active, trim(cmd.id));
        return { ok: true };
      }
      if (type === "open_proposal") {
        (_t = active.openProposal) == null ? void 0 : _t.call(active, trim(cmd.proposalId));
        return { ok: true };
      }
      if (type === "upsert_reminder" && cmd.reminder && typeof cmd.reminder === "object") {
        void ((_u = active.upsertReminder) == null ? void 0 : _u.call(active, cmd.reminder));
        return { ok: true };
      }
      if (type === "mark_task_done") {
        void ((_v = active.markTaskDone) == null ? void 0 : _v.call(active, trim(cmd.id || cmd.taskId)));
        return { ok: true };
      }
      if (type === "refresh_reminders") {
        void ((_w = active.refreshReminders) == null ? void 0 : _w.call(active));
        return { ok: true };
      }
      return { ok: false };
    }
    ...
    async function startVoice() {
      ...
      try {
        ...
        if (!speechRecognitionCtor()) {
          ...
          pushTimeline("info", isPhonePage() ? "\u05DE\u05E7\u05E9\u05D9\u05D1 \u05D1\u05D8\u05DC\u05E4\u05D5\u05DF. \u05D0\u05E4\u05E9\u05E8 \u05D2\u05DD \u05DC\u05DB\u05EA\u05D5\u05D1 \u05D1\u05EA\u05D9\u05D1\u05D4." : "\u05DE\u05E7\u05E9\u05D9\u05D1. \u05D0\u05E4\u05E9\u05E8 \u05D2\u05DD \u05DC\u05DB\u05EA\u05D5\u05D1 \u05D1\u05EA\u05D9\u05D1\u05D4.");
        }
      } catch (err) {
