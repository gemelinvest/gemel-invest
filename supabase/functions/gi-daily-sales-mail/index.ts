...
// clock 2026-09-06: one send per Israel slot from the CRM sales-screen snapshot.
...
// request snapshot when present.
//
// send-slot sends the stored snapshot built by DashboardUI.buildDailySalesPrintModel
// in the browser. It does not rebuild agent rows on the server. That path
// zeroed Haifa/Modiin KPIs and used a different sector map than the sales screen.
// Dedup is once per Israel slot (12:30 / 15:00 / 20:00) per date_key — not a
// 90-minute window that expires while GitHub is still polling the same slot.
//
// Hosted Supabase does not support Deno.cron; register it anyway if the runtime
// has it. The clock is .github/workflows/daily-sales-mail.yml. Deploy this
// function separately; changing git does not update the live Edge Function.
