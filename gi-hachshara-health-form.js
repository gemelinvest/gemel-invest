/* GEMEL INVEST — טופס מקורי בריאות הכשרה (2400 / הצהרה 2498)
   נטען לפי דרישה ממסמכי לקוח. ממלא רק ערכים שכבר שמורים בתיק. */
(function installHachsharaHealthForm(global){
  "use strict";
  ...
  const HachsharaHealthForm = {
    TEMPLATE_BASE: "./forms/hachshara-health/",
    TEMPLATE_FILE: "hachshara-health-join.pdf",
    FONT_URL: "./fonts/Heebo-Bold.ttf",
    VERSION: "20260826-hach-health-form-v1",
    DOC_ID: "doc_hachshara_health_form",
    DOC_TYPE: "hachshara_health_form",
    ...
    policyBlob(policy){
      return [policy?.type, policy?.productName, policy?.planName, policy?.label, policy?.cover]
        .map(safeTrim).join(" ");
    },
    isHachsharaHealthPolicy(policy){
      ...
      return /בריאות/.test(blob) || safeTrim(policy.type) === "בריאות";
    },
    listHachsharaHealthPolicies(payload){
      const list = Array.isArray(payload?.newPolicies) ? payload.newPolicies : [];
      return list.filter((p) => this.isHachsharaHealthPolicy(p));
    },
    listPolicyCovers(policy){
      const helper = global.GI_OFFICIAL_FORM_FILL;
      if(helper && helper.listStoredHealthCovers){
        return helper.listStoredHealthCovers(policy);
      }
      return [];
    },
    coverLetters(covers){
      ...
      const letters = [];
      const add = (letter) => {
        if(letter && letters.indexOf(letter) < 0) letters.push(letter);
      };
      list.forEach((c) => {
        const s = String(c);
        if(/transplant|השתל/.test(s)) add("C");
        if(/drugs|תרופ/.test(s)) add("D");
        if(/abroad_surgery/.test(s) || (/ניתוח/.test(s) && /מחוץ|חו.?ל/.test(s))) add("E");
        if(/surgery_first_shekel|מהשקל/.test(s)) add("B");
        if(/surgery_shaban_5000|5000|5,000|5\.000/.test(s)) add("A");
        else if(/surgery_shaban|משלים\s*שב/.test(s)) add("R");
        if(/ambulatory|ייעוץ\s*ובדיקות|ייעוצים\s*ובדיקות/.test(s)) add("F");
        if(/child_premium|פרימיום\s*לילד|התפתחות\s*הילד/.test(s)) add("M");
      });
      return letters;
    },
    qualifies(payload){
      return this.listHachsharaHealthPolicies(payload).length > 0;
    },
    ...
    applyPolicyToPerson(person, insured, policy, letters, isChild){
    ...
    buildDraft(rec){
      ...
      const policies = this.listHachsharaHealthPolicies(payload);
      ...
      policies.forEach((p) => {
        ...
        this.applyPolicyToPerson(primaryPerson, primary, p, letters, false);
        this.applyPolicyToPerson(spousePerson, spouse, p, letters, false);
        childPeople.forEach((person, idx) => this.applyPolicyToPerson(person, children[idx], p, letters, true));
      });
    ...
    applyPerson(form, person, suffix, font){
      ...
      // ילדים בטופס 2400: אין שדות אימייל/כתובת/מקצוע — רק זהות, קופ״ח, גובה/משקל, כיסויים.
      if(!isChild){
      ...
      // HMOName is סניף (branch). The fund itself is HMORadio / HMORadioSpouse.
      const helper = global.GI_OFFICIAL_FORM_FILL;
      if(isChild){
        this.setTextSafe(form, "HMO" + s, person.clinic, font);
      } else {
        const hmoField = s === "Spouse" ? "HMORadioSpouse" : "HMORadio";
      ...
      const prefix = !s ? "chkMain" : (s === "Spouse" ? "chkBzug" : ("chkChild" + s.replace(/^Child/, "")));
    ...
    async loadPdfDoc(){
      ...
      const templateBytes = await this.fetchFirstOk(
        this.candidateUrls("forms/hachshara-health/", this.TEMPLATE_FILE),
        "לא נמצא טופס בריאות של הכשרה"
      );
    ...
    fileName(draft){
      ...
      return "בריאות_הכשרה_" + name.replace(/[\\/:*?\"<>|]/g, "_") + "_" + nowISO().slice(0, 10) + ".pdf";
    },
    ...
    renderPreviewHtml(draft){
      ...
      const rows = people.map((p, idx) => {
        ...
        return `<div class="hachHealthFormPreview__row"><span>${escapeHtml(role)}</span><strong>${escapeHtml(p.fullName || "—")}</strong></div>`;
      }).join("");
      return `<div class="hachHealthFormPreview">
        <div class="hachHealthFormPreview__row"><span>חברה / מוצר</span><strong>הכשרה · בריאות</strong></div>
        <div class="hachHealthFormPreview__row"><span>תחילת ביטוח</span><strong>${escapeHtml(draft.insuranceBegin || "—")}</strong></div>
        ${rows}
        ${global.CustomerDocuments?.canDownloadOfficialJoinForm?.() ? `<button class="btn btn--primary" type="button" data-hachhealth-open="1">פתח טופס דיגיטלי</button>` : ""}
      </div>`;
    },
    ...
    bind(modal){
      modal.querySelectorAll("[data-hachhealth-close]").forEach((el) => {
        el.addEventListener("click", () => this.close());
      });
      const dl = modal.querySelector("[data-hachhealth-download]");
      if(dl){
        dl.addEventListener("click", async () => {
          ...
          try {
            ...
            try { console.error("HACHSHARA_HEALTH_PDF_FAILED", err); } catch(_e) {}
    ...
    async open(rec){
      ...
      modal.className = "giValModal hachHealthFormModal giPdfEditorModal is-open giValModal--visible";
      modal.innerHTML = `
        <div class="giValModal__backdrop" data-hachhealth-close="1"></div>
        <div class="giValModal__card">
          <div class="giValModal__head">
            <div class="giValModal__headText">
              <div class="giValModal__title">טופס מקורי — בריאות · הכשרה</div>
              <div class="giValModal__sub">טוען את כל שדות הטופס הרשמי…</div>
            </div>
            <button type="button" class="giValModal__closeX" data-hachhealth-close="1" aria-label="סגירה">✕</button>
          </div>
          <div class="giValModal__body"><div class="gi-pdf-ed-hint">טוען שדות הטופס…</div></div>
          <div class="giValModal__foot">
            <button type="button" class="btn" data-hachhealth-close="1">סגור</button>
            <button type="button" class="btn btn--primary" data-hachhealth-download="1" disabled>הורד PDF רשמי</button>
          </div>
        </div>`;
      ...
      try {
        ...
        try { console.error("HACHSHARA_HEALTH_EDITOR_FAILED", err); } catch(_e) {}
  ...
  global.HachsharaHealthForm = HachsharaHealthForm;
