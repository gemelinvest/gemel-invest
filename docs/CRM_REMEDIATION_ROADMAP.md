...
| 3 | Pא / R4 | הקשחת Edge לפי פונקציה — קודם `gi-daily-sales-mail` (לא `verify_jwt` גלובלי) | בינוני | אושר | **ביישום** (PR #209) |
...
| 9e | F1.3 | מילוי payload לתיק פתוח קודם; בלי לעצור מילוי אצל נציג רגיל | נמוך | אושר | **הושלם** (PR #207) |
| 9f | F2 | דלתא רזה לכל הסשנים (בלי `select("*")`); שימור payload קיים | בינוני | אושר | **הושלם** (PR #210) |
| 9f2 | F-files | תצוגת מסמך כ־blob URL + prefetch לשכן; בלי base64 על ה־thread הראשי | נמוך | אושר | **ביישום** (PR #211) |
| 9g | Pז / R6–R8 ישן | select צר / איחוד sync — מכוסה ב־F1–F3 | נמוך–בינוני | כן | אחרי F0 |
