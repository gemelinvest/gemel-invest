...
**נתיב מקביל (אושר 2026-09-13):** שיפור ביצועי המערכת והאוטומציות — [`CRM_PERFORMANCE_PROGRAM.md`](CRM_PERFORMANCE_PROGRAM.md). לא מחליף את נתיב האבטחה ולא מחכה לחיתוך RLS.
...
flowchart TD
  ...
  PE --> PF[Pו Storage]
  F0[F0–F3 ביצועים מקבילים] -.-> PA
  F0 -.-> PC
  PE -.->|הקפאת ביצועים + מדידה מחדש| F4[F4 אחרי חיתוך]
```

Pב יכול לרוץ במקביל ל־Pא. **אסור** ש־Pה יתחיל לפני Pג+Pד.  
נתיב הביצועים (F0–F3) רץ במקביל ל־Pא–Pד לפי [`CRM_PERFORMANCE_PROGRAM.md`](CRM_PERFORMANCE_PROGRAM.md). **מוקפא בשבוע Pה.**

Pז הישן במפת התיקונים פוצל: העבודה לא מחכה לסוף האבטחה. אחרי Pה נשאר רק F4 (מדידה מחדש + אינדקסים).
...
### ביצועים — לא מחכים ל־Pז

הנתיב המלא (מערכת + אוטומציות) הוא [`CRM_PERFORMANCE_PROGRAM.md`](CRM_PERFORMANCE_PROGRAM.md): F0 מדידה → F1 טיימרים/Realtime/hydrate → F2 כרון/עוזר → F3 ליבת טעינה → F4 אחרי Pה.

R10–R14 (פיצול מונולית, migrations סדורות, ייבוש טפסים) — מחוץ לשני הנתיבים עד אישור נפרד.
...
| להכניס נתיב ביצועים מקביל (מערכת + אוטומציות) | בלי זה ביצועים נדחים לסוף האבטחה | **אושר 2026-09-13** — ראו תוכנית הביצועים |
| להתחיל F0 (מדידה בלבד) | בסיס SLO | כן, בלי שינוי מוצר |
...
| F1–F3 ביצועים | נמוך–בינוני (רענון פחות חי / מסך איטי אם select צר מדי) | מוקפא ב־Pה; לא נוגע ב־RLS |

---

## 12. הצעד הבא המקצועי

לא R5. לא RLS. לא פיצול `app.js`.

שני צעדים נפרדים, כל אחד רק אחרי אישור יישום:

1. **אבטחה:** Pא על `gi-daily-sales-mail` בלבד — secret לכרון + אימות ל־actions של המסך.
2. **ביצועים:** F0 מדידה בלבד — מלאי טיימרים + `giPerf` למנהל ולנציג. בלי שינוי מוצר.

אסור לאחד אותם לאותו PR. Pג מתחיל רק אחרי שמחליטים איך נרשמים 14 המשתמשים בלי `auth_user_id`.

עד אישור היישום — התוכניות נשארות מסמך. אין הרצת SQL של נעילה ואין שינוי טיימרים.
